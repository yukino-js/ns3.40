...
Tap::Tap() : m_amplitude(0.0), m_delay(Seconds(0)) {}

Tap::Tap(Time delay, std::complex<double> amp)
    : m_amplitude(amp), m_delay(delay) {}

std::complex<double> Tap::GetAmp() const { return m_amplitude; }

Time Tap::GetDelay() const { return m_delay; }

UanPdp::UanPdp() {}

UanPdp::UanPdp(std::vector<Tap> taps, Time resolution)
    : m_taps(taps), m_resolution(resolution) {}
...
UanPdp::~UanPdp() { m_taps.clear(); }

void UanPdp::SetTap(std::complex<double> amp, uint32_t index) {
...
const Tap &UanPdp::GetTap(uint32_t i) const {
  NS_ASSERT_MSG(i < GetNTaps(),
                "Call to UanPdp::GetTap with requested tap out of range");
  return m_taps[i];
}

void UanPdp::SetNTaps(uint32_t nTaps) { m_taps.resize(nTaps); }

void UanPdp::SetResolution(Time resolution) { m_resolution = resolution; }

UanPdp::Iterator UanPdp::GetBegin() const { return m_taps.begin(); }

UanPdp::Iterator UanPdp::GetEnd() const { return m_taps.end(); }

uint32_t UanPdp::GetNTaps() const {
  return static_cast<uint32_t>(m_taps.size());
}

Time UanPdp::GetResolution() const { return m_resolution; }

std::complex<double> UanPdp::SumTapsFromMaxC(Time delay, Time duration) const {
...
double UanPdp::SumTapsFromMaxNc(Time delay, Time duration) const {
  if (m_resolution <= Seconds(0)) {
    NS_ASSERT_MSG(GetNTaps() == 1,
                  "Attempted to sum taps over time interval in "
                  "UanPdp with resolution 0 and multiple taps");
...
double UanPdp::SumTapsNc(Time begin, Time end) const {
  if (m_resolution <= Seconds(0)) {
    NS_ASSERT_MSG(GetNTaps() == 1,
                  "Attempted to sum taps over time interval in "
                  "UanPdp with resolution 0 and multiple taps");
...
std::complex<double> UanPdp::SumTapsC(Time begin, Time end) const {
  if (m_resolution <= Seconds(0)) {
    NS_ASSERT_MSG(GetNTaps() == 1,
                  "Attempted to sum taps over time interval in "
                  "UanPdp with resolution 0 and multiple taps");
...
UanPdp UanPdp::NormalizeToSumNc() const {
  double sumNc = 0.0;
...
UanPdp UanPdp::CreateImpulsePdp() {
  UanPdp pdp;
...
TypeId UanPropModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanPropModel").SetParent<Object>().SetGroupName("Uan");
  return tid;
}

void UanPropModel::Clear() {}

void UanPropModel::DoDispose() {
  Clear();
  Object::DoDispose();
}
