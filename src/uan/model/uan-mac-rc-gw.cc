...
UanMacRcGw::UanMacRcGw()
    : UanMac(), m_state(IDLE), m_currentRateNum(0), m_cleared(false) {
  UanHeaderCommon ch;
  UanHeaderRcRts rts;
  UanHeaderRcCts cts;
  UanHeaderRcAck ack;
...
void UanMacRcGw::DoDispose() {
  Clear();
  UanMac::DoDispose();
}

TypeId UanMacRcGw::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanMacRcGw")
          ...
          .AddAttribute("NumberOfRates", "Number of rates per Phy layer.",
                        UintegerValue(1023),
          ...
          .AddAttribute(
              ...
              MakeUintegerChecker<uint32_t>())
          .AddAttribute("MinRetryRate", "Smallest allowed RTS retry rate.",
                        DoubleValue(0.01),
          ...
          .AddAttribute(
              ...
              MakeUintegerChecker<uint32_t>())
          .AddAttribute("FrameSize", "Size of data frames in bytes.",
                        UintegerValue(1000),
...
void UanMacRcGw::SetForwardUpCb(
    Callback<void, Ptr<Packet>, uint16_t, const Mac8Address &> cb) {
  m_forwardUpCb = cb;
}

void UanMacRcGw::AttachPhy(Ptr<UanPhy> phy) {
  m_phy = phy;
...
void UanMacRcGw::ReceivePacket(Ptr<Packet> pkt, double /* sinr */,
  ...
  case UanMacRc::TYPE_DATA: {
    ...
    if (m_ackData.find(ch.GetSrc()) == m_ackData.end()) {
      ...
      NS_LOG_DEBUG(Now().As(Time::S)
                   << " GW Received data packet from " << ch.GetSrc()
                   << " length = " << pkt->GetSize());
  ...
  case UanMacRc::TYPE_RTS:
...

    {
      ...
      if (m_requests.find(ch.GetSrc()) == m_requests.end()) {
        ...
        NS_LOG_DEBUG(Now().As(Time::S)
                     << " GW storing reservation from " << ch.GetSrc()
...
void UanMacRcGw::StartCycle() {
  ...
  double thAlpha = ComputeAlpha(totalFrames, totalBytes, m_numNodes, optA,
                                (pDelay / 2.0).GetSeconds());
  ...
  NS_LOG_DEBUG("Found theoretical alpha: "
               << thAlpha << " Found associated rate = " << thCtlRate
  ...
  uint32_t ctlRate =
      m_phy->GetMode(m_currentRateNum + m_numRates).GetDataRateBps();

  Time winSize =
      Seconds(totalBytes * 8.0 / dataRate) + m_sifs * totalFrames + pDelay;
  if (numRts == 0) {
  ...
  if (numRts == 0) {
    ...
    m_state = INCYCLE;
    m_cycleLogger(Simulator::Now(), Seconds(0), numRts, totalBytes,
                  effWinSize.GetSeconds(), ctlRate, actualX);
    return;
  }
  ...
  for (; it != m_sortedRes.end(); it++) {
    ...
    NS_LOG_DEBUG(Now().As(Time::S)
                 << " GW: Scheduling request for prop. delay "
                 << pdelay.As(Time::S) << " for " << (*it).second
                 << " Earliest possible arrival=" << earliestArr.As(Time::S)
                 << "  Next arrival time=" << nextEarliest.As(Time::S));
    nextEarliest = arrivalTime + Seconds(req.length * 8.0 / dataRate) +
                   m_sifs * req.numFrames;
    ...
    NS_LOG_DEBUG(Now().As(Time::S)
                 ...
                 << " frames at "
                 << (Simulator::Now() + arrivalTime).As(Time::S)
                 << "  (delaytiltx of " << arrivalTime.As(Time::S)
                 << ")  Total length is " << req.length << " with txtime "
                 << req.length * 8 / dataRate << " seconds");
  }
  ...
  m_cycleLogger(Simulator::Now(), minPdelay, numRts, totalBytes,
                cycleSeconds.GetSeconds(), ctlRate, actualX);
}

void UanMacRcGw::CycleStarted() { m_state = INCYCLE; }

void UanMacRcGw::EndCycle() {
  NS_LOG_DEBUG(Now().As(Time::S) << " GW Ending cycle");

  Time nextAck = Seconds(0);

  Time ackTime = Seconds(m_ackSize * 8.0 /
                         m_phy->GetMode(m_currentRateNum).GetDataRateBps());
  ...
  for (; it != m_ackData.end(); it++) {
    ...
    ack->AddHeader(ah);
    ack->AddHeader(ch);
    Simulator::Schedule(nextAck, &UanMacRcGw::SendPacket, this, ack,
                        m_currentRateNum);
...
void UanMacRcGw::SendPacket(Ptr<Packet> pkt, uint32_t rate) {
  UanHeaderCommon ch;
  pkt->PeekHeader(ch);
  std::string type;
  switch (ch.GetType()) {
  ...
  NS_LOG_DEBUG(Now().As(Time::S)
               << " GW sending " << type << " packet with size "
               << pkt->GetSize() << " to " << ch.GetDest() << " at rate "
               << rate);
...
double UanMacRcGw::ComputeAlpha(uint32_t totalFrames, uint32_t totalBytes,
  ...
  if (totalFrames == 0) {
    alpha = (2.0 * lrae + 8.0 * m_rtsSize -
             std::sqrt(m_ctsSizeG * 8.0 * 8.0 * m_rtsSize +
                       2 * 8.0 * m_ctsSizeG * 8.0 * m_rtsSize * a *
                           std::exp(1.0))) /
            (2 * lrae + 8.0 * m_rtsSize - 8.0 * m_ctsSizeG);
  } else {
    double w =
        totalBytes * 8.0 + totalFrames * m_sifs.GetSeconds() * m_totalRate;
    double v = m_rtsSize * 8.0 + 2 * lrae;
    double u = (2 * m_maxDelta.GetSeconds() - 2 * deltaK) * m_totalRate;

    double gamma = (w - u + v) /
                   (2 * (u - totalFrames * m_sifs.GetSeconds() * m_totalRate));

    alpha =
        -gamma +
        std::sqrt(gamma * gamma +
  ...
  NS_ASSERT_MSG(alpha > 0 && alpha < 1,
                "Error computing alpha.  Alpha out of valid range!");
  return alpha;
}

std::vector<double> UanMacRcGw::GetExpPdk() {
...
double UanMacRcGw::ComputeExpS(uint32_t a, uint32_t ld,
                               std::vector<double> exppdk) {
  UanHeaderCommon ch;
  ...
  exptime += ComputeExpBOverA(n, a, ld + lh, exppdk) +
             expk * 2 * m_sifs.GetSeconds() + m_sifs.GetSeconds() + 2 * expp;
...
uint32_t UanMacRcGw::CompExpMinIndex(uint32_t n, uint32_t k) {
  double sum = 0;
  for (uint32_t i = 1; i <= n - k + 1; i++) {
    auto nChK = static_cast<double>(NchooseK(n, k));
    double p = (nChK > 0) ? (static_cast<double>(NchooseK(n - i, k - 1)) / nChK)
                          : DBL_MAX;
    sum += p * i;
  }
...
double UanMacRcGw::ComputePiK(uint32_t a, uint32_t n, uint32_t k) {
  auto nck = (double)NchooseK(n, k);
  return nck * std::pow((std::exp((double)a / (double)n) - 1.0), (double)k) *
         std::exp(-((double)a));
}

double UanMacRcGw::ComputeExpBOverA(uint32_t n, uint32_t a, uint32_t ldlh,
  ...
  for (uint32_t k = 1; k <= n; k++) {
    ...
    double denom =
        (1.0 - ComputeAlpha(k, k * ldlh, n, a, deltaK[k])) * m_totalRate;
...
uint64_t UanMacRcGw::NchooseK(uint32_t n, uint32_t k) {
  if (k > n) {
    return 0;
  }
...
int64_t UanMacRcGw::AssignStreams(int64_t stream) {
