...
UanMacCw::UanMacCw()
    : UanMac(), m_phy(nullptr), m_pktTx(nullptr), m_txOngoing(false),
      m_state(IDLE), m_cleared(false)
...
void UanMacCw::DoDispose() {
  Clear();
  UanMac::DoDispose();
}

TypeId UanMacCw::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanMacCw")
          .SetParent<UanMac>()
          .SetGroupName("Uan")
...
bool UanMacCw::Enqueue(Ptr<Packet> packet, uint16_t protocolNumber,
                       const Address &dest) {
  switch (m_state) {
  case CCABUSY:
    ...
    NS_ASSERT(!m_phy->GetTransducer()->GetArrivalList().empty() ||
              m_phy->IsStateTx());
    return false;
  case RUNNING:
  ...
  case IDLE: {
    ...
    if (m_phy->IsStateBusy()) {
      ...
      NS_LOG_DEBUG("Time " << Now().As(Time::S) << ": Addr " << GetAddress()
                           ...
                           << cw << ", Sending at " << m_sendTime.As(Time::S)
...
void UanMacCw::NotifyRxEndError() {
  if (m_state == CCABUSY && !m_phy->IsStateCcaBusy()) {
    NS_LOG_DEBUG("Time " << Now().As(Time::S) << " Addr " << GetAddress()
                         << ": Switching to channel idle");
    m_state = RUNNING;
...
void UanMacCw::NotifyTxStart(Time duration) {
  m_txOngoing = true;
...
void UanMacCw::NotifyTxEnd() {
  m_txOngoing = false;

  EndTx();
}

int64_t UanMacCw::AssignStreams(int64_t stream) {
...
void UanMacCw::SetCw(uint32_t cw) { m_cw = cw; }

void UanMacCw::SetSlotTime(Time duration) { m_slotTime = duration; }

uint32_t UanMacCw::GetCw() { return m_cw; }

Time UanMacCw::GetSlotTime() { return m_slotTime; }

void UanMacCw::PhyRxPacketGood(Ptr<Packet> packet, double /* sinr */,
                               UanTxMode /* mode */) {
  UanHeaderCommon header;
...
void UanMacCw::PhyRxPacketError(Ptr<Packet> /* packet */, double /* sinr */) {}

void UanMacCw::SaveTimer() {
  NS_LOG_DEBUG("Time " << Now().As(Time::S) << " Addr " << GetAddress()
                       << " Saving timer (Delay = "
...
void UanMacCw::StartTimer() {
  m_sendTime = Simulator::Now() + m_savedDelayS;
  if (m_sendTime == Simulator::Now()) {
    SendPacket();
  } else {
    m_sendEvent =
        Simulator::Schedule(m_savedDelayS, &UanMacCw::SendPacket, this);
    NS_LOG_DEBUG("Time " << Now().As(Time::S) << " Addr " << GetAddress()
                         << " Starting timer (New send time = "
                         << this->m_sendTime.As(Time::S) << ")");
  }
}

void UanMacCw::SendPacket() {
  NS_LOG_DEBUG("Time " << Now().As(Time::S) << " Addr " << GetAddress()
                       << " Transmitting ");
