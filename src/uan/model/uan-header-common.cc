...
UanHeaderCommon::UanHeaderCommon(const Mac8Address src, const Mac8Address dest,
                                 uint8_t type, uint8_t protocolNumber)
    : Header(), m_dest(dest), m_src(src) {
  SetProtocolNumber(protocolNumber);
  m_uanProtocolBits.m_type = type;
}

TypeId UanHeaderCommon::GetTypeId() {
...
TypeId UanHeaderCommon::GetInstanceTypeId() const { return GetTypeId(); }

UanHeaderCommon::~UanHeaderCommon() {}

void UanHeaderCommon::SetDest(Mac8Address dest) { m_dest = dest; }

void UanHeaderCommon::SetSrc(Mac8Address src) { m_src = src; }

void UanHeaderCommon::SetType(uint8_t type) { m_uanProtocolBits.m_type = type; }

void UanHeaderCommon::SetProtocolNumber(uint16_t protocolNumber) {
  if (protocolNumber == 0) {
    ...
    NS_ASSERT_MSG(
        false, "UanHeaderCommon::SetProtocolNumber(): Protocol not supported");
  }
}

Mac8Address UanHeaderCommon::GetDest() const { return m_dest; }

Mac8Address UanHeaderCommon::GetSrc() const { return m_src; }

uint8_t UanHeaderCommon::GetType() const { return m_uanProtocolBits.m_type; }

uint16_t UanHeaderCommon::GetProtocolNumber() const {
...
uint32_t UanHeaderCommon::GetSerializedSize() const { return 1 + 1 + 1; }

void UanHeaderCommon::Serialize(Buffer::Iterator start) const {
  uint8_t address = 0;
...
uint32_t UanHeaderCommon::Deserialize(Buffer::Iterator start) {
...
void UanHeaderCommon::Print(std::ostream &os) const {
