...
TypeId UanPhyCalcSinrDual::GetTypeId() {
...
double UanPhyCalcSinrDual::CalcSinrDb(
      ...
      if (pkt) {
        if (ch.GetType() == UanMacRc::TYPE_DATA) {
          NS_LOG_DEBUG("Adding interferer from "
                       ...
                       << ": PktRxMode: " << mode.GetName() << " Int mode: "
                       << it->GetTxMode().GetName() << " Separation: "
...
void UanPhyDual::DoDispose() {
  Clear();
  UanPhy::DoDispose();
}

TypeId UanPhyDual::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanPhyDual")
          ...
          .AddTraceSource("RxError", "A packet was received unsuccessfuly.",
...
void UanPhyDual::SetEnergyModelCallback(
    DeviceEnergyModel::ChangeStateCallback callback) {
  NS_LOG_DEBUG("Not Implemented");
}

void UanPhyDual::EnergyDepletionHandler() { NS_LOG_DEBUG("Not Implemented"); }

void UanPhyDual::EnergyRechargeHandler() { NS_LOG_DEBUG("Not Implemented"); }

void UanPhyDual::SendPacket(Ptr<Packet> pkt, uint32_t modeNum) {
...
void UanPhyDual::RegisterListener(UanPhyListener *listener) {
...
void UanPhyDual::SetReceiveErrorCallback(RxErrCallback cb) {
  m_phy1->SetReceiveErrorCallback(cb);
  m_phy2->SetReceiveErrorCallback(cb);
}

void UanPhyDual::SetTxPowerDb(double txpwr) {
  m_phy1->SetTxPowerDb(txpwr);
  m_phy2->SetTxPowerDb(txpwr);
}

void UanPhyDual::SetTxPowerDbPhy1(double txpwr) { m_phy1->SetTxPowerDb(txpwr); }

void UanPhyDual::SetTxPowerDbPhy2(double txpwr) { m_phy2->SetTxPowerDb(txpwr); }

void UanPhyDual::SetRxThresholdDb(double thresh) {
  m_phy1->SetRxThresholdDb(thresh);
  m_phy2->SetRxThresholdDb(thresh);
}

void UanPhyDual::SetCcaThresholdDb(double thresh) {
  m_phy1->SetCcaThresholdDb(thresh);
  m_phy2->SetCcaThresholdDb(thresh);
}

void UanPhyDual::SetCcaThresholdPhy1(double thresh) {
  m_phy1->SetCcaThresholdDb(thresh);
}

void UanPhyDual::SetCcaThresholdPhy2(double thresh) {
  m_phy2->SetCcaThresholdDb(thresh);
}

double UanPhyDual::GetTxPowerDb() {
  NS_LOG_WARN("Warning: Dual Phy only returns TxPowerDb of Phy 1");
  return m_phy1->GetTxPowerDb();
}

double UanPhyDual::GetTxPowerDbPhy1() const { return m_phy1->GetTxPowerDb(); }

double UanPhyDual::GetTxPowerDbPhy2() const { return m_phy2->GetTxPowerDb(); }

double UanPhyDual::GetRxThresholdDb() { return m_phy1->GetRxThresholdDb(); }

double UanPhyDual::GetCcaThresholdDb() {
  NS_LOG_WARN("Dual Phy only returns CCAThreshold of Phy 1");
  return m_phy1->GetCcaThresholdDb();
}

double UanPhyDual::GetCcaThresholdPhy1() const {
  return m_phy1->GetCcaThresholdDb();
}

double UanPhyDual::GetCcaThresholdPhy2() const {
  return m_phy2->GetCcaThresholdDb();
}

bool UanPhyDual::IsPhy1Idle() { return m_phy1->IsStateIdle(); }

bool UanPhyDual::IsPhy2Idle() { return m_phy2->IsStateIdle(); }

bool UanPhyDual::IsPhy1Rx() { return m_phy1->IsStateRx(); }

bool UanPhyDual::IsPhy2Rx() { return m_phy2->IsStateRx(); }

bool UanPhyDual::IsPhy1Tx() { return m_phy1->IsStateTx(); }

Ptr<Packet> UanPhyDual::GetPhy1PacketRx() const {
  return m_phy1->GetPacketRx();
}

Ptr<Packet> UanPhyDual::GetPhy2PacketRx() const {
  return m_phy2->GetPacketRx();
}

bool UanPhyDual::IsPhy2Tx() { return m_phy2->IsStateTx(); }

bool UanPhyDual::IsStateSleep() {
  return m_phy1->IsStateSleep() && m_phy2->IsStateSleep();
}

bool UanPhyDual::IsStateIdle() {
  return m_phy1->IsStateIdle() && m_phy2->IsStateIdle();
}

bool UanPhyDual::IsStateBusy() { return !IsStateIdle() || !IsStateSleep(); }

bool UanPhyDual::IsStateRx() {
  return m_phy1->IsStateRx() || m_phy2->IsStateRx();
}

bool UanPhyDual::IsStateTx() {
  return m_phy1->IsStateTx() || m_phy2->IsStateTx();
}

bool UanPhyDual::IsStateCcaBusy() {
  return m_phy1->IsStateCcaBusy() || m_phy2->IsStateCcaBusy();
}

Ptr<UanChannel> UanPhyDual::GetChannel() const { return m_phy1->GetChannel(); }

Ptr<UanNetDevice> UanPhyDual::GetDevice() const { return m_phy1->GetDevice(); }

void UanPhyDual::SetChannel(Ptr<UanChannel> channel) {
  m_phy1->SetChannel(channel);
  m_phy2->SetChannel(channel);
}

void UanPhyDual::SetDevice(Ptr<UanNetDevice> device) {
  m_phy1->SetDevice(device);
  m_phy2->SetDevice(device);
}

void UanPhyDual::SetMac(Ptr<UanMac> mac) {
  m_phy1->SetMac(mac);
  m_phy2->SetMac(mac);
}
