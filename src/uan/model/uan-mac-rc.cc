...
Reservation::Reservation()
    : m_length(0), m_frameNo(0), m_retryNo(0), m_transmitted(false) {}
...
Reservation::Reservation(std::list<std::pair<Ptr<Packet>, Mac8Address>> &list,
  ...
  for (uint32_t i = 0; i < numPkts; i++) {
    length += list.front().first->GetSize() + ch.GetSerializedSize() +
...
uint32_t Reservation::GetNoFrames() const {
  return static_cast<uint32_t>(m_pktList.size());
}

uint32_t Reservation::GetLength() const { return m_length; }
...
uint8_t Reservation::GetFrameNo() const { return m_frameNo; }

uint8_t Reservation::GetRetryNo() const { return m_retryNo; }

Time Reservation::GetTimestamp(uint8_t n) const { return m_timestamp[n]; }

bool Reservation::IsTransmitted() const { return m_transmitted; }

void Reservation::SetFrameNo(uint8_t fn) { m_frameNo = fn; }

void Reservation::AddTimestamp(Time t) { m_timestamp.push_back(t); }
...
UanMacRc::UanMacRc()
    : UanMac(), m_state(UNASSOCIATED), m_rtsBlocked(false), m_currentRate(10),
      m_frameNo(0), m_cleared(false) {
...
void UanMacRc::DoDispose() {
  Clear();
  UanMac::DoDispose();
}

TypeId UanMacRc::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanMacRc")
          .SetParent<UanMac>()
          .SetGroupName("Uan")
          ...
          .AddAttribute("QueueLimit", "Maximum packets to queue at MAC.",
                        UintegerValue(10),
...
int64_t UanMacRc::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_ev->SetStream(stream);
  return 1;
}

bool UanMacRc::Enqueue(Ptr<Packet> packet, uint16_t protocolNumber,
                       const Address &dest) {
  if (protocolNumber > 0) {
...
void UanMacRc::SetForwardUpCb(
    Callback<void, Ptr<Packet>, uint16_t, const Mac8Address &> cb) {
  m_forwardUpCb = cb;
}

void UanMacRc::AttachPhy(Ptr<UanPhy> phy) {
  m_phy = phy;
...
void UanMacRc::ReceiveOkFromPhy(Ptr<Packet> pkt, double /* sinr */,
  ...
  default:
    NS_FATAL_ERROR("Unknown packet type "
                   << ch.GetType() << " received at node " << GetAddress());
  }
}

void UanMacRc::ScheduleData(const UanHeaderRcCts &ctsh,
  ...
  m_learnedProp = Simulator::Now() - ctsg.GetTxTimeStamp() -
                  Seconds(ctsBytes * 8.0 / currentBps);
  ...
  for (uint8_t i = 0; i < it->GetNoFrames(); i++, pit++) {
    ...
    Simulator::Schedule(eventTime, &UanMacRc::SendPacket, this, pkt,
                        m_currentRate);
  ...
  if (!m_pktQueue.empty()) {
    ...
    m_rtsEvent =
        Simulator::Schedule(Seconds(timeout), &UanMacRc::SendRts, this);
  }
}

void UanMacRc::SendPacket(Ptr<Packet> pkt, uint32_t rate) {
  UanHeaderCommon ch;
  pkt->PeekHeader(ch);
  std::string type;
  switch (ch.GetType()) {
  ...
  NS_LOG_DEBUG(Now().As(Time::S)
               << " Node " << Mac8Address::ConvertFrom(GetAddress())
               << " transmitting " << pkt->GetSize() << " byte packet of type "
               << type << " with rate " << rate << "("
               << m_phy->GetMode(rate).GetDataRateBps() << ") to "
               << ch.GetDest());
  m_dequeueLogger(pkt, rate);
  m_phy->SendPacket(pkt, rate);
}

void UanMacRc::ProcessAck(Ptr<Packet> ack) {
  UanHeaderRcAck ah;
  ack->RemoveHeader(ah);
...
UanHeaderRcRts UanMacRc::CreateRtsHeader(const Reservation &res) {
...
void UanMacRc::Associate() {
  ...
  double timeout = m_ev->GetValue();
  m_rtsEvent =
      Simulator::Schedule(Seconds(timeout), &UanMacRc::AssociateTimeout, this);
}

void UanMacRc::AssociateTimeout() {
  m_cntrlSends++;
  if (m_state != GWPSENT) {
    return;
  }
...
void UanMacRc::BlockRtsing() { m_rtsBlocked = true; }
