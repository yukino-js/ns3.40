...
TypeId UanPhyCalcSinrDefault::GetTypeId() {
...
double UanPhyCalcSinrDefault::CalcSinrDb(
...
TypeId UanPhyCalcSinrFhFsk::GetTypeId() {
  static TypeId tid =
...
double UanPhyCalcSinrFhFsk::CalcSinrDb(
    Ptr<Packet> pkt, Time arrTime, double rxPowerDb, double ambNoiseDb,
    UanTxMode mode, UanPdp pdp,
    const UanTransducer::ArrivalList &arrivalList) const {
  if ((mode.GetModType() != UanTxMode::FSK) &&
      (mode.GetConstellationSize() != 13)) {
...
TypeId UanPhyPerGenDefault::GetTypeId() {
  static TypeId tid =
...
TypeId UanPhyPerCommonModes::GetTypeId() {
...
double UanPhyPerCommonModes::CalcPer(Ptr<Packet> pkt, double sinrDb,
                                     UanTxMode mode) {
  ...
  case UanTxMode::PSK:
    ...
    default:
      NS_FATAL_ERROR("constellation " << mode.GetConstellationSize()
                                      << " not supported");
      break;
    }
    break;
  ...
  case UanTxMode::QAM: {
    ...
    for (int k = 0; k < log2sqrtM; k++) {
      ...
      NS_LOG_DEBUG("k=" << k << "; sum_items=" << sum_items
                        << "; pow2k=" << pow2k);
...
double UanPhyPerUmodem::NChooseK(uint32_t n, uint32_t k) {
  double result;
...
double UanPhyPerUmodem::CalcPer(Ptr<Packet> pkt, double sinr, UanTxMode mode) {
...
UanPhyGen::UanPhyGen()
    : UanPhy(), m_state(IDLE), m_channel(nullptr), m_transducer(nullptr),
      ...
      m_ccaThreshDb(0), m_pktRx(nullptr), m_pktTx(nullptr), m_cleared(false) {
...
void UanPhyGen::SetEnergyModelCallback(
    DeviceEnergyModel::ChangeStateCallback cb) {
  NS_LOG_FUNCTION(this);
  m_energyCallback = cb;
}

void UanPhyGen::UpdatePowerConsumption(const State state) {
  NS_LOG_FUNCTION(this);
...
void UanPhyGen::EnergyDepletionHandler() {
  NS_LOG_FUNCTION(this);
...
void UanPhyGen::EnergyRechargeHandler() {
  NS_LOG_FUNCTION(this);
...
void UanPhyGen::SendPacket(Ptr<Packet> pkt, uint32_t modeNum) {
...
void UanPhyGen::RegisterListener(UanPhyListener *listener) {
  m_listeners.push_back(listener);
}

void UanPhyGen::StartRxPacket(Ptr<Packet> pkt, double rxPowerDb,
                              UanTxMode txMode, UanPdp pdp) {
  NS_LOG_DEBUG("PHY " << m_mac->GetAddress() << ": rx power after RX gain = "
                      << rxPowerDb << " dB re uPa");
  ...
  case IDLE: {
    ...
    if (newsinr > m_rxThreshDb) {
      ...
      double txdelay = pkt->GetSize() * 8.0 / txMode.GetDataRateBps();
      m_rxEndEvent =
          Simulator::Schedule(Seconds(txdelay), &UanPhyGen::RxEndEvent, this,
  ...
  if (m_state == IDLE &&
      GetInterferenceDb((Ptr<Packet>)nullptr) > m_ccaThreshDb) {
    m_state = CCABUSY;
    NotifyListenersCcaStart();
  }
}
...
void UanPhyGen::SetReceiveOkCallback(RxOkCallback cb) { m_recOkCb = cb; }

void UanPhyGen::SetReceiveErrorCallback(RxErrCallback cb) { m_recErrCb = cb; }

bool UanPhyGen::IsStateSleep() { return m_state == SLEEP; }

bool UanPhyGen::IsStateIdle() { return m_state == IDLE; }

bool UanPhyGen::IsStateBusy() { return !IsStateIdle() && !IsStateSleep(); }

bool UanPhyGen::IsStateRx() { return m_state == RX; }

bool UanPhyGen::IsStateTx() { return m_state == TX; }

bool UanPhyGen::IsStateCcaBusy() { return m_state == CCABUSY; }

void UanPhyGen::SetTxPowerDb(double txpwr) { m_txPwrDb = txpwr; }

void UanPhyGen::SetRxThresholdDb(double thresh) { m_rxThreshDb = thresh; }

void UanPhyGen::SetCcaThresholdDb(double thresh) { m_ccaThreshDb = thresh; }

double UanPhyGen::GetTxPowerDb() { return m_txPwrDb; }

double UanPhyGen::GetRxThresholdDb() { return m_rxThreshDb; }

double UanPhyGen::GetCcaThresholdDb() { return m_ccaThreshDb; }

Ptr<UanChannel> UanPhyGen::GetChannel() const { return m_channel; }

Ptr<UanNetDevice> UanPhyGen::GetDevice() const { return m_device; }

Ptr<UanTransducer> UanPhyGen::GetTransducer() { return m_transducer; }

void UanPhyGen::SetChannel(Ptr<UanChannel> channel) { m_channel = channel; }

void UanPhyGen::SetDevice(Ptr<UanNetDevice> device) { m_device = device; }

void UanPhyGen::SetMac(Ptr<UanMac> mac) { m_mac = mac; }

void UanPhyGen::SetTransducer(Ptr<UanTransducer> trans) {
  m_transducer = trans;
  m_transducer->AddPhy(this);
}

void UanPhyGen::SetSleepMode(bool sleep) {
  if (sleep) {
    m_state = SLEEP;
    if (!m_energyCallback.IsNull()) {
...
int64_t UanPhyGen::AssignStreams(int64_t stream) {
...
void UanPhyGen::NotifyIntChange() {
...
double UanPhyGen::CalculateSinrDb(Ptr<Packet> pkt, Time arrTime,
                                  double rxPowerDb, UanTxMode mode,
                                  UanPdp pdp) {
  double noiseDb =
...
double UanPhyGen::GetInterferenceDb(Ptr<Packet> pkt) {
...
double UanPhyGen::DbToKp(double db) { return std::pow(10, db / 10.0); }

double UanPhyGen::KpToDb(double kp) { return 10 * std::log10(kp); }

void UanPhyGen::NotifyListenersRxStart() {
...
void UanPhyGen::NotifyListenersRxGood() {
  auto it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyRxEndOk();
  }
...
void UanPhyGen::NotifyListenersRxBad() {
  auto it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyRxEndError();
  }
...
void UanPhyGen::NotifyListenersCcaStart() {
  auto it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyCcaStart();
  }
...
void UanPhyGen::NotifyListenersCcaEnd() {
  auto it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyCcaEnd();
  }
...
void UanPhyGen::NotifyListenersTxStart(Time duration) {
  auto it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
...
void UanPhyGen::NotifyListenersTxEnd() {
  auto it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyTxEnd();
  }
...
uint32_t UanPhyGen::GetNModes() { return m_modes.GetNModes(); }

UanTxMode UanPhyGen::GetMode(uint32_t n) {
  NS_ASSERT(n < m_modes.GetNModes());

  return m_modes[n];
}

Ptr<Packet> UanPhyGen::GetPacketRx() const { return m_pktRx; }
