...
TypeId UanNoiseModelDefault::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanNoiseModelDefault")
          .SetParent<UanNoiseModel>()
          .SetGroupName("Uan")
          .AddConstructor<UanNoiseModelDefault>()
          .AddAttribute("Wind", "Wind speed in m/s.", DoubleValue(1),
...
double UanNoiseModelDefault::GetNoiseDbHz(double fKhz) const {
  double turb;
  double wind;
  double ship;
  double thermal;
  ...
  shipDb = 40.0 + 20.0 * (m_shipping - 0.5) + 26.0 * log_fKhz -
           60.0 * std::log10(fKhz + 0.03);
  ship = std::pow(10.0, (shipDb * 0.1));

  windDb = 50.0 + 7.5 * std::pow(m_wind, 0.5) + 20.0 * log_fKhz -
