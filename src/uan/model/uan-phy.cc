...
TypeId UanPhyCalcSinr::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanPhyCalcSinr").SetParent<Object>().SetGroupName("Uan");
  return tid;
}

void UanPhyCalcSinr::Clear() {}

void UanPhyCalcSinr::DoDispose() {
  Clear();
  Object::DoDispose();
}
...
TypeId UanPhyPer::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanPhyPer").SetParent<Object>().SetGroupName("Uan");
  return tid;
}

void UanPhyPer::Clear() {}

void UanPhyPer::DoDispose() {
  Clear();
  Object::DoDispose();
}
...
void UanPhy::NotifyTxBegin(Ptr<const Packet> packet) {
  m_phyTxBeginTrace(packet);
}

void UanPhy::NotifyTxEnd(Ptr<const Packet> packet) { m_phyTxEndTrace(packet); }

void UanPhy::NotifyTxDrop(Ptr<const Packet> packet) {
  m_phyTxDropTrace(packet);
}

void UanPhy::NotifyRxBegin(Ptr<const Packet> packet) {
  m_phyRxBeginTrace(packet);
}

void UanPhy::NotifyRxEnd(Ptr<const Packet> packet) { m_phyRxEndTrace(packet); }

void UanPhy::NotifyRxDrop(Ptr<const Packet> packet) {
