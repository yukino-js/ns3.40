...
void UanNetDevice::DoInitialize() {
  m_phy->Initialize();
  m_mac->Initialize();
...
void UanNetDevice::DoDispose() {
  Clear();
  NetDevice::DoDispose();
}

TypeId UanNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanNetDevice")
          ...
          .AddAttribute(
              "Mac", "The MAC layer attached to this device.", PointerValue(),
...
void UanNetDevice::SetMac(Ptr<UanMac> mac) {
  if (mac) {
    m_mac = mac;
...
void UanNetDevice::SetPhy(Ptr<UanPhy> phy) {
  if (phy) {
    m_phy = phy;
...
void UanNetDevice::SetChannel(Ptr<UanChannel> channel) {
  if (channel) {
    m_channel = channel;
...
Ptr<UanChannel> UanNetDevice::DoGetChannel() const { return m_channel; }

Ptr<UanMac> UanNetDevice::GetMac() const { return m_mac; }

Ptr<UanPhy> UanNetDevice::GetPhy() const { return m_phy; }

void UanNetDevice::SetIfIndex(uint32_t index) { m_ifIndex = index; }

uint32_t UanNetDevice::GetIfIndex() const { return m_ifIndex; }

Ptr<Channel> UanNetDevice::GetChannel() const { return m_channel; }
...
uint16_t UanNetDevice::GetMtu() const { return m_mtu; }

bool UanNetDevice::IsLinkUp() const { return (m_linkup && m_phy); }

bool UanNetDevice::IsBroadcast() const { return true; }

Address UanNetDevice::GetBroadcast() const { return m_mac->GetBroadcast(); }
...
Address UanNetDevice::GetMulticast(Ipv6Address addr) const {
  return m_mac->GetBroadcast();
}

bool UanNetDevice::IsBridge() const { return false; }

bool UanNetDevice::IsPointToPoint() const { return false; }

bool UanNetDevice::Send(Ptr<Packet> packet, const Address &dest,
...
Ptr<Node> UanNetDevice::GetNode() const { return m_node; }

void UanNetDevice::SetNode(Ptr<Node> node) { m_node = node; }

bool UanNetDevice::NeedsArp() const { return true; }

void UanNetDevice::SetReceiveCallback(NetDevice::ReceiveCallback cb) {
  m_forwardUp = cb;
}

void UanNetDevice::ForwardUp(Ptr<Packet> pkt, uint16_t protocolNumber,
...
Ptr<UanTransducer> UanNetDevice::GetTransducer() const { return m_trans; }

void UanNetDevice::SetTransducer(Ptr<UanTransducer> trans) {
  if (trans) {
    m_trans = trans;
...
void UanNetDevice::AddLinkChangeCallback(Callback<void> callback) {
...
bool UanNetDevice::SupportsSendFrom() const { return false; }

void UanNetDevice::SetAddress(Address address) {
  NS_ASSERT_MSG(m_mac, "Tried to set MAC address with no MAC");
  m_mac->SetAddress(Mac8Address::ConvertFrom(address));
}

void UanNetDevice::SetSleepMode(bool sleep) { m_phy->SetSleepMode(sleep); }

void UanNetDevice::SetTxModeIndex(uint32_t txModeIndex) {
  m_mac->SetTxModeIndex(txModeIndex);
}

uint32_t UanNetDevice::GetTxModeIndex() { return m_mac->GetTxModeIndex(); }
