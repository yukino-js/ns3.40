...
UanTransducerHd::UanTransducerHd()
    : UanTransducer(), m_state(RX), m_endTxTime(Seconds(0)), m_cleared(false),
      m_rxGainDb(0) {}

UanTransducerHd::~UanTransducerHd() {}

void UanTransducerHd::Clear() {
  if (m_cleared) {
    return;
  }
  m_cleared = true;
...
void UanTransducerHd::DoDispose() {
  Clear();
  UanTransducer::DoDispose();
}

TypeId UanTransducerHd::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UanTransducerHd")
          .SetParent<UanTransducer>()
          .SetGroupName("Uan")
          .AddConstructor<UanTransducerHd>()
          .AddAttribute(
              "RxGainDb", "Gain in Db added to incoming signal at receiver.",
              DoubleValue(0), MakeDoubleAccessor(&UanTransducerHd::m_rxGainDb),
              MakeDoubleChecker<double>());
  return tid;
}

UanTransducer::State UanTransducerHd::GetState() const { return m_state; }

bool UanTransducerHd::IsRx() const { return m_state == RX; }

bool UanTransducerHd::IsTx() const { return m_state == TX; }

const UanTransducer::ArrivalList &UanTransducerHd::GetArrivalList() const {
  return m_arrivalList;
}

void UanTransducerHd::SetRxGainDb(double gainDb) { m_rxGainDb = gainDb; }

double UanTransducerHd::GetRxGainDb() { return m_rxGainDb; }

double UanTransducerHd::ApplyRxGainDb(double rxPowerDb, UanTxMode mode) {
...
void UanTransducerHd::Transmit(Ptr<UanPhy> src, Ptr<Packet> packet,
...
void UanTransducerHd::SetChannel(Ptr<UanChannel> chan) {
  NS_LOG_DEBUG("Transducer setting channel");
  m_channel = chan;
}

Ptr<UanChannel> UanTransducerHd::GetChannel() const { return m_channel; }

void UanTransducerHd::AddPhy(Ptr<UanPhy> phy) { m_phyList.push_back(phy); }

const UanTransducer::UanPhyList &UanTransducerHd::GetPhyList() const {
  return m_phyList;
}
