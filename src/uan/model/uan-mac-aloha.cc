...
bool UanMacAloha::Enqueue(Ptr<Packet> packet, uint16_t protocolNumber,
                          const Address &dest) {
  NS_LOG_DEBUG("" << Now().As(Time::S) << " MAC "
...
void UanMacAloha::SetForwardUpCb(
...
void UanMacAloha::AttachPhy(Ptr<UanPhy> phy) {
  ...
  m_phy->SetReceiveErrorCallback(
      MakeCallback(&UanMacAloha::RxPacketError, this));
}

void UanMacAloha::RxPacketGood(Ptr<Packet> pkt, double /* sinr */,
                               UanTxMode /* txMode */) {
  UanHeaderCommon header;
  pkt->RemoveHeader(header);
  NS_LOG_DEBUG("Receiving packet from " << header.GetSrc() << " For "
                                        << header.GetDest());

  if (header.GetDest() == GetAddress() ||
...
void UanMacAloha::RxPacketError(Ptr<Packet> pkt, double sinr) {
  NS_LOG_DEBUG("" << Simulator::Now() << " MAC "
                  << Mac8Address::ConvertFrom(GetAddress())
