...
UanChannel::UanChannel() : Channel(), m_prop(nullptr), m_cleared(false) {}

UanChannel::~UanChannel() {}

void UanChannel::Clear() {
  if (m_cleared) {
    return;
  }
  m_cleared = true;
...
void UanChannel::DoDispose() {
  Clear();
  Channel::DoDispose();
}

void UanChannel::SetPropagationModel(Ptr<UanPropModel> prop) {
  NS_LOG_DEBUG("Set Prop Model " << this);
  m_prop = prop;
}

std::size_t UanChannel::GetNDevices() const { return m_devList.size(); }

Ptr<NetDevice> UanChannel::GetDevice(std::size_t i) const {
  return m_devList[i].first;
}

void UanChannel::AddDevice(Ptr<UanNetDevice> dev, Ptr<UanTransducer> trans) {
...
void UanChannel::TxPacket(Ptr<UanTransducer> src, Ptr<Packet> packet,
  ...
  for (; i != m_devList.end(); i++) {
    if (src != i->second) {
      ...
      Ptr<MobilityModel> rcvrMobility =
          i->first->GetNode()->GetObject<MobilityModel>();
      Time delay = m_prop->GetDelay(senderMobility, rcvrMobility, txMode);
      UanPdp pdp = m_prop->GetPdp(senderMobility, rcvrMobility, txMode);
      double rxPowerDb = txPowerDb - m_prop->GetPathLossDb(
                                         senderMobility, rcvrMobility, txMode);

      NS_LOG_DEBUG("txPowerDb=" << txPowerDb << "dB, rxPowerDb=" << rxPowerDb
                                << "dB, distance="
      ...
      Simulator::ScheduleWithContext(dstNodeId, delay, &UanChannel::SendUp,
                                     this, j, copy, rxPowerDb, txMode, pdp);
    }
    j++;
  }
}

void UanChannel::SetNoiseModel(Ptr<UanNoiseModel> noise) {
  NS_ASSERT(noise);
  m_noise = noise;
}

void UanChannel::SendUp(uint32_t i, Ptr<Packet> packet, double rxPowerDb,
                        UanTxMode txMode, UanPdp pdp) {
  NS_LOG_DEBUG("Channel:  In sendup");
  m_devList[i].second->Receive(packet, rxPowerDb, txMode, pdp);
}

double UanChannel::GetNoiseDbHz(double fKhz) {
  NS_ASSERT(m_noise);
