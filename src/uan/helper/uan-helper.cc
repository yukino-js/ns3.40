...
static void AsciiPhyTxEvent(std::ostream *os, std::string context,
  ...
  *os << "+ " << Simulator::Now().GetSeconds() << " " << context << " "
      << *packet << std::endl;
}
...
void UanHelper::EnableAscii(std::ostream &os, uint32_t nodeid,
                            uint32_t deviceid) {
...
void UanHelper::EnableAscii(std::ostream &os, NetDeviceContainer d) {
...
void UanHelper::EnableAscii(std::ostream &os, NodeContainer n) {
...
void UanHelper::EnableAsciiAll(std::ostream &os) {
  EnableAscii(os, NodeContainer::GetGlobal());
}

NetDeviceContainer UanHelper::Install(NodeContainer c) const {
...
NetDeviceContainer UanHelper::Install(NodeContainer c,
                                      Ptr<UanChannel> channel) const {
...
Ptr<UanNetDevice> UanHelper::Install(Ptr<Node> node,
                                     Ptr<UanChannel> channel) const {
...
int64_t UanHelper::AssignStreams(NetDeviceContainer c, int64_t stream) {
