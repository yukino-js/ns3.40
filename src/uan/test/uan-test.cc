...
private:
   ...
   */
  uint32_t DoOnePhyTest(Time t1, Time t2, uint32_t r1, uint32_t r2,
                        Ptr<UanPropModel> prop, uint16_t mode1 = 0,
                        uint16_t mode2 = 0);
  /**
...
bool UanTest::RxPacket(Ptr<NetDevice> /* dev */, Ptr<const Packet> pkt,
                       uint16_t /* mode */, const Address & /* sender */) {
  m_bytesRx += pkt->GetSize();
  return true;
}

void UanTest::SendOnePacket(Ptr<UanNetDevice> dev, uint16_t mode) {
  Ptr<Packet> pkt = Create<Packet>(17);
  dev->SetTxModeIndex(mode);
  dev->Send(pkt, dev->GetBroadcast(), 0);
}

Ptr<UanNetDevice> UanTest::CreateNode(Vector pos, Ptr<UanChannel> chan) {
...
uint32_t UanTest::DoOnePhyTest(Time txTime1, Time txTime2, uint32_t r1,
                               uint32_t r2, Ptr<UanPropModel> prop,
...
bool UanTest::DoPhyTests() {
  // Phy Gen / Default PER / Default SINR
  UanModesList mList;
  UanTxMode mode = UanTxModeFactory::CreateMode(UanTxMode::FSK, 80, 80, 10000,
                                                4000, 2, "TestMode");
  ...
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL(
      DoOnePhyTest(Seconds(1.0), Seconds(3.001), 50, 50, prop), 34,
      "Should have received 34 bytes from 2 disjoint packets");

  // Collision (Lose both packets)
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL(
      DoOnePhyTest(Seconds(1.0), Seconds(2.99), 50, 50, prop), 0,
  ...
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL(
      DoOnePhyTest(Seconds(1.0), Seconds(1.0126), 50, 50, prop), 17,
      "Should have received 17 bytes from first arriving packet");

  // Packets should collide and both be lost
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL(
      DoOnePhyTest(Seconds(1.0), Seconds(1.0 + 7.01 * (13.0 / 80.0)), 50, 50,
                   prop),
      0, "Packets should collide, but received data");

  // Phy Dual
  UanTxMode mode00 = UanTxModeFactory::CreateMode(UanTxMode::FSK, 80, 80, 10000,
                                                  4000, 2, "TestMode00");
  UanTxMode mode10 = UanTxModeFactory::CreateMode(UanTxMode::FSK, 80, 80, 11000,
                                                  4000, 2, "TestMode10");
  UanTxMode mode20 = UanTxModeFactory::CreateMode(UanTxMode::FSK, 80, 80, 15000,
                                                  4000, 2, "TestMode20");
  UanTxMode mode01 = UanTxModeFactory::CreateMode(UanTxMode::FSK, 80, 80, 10000,
                                                  4000, 2, "TestMode01");
  UanTxMode mode11 = UanTxModeFactory::CreateMode(UanTxMode::FSK, 80, 80, 11000,
                                                  4000, 2, "TestMode11");
  UanTxMode mode21 = UanTxModeFactory::CreateMode(UanTxMode::FSK, 80, 80, 15000,
                                                  4000, 2, "TestMode21");
  ...
  // No collision (Get 2 packets)
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL(
      DoOnePhyTest(Seconds(1.0), Seconds(3.01), 50, 50, prop), 34,
...
void UanTest::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL(error, 0.539, 0.001,
