...
Experiment::Experiment()
    : m_numNodes(15), m_dataRate(80), m_depth(70), m_boundary(500),
      m_packetSize(32), m_bytesTotal(0), m_cwMin(10), m_cwMax(400),
      m_cwStep(10), m_avgs(3), m_slotTime(Seconds(0.2)),
      m_simTime(Seconds(1000)), m_gnudatfile("uan-cw-example.gpl"),
...
void Experiment::IncrementCw(uint32_t cw) {
  ...
  NS_LOG_DEBUG("Average for cw=" << cw << " over " << m_avgs
                                 << " runs: " << avgThroughput);
}
...
void Experiment::UpdatePositions(NodeContainer &nodes) const {
  ...
  for (; it != nodes.End(); it++) {
    ...
    mp->SetPosition(
        Vector(uv->GetValue(0, m_boundary), uv->GetValue(0, m_boundary), 70.0));
  }
}
...
int main(int argc, char **argv) {
  ...
  mode = UanTxModeFactory::CreateMode(UanTxMode::FSK, exp.m_dataRate,
                                      exp.m_dataRate, 12000, exp.m_dataRate, 2,
                                      "Default mode");
  UanModesList myModes;
  myModes.AppendMode(mode);

  uan.SetPhy("ns3::UanPhyGen", "PerModel", PointerValue(per), "SinrModel",
             PointerValue(sinr), "SupportedModes", UanModesListValue(myModes));
