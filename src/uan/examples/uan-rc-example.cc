...
Experiment::Experiment()
    : m_simMin(1), m_simMax(1), m_simStep(1), m_numRates(1023),
      m_totalRate(4096), m_maxRange(3000), m_numNodes(15), m_pktSize(1000),
      m_doNode(true), m_sifs(Seconds(0.05)), m_simTime(Seconds(5000)),
      m_gnuplotfile("uan-rc-example.gpl"), m_bytesTotal(0) {}
...
UanTxMode Experiment::CreateMode(uint32_t kass, uint32_t fc, bool upperblock,
  ...
  if (upperblock) {
    ...
    fcmode =
        (uint32_t)((-((double)m_totalRate) + (double)bw) / 2.0 + (double)fc);
  }

  uint32_t phyrate = m_totalRate;

  UanTxMode mode;
  mode = UanTxModeFactory::CreateMode(UanTxMode::OTHER, rate, phyrate, fcmode,
                                      bw, 2, buf.str());
  return mode;
}
...
uint32_t Experiment::Run(uint32_t param) {
  ...
  uan.SetPhy("ns3::UanPhyDual", "SupportedModesPhy1",
             UanModesListValue(m_dataModes), "SupportedModesPhy2",
             UanModesListValue(m_controlModes));

  uan.SetMac("ns3::UanMacRcGw", "NumberOfRates", UintegerValue(m_numRates),
             "NumberOfNodes", UintegerValue(nNodes), "MaxReservations",
             UintegerValue(a), "SIFS", TimeValue(m_sifs), "MaxPropDelay",
             TimeValue(pDelay), "FrameSize", UintegerValue(m_pktSize));
  ...
  uan.SetMac("ns3::UanMacRc", "NumberOfRates", UintegerValue(m_numRates),
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue(
      ...
      exp.m_numRates);
  cmd.AddValue("MaxRange", "Maximum range between gateway and acoustic node",
               exp.m_maxRange);
  cmd.AddValue(
      "SimMin",
  ...
  cmd.AddValue("NumberNodes", "Number of nodes (invalid for doNode=1)",
               exp.m_numNodes);
  ...
  Gnuplot2dDataset ds;
  for (uint32_t param = exp.m_simMin; param <= exp.m_simMax;
       param += exp.m_simStep) {
    uint32_t bytesRx = exp.Run(param);
    NS_LOG_DEBUG("param=" << param << ":  Received " << bytesRx
                          << " bytes at sink");

    double util =
        bytesRx * 8.0 / (exp.m_simTime.GetSeconds() * exp.m_totalRate);
