...
void BufferTest::EnsureWrittenBytes(Buffer b, uint32_t n, uint8_t array[]) {
  bool success = true;
...
void BufferTest::DoRun() {
  ...
  i.Next(8);
  i.WriteU16(0);
  ENSURE_WRITTEN_BYTES(buffer, 10, 0, 0, 0x63, 0x69, 0xde, 0xad, 0xbe, 0xaf, 0,
                       0);
  ...
  i.Prev(2);
  NS_TEST_ASSERT_MSG_EQ(i.ReadNtohU16(), 0xff00,
  ...
  i = buff64.Begin();
  NS_TEST_ASSERT_MSG_EQ(i.ReadU64(), 0x0123456789abcdefLLU,
                        "Could not read expected value");
  i = buff64.Begin();
  i.WriteHtolsbU64(0x0123456789ABCDEFLLU);
  ENSURE_WRITTEN_BYTES(buff64, 8, 0xef, 0xcd, 0xab, 0x89, 0x67, 0x45, 0x23,
                       0x01);
  i = buff64.Begin();
  ...
  ENSURE_WRITTEN_BYTES(buff64, 8, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd,
                       0xef);
  i = buff64.Begin();
...
#pragma clang diagnostic pop
  ...
  i.WriteU8(0x31);
  i.WriteU8(0x32);
  ENSURE_WRITTEN_BYTES(buffer, 11, 0x30, 0x31, 0x32, 0, 0, 0xac, 0xad, 0xae,
                       0xaf, 0xba, 0xbb);
  ...
  // See \bugid{54}
  {
    ...
    NS_TEST_EXPECT_MSG_EQ(
        memcmp(inputBuffer.PeekData(), outputBuffer.PeekData(), chunkSize), 0,
        "memcp works");
  }
  ...
  i.WriteU8(0x3);
  i.WriteU8(0x4);
  ENSURE_WRITTEN_BYTES(buffer, 9, 0x1, 0x2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3,
                       0x4);
  Buffer other;
  other.AddAtStart(9);
  i = other.Begin();
