...
void DropTailQueueTestCase::DoRun() {
  Ptr<DropTailQueue<Packet>> queue = CreateObject<DropTailQueue<Packet>>();
  NS_TEST_EXPECT_MSG_EQ(
      queue->SetAttributeFailSafe("MaxSize", StringValue("3p")), true,
  ...
  NS_TEST_EXPECT_MSG_EQ(queue->GetNPackets(), 0,
                        "There should be no packets in there");
  queue->Enqueue(p1);
  NS_TEST_EXPECT_MSG_EQ(queue->GetNPackets(), 1,
                        "There should be one packet in there");
  queue->Enqueue(p2);
  NS_TEST_EXPECT_MSG_EQ(queue->GetNPackets(), 2,
  ...
  NS_TEST_EXPECT_MSG_EQ(queue->GetNPackets(), 2,
                        "There should be two packets in there");
  NS_TEST_EXPECT_MSG_EQ(packet->GetUid(), p1->GetUid(),
  ...
  NS_TEST_EXPECT_MSG_EQ(queue->GetNPackets(), 1,
                        "There should be one packet in there");
  NS_TEST_EXPECT_MSG_EQ(packet->GetUid(), p2->GetUid(),
  ...
  NS_TEST_EXPECT_MSG_EQ(queue->GetNPackets(), 0,
                        "There should be no packets in there");
  NS_TEST_EXPECT_MSG_EQ(packet->GetUid(), p3->GetUid(),
                        "Was this the third packet ?");

  packet = queue->Dequeue();
  NS_TEST_EXPECT_MSG_EQ(packet, nullptr,
