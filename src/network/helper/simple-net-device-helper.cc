...
void SimpleNetDeviceHelper::SetDeviceAttribute(std::string n1,
                                               const AttributeValue &v1) {
  m_deviceFactory.Set(n1, v1);
}

void SimpleNetDeviceHelper::SetChannelAttribute(std::string n1,
                                                const AttributeValue &v1) {
  m_channelFactory.Set(n1, v1);
}

void SimpleNetDeviceHelper::SetNetDevicePointToPointMode(
    bool pointToPointMode) {
  m_pointToPointMode = pointToPointMode;
}

void SimpleNetDeviceHelper::DisableFlowControl() {
  m_enableFlowControl = false;
}

NetDeviceContainer SimpleNetDeviceHelper::Install(Ptr<Node> node) const {
  Ptr<SimpleChannel> channel = m_channelFactory.Create<SimpleChannel>();
  return Install(node, channel);
}

NetDeviceContainer
SimpleNetDeviceHelper::Install(Ptr<Node> node,
...
NetDeviceContainer
SimpleNetDeviceHelper::Install(const NodeContainer &c,
                               Ptr<SimpleChannel> channel) const {
...
Ptr<NetDevice>
SimpleNetDeviceHelper::InstallPriv(Ptr<Node> node,
                                   Ptr<SimpleChannel> channel) const {
