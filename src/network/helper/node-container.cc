...
NodeContainer::NodeContainer(Ptr<Node> node) { m_nodes.push_back(node); }
...
NodeContainer::Iterator NodeContainer::Begin() const { return m_nodes.begin(); }

NodeContainer::Iterator NodeContainer::End() const { return m_nodes.end(); }

uint32_t NodeContainer::GetN() const { return m_nodes.size(); }

Ptr<Node> NodeContainer::Get(uint32_t i) const { return m_nodes[i]; }
...
void NodeContainer::Add(const NodeContainer &nc) {
  for (auto i = nc.Begin(); i != nc.End(); i++) {
    m_nodes.push_back(*i);
  }
}

void NodeContainer::Add(Ptr<Node> node) { m_nodes.push_back(node); }
