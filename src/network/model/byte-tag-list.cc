...
bool ByteTagList::Iterator::HasNext() const {
  NS_LOG_FUNCTION(this);
  return m_current < m_end;
}

ByteTagList::Iterator::Item ByteTagList::Iterator::Next() {
  NS_ASSERT(HasNext());
...
void ByteTagList::Iterator::PrepareForNext() {
  NS_LOG_FUNCTION(this);
...
ByteTagList::Iterator::Iterator(uint8_t *start, uint8_t *end,
                                int32_t offsetStart, int32_t offsetEnd,
                                int32_t adjustment)
    : m_current(start), m_end(end), m_offsetStart(offsetStart),
      m_offsetEnd(offsetEnd), m_adjustment(adjustment) {
  NS_LOG_FUNCTION(this << &start << &end << offsetStart << offsetEnd
                       << adjustment);
  PrepareForNext();
}

uint32_t ByteTagList::Iterator::GetOffsetStart() const {
  NS_LOG_FUNCTION(this);
  return m_offsetStart;
}

ByteTagList::ByteTagList()
    : m_minStart(INT32_MAX), m_maxEnd(INT32_MIN), m_adjustment(0), m_used(0),
      m_data(nullptr) {
  NS_LOG_FUNCTION(this);
}

ByteTagList::ByteTagList(const ByteTagList &o)
    : m_minStart(o.m_minStart), m_maxEnd(o.m_maxEnd),
      m_adjustment(o.m_adjustment), m_used(o.m_used), m_data(o.m_data) {
...
ByteTagList &ByteTagList::operator=(const ByteTagList &o) {
  if (this == &o) {
    return *this;
  }
...
TagBuffer ByteTagList::Add(TypeId tid, uint32_t bufferSize, int32_t start,
                           int32_t end) {
...
void ByteTagList::Add(const ByteTagList &o) {
  NS_LOG_FUNCTION(this << &o);
...
ByteTagList::Iterator ByteTagList::Begin(int32_t offsetStart,
...
void ByteTagList::AddAtEnd(int32_t appendOffset) {
...
void ByteTagList::AddAtStart(int32_t prependOffset) {
...
ByteTagListData *ByteTagList::Allocate(uint32_t size) {
  ...
  auto buffer =
      new uint8_t[std::max(size, g_maxSize) + sizeof(ByteTagListData) - 4];
...
void ByteTagList::Deallocate(ByteTagListData *data) {
...
uint32_t ByteTagList::Serialize(uint32_t *buffer, uint32_t maxSize) const {
...
uint32_t ByteTagList::Deserialize(const uint32_t *buffer, uint32_t size) {
