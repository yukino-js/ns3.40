...
Node::~Node() { NS_LOG_FUNCTION(this); }
...
Time Node::GetLocalTime() const {
  NS_LOG_FUNCTION(this);
  return Simulator::Now();
}

uint32_t Node::GetSystemId() const {
  NS_LOG_FUNCTION(this);
  return m_sid;
}
...
uint32_t Node::AddDevice(Ptr<NetDevice> device) {
  ...
  Simulator::ScheduleWithContext(GetId(), Seconds(0.0), &NetDevice::Initialize,
                                 device);
...
uint32_t Node::GetNDevices() const {
  NS_LOG_FUNCTION(this);
...
uint32_t Node::GetNApplications() const {
  NS_LOG_FUNCTION(this);
...
void Node::UnregisterDeviceAdditionListener(DeviceAdditionListener listener) {
  NS_LOG_FUNCTION(this << &listener);
  for (auto i = m_deviceAdditionListeners.begin();
