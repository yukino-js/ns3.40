...
Address::Address(uint8_t type, const uint8_t *buffer, uint8_t len)
  ...
  NS_LOG_FUNCTION(this << static_cast<uint32_t>(type) << &buffer
...
Address::Address(const Address &address)
    : m_type(address.m_type), m_len(address.m_len) {
  NS_ASSERT(m_len <= MAX_SIZE);
...
uint8_t Address::GetLength() const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(m_len <= MAX_SIZE);
  return m_len;
}

uint32_t Address::CopyTo(uint8_t buffer[MAX_SIZE]) const {
...
uint32_t Address::CopyAllTo(uint8_t *buffer, uint8_t len) const {
...
uint32_t Address::CopyFrom(const uint8_t *buffer, uint8_t len) {
  NS_LOG_FUNCTION(this << &buffer << static_cast<uint32_t>(len));
...
uint32_t Address::CopyAllFrom(const uint8_t *buffer, uint8_t len) {
  NS_LOG_FUNCTION(this << &buffer << static_cast<uint32_t>(len));
  NS_ASSERT(len >= 2);
  m_type = buffer[0];
  m_len = buffer[1];
...
bool Address::CheckCompatible(uint8_t type, uint8_t len) const {
...
bool Address::IsMatchingType(uint8_t type) const {
  NS_LOG_FUNCTION(this << static_cast<uint32_t>(type));
  return m_type == type;
}

uint8_t Address::Register() {
...
uint32_t Address::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
  return 1 + 1 + m_len;
}

void Address::Serialize(TagBuffer buffer) const {
  NS_LOG_FUNCTION(this << &buffer);
...
void Address::Deserialize(TagBuffer buffer) {
  NS_LOG_FUNCTION(this << &buffer);
