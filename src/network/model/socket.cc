...
TypeId Socket::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Socket").SetParent<Object>().SetGroupName("Network");
  return tid;
}

Socket::Socket()
    : m_manualIpTtl(false), m_ipRecvTos(false), m_ipRecvTtl(false),
      m_manualIpv6Tclass(false), m_manualIpv6HopLimit(false),
      m_ipv6RecvTclass(false), m_ipv6RecvHopLimit(false) {
...
Socket::~Socket() { NS_LOG_FUNCTION(this); }

Ptr<Socket> Socket::CreateSocket(Ptr<Node> node, TypeId tid) {
...
void Socket::SetCloseCallbacks(Callback<void, Ptr<Socket>> normalClose,
...
void Socket::SetDataSentCallback(
    Callback<void, Ptr<Socket>, uint32_t> dataSent) {
  NS_LOG_FUNCTION(this << &dataSent);
  m_dataSent = dataSent;
}

void Socket::SetSendCallback(Callback<void, Ptr<Socket>, uint32_t> sendCb) {
  NS_LOG_FUNCTION(this << &sendCb);
  m_sendCb = sendCb;
}

void Socket::SetRecvCallback(Callback<void, Ptr<Socket>> receivedData) {
...
Ptr<Packet> Socket::RecvFrom(Address &fromAddress) {
...
void Socket::NotifyConnectionSucceeded() {
  NS_LOG_FUNCTION(this);
...
void Socket::NotifyConnectionFailed() {
  NS_LOG_FUNCTION(this);
...
bool Socket::NotifyConnectionRequest(const Address &from) {
...
void Socket::NotifyNewConnectionCreated(Ptr<Socket> socket,
                                        const Address &from) {
...
void Socket::NotifyDataSent(uint32_t size) {
...
void Socket::NotifySend(uint32_t spaceAvailable) {
...
void Socket::BindToNetDevice(Ptr<NetDevice> netdevice) {
...
Ptr<NetDevice> Socket::GetBoundNetDevice() {
  NS_LOG_FUNCTION(this);
...
bool Socket::IsManualIpv6Tclass() const { return m_manualIpv6Tclass; }

bool Socket::IsManualIpTtl() const { return m_manualIpTtl; }

bool Socket::IsManualIpv6HopLimit() const { return m_manualIpv6HopLimit; }

void Socket::SetPriority(uint8_t priority) {
  NS_LOG_FUNCTION(this << priority);
  m_priority = priority;
}

uint8_t Socket::GetPriority() const { return m_priority; }

uint8_t Socket::IpTos2Priority(uint8_t ipTos) {
...
void Socket::SetIpTos(uint8_t tos) {
  Address address;
  GetSockName(address);
...
uint8_t Socket::GetIpTos() const { return m_ipTos; }

void Socket::SetIpRecvTos(bool ipv4RecvTos) { m_ipRecvTos = ipv4RecvTos; }

bool Socket::IsIpRecvTos() const { return m_ipRecvTos; }

void Socket::SetIpv6Tclass(int tclass) {
  Address address;
  GetSockName(address);
...
uint8_t Socket::GetIpv6Tclass() const { return m_ipv6Tclass; }

void Socket::SetIpv6RecvTclass(bool ipv6RecvTclass) {
  m_ipv6RecvTclass = ipv6RecvTclass;
}

bool Socket::IsIpv6RecvTclass() const { return m_ipv6RecvTclass; }

void Socket::SetIpTtl(uint8_t ttl) {
  m_manualIpTtl = true;
  m_ipTtl = ttl;
}

uint8_t Socket::GetIpTtl() const { return m_ipTtl; }

void Socket::SetIpRecvTtl(bool ipv4RecvTtl) { m_ipRecvTtl = ipv4RecvTtl; }

bool Socket::IsIpRecvTtl() const { return m_ipRecvTtl; }

void Socket::SetIpv6HopLimit(uint8_t ipHopLimit) {
  m_manualIpv6HopLimit = true;
  m_ipv6HopLimit = ipHopLimit;
}

uint8_t Socket::GetIpv6HopLimit() const { return m_ipv6HopLimit; }

void Socket::SetIpv6RecvHopLimit(bool ipv6RecvHopLimit) {
  m_ipv6RecvHopLimit = ipv6RecvHopLimit;
}

bool Socket::IsIpv6RecvHopLimit() const { return m_ipv6RecvHopLimit; }
...
void SocketIpTtlTag::SetTtl(uint8_t ttl) {
  NS_LOG_FUNCTION(this << static_cast<uint32_t>(ttl));
  m_ttl = ttl;
}

uint8_t SocketIpTtlTag::GetTtl() const {
  NS_LOG_FUNCTION(this);
  return m_ttl;
}
...
TypeId SocketIpTtlTag::GetTypeId() {
...
TypeId SocketIpTtlTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t SocketIpTtlTag::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
  return 1;
}

void SocketIpTtlTag::Serialize(TagBuffer i) const {
  NS_LOG_FUNCTION(this << &i);
  i.WriteU8(m_ttl);
}

void SocketIpTtlTag::Deserialize(TagBuffer i) {
  NS_LOG_FUNCTION(this << &i);
  m_ttl = i.ReadU8();
}

void SocketIpTtlTag::Print(std::ostream &os) const {
...
void SocketIpv6HopLimitTag::SetHopLimit(uint8_t hopLimit) {
  m_hopLimit = hopLimit;
}

uint8_t SocketIpv6HopLimitTag::GetHopLimit() const { return m_hopLimit; }

NS_OBJECT_ENSURE_REGISTERED(SocketIpv6HopLimitTag);

TypeId SocketIpv6HopLimitTag::GetTypeId() {
...
TypeId SocketIpv6HopLimitTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t SocketIpv6HopLimitTag::GetSerializedSize() const { return 1; }

void SocketIpv6HopLimitTag::Serialize(TagBuffer i) const {
  i.WriteU8(m_hopLimit);
}

void SocketIpv6HopLimitTag::Deserialize(TagBuffer i) {
  m_hopLimit = i.ReadU8();
}

void SocketIpv6HopLimitTag::Print(std::ostream &os) const {
  os << "HopLimit=" << (uint32_t)m_hopLimit;
}

SocketSetDontFragmentTag::SocketSetDontFragmentTag() { NS_LOG_FUNCTION(this); }

void SocketSetDontFragmentTag::Enable() {
  NS_LOG_FUNCTION(this);
  m_dontFragment = true;
}

void SocketSetDontFragmentTag::Disable() {
  NS_LOG_FUNCTION(this);
  m_dontFragment = false;
}

bool SocketSetDontFragmentTag::IsEnabled() const {
  NS_LOG_FUNCTION(this);
...
TypeId SocketSetDontFragmentTag::GetTypeId() {
...
TypeId SocketSetDontFragmentTag::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t SocketSetDontFragmentTag::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
  return 1;
}

void SocketSetDontFragmentTag::Serialize(TagBuffer i) const {
  NS_LOG_FUNCTION(this << &i);
  i.WriteU8(m_dontFragment ? 1 : 0);
}

void SocketSetDontFragmentTag::Deserialize(TagBuffer i) {
  NS_LOG_FUNCTION(this << &i);
  m_dontFragment = (i.ReadU8() == 1);
}

void SocketSetDontFragmentTag::Print(std::ostream &os) const {
  NS_LOG_FUNCTION(this << &os);
...
void SocketIpTosTag::SetTos(uint8_t ipTos) { m_ipTos = ipTos; }

uint8_t SocketIpTosTag::GetTos() const { return m_ipTos; }

NS_OBJECT_ENSURE_REGISTERED(SocketIpTosTag);

TypeId SocketIpTosTag::GetTypeId() {
...
TypeId SocketIpTosTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t SocketIpTosTag::GetSerializedSize() const { return sizeof(uint8_t); }
...
void SocketIpTosTag::Print(std::ostream &os) const {
  os << "IP_TOS = " << m_ipTos;
}

SocketPriorityTag::SocketPriorityTag() {}

void SocketPriorityTag::SetPriority(uint8_t priority) { m_priority = priority; }

uint8_t SocketPriorityTag::GetPriority() const { return m_priority; }

NS_OBJECT_ENSURE_REGISTERED(SocketPriorityTag);

TypeId SocketPriorityTag::GetTypeId() {
...
TypeId SocketPriorityTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t SocketPriorityTag::GetSerializedSize() const {
  return sizeof(uint8_t);
}

void SocketPriorityTag::Serialize(TagBuffer i) const { i.WriteU8(m_priority); }

void SocketPriorityTag::Deserialize(TagBuffer i) { m_priority = i.ReadU8(); }

void SocketPriorityTag::Print(std::ostream &os) const {
  os << "SO_PRIORITY = " << m_priority;
}

SocketIpv6TclassTag::SocketIpv6TclassTag() {}

void SocketIpv6TclassTag::SetTclass(uint8_t tclass) { m_ipv6Tclass = tclass; }

uint8_t SocketIpv6TclassTag::GetTclass() const { return m_ipv6Tclass; }

NS_OBJECT_ENSURE_REGISTERED(SocketIpv6TclassTag);

TypeId SocketIpv6TclassTag::GetTypeId() {
...
TypeId SocketIpv6TclassTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t SocketIpv6TclassTag::GetSerializedSize() const {
  return sizeof(uint8_t);
}

void SocketIpv6TclassTag::Serialize(TagBuffer i) const {
  i.WriteU8(m_ipv6Tclass);
}

void SocketIpv6TclassTag::Deserialize(TagBuffer i) {
  m_ipv6Tclass = i.ReadU8();
}

void SocketIpv6TclassTag::Print(std::ostream &os) const {
