...
void PacketMetadata::EnableChecking() {
  NS_LOG_FUNCTION_NOARGS();
  Enable();
  m_enableChecking = true;
}

void PacketMetadata::ReserveCopy(uint32_t size) {
...
void PacketMetadata::Reserve(uint32_t size) {
  NS_LOG_FUNCTION(this << size);
...
bool PacketMetadata::IsSharedPointerOk(uint16_t pointer) const {
  NS_LOG_FUNCTION(this << pointer);
  bool ok = pointer == 0xffff || pointer <= m_data->m_size;
  return ok;
}

bool PacketMetadata::IsPointerOk(uint16_t pointer) const {
  NS_LOG_FUNCTION(this << pointer);
  bool ok = pointer == 0xffff || pointer <= m_used;
  return ok;
}

bool PacketMetadata::IsStateOk() const {
  NS_LOG_FUNCTION(this);
...
uint32_t PacketMetadata::GetUleb128Size(uint32_t value) const {
...
uint32_t PacketMetadata::ReadUleb128(const uint8_t **pBuffer) const {
...
void PacketMetadata::Append16(uint16_t value, uint8_t *buffer) {
...
void PacketMetadata::Append32(uint32_t value, uint8_t *buffer) {
  NS_LOG_FUNCTION(this << value << &buffer);
  buffer[0] = value & 0xff;
...
void PacketMetadata::AppendValueExtra(uint32_t value, uint8_t *buffer) {
  NS_LOG_FUNCTION(this << value << &buffer);
  if (value < 0x200000) {
...
void PacketMetadata::AppendValue(uint32_t value, uint8_t *buffer) {
  NS_LOG_FUNCTION(this << value << &buffer);
  if (value < 0x80) {
    buffer[0] = value;
    return;
...
void PacketMetadata::UpdateTail(uint16_t written) {
...
void PacketMetadata::UpdateHead(uint16_t written) {
  NS_LOG_FUNCTION(this << written);
  if (m_head == 0xffff) {
    NS_ASSERT(m_tail == 0xffff);
    m_head = m_used;
...
uint16_t PacketMetadata::AddSmall(const PacketMetadata::SmallItem *item) {
  NS_LOG_FUNCTION(this << item->next << item->prev << item->typeUid
                       << item->size << item->chunkUid);
  NS_ASSERT(m_data != nullptr);
...
uint16_t PacketMetadata::AddBig(uint32_t next, uint32_t prev,
                                const PacketMetadata::SmallItem *item,
                                const PacketMetadata::ExtraItem *extraItem) {
  NS_LOG_FUNCTION(this << next << prev << item->next << item->prev
                       << item->typeUid << item->size << item->chunkUid
                       << extraItem->fragmentStart << extraItem->fragmentEnd
                       << extraItem->packetUid);
  NS_ASSERT(m_data != nullptr);
  uint32_t typeUid =
      ((item->typeUid & 0x1) == 0x1) ? item->typeUid : item->typeUid + 1;
  ...
  uint32_t n =
      2 + 2 + typeUidSize + sizeSize + 2 + fragStartSize + fragEndSize + 4;
...
void PacketMetadata::ReplaceTail(PacketMetadata::SmallItem *item,
                                 PacketMetadata::ExtraItem *extraItem,
                                 uint32_t available) {
  NS_LOG_FUNCTION(this << item->next << item->prev << item->typeUid
                       << item->size << item->chunkUid
...
uint32_t PacketMetadata::ReadItems(uint16_t current,
                                   PacketMetadata::SmallItem *item,
                                   PacketMetadata::ExtraItem *extraItem) const {
  NS_LOG_FUNCTION(this << current << item->chunkUid << item->prev << item->next
                       << item->size << item->typeUid << extraItem->fragmentEnd
...
PacketMetadata::Data *PacketMetadata::Create(uint32_t size) {
...
void PacketMetadata::Recycle(PacketMetadata::Data *data) {
  ...
  while (m_freeListUsing.exchange(true, std::memory_order_acquire))
    ;
#endif
  NS_LOG_LOGIC("recycle size=" << data->m_size
...
PacketMetadata::Data *PacketMetadata::Allocate(uint32_t n) {
  NS_LOG_FUNCTION(n);
...
void PacketMetadata::Deallocate(PacketMetadata::Data *data) {
  NS_LOG_FUNCTION(data);
  auto buf = (uint8_t *)data;
  delete[] buf;
}

PacketMetadata PacketMetadata::CreateFragment(uint32_t start,
                                              uint32_t end) const {
...
void PacketMetadata::AddHeader(const Header &header, uint32_t size) {
...
void PacketMetadata::DoAddHeader(uint32_t uid, uint32_t size) {
...
void PacketMetadata::RemoveHeader(const Header &header, uint32_t size) {
  ...
  if ((item.typeUid & 0xfffffffe) != uid || item.size != size) {
    ...
    return;
             ...
             (extraItem.fragmentStart != 0 || extraItem.fragmentEnd != size)) {
    if (m_enableChecking) {
...
void PacketMetadata::AddTrailer(const Trailer &trailer, uint32_t size) {
...
void PacketMetadata::RemoveTrailer(const Trailer &trailer, uint32_t size) {
  uint32_t uid = trailer.GetInstanceTypeId().GetUid() << 1;
  NS_LOG_FUNCTION(this << &trailer << size);
  if (!m_enable) {
    m_metadataSkipped = true;
...
void PacketMetadata::AddAtEnd(const PacketMetadata &o) {
...
void PacketMetadata::AddPaddingAtEnd(uint32_t end) {
...
void PacketMetadata::RemoveAtStart(uint32_t start) {
  ...
  while (current != 0xffff && leftToRemove > 0) {
    ...
    if (itemRealSize <= leftToRemove) {
      ...
      leftToRemove = 0;
      uint16_t written =
          fragment.AddBig(0xffff, fragment.m_tail, &item, &extraItem);
...
void PacketMetadata::RemoveAtEnd(uint32_t end) {
  NS_LOG_FUNCTION(this << end);
  if (!m_enable) {
    m_metadataSkipped = true;
    return;
  ...
  while (current != 0xffff && leftToRemove > 0) {
    ...
    if (itemRealSize <= leftToRemove) {
      ...
      leftToRemove = 0;
      uint16_t written =
          fragment.AddBig(fragment.m_head, 0xffff, &item, &extraItem);
...
uint32_t PacketMetadata::GetTotalSize() const {
  NS_LOG_FUNCTION(this);
...
uint64_t PacketMetadata::GetUid() const {
  NS_LOG_FUNCTION(this);
  return m_packetUid;
}

PacketMetadata::ItemIterator PacketMetadata::BeginItem(Buffer buffer) const {
  NS_LOG_FUNCTION(this << &buffer);
  return ItemIterator(this, buffer);
}

PacketMetadata::ItemIterator::ItemIterator(const PacketMetadata *metadata,
                                           Buffer buffer)
    : m_metadata(metadata), m_buffer(buffer), m_current(metadata->m_head),
      m_offset(0), m_hasReadTail(false) {
  NS_LOG_FUNCTION(this << metadata << &buffer);
}

bool PacketMetadata::ItemIterator::HasNext() const {
  NS_LOG_FUNCTION(this);
  if (m_current == 0xffff) {
    return false;
  }
...
PacketMetadata::Item PacketMetadata::ItemIterator::Next() {
  NS_LOG_FUNCTION(this);
  ...
  item.isFragment =
      (extraItem.fragmentStart != 0 || extraItem.fragmentEnd != smallItem.size);
  TypeId tid;
  tid.SetUid(uid);
  if (uid == 0) {
...
uint32_t PacketMetadata::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
  uint32_t totalSize = 0;
...
uint32_t PacketMetadata::Deserialize(const uint8_t *buffer, uint32_t size) {
  ...
  while (desSize > 0) {
    ...
    desSize -= 8;
    NS_LOG_LOGIC("size=" << size << ", typeUid=" << item.typeUid << ", size="
...
uint8_t *PacketMetadata::AddToRaw(const uint8_t *data, uint32_t dataSize,
...
uint8_t *PacketMetadata::ReadFromRawU8(uint8_t &data, const uint8_t *start,
...
uint8_t *PacketMetadata::ReadFromRawU16(uint16_t &data, const uint8_t *start,
                                        const uint8_t *current,
                                        uint32_t maxSize) {
  NS_LOG_FUNCTION(data << &start << &current << maxSize);
  // First check buffer underflow
...
uint8_t *PacketMetadata::ReadFromRawU32(uint32_t &data, const uint8_t *start,
                                        const uint8_t *current,
                                        uint32_t maxSize) {
  NS_LOG_FUNCTION(data << &start << &current << maxSize);
  // First check buffer underflow
...
uint8_t *PacketMetadata::ReadFromRawU64(uint64_t &data, const uint8_t *start,
                                        const uint8_t *current,
                                        uint32_t maxSize) {
  NS_LOG_FUNCTION(data << &start << &current << maxSize);
  // First check buffer underflow
