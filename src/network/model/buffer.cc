...
#define LOG_INTERNAL_STATE(y)                                                  \
  NS_LOG_LOGIC(y << "start=" << m_start << ", end=" << m_end                   \
                 << ", zero start=" << m_zeroAreaStart << ", zero end="        \
                 << m_zeroAreaEnd << ", count=" << m_data->m_count             \
                 << ", size=" << m_data->m_size                                \
...
Buffer::Data *Buffer::Create(uint32_t size) {
...
void Buffer::Deallocate(Buffer::Data *data) {
  NS_LOG_FUNCTION(data);
  NS_ASSERT(data->m_count == 0);
...
void Buffer::Initialize(uint32_t zeroSize) {
...
uint32_t Buffer::GetInternalSize() const {
  NS_LOG_FUNCTION(this);
  return m_zeroAreaStart - m_start + m_end - m_zeroAreaEnd;
}

uint32_t Buffer::GetInternalEnd() const {
  NS_LOG_FUNCTION(this);
  return m_end - (m_zeroAreaEnd - m_zeroAreaStart);
}

void Buffer::AddAtStart(uint32_t start) {
  ...
  if (m_start >= start && !isDirty) {
    ...
    memcpy(newData->m_data + start, m_data->m_data + m_start,
           GetInternalSize());
    if (m_data->m_count-- == 1) {
...
void Buffer::AddAtEnd(uint32_t end) {
...
void Buffer::AddAtEnd(const Buffer &o) {
...
void Buffer::RemoveAtStart(uint32_t start) {
  NS_LOG_FUNCTION(this << start);
  NS_ASSERT(CheckInternalState());
...
void Buffer::RemoveAtEnd(uint32_t end) {
  NS_LOG_FUNCTION(this << end);
  NS_ASSERT(CheckInternalState());
...
Buffer Buffer::CreateFragment(uint32_t start, uint32_t length) const {
...
Buffer Buffer::CreateFullCopy() const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(CheckInternalState());
...
uint32_t Buffer::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
...
uint32_t Buffer::Deserialize(const uint8_t *buffer, uint32_t size) {
...
void Buffer::TransformIntoRealBuffer() const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(CheckInternalState());
...
const uint8_t *Buffer::PeekData() const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(CheckInternalState());
...
void Buffer::CopyData(std::ostream *os, uint32_t size) const {
...
uint32_t Buffer::CopyData(uint8_t *buffer, uint32_t size) const {
  NS_LOG_FUNCTION(this << &buffer << size);
  ...
  if (size > 0) {
    ...
    if (size > 0) {
      ...
      if (size > 0) {
        tmpsize = std::min(m_end - m_zeroAreaEnd, size);
        memcpy(buffer, (const char *)(m_data->m_data + m_zeroAreaStart),
               tmpsize);
        size -= tmpsize;
      }
    }
...
bool Buffer::Iterator::IsEnd() const {
  NS_LOG_FUNCTION(this);
  return m_current == m_dataEnd;
}

bool Buffer::Iterator::IsStart() const {
  NS_LOG_FUNCTION(this);
  return m_current == m_dataStart;
}

bool Buffer::Iterator::CheckNoZero(uint32_t start, uint32_t end) const {
  NS_LOG_FUNCTION(this << &start << &end);
  return !(start < m_dataStart || end > m_dataEnd ||
           (end > m_zeroStart && start < m_zeroEnd &&
            m_zeroEnd != m_zeroStart && start != end));
}

bool Buffer::Iterator::Check(uint32_t i) const {
  NS_LOG_FUNCTION(this << &i);
  return i >= m_dataStart && !(i >= m_zeroStart && i < m_zeroEnd) &&
         i <= m_dataEnd;
}

void Buffer::Iterator::Write(Iterator start, Iterator end) {
  NS_LOG_FUNCTION(this << &start << &end);
  ...
  uint8_t *from =
      &start.m_data[start.m_current - (start.m_zeroEnd - start.m_zeroStart)];
  uint8_t *to = &m_data[m_current];
  memcpy(to, from, toCopy);
  m_current += toCopy;
}

void Buffer::Iterator::WriteU16(uint16_t data) {
...
void Buffer::Iterator::WriteU32(uint32_t data) {
  NS_LOG_FUNCTION(this << data);
  WriteU8(data & 0xff);
  data >>= 8;
  WriteU8(data & 0xff);
...
void Buffer::Iterator::WriteU64(uint64_t data) {
  NS_LOG_FUNCTION(this << data);
  WriteU8(data & 0xff);
  data >>= 8;
  WriteU8(data & 0xff);
...
void Buffer::Iterator::WriteHtolsbU16(uint16_t data) {
  NS_LOG_FUNCTION(this << data);
  WriteU8((data >> 0) & 0xff);
  WriteU8((data >> 8) & 0xff);
}

void Buffer::Iterator::WriteHtolsbU32(uint32_t data) {
  NS_LOG_FUNCTION(this << data);
  WriteU8((data >> 0) & 0xff);
  WriteU8((data >> 8) & 0xff);
...
void Buffer::Iterator::WriteHtolsbU64(uint64_t data) {
  NS_LOG_FUNCTION(this << data);
  WriteU8((data >> 0) & 0xff);
  WriteU8((data >> 8) & 0xff);
  WriteU8((data >> 16) & 0xff);
...
void Buffer::Iterator::WriteHtonU64(uint64_t data) {
  NS_LOG_FUNCTION(this << data);
  WriteU8((data >> 56) & 0xff);
  WriteU8((data >> 48) & 0xff);
  WriteU8((data >> 40) & 0xff);
...
void Buffer::Iterator::Write(const uint8_t *buffer, uint32_t size) {
  NS_LOG_FUNCTION(this << &buffer << size);
...
uint32_t Buffer::Iterator::ReadU32() {
  NS_LOG_FUNCTION(this);
...
uint64_t Buffer::Iterator::ReadU64() {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint8_t byte2 = ReadU8();
...
uint16_t Buffer::Iterator::SlowReadNtohU16() {
  NS_LOG_FUNCTION(this);
  uint16_t retval = 0;
  retval |= ReadU8();
  retval <<= 8;
...
uint32_t Buffer::Iterator::SlowReadNtohU32() {
  NS_LOG_FUNCTION(this);
  uint32_t retval = 0;
  retval |= ReadU8();
  retval <<= 8;
...
uint64_t Buffer::Iterator::ReadNtohU64() {
  NS_LOG_FUNCTION(this);
  uint64_t retval = 0;
  retval |= ReadU8();
  retval <<= 8;
...
uint16_t Buffer::Iterator::ReadLsbtohU16() {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint16_t data = byte1;
...
uint32_t Buffer::Iterator::ReadLsbtohU32() {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint8_t byte2 = ReadU8();
...
uint64_t Buffer::Iterator::ReadLsbtohU64() {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint8_t byte2 = ReadU8();
...
void Buffer::Iterator::Read(uint8_t *buffer, uint32_t size) {
  NS_LOG_FUNCTION(this << &buffer << size);
...
uint32_t Buffer::Iterator::GetSize() const {
  NS_LOG_FUNCTION(this);
  return m_dataEnd - m_dataStart;
}

uint32_t Buffer::Iterator::GetRemainingSize() const {
  NS_LOG_FUNCTION(this);
  return m_dataEnd - m_current;
}

std::string Buffer::Iterator::GetReadErrorMessage() const {
  NS_LOG_FUNCTION(this);
  std::string str =
...
std::string Buffer::Iterator::GetWriteErrorMessage() const {
  NS_LOG_FUNCTION(this);
  std::string str;
