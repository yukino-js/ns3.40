...
Ipv4Mask::Ipv4Mask() : m_mask(0x66666666) { NS_LOG_FUNCTION(this); }
...
uint32_t Ipv4Mask::Get() const {
  NS_LOG_FUNCTION(this);
  return m_mask;
}

void Ipv4Mask::Set(uint32_t mask) {
  NS_LOG_FUNCTION(this << mask);
  m_mask = mask;
}

uint32_t Ipv4Mask::GetInverse() const {
  NS_LOG_FUNCTION(this);
  return ~m_mask;
}

void Ipv4Mask::Print(std::ostream &os) const {
  NS_LOG_FUNCTION(this << &os);
  os << ((m_mask >> 24) & 0xff) << "." << ((m_mask >> 16) & 0xff) << "."
     << ((m_mask >> 8) & 0xff) << "." << ((m_mask >> 0) & 0xff);
}

Ipv4Mask Ipv4Mask::GetLoopback() {
  NS_LOG_FUNCTION_NOARGS();
  static Ipv4Mask loopback("255.0.0.0");
  return loopback;
}

Ipv4Mask Ipv4Mask::GetZero() {
  NS_LOG_FUNCTION_NOARGS();
  static Ipv4Mask zero("0.0.0.0");
  return zero;
}

Ipv4Mask Ipv4Mask::GetOnes() {
  NS_LOG_FUNCTION_NOARGS();
  static Ipv4Mask ones("255.255.255.255");
  return ones;
}

uint16_t Ipv4Mask::GetPrefixLength() const {
  NS_LOG_FUNCTION(this);
  uint16_t tmp = 0;
...
Ipv4Address::Ipv4Address(const char *address) {
  ...
  if (inet_pton(AF_INET, address, &m_address) <= 0) {
    NS_LOG_LOGIC("Error, can not build an IPv4 address from an invalid string: "
                 << address);
    m_address = 0;
    m_initialized = false;
    return;
...
uint32_t Ipv4Address::Get() const {
  NS_LOG_FUNCTION(this);
  return m_address;
}
...
void Ipv4Address::Set(const char *address) {
  NS_LOG_FUNCTION(this << address);
  if (inet_pton(AF_INET, address, &m_address) <= 0) {
    NS_LOG_LOGIC("Error, can not build an IPv4 address from an invalid string: "
                 << address);
...
Ipv4Address Ipv4Address::CombineMask(const Ipv4Mask &mask) const {
  NS_LOG_FUNCTION(this << mask);
...
Ipv4Address::GetSubnetDirectedBroadcast(const Ipv4Mask &mask) const {
  ...
  if (mask == Ipv4Mask::GetOnes()) {
    NS_ASSERT_MSG(false, "Trying to get subnet-directed broadcast address with "
                         "an all-ones netmask");
  }
...
void Ipv4Address::Serialize(uint8_t buf[4]) const {
...
Ipv4Address Ipv4Address::Deserialize(const uint8_t buf[4]) {
  NS_LOG_FUNCTION(&buf);
  Ipv4Address ipv4;
  ipv4.m_address = 0;
...
bool Ipv4Address::IsMatchingType(const Address &address) {
  NS_LOG_FUNCTION(&address);
  return address.CheckCompatible(GetType(), 4);
}

Ipv4Address::operator Address() const { return ConvertTo(); }

Address Ipv4Address::ConvertTo() const {
  NS_LOG_FUNCTION(this);
  uint8_t buf[4];
  Serialize(buf);
...
Ipv4Address Ipv4Address::ConvertFrom(const Address &address) {
  NS_LOG_FUNCTION(&address);
...
uint8_t Ipv4Address::GetType() {
  NS_LOG_FUNCTION_NOARGS();
  static uint8_t type = Address::Register();
  return type;
}

Ipv4Address Ipv4Address::GetZero() {
  NS_LOG_FUNCTION_NOARGS();
  static Ipv4Address zero("0.0.0.0");
  return zero;
}

Ipv4Address Ipv4Address::GetAny() {
  NS_LOG_FUNCTION_NOARGS();
...
Ipv4Address Ipv4Address::GetLoopback() {
  NS_LOG_FUNCTION_NOARGS();
  Ipv4Address loopback("127.0.0.1");
  return loopback;
}

size_t Ipv4AddressHash::operator()(const Ipv4Address &x) const {
