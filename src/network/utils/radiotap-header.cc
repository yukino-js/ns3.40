...
RadiotapHeader::RadiotapHeader()
    : m_length(8), m_present(0), m_tsft(0), m_flags(FRAME_FLAG_NONE), m_rate(0),
      m_channelFreq(0), m_channelFlags(CHANNEL_FLAG_NONE), m_antennaSignal(0),
      m_antennaNoise(0), m_ampduStatusRef(0), m_ampduStatusFlags(0),
      m_ampduStatusCRC(0), m_vhtPad(0), m_vhtKnown(0), m_vhtFlags(0),
      m_vhtBandwidth(0), m_vhtCoding(0), m_vhtGroupId(0), m_vhtPartialAid(0),
      m_hePad(0), m_heData1(0), m_heData2(0), m_heData3(0), m_heData4(0),
      m_heData5(0), m_heData6(0), m_heMuPad(0), m_heMuFlags1(0),
      m_heMuFlags2(0), m_heMuOtherUserPad(0), m_heMuPerUser1(0),
      m_heMuPerUser2(0), m_heMuPerUserPosition(0), m_heMuPerUserKnown(0) {
...
TypeId RadiotapHeader::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t RadiotapHeader::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
  return m_length;
}
...
void RadiotapHeader::Print(std::ostream &os) const {
  NS_LOG_FUNCTION(this << &os);
  os << " tsft=" << m_tsft << " flags=" << std::hex << m_flags << std::dec
     << " rate=" << +m_rate << " freq=" << m_channelFreq
     << " chflags=" << std::hex << +m_channelFlags << std::dec
     << " signal=" << +m_antennaSignal << " noise=" << +m_antennaNoise
     << " mcsKnown=" << m_mcsKnown << " mcsFlags=" << m_mcsFlags
     << " mcsRate=" << m_mcsRate << " ampduStatusFlags=" << +m_ampduStatusFlags
     << " vhtKnown=" << m_vhtKnown << " vhtFlags=" << m_vhtFlags
     ...
     << " heData2=" << m_heData2 << " heData3=" << m_heData3
     << " heData4=" << m_heData4 << " heData5=" << m_heData5
     << " heData6=" << m_heData6 << " heMuFlags1=" << m_heMuFlags1
...
void RadiotapHeader::SetTsft(uint64_t value) {
  ...
  NS_LOG_LOGIC(this << " m_length=" << m_length << " m_present=0x" << std::hex
                    << m_present << std::dec);
}

void RadiotapHeader::SetFrameFlags(uint8_t flags) {
...
void RadiotapHeader::SetChannelFrequencyAndFlags(uint16_t frequency,
                                                 uint16_t flags) {
...
void RadiotapHeader::SetAntennaSignalPower(double signal) {
...
void RadiotapHeader::SetAntennaNoisePower(double noise) {
...
void RadiotapHeader::SetMcsFields(uint8_t known, uint8_t flags, uint8_t mcs) {
...
void RadiotapHeader::SetAmpduStatus(uint32_t referenceNumber, uint16_t flags,
                                    uint8_t crc) {
...
void RadiotapHeader::SetVhtFields(uint16_t known, uint8_t flags,
                                  uint8_t bandwidth, uint8_t mcs_nss[4],
                                  uint8_t coding, uint8_t group_id,
                                  uint16_t partial_aid) {
  NS_LOG_FUNCTION(this << known << flags << +mcs_nss[0] << +mcs_nss[1]
                       << +mcs_nss[2] << +mcs_nss[3] << +coding << +group_id
                       << +partial_aid);
  m_vhtKnown = known;
  m_vhtFlags = flags;
...
void RadiotapHeader::SetHeFields(uint16_t data1, uint16_t data2, uint16_t data3,
...
void RadiotapHeader::SetHeMuPerUserFields(uint16_t perUser1, uint16_t perUser2,
                                          uint8_t perUserPosition,
                                          uint8_t perUserKnown) {
  NS_LOG_FUNCTION(this << perUser1 << perUser2 << +perUserPosition
                       << +perUserKnown);
