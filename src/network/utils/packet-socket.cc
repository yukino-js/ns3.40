...
void PacketSocket::SetNode(Ptr<Node> node) {
  NS_LOG_FUNCTION(this << node);
  m_node = node;
}

PacketSocket::~PacketSocket() { NS_LOG_FUNCTION(this); }

void PacketSocket::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_device = 0;
}

Socket::SocketErrno PacketSocket::GetErrno() const {
  NS_LOG_FUNCTION(this);
  return m_errno;
}

Socket::SocketType PacketSocket::GetSocketType() const {
  NS_LOG_FUNCTION(this);
  return NS3_SOCK_RAW;
}

Ptr<Node> PacketSocket::GetNode() const {
  NS_LOG_FUNCTION(this);
  return m_node;
}

int PacketSocket::Bind() {
  NS_LOG_FUNCTION(this);
...
int PacketSocket::Close() {
  ...
  if (m_state == STATE_CLOSED) {
    ...
    m_node->UnregisterProtocolHandler(
        MakeCallback(&PacketSocket::ForwardUp, this));
  }
...
uint32_t PacketSocket::GetMinMtu(PacketSocketAddress ad) const {
  NS_LOG_FUNCTION(this << ad);
...
uint32_t PacketSocket::GetTxAvailable() const {
  NS_LOG_FUNCTION(this);
  if (m_state == STATE_CONNECTED) {
...
int PacketSocket::SendTo(Ptr<Packet> p, uint32_t flags,
...
void PacketSocket::ForwardUp(Ptr<NetDevice> device, Ptr<const Packet> packet,
  ...
  NS_LOG_FUNCTION(this << device << packet << protocol << from << to
                       << packetType);
  if (m_shutdownRecv) {
    return;
  }
...
Ptr<Packet> PacketSocket::Recv(uint32_t maxSize, uint32_t flags) {
...
Ptr<Packet> PacketSocket::RecvFrom(uint32_t maxSize, uint32_t flags,
                                   Address &fromAddress) {
  NS_LOG_FUNCTION(this << maxSize << flags);
...
bool PacketSocket::SetAllowBroadcast(bool allowBroadcast) {
  NS_LOG_FUNCTION(this << allowBroadcast);
  return !allowBroadcast;
}

bool PacketSocket::GetAllowBroadcast() const {
  NS_LOG_FUNCTION(this);
  return false;
}
...
NetDevice::PacketType PacketSocketTag::GetPacketType() const {
  return m_packetType;
}

void PacketSocketTag::SetDestAddress(Address a) { m_destAddr = a; }

Address PacketSocketTag::GetDestAddress() const { return m_destAddr; }

NS_OBJECT_ENSURE_REGISTERED(PacketSocketTag);

TypeId PacketSocketTag::GetTypeId() {
...
TypeId PacketSocketTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t PacketSocketTag::GetSerializedSize() const {
  return 1 + m_destAddr.GetSerializedSize();
}

void PacketSocketTag::Serialize(TagBuffer i) const {
  i.WriteU8(m_packetType);
  m_destAddr.Serialize(i);
}

void PacketSocketTag::Deserialize(TagBuffer i) {
  m_packetType = (NetDevice::PacketType)i.ReadU8();
  m_destAddr.Deserialize(i);
}

void PacketSocketTag::Print(std::ostream &os) const {
...
std::string DeviceNameTag::GetDeviceName() const { return m_deviceName; }

NS_OBJECT_ENSURE_REGISTERED(DeviceNameTag);

TypeId DeviceNameTag::GetTypeId() {
...
void DeviceNameTag::Serialize(TagBuffer i) const {
...
void DeviceNameTag::Deserialize(TagBuffer i) {
  uint8_t l = i.ReadU8();
  char buf[256];
...
void DeviceNameTag::Print(std::ostream &os) const {
