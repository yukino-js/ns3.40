...
Mac8Address::Mac8Address() { m_address = 255; }

Mac8Address::Mac8Address(uint8_t addr) : m_address(addr) {}

Mac8Address::~Mac8Address() {}

uint8_t Mac8Address::GetType() {
  static uint8_t type = Address::Register();
  return type;
}

Address Mac8Address::ConvertTo() const {
  return Address(GetType(), &m_address, 1);
}

Mac8Address Mac8Address::ConvertFrom(const Address &address) {
...
bool Mac8Address::IsMatchingType(const Address &address) {
  return address.CheckCompatible(GetType(), 1);
}

Mac8Address::operator Address() const { return ConvertTo(); }

void Mac8Address::CopyFrom(const uint8_t *pBuffer) { m_address = *pBuffer; }

void Mac8Address::CopyTo(uint8_t *pBuffer) const { *pBuffer = m_address; }

Mac8Address Mac8Address::GetBroadcast() { return Mac8Address(255); }

Mac8Address Mac8Address::Allocate() {
...
void Mac8Address::ResetAllocationIndex() {
  NS_LOG_FUNCTION_NOARGS();
