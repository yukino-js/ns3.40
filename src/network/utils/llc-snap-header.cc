...
LlcSnapHeader::LlcSnapHeader() { NS_LOG_FUNCTION(this); }

void LlcSnapHeader::SetType(uint16_t type) {
  NS_LOG_FUNCTION(this);
  m_etherType = type;
}

uint16_t LlcSnapHeader::GetType() {
  NS_LOG_FUNCTION(this);
  return m_etherType;
}

uint32_t LlcSnapHeader::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
  return LLC_SNAP_HEADER_LENGTH;
}

TypeId LlcSnapHeader::GetTypeId() {
...
TypeId LlcSnapHeader::GetInstanceTypeId() const { return GetTypeId(); }

void LlcSnapHeader::Print(std::ostream &os) const {
