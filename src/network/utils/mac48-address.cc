...
bool Mac48Address::IsMatchingType(const Address &address) {
  NS_LOG_FUNCTION(&address);
  return address.CheckCompatible(GetType(), 6);
}

Mac48Address::operator Address() const { return ConvertTo(); }

Address Mac48Address::ConvertTo() const {
  NS_LOG_FUNCTION(this);
  return Address(GetType(), m_address, 6);
}

Mac48Address Mac48Address::ConvertFrom(const Address &address) {
  NS_LOG_FUNCTION(&address);
...
Mac48Address Mac48Address::Allocate() {
...
void Mac48Address::ResetAllocationIndex() {
  NS_LOG_FUNCTION_NOARGS();
  m_allocationIndex = 0;
}

uint8_t Mac48Address::GetType() {
  NS_LOG_FUNCTION_NOARGS();
  static uint8_t type = Address::Register();
  return type;
}

bool Mac48Address::IsBroadcast() const {
  NS_LOG_FUNCTION(this);
  return *this == GetBroadcast();
}

bool Mac48Address::IsGroup() const {
  NS_LOG_FUNCTION(this);
  return (m_address[0] & 0x01) == 0x01;
}

Mac48Address Mac48Address::GetBroadcast() {
  NS_LOG_FUNCTION_NOARGS();
  static Mac48Address broadcast("ff:ff:ff:ff:ff:ff");
  return broadcast;
}

Mac48Address Mac48Address::GetMulticastPrefix() {
  NS_LOG_FUNCTION_NOARGS();
  static Mac48Address multicast("01:00:5e:00:00:00");
  return multicast;
}

Mac48Address Mac48Address::GetMulticast6Prefix() {
  NS_LOG_FUNCTION_NOARGS();
