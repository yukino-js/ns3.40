...
bool PcapFile::Fail() const {
  NS_LOG_FUNCTION(this);
  return m_file.fail();
}

bool PcapFile::Eof() const {
  NS_LOG_FUNCTION(this);
  return m_file.eof();
}
...
uint32_t PcapFile::GetMagic() {
  NS_LOG_FUNCTION(this);
  return m_fileHeader.m_magicNumber;
}

uint16_t PcapFile::GetVersionMajor() {
  NS_LOG_FUNCTION(this);
  return m_fileHeader.m_versionMajor;
}

uint16_t PcapFile::GetVersionMinor() {
  NS_LOG_FUNCTION(this);
  return m_fileHeader.m_versionMinor;
}

int32_t PcapFile::GetTimeZoneOffset() {
  NS_LOG_FUNCTION(this);
  return m_fileHeader.m_zone;
}

uint32_t PcapFile::GetSigFigs() {
  NS_LOG_FUNCTION(this);
  return m_fileHeader.m_sigFigs;
}

uint32_t PcapFile::GetSnapLen() {
  NS_LOG_FUNCTION(this);
  return m_fileHeader.m_snapLen;
}

uint32_t PcapFile::GetDataLinkType() {
  NS_LOG_FUNCTION(this);
  return m_fileHeader.m_type;
}

bool PcapFile::GetSwapMode() {
  NS_LOG_FUNCTION(this);
  return m_swapMode;
}

bool PcapFile::IsNanoSecMode() {
  NS_LOG_FUNCTION(this);
  return m_nanosecMode;
}

uint8_t PcapFile::Swap(uint8_t val) {
  NS_LOG_FUNCTION(this << static_cast<uint32_t>(val));
  return val;
}

uint16_t PcapFile::Swap(uint16_t val) {
  NS_LOG_FUNCTION(this << val);
  return ((val >> 8) & 0x00ff) | ((val << 8) & 0xff00);
}

uint32_t PcapFile::Swap(uint32_t val) {
  NS_LOG_FUNCTION(this << val);
  return ((val >> 24) & 0x000000ff) | ((val >> 8) & 0x0000ff00) |
         ((val << 8) & 0x00ff0000) | ((val << 24) & 0xff000000);
}

void PcapFile::Swap(PcapFileHeader *from, PcapFileHeader *to) {
...
void PcapFile::Swap(PcapRecordHeader *from, PcapRecordHeader *to) {
  NS_LOG_FUNCTION(this << from << to);
...
void PcapFile::WriteFileHeader() {
  ...
  m_file.write((const char *)&headerOut->m_versionMinor,
               sizeof(headerOut->m_versionMinor));
  m_file.write((const char *)&headerOut->m_zone, sizeof(headerOut->m_zone));
  m_file.write((const char *)&headerOut->m_sigFigs,
               sizeof(headerOut->m_sigFigs));
  m_file.write((const char *)&headerOut->m_snapLen,
...
void PcapFile::ReadAndVerifyFileHeader() {
  ...
  m_file.read((char *)&m_fileHeader.m_versionMinor,
...
void PcapFile::Open(const std::string &filename, std::ios::openmode mode) {
...
void PcapFile::Init(uint32_t dataLinkType, uint32_t snapLen,
                    int32_t timeZoneCorrection, bool swapMode,
                    bool nanosecMode) {
...
uint32_t PcapFile::WritePacketHeader(uint32_t tsSec, uint32_t tsUsec,
                                     uint32_t totalLen) {
  NS_LOG_FUNCTION(this << tsSec << tsUsec << totalLen);
  NS_ASSERT(m_file.good());

  uint32_t inclLen =
      totalLen > m_fileHeader.m_snapLen ? m_fileHeader.m_snapLen : totalLen;
...
void PcapFile::Write(uint32_t tsSec, uint32_t tsUsec, const uint8_t *const data,
                     uint32_t totalLen) {
...
void PcapFile::Write(uint32_t tsSec, uint32_t tsUsec, Ptr<const Packet> p) {
...
void PcapFile::Write(uint32_t tsSec, uint32_t tsUsec, const Header &header,
                     Ptr<const Packet> p) {
...
void PcapFile::Read(uint8_t *const data, uint32_t maxBytes, uint32_t &tsSec,
                    ...
                    uint32_t &readLen) {
  NS_LOG_FUNCTION(this << &data << maxBytes << tsSec << tsUsec << inclLen
                       << origLen << readLen);
  NS_ASSERT(m_file.good());
...
bool PcapFile::Diff(const std::string &f1, const std::string &f2, uint32_t &sec,
                    uint32_t &usec, uint32_t &packets, uint32_t snapLen) {
