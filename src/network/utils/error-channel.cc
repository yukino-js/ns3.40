...
void ErrorChannel::SetJumpingTime(Time delay) { m_jumpingTime = delay; }

void ErrorChannel::SetJumpingMode(bool mode) {
  m_jumping = mode;
  m_jumpingState = 0;
}

void ErrorChannel::SetDuplicateTime(Time delay) { m_duplicateTime = delay; }

void ErrorChannel::SetDuplicateMode(bool mode) {
  m_duplicate = mode;
  m_duplicateState = 0;
}

void ErrorChannel::Send(Ptr<Packet> p, uint16_t protocol, Mac48Address to,
    ...
    if (m_jumping) {
      if (m_jumpingState % 2) {
        Simulator::ScheduleWithContext(tmp->GetNode()->GetId(), Seconds(0),
                                       &SimpleNetDevice::Receive, tmp,
                                       p->Copy(), protocol, to, from);
      } else {
...
void ErrorChannel::Add(Ptr<SimpleNetDevice> device) {
  m_devices.push_back(device);
}

std::size_t ErrorChannel::GetNDevices() const { return m_devices.size(); }

Ptr<NetDevice> ErrorChannel::GetDevice(std::size_t i) const {
  return m_devices[i];
}
