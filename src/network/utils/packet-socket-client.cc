...
TypeId PacketSocketClient::GetTypeId() {
  static TypeId tid =
...
PacketSocketClient::~PacketSocketClient() { NS_LOG_FUNCTION(this); }

void PacketSocketClient::SetRemote(PacketSocketAddress addr) {
  NS_LOG_FUNCTION(this << addr);
  m_peerAddress = addr;
  m_peerAddressSet = true;
}

void PacketSocketClient::DoDispose() {
  NS_LOG_FUNCTION(this);
  Application::DoDispose();
}

void PacketSocketClient::SetPriority(uint8_t priority) {
  m_priority = priority;
  if (m_socket) {
...
uint8_t PacketSocketClient::GetPriority() const { return m_priority; }

void PacketSocketClient::StartApplication() {
  NS_LOG_FUNCTION(this);
...
void PacketSocketClient::StopApplication() {
  NS_LOG_FUNCTION(this);
  Simulator::Cancel(m_sendEvent);
  m_socket->Close();
}

void PacketSocketClient::Send() {
  NS_LOG_FUNCTION(this);
  ...
  if ((m_socket->Send(p)) >= 0) {
    ...
    NS_LOG_INFO("TraceDelay TX "
                << m_size << " bytes to " << peerAddressStringStream.str()
                << " Uid: " << p->GetUid()
  ...
  if ((m_sent < m_maxPackets) || (m_maxPackets == 0)) {
    m_sendEvent =
        Simulator::Schedule(m_interval, &PacketSocketClient::Send, this);
  }
}
