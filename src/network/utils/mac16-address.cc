...
void Mac16Address::CopyFrom(const uint8_t buffer[2]) {
  NS_LOG_FUNCTION(this << &buffer);
  memcpy(m_address, buffer, 2);
}

void Mac16Address::CopyTo(uint8_t buffer[2]) const {
  NS_LOG_FUNCTION(this << &buffer);
  memcpy(buffer, m_address, 2);
}

bool Mac16Address::IsMatchingType(const Address &address) {
  NS_LOG_FUNCTION(&address);
  return address.CheckCompatible(GetType(), 2);
}

Mac16Address::operator Address() const { return ConvertTo(); }

Mac16Address Mac16Address::ConvertFrom(const Address &address) {
  NS_LOG_FUNCTION(address);
...
Address Mac16Address::ConvertTo() const {
  NS_LOG_FUNCTION(this);
  return Address(GetType(), m_address, 2);
}

uint16_t Mac16Address::ConvertToInt() const {
...
Mac16Address Mac16Address::Allocate() {
...
void Mac16Address::ResetAllocationIndex() {
  NS_LOG_FUNCTION_NOARGS();
  m_allocationIndex = 0;
}

uint8_t Mac16Address::GetType() {
  NS_LOG_FUNCTION_NOARGS();
...
Mac16Address Mac16Address::GetBroadcast() {
  NS_LOG_FUNCTION_NOARGS();
...
Mac16Address Mac16Address::GetMulticast(Ipv6Address address) {
  NS_LOG_FUNCTION(address);
...
bool Mac16Address::IsBroadcast() const {
  NS_LOG_FUNCTION(this);
  return m_address[0] == 0xff && m_address[1] == 0xff;
}

bool Mac16Address::IsMulticast() const {
  NS_LOG_FUNCTION(this);
