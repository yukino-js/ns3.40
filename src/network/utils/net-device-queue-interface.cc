...
TypeId NetDeviceQueue::GetTypeId() {
...
NetDeviceQueue::NetDeviceQueue()
    : m_stoppedByDevice(false), m_stoppedByQueueLimits(false),
...
bool NetDeviceQueue::IsStopped() const {
  NS_LOG_FUNCTION(this);
  return m_stoppedByDevice || m_stoppedByQueueLimits;
}

void NetDeviceQueue::Start() {
  NS_LOG_FUNCTION(this);
  m_stoppedByDevice = false;
}

void NetDeviceQueue::Stop() {
  NS_LOG_FUNCTION(this);
  m_stoppedByDevice = true;
}

void NetDeviceQueue::Wake() {
  NS_LOG_FUNCTION(this);
...
void NetDeviceQueue::NotifyAggregatedObject(Ptr<NetDeviceQueueInterface> ndqi) {
  NS_LOG_FUNCTION(this << ndqi);

  m_device = ndqi->GetObject<NetDevice>();
  NS_ABORT_MSG_IF(
      !m_device,
      "No NetDevice object was aggregated to the NetDeviceQueueInterface");
}

void NetDeviceQueue::SetWakeCallback(WakeCallback cb) { m_wakeCallback = cb; }

void NetDeviceQueue::NotifyQueuedBytes(uint32_t bytes) {
...
void NetDeviceQueue::NotifyTransmittedBytes(uint32_t bytes) {
  NS_LOG_FUNCTION(this << bytes);
...
void NetDeviceQueue::ResetQueueLimits() {
  NS_LOG_FUNCTION(this);
  if (!m_queueLimits) {
    return;
  }
...
void NetDeviceQueue::SetQueueLimits(Ptr<QueueLimits> ql) {
  NS_LOG_FUNCTION(this << ql);
  m_queueLimits = ql;
}

Ptr<QueueLimits> NetDeviceQueue::GetQueueLimits() {
  NS_LOG_FUNCTION(this);
...
TypeId NetDeviceQueueInterface::GetTypeId() {
  static TypeId tid =
...
NetDeviceQueueInterface::~NetDeviceQueueInterface() { NS_LOG_FUNCTION(this); }

Ptr<NetDeviceQueue> NetDeviceQueueInterface::GetTxQueue(std::size_t i) const {
  NS_ASSERT(i < m_txQueuesVector.size());
  return m_txQueuesVector[i];
}

std::size_t NetDeviceQueueInterface::GetNTxQueues() const {
  return m_txQueuesVector.size();
}

void NetDeviceQueueInterface::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void NetDeviceQueueInterface::SetTxQueuesType(TypeId type) {
...
void NetDeviceQueueInterface::SetNTxQueues(std::size_t numTxQueues) {
...
void NetDeviceQueueInterface::SetSelectQueueCallback(SelectQueueCallback cb) {
