...
void Mac64Address::CopyFrom(const uint8_t buffer[8]) {
  NS_LOG_FUNCTION(this << &buffer);
  std::memcpy(m_address, buffer, 8);
}

void Mac64Address::CopyTo(uint8_t buffer[8]) const {
  NS_LOG_FUNCTION(this << &buffer);
  std::memcpy(buffer, m_address, 8);
}

bool Mac64Address::IsMatchingType(const Address &address) {
  NS_LOG_FUNCTION(&address);
  return address.CheckCompatible(GetType(), 8);
}

Mac64Address::operator Address() const { return ConvertTo(); }

Mac64Address Mac64Address::ConvertFrom(const Address &address) {
  NS_LOG_FUNCTION(address);
...
Address Mac64Address::ConvertTo() const {
  NS_LOG_FUNCTION(this);
  return Address(GetType(), m_address, 8);
}

uint64_t Mac64Address::ConvertToInt() const {
  uint64_t shift = 0xFF;
...
Mac64Address Mac64Address::Allocate() {
...
void Mac64Address::ResetAllocationIndex() {
  NS_LOG_FUNCTION_NOARGS();
  m_allocationIndex = 0;
}

uint8_t Mac64Address::GetType() {
  NS_LOG_FUNCTION_NOARGS();
