...
PacketSocketAddress::PacketSocketAddress() { NS_LOG_FUNCTION(this); }

void PacketSocketAddress::SetProtocol(uint16_t protocol) {
  NS_LOG_FUNCTION(this << protocol);
  m_protocol = protocol;
}

void PacketSocketAddress::SetAllDevices() {
  NS_LOG_FUNCTION(this);
  m_isSingleDevice = false;
  m_device = 0;
}

void PacketSocketAddress::SetSingleDevice(uint32_t index) {
  NS_LOG_FUNCTION(this << index);
  m_isSingleDevice = true;
  m_device = index;
}

void PacketSocketAddress::SetPhysicalAddress(const Address address) {
  NS_LOG_FUNCTION(this << address);
  m_address = address;
}

uint16_t PacketSocketAddress::GetProtocol() const {
  NS_LOG_FUNCTION(this);
  return m_protocol;
}

bool PacketSocketAddress::IsSingleDevice() const {
  NS_LOG_FUNCTION(this);
  return m_isSingleDevice;
}

uint32_t PacketSocketAddress::GetSingleDevice() const {
  NS_LOG_FUNCTION(this);
  return m_device;
}

Address PacketSocketAddress::GetPhysicalAddress() const {
  NS_LOG_FUNCTION(this);
  return m_address;
}
...
Address PacketSocketAddress::ConvertTo() const {
  NS_LOG_FUNCTION(this);
  Address address;
...
PacketSocketAddress PacketSocketAddress::ConvertFrom(const Address &address) {
...
bool PacketSocketAddress::IsMatchingType(const Address &address) {
  NS_LOG_FUNCTION(address);
  return address.IsMatchingType(GetType());
}

uint8_t PacketSocketAddress::GetType() {
