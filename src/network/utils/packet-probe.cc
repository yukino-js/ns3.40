...
PacketProbe::~PacketProbe() { NS_LOG_FUNCTION(this); }

void PacketProbe::SetValue(Ptr<const Packet> packet) {
...
void PacketProbe::SetValueByPath(std::string path, Ptr<const Packet> packet) {
...
bool PacketProbe::ConnectByObject(std::string traceSource, Ptr<Object> obj) {
...
void PacketProbe::ConnectByPath(std::string path) {
  NS_LOG_FUNCTION(this << path);
  NS_LOG_DEBUG("Name of probe to search for in config database: " << path);
  Config::ConnectWithoutContext(
      path, MakeCallback(&ns3::PacketProbe::TraceSink, this));
}

void PacketProbe::TraceSink(Ptr<const Packet> packet) {
  NS_LOG_FUNCTION(this << packet);
  if (IsEnabled()) {
    m_packet = packet;
    m_output(packet);
