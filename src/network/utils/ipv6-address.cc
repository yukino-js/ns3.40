...
Ipv6Address::Ipv6Address(const char *address) {
  ...
  if (inet_pton(AF_INET6, address, m_address) <= 0) {
    memset(m_address, 0x00, 16);
    NS_LOG_LOGIC("Error, can not build an IPv6 address from an invalid string: "
                 << address);
    m_initialized = false;
    return;
  }
...
Ipv6Address Ipv6Address::MakeAutoconfiguredAddress(Address addr,
                                                   Ipv6Address prefix) {
  Ipv6Address ipv6Addr = Ipv6Address::GetAny();

  if (Mac64Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredAddress(
        Mac64Address::ConvertFrom(addr), prefix);
  } else if (Mac48Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredAddress(
        Mac48Address::ConvertFrom(addr), prefix);
  } else if (Mac16Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredAddress(
        Mac16Address::ConvertFrom(addr), prefix);
  } else if (Mac8Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredAddress(
        Mac8Address::ConvertFrom(addr), prefix);
  }
...
Ipv6Address Ipv6Address::MakeAutoconfiguredAddress(Mac16Address addr,
                                                   Ipv6Address prefix) {
...
Ipv6Address Ipv6Address::MakeAutoconfiguredAddress(Mac48Address addr,
                                                   Ipv6Address prefix) {
  NS_LOG_FUNCTION(addr << prefix);
  Ipv6Address ret;
  uint8_t buf[16];
...
Ipv6Address Ipv6Address::MakeAutoconfiguredAddress(Mac64Address addr,
                                                   Ipv6Address prefix) {
  NS_LOG_FUNCTION(addr << prefix);
  Ipv6Address ret;
  uint8_t buf[8];
...
Ipv6Address Ipv6Address::MakeAutoconfiguredAddress(Mac8Address addr,
                                                   Ipv6Address prefix) {
  NS_LOG_FUNCTION(addr << prefix);
  Ipv6Address ret;
  uint8_t buf[2];
...
Ipv6Address Ipv6Address::MakeAutoconfiguredLinkLocalAddress(Address addr) {
  ...
  if (Mac64Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredLinkLocalAddress(
        Mac64Address::ConvertFrom(addr));
  } else if (Mac48Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredLinkLocalAddress(
        Mac48Address::ConvertFrom(addr));
  } else if (Mac16Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredLinkLocalAddress(
        Mac16Address::ConvertFrom(addr));
  } else if (Mac8Address::IsMatchingType(addr)) {
    ipv6Addr = Ipv6Address::MakeAutoconfiguredLinkLocalAddress(
        Mac8Address::ConvertFrom(addr));
  }
...
Ipv6Address Ipv6Address::CombinePrefix(const Ipv6Prefix &prefix) const {
...
bool Ipv6Address::IsAllRoutersMulticast() const {
  ...
  return (*this == allroutersI || *this == allroutersL ||
          *this == allroutersR || *this == allroutersS);
}
...
bool Ipv6Address::HasPrefix(const Ipv6Prefix &prefix) const {
  NS_LOG_FUNCTION(this << prefix);
...
Ipv6Address::operator Address() const { return ConvertTo(); }
...
Ipv6Prefix::Ipv6Prefix(const char *prefix) {
  ...
  if (inet_pton(AF_INET6, prefix, m_prefix) <= 0) {
    NS_ABORT_MSG("Error, can not build an IPv6 prefix from an invalid string: "
                 << prefix);
  }
...
size_t Ipv6AddressHash::operator()(const Ipv6Address &x) const {
  uint8_t buf[16];
