...
QueueBase::QueueBase()
    : m_nBytes(0), m_nTotalReceivedBytes(0), m_nPackets(0),
      m_nTotalReceivedPackets(0), m_nTotalDroppedBytes(0),
      m_nTotalDroppedBytesBeforeEnqueue(0), m_nTotalDroppedBytesAfterDequeue(0),
      m_nTotalDroppedPackets(0), m_nTotalDroppedPacketsBeforeEnqueue(0),
      m_nTotalDroppedPacketsAfterDequeue(0) {
  NS_LOG_FUNCTION(this);
  m_maxSize =
      QueueSize(QueueSizeUnit::PACKETS, std::numeric_limits<uint32_t>::max());
}

QueueBase::~QueueBase() { NS_LOG_FUNCTION(this); }

void QueueBase::AppendItemTypeIfNotPresent(std::string &typeId,
...
bool QueueBase::IsEmpty() const {
  NS_LOG_FUNCTION(this);
  NS_LOG_LOGIC("returns " << (m_nPackets.Get() == 0));
  return m_nPackets.Get() == 0;
}

uint32_t QueueBase::GetNPackets() const {
  NS_LOG_FUNCTION(this);
  NS_LOG_LOGIC("returns " << m_nPackets);
  return m_nPackets;
}

uint32_t QueueBase::GetNBytes() const {
  NS_LOG_FUNCTION(this);
  NS_LOG_LOGIC(" returns " << m_nBytes);
  return m_nBytes;
}

QueueSize QueueBase::GetCurrentSize() const {
  NS_LOG_FUNCTION(this);
...
void QueueBase::SetMaxSize(QueueSize size) {
...
QueueSize QueueBase::GetMaxSize() const {
  NS_LOG_FUNCTION(this);
  return m_maxSize;
}

bool QueueBase::WouldOverflow(uint32_t nPackets, uint32_t nBytes) const {
  if (m_maxSize.GetUnit() == QueueSizeUnit::PACKETS) {
