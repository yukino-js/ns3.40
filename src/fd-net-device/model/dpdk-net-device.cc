...
TypeId DpdkNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::DpdkNetDevice")
          ...
          .AddAttribute("MaxTxBurst", "Size of Tx Burst.", UintegerValue(64),
                        ...
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute(
              "MempoolCacheSize", "Size of mempool cache.", UintegerValue(256),
              MakeUintegerAccessor(&DpdkNetDevice::m_mempoolCacheSize),
              MakeUintegerChecker<uint32_t>())
          .AddAttribute("NbRxDesc", "Number of Rx descriptors.",
                        UintegerValue(1024),
                        MakeUintegerAccessor(&DpdkNetDevice::m_nbRxDesc),
                        MakeUintegerChecker<uint16_t>())
          .AddAttribute("NbTxDesc", "Number of Tx descriptors.",
                        UintegerValue(1024),
...
DpdkNetDevice::DpdkNetDevice() : m_mempool(nullptr) { NS_LOG_FUNCTION(this); }
...
void DpdkNetDevice::SetDeviceName(std::string deviceName) {
  NS_LOG_FUNCTION(this);
...
void DpdkNetDevice::SignalHandler(int signum) {
...
void DpdkNetDevice::HandleTx() {
  int queueId = 0;
  rte_eth_tx_buffer_flush(m_portId, queueId, m_txBuffer);
}

void DpdkNetDevice::HandleRx() {
  int queueId = 0;
  m_rxBuffer->length =
      rte_eth_rx_burst(m_portId, queueId, m_rxBuffer->pkts, m_maxRxPktBurst);
...
int DpdkNetDevice::LaunchCore(void *arg) {
...
void DpdkNetDevice::InitDpdk(int argc, char **argv, std::string dpdkDriver) {
  ...
  unsigned int nbMbufs =
      RTE_MAX(nbPorts * (m_nbRxDesc + m_nbTxDesc + m_maxRxPktBurst +
                         m_maxTxPktBurst + nbLcores * m_mempoolCacheSize),
              8192U);

  NS_LOG_INFO("Create the mbuf pool");
  m_mempool =
      rte_pktmbuf_pool_create("mbuf_pool", nbMbufs, m_mempoolCacheSize, 0,
  ...
  ret = rte_eth_rx_queue_setup(m_portId, 0, m_nbRxDesc,
                               rte_eth_dev_socket_id(m_portId), &reqConf,
                               m_mempool);
  if (ret < 0) {
  ...
  ret = rte_eth_tx_queue_setup(m_portId, 0, m_nbTxDesc,
                               rte_eth_dev_socket_id(m_portId), &txqConf);
  if (ret < 0) {
...
uint8_t *DpdkNetDevice::AllocateBuffer(size_t len) {
...
void DpdkNetDevice::FreeBuffer(uint8_t *buf) {
  struct rte_mbuf *pkt;

  if (!buf) {
    return;
  }
  pkt = (struct rte_mbuf *)RTE_PTR_SUB(buf, sizeof(struct rte_mbuf) +
                                                RTE_PKTMBUF_HEADROOM);

  rte_pktmbuf_free(pkt);
}

ssize_t DpdkNetDevice::Write(uint8_t *buffer, size_t length) {
  ...
  pkt[0] = (struct rte_mbuf *)RTE_PTR_SUB(buffer, sizeof(struct rte_mbuf) +
                                                      RTE_PKTMBUF_HEADROOM);
