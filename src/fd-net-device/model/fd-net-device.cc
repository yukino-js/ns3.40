...
FdNetDevice::~FdNetDevice() { NS_LOG_FUNCTION(this); }

void FdNetDevice::DoInitialize() {
  NS_LOG_FUNCTION(this);
  Start(m_tStart);
...
void FdNetDevice::DoDispose() {
  NS_LOG_FUNCTION(this);
  StopDevice();
  NetDevice::DoDispose();
}

void FdNetDevice::SetEncapsulationMode(EncapsulationMode mode) {
  NS_LOG_FUNCTION(this << mode);
  m_encapMode = mode;
  NS_LOG_LOGIC("m_encapMode = " << m_encapMode);
}

FdNetDevice::EncapsulationMode FdNetDevice::GetEncapsulationMode() const {
  NS_LOG_FUNCTION(this);
  return m_encapMode;
}

void FdNetDevice::Start(Time tStart) {
  NS_LOG_FUNCTION(this << tStart);
  Simulator::Cancel(m_startEvent);
  m_startEvent = Simulator::Schedule(tStart, &FdNetDevice::StartDevice, this);
}

void FdNetDevice::Stop(Time tStop) {
  NS_LOG_FUNCTION(this << tStop);
  Simulator::Cancel(m_stopEvent);
  m_stopEvent = Simulator::Schedule(tStop, &FdNetDevice::StopDevice, this);
}

void FdNetDevice::StartDevice() {
  NS_LOG_FUNCTION(this);
...
Ptr<FdReader> FdNetDevice::DoCreateFdReader() {
  NS_LOG_FUNCTION(this);
...
void FdNetDevice::DoFinishStartingDevice() { NS_LOG_FUNCTION(this); }

void FdNetDevice::DoFinishStoppingDevice() { NS_LOG_FUNCTION(this); }

void FdNetDevice::StopDevice() {
  NS_LOG_FUNCTION(this);
...
void FdNetDevice::ReceiveCallback(uint8_t *buf, ssize_t len) {
...
uint8_t *FdNetDevice::AllocateBuffer(size_t len) {
  return (uint8_t *)malloc(len);
}

void FdNetDevice::FreeBuffer(uint8_t *buf) { free(buf); }

void FdNetDevice::ForwardUp() {
  NS_LOG_FUNCTION(this);
...
bool FdNetDevice::Send(Ptr<Packet> packet, const Address &destination,
                       uint16_t protocolNumber) {
  NS_LOG_FUNCTION(this << packet << destination << protocolNumber);
  return SendFrom(packet, m_address, destination, protocolNumber);
}

bool FdNetDevice::SendFrom(Ptr<Packet> packet, const Address &src,
  ...
  NS_ASSERT_MSG(packet->GetSize() <= m_mtu,
                "FdNetDevice::SendFrom(): Packet too big "
                    << packet->GetSize());
...
ssize_t FdNetDevice::Write(uint8_t *buffer, size_t length) {
...
void FdNetDevice::SetFileDescriptor(int fd) {
  if (m_fd == -1 and fd > 0) {
    m_fd = fd;
  }
}

int FdNetDevice::GetFileDescriptor() const { return m_fd; }

void FdNetDevice::SetAddress(Address address) {
  m_address = Mac48Address::ConvertFrom(address);
}

Address FdNetDevice::GetAddress() const { return m_address; }

void FdNetDevice::NotifyLinkUp() {
  m_linkUp = true;
  m_linkChangeCallbacks();
}

void FdNetDevice::SetIfIndex(const uint32_t index) { m_ifIndex = index; }

uint32_t FdNetDevice::GetIfIndex() const { return m_ifIndex; }
...
uint16_t FdNetDevice::GetMtu() const { return m_mtu; }

bool FdNetDevice::IsLinkUp() const { return m_linkUp; }

void FdNetDevice::AddLinkChangeCallback(Callback<void> callback) {
  m_linkChangeCallbacks.ConnectWithoutContext(callback);
}

bool FdNetDevice::IsBroadcast() const { return m_isBroadcast; }

void FdNetDevice::SetIsBroadcast(bool broadcast) { m_isBroadcast = broadcast; }

Address FdNetDevice::GetBroadcast() const {
  return Mac48Address("ff:ff:ff:ff:ff:ff");
}

bool FdNetDevice::IsMulticast() const { return m_isMulticast; }

void FdNetDevice::SetIsMulticast(bool multicast) { m_isMulticast = multicast; }

Address FdNetDevice::GetMulticast(Ipv4Address multicastGroup) const {
  return Mac48Address::GetMulticast(multicastGroup);
}

Address FdNetDevice::GetMulticast(Ipv6Address addr) const {
  return Mac48Address::GetMulticast(addr);
}

bool FdNetDevice::IsBridge() const { return false; }

bool FdNetDevice::IsPointToPoint() const { return false; }

Ptr<Node> FdNetDevice::GetNode() const { return m_node; }
...
bool FdNetDevice::NeedsArp() const { return true; }

void FdNetDevice::SetReceiveCallback(NetDevice::ReceiveCallback cb) {
  m_rxCallback = cb;
}

void FdNetDevice::SetPromiscReceiveCallback(PromiscReceiveCallback cb) {
  m_promiscRxCallback = cb;
}

bool FdNetDevice::SupportsSendFrom() const { return true; }
