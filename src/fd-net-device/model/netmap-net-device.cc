...
TypeId NetDeviceQueueLock::GetTypeId() {
...
bool NetDeviceQueueLock::IsStopped() const {
  m_mutex.lock();
...
void NetDeviceQueueLock::Start() {
  m_mutex.lock();
  NetDeviceQueue::Start();
  m_mutex.unlock();
}

void NetDeviceQueueLock::Stop() {
  m_mutex.lock();
  NetDeviceQueue::Stop();
  m_mutex.unlock();
}

void NetDeviceQueueLock::Wake() {
  m_mutex.lock();
  NetDeviceQueue::Wake();
  m_mutex.unlock();
}

void NetDeviceQueueLock::NotifyQueuedBytes(uint32_t bytes) {
  m_mutex.lock();
  NetDeviceQueue::NotifyQueuedBytes(bytes);
  m_mutex.unlock();
}

void NetDeviceQueueLock::NotifyTransmittedBytes(uint32_t bytes) {
  m_mutex.lock();
...
void NetmapNetDeviceFdReader::SetNetmapIfp(struct netmap_if *nifp) {
  NS_LOG_FUNCTION(this << nifp);
  m_nifp = nifp;
}

FdReader::Data NetmapNetDeviceFdReader::DoRead() {
...
TypeId NetmapNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::NetmapNetDevice")
          ...
          .AddAttribute("SyncAndNotifyQueuePeriod",
                        ...
                        MakeUintegerAccessor(
                            &NetmapNetDevice::m_syncAndNotifyQueuePeriod),
...
Ptr<FdReader> NetmapNetDevice::DoCreateFdReader() {
  NS_LOG_FUNCTION(this);
...
void NetmapNetDevice::DoFinishStartingDevice() {
  NS_LOG_FUNCTION(this);
...
void NetmapNetDevice::DoFinishStoppingDevice() {
  NS_LOG_FUNCTION(this);
...
uint32_t NetmapNetDevice::GetBytesInNetmapTxRing() {
  NS_LOG_FUNCTION(this);
...
void NetmapNetDevice::SetNetDeviceQueue(Ptr<NetDeviceQueue> queue) {
  NS_LOG_FUNCTION(this);

  m_queue = queue;
}

void NetmapNetDevice::SetNetmapInterfaceRepresentation(struct netmap_if *nifp) {
  NS_LOG_FUNCTION(this << nifp);

  m_nifp = nifp;
}

void NetmapNetDevice::SetTxRingsInfo(uint32_t nTxRings,
...
void NetmapNetDevice::SetRxRingsInfo(uint32_t nRxRings,
...
void NetmapNetDevice::SyncAndNotifyQueue() {
  ...
  while (m_syncAndNotifyQueueThreadRun == true) {
    ...
    NS_LOG_DEBUG("Space in the netmap ring of " << nm_ring_space(txring)
                                                << " packets");
  }
...
ssize_t NetmapNetDevice::Write(uint8_t *buffer, size_t length) {
