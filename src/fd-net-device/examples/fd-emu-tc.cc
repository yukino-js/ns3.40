...
void TcPacketsInQueue(Ptr<QueueDisc> q, Ptr<OutputStreamWrapper> stream) {
  ...
  *stream->GetStream() << Simulator::Now().GetSeconds() << " backlog "
                       << q->GetNPackets() << "p " << q->GetNBytes() << "b "
                       << " dropped " << q->GetStats().nTotalDroppedPackets
                       << "p " << q->GetStats().nTotalDroppedBytes << "b "
                       << std::endl;
}

#ifdef HAVE_NETMAP_USER_H
void Inflight(Ptr<NetmapNetDevice> dev, Ptr<OutputStreamWrapper> stream) {
...
int main(int argc, char *argv[]) {
  ...
  if (queueDiscType == "PfifoFast") {
    tch1.SetRootQueueDisc("ns3::PfifoFastQueueDisc", "MaxSize",
                          StringValue("1000p"));
  ...
  if (writer) {
    ...
    Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream(
        "exp-" + std::to_string(index) + "-router-tc-stats.txt");
    Simulator::Schedule(Seconds(0.001), &TcPacketsInQueue, q, stream);

#ifdef HAVE_NETMAP_USER_H
    if (emuMode.compare("netmap") == 0) {
      Ptr<NetmapNetDevice> dev = StaticCast<NetmapNetDevice>(device1);
      Ptr<OutputStreamWrapper> stream2 = ascii.CreateFileStream(
          "exp-" + std::to_string(index) + "-router-inflight.txt");
