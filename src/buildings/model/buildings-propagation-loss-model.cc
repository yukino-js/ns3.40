...
double BuildingsPropagationLossModel::ShadowingLoss::GetLoss() const {
...
TypeId BuildingsPropagationLossModel::GetTypeId() {
  static TypeId tid =
...
double BuildingsPropagationLossModel::ExternalWallLoss(
    Ptr<MobilityBuildingInfo> a) const {
  double loss = 0.0;
...
double BuildingsPropagationLossModel::HeightLoss(
    Ptr<MobilityBuildingInfo> node) const {
  double loss = 0.0;
...
double BuildingsPropagationLossModel::InternalWallsLoss(
    Ptr<MobilityBuildingInfo> a, Ptr<MobilityBuildingInfo> b) const {
...
double BuildingsPropagationLossModel::GetShadowing(Ptr<MobilityModel> a,
...
double BuildingsPropagationLossModel::EvaluateSigma(
    Ptr<MobilityBuildingInfo> a, Ptr<MobilityBuildingInfo> b) const {
  ...
  if (!isAIndoor) // a is outdoor
  {
    if (!isBIndoor) // b is outdoor
    {
      ...
      double sigma =
          std::sqrt((m_shadowingSigmaOutdoor * m_shadowingSigmaOutdoor) +
...
double BuildingsPropagationLossModel::DoCalcRxPower(
    double txPowerDbm, Ptr<MobilityModel> a, Ptr<MobilityModel> b) const {
  return txPowerDbm - GetLoss(a, b) - GetShadowing(a, b);
}

int64_t BuildingsPropagationLossModel::DoAssignStreams(int64_t stream) {
