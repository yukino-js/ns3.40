  ...
  static TypeId tid =
      TypeId("ns3::Building")
          ...
          .AddAttribute("NRoomsX", "The number of rooms in the X axis.",
                        UintegerValue(1),
                        MakeUintegerAccessor(&Building::GetNRoomsX,
                                             &Building::SetNRoomsX),
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute("NRoomsY", "The number of rooms in the Y axis.",
                        UintegerValue(1),
                        MakeUintegerAccessor(&Building::GetNRoomsY,
                                             &Building::SetNRoomsY),
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute("NFloors", "The number of floors of this building.",
                        UintegerValue(1),
                        MakeUintegerAccessor(&Building::GetNFloors,
                                             &Building::SetNFloors),
                        MakeUintegerChecker<uint32_t>())
          ...
          .AddAttribute(
              ...
              BoxValue(Box()),
              MakeBoxAccessor(&Building::GetBoundaries,
                              &Building::SetBoundaries),
              MakeBoxChecker())
          .AddAttribute("Type", "The type of building",
                        EnumValue(Building::Residential),
                        MakeEnumAccessor(&Building::GetBuildingType,
          ...
          .AddAttribute(
              ...
              MakeEnumAccessor(&Building::GetExtWallsType,
                               &Building::SetExtWallsType),
              MakeEnumChecker(
                  Building::Wood, "Wood", Building::ConcreteWithWindows,
                  "ConcreteWithWindows", Building::ConcreteWithoutWindows,
                  "ConcreteWithoutWindows", Building::StoneBlocks,
                  "StoneBlocks"));
  return tid;
}

Building::Building(double xMin, double xMax, double yMin, double yMax,
...
Building::~Building() { NS_LOG_FUNCTION(this); }

void Building::DoDispose() { NS_LOG_FUNCTION(this); }

uint32_t Building::GetId() const {
  NS_LOG_FUNCTION(this);
  return m_buildingId;
}

void Building::SetBoundaries(Box boundaries) {
  NS_LOG_FUNCTION(this << boundaries);
  m_buildingBounds = boundaries;
}

void Building::SetBuildingType(Building::BuildingType_t t) {
  NS_LOG_FUNCTION(this << t);
  m_buildingType = t;
}

void Building::SetExtWallsType(Building::ExtWallsType_t t) {
  NS_LOG_FUNCTION(this << t);
  m_externalWalls = t;
}

void Building::SetNFloors(uint16_t nfloors) {
  NS_LOG_FUNCTION(this << nfloors);
  m_floors = nfloors;
}

void Building::SetNRoomsX(uint16_t nroomx) {
  NS_LOG_FUNCTION(this << nroomx);
  m_roomsX = nroomx;
}

void Building::SetNRoomsY(uint16_t nroomy) {
  NS_LOG_FUNCTION(this << nroomy);
  m_roomsY = nroomy;
}

Box Building::GetBoundaries() const {
  NS_LOG_FUNCTION(this);
  return m_buildingBounds;
}

Building::BuildingType_t Building::GetBuildingType() const {
  return (m_buildingType);
}

Building::ExtWallsType_t Building::GetExtWallsType() const {
  return (m_externalWalls);
}

uint16_t Building::GetNFloors() const { return (m_floors); }

uint16_t Building::GetNRoomsX() const { return (m_roomsX); }

uint16_t Building::GetNRoomsY() const { return (m_roomsY); }

bool Building::IsInside(Vector position) const {
  return m_buildingBounds.IsInside(position);
}

uint16_t Building::GetRoomX(Vector position) const {
  ...
  if (position.x == m_buildingBounds.xMax) {
    ...
    NS_LOG_LOGIC("xLength=" << xLength << ", x=" << x
                            << ", m_roomsX=" << m_roomsX);
  }
...
uint16_t Building::GetRoomY(Vector position) const {
  NS_ASSERT(IsInside(position));
  uint16_t n;
  ...
  if (position.y == m_buildingBounds.yMax) {
    ...
    NS_LOG_LOGIC("yLength=" << yLength << ", y=" << y
                            << ", m_roomsY=" << m_roomsY);
  }
...
uint16_t Building::GetFloor(Vector position) const {
  NS_ASSERT(IsInside(position));
  uint16_t n;
  ...
  if (position.z == m_buildingBounds.zMax) {
    ...
    NS_LOG_LOGIC("zLength=" << zLength << ", z=" << z
                            << ", m_floors=" << m_floors);
  }
...
bool Building::IsIntersect(const Vector &l1, const Vector &l2) const {
