...
TypeId MobilityBuildingInfo::GetTypeId() {
...
void MobilityBuildingInfo::DoInitialize() {
...
bool MobilityBuildingInfo::IsIndoor() {
  NS_LOG_FUNCTION(this);
  Ptr<MobilityModel> mm = this->GetObject<MobilityModel>();
  Vector currentPosition = mm->GetPosition();
  bool posNotEqual = (currentPosition < m_cachedPosition) ||
...
void MobilityBuildingInfo::SetIndoor(Ptr<Building> building, uint8_t nfloor,
                                     uint8_t nroomx, uint8_t nroomy) {
  NS_LOG_FUNCTION(this);
  m_indoor = true;
  m_myBuilding = building;
  m_nFloor = nfloor;
...
void MobilityBuildingInfo::SetIndoor(uint8_t nfloor, uint8_t nroomx,
                                     uint8_t nroomy) {
  NS_LOG_FUNCTION(this);
  m_indoor = true;
  m_nFloor = nfloor;
...
void MobilityBuildingInfo::SetOutdoor() {
  NS_LOG_FUNCTION(this);
  m_indoor = false;
}

uint8_t MobilityBuildingInfo::GetFloorNumber() {
  NS_LOG_FUNCTION(this);
  return (m_nFloor);
}

uint8_t MobilityBuildingInfo::GetRoomNumberX() {
  NS_LOG_FUNCTION(this);
  return (m_roomX);
}

uint8_t MobilityBuildingInfo::GetRoomNumberY() {
  NS_LOG_FUNCTION(this);
  return (m_roomY);
}

Ptr<Building> MobilityBuildingInfo::GetBuilding() {
  NS_LOG_FUNCTION(this);
  return (m_myBuilding);
}

void MobilityBuildingInfo::MakeConsistent(Ptr<MobilityModel> mm) {
  bool found = false;
