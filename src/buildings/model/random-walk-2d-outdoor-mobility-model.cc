...
TypeId RandomWalk2dOutdoorMobilityModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::RandomWalk2dOutdoorMobilityModel")
          ...
          .AddAttribute("Bounds", "Bounds of the area to cruise.",
                        ...
                        MakeRectangleAccessor(
                            &RandomWalk2dOutdoorMobilityModel::m_bounds),
          ...
          .AddAttribute("Distance",
                        ...
                        DoubleValue(30.0),
                        MakeDoubleAccessor(
                            &RandomWalk2dOutdoorMobilityModel::m_modeDistance),
          ...
          .AddAttribute(
              ...
              MakeEnumChecker(
                  RandomWalk2dOutdoorMobilityModel::MODE_DISTANCE, "Distance",
          ...
          .AddAttribute(
              ...
              MakePointerAccessor(
                  &RandomWalk2dOutdoorMobilityModel::m_direction),
          ...
          .AddAttribute(
              ...
              MakeUintegerAccessor(
                  &RandomWalk2dOutdoorMobilityModel::m_maxIter),
              MakeUintegerChecker<uint32_t>());
  return tid;
}

void RandomWalk2dOutdoorMobilityModel::DoInitialize() {
  DoInitializePrivate();
  MobilityModel::DoInitialize();
}

void RandomWalk2dOutdoorMobilityModel::DoInitializePrivate() {
  m_helper.Update();
...
void RandomWalk2dOutdoorMobilityModel::DoWalk(Time delayLeft) {
  ...
  if (m_bounds.IsInside(nextPosition)) {
    if (outdoor) {
      m_event = Simulator::Schedule(
          delayLeft, &RandomWalk2dOutdoorMobilityModel::DoInitializePrivate,
          this);
    } else {
      NS_LOG_LOGIC("NextPosition would lead into a building");
      nextPosition = CalculateIntersectionFromOutside(
          position, nextPosition, building->GetBoundaries());
      ...
      m_event = Simulator::Schedule(
          delay, &RandomWalk2dOutdoorMobilityModel::AvoidBuilding, this,
          delayLeft - delay, nextPosition);
    }
  } else {
    ...
    if (outdoor) {
      ...
      if (speed.x != 0) {
        delaySeconds = std::min(
            delaySeconds, std::abs((nextPosition.x - position.x) / speed.x));
      } else if (speed.y != 0) {
        delaySeconds = std::min(
            delaySeconds, std::abs((nextPosition.y - position.y) / speed.y));
      } else {
        NS_ABORT_MSG(
      ...
      m_event =
          Simulator::Schedule(delay, &RandomWalk2dOutdoorMobilityModel::Rebound,
      ...
      double delaySecondsY = std::numeric_limits<double>::max();
      if (speed.x != 0) {
        delaySecondsX = std::min(
            delaySecondsX, std::abs((nextPosition.x - position.x) / speed.x));
      }
      if (speed.y != 0) {
        delaySecondsY = std::min(
            delaySecondsY, std::abs((nextPosition.y - position.y) / speed.y));
      }
...
RandomWalk2dOutdoorMobilityModel::IsLineClearOfBuildings(
    Vector currentPosition, Vector nextPosition) const {
...
Vector RandomWalk2dOutdoorMobilityModel::CalculateIntersectionFromOutside(
    const Vector &current, const Vector &next, Box boundaries) const {
  ...
  case Rectangle::RIGHTSIDE:
    ...
    yIntersect =
        (next.y - current.y) / (next.x - current.x) * (xIntersect - current.x) +
        current.y;
    break;
  ...
  case Rectangle::TOPSIDE:
    ...
    xIntersect =
        (next.x - current.x) / (next.y - current.y) * (yIntersect - current.y) +
        current.x;
    break;
...
void RandomWalk2dOutdoorMobilityModel::Rebound(Time delayLeft) {
  NS_LOG_FUNCTION(this << delayLeft.GetSeconds());
...
void RandomWalk2dOutdoorMobilityModel::AvoidBuilding(Time delayLeft,
  ...
  while (nextWouldBeInside && iter < m_maxIter) {
    ...
    if (!outdoor) {
      ...
      NS_LOG_LOGIC("inside loop intersect " << intersectPosition
...
Vector RandomWalk2dOutdoorMobilityModel::DoGetPosition() const {
  m_helper.UpdateWithBounds(m_bounds);
  return m_helper.GetCurrentPosition();
}

void RandomWalk2dOutdoorMobilityModel::DoSetPosition(const Vector &position) {
  NS_ASSERT(m_bounds.IsInside(position));
  m_helper.SetPosition(position);
  Simulator::Remove(m_event);
  m_event = Simulator::ScheduleNow(
      &RandomWalk2dOutdoorMobilityModel::DoInitializePrivate, this);
}

Vector RandomWalk2dOutdoorMobilityModel::DoGetVelocity() const {
  return m_helper.GetVelocity();
}

int64_t RandomWalk2dOutdoorMobilityModel::DoAssignStreams(int64_t stream) {
