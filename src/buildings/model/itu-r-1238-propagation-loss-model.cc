...
TypeId ItuR1238PropagationLossModel::GetTypeId() {
  static TypeId tid =
...
double ItuR1238PropagationLossModel::GetLoss(Ptr<MobilityModel> a1,
                                             Ptr<MobilityModel> b1) const {
  ...
  NS_LOG_LOGIC(this << " A floor " << (uint16_t)a->GetFloorNumber()
                    << " B floor " << (uint16_t)b->GetFloorNumber() << " n "
                    << n);
  double Lf = 0.0;
  ...
  NS_LOG_INFO(this << " Node " << a1->GetPosition() << " <-> "
                   << b1->GetPosition() << " loss = " << loss << " dB");

  return loss;
}

double ItuR1238PropagationLossModel::DoCalcRxPower(double txPowerDbm,
                                                   Ptr<MobilityModel> a,
                                                   Ptr<MobilityModel> b) const {
  return (txPowerDbm - GetLoss(a, b));
}

int64_t ItuR1238PropagationLossModel::DoAssignStreams(int64_t stream) {
  return 0;
}
