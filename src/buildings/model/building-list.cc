...
TypeId BuildingListPriv::GetTypeId() {
  static TypeId tid =
...
Ptr<BuildingListPriv> BuildingListPriv::Get() { return *DoGet(); }

Ptr<BuildingListPriv> *BuildingListPriv::DoGet() {
...
void BuildingListPriv::Delete() {
  NS_LOG_FUNCTION_NOARGS();
  Config::UnregisterRootNamespaceObject(Get());
  (*DoGet()) = nullptr;
}

BuildingListPriv::BuildingListPriv() { NS_LOG_FUNCTION_NOARGS(); }

BuildingListPriv::~BuildingListPriv() {}

void BuildingListPriv::DoDispose() {
  NS_LOG_FUNCTION_NOARGS();
...
uint32_t BuildingListPriv::Add(Ptr<Building> building) {
  uint32_t index = m_buildings.size();
  m_buildings.push_back(building);
  Simulator::ScheduleWithContext(index, TimeStep(0), &Building::Initialize,
                                 building);
  return index;
}

BuildingList::Iterator BuildingListPriv::Begin() const {
  return m_buildings.begin();
}

BuildingList::Iterator BuildingListPriv::End() const {
  return m_buildings.end();
}

uint32_t BuildingListPriv::GetNBuildings() { return m_buildings.size(); }

Ptr<Building> BuildingListPriv::GetBuilding(uint32_t n) {
  NS_ASSERT_MSG(n < m_buildings.size(),
                "Building index " << n << " is out of range (only have "
                                  << m_buildings.size() << " buildings).");
...
uint32_t BuildingList::Add(Ptr<Building> building) {
  return BuildingListPriv::Get()->Add(building);
}

BuildingList::Iterator BuildingList::Begin() {
  return BuildingListPriv::Get()->Begin();
}

BuildingList::Iterator BuildingList::End() {
  return BuildingListPriv::Get()->End();
}

Ptr<Building> BuildingList::GetBuilding(uint32_t n) {
  return BuildingListPriv::Get()->GetBuilding(n);
}

uint32_t BuildingList::GetNBuildings() {
