...
TypeId GridBuildingAllocator::GetTypeId() {
  static TypeId tid =
...
void GridBuildingAllocator::SetBuildingAttribute(std::string n,
                                                 const AttributeValue &v) {
  NS_LOG_FUNCTION(this);
  m_buildingFactory.Set(n, v);
}

BuildingContainer GridBuildingAllocator::Create(uint32_t n) const {
  NS_LOG_FUNCTION(this);
  PushAttributes();
...
void GridBuildingAllocator::PushAttributes() const {
  NS_LOG_FUNCTION(this);
