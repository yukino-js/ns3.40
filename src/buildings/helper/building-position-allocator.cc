...
TypeId RandomBuildingPositionAllocator::GetTypeId() {
  static TypeId tid =
...
Vector RandomBuildingPositionAllocator::GetNext() const {
  ...
  if (m_withReplacement) {
    ...
    if (m_buildingListWithoutReplacement.empty()) {
      for (auto bit = BuildingList::Begin(); bit != BuildingList::End();
           ++bit) {
        m_buildingListWithoutReplacement.push_back(*bit);
      }
    }
    uint32_t n =
        m_rand->GetInteger(0, m_buildingListWithoutReplacement.size() - 1);
    b = m_buildingListWithoutReplacement.at(n);
    m_buildingListWithoutReplacement.erase(
        m_buildingListWithoutReplacement.begin() + n);
  }

  Ptr<RandomBoxPositionAllocator> pa =
...
int64_t RandomBuildingPositionAllocator::AssignStreams(int64_t stream) {
...
TypeId OutdoorPositionAllocator::GetTypeId() {
  static TypeId tid =
...
void OutdoorPositionAllocator::SetX(Ptr<RandomVariableStream> x) { m_x = x; }

void OutdoorPositionAllocator::SetY(Ptr<RandomVariableStream> y) { m_y = y; }

void OutdoorPositionAllocator::SetZ(Ptr<RandomVariableStream> z) { m_z = z; }

Vector OutdoorPositionAllocator::GetNext() const {
  ...
  while (!outdoor && attempts < m_maxAttempts) {
    ...
    if (inside) {
      NS_LOG_INFO("Inside a building, attempt " << attempts << " out of "
                                                << m_maxAttempts);
      attempts++;
    } else {
...
int64_t OutdoorPositionAllocator::AssignStreams(int64_t stream) {
...
TypeId RandomRoomPositionAllocator::GetTypeId() {
...
Vector RandomRoomPositionAllocator::GetNext() const {
  ...
  NS_LOG_LOGIC("considering building " << r.b->GetId() << " room (" << r.roomx
                                       << ", " << r.roomy << ", " << r.floor
                                       << ")");
...
int64_t RandomRoomPositionAllocator::AssignStreams(int64_t stream) {
  m_rand->SetStream(stream);
  return 1;
}
...
SameRoomPositionAllocator::SameRoomPositionAllocator(NodeContainer c)
  ...
  for (auto it = m_nodes.Begin(); it != m_nodes.End(); ++it) {
    ...
    NS_ASSERT_MSG(bmm, "MobilityBuildingInfo has not been aggregated to this "
                       "node mobility model");
    bmm->MakeConsistent(mm);
  }
}

TypeId SameRoomPositionAllocator::GetTypeId() {
...
Vector SameRoomPositionAllocator::GetNext() const {
  NS_LOG_FUNCTION(this);
  ...
  NS_LOG_LOGIC("considering building " << bmm->GetBuilding()->GetId()
                                       << " room (" << roomx << ", " << roomy
                                       << ", " << floor << ")");
...
int64_t SameRoomPositionAllocator::AssignStreams(int64_t stream) {
  m_rand->SetStream(stream);
  return 1;
}
...
FixedRoomPositionAllocator::FixedRoomPositionAllocator(uint32_t x, uint32_t y,
                                                       uint32_t z,
                                                       Ptr<Building> pbtr) {
  m_rand = CreateObject<UniformRandomVariable>();
  roomx = x;
...
TypeId FixedRoomPositionAllocator::GetTypeId() {
...
Vector FixedRoomPositionAllocator::GetNext() const {
  NS_LOG_LOGIC("considering building " << bptr->GetId() << " room (" << roomx
                                       << ", " << roomy << ", " << floor
                                       << ")");
...
int64_t FixedRoomPositionAllocator::AssignStreams(int64_t stream) {
  m_rand->SetStream(stream);
  return 1;
}
