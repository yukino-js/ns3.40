...
std::string BuildingsHelperOneTestCase::BuildNameString(PositionInBuilding pib,
  ...
  oss << "pos=" << pib.pos;
  if (pib.indoor) {
    oss << ", bid=" << pib.bid << ", rx=" << pib.rx << ", ry=" << pib.ry
        << ", fn=" << pib.fn;
  } else {
    oss << ", outdoor";
  }
...
BuildingsHelperOneTestCase::BuildingsHelperOneTestCase(PositionInBuilding pib,
                                                       BuildingData bd)
    : TestCase(BuildNameString(pib, bd)), m_pib(pib), m_bd(bd) {}

void BuildingsHelperOneTestCase::DoRun() {
