...
void OpenFlowSwitchHelper::SetDeviceAttribute(std::string n1,
...
OpenFlowSwitchHelper::Install(Ptr<Node> node, NetDeviceContainer c,
  ...
  Ptr<OpenFlowSwitchNetDevice> dev =
      m_deviceFactory.Create<OpenFlowSwitchNetDevice>();
  devs.Add(dev);
  node->AddDevice(dev);
...
NetDeviceContainer OpenFlowSwitchHelper::Install(Ptr<Node> node,
...
NetDeviceContainer OpenFlowSwitchHelper::Install(std::string nodeName,
                                                 NetDeviceContainer c) {
  NS_LOG_FUNCTION_NOARGS();
