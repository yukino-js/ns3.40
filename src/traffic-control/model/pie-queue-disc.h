...
private:
  ...
  Time m_maxBurst; //!< Maximum burst allowed before random early dropping kicks
                   //!< in
  ...
  uint32_t m_dqThreshold; //!< Minimum queue size in bytes before dequeue rate
                          //!< is measured
  bool m_useDqRateEstimator; //!< Enable/Disable usage of dequeue rate estimator
                             //!< for queue delay calculation
  ...
  double m_markEcnTh; //!< ECN marking threshold (default 10% as suggested in
                      //!< RFC 8033)
  Time
      m_activeThreshold; //!< Threshold for activating PIE (disabled by default)
  ...
  Time m_burstAllowance; //!< Current max burst value in seconds that is allowed
                         //!< before random drops kick in
