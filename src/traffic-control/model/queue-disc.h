...
private:
  ...
  TracedCallback<Time>
      m_sojourn;       //!< Sojourn time of the latest dequeued packet
  ...
  SendCallback
      m_send;     //!< Callback used to send a packet to the receiving object
  bool m_running; //!< The queue disc is performing multiple dequeue operations
  Ptr<QueueDiscItem>
      m_requeued; //!< The last packet that failed to be transmitted
  bool m_peeked;  //!< A packet was dequeued because Peek was called
  std::string m_childQueueDiscDropMsg; //!< Reason why a packet was dropped by a
                                       //!< child queue disc
