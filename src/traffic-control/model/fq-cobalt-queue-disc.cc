...
FqCobaltFlow::~FqCobaltFlow() { NS_LOG_FUNCTION(this); }

void FqCobaltFlow::SetDeficit(uint32_t deficit) {
  NS_LOG_FUNCTION(this << deficit);
  m_deficit = deficit;
}

int32_t FqCobaltFlow::GetDeficit() const {
  NS_LOG_FUNCTION(this);
  return m_deficit;
}

void FqCobaltFlow::IncreaseDeficit(int32_t deficit) {
  NS_LOG_FUNCTION(this << deficit);
  m_deficit += deficit;
}

void FqCobaltFlow::SetStatus(FlowStatus status) {
  NS_LOG_FUNCTION(this);
  m_status = status;
}

FqCobaltFlow::FlowStatus FqCobaltFlow::GetStatus() const {
  NS_LOG_FUNCTION(this);
  return m_status;
}

void FqCobaltFlow::SetIndex(uint32_t index) {
  NS_LOG_FUNCTION(this);
  m_index = index;
}

uint32_t FqCobaltFlow::GetIndex() const { return m_index; }
  ...
  static TypeId tid =
      TypeId("ns3::FqCobaltQueueDisc")
          ...
          .AddAttribute("EnableSetAssociativeHash",
                        ...
                        BooleanValue(false),
                        MakeBooleanAccessor(
                            &FqCobaltQueueDisc::m_enableSetAssociativeHash),
                        MakeBooleanChecker())
          .AddAttribute(
              "SetWays",
...
FqCobaltQueueDisc::~FqCobaltQueueDisc() { NS_LOG_FUNCTION(this); }

void FqCobaltQueueDisc::SetQuantum(uint32_t quantum) {
  NS_LOG_FUNCTION(this << quantum);
  m_quantum = quantum;
}

uint32_t FqCobaltQueueDisc::GetQuantum() const { return m_quantum; }

uint32_t FqCobaltQueueDisc::SetAssociativeHash(uint32_t flowHash) {
...
bool FqCobaltQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item) {
...
Ptr<QueueDiscItem> FqCobaltQueueDisc::DoDequeue() {
  NS_LOG_FUNCTION(this);
...
bool FqCobaltQueueDisc::CheckConfig() {
  NS_LOG_FUNCTION(this);
...
void FqCobaltQueueDisc::InitializeParams() {
  NS_LOG_FUNCTION(this);
...
uint32_t FqCobaltQueueDisc::FqCobaltDrop() {
  NS_LOG_FUNCTION(this);
