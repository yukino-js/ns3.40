...
FqPieFlow::~FqPieFlow() { NS_LOG_FUNCTION(this); }

void FqPieFlow::SetDeficit(uint32_t deficit) {
  NS_LOG_FUNCTION(this << deficit);
  m_deficit = deficit;
}

int32_t FqPieFlow::GetDeficit() const {
  NS_LOG_FUNCTION(this);
  return m_deficit;
}

void FqPieFlow::IncreaseDeficit(int32_t deficit) {
  NS_LOG_FUNCTION(this << deficit);
  m_deficit += deficit;
}

void FqPieFlow::SetStatus(FlowStatus status) {
  NS_LOG_FUNCTION(this);
  m_status = status;
}

FqPieFlow::FlowStatus FqPieFlow::GetStatus() const {
  NS_LOG_FUNCTION(this);
  return m_status;
}

void FqPieFlow::SetIndex(uint32_t index) {
  NS_LOG_FUNCTION(this);
  m_index = index;
}

uint32_t FqPieFlow::GetIndex() const { return m_index; }
...
FqPieQueueDisc::~FqPieQueueDisc() { NS_LOG_FUNCTION(this); }

void FqPieQueueDisc::SetQuantum(uint32_t quantum) {
  NS_LOG_FUNCTION(this << quantum);
  m_quantum = quantum;
}

uint32_t FqPieQueueDisc::GetQuantum() const { return m_quantum; }

uint32_t FqPieQueueDisc::SetAssociativeHash(uint32_t flowHash) {
...
bool FqPieQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item) {
...
Ptr<QueueDiscItem> FqPieQueueDisc::DoDequeue() {
  NS_LOG_FUNCTION(this);
  ...
  do {
    ...
    while (!found && !m_newFlows.empty()) {
      ...
      if (flow->GetDeficit() <= 0) {
        NS_LOG_DEBUG("Increase deficit for new flow index "
                     << flow->GetIndex());
        ...
        NS_LOG_DEBUG("Found a new flow " << flow->GetIndex()
    ...
    while (!found && !m_oldFlows.empty()) {
      ...
      if (flow->GetDeficit() <= 0) {
        NS_LOG_DEBUG("Increase deficit for old flow index "
                     << flow->GetIndex());
        flow->IncreaseDeficit(m_quantum);
        m_oldFlows.push_back(flow);
        m_oldFlows.pop_front();
      } else {
        NS_LOG_DEBUG("Found an old flow " << flow->GetIndex()
                                          << " with positive deficit");
        found = true;
      }
    }
...
bool FqPieQueueDisc::CheckConfig() {
  NS_LOG_FUNCTION(this);
  ...
  if (!m_quantum) {
    ...
    if (ndqi && (dev = ndqi->GetObject<NetDevice>())) {
      ...
      NS_LOG_DEBUG(
          "Setting the quantum to the MTU of the device: " << m_quantum);
    }
...
void FqPieQueueDisc::InitializeParams() {
  NS_LOG_FUNCTION(this);
  ...
  m_queueDiscFactory.Set("UseDequeueRateEstimator",
                         BooleanValue(m_useDqRateEstimator));
  m_queueDiscFactory.Set("UseCapDropAdjustment",
                         BooleanValue(m_isCapDropAdjustment));
  m_queueDiscFactory.Set("UseDerandomization",
                         BooleanValue(m_useDerandomization));
}

uint32_t FqPieQueueDisc::FqPieDrop() {
  NS_LOG_FUNCTION(this);
  ...
  do {
    NS_LOG_DEBUG("Drop packet (overflow); count: "
                 << count << " len: " << len << " threshold: " << threshold);
