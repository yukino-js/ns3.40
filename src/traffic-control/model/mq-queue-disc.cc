...
MqQueueDisc::MqQueueDisc() : QueueDisc(QueueDiscSizePolicy::NO_LIMITS) {
  NS_LOG_FUNCTION(this);
}

MqQueueDisc::~MqQueueDisc() { NS_LOG_FUNCTION(this); }

MqQueueDisc::WakeMode MqQueueDisc::GetWakeMode() const { return WAKE_CHILD; }

bool MqQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item) {
  NS_FATAL_ERROR("MqQueueDisc: DoEnqueue should never be called");
}

Ptr<QueueDiscItem> MqQueueDisc::DoDequeue() {
  NS_FATAL_ERROR("MqQueueDisc: DoDequeue should never be called");
}

Ptr<const QueueDiscItem> MqQueueDisc::DoPeek() {
  NS_FATAL_ERROR("MqQueueDisc: DoPeek should never be called");
}

bool MqQueueDisc::CheckConfig() {
  NS_LOG_FUNCTION(this);
...
void MqQueueDisc::InitializeParams() { NS_LOG_FUNCTION(this); }
