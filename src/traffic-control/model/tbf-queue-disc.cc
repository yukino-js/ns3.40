...
TypeId TbfQueueDisc::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TbfQueueDisc")
          ...
          .AddAttribute("Burst", "Size of the first bucket in bytes",
          ...
          .AddAttribute(
              ...
              UintegerValue(0), MakeUintegerAccessor(&TbfQueueDisc::SetMtu),
              MakeUintegerChecker<uint32_t>())
          .AddAttribute(
              "Rate",
...
TbfQueueDisc::~TbfQueueDisc() { NS_LOG_FUNCTION(this); }

void TbfQueueDisc::DoDispose() {
  NS_LOG_FUNCTION(this);
  QueueDisc::DoDispose();
}

void TbfQueueDisc::SetBurst(uint32_t burst) {
  NS_LOG_FUNCTION(this << burst);
  m_burst = burst;
}

uint32_t TbfQueueDisc::GetBurst() const {
  NS_LOG_FUNCTION(this);
  return m_burst;
}

void TbfQueueDisc::SetMtu(uint32_t mtu) {
  NS_LOG_FUNCTION(this << mtu);
  m_mtu = mtu;
}

uint32_t TbfQueueDisc::GetMtu() const {
  NS_LOG_FUNCTION(this);
  return m_mtu;
}

void TbfQueueDisc::SetRate(DataRate rate) {
  NS_LOG_FUNCTION(this << rate);
  m_rate = rate;
}

DataRate TbfQueueDisc::GetRate() const {
  NS_LOG_FUNCTION(this);
  return m_rate;
}

void TbfQueueDisc::SetPeakRate(DataRate peakRate) {
  NS_LOG_FUNCTION(this << peakRate);
  m_peakRate = peakRate;
}

DataRate TbfQueueDisc::GetPeakRate() const {
  NS_LOG_FUNCTION(this);
  return m_peakRate;
}

uint32_t TbfQueueDisc::GetFirstBucketTokens() const {
  NS_LOG_FUNCTION(this);
  return m_btokens;
}

uint32_t TbfQueueDisc::GetSecondBucketTokens() const {
  NS_LOG_FUNCTION(this);
  return m_ptokens;
}

bool TbfQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item) {
...
Ptr<QueueDiscItem> TbfQueueDisc::DoDequeue() {
  NS_LOG_FUNCTION(this);
  Ptr<const QueueDiscItem> itemPeek =
      GetQueueDiscClass(0)->GetQueueDisc()->Peek();
  ...
  if (itemPeek) {
    ...
    if (m_id.IsExpired()) {
      ...
      NS_LOG_LOGIC("Waking Event Scheduled in "
                   << requiredDelayTime.As(Time::S));
    }
  }
  return nullptr;
}

bool TbfQueueDisc::CheckConfig() {
  NS_LOG_FUNCTION(this);
