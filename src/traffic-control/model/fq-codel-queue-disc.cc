...
FqCoDelFlow::~FqCoDelFlow() { NS_LOG_FUNCTION(this); }

void FqCoDelFlow::SetDeficit(uint32_t deficit) {
  NS_LOG_FUNCTION(this << deficit);
  m_deficit = deficit;
}

int32_t FqCoDelFlow::GetDeficit() const {
  NS_LOG_FUNCTION(this);
  return m_deficit;
}

void FqCoDelFlow::IncreaseDeficit(int32_t deficit) {
  NS_LOG_FUNCTION(this << deficit);
  m_deficit += deficit;
}

void FqCoDelFlow::SetStatus(FlowStatus status) {
  NS_LOG_FUNCTION(this);
  m_status = status;
}

FqCoDelFlow::FlowStatus FqCoDelFlow::GetStatus() const {
  NS_LOG_FUNCTION(this);
  return m_status;
}

void FqCoDelFlow::SetIndex(uint32_t index) {
  NS_LOG_FUNCTION(this);
  m_index = index;
}

uint32_t FqCoDelFlow::GetIndex() const { return m_index; }
  ...
  static TypeId tid =
      TypeId("ns3::FqCoDelQueueDisc")
          ...
          .AddAttribute("EnableSetAssociativeHash",
                        ...
                        BooleanValue(false),
                        MakeBooleanAccessor(
                            &FqCoDelQueueDisc::m_enableSetAssociativeHash),
                        MakeBooleanChecker())
          .AddAttribute(
              "SetWays",
...
FqCoDelQueueDisc::~FqCoDelQueueDisc() { NS_LOG_FUNCTION(this); }

void FqCoDelQueueDisc::SetQuantum(uint32_t quantum) {
  NS_LOG_FUNCTION(this << quantum);
  m_quantum = quantum;
}

uint32_t FqCoDelQueueDisc::GetQuantum() const { return m_quantum; }

uint32_t FqCoDelQueueDisc::SetAssociativeHash(uint32_t flowHash) {
...
bool FqCoDelQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item) {
...
Ptr<QueueDiscItem> FqCoDelQueueDisc::DoDequeue() {
  NS_LOG_FUNCTION(this);
...
bool FqCoDelQueueDisc::CheckConfig() {
  NS_LOG_FUNCTION(this);
...
void FqCoDelQueueDisc::InitializeParams() {
  NS_LOG_FUNCTION(this);
...
uint32_t FqCoDelQueueDisc::FqCoDelDrop() {
  NS_LOG_FUNCTION(this);
