...
TypeId CoDelQueueDisc::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::CoDelQueueDisc")
          ...
          .AddAttribute(
              "Interval", "The CoDel algorithm interval", StringValue("100ms"),
              MakeTimeAccessor(&CoDelQueueDisc::m_interval), MakeTimeChecker())
          .AddAttribute("Target", "The CoDel algorithm target queue delay",
                        StringValue("5ms"),
...
CoDelQueueDisc::CoDelQueueDisc()
    : QueueDisc(QueueDiscSizePolicy::SINGLE_INTERNAL_QUEUE), m_count(0),
      m_lastCount(0), m_dropping(false),
      m_recInvSqrt(~0U >> REC_INV_SQRT_SHIFT), m_firstAboveTime(0),
      m_dropNext(0) {
  NS_LOG_FUNCTION(this);
}

CoDelQueueDisc::~CoDelQueueDisc() { NS_LOG_FUNCTION(this); }

uint16_t CoDelQueueDisc::NewtonStep(uint16_t recInvSqrt, uint32_t count) {
...
uint32_t CoDelQueueDisc::ControlLaw(uint32_t t, uint32_t interval,
                                    uint32_t recInvSqrt) {
  NS_LOG_FUNCTION_NOARGS();
  return t + ReciprocalDivide(interval, recInvSqrt << REC_INV_SQRT_SHIFT);
}

bool CoDelQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item) {
...
bool CoDelQueueDisc::OkToDrop(Ptr<QueueDiscItem> item, uint32_t now) {
  NS_LOG_FUNCTION(this);
  bool okToDrop;
...
Ptr<QueueDiscItem> CoDelQueueDisc::DoDequeue() {
  NS_LOG_FUNCTION(this);
      ...
      while (m_dropping && CoDelTimeAfterEq(now, m_dropNext)) {
        ...
        if (!OkToDrop(item, now)) {
          ...
          NS_LOG_LOGIC("Running ControlLaw for input m_dropNext: "
                       << (double)m_dropNext / 1000000);
          m_dropNext =
              ControlLaw(m_dropNext, Time2CoDel(m_interval), m_recInvSqrt);
          NS_LOG_LOGIC("Scheduled next drop at "
                       << (double)m_dropNext / 1000000);
        }
      }
...
Time CoDelQueueDisc::GetTarget() { return m_target; }

Time CoDelQueueDisc::GetInterval() { return m_interval; }

uint32_t CoDelQueueDisc::GetDropNext() { return m_dropNext; }

bool CoDelQueueDisc::CoDelTimeAfter(uint32_t a, uint32_t b) {
  return ((int64_t)(a) - (int64_t)(b) > 0);
}

bool CoDelQueueDisc::CoDelTimeAfterEq(uint32_t a, uint32_t b) {
  return ((int64_t)(a) - (int64_t)(b) >= 0);
}

bool CoDelQueueDisc::CoDelTimeBefore(uint32_t a, uint32_t b) {
  return ((int64_t)(a) - (int64_t)(b) < 0);
}

bool CoDelQueueDisc::CoDelTimeBeforeEq(uint32_t a, uint32_t b) {
  return ((int64_t)(a) - (int64_t)(b) <= 0);
}

uint32_t CoDelQueueDisc::Time2CoDel(Time t) {
  return static_cast<uint32_t>(t.GetNanoSeconds() >> CODEL_SHIFT);
}

bool CoDelQueueDisc::CheckConfig() {
  NS_LOG_FUNCTION(this);
...
void CoDelQueueDisc::InitializeParams() { NS_LOG_FUNCTION(this); }
