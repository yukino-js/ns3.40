...
void QueueDiscFactory::AddInternalQueue(ObjectFactory factory) {
  m_internalQueuesFactory.push_back(factory);
}

void QueueDiscFactory::AddPacketFilter(ObjectFactory factory) {
  m_packetFiltersFactory.push_back(factory);
}

uint16_t QueueDiscFactory::AddQueueDiscClass(ObjectFactory factory) {
  m_queueDiscClassesFactory.push_back(factory);
  return static_cast<uint16_t>(m_queueDiscClassesFactory.size() - 1);
}

void QueueDiscFactory::SetChildQueueDisc(uint16_t classId, uint16_t handle) {
...
Ptr<QueueDisc> QueueDiscFactory::CreateQueueDisc(
  ...
  for (std::size_t i = 0; i < m_queueDiscClassesFactory.size(); i++) {
    ...
    NS_ABORT_MSG_IF(handle >= queueDiscs.size() || !queueDiscs[handle],
                    "A queue disc with handle " << handle
                                                << " has not been created yet");

    m_queueDiscClassesFactory[i].Set("QueueDisc",
                                     PointerValue(queueDiscs[handle]));
    qd->AddQueueDiscClass(
        m_queueDiscClassesFactory[i].Create<QueueDiscClass>());
  }
...
TrafficControlHelper TrafficControlHelper::Default(std::size_t nTxQueues) {
  ...
  if (nTxQueues == 1) {
    ...
    ClassIdList cls =
        helper.AddQueueDiscClasses(handle, nTxQueues, "ns3::QueueDiscClass");
    helper.AddChildQueueDiscs(handle, cls, "ns3::FqCoDelQueueDisc");
  }
  return helper;
}

uint16_t TrafficControlHelper::DoSetRootQueueDisc(ObjectFactory factory) {
  NS_ABORT_MSG_UNLESS(
      m_queueDiscFactory.empty(),
...
void TrafficControlHelper::DoAddInternalQueues(uint16_t handle, uint16_t count,
...
void TrafficControlHelper::DoAddPacketFilter(uint16_t handle,
                                             ObjectFactory factory) {
  NS_ABORT_MSG_IF(handle >= m_queueDiscFactory.size(),
                  "A queue disc with handle " << handle << " does not exist");
...
TrafficControlHelper::DoAddQueueDiscClasses(uint16_t handle, uint16_t count,
                                            ObjectFactory factory) {
  NS_ABORT_MSG_IF(handle >= m_queueDiscFactory.size(),
                  "A queue disc with handle " << handle << " does not exist");
...
uint16_t TrafficControlHelper::DoAddChildQueueDisc(uint16_t handle,
                                                   uint16_t classId,
                                                   ObjectFactory factory) {
  NS_ABORT_MSG_IF(handle >= m_queueDiscFactory.size(),
                  "A queue disc with handle " << handle << " does not exist");
...
TrafficControlHelper::HandleList TrafficControlHelper::DoAddChildQueueDiscs(
    uint16_t handle, const TrafficControlHelper::ClassIdList &classes,
    ObjectFactory factory) {
  HandleList list;
...
QueueDiscContainer TrafficControlHelper::Install(NetDeviceContainer c) {
  QueueDiscContainer container;
...
void TrafficControlHelper::Uninstall(Ptr<NetDevice> d) {
  Ptr<TrafficControlLayer> tc = d->GetNode()->GetObject<TrafficControlLayer>();
  NS_ASSERT(tc);
...
void TrafficControlHelper::Uninstall(NetDeviceContainer c) {
  for (auto i = c.Begin(); i != c.End(); ++i) {
    Uninstall(*i);
  }
}
