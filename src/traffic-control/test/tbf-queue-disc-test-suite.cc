...
void TbfQueueDiscTestItem::AddHeader() {}
...
void TbfQueueDiscTestCase::RunTbfTest(QueueSizeUnit mode) {
  ...
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("Burst", UintegerValue(burst)), true,
  ...
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("Rate", DataRateValue(rate)), true,
      "Verify that we can actually set the attribute Rate");
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("PeakRate", DataRateValue(peakRate)), true,
  ...
  NS_TEST_ASSERT_MSG_EQ(
      queue->GetFirstBucketTokens(), burst - (1 * pktSize),
  ...
  NS_TEST_ASSERT_MSG_EQ(
      queue->GetFirstBucketTokens(), burst - (2 * pktSize),
  ...
  for (uint32_t i = 1; i <= nPkt; i++) {
    Simulator::Schedule(Time(Seconds((i + 1) * delay)),
                        &TbfQueueDiscTestCase::Enqueue, this, queue, dest,
                        pktSize);
  }
  delay = 0.1;
  for (uint32_t i = 1; i <= nPkt; i++) {
    Simulator::Schedule(Time(Seconds((i + 1) * delay)),
                        ...
                        true, "No packet should be blocked");
  }
  ...
  for (uint32_t i = 1; i <= nPkt; i++) {
    if (i == 10) {
      Simulator::Schedule(Time(Seconds((i + 1) * delay)),
                          &TbfQueueDiscTestCase::DequeueAndCheck, this, queue,
                          false, "10th packet should be blocked");
    } else {
      Simulator::Schedule(Time(Seconds((i + 1) * delay)),
                          &TbfQueueDiscTestCase::DequeueAndCheck, this, queue,
                          true, "This packet should not be blocked");
    }
  }
  ...
  for (uint32_t i = 1; i <= nPkt; i++) {
    if (i % 2 == 1) {
      Simulator::Schedule(Seconds(nextDelay),
                          &TbfQueueDiscTestCase::DequeueAndCheck, this, queue,
                          true, "1st packet should not be blocked");
      nextDelay += 0.04;
    } else {
      Simulator::Schedule(Seconds(nextDelay),
                          &TbfQueueDiscTestCase::DequeueAndCheck, this, queue,
...
void TbfQueueDiscTestCase::Enqueue(Ptr<TbfQueueDisc> queue, Address dest,
                                   uint32_t size) {
  queue->Enqueue(Create<TbfQueueDiscTestItem>(Create<Packet>(size), dest));
}

void TbfQueueDiscTestCase::DequeueAndCheck(Ptr<TbfQueueDisc> queue, bool flag,
                                           std::string printStatement) {
  Ptr<QueueDiscItem> item = queue->Dequeue();
  NS_TEST_EXPECT_MSG_EQ((item != nullptr), flag, printStatement);
}

void TbfQueueDiscTestCase::DoRun() {
