...
void RedQueueDiscTestItem::AddHeader() {}
...
void RedQueueDiscTestCase::RunRedTest(QueueSizeUnit mode) {
  ...
  NS_TEST_ASSERT_MSG_EQ(queue->SetAttributeFailSafe("QW", DoubleValue(0.020)),
                        ...
                        "Verify that we can actually set the attribute QW");
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("LInterm", DoubleValue(5)), true,
  ...
  NS_TEST_ASSERT_MSG_EQ(queue->SetAttributeFailSafe("QW", DoubleValue(0.020)),
                        ...
                        "Verify that we can actually set the attribute QW");
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("Gentle", BooleanValue(false)), true,
  ...
  NS_TEST_ASSERT_MSG_EQ(queue->SetAttributeFailSafe("QW", DoubleValue(0.020)),
                        ...
                        "Verify that we can actually set the attribute QW");
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("Wait", BooleanValue(false)), true,
  ...
  NS_TEST_ASSERT_MSG_EQ(
      ...
      "Verify that we can actually set the attribute LInterm");
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("Gentle", BooleanValue(true)), true,
      "Verify that we can actually set the attribute Gentle");
  NS_TEST_ASSERT_MSG_EQ(
  ...
  NS_TEST_ASSERT_MSG_EQ(
      ...
      "Verify that we can actually set the attribute Gentle");
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("FengAdaptive", BooleanValue(true)), true,
  ...
  NS_TEST_ASSERT_MSG_EQ(
      ...
      "Verify that we can actually set the attribute Gentle");
  NS_TEST_ASSERT_MSG_EQ(
      queue->SetAttributeFailSafe("NLRED", BooleanValue(true)), true,
  ...
  NS_TEST_ASSERT_MSG_LT(
      drop.test13, drop.test11,
      "Test 13 should have less drops due to probability mark than test 11");
}

void RedQueueDiscTestCase::Enqueue(Ptr<RedQueueDisc> queue, uint32_t size,
                                   uint32_t nPkt, bool ecnCapable) {
  Address dest;
  for (uint32_t i = 0; i < nPkt; i++) {
    queue->Enqueue(
        Create<RedQueueDiscTestItem>(Create<Packet>(size), dest, ecnCapable));
  }
}

void RedQueueDiscTestCase::DoRun() {
