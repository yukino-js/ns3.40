...
void CheckQueueDiscSize(Ptr<QueueDisc> queue) {
  ...
  std::ofstream fPlotQueueDiscAvg(filePlotQueueDiscAvg.str(),
                                  std::ios::out | std::ios::app);
  fPlotQueueDiscAvg << Simulator::Now().GetSeconds() << " "
                    << avgQueueDiscSize / checkTimes << std::endl;
...
void BuildAppsTest(uint32_t test) {
  ...
  if (test == 6 || test == 7 || test == 8 || test == 9 || test == 10 ||
      test == 12) {
...
int main(int argc, char *argv[]) {
  ...
  tchRed.SetRootQueueDisc("ns3::RedQueueDisc", "LinkBandwidth",
                          StringValue(aredLinkDataRate), "LinkDelay",
  ...
  if (aredTest == 1 || aredTest == 2 || aredTest == 3 || aredTest == 4) {
    ...
    devn3n5 = p2p.Install(n3n5);
    tchPfifo.Install(devn3n5);
  } else if (aredTest == 6 || aredTest == 7 || aredTest == 8 || aredTest == 9 ||
             aredTest == 10 || aredTest == 11 || aredTest == 12) {
    p2p.SetQueue("ns3::DropTailQueue");
