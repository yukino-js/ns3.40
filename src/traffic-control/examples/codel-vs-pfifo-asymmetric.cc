...
static void TraceSojourn(std::string sojournTrFileName) {
  ...
  if (sojournTrFileName.empty()) {
    ...
    Config::ConnectWithoutContext(
        "/NodeList/2/$ns3::TrafficControlLayer/RootQueueDiscList/0/"
...
static void TraceQueueLength(std::string queueLengthTrFileName) {
  ...
  if (queueLengthTrFileName.empty()) {
    ...
    Ptr<OutputStreamWrapper> stream =
        ascii.CreateFileStream(queueLengthTrFileName);
    Config::ConnectWithoutContext(
        "/NodeList/2/$ns3::TrafficControlLayer/RootQueueDiscList/0/"
        "BytesInQueue",
...
static void TraceEveryDrop(std::string everyDropTrFileName) {
  ...
  if (everyDropTrFileName.empty()) {
    ...
    Ptr<OutputStreamWrapper> stream =
        ascii.CreateFileStream(everyDropTrFileName);
    Config::ConnectWithoutContext(
...
static void TraceDroppingState(std::string dropStateTrFileName) {
  ...
  if (dropStateTrFileName.empty()) {
    ...
    Ptr<OutputStreamWrapper> stream =
        ascii.CreateFileStream(dropStateTrFileName);
    Config::ConnectWithoutContext(
        "/NodeList/2/$ns3::TrafficControlLayer/RootQueueDiscList/0/"
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("serverCmtsDelay", "Link delay between server and CMTS",
               serverCmtsDelay);
  cmd.AddValue("cmtsRouterDelay", "Link delay between CMTS and rounter",
               cmtsRouterDelay);
  cmd.AddValue("routerHostDelay", "Link delay between router and host",
               routerHostDelay);
  cmd.AddValue("serverLanDataRate", "Server LAN net device data rate",
               serverLanDataRate);
  cmd.AddValue("cmtsLanDataRate", "CMTS LAN net device data rate",
               cmtsLanDataRate);
  cmd.AddValue("cmtsWanDataRate", "CMTS WAN net device data rate",
               cmtsWanDataRate);
  cmd.AddValue("routerWanDataRate", "Router WAN net device data rate",
               routerWanDataRate);
  cmd.AddValue("routerLanDataRate", "Router LAN net device data rate",
               routerLanDataRate);
  cmd.AddValue("hostLanDataRate", "Host LAN net device data rate",
               hostLanDataRate);
  ...
  cmd.AddValue("numOfUpLoadOnOffFlows", "Number of upload OnOff flows",
  ...
  std::string cwndTrFileName =
      fileNamePrefix + "-" + routerWanQueueDiscType + "-cwnd" + ".tr";
  std::string attributeFileName =
      fileNamePrefix + "-" + routerWanQueueDiscType + ".attr";
  ...
  AddressValue remoteAddress1(
      InetSocketAddress(routerHostInterface.GetAddress(1), port1));
  ...
  AddressValue remoteAddress2(
      InetSocketAddress(serverCmtsInterface.GetAddress(0), port2));
