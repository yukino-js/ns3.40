...
int main(int argc, char **argv) {
  ...
  node = CreateObject<Node>();
  batteryModel = DynamicCast<GenericBatteryModel>(
      batteryHelper.Install(node, CSB_GP1272_LEADACID));
  ...
  node = CreateObject<Node>();
  batteryModel = DynamicCast<GenericBatteryModel>(
      batteryHelper.Install(node, PANASONIC_CGR18650DA_LION));
  ...
  node = CreateObject<Node>();
  batteryModel = DynamicCast<GenericBatteryModel>(
      batteryHelper.Install(node, RSPRO_LGP12100_LEADACID));
  ...
  node = CreateObject<Node>();
  batteryModel = DynamicCast<GenericBatteryModel>(
      batteryHelper.Install(node, PANASONIC_N700AAC_NICD));
