...
TypeId LiIonEnergySource::GetTypeId() {
  static TypeId tid =
...
LiIonEnergySource::LiIonEnergySource()
    : m_drainedCapacity(0.0), m_lastUpdateTime(Seconds(0.0)) {
  NS_LOG_FUNCTION(this);
}

LiIonEnergySource::~LiIonEnergySource() { NS_LOG_FUNCTION(this); }

void LiIonEnergySource::SetInitialEnergy(double initialEnergyJ) {
...
double LiIonEnergySource::GetInitialEnergy() const {
  NS_LOG_FUNCTION(this);
  return m_initialEnergyJ;
}

void LiIonEnergySource::SetInitialSupplyVoltage(double supplyVoltageV) {
  NS_LOG_FUNCTION(this << supplyVoltageV);
  m_eFull = supplyVoltageV;
  m_supplyVoltageV = supplyVoltageV;
}

double LiIonEnergySource::GetSupplyVoltage() const {
  NS_LOG_FUNCTION(this);
  return m_supplyVoltageV;
}

void LiIonEnergySource::SetEnergyUpdateInterval(Time interval) {
  NS_LOG_FUNCTION(this << interval);
  m_energyUpdateInterval = interval;
}

Time LiIonEnergySource::GetEnergyUpdateInterval() const {
  NS_LOG_FUNCTION(this);
...
void LiIonEnergySource::DecreaseRemainingEnergy(double energyJ) {
...
void LiIonEnergySource::IncreaseRemainingEnergy(double energyJ) {
  NS_LOG_FUNCTION(this << energyJ);
  NS_ASSERT(energyJ >= 0);
  m_remainingEnergyJ += energyJ;
}

void LiIonEnergySource::UpdateEnergySource() {
  NS_LOG_FUNCTION(this);
