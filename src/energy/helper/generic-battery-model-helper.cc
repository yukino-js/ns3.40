...
void GenericBatteryModelHelper::Set(std::string name, const AttributeValue &v) {
  m_batteryModel.Set(name, v);
}

Ptr<EnergySource> GenericBatteryModelHelper::DoInstall(Ptr<Node> node) const {
...
GenericBatteryModelHelper::Install(NodeContainer c) const {
  Ptr<EnergySourceContainer> batteryContainer =
...
Ptr<EnergySource> GenericBatteryModelHelper::Install(Ptr<Node> node,
                                                     BatteryModel bm) const {
  NS_ASSERT(node != nullptr);
  Ptr<EnergySource> energySource = m_batteryModel.Create<EnergySource>();
  NS_ASSERT(energySource != nullptr);

  energySource->SetAttribute("FullVoltage",
                             DoubleValue(g_batteryPreset[bm].vFull));
  energySource->SetAttribute("MaxCapacity",
                             DoubleValue(g_batteryPreset[bm].qMax));

  energySource->SetAttribute("NominalVoltage",
                             DoubleValue(g_batteryPreset[bm].vNom));
  energySource->SetAttribute("NominalCapacity",
                             DoubleValue(g_batteryPreset[bm].qNom));

  energySource->SetAttribute("ExponentialVoltage",
                             DoubleValue(g_batteryPreset[bm].vExp));
  energySource->SetAttribute("ExponentialCapacity",
                             DoubleValue(g_batteryPreset[bm].qExp));

  energySource->SetAttribute(
      "InternalResistance",
  ...
  energySource->SetAttribute("CutoffVoltage",
                             DoubleValue(g_batteryPreset[bm].cuttoffVoltage));

  energySource->SetAttribute("BatteryType",
                             EnumValue(g_batteryPreset[bm].batteryType));
...
void GenericBatteryModelHelper::SetCellPack(Ptr<EnergySource> energySource,
                                            uint8_t series,
  ...
  energySource->SetAttribute("NominalVoltage",
                             DoubleValue(vNom.Get() * series));
  energySource->SetAttribute("NominalCapacity",
                             DoubleValue(qNom.Get() * parallel));

  energySource->SetAttribute("ExponentialVoltage",
                             DoubleValue(vExp.Get() * series));
  energySource->SetAttribute("ExponentialCapacity",
                             DoubleValue(qExp.Get() * parallel));

  energySource->SetAttribute("InternalResistance",
                             DoubleValue(r.Get() * (series / parallel)));
}

void GenericBatteryModelHelper::SetCellPack(
    EnergySourceContainer energySourceContainer, uint8_t series,
    uint8_t parallel) const {
  NS_ASSERT_MSG(energySourceContainer.GetN() > 0,
                "This energy container is empty");

  for (auto i = energySourceContainer.Begin(); i != energySourceContainer.End();
       i++) {
