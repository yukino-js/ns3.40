...
TypeId PyVizPacketTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t PyVizPacketTag::GetSerializedSize() const { return 4; }

void PyVizPacketTag::Serialize(TagBuffer buf) const {
  buf.WriteU32(m_packetId);
}

void PyVizPacketTag::Deserialize(TagBuffer buf) { m_packetId = buf.ReadU32(); }

void PyVizPacketTag::Print(std::ostream &os) const {
...
void PyViz::SetPacketCaptureOptions(uint32_t nodeId,
                                    PacketCaptureOptions options) {
  NS_LOG_DEBUG("  SetPacketCaptureOptions "
               << nodeId << " PacketCaptureOptions (headers size = "
               << options.headers.size() << " mode = " << options.mode
               << " numLastPackets = " << options.numLastPackets << ")");
  m_packetCaptureOptions[nodeId] = options;
}

void PyViz::RegisterDropTracePath(const std::string &tracePath) {
...
void PyViz::DoPause(const std::string &message) {
  m_pauseMessages.push_back(message);
  m_stop = true;
  NS_LOG_LOGIC(Simulator::Now().As(Time::S)
               << ": Have " << g_visualizer->m_pauseMessages.size()
               << " pause messages");
}

void PyViz::Pause(const std::string &message) {
  NS_ASSERT(g_visualizer);
  g_visualizer->DoPause(message);
}

std::vector<std::string> PyViz::GetPauseMessages() const {
  NS_LOG_LOGIC(Simulator::Now().As(Time::S)
...
void PyViz::SimulatorRunUntil(Time time) {
...
bool PyViz::TransmissionSampleKey::operator<(
    const PyViz::TransmissionSampleKey &other) const {
...
bool PyViz::TransmissionSampleKey::operator==(
  ...
  bool retval = (transmitter == other.transmitter) &&
                (receiver == other.receiver) && (channel == other.channel);
  return retval;
}

PyViz::NetDeviceStatistics &PyViz::FindNetDeviceStatistics(int node,
                                                           int interface) {
...
bool PyViz::GetPacketCaptureOptions(
    uint32_t nodeId, const PacketCaptureOptions **outOptions) const {
...
bool PyViz::FilterPacket(Ptr<const Packet> packet,
...
void PyViz::TraceDevQueueDrop(std::string context, Ptr<const Packet> packet) {
...
void PyViz::TraceIpv4Drop(std::string context, const ns3::Ipv4Header &hdr,
...
void PyViz::TraceNetDevTxCommon(const std::string &context,
  ...
  if (GetPacketCaptureOptions(nodeIndex, &captureOptions) &&
    ...
    while (last.lastTransmittedPackets.size() >
           captureOptions->numLastPackets) {
...
void PyViz::TraceNetDevTxCsma(std::string context, Ptr<const Packet> packet) {
  EthernetHeader ethernetHeader;
  NS_ABORT_IF(packet->PeekHeader(ethernetHeader) == 0);
  TraceNetDevTxCommon(context, packet, ethernetHeader.GetDestination());
}

void PyViz::TraceNetDevTxPointToPoint(std::string context,
                                      Ptr<const Packet> packet) {
...
void PyViz::TraceNetDevRxCommon(const std::string &context,
  ...
  if (g_log.IsEnabled(ns3::LOG_DEBUG)) {
    for (auto iter = m_transmissionSamples.begin();
      ...
      NS_LOG_DEBUG(iter->first.transmitter
                   ...
                   << iter->first.channel << " => " << iter->second.bytes
                   << " (@ " << &iter->second << ")");
    }
  }
  ...
  if (iter == m_transmissionSamples.end()) {
    ...
    NS_LOG_DEBUG("RX: from "
                 << key.transmitter << "/" << key.transmitter->GetId() << " to "
...
void PyViz::TraceNetDevRxCsma(std::string context, Ptr<const Packet> packet) {
  EthernetHeader ethernetHeader;
  NS_ABORT_IF(packet->PeekHeader(ethernetHeader) == 0);
  TraceNetDevRxCommon(context, packet, ethernetHeader.GetSource());
}

void PyViz::TraceNetDevRxPointToPoint(std::string context,
                                      Ptr<const Packet> packet) {
  TraceNetDevRxCommon(context, packet, Mac48Address());
}

void PyViz::TraceNetDevPromiscRxCsma(std::string context,
                                     Ptr<const Packet> packet) {
  EthernetHeader ethernetHeader;
  NS_ABORT_IF(packet->PeekHeader(ethernetHeader) == 0);
...
void PyViz::TraceNetDevTxWimax(std::string context, Ptr<const Packet> packet,
                               const Mac48Address &destination) {
  NS_LOG_FUNCTION(context);
  TraceNetDevTxCommon(context, packet, destination);
}

void PyViz::TraceNetDevRxWimax(std::string context, Ptr<const Packet> packet,
                               const Mac48Address &source) {
  NS_LOG_FUNCTION(context);
  TraceNetDevRxCommon(context, packet, source);
}

void PyViz::TraceNetDevTxLte(std::string context, Ptr<const Packet> packet,
                             const Mac48Address &destination) {
  NS_LOG_FUNCTION(context);
  TraceNetDevTxCommon(context, packet, destination);
}

void PyViz::TraceNetDevRxLte(std::string context, Ptr<const Packet> packet,
                             const Mac48Address &source) {
  NS_LOG_FUNCTION(context);
  TraceNetDevRxCommon(context, packet, source);
}
...
PyViz::TransmissionSampleList PyViz::GetTransmissionSamples() const {
  ...
  for (auto iter = m_transmissionSamples.begin();
    ...
    NS_LOG_DEBUG("from " << sample.transmitter->GetId() << " to "
                         << sample.receiver->GetId() << ": " << sample.bytes
                         << " bytes.");
...
PyViz::PacketDropSampleList PyViz::GetPacketDropSamples() const {
...
void PyViz::SetNodesOfInterest(std::set<uint32_t> nodes) {
  m_nodesOfInterest = nodes;
}

std::vector<PyViz::NodeStatistics> PyViz::GetNodesStatistics() const {
  std::vector<PyViz::NodeStatistics> retval;
  for (auto iter = m_nodesStatistics.begin(); iter != m_nodesStatistics.end();
       iter++) {
...
PyViz::LastPacketsSample PyViz::GetLastPackets(uint32_t nodeId) const {
...
void PyViz::LineClipping(double boundsX1, double boundsY1, double boundsX2,
