...
TypeId VisualSimulatorImpl::GetTypeId() {
  static TypeId tid =
...
VisualSimulatorImpl::VisualSimulatorImpl() { PacketMetadata::Enable(); }

VisualSimulatorImpl::~VisualSimulatorImpl() {}

void VisualSimulatorImpl::DoDispose() {
  if (m_simulator) {
    m_simulator->Dispose();
    m_simulator = nullptr;
  }
...
void VisualSimulatorImpl::Destroy() { m_simulator->Destroy(); }

void VisualSimulatorImpl::SetScheduler(ObjectFactory schedulerFactory) {
  m_simulator->SetScheduler(schedulerFactory);
}

// System ID for non-distributed simulation is always zero
uint32_t VisualSimulatorImpl::GetSystemId() const {
  return m_simulator->GetSystemId();
}

bool VisualSimulatorImpl::IsFinished() const {
  return m_simulator->IsFinished();
}

void VisualSimulatorImpl::Run() {
  if (!Py_IsInitialized()) {
    Py_Initialize();
...
void VisualSimulatorImpl::Stop() { m_simulator->Stop(); }

void VisualSimulatorImpl::Stop(const Time &delay) { m_simulator->Stop(delay); }

//
// Schedule an event for a _relative_ time in the future.
//
EventId VisualSimulatorImpl::Schedule(const Time &delay, EventImpl *event) {
  return m_simulator->Schedule(delay, event);
}

void VisualSimulatorImpl::ScheduleWithContext(uint32_t context,
                                              const Time &delay,
                                              EventImpl *event) {
  m_simulator->ScheduleWithContext(context, delay, event);
}

EventId VisualSimulatorImpl::ScheduleNow(EventImpl *event) {
  return m_simulator->ScheduleNow(event);
}

EventId VisualSimulatorImpl::ScheduleDestroy(EventImpl *event) {
  return m_simulator->ScheduleDestroy(event);
}

Time VisualSimulatorImpl::Now() const { return m_simulator->Now(); }

Time VisualSimulatorImpl::GetDelayLeft(const EventId &id) const {
  return m_simulator->GetDelayLeft(id);
}

void VisualSimulatorImpl::Remove(const EventId &id) { m_simulator->Remove(id); }

void VisualSimulatorImpl::Cancel(const EventId &id) { m_simulator->Cancel(id); }

bool VisualSimulatorImpl::IsExpired(const EventId &id) const {
  return m_simulator->IsExpired(id);
}

Time VisualSimulatorImpl::GetMaximumSimulationTime() const {
  return m_simulator->GetMaximumSimulationTime();
}

uint32_t VisualSimulatorImpl::GetContext() const {
  return m_simulator->GetContext();
}

uint64_t VisualSimulatorImpl::GetEventCount() const {
  return m_simulator->GetEventCount();
}

void VisualSimulatorImpl::RunRealSimulator() { m_simulator->Run(); }
