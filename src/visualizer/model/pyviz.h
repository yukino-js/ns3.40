...
public:
  ...
  typedef std::vector<PacketDropSample>
      PacketDropSampleList; ///< PacketDropSampleList typedef
  /**
  ...
  struct LastPacketsSample {
    ...
    std::vector<TxPacketSample>
        lastTransmittedPackets;                   ///< last transmitted packets
  ...
  enum PacketCaptureMode {
    ...
    PACKET_CAPTURE_FILTER_HEADERS_OR,  // packet capture if any of the indicated
                                       // headers is present
    PACKET_CAPTURE_FILTER_HEADERS_AND, // packet capture if all of the indicated
                                       // headers are present
  };
...
private:
  ...
  typedef std::pair<Ptr<Channel>, uint32_t>
      TxRecordKey; ///< TxRecordKey typedef
  ...
  std::map<uint32_t, PacketCaptureOptions>
      m_packetCaptureOptions;                       ///< packet capture options
