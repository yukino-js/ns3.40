...
void DsrAckReqHeaderTest::DoRun() {
...
void DsrCacheEntryTest::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(entry.GetDestination(), Ipv4Address("0.0.0.1"),
                        "trivial");
  ...
  NS_TEST_EXPECT_MSG_EQ(entry.GetDestination(), Ipv4Address("1.1.1.1"),
                        "trivial");
  ...
  NS_TEST_EXPECT_MSG_EQ(rcache->DeleteRoute(Ipv4Address("2.2.2.2")), false,
                        "trivial");

  NS_TEST_EXPECT_MSG_EQ(rcache->DeleteRoute(Ipv4Address("1.1.1.1")), true,
                        "trivial");
  NS_TEST_EXPECT_MSG_EQ(rcache->DeleteRoute(Ipv4Address("1.1.1.1")), false,
                        "trivial");
}
...
void DsrSendBuffTest::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(q.Dequeue(Ipv4Address("0.0.0.3"), e3), false,
                        "trivial");
  ...
  Simulator::Schedule(q.GetSendBufferTimeout() + Seconds(1),
                      &DsrSendBuffTest::CheckTimeout, this);

  Simulator::Run();
  Simulator::Destroy();
}

void DsrSendBuffTest::CheckSizeLimit() {
...
void DsrSendBuffTest::CheckTimeout() {
