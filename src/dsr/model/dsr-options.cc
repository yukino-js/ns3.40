  ...
  if (dupRequest) {
    ...
    if (targetAddress == ipv4Address) {
      ...
      if (nodeList.size() == 1) {
        ...
        /// way, check later
        m_finalRoute.push_back(
            source); // push back the request originator's address
        ...
        for (auto i = changeRoute.begin(); i != changeRoute.end(); ++i) {
          m_finalRoute.push_back(
              *i); // Get the full route from source to destination
        }
        PrintVector(m_finalRoute);
        nextHop =
            ReverseSearchNextHop(ipv4Address, m_finalRoute); // get the next hop
      }
      ...
      if (ReverseRoutes(m_finalRoute)) {
        ...
        if (addRoute) {
          ...
          Ipv4Address nextHop = SearchNextHop(
              ipv4Address, m_finalRoute); // Get the next hop address
          NS_LOG_DEBUG("The nextHop address " << nextHop);
...
uint8_t DsrOptionRrep::Process(Ptr<Packet> packet, Ptr<Packet> dsrP,
  ...
  rrep.SetNumberAddress(
      numberAddress); // Set the number of ip address in the header to reserver
  ...
  if (targetAddress == ipv4Address) {
    ...
    if (addRoute) {
      ...
      sourceRoute.SetSalvage(0);
      Ipv4Address nextHop =
          SearchNextHop(ipv4Address, nodeList); // Get the next hop address
      NS_LOG_DEBUG("The nextHop address " << nextHop);
      if (nextHop == "0.0.0.0") {
        dsr->PacketNewRoute(dsrP, ipv4Address, dst, protocol);
        return 0;
    ...
    uint8_t length =
        rrep.GetLength() - 2; // The get length - 2 is to get aligned for the
...
uint8_t DsrOptionSR::Process(Ptr<Packet> packet, Ptr<Packet> dsrP,
  ...
  if (isPromisc) {
    ...
    if (optionType == 160) {
      ...
      if (!nodeList.empty()) {
        if (segsLeft >
            numberAddress) // The segmentsLeft field should not be larger than
