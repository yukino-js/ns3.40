...
TypeId DsrOptionHeader::GetInstanceTypeId() const { return GetTypeId(); }

DsrOptionHeader::DsrOptionHeader() : m_type(0), m_length(0) {}

DsrOptionHeader::~DsrOptionHeader() {}

void DsrOptionHeader::SetType(uint8_t type) { m_type = type; }

uint8_t DsrOptionHeader::GetType() const { return m_type; }

void DsrOptionHeader::SetLength(uint8_t length) { m_length = length; }

uint8_t DsrOptionHeader::GetLength() const { return m_length; }
...
uint32_t DsrOptionHeader::GetSerializedSize() const { return m_length + 2; }
...
TypeId DsrOptionPad1Header::GetInstanceTypeId() const { return GetTypeId(); }

DsrOptionPad1Header::DsrOptionPad1Header() { SetType(224); }
...
uint32_t DsrOptionPad1Header::GetSerializedSize() const { return 1; }
...
TypeId DsrOptionPadnHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
TypeId DsrOptionRreqHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
Ipv4Address DsrOptionRreqHeader::GetTarget() { return m_target; }

void DsrOptionRreqHeader::SetTarget(Ipv4Address target) { m_target = target; }
...
void DsrOptionRreqHeader::SetNodesAddress(
...
uint16_t DsrOptionRreqHeader::GetId() const { return m_identification; }
...
TypeId DsrOptionRrepHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
void DsrOptionRrepHeader::SetNodesAddress(
    std::vector<Ipv4Address> ipv4Address) {
  m_ipv4Address = ipv4Address;
  SetLength(2 + m_ipv4Address.size() * 4);
}
...
Ipv4Address DsrOptionRrepHeader::GetTargetAddress(
    std::vector<Ipv4Address> ipv4Address) const {
...
TypeId DsrOptionSRHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
uint8_t DsrOptionSRHeader::GetSegmentsLeft() const { return m_segmentsLeft; }

void DsrOptionSRHeader::SetSalvage(uint8_t salvage) { m_salvage = salvage; }

uint8_t DsrOptionSRHeader::GetSalvage() const { return m_salvage; }
...
TypeId DsrOptionRerrHeader::GetInstanceTypeId() const { return GetTypeId(); }

DsrOptionRerrHeader::DsrOptionRerrHeader()
    : m_errorType(0), m_salvage(0), m_errorLength(4) {
  SetType(3);
  SetLength(18);
}
...
uint8_t DsrOptionRerrHeader::GetErrorType() const { return m_errorType; }

void DsrOptionRerrHeader::SetSalvage(uint8_t salvage) { m_salvage = salvage; }

uint8_t DsrOptionRerrHeader::GetSalvage() const { return m_salvage; }
...
void DsrOptionRerrHeader::Print(std::ostream &os) const {
  os << "( type = " << (uint32_t)GetType()
     << " length = " << (uint32_t)GetLength()
     << " errorType = " << (uint32_t)m_errorType
     << " salvage = " << (uint32_t)m_salvage
     << " error source = " << m_errorSrcAddress
     << " error dst = " << m_errorDstAddress << " )";
}

uint32_t DsrOptionRerrHeader::GetSerializedSize() const { return 20; }
...
uint8_t DsrOptionRerrUnreachHeader::GetSalvage() const { return m_salvage; }
...
uint32_t DsrOptionRerrUnreachHeader::GetSerializedSize() const { return 20; }
...
TypeId DsrOptionRerrUnsupportedHeader::GetTypeId() {
...
TypeId DsrOptionRerrUnsupportedHeader::GetInstanceTypeId() const {
  return GetTypeId();
}
...
void DsrOptionRerrUnsupportedHeader::SetSalvage(uint8_t salvage) {
  m_salvage = salvage;
}

uint8_t DsrOptionRerrUnsupportedHeader::GetSalvage() const { return m_salvage; }

void DsrOptionRerrUnsupportedHeader::SetErrorSrc(Ipv4Address errorSrcAddress) {
  m_errorSrcAddress = errorSrcAddress;
}

Ipv4Address DsrOptionRerrUnsupportedHeader::GetErrorSrc() const {
  return m_errorSrcAddress;
}

void DsrOptionRerrUnsupportedHeader::SetErrorDst(Ipv4Address errorDstAddress) {
  m_errorDstAddress = errorDstAddress;
}

Ipv4Address DsrOptionRerrUnsupportedHeader::GetErrorDst() const {
  return m_errorDstAddress;
}

void DsrOptionRerrUnsupportedHeader::SetUnsupported(uint16_t unsupported) {
  m_unsupported = unsupported;
}

uint16_t DsrOptionRerrUnsupportedHeader::GetUnsupported() const {
  return m_unsupported;
}

void DsrOptionRerrUnsupportedHeader::Print(std::ostream &os) const {
  os << "( type = " << (uint32_t)GetType()
     << " length = " << (uint32_t)GetLength()
     << " errorType = " << (uint32_t)m_errorType
     << " salvage = " << (uint32_t)m_salvage
...
uint32_t DsrOptionRerrUnsupportedHeader::GetSerializedSize() const {
  return 16;
}

void DsrOptionRerrUnsupportedHeader::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t DsrOptionRerrUnsupportedHeader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
TypeId DsrOptionAckReqHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
uint16_t DsrOptionAckReqHeader::GetAckId() const { return m_identification; }

void DsrOptionAckReqHeader::Print(std::ostream &os) const {
  os << "( type = " << (uint32_t)GetType()
     << " length = " << (uint32_t)GetLength() << " id = " << m_identification
     << " )";
}

uint32_t DsrOptionAckReqHeader::GetSerializedSize() const { return 4; }
...
TypeId DsrOptionAckHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
uint16_t DsrOptionAckHeader::GetAckId() const { return m_identification; }

void DsrOptionAckHeader::SetRealSrc(Ipv4Address realSrcAddress) {
  m_realSrcAddress = realSrcAddress;
}

Ipv4Address DsrOptionAckHeader::GetRealSrc() const { return m_realSrcAddress; }

void DsrOptionAckHeader::SetRealDst(Ipv4Address realDstAddress) {
  m_realDstAddress = realDstAddress;
}

Ipv4Address DsrOptionAckHeader::GetRealDst() const { return m_realDstAddress; }

void DsrOptionAckHeader::Print(std::ostream &os) const {
  os << "( type = " << (uint32_t)GetType()
     << " length = " << (uint32_t)GetLength() << " id = " << m_identification
     << " real src = " << m_realSrcAddress << " real dst = " << m_realDstAddress
     << " )";
}

uint32_t DsrOptionAckHeader::GetSerializedSize() const { return 12; }
