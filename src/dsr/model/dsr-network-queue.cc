...
TypeId DsrNetworkQueue::GetTypeId() {
...
DsrNetworkQueue::DsrNetworkQueue() : m_size(0) { NS_LOG_FUNCTION(this); }
...
void DsrNetworkQueue::SetMaxNetworkSize(uint32_t maxSize) {
  m_maxSize = maxSize;
}

void DsrNetworkQueue::SetMaxNetworkDelay(Time delay) { m_maxDelay = delay; }

uint32_t DsrNetworkQueue::GetMaxNetworkSize() const { return m_maxSize; }

Time DsrNetworkQueue::GetMaxNetworkDelay() const { return m_maxDelay; }

bool DsrNetworkQueue::FindPacketWithNexthop(Ipv4Address nextHop,
                                            DsrNetworkQueueEntry &entry) {
  Cleanup();
...
bool DsrNetworkQueue::Find(Ipv4Address nextHop) {
  Cleanup();
  for (auto i = m_dsrNetworkQueue.begin(); i != m_dsrNetworkQueue.end(); ++i) {
    if (i->GetNextHopAddress() == nextHop) {
      return true;
...
bool DsrNetworkQueue::Enqueue(DsrNetworkQueueEntry &entry) {
...
bool DsrNetworkQueue::Dequeue(DsrNetworkQueueEntry &entry) {
  NS_LOG_FUNCTION(this);
  Cleanup();
...
uint32_t DsrNetworkQueue::GetSize() {
  NS_LOG_FUNCTION(this);
  return m_size;
}

void DsrNetworkQueue::Flush() {
  NS_LOG_FUNCTION(this);
