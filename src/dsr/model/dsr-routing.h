...
protected:
  ...
  TracedCallback<const DsrOptionSRHeader &>
      m_txPacketTrace; ///< packet trace callback
...
private:
  ...
  IpL4Protocol::DownTargetCallback
      m_downTarget; ///< The callback for down layer
  ...
  uint32_t
      m_sendRetries; ///< # of retries have been sent for network acknowledgment

  uint32_t m_passiveRetries; ///< # of retries have been sent for passive
                             ///< acknowledgment

  uint32_t
      m_linkRetries; ///< # of retries have been sent for link acknowledgment

  uint32_t m_rreqRetries; ///< Maximum number of retransmissions of RREQ with
                          ///< TTL = NetDiameter to discover a route

  uint32_t
      m_maxMaintRexmt; ///< Maximum number of retransmissions of data packets
  ...
  uint32_t
      m_maxRreqId; ///< The max number of request ids for a single destination
  ...
  uint32_t
      m_broadcastJitter; ///< The max time to delay route request broadcast.
  ...
  uint32_t
      m_stabilityDecrFactor; ///< The initial decrease factor for link cache

  uint32_t
      m_stabilityIncrFactor; ///< The initial increase factor for link cache
  ...
  std::map<Ipv4Address, Timer>
      m_addressReqTimer; ///< Map IP address + RREQ timer.

  std::map<Ipv4Address, Timer>
      m_nonPropReqTimer; ///< Map IP address + RREQ timer.

  std::map<NetworkKey, Timer>
      m_addressForwardTimer; ///< Map network key + forward timer.

  std::map<NetworkKey, uint32_t>
      m_addressForwardCnt; ///< Map network key + forward counts.

  std::map<PassiveKey, uint32_t>
      m_passiveCnt; ///< Map packet key + passive forward counts.

  std::map<PassiveKey, Timer>
      m_passiveAckTimer; ///< The timer for passive acknowledgment

  std::map<LinkKey, uint32_t>
      m_linkCnt; ///< Map packet key + link forward counts.

  std::map<LinkKey, Timer>
      m_linkAckTimer; ///< The timer for link acknowledgment

  Ptr<dsr::DsrRouteCache>
      m_routeCache; ///< A "drop-front" queue used by the routing layer to cache
                    ///< routes found.

  Ptr<dsr::DsrRreqTable>
      m_rreqTable; ///< A "drop-front" queue used by the routing layer to cache
                   ///< route request sent.

  Ptr<dsr::DsrPassiveBuffer>
      m_passiveBuffer; ///< A "drop-front" queue used by the routing layer
  ...
  std::map<uint32_t, Ptr<dsr::DsrNetworkQueue>>
      m_priorityQueue; ///< priority queues
  ...
  std::vector<Ipv4Address>
      m_clearList; ///< The node that is clear to send packet to

  std::vector<Ipv4Address>
      m_addresses; ///< The bind ipv4 addresses with next hop, src,
                   ///< destination address in sequence

  std::map<std::string, uint32_t>
      m_macToNodeIdMap; ///< The map of mac address to node id

  Ptr<UniformRandomVariable>
      m_uniformRandomVariable; ///< Provides uniform random variables.
};
} /* namespace dsr */
} /* namespace ns3 */
