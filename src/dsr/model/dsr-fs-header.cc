...
TypeId DsrFsHeader::GetInstanceTypeId() const { return GetTypeId(); }

DsrFsHeader::DsrFsHeader()
    : m_nextHeader(0), m_messageType(0), m_payloadLen(0), m_sourceId(0),
      m_destId(0), m_data(0) {}

DsrFsHeader::~DsrFsHeader() {}

void DsrFsHeader::SetNextHeader(uint8_t protocol) { m_nextHeader = protocol; }

uint8_t DsrFsHeader::GetNextHeader() const { return m_nextHeader; }

void DsrFsHeader::SetMessageType(uint8_t messageType) {
  m_messageType = messageType;
}

uint8_t DsrFsHeader::GetMessageType() const { return m_messageType; }

void DsrFsHeader::SetPayloadLength(uint16_t length) { m_payloadLen = length; }

uint16_t DsrFsHeader::GetPayloadLength() const { return m_payloadLen; }

void DsrFsHeader::SetSourceId(uint16_t sourceId) { m_sourceId = sourceId; }

uint16_t DsrFsHeader::GetSourceId() const { return m_sourceId; }

void DsrFsHeader::SetDestId(uint16_t destId) { m_destId = destId; }

uint16_t DsrFsHeader::GetDestId() const { return m_destId; }

void DsrFsHeader::Print(std::ostream &os) const {
  os << "nextHeader: " << (uint32_t)GetNextHeader()
     << " messageType: " << (uint32_t)GetMessageType()
     << " sourceId: " << (uint32_t)GetSourceId()
     << " destinationId: " << (uint32_t)GetDestId()
     << " length: " << (uint32_t)GetPayloadLength();
}

uint32_t DsrFsHeader::GetSerializedSize() const { return 8; }
...
void DsrOptionField::AddDsrOption(const DsrOptionHeader &option) {
...
uint32_t DsrOptionField::GetDsrOptionsOffset() const { return m_optionsOffset; }

Buffer DsrOptionField::GetDsrOptionBuffer() { return m_optionData; }
...
TypeId DsrRoutingHeader::GetInstanceTypeId() const { return GetTypeId(); }
