...
TypeId DsrRouting::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::dsr::DsrRouting")
          ...
          .AddAttribute(
              "StabilityDecrFactor",
              "The stability decrease factor for link cache", UintegerValue(2),
              MakeUintegerAccessor(&DsrRouting::m_stabilityDecrFactor),
              MakeUintegerChecker<uint32_t>())
          .AddAttribute(
              "StabilityIncrFactor",
              "The stability increase factor for link cache", UintegerValue(4),
          ...
          .AddAttribute(
              ...
              MakeTimeAccessor(&DsrRouting::m_initStability), MakeTimeChecker())
          ...
          .AddAttribute(
              "LinkAcknowledgment",
              "Enable Link layer acknowledgment mechanism", BooleanValue(true),
              MakeBooleanAccessor(&DsrRouting::m_linkAck), MakeBooleanChecker())
...
DsrRouting::~DsrRouting() { NS_LOG_FUNCTION_NOARGS(); }
...
void DsrRouting::Start() {
  ...
  for (uint32_t i = 0; i < m_numPriorityQueues; i++) {
    ...
    NS_LOG_INFO("The network queue size " << m_maxNetworkSize
                                          << " and the queue delay "
  ...
  Ptr<dsr::DsrPassiveBuffer> passiveBuffer =
      CreateObject<dsr::DsrPassiveBuffer>();
  ...
  if (m_mainAddress == Ipv4Address()) {
    ...
    for (uint32_t i = 0; i < m_ipv4->GetNInterfaces(); i++) {
      ...
      if (addr != loopback) {
        ...
        // possible
        Ptr<NetDevice> dev =
            m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(addr));
...
Ptr<NetDevice> DsrRouting::GetNetDeviceFromContext(std::string context) {
...
void DsrRouting::SetNode(Ptr<Node> node) { m_node = node; }

Ptr<Node> DsrRouting::GetNode() const {
  NS_LOG_FUNCTION_NOARGS();
  return m_node;
}
...
Ptr<dsr::DsrRouteCache> DsrRouting::GetRouteCache() const {
...
Ptr<dsr::DsrRreqTable> DsrRouting::GetRequestTable() const {
...
Ptr<dsr::DsrPassiveBuffer> DsrRouting::GetPassiveBuffer() const {
  // / Get the request table to use
  return m_passiveBuffer;
}

Ptr<Node> DsrRouting::GetNodeWithAddress(Ipv4Address ipv4Address) {
...
bool DsrRouting::IsLinkCache() { return m_routeCache->IsLinkCache(); }
...
bool DsrRouting::AddRoute_Link(DsrRouteCacheEntry::IP_VECTOR nodelist,
                               Ipv4Address source) {
...
void DsrRouting::DeleteAllRoutesIncludeLink(Ipv4Address errorSrc,
...
bool DsrRouting::FindSourceEntry(Ipv4Address src, Ipv4Address dst,
                                 uint16_t id) {
  return m_rreqTable->FindSourceEntry(src, dst, id);
}

Ipv4Address DsrRouting::GetIPfromMAC(Mac48Address address) {
...
Ipv4Address DsrRouting::SearchNextHop(Ipv4Address ipv4Address,
...
Ptr<Ipv4Route> DsrRouting::SetRoute(Ipv4Address nextHop,
...
int DsrRouting::GetProtocolNumber() const {
  // / This is the protocol number for DSR which is 48
  return PROT_NUMBER;
}

uint16_t DsrRouting::GetIDfromIP(Ipv4Address address) {
  int32_t nNodes = NodeList::GetNNodes();
  for (int32_t i = 0; i < nNodes; ++i) {
    Ptr<Node> node = NodeList::GetNode(i);
    Ptr<Ipv4> ipv4 = node->GetObject<Ipv4>();
...
Ipv4Address DsrRouting::GetIPfromID(uint16_t id) {
  if (id >= 256) {
...
uint32_t DsrRouting::GetPriority(DsrMessageType messageType) {
...
void DsrRouting::CheckSendBuffer() {
  NS_LOG_INFO(Simulator::Now().As(Time::S)
              << " Checking send buffer at " << m_mainAddress << " with size "
              << m_sendBuffer.GetSize());

  for (auto i = m_sendBuffer.GetBuffer().begin();
      ...
      if (optionType == 3) {
        ...
        if (errorType == 1) // This is the Route Error Option
        {
          ...
          newUnreach.SetSalvage(
              rerr.GetSalvage()); // Set the value about whether to
          ...
          Ipv4Address nextHop = SearchNextHop(
              m_mainAddress, errorRoute); // Get the next hop address
          ...
          newPacket->AddHeader(
              dsrRoutingHeader); // Add the routing header with rerr and
          ...
          uint32_t priority =
              GetPriority(DSR_CONTROL_PACKET); /// This will be priority 0
        ...
        sourceRoute.SetNodesAddress(
            nodeList); // Save the whole route in the source route header of the
                       // packet
        ...
        Simulator::Schedule(m_sendBuffInterval,
                            &DsrRouting::SendBuffTimerExpire, this);
        return;
      }
    } else {
      ++i;
...
bool DsrRouting::PromiscReceive(Ptr<NetDevice> device, Ptr<const Packet> packet,
  ...
  if (packetType == NetDevice::PACKET_OTHERHOST) {
    ...
    uint8_t offset =
        dsrRouting.GetDsrOptionsOffset(); // Get the offset for option header, 4
                                          // bytes in this case
    ...
    if (optionType == 96) // This is the source route option
    {
      ...
      NS_LOG_DEBUG(Simulator::Now().As(Time::S)
                   ...
                   << " from " << promiscSource << " to "
                   << GetIPfromMAC(Mac48Address::ConvertFrom(to))
      ...
      dsrOption->Process(pktMinusIpHdr, pktMinusDsrHdr, m_mainAddress, source,
                         ipv4Header, nextHeader, isPromisc, promiscSource);
      return true;
    }
  }
  return false;
...
void DsrRouting::PacketNewRoute(Ptr<Packet> packet, Ipv4Address source,
  ...
  if (!findRoute) {
    ...
    std::vector<Ipv4Address> nodeList =
        toDst.GetVector(); // Get the route from the route entry we found
    Ipv4Address nextHop = SearchNextHop(
        m_mainAddress, nodeList); // Get the next hop address for the route
    if (nextHop == "0.0.0.0") {
...
void DsrRouting::SendUnreachError(Ipv4Address unreachNode,
                                  Ipv4Address destination,
                                  Ipv4Address originalDst, uint8_t salvage,
                                  uint8_t protocol) {
  NS_LOG_FUNCTION(this << unreachNode << destination << originalDst
                       << (uint32_t)salvage << (uint32_t)protocol);
  DsrRoutingHeader dsrRoutingHeader;
  dsrRoutingHeader.SetNextHeader(protocol);
  dsrRoutingHeader.SetMessageType(1);
  dsrRoutingHeader.SetSourceId(GetIDfromIP(m_mainAddress));
  ...
  if (!findRoute) {
    if (destination == m_mainAddress) {
      NS_LOG_INFO("We are the error source, send request to original dst "
                  << originalDst);
      ...
      bool result =
          m_errorBuffer.Enqueue(newEntry); // Enqueue the packet in send buffer
      if (result) {
        NS_LOG_INFO(Simulator::Now().As(Time::S)
    ...
    Ptr<NetDevice> dev =
        m_ip->GetNetDevice(m_ip->GetInterfaceForAddress(m_mainAddress));
    m_ipv4Route->SetOutputDevice(dev);
    NS_LOG_INFO("Send the packet to the next hop address "
                << nextHop << " from " << m_mainAddress << " with the size "
                << newPacket->GetSize());
...
void DsrRouting::ForwardErrPacket(DsrOptionRerrUnreachHeader &rerr,
                                  DsrOptionSRHeader &sourceRoute,
                                  Ipv4Address nextHop, uint8_t protocol,
                                  Ptr<Ipv4Route> route) {
  NS_LOG_FUNCTION(this << rerr << sourceRoute << nextHop << (uint32_t)protocol
                       << route);
  NS_ASSERT_MSG(!m_downTarget.IsNull(),
                "Error, DsrRouting cannot send downward");
  DsrRoutingHeader dsrRoutingHeader;
  dsrRoutingHeader.SetNextHeader(protocol);
  dsrRoutingHeader.SetMessageType(1);
  ...
  /// \todo New DsrNetworkQueueEntry
  DsrNetworkQueueEntry newEntry(packet, m_mainAddress, nextHop,
                                Simulator::Now(), route);
...
void DsrRouting::Send(Ptr<Packet> packet, Ipv4Address source,
                      Ipv4Address destination, uint8_t protocol,
                      Ptr<Ipv4Route> route) {
...
uint16_t DsrRouting::AddAckReqHeader(Ptr<Packet> &packet, Ipv4Address nextHop) {
  ...
  tmpP->RemoveHeader(
      sourceRoute); // this is a clean packet without any dsr involved headers
...
void DsrRouting::SendPacket(Ptr<Packet> packet, Ipv4Address source,
                            Ipv4Address nextHop, uint8_t protocol) {
  ...
  /// \todo New DsrNetworkQueueEntry
  DsrNetworkQueueEntry newEntry(packet, source, nextHop, Simulator::Now(),
                                m_ipv4Route);
...
void DsrRouting::Scheduler(uint32_t priority) {
  NS_LOG_FUNCTION(this);
  PriorityScheduler(priority, true);
}

void DsrRouting::PriorityScheduler(uint32_t priority, bool continueWithFirst) {
  ...
  // priority
  for (uint32_t i = priority; numPriorities < m_numPriorityQueues;
       numPriorities++) {
    ...
    if (queueSize == 0) {
      ...
      for (auto j = m_priorityQueue.begin(); j != m_priorityQueue.end(); j++) {
        NS_LOG_INFO("The size of the network queue for "
                    << j->first << " is " << j->second->GetSize());
      ...
      if (SendRealDown(newEntry)) {
        ...
        Simulator::Schedule(Seconds(0), &DsrRouting::PriorityScheduler, this, i,
                            false);
      }
...
void DsrRouting::IncreaseRetransTimer() {
  NS_LOG_FUNCTION(this);
  ...
  std::vector<DsrNetworkQueueEntry> newNetworkQueue =
      dsrNetworkQueue->GetQueue();
  for (auto i = newNetworkQueue.begin(); i != newNetworkQueue.end(); i++) {
    Ipv4Address nextHop = i->GetNextHopAddress();
    for (auto j = m_addressForwardTimer.begin();
...
bool DsrRouting::SendRealDown(DsrNetworkQueueEntry &newEntry) {
  NS_LOG_FUNCTION(this);
...
void DsrRouting::SendPacketFromBuffer(const DsrOptionSRHeader &sourceRoute,
                                      Ipv4Address nextHop, uint8_t protocol) {
  ...
  NS_LOG_INFO("The nexthop address " << nextHop << " the source " << source
  ...
  if (m_sendBuffer.Find(destination)) {
    ...
    if (m_sendBuffer.Dequeue(destination, entry)) {
      ...
      NS_LOG_DEBUG("send buffer size here and the destination "
                   << m_sendBuffer.GetSize() << " " << destination);
      if (m_sendBuffer.GetSize() != 0 && m_sendBuffer.Find(destination)) {
   ...
   */
    ...
    if (m_errorBuffer.Dequeue(destination, entry)) {
      ...
      uint8_t optionType = 0;
      optionType = *(data);
      NS_LOG_DEBUG("The option type value in send packet "
                   << (uint32_t)optionType);
      if (optionType == 3) {
        ...
        if (errorType == 1) {
          ...
          Ptr<Packet> newPacket = Create<Packet>();
          newPacket->AddHeader(
              newRoutingHeader); // Add the extension header with rerr
...
bool DsrRouting::PassiveEntryCheck(Ptr<Packet> packet, Ipv4Address source,
                                   Ipv4Address destination, uint8_t segsLeft,
                                   uint16_t fragmentOffset,
                                   uint16_t identification, bool saveEntry) {
  NS_LOG_FUNCTION(this << packet << source << destination
                       << (uint32_t)segsLeft);
  ...
  newEntry.SetSegsLeft(
      segsLeft); // We try to make sure the segments left is larger for 1
...
bool DsrRouting::CancelPassiveTimer(Ptr<Packet> packet, Ipv4Address source,
                                    Ipv4Address destination, uint8_t segsLeft) {
  NS_LOG_FUNCTION(this << packet << source << destination
                       << (uint32_t)segsLeft);
...
void DsrRouting::CallCancelPacketTimer(uint16_t ackId,
  ...
  CancelNetworkPacketTimer(
      newEntry); // Only need to cancel network packet timer
}

void DsrRouting::CancelPacketAllTimer(DsrMaintainBuffEntry &mb) {
  NS_LOG_FUNCTION(this);
...
void DsrRouting::CancelLinkPacketTimer(DsrMaintainBuffEntry &mb) {
  NS_LOG_FUNCTION(this);
  LinkKey linkKey;
...
void DsrRouting::CancelNetworkPacketTimer(DsrMaintainBuffEntry &mb) {
  NS_LOG_FUNCTION(this);
  NetworkKey networkKey;
  ...
  NS_LOG_INFO("ackId " << mb.GetAckId() << " ourAdd " << mb.GetOurAdd()
                       << " nextHop " << mb.GetNextHop() << " source "
                       << mb.GetSrc() << " destination " << mb.GetDst()
...
void DsrRouting::CancelPassivePacketTimer(DsrMaintainBuffEntry &mb) {
  NS_LOG_FUNCTION(this);
  PassiveKey passiveKey;
  passiveKey.m_ackId = 0;
...
void DsrRouting::CancelPacketTimerNextHop(Ipv4Address nextHop,
                                          uint8_t protocol) {
  NS_LOG_FUNCTION(this << nextHop << (uint32_t)protocol);
  ...
  if (m_maintainBuffer.Dequeue(nextHop, entry)) {
    ...
    if (std::find(previousErrorDst.begin(), previousErrorDst.end(),
                  destination) == previousErrorDst.end()) {
...
void DsrRouting::SalvagePacket(Ptr<const Packet> packet, Ipv4Address source,
  ...
  if (findRoute && (salvage < m_maxSalvageCount)) {
    ...
    uint8_t length = sourceRoute.GetLength();
    NS_LOG_INFO("length of source route header "
                << (uint32_t)(sourceRoute.GetLength()));
    ...
    /// \todo New DsrNetworkQueueEntry
    DsrNetworkQueueEntry newEntry(p, m_mainAddress, nextHop, Simulator::Now(),
                                  m_ipv4Route);
...
void DsrRouting::ScheduleLinkPacketRetry(DsrMaintainBuffEntry &mb,
                                         uint8_t protocol) {
  ...
  m_linkAckTimer[linkKey].SetFunction(&DsrRouting::LinkScheduleTimerExpire,
                                      this);
  m_linkAckTimer[linkKey].Cancel();
...
void DsrRouting::SchedulePassivePacketRetry(DsrMaintainBuffEntry &mb,
                                            uint8_t protocol) {
  NS_LOG_FUNCTION(this << (uint32_t)protocol);
  ...
  m_passiveAckTimer[passiveKey].SetFunction(
      &DsrRouting::PassiveScheduleTimerExpire, this);
  m_passiveAckTimer[passiveKey].Cancel();
  m_passiveAckTimer[passiveKey].SetArguments(mb, protocol);
  m_passiveAckTimer[passiveKey].Schedule(m_passiveAckTimeout);
}

void DsrRouting::ScheduleNetworkPacketRetry(DsrMaintainBuffEntry &mb,
                                            bool isFirst, uint8_t protocol) {
  Ptr<Packet> p = Create<Packet>();
  ...
  if (isFirst) {
    ...
    m_addressForwardTimer[networkKey].SetFunction(
        &DsrRouting::NetworkScheduleTimerExpire, this);
    m_addressForwardTimer[networkKey].Cancel();
    m_addressForwardTimer[networkKey].SetArguments(newEntry, protocol);
    NS_LOG_DEBUG("The packet retries time for "
                 << newEntry.GetAckId() << " is " << m_sendRetries
    ...
    NS_LOG_DEBUG("The packet retries time for "
                 << mb.GetAckId() << " is " << m_sendRetries
                 << " and the delay time is "
                 << Time(2 * m_sendRetries * m_nodeTraversalTime).As(Time::S));
    // Back-off mechanism
    m_addressForwardTimer[networkKey].Schedule(
        Time(2 * m_sendRetries * m_nodeTraversalTime));
  }
}

void DsrRouting::LinkScheduleTimerExpire(DsrMaintainBuffEntry &mb,
                                         uint8_t protocol) {
  NS_LOG_FUNCTION(this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop();
...
void DsrRouting::PassiveScheduleTimerExpire(DsrMaintainBuffEntry &mb,
                                            uint8_t protocol) {
  NS_LOG_FUNCTION(this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop();
  Ptr<const Packet> packet = mb.GetPacket();
...
int64_t DsrRouting::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_uniformRandomVariable->SetStream(stream);
  return 1;
}

void DsrRouting::NetworkScheduleTimerExpire(DsrMaintainBuffEntry &mb,
                                            uint8_t protocol) {
  Ptr<Packet> p = mb.GetPacket()->Copy();
  Ipv4Address source = mb.GetSrc();
  Ipv4Address nextHop = mb.GetNextHop();
...
void DsrRouting::ForwardPacket(Ptr<const Packet> packet,
                               DsrOptionSRHeader &sourceRoute,
                               const Ipv4Header &ipv4Header, Ipv4Address source,
                               Ipv4Address nextHop, Ipv4Address targetAddress,
                               uint8_t protocol, Ptr<Ipv4Route> route) {
  NS_LOG_FUNCTION(this << packet << sourceRoute << source << nextHop
                       << targetAddress << (uint32_t)protocol << route);
  NS_ASSERT_MSG(!m_downTarget.IsNull(),
                "Error, DsrRouting cannot send downward");
...
void DsrRouting::SendInitialRequest(Ipv4Address source, Ipv4Address destination,
                                    uint8_t protocol) {
  ...
  m_requestId = m_rreqTable->CheckUniqueRreqId(
      destination); // Check the Id cache for duplicate ones
  rreqHeader.SetId(m_requestId);

  dsrRoutingHeader.AddDsrOption(
      rreqHeader); // Add the rreqHeader to the dsr extension header
...
void DsrRouting::SendErrorRequest(DsrOptionRerrUnreachHeader &rerr,
                                  uint8_t protocol) {
  NS_LOG_FUNCTION(this << (uint32_t)protocol);
  NS_ASSERT_MSG(!m_downTarget.IsNull(),
                "Error, DsrRouting cannot send downward");
  ...
  NS_LOG_DEBUG("our own address here "
               << m_mainAddress << " error source " << rerr.GetErrorSrc()
  ...
  if (m_routeCache->LookupRoute(dst, toDst)) {
    ...
    sourceRoute.SetSalvage(salvage);
    Ipv4Address nextHop =
        SearchNextHop(m_mainAddress, ip); // Get the next hop address
    ...
    DsrOptionRreqHeader rreqHeader; // has an alignment of 4n+0
    rreqHeader.AddNodeAddress(
        m_mainAddress); // Add our own address in the header
    rreqHeader.SetTarget(originalDst);
    m_requestId = m_rreqTable->CheckUniqueRreqId(
        originalDst); // Check the Id cache for duplicate ones
    rreqHeader.SetId(m_requestId);
...
void DsrRouting::CancelRreqTimer(Ipv4Address dst, bool isRemove) {
...
void DsrRouting::ScheduleRreqRetry(Ptr<Packet> packet,
                                   std::vector<Ipv4Address> address,
                                   bool nonProp, uint32_t requestId,
                                   uint8_t protocol) {
  ...
  if (nonProp) {
    ...
    m_nonPropReqTimer[dst].SetFunction(&DsrRouting::RouteRequestTimerExpire,
                                       this);
    m_nonPropReqTimer[dst].Cancel();
    ...
    address.push_back(source);
    address.push_back(dst);
    m_addressReqTimer[dst].SetFunction(&DsrRouting::RouteRequestTimerExpire,
                                       this);
    m_addressReqTimer[dst].Cancel();
    ...
    if (m_rreqTable->GetRreqCnt(dst)) {
      ...
      rreqDelay = Time(
          std::pow(static_cast<double>(m_rreqTable->GetRreqCnt(dst)), 2.0) *
          m_requestPeriod);
    } else {
      // This is the first route request retry
      rreqDelay = m_requestPeriod;
    }
    NS_LOG_LOGIC("Request count for "
                 << dst << " " << m_rreqTable->GetRreqCnt(dst)
                 << " with delay time " << rreqDelay.As(Time::S));
    if (rreqDelay > m_maxRequestPeriod) {
      // use the max request period
      NS_LOG_LOGIC("The max request delay time "
                   << m_maxRequestPeriod.As(Time::S));
...
void DsrRouting::RouteRequestTimerExpire(Ptr<Packet> packet,
                                         std::vector<Ipv4Address> address,
   ...
   */
  NS_LOG_LOGIC("The new request count for "
               << dst << " is " << m_rreqTable->GetRreqCnt(dst) << " the max "
               << m_rreqRetries);
  if (m_rreqTable->GetRreqCnt(dst) >= m_rreqRetries) {
    NS_LOG_LOGIC("Route discovery to " << dst << " has been attempted "
                                       << m_rreqRetries << " times");
    CancelRreqTimer(dst, true);
...
void DsrRouting::SendRequest(Ptr<Packet> packet, Ipv4Address source) {
  ...
  /// \todo New DsrNetworkQueueEntry
  DsrNetworkQueueEntry newEntry(packet, source, m_broadcast, Simulator::Now(),
                                nullptr);
  if (dsrNetworkQueue->Enqueue(newEntry)) {
    Scheduler(priority);
  } else {
...
void DsrRouting::ScheduleInterRequest(Ptr<Packet> packet) {
   ...
   */
  Simulator::Schedule(
      MilliSeconds(m_uniformRandomVariable->GetInteger(0, m_broadcastJitter)),
      &DsrRouting::SendRequest, this, packet, m_mainAddress);
}

void DsrRouting::SendGratuitousReply(Ipv4Address source, Ipv4Address srcAddress,
                                     std::vector<Ipv4Address> &nodeList,
                                     uint8_t protocol) {
  NS_LOG_FUNCTION(this << source << srcAddress << (uint32_t)protocol);
  if (!(m_graReply.FindAndUpdate(
          source, srcAddress,
          m_gratReplyHoldoff))) // Find the gratuitous reply entry
  {
    NS_LOG_LOGIC("Update gratuitous reply " << source);
    GraReplyEntry graReplyEntry(source, srcAddress,
                                m_gratReplyHoldoff + Simulator::Now());
    ...
    rrep.SetNodesAddress(
        m_finalRoute); // Set the node addresses in the route reply header
...
void DsrRouting::SendReply(Ptr<Packet> packet, Ipv4Address source,
...
void DsrRouting::ScheduleInitialReply(Ptr<Packet> packet, Ipv4Address source,
                                      Ipv4Address nextHop,
                                      Ptr<Ipv4Route> route) {
  NS_LOG_FUNCTION(this << packet << source << nextHop);
  Simulator::ScheduleNow(&DsrRouting::SendReply, this, packet, source, nextHop,
                         route);
}

void DsrRouting::ScheduleCachedReply(Ptr<Packet> packet, Ipv4Address source,
                                     Ipv4Address destination,
                                     Ptr<Ipv4Route> route, double hops) {
  NS_LOG_FUNCTION(this << packet << source << destination);
  Simulator::Schedule(
      Time(2 * m_nodeTraversalTime *
           (hops - 1 + m_uniformRandomVariable->GetValue(0, 1))),
      &DsrRouting::SendReply, this, packet, source, destination, route);
}

void DsrRouting::SendAck(uint16_t ackId, Ipv4Address destination,
                         Ipv4Address realSrc, Ipv4Address realDst,
                         uint8_t protocol, Ptr<Ipv4Route> route) {
  NS_LOG_FUNCTION(this << ackId << destination << realSrc << realDst
  ...
  NS_LOG_LOGIC("Will be inserting into priority queue "
               << dsrNetworkQueue << " number: " << priority);
  ...
  /// \todo New DsrNetworkQueueEntry
  DsrNetworkQueueEntry newEntry(packet, m_mainAddress, destination,
                                Simulator::Now(), route);
  if (dsrNetworkQueue->Enqueue(newEntry)) {
    Scheduler(priority);
  } else {
...
DsrRouting::Receive(Ptr<Packet> p, const Ipv4Header &ip,
                    Ptr<Ipv4Interface> incomingInterface) {
  NS_LOG_FUNCTION(this << p << ip << incomingInterface);

  NS_LOG_INFO("Our own IP address " << m_mainAddress
                                    << " The incoming interface address "
                                    << incomingInterface);
  ...
  uint32_t sourceId = dsrRoutingHeader.GetSourceId();
  Ipv4Address source = GetIPfromID(sourceId);
  NS_LOG_INFO("The source address " << source << " with source id "
                                    << sourceId);
  /*
  ...
  uint32_t offset =
      dsrRoutingHeader.GetDsrOptionsOffset(); // Get the offset for option
  ...
  if (optionType == 1)       // This is the request option
  {
    ...
    optionLength = dsrOption->Process(p, packet, m_mainAddress, source, ip,
                                      protocol, isPromisc, promiscSource);
  ...
  else if (optionType == 96) // This is the source route option
  {
    ...
    rerrUnsupportedHeader.SetErrorType(
        3); // The error type 3 means Option not supported
    rerrUnsupportedHeader.SetErrorSrc(
        m_mainAddress); // The error source address is our own address
    rerrUnsupportedHeader.SetUnsupported(
        optionType); // The unsupported option type number
...
IpL4Protocol::RxStatus
DsrRouting::Receive(Ptr<Packet> p, const Ipv6Header &ip,
...
void DsrRouting::SetDownTarget(DownTargetCallback callback) {
  m_downTarget = callback;
}

void DsrRouting::SetDownTarget6(DownTargetCallback6 callback) {
  NS_FATAL_ERROR("Unimplemented");
}

IpL4Protocol::DownTargetCallback DsrRouting::GetDownTarget() const {
  return m_downTarget;
}

IpL4Protocol::DownTargetCallback6 DsrRouting::GetDownTarget6() const {
  NS_FATAL_ERROR("Unimplemented");
  return MakeNullCallback<void, Ptr<Packet>, Ipv6Address, Ipv6Address, uint8_t,
                          Ptr<Ipv6Route>>();
}
