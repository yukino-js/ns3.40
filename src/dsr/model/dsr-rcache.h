...
public:
  typedef std::vector<Ipv4Address>
      IP_VECTOR; ///< Define the vector to hold Ip address
...
private:
  ...
  Time
      m_blackListTimeout; ///< Time for which the node is put into the blacklist
...
private:
  ...
  Time RouteCacheTimeout; ///< The maximum period of time that dsr is allowed to
                          ///< for an unused route.
  ...
  std::map<Ipv4Address, uint16_t>
      m_ackIdCache; ///< The id cache to ensure all the ids are unique
...
#define MAXWEIGHT 0xFFFF;
  ...
  std::map<Link, DsrLinkStab>
      m_linkCache; ///< The data structure to store link info
  std::map<Ipv4Address, DsrNodeStab>
      m_nodeCache; ///< The data structure to store node info
  /**
...
public:
  ...
  Callback<void, Ipv4Address, uint8_t>
      m_handleLinkFailure; ///< link failure callback

  Timer m_ntimer; ///< Timer for neighbor's list. Schedule Purge().

  std::vector<Neighbor> m_nb; ///< vector of entries

  std::vector<Ptr<ArpCache>> m_arp; ///< list of ARP cached to be used for layer
