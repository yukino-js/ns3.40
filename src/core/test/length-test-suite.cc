...
void LengthTestCase::TestConstructLengthFromSIUnits() {
  ...
  const std::initializer_list<TestEntry> inputs{
      ...
      std::make_tuple(Length((1 / 1852.0), Unit::NauticalMile),
                      "nautical_mile")};
...
void LengthTestCase::TestConstructLengthFromUSUnits() {
  ...
  for (const TestEntry &entry : inputs) {
    ...
    NS_TEST_ASSERT_MSG_EQ_TOL(l.GetDouble(), expectedMeters, tolerance,
                              "constructed length from US unit ("
                                  << context << ") has wrong value");
  }
}
...
void LengthTestCase::TestConstructLengthFromString(
    double unitValue, double meterValue, double tolerance,
    const std::initializer_list<std::string> &symbols) {
  ...
  for (const std::string &symbol : symbols) {
    for (const std::string &separator : SEPARATORS) {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL(l.GetDouble(), meterValue, tolerance,
                                msg.str());
    }
  }
}
...
void LengthTestCase::TestConstructLengthFromBoostUnitsKiloMeters() {
  namespace bu = boost::units;
...
void LengthTestCase::TestConstructLengthFromBoostUnitsFeet() {
  namespace bu = boost::units;
...
void LengthTestCase::TestTryParseReturnsTrue() {
  ...
  for (auto &entry : tests) {
    ...
    NS_TEST_ASSERT_MSG_EQ_TOL(l->GetDouble(), args.first, args.second,
                              stream.str());
  }
}
...
template <class T>
void LengthTestCase::TestLengthSerialization(const Length &l, const T &unit,
                                             const std::string &expectedOutput,
                                             const std::string &context) {
  const std::string msg =
      context + ": unexpected output when serializing length";
...
void LengthTestCase::TestSerializeLengthWithUnit() {
  ...
  TestLengthSerialization(l, Unit::Nanometer, "1000000000.00000 nm",
                          "nanometers");
  TestLengthSerialization(l, Unit::Micrometer, "1000000.00000 um",
                          "micrometers");
  ...
  TestLengthSerialization(l, Unit::NauticalMile, "0.00054 nmi",
                          "nautical_mile");
...
TypeId LengthValueTestCase::TestObject::GetTypeId() {
  static TypeId tid =
...
void LengthValueTestCase::TestAttributeConstructor() {
...
void LengthValueTestCase::TestAttributeSerialization() {
...
void LengthValueTestCase::TestAttributeDeserialization() {
  Ptr<const AttributeChecker> checker = MakeLengthChecker();
  ...
  NS_TEST_ASSERT_MSG_EQ(
      value.Get(), l, "Length attribute has wrong value after deserialization");
}

void LengthValueTestCase::TestObjectAttribute() {
...
void LengthValueTestCase::TestSetAttributeUsingStringValue() {
  Length expected(5, Unit::Kilometer);
  Ptr<TestObject> obj = CreateObject<TestObject>();
...
void LengthValueTestCase::DoRun() {
