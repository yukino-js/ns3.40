 ...
 */
template <class T> class ValArrayTestCase : public TestCase {
public:
  /** Default constructor*/
...
template <class T> ValArrayTestCase<T>::~ValArrayTestCase<T>() {}

template <class T> void ValArrayTestCase<T>::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ(v1.GetNumRows(), v2.GetNumRows(),
                        "The number of rows are not equal.");
  NS_TEST_ASSERT_MSG_EQ(v1.GetNumCols(), v2.GetNumCols(),
  ...
  ValArray<T> v3 = v1;
  NS_TEST_ASSERT_MSG_EQ(v1.GetNumRows(), v3.GetNumRows(),
                        "The number of rows are not equal.");
  NS_TEST_ASSERT_MSG_EQ(v1.GetNumCols(), v3.GetNumCols(),
                        "The number of cols are not equal.");
  for (size_t i = 0; i < v1.GetNumRows(); ++i) {
    for (size_t j = 0; j < v1.GetNumCols(); ++j) {
      NS_TEST_ASSERT_MSG_EQ(v1(i, j), v2(i, j), "The elements are not equal.");
  ...
  NS_TEST_ASSERT_MSG_EQ(v3size, v5.GetSize(),
                        "The number of elements are not equal.");
  ...
  for (size_t i = 0; i < v6.GetNumRows(); ++i) {
    for (size_t j = 0; j < v6.GetNumCols(); ++j) {
      NS_TEST_ASSERT_MSG_EQ(v6(i, j), static_cast<T>(initArray2[testIndex]),
  ...
  for (size_t p = 0; p < v8.GetNumPages(); ++p) {
    for (size_t i = 0; i < v8.GetNumRows(); ++i) {
      for (size_t j = 0; j < v8.GetNumCols(); ++j) {
        NS_TEST_ASSERT_MSG_EQ(v7(i, j, p) * (static_cast<T>(5.0)), v8(i, j, p),
                              "The values are not equal");
      }
    }
  }
  ...
  // test == and != operators
  NS_TEST_ASSERT_MSG_EQ(bool(v9 == v8), true,
                        "Matrices v8 and v9 should be equal");
  NS_TEST_ASSERT_MSG_EQ(bool(v10 == v8), false,
                        "Matrices v8 and v10 should not be equal");
  NS_TEST_ASSERT_MSG_EQ(bool(v10 != v8), true,
                        "Matrices v8 and v10 should not be equal");
  // test whether arrays are equal when they have different lengths
  NS_TEST_ASSERT_MSG_NE(
      ValArray<int>(std::valarray({1, 2, 3})),
  ...
  NS_TEST_ASSERT_MSG_EQ(v9.IsAlmostEqual(v8, 2) && (v9 != v8), true,
...
ValArrayTestSuite::ValArrayTestSuite() : TestSuite("val-array-test") {
  AddTestCase(new ValArrayTestCase<double>("Test ValArray<double>"));
  AddTestCase(new ValArrayTestCase<std::complex<double>>(
