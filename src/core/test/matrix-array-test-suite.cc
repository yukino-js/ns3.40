...
template <class T> void MatrixArrayTestCase<T>::DoRun() {
  ...
  for (size_t i = 0; i < m4.GetNumCols(); ++i) {
    for (size_t j = 0; j < m4.GetNumRows(); ++j) {
      NS_TEST_ASSERT_MSG_EQ(m3(i, j) * (static_cast<T>(5.0)), m4(i, j),
  ...
  NS_TEST_ASSERT_MSG_EQ(
      m7.GetNumRows(), m5.GetNumRows(),
      "The number of rows in resulting matrix is not correct");
  NS_TEST_ASSERT_MSG_EQ(
      m7.GetNumCols(), m6.GetNumCols(),
      "The number of cols in resulting matrix is not correct");
  NS_TEST_ASSERT_MSG_EQ(m7.GetNumRows(), m7.GetNumCols(),
                        "The number of rows and cols should be equal");

  for (size_t p = 0; p < m7.GetNumPages(); ++p) {
    for (size_t i = 0; i < m7.GetNumCols(); ++i) {
      for (size_t j = 0; j < m7.GetNumRows(); ++j) {
        NS_TEST_ASSERT_MSG_EQ(std::real(m7(i, j, p)), m5.GetNumCols(),
  ...
  NS_TEST_ASSERT_MSG_EQ((m9.GetNumRows() == 8) && (m9.GetNumCols() == 1) &&
                            (m9.GetNumPages() == 1),
                        true, "Creation of vector is not correct.");
  ...
  NS_TEST_ASSERT_MSG_EQ(m13.GetNumCols(), m14.GetNumCols(),
                        "The number of columns is not as expected.");
  NS_TEST_ASSERT_MSG_EQ(m13.GetNumRows(), m14.GetNumRows(),
  ...
  c = std::valarray<int>(
      {2, 3, 2, 3, 4, 6, 4, 6, 20, 30, 20, 30, 40, 60, 40, 60});
  aCasted = std::valarray<T>(a.size());
  bCasted = std::valarray<T>(b.size());
  ...
  NS_TEST_ASSERT_MSG_EQ(lCastedSize, m25.GetSize(),
  ...
  NS_TEST_ASSERT_MSG_EQ(hCastedSize, m26.GetSize(),
                        "The number of elements are not equal.");
  ...
  NS_TEST_ASSERT_MSG_EQ(jCastedSize, m27.GetSize(),
                        "The number of elements are not equal.");
}
...
void ComplexMatrixArrayTestCase::DoRun() {
...
MatrixArrayTestSuite::MatrixArrayTestSuite() : TestSuite("matrix-array-test") {
  AddTestCase(new MatrixArrayTestCase<double>("Test MatrixArray<double>"));
  AddTestCase(new MatrixArrayTestCase<std::complex<double>>(
      "Test MatrixArray<std::complex<double>>"));
