...
void UniqueTypeIdTestCase::DoRun() {
  ...
  for (uint16_t i = 0; i < nids; ++i) {
    ...
    NS_TEST_ASSERT_MSG_EQ(
        tid.GetUid(), TypeId::LookupByHash(tid.GetHash()).GetUid(),
...
void DeprecatedAttributeTestCase::DoRun() {
  ...
  std::cerr << suite << "lookup old attribute:"
            << (ainfo.supportLevel == TypeId::DEPRECATED ? "deprecated"
                                                         : "error")
            << std::endl;
  ...
  std::cerr << suite << "lookup old trace source:"
            << (tinfo.supportLevel == TypeId::DEPRECATED ? "deprecated"
                                                         : "error")
            << std::endl;
}
...
void LookupTimeTestCase::DoSetup() {
  uint32_t nids = TypeId::GetRegisteredN();

  std::cout << suite << "Lookup time: reps: " << REPETITIONS
            << ", num TypeId's: " << nids << std::endl;
}

void LookupTimeTestCase::Report(const std::string how,
  ...
  std::cout << suite << "Lookup time: by " << how << ": "
            << "ticks: " << delta << "\tper: " << per << " microsec/lookup"
            << std::endl;
}
