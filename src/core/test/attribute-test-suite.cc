...
private:
  ...
  std::map<uint32_t, Ptr<Derived>>
      m_map1; //!< Map of uint32_t, derived objects.
  ...
  TracedCallback<double, int, float>
      m_cb; //!< TracedCallback (double, int, float).
...
static AttributesTestSuite
    g_attributesTestSuite; //!< Static variable for test initialization
