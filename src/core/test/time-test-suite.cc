...
void TimeSimpleTestCase::DoTimeOperations() {
  ...
  NS_TEST_ASSERT_MSG_EQ((t2 % t1).GetSeconds(), 81,
                        "Remainder Operation (2000 % 101 = 81)");
  NS_TEST_ASSERT_MSG_EQ(Div(t2, t1), 19, "Modular Division");
  NS_TEST_ASSERT_MSG_EQ(Rem(t2, t1).GetSeconds(), 81,
                        "Remainder Operation (2000 % 101 = 81)");
}
...
void TimeSimpleTestCase::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL(Hours(10.0).GetHours(), 10.0, Hours(1).GetHours(),
                            "is 10 really 10 ?");
  NS_TEST_ASSERT_MSG_EQ_TOL(Minutes(1.0).GetMinutes(), 1.0,
                            Minutes(1).GetMinutes(), "is 1 really 1 ?");
  NS_TEST_ASSERT_MSG_EQ_TOL(Minutes(10.0).GetMinutes(), 10.0,
                            Minutes(1).GetMinutes(), "is 10 really 10 ?");
  NS_TEST_ASSERT_MSG_EQ_TOL(Seconds(1.0).GetSeconds(), 1.0,
                            TimeStep(1).GetSeconds(), "is 1 really 1 ?");
  NS_TEST_ASSERT_MSG_EQ_TOL(Seconds(10.0).GetSeconds(), 10.0,
                            TimeStep(1).GetSeconds(), "is 10 really 10 ?");
...
template <typename T>
void TimeSimpleTestCase::TestMultiplication(Time t, Time expected, T val,
...
template <typename T>
void TimeSimpleTestCase::TestDivision(Time t, Time expected, T val,
                                      const std::string &msg) {
  Time result = t / val;
...
void TimeWithSignTestCase::DoSetup() {}

void TimeWithSignTestCase::DoRun() {
...
void TimeInputOutputTestCase::Check(const std::string &str) {
  ...
  std::cout << GetParent()->GetName()
            << " InputOutput:    " << (pass ? "pass " : "FAIL ") << "\"" << str
            << "\"";
  if (!pass) {
    std::cout << ", got " << ss.str();
  }
...
void TimeInputOutputTestCase::CheckAs(const Time t, const std::string expect) {
  std::stringstream ss;
...
void TimeInputOutputTestCase::DoRun() {
  std::cout << std::endl;
  ...
  std::cout << GetParent()->GetName() << " InputOutput: " << std::fixed
            << std::setprecision(9) << "example: in s:  " << t.As(Time::S)
            << std::endl;
