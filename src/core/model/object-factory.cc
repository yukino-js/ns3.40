...
ObjectFactory::ObjectFactory() { NS_LOG_FUNCTION(this); }

void ObjectFactory::SetTypeId(TypeId tid) {
  NS_LOG_FUNCTION(this << tid.GetName());
  m_tid = tid;
}

void ObjectFactory::SetTypeId(std::string tid) {
  NS_LOG_FUNCTION(this << tid);
  m_tid = TypeId::LookupByName(tid);
}

bool ObjectFactory::IsTypeIdSet() const { return m_tid.GetUid() != 0; }

void ObjectFactory::DoSet(const std::string &name,
  ...
  if (!m_tid.LookupAttributeByName(name, &info)) {
    NS_FATAL_ERROR("Invalid attribute set (" << name << ") on "
                                             << m_tid.GetName());
    return;
  }
...
TypeId ObjectFactory::GetTypeId() const {
  NS_LOG_FUNCTION(this);
  return m_tid;
}

Ptr<Object> ObjectFactory::Create() const {
  NS_LOG_FUNCTION(this);
...
std::ostream &operator<<(std::ostream &os, const ObjectFactory &factory) {
  ...
  bool first = true;
  for (auto i = factory.m_parameters.Begin(); i != factory.m_parameters.End();
       ++i) {
