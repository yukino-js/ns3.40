...
TypeId PriorityQueueScheduler::GetTypeId() {
...
PriorityQueueScheduler::PriorityQueueScheduler() { NS_LOG_FUNCTION(this); }

PriorityQueueScheduler::~PriorityQueueScheduler() { NS_LOG_FUNCTION(this); }

void PriorityQueueScheduler::Insert(const Event &ev) {
  NS_LOG_FUNCTION(this << ev.impl << ev.key.m_ts << ev.key.m_uid);
  m_queue.push(ev);
}

bool PriorityQueueScheduler::IsEmpty() const {
  NS_LOG_FUNCTION(this);
  return m_queue.empty();
}

Scheduler::Event PriorityQueueScheduler::PeekNext() const {
  NS_LOG_FUNCTION(this);
  return m_queue.top();
}

Scheduler::Event PriorityQueueScheduler::RemoveNext() {
  NS_LOG_FUNCTION(this);
...
bool PriorityQueueScheduler::EventPriorityQueue::remove(
    const Scheduler::Event &ev) {
...
void PriorityQueueScheduler::Remove(const Scheduler::Event &ev) {
  NS_LOG_FUNCTION(this);
  m_queue.remove(ev);
}
