...
GlobalValue::GlobalValue(std::string name, std::string help,
    ...
    : m_name(name), m_help(help), m_initialValue(nullptr),
      m_currentValue(nullptr), m_checker(checker) {
...
std::string GlobalValue::GetName() const {
  NS_LOG_FUNCTION_NOARGS();
  return m_name;
}

std::string GlobalValue::GetHelp() const {
  NS_LOG_FUNCTION_NOARGS();
  return m_help;
}

void GlobalValue::GetValue(AttributeValue &value) const {
  ...
  if (str == nullptr) {
    NS_FATAL_ERROR("GlobalValue name=" << m_name
                                       << ": input value is not a string");
  }
  str->Set(m_currentValue->SerializeToString(m_checker));
}

Ptr<const AttributeChecker> GlobalValue::GetChecker() const {
  NS_LOG_FUNCTION(this);

  return m_checker;
}

bool GlobalValue::SetValue(const AttributeValue &value) {
  NS_LOG_FUNCTION(&value);
...
void GlobalValue::Bind(std::string name, const AttributeValue &value) {
...
bool GlobalValue::BindFailSafe(std::string name, const AttributeValue &value) {
  NS_LOG_FUNCTION(name << &value);
...
GlobalValue::Iterator GlobalValue::Begin() {
  NS_LOG_FUNCTION_NOARGS();

  return GetVector()->begin();
}

GlobalValue::Iterator GlobalValue::End() {
  NS_LOG_FUNCTION_NOARGS();
  return GetVector()->end();
}

void GlobalValue::ResetInitialValue() {
  NS_LOG_FUNCTION(this);
  m_currentValue = m_initialValue;
}

bool GlobalValue::GetValueByNameFailSafe(std::string name,
...
void GlobalValue::GetValueByName(std::string name, AttributeValue &value) {
  NS_LOG_FUNCTION(name << &value);
...
GlobalValue::Vector *GlobalValue::GetVector() {
  NS_LOG_FUNCTION_NOARGS();
