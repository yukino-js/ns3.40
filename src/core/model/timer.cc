...
Timer::Timer()
    : m_flags(CHECK_ON_DESTROY), m_delay(FemtoSeconds(0)), m_event(),
      m_impl(nullptr) {
  NS_LOG_FUNCTION(this);
}

Timer::Timer(DestroyPolicy destroyPolicy)
    : m_flags(destroyPolicy), m_delay(FemtoSeconds(0)), m_event(),
      m_impl(nullptr) {
...
void Timer::SetDelay(const Time &time) {
  NS_LOG_FUNCTION(this << time);
  m_delay = time;
}

Time Timer::GetDelay() const {
  NS_LOG_FUNCTION(this);
  return m_delay;
}

Time Timer::GetDelayLeft() const {
  NS_LOG_FUNCTION(this);
  switch (GetState()) {
  case Timer::RUNNING:
