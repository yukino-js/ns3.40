...
TypeId CalendarScheduler::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::CalendarScheduler")
          .SetParent<Scheduler>()
          .SetGroupName("Core")
          .AddConstructor<CalendarScheduler>()
          .AddAttribute("Reverse",
                        "Store events in reverse chronological order",
                        TypeId::ATTR_CONSTRUCT, BooleanValue(false),
...
void CalendarScheduler::SetReverse(bool reverse) {
...
void CalendarScheduler::Init(uint32_t nBuckets, uint64_t width,
                             uint64_t startPrio) {
...
void CalendarScheduler::PrintInfo() {
  NS_LOG_FUNCTION(this);
...
uint32_t CalendarScheduler::Hash(uint64_t ts) const {
  NS_LOG_FUNCTION(this);
...
void CalendarScheduler::Insert(const Event &ev) {
...
bool CalendarScheduler::IsEmpty() const {
  NS_LOG_FUNCTION(this);
  return m_qSize == 0;
}

Scheduler::Event CalendarScheduler::PeekNext() const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(!IsEmpty());
...
Scheduler::Event CalendarScheduler::DoRemoveNext() {
  NS_LOG_FUNCTION(this);
...
Scheduler::Event CalendarScheduler::RemoveNext() {
...
void CalendarScheduler::ResizeUp() {
  NS_LOG_FUNCTION(this);
...
void CalendarScheduler::ResizeDown() {
  NS_LOG_FUNCTION(this);
...
uint64_t CalendarScheduler::CalculateNewWidth() {
  NS_LOG_FUNCTION(this);
...
void CalendarScheduler::DoResize(uint32_t newSize, uint64_t newWidth) {
