...
TrickleTimer::TrickleTimer()
    : m_impl(nullptr), m_timerExpiration(), m_intervalExpiration(),
      m_currentInterval(Time(0)), m_counter(0),
...
TrickleTimer::TrickleTimer(Time minInterval, uint8_t doublings,
                           uint16_t redundancy)
    : m_impl(nullptr), m_timerExpiration(), m_intervalExpiration(),
      m_currentInterval(Time(0)), m_counter(0),
      m_uniRand(CreateObject<UniformRandomVariable>()) {
...
int64_t TrickleTimer::AssignStreams(int64_t streamNum) {
  m_uniRand->SetStream(streamNum);
  return 1;
}

void TrickleTimer::SetParameters(Time minInterval, uint8_t doublings,
                                 uint16_t redundancy) {
  NS_LOG_FUNCTION(this << minInterval << doublings << redundancy);
  NS_ASSERT_MSG(doublings < std::numeric_limits<decltype(m_ticks)>::digits,
                "Doublings value is too large");
...
Time TrickleTimer::GetMinInterval() const {
  NS_LOG_FUNCTION(this);
  return m_minInterval;
}

Time TrickleTimer::GetMaxInterval() const {
  NS_LOG_FUNCTION(this);
  return m_maxInterval;
}

uint8_t TrickleTimer::GetDoublings() const {
  NS_LOG_FUNCTION(this);
...
uint16_t TrickleTimer::GetRedundancy() const {
  NS_LOG_FUNCTION(this);
  return m_redundancy;
}

Time TrickleTimer::GetDelayLeft() const {
  NS_LOG_FUNCTION(this);
...
Time TrickleTimer::GetIntervalLeft() const {
  NS_LOG_FUNCTION(this);
...
void TrickleTimer::Enable() {
  ...
  m_intervalExpiration = Simulator::Schedule(
      m_currentInterval, &TrickleTimer::IntervalExpire, this);

  m_counter = 0;

  Time timerExpitation = m_uniRand->GetValue(0.5, 1) * m_currentInterval;
  m_timerExpiration =
      Simulator::Schedule(timerExpitation, &TrickleTimer::TimerExpire, this);
}

void TrickleTimer::ConsistentEvent() {
  NS_LOG_FUNCTION(this);
  m_counter++;
}

void TrickleTimer::InconsistentEvent() {
  NS_LOG_FUNCTION(this);
...
void TrickleTimer::IntervalExpire() {
  NS_LOG_FUNCTION(this);
