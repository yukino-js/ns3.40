...
EnumValue::EnumValue() : m_value() { NS_LOG_FUNCTION(this); }
...
Ptr<AttributeValue> EnumValue::Copy() const {
  NS_LOG_FUNCTION(this);
...
bool EnumValue::DeserializeFromString(std::string value,
...
EnumChecker::EnumChecker() { NS_LOG_FUNCTION(this); }

void EnumChecker::AddDefault(int value, std::string name) {
  NS_LOG_FUNCTION(this << value << name);
  m_valueSet.emplace_front(value, name);
}

void EnumChecker::Add(int value, std::string name) {
  NS_LOG_FUNCTION(this << value << name);
  m_valueSet.emplace_back(value, name);
}

std::string EnumChecker::GetName(int value) const {
...
int EnumChecker::GetValue(const std::string name) const {
  auto it = std::find_if(m_valueSet.begin(), m_valueSet.end(),
  ...
  NS_ASSERT_MSG(
      ...
      "name " << name
              << " is not a valid enum value. Missed entry in "
                 "MakeEnumChecker?\nAvailable values: "
              << std::accumulate(m_valueSet.begin(), m_valueSet.end(),
                                 std::string{}, [](std::string a, Value v) {
                                   if (a.empty()) {
                                     return v.second;
                                   } else {
...
bool EnumChecker::Check(const AttributeValue &value) const {
  NS_LOG_FUNCTION(this << &value);
...
std::string EnumChecker::GetValueTypeName() const {
  NS_LOG_FUNCTION(this);
  return "ns3::EnumValue";
}

bool EnumChecker::HasUnderlyingTypeInformation() const {
  NS_LOG_FUNCTION(this);
  return true;
}

std::string EnumChecker::GetUnderlyingTypeInformation() const {
  NS_LOG_FUNCTION(this);
...
Ptr<AttributeValue> EnumChecker::Create() const {
  NS_LOG_FUNCTION(this);
  return ns3::Create<EnumValue>();
}

bool EnumChecker::Copy(const AttributeValue &source,
