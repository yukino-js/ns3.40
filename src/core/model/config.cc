...
MatchContainer::MatchContainer() { NS_LOG_FUNCTION(this); }
...
MatchContainer::Iterator MatchContainer::Begin() const {
  NS_LOG_FUNCTION(this);
  return m_objects.begin();
}

MatchContainer::Iterator MatchContainer::End() const {
  NS_LOG_FUNCTION(this);
  return m_objects.end();
}

std::size_t MatchContainer::GetN() const {
  NS_LOG_FUNCTION(this);
  return m_objects.size();
}

Ptr<Object> MatchContainer::Get(std::size_t i) const {
  NS_LOG_FUNCTION(this << i);
  return m_objects[i];
}

std::string MatchContainer::GetMatchedPath(uint32_t i) const {
  NS_LOG_FUNCTION(this << i);
  return m_contexts[i];
}

std::string MatchContainer::GetPath() const {
  NS_LOG_FUNCTION(this);
  return m_path;
}

void MatchContainer::Set(std::string name, const AttributeValue &value) {
...
bool MatchContainer::SetFailSafe(std::string name,
...
void MatchContainer::Connect(std::string name, const CallbackBase &cb) {
  if (!ConnectFailSafe(name, cb)) {
    NS_FATAL_ERROR("Could not connect callback to " << name);
  }
}

bool MatchContainer::ConnectFailSafe(std::string name, const CallbackBase &cb) {
...
void MatchContainer::ConnectWithoutContext(std::string name,
...
bool MatchContainer::ConnectWithoutContextFailSafe(std::string name,
                                                   const CallbackBase &cb) {
  NS_LOG_FUNCTION(this << name << &cb);
  bool ok = false;
  for (auto tmp = Begin(); tmp != End(); ++tmp) {
...
void MatchContainer::Disconnect(std::string name, const CallbackBase &cb) {
  NS_LOG_FUNCTION(this << name << &cb);
  NS_ASSERT(m_objects.size() == m_contexts.size());
  for (uint32_t i = 0; i < m_objects.size(); ++i) {
    Ptr<Object> object = m_objects[i];
...
void MatchContainer::DisconnectWithoutContext(std::string name,
                                              const CallbackBase &cb) {
  NS_LOG_FUNCTION(this << name << &cb);
  for (auto tmp = Begin(); tmp != End(); ++tmp) {
    Ptr<Object> object = *tmp;
...
bool ArrayMatcher::Matches(std::size_t i) const {
  ...
  if (leftBracket == 0 && rightBracket == m_element.size() - 1 &&
    ...
    uint32_t min;
    uint32_t max;
    if (StringToUint32(lowerBound, &min) && StringToUint32(upperBound, &max) &&
        i >= min && i <= max) {
...
Resolver::~Resolver() { NS_LOG_FUNCTION(this); }

void Resolver::Canonicalize() {
  NS_LOG_FUNCTION(this);
...
void Resolver::Resolve(Ptr<Object> root) {
  NS_LOG_FUNCTION(this << root);

  DoResolve(m_path, root);
}

std::string Resolver::GetResolvedPath() const {
  NS_LOG_FUNCTION(this);
...
void Resolver::DoResolveOne(Ptr<Object> object) {
...
void Resolver::DoResolve(std::string path, Ptr<Object> root) {
  ...
  if (namedObject) {
    NS_LOG_DEBUG("Name system resolved item = " << item << " to "
                                                << namedObject);
    m_workStack.push_back(item);
  ...
  if (dollarPos == 0) {
    ...
    if (!object) {
      NS_LOG_DEBUG("GetObject (" << tidString
    ...
    do {
      ...
      for (uint32_t i = 0; i < tid.GetAttributeN(); i++) {
        ...
        if (pChecker != nullptr) {
          NS_LOG_DEBUG("GetAttribute(ptr)=" << info.name << " on path="
                                            << GetResolvedPath());
          PointerValue pValue;
          root->GetAttribute(info.name, pValue);
          Ptr<Object> object = pValue.Get<Object>();
          if (!object) {
            NS_LOG_ERROR("Requested object name=\""
                         << item << "\" exists on path=\"" << GetResolvedPath()
                         << "\""
                            " but is null.");
            continue;
          }
    ...
    if (!foundMatch) {
      NS_LOG_DEBUG("Requested item=" << item << " does not exist on path="
                                     << GetResolvedPath());
      return;
    }
  }
...
void Resolver::DoArrayResolve(std::string path,
...
void ConfigImpl::ParsePath(std::string path, std::string *root,
...
void ConfigImpl::Set(std::string path, const AttributeValue &value) {
...
bool ConfigImpl::SetFailSafe(std::string path, const AttributeValue &value) {
  NS_LOG_FUNCTION(this << path << &value);
...
bool ConfigImpl::ConnectWithoutContextFailSafe(std::string path,
                                               const CallbackBase &cb) {
...
void ConfigImpl::DisconnectWithoutContext(std::string path,
                                          const CallbackBase &cb) {
  NS_LOG_FUNCTION(this << path << &cb);
  std::string root;
  std::string leaf;
  ...
  if (container.GetN() == 0) {
    ...
    NS_LOG_WARN("Failed to disconnect "
                << leaf << ", the Requested object name = "
                << root.substr(lastFwdSlash + 1) << " does not exits on path "
                << root.substr(0, lastFwdSlash));
  }
  container.DisconnectWithoutContext(leaf, cb);
}

bool ConfigImpl::ConnectFailSafe(std::string path, const CallbackBase &cb) {
  NS_LOG_FUNCTION(this << path << &cb);
...
void ConfigImpl::Disconnect(std::string path, const CallbackBase &cb) {
  NS_LOG_FUNCTION(this << path << &cb);
...
MatchContainer ConfigImpl::LookupMatches(std::string path) {
  NS_LOG_FUNCTION(this << path);
...
void ConfigImpl::RegisterRootNamespaceObject(Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << obj);
  m_roots.push_back(obj);
}

void ConfigImpl::UnregisterRootNamespaceObject(Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << obj);
...
std::size_t ConfigImpl::GetRootNamespaceObjectN() const {
  NS_LOG_FUNCTION(this);
  return m_roots.size();
}

Ptr<Object> ConfigImpl::GetRootNamespaceObject(std::size_t i) const {
  NS_LOG_FUNCTION(this << i);
  return m_roots[i];
}
