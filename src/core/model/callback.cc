...
CallbackValue::CallbackValue() : m_value() { NS_LOG_FUNCTION(this); }

CallbackValue::CallbackValue(const CallbackBase &base) : m_value(base) {}

CallbackValue::~CallbackValue() { NS_LOG_FUNCTION(this); }

void CallbackValue::Set(CallbackBase base) {
  NS_LOG_FUNCTION(&base);

  m_value = base;
}

Ptr<AttributeValue> CallbackValue::Copy() const {
...
bool CallbackValue::DeserializeFromString(std::string value,
...
std::string CallbackImplBase::Demangle(const std::string &mangled) {
