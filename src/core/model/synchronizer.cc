...
TypeId Synchronizer::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Synchronizer").SetParent<Object>().SetGroupName("Core");
  return tid;
}

Synchronizer::Synchronizer() : m_realtimeOriginNano(0), m_simOriginNano(0) {
  NS_LOG_FUNCTION(this);
}

Synchronizer::~Synchronizer() { NS_LOG_FUNCTION(this); }

bool Synchronizer::Realtime() {
  NS_LOG_FUNCTION(this);
  return DoRealtime();
}

uint64_t Synchronizer::GetCurrentRealtime() {
  NS_LOG_FUNCTION(this);
  return NanosecondToTimeStep(DoGetCurrentRealtime());
}

void Synchronizer::SetOrigin(uint64_t ts) {
...
int64_t Synchronizer::GetDrift(uint64_t ts) {
  NS_LOG_FUNCTION(this << ts);
...
bool Synchronizer::Synchronize(uint64_t tsCurrent, uint64_t tsDelay) {
...
void Synchronizer::SetCondition(bool cond) {
  NS_LOG_FUNCTION(this << cond);
  DoSetCondition(cond);
}

void Synchronizer::EventStart() {
  NS_LOG_FUNCTION(this);
  DoEventStart();
}

uint64_t Synchronizer::EventEnd() {
  NS_LOG_FUNCTION(this);
  return NanosecondToTimeStep(DoEventEnd());
}

uint64_t Synchronizer::TimeStepToNanosecond(uint64_t ts) {
  NS_LOG_FUNCTION(this << ts);
  return TimeStep(ts).GetNanoSeconds();
}

uint64_t Synchronizer::NanosecondToTimeStep(uint64_t ns) {
