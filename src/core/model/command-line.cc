...
CommandLine::CommandLine()
    : m_NNonOptions(0), m_nonOptionCount(0), m_usage(), m_shortName() {
...
CommandLine::CommandLine(const CommandLine &cmd) { Copy(cmd); }

CommandLine &CommandLine::operator=(const CommandLine &cmd) {
  Clear();
  Copy(cmd);
  return *this;
}
...
void CommandLine::Copy(const CommandLine &cmd) {
  NS_LOG_FUNCTION(&cmd);

  std::copy(cmd.m_options.begin(), cmd.m_options.end(), m_options.end());
  std::copy(cmd.m_nonOptions.begin(), cmd.m_nonOptions.end(),
            m_nonOptions.end());
...
void CommandLine::Usage(const std::string &usage) { m_usage = usage; }

std::string CommandLine::GetName() const { return m_shortName; }

CommandLine::Item::~Item() { NS_LOG_FUNCTION(this); }

void CommandLine::Parse(std::vector<std::string> args) {
...
void CommandLine::HandleHardOptions(
...
bool CommandLine::HandleOption(const std::string &param) const {
  auto [isOpt, name, value] = GetOptionName(param);
  if (!isOpt) {
    return false;
  }
...
bool CommandLine::HandleNonOption(const std::string &value) {
  ...
  if (!i->Parse(value)) {
    std::cerr << "Invalid non-option argument value " << value << " for "
              << i->m_name << std::endl;
    PrintHelp(std::cerr);
    std::exit(1);
  }
...
void CommandLine::PrintVersion(std::ostream &os) const {
  os << GetVersion() << std::endl;
}

void CommandLine::PrintDoxygenUsage() const {
  NS_LOG_FUNCTION(this);
  ...
  os << "/**\n \\file " << m_shortName << ".cc\n"
     ...
     << (!m_options.empty() ? " [Program Options]" : "")
  ...
  auto listOptions = [&os](const std::string &head, const Items &items,
                           std::string pre) {
    os << "\n<h3>" << head << "</h3>\n<dl>\n";
...
void CommandLine::PrintGlobals(std::ostream &os) const {
  NS_LOG_FUNCTION(this);
...
void CommandLine::PrintAttributeList(std::ostream &os, const TypeId tid,
...
void CommandLine::PrintAttributes(std::ostream &os,
...
void CommandLine::PrintGroup(std::ostream &os, const std::string &group) const {
  NS_LOG_FUNCTION(this);
...
void CommandLine::PrintTypeIds(std::ostream &os) const {
  NS_LOG_FUNCTION(this);
...
void CommandLine::PrintGroups(std::ostream &os) const {
  NS_LOG_FUNCTION(this);
...
bool CommandLine::HandleArgument(const std::string &name,
  ...
  auto item = std::find_if(
      ...
      [name](std::shared_ptr<Item> it) { return it->m_name == name; });
...
bool CommandLine::CallbackItem::HasDefault() const {
  return !m_default.empty();
}

std::string CommandLine::CallbackItem::GetDefault() const { return m_default; }

bool CommandLine::CallbackItem::Parse(const std::string &value) const {
  NS_LOG_FUNCTION(this);
...
std::string CommandLine::GetExtraNonOption(std::size_t i) const {
  std::string value;

  if (m_nonOptions.size() >= i + m_NNonOptions) {
    auto ip =
        std::dynamic_pointer_cast<StringItem>(m_nonOptions[i + m_NNonOptions]);
    if (ip != nullptr) {
      value = ip->m_value;
    }
  }
...
std::size_t CommandLine::GetNExtraNonOptions() const {
...
bool CommandLine::StringItem::HasDefault() const { return false; }

std::string CommandLine::StringItem::GetDefault() const { return ""; }

bool CommandLine::CharStarItem::Parse(const std::string &value) const {
  if (value.size() > m_size - 1) {
    std::cerr << "Value \"" << value << "\" (" << value.size()
              << " bytes) is too long for " << m_name << " buffer (" << m_size
              << " bytes, including terminating null)." << std::endl;
    return false;
  }
...
bool CommandLine::CharStarItem::HasDefault() const { return true; }

std::string CommandLine::CharStarItem::GetDefault() const { return m_default; }
...
template <>
bool CommandLineHelper::UserItemParse<uint8_t>(const std::string &value,
                                               uint8_t &dest) {
