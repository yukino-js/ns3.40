...
TypeId DefaultSimulatorImpl::GetTypeId() {
...
DefaultSimulatorImpl::~DefaultSimulatorImpl() { NS_LOG_FUNCTION(this); }

void DefaultSimulatorImpl::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void DefaultSimulatorImpl::Destroy() {
  NS_LOG_FUNCTION(this);
...
void DefaultSimulatorImpl::SetScheduler(ObjectFactory schedulerFactory) {
...
bool DefaultSimulatorImpl::IsFinished() const {
  return m_events->IsEmpty() || m_stop;
}

void DefaultSimulatorImpl::ProcessEventsWithContext() {
...
void DefaultSimulatorImpl::Stop() {
  NS_LOG_FUNCTION(this);
  m_stop = true;
}

void DefaultSimulatorImpl::Stop(const Time &delay) {
...
EventId DefaultSimulatorImpl::Schedule(const Time &delay, EventImpl *event) {
  ...
  NS_ASSERT_MSG(delay.IsPositive(),
                "DefaultSimulatorImpl::Schedule(): Negative delay");
...
void DefaultSimulatorImpl::ScheduleWithContext(uint32_t context,
                                               const Time &delay,
                                               EventImpl *event) {
...
EventId DefaultSimulatorImpl::ScheduleDestroy(EventImpl *event) {
  NS_ASSERT_MSG(m_mainThreadId == std::this_thread::get_id(),
...
void DefaultSimulatorImpl::Cancel(const EventId &id) {
  if (!IsExpired(id)) {
    id.PeekEventImpl()->Cancel();
  }
}

bool DefaultSimulatorImpl::IsExpired(const EventId &id) const {
  if (id.GetUid() == EventId::UID::DESTROY) {
...
Time DefaultSimulatorImpl::GetMaximumSimulationTime() const {
  return TimeStep(0x7fffffffffffffffLL);
}

uint32_t DefaultSimulatorImpl::GetContext() const { return m_currentContext; }

uint64_t DefaultSimulatorImpl::GetEventCount() const { return m_eventCount; }
