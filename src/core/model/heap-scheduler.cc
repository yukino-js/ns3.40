...
TypeId HeapScheduler::GetTypeId() {
...
HeapScheduler::~HeapScheduler() { NS_LOG_FUNCTION(this); }

std::size_t HeapScheduler::Parent(std::size_t id) const {
  NS_LOG_FUNCTION(this << id);
  return id / 2;
}

std::size_t HeapScheduler::Sibling(std::size_t id) const {
  NS_LOG_FUNCTION(this << id);
  return id + 1;
}

std::size_t HeapScheduler::LeftChild(std::size_t id) const {
  NS_LOG_FUNCTION(this << id);
  return id * 2;
}

std::size_t HeapScheduler::RightChild(std::size_t id) const {
  NS_LOG_FUNCTION(this << id);
  return id * 2 + 1;
}

std::size_t HeapScheduler::Root() const {
  NS_LOG_FUNCTION(this);
  return 1;
}

bool HeapScheduler::IsRoot(std::size_t id) const {
  NS_LOG_FUNCTION(this << id);
  return (id == Root());
}

std::size_t HeapScheduler::Last() const {
  NS_LOG_FUNCTION(this);
  return m_heap.size() - 1;
}

bool HeapScheduler::IsBottom(std::size_t id) const {
  NS_LOG_FUNCTION(this << id);
  return (id >= m_heap.size());
}

void HeapScheduler::Exch(std::size_t a, std::size_t b) {
...
bool HeapScheduler::IsLessStrictly(std::size_t a, std::size_t b) const {
  NS_LOG_FUNCTION(this << a << b);
  return m_heap[a] < m_heap[b];
}

std::size_t HeapScheduler::Smallest(std::size_t a, std::size_t b) const {
  NS_LOG_FUNCTION(this << a << b);
  return IsLessStrictly(a, b) ? a : b;
}

bool HeapScheduler::IsEmpty() const {
  NS_LOG_FUNCTION(this);
  return (m_heap.size() == 1);
}

void HeapScheduler::BottomUp() {
  NS_LOG_FUNCTION(this);
...
void HeapScheduler::TopDown(std::size_t start) {
...
void HeapScheduler::Insert(const Event &ev) {
  NS_LOG_FUNCTION(this << &ev);
  m_heap.push_back(ev);
  BottomUp();
}

Scheduler::Event HeapScheduler::PeekNext() const {
  NS_LOG_FUNCTION(this);
  return m_heap[Root()];
}

Scheduler::Event HeapScheduler::RemoveNext() {
  NS_LOG_FUNCTION(this);
...
void HeapScheduler::Remove(const Event &ev) {
  NS_LOG_FUNCTION(this << &ev);
