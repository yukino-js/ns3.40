...
MapScheduler::MapScheduler() { NS_LOG_FUNCTION(this); }

MapScheduler::~MapScheduler() { NS_LOG_FUNCTION(this); }

void MapScheduler::Insert(const Event &ev) {
...
bool MapScheduler::IsEmpty() const {
  NS_LOG_FUNCTION(this);
  return m_list.empty();
}

Scheduler::Event MapScheduler::PeekNext() const {
  NS_LOG_FUNCTION(this);
  auto i = m_list.begin();
  ...
  ev.impl = i->second;
  ev.key = i->first;
  NS_LOG_DEBUG(this << ": " << ev.impl << ", " << ev.key.m_ts << ", "
                    << ev.key.m_uid);
  return ev;
}

Scheduler::Event MapScheduler::RemoveNext() {
  NS_LOG_FUNCTION(this);
  auto i = m_list.begin();
  NS_ASSERT(i != m_list.end());
  Event ev;
...
void MapScheduler::Remove(const Event &ev) {
  NS_LOG_FUNCTION(this << ev.impl << ev.key.m_ts << ev.key.m_uid);
