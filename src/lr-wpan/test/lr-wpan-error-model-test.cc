...
LrWpanErrorDistanceTestCase::LrWpanErrorDistanceTestCase()
    : TestCase("Test the 802.15.4 error model vs distance"), m_received(0) {}

LrWpanErrorDistanceTestCase::~LrWpanErrorDistanceTestCase() {}

void LrWpanErrorDistanceTestCase::Callback(McpsDataIndicationParams params,
                                           Ptr<Packet> p) {
  m_received++;
}
...
void LrWpanErrorDistanceTestCase::DoRun() {
  ...
  Ptr<LogDistancePropagationLossModel> model =
  ...
  for (int i = 0; i < 1000; i++) {
    p = Create<Packet>(20);
    Simulator::Schedule(Seconds(i), &LrWpanMac::McpsDataRequest, dev0->GetMac(),
                        params, p);
  }
...
void LrWpanErrorModelTestCase::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL(ber, 7.38e-14, 0.01e-14,
                            "Model fails for SNR = " << snr);
  snr = 2;
  ber = 1.0 - model->GetChunkSuccessRate(pow(10.0, snr / 10.0), 1);
  NS_TEST_ASSERT_MSG_EQ_TOL(ber, 5.13e-7, 0.01e-7,
                            "Model fails for SNR = " << snr);
  snr = -1;
  ber = 1.0 - model->GetChunkSuccessRate(pow(10.0, snr / 10.0), 1);
  NS_TEST_ASSERT_MSG_EQ_TOL(ber, 0.00114, 0.00001,
                            "Model fails for SNR = " << snr);
  snr = -4;
  ber = 1.0 - model->GetChunkSuccessRate(pow(10.0, snr / 10.0), 1);
  NS_TEST_ASSERT_MSG_EQ_TOL(ber, 0.0391, 0.0001,
                            "Model fails for SNR = " << snr);
  snr = -7;
  ber = 1.0 - model->GetChunkSuccessRate(pow(10.0, snr / 10.0), 1);
