...
struct LrWpanMacPibAttributes : public SimpleRefCount<LrWpanMacPibAttributes> {
  ...
  uint8_t macBeaconPayloadLength{
      0};                       //!< The length in octets of the beacon payload.
...
struct PanDescriptor {
  LrWpanAddressMode m_coorAddrMode{
      SHORT_ADDR}; //!< The coordinator addressing mode corresponding
                   //!< to the received beacon frame.
  uint16_t m_coorPanId{
      0xffff}; //!< The PAN ID of the coordinator as specified in
  ...
  Mac64Address m_coorExtAddr; //!< The coordinator extended address as specified
                              //!< in the coordinator address mode.
...
struct McpsDataIndicationParams {
  ...
  uint8_t m_mpduLinkQuality{
      0};           //!< LQI value measured during reception of the MPDU
...
struct MlmeAssociateIndicationParams {
  ...
  uint8_t lqi{0}; //!< The link quality indicator of the received associate
                  //!< request command (Not officially supported in the standard
                  //!< but found in implementations)
};
...
struct MlmeAssociateResponseParams {
  Mac64Address m_extDevAddr; //!< The extended address of the device requesting
                             //!< association
  Mac16Address
      m_assocShortAddr; //!< The short address allocated by the coordinator on
                        //!< successful assoc. FF:FF = Unsuccessful
  LrWpanAssociationStatus m_status{
      DISASSOCIATED}; //!< The status of the association attempt (As
...
struct MlmeStartRequestParams {
  ...
  uint32_t m_startTime{
      0}; //!< Time at which to begin transmitting beacons (Used by Coordinator
          //!< not PAN Coordinators). The time is specified in symbols.
  uint8_t m_bcnOrd{
      15}; //!< Beacon Order, Used to calculate the beacon interval, a value of
           //!< 15 indicates no periodic beacons will be transmitted.
  uint8_t m_sfrmOrd{
      15}; //!< Superframe Order, indicates the length of the CAP in time slots.
...
struct MlmePollRequestParams {
  LrWpanAddressMode m_coorAddrMode{
      SHORT_ADDR}; //!< The addressing mode of the coordinator to which the pool
                   //!< is intended.
  uint16_t m_coorPanId{
      0}; //!< The PAN id of the coordinator to which the poll is intended.
...
struct MlmeScanRequestParams {
  LrWpanMlmeScanType m_scanType{
      MLMESCAN_PASSIVE}; //!< Indicates the type of scan performed as
  ...
  uint8_t m_scanDuration{
      14}; //!< The factor (0-14) used to calculate the length of time
           //!< to spend scanning.
           //!< scanDurationSymbols =
...
struct MlmeScanConfirmParams {
  LrWpanMlmeScanConfirmStatus m_status{
      MLMESCAN_INVALID_PARAMETER}; //!< The status the request.
  LrWpanMlmeScanType m_scanType{
      MLMESCAN_PASSIVE}; //!< Indicates the type of scan
                         //!<  performed (ED,ACTIVE,PASSIVE,ORPHAN).
  uint32_t m_chPage{0};  //!< The channel page on which the scan was performed.
  std::vector<uint8_t>
      m_unscannedCh; //!< A list of channels given in the request which
                     //!<  were not scanned (Not valid for ED scans).
  uint8_t m_resultListSize{
      0}; //!< The number of elements returned in the appropriate
          //!<  result list. (Not valid for Orphan scan).
  std::vector<uint8_t>
      m_energyDetList; //!< A list of energy measurements, one for each
                       //!< channel searched during ED scan
                       //!< (Not valid for Active, Passive or Orphan Scans)
  std::vector<PanDescriptor>
      m_panDescList; //!< A list of PAN descriptor, one for each beacon
...
struct MlmeAssociateRequestParams {
  ...
  uint16_t m_coordPanId{
      0}; //!< The identifier of the PAN with which to associate.
  Mac16Address m_coordShortAddr; //!< The short address of the coordinator with
                                 //!< which to associate.
  Mac64Address m_coordExtAddr; //!< The extended address of the coordinator with
                               //!< which to associate.
...
struct MlmeAssociateConfirmParams {
  Mac16Address
      m_assocShortAddr; //!< The short address used in the association request
...
struct MlmeBeaconNotifyIndicationParams {
  ...
  PanDescriptor
      m_panDescriptor; //!< The PAN descriptor for the received beacon.
  uint32_t m_sduLength{
      0};            //!< The number of octets contained in the beacon payload.
...
struct MlmeSyncLossIndicationParams {
  LrWpanSyncLossReason m_lossReason{
      MLMESYNCLOSS_PAN_ID_CONFLICT}; //!< The reason for the lost of
                                     //!< synchronization.
...
struct MlmeCommStatusIndicationParams {
  uint16_t m_panId{
      0}; //!< The PAN identifier of the device from which the frame was
          //!< received or to which the frame was being sent.
  uint8_t m_srcAddrMode{
      SHORT_ADDR}; //!< The source addressing mode for this primitive
  ...
  uint8_t m_dstAddrMode{
      SHORT_ADDR}; //!< The destination addressing mode for this primitive.
  ...
  LrWpanMlmeCommStatus m_status{
      MLMECOMMSTATUS_INVALID_PARAMETER}; //!< The communication status
};
...
struct MlmeSetConfirmParams {
  LrWpanMlmeSetConfirmStatus m_status{
      MLMESET_UNSUPPORTED_ATTRIBUTE}; //!< The result of
                                      //!< the request to write
                                      //!< the PIB attribute.
  LrWpanMacPibAttributeIdentifier
      id; //!< The id of the PIB attribute that was written.
};
