...
TypeId LrWpanMacTrailer::GetTypeId() {
...
TypeId LrWpanMacTrailer::GetInstanceTypeId() const { return GetTypeId(); }

void LrWpanMacTrailer::Print(std::ostream &os) const {
  os << " FCS = " << m_fcs;
}

uint32_t LrWpanMacTrailer::GetSerializedSize() const {
...
uint16_t LrWpanMacTrailer::GetFcs() const { return m_fcs; }
...
bool LrWpanMacTrailer::IsFcsEnabled() const { return m_calcFcs; }
