...
void SuperframeField::SetSuperframeOrder(uint8_t frmOrder) {
  if (frmOrder > 15) {
...
void SuperframeField::SetFinalCapSlot(uint8_t capSlot) {
  if (capSlot > 15) {
    NS_ABORT_MSG(
        "The final slot cannot be greater than the slots in a CAP (15)");
  } else {
    m_sspecFnlCapSlot = capSlot;
  }
}

void SuperframeField::SetBattLifeExt(bool battLifeExt) {
  m_sspecBatLifeExt = battLifeExt;
}

void SuperframeField::SetPanCoor(bool panCoor) { m_sspecPanCoor = panCoor; }

void SuperframeField::SetAssocPermit(bool assocPermit) {
  m_sspecAssocPermit = assocPermit;
}

uint8_t SuperframeField::GetBeaconOrder() const { return m_sspecBcnOrder; }

uint8_t SuperframeField::GetFrameOrder() const { return m_sspecSprFrmOrder; }

uint8_t SuperframeField::GetFinalCapSlot() const { return m_sspecFnlCapSlot; }

bool SuperframeField::IsBattLifeExt() const { return m_sspecBatLifeExt; }

bool SuperframeField::IsPanCoor() const { return m_sspecPanCoor; }

bool SuperframeField::IsAssocPermit() const { return m_sspecAssocPermit; }
...
Buffer::Iterator SuperframeField::Deserialize(Buffer::Iterator i) {
...
Buffer::Iterator GtsFields::Serialize(Buffer::Iterator i) const {
...
Buffer::Iterator GtsFields::Deserialize(Buffer::Iterator i) {
...
uint8_t PendingAddrFields::GetNumShortAddr() const {
  return m_pndAddrSpecNumShortAddr;
}

uint8_t PendingAddrFields::GetNumExtAddr() const {
...
void PendingAddrFields::AddAddress(Mac16Address shortAddr) {
...
void PendingAddrFields::AddAddress(Mac64Address extAddr) {
  uint8_t totalPendAddr = m_pndAddrSpecNumShortAddr + m_pndAddrSpecNumExtAddr;
...
bool PendingAddrFields::SearchAddress(Mac16Address shortAddr) {
...
bool PendingAddrFields::SearchAddress(Mac64Address extAddr) {
...
Buffer::Iterator PendingAddrFields::Serialize(Buffer::Iterator i) const {
...
Buffer::Iterator PendingAddrFields::Deserialize(Buffer::Iterator i) {
...
CapabilityField::CapabilityField(uint8_t bitmap) { SetCapability(bitmap); }
...
Buffer::Iterator CapabilityField::Serialize(Buffer::Iterator i) const {
  i.WriteU8(GetCapability());
  return i;
}

Buffer::Iterator CapabilityField::Deserialize(Buffer::Iterator i) {
  uint8_t capability = i.ReadU8();
  SetCapability(capability);
  return i;
}

bool CapabilityField::IsDeviceTypeFfd() const { return m_deviceType; }

bool CapabilityField::IsPowSrcAvailable() const { return m_powerSource; }

bool CapabilityField::IsReceiverOnWhenIdle() const {
  return m_receiverOnWhenIdle;
}

bool CapabilityField::IsSecurityCapability() const { return m_securityCap; }

bool CapabilityField::IsShortAddrAllocOn() const { return m_allocAddr; }

void CapabilityField::SetFfdDevice(bool devType) { m_deviceType = devType; }

void CapabilityField::SetPowSrcAvailable(bool pow) { m_powerSource = pow; }

void CapabilityField::SetRxOnWhenIdle(bool rxIdle) {
  m_receiverOnWhenIdle = rxIdle;
}

void CapabilityField::SetSecurityCap(bool sec) { m_securityCap = sec; }

void CapabilityField::SetShortAddrAllocOn(bool addrAlloc) {
...
std::ostream &operator<<(std::ostream &os,
  ...
  os << " FFD device capable = " << bool(capabilityField.IsDeviceTypeFfd())
     << ", Alternate Power Current Available  = "
     << bool(capabilityField.IsPowSrcAvailable())
     << ", Receiver On When Idle  = "
     << bool(capabilityField.IsReceiverOnWhenIdle())
     << ", Security Capable  = " << bool(capabilityField.IsSecurityCapability())
     << ", Coordinator Allocate Short Address  = "
