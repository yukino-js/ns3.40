...
TypeId LrWpanNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LrWpanNetDevice")
          ...
          .AddAttribute(
              ...
              MakeEnumChecker(
                  LrWpanNetDevice::RFC6282, "RFC 6282 (don't use PanId)",
...
LrWpanNetDevice::~LrWpanNetDevice() { NS_LOG_FUNCTION(this); }

void LrWpanNetDevice::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_mac->Dispose();
  m_phy->Dispose();
  m_csmaca->Dispose();
...
void LrWpanNetDevice::CompleteConfig() {
  NS_LOG_FUNCTION(this);
  ...
  m_mac->SetMcpsDataIndicationCallback(
      MakeCallback(&LrWpanNetDevice::McpsDataIndication, this));
  ...
  m_phy->SetPdDataIndicationCallback(
      MakeCallback(&LrWpanMac::PdDataIndication, m_mac));
  m_phy->SetPdDataConfirmCallback(
      MakeCallback(&LrWpanMac::PdDataConfirm, m_mac));
  m_phy->SetPlmeEdConfirmCallback(
      MakeCallback(&LrWpanMac::PlmeEdConfirm, m_mac));
  ...
  m_csmaca->SetLrWpanMacStateCallback(
      MakeCallback(&LrWpanMac::SetLrWpanMacState, m_mac));
  m_phy->SetPlmeCcaConfirmCallback(
      MakeCallback(&LrWpanCsmaCa::PlmeCcaConfirm, m_csmaca));
  m_configComplete = true;
}

void LrWpanNetDevice::SetMac(Ptr<LrWpanMac> mac) {
  NS_LOG_FUNCTION(this);
  m_mac = mac;
  CompleteConfig();
}

void LrWpanNetDevice::SetPhy(Ptr<LrWpanPhy> phy) {
  NS_LOG_FUNCTION(this);
  m_phy = phy;
  CompleteConfig();
}

void LrWpanNetDevice::SetCsmaCa(Ptr<LrWpanCsmaCa> csmaca) {
  NS_LOG_FUNCTION(this);
  m_csmaca = csmaca;
  CompleteConfig();
}

void LrWpanNetDevice::SetChannel(Ptr<SpectrumChannel> channel) {
...
Ptr<LrWpanCsmaCa> LrWpanNetDevice::GetCsmaCa() const {
  NS_LOG_FUNCTION(this);
  return m_csmaca;
}

void LrWpanNetDevice::SetIfIndex(const uint32_t index) {
  NS_LOG_FUNCTION(this << index);
  m_ifIndex = index;
}

uint32_t LrWpanNetDevice::GetIfIndex() const {
  NS_LOG_FUNCTION(this);
  return m_ifIndex;
}

Ptr<Channel> LrWpanNetDevice::GetChannel() const {
  NS_LOG_FUNCTION(this);
  return m_phy->GetChannel();
}

void LrWpanNetDevice::LinkUp() {
  NS_LOG_FUNCTION(this);
  m_linkUp = true;
  m_linkChanges();
}

void LrWpanNetDevice::LinkDown() {
  NS_LOG_FUNCTION(this);
  m_linkUp = false;
  m_linkChanges();
}

Ptr<SpectrumChannel> LrWpanNetDevice::DoGetChannel() const {
  NS_LOG_FUNCTION(this);
  return m_phy->GetChannel();
}

void LrWpanNetDevice::SetAddress(Address address) {
  NS_LOG_FUNCTION(this);
...
Address LrWpanNetDevice::GetAddress() const {
  NS_LOG_FUNCTION(this);
...
void LrWpanNetDevice::SetPanAssociation(uint16_t panId,
...
bool LrWpanNetDevice::SetMtu(const uint16_t mtu) {
...
bool LrWpanNetDevice::IsLinkUp() const {
  NS_LOG_FUNCTION(this);
  return m_phy && m_linkUp;
}

void LrWpanNetDevice::AddLinkChangeCallback(Callback<void> callback) {
  NS_LOG_FUNCTION(this);
  m_linkChanges.ConnectWithoutContext(callback);
}

bool LrWpanNetDevice::IsBroadcast() const {
  NS_LOG_FUNCTION(this);
  return true;
}

Address LrWpanNetDevice::GetBroadcast() const {
  NS_LOG_FUNCTION(this);
...
bool LrWpanNetDevice::IsMulticast() const {
  NS_LOG_FUNCTION(this);
  return true;
}

Address LrWpanNetDevice::GetMulticast(Ipv4Address multicastGroup) const {
  NS_ABORT_MSG("Unsupported");
  return Address();
}

Address LrWpanNetDevice::GetMulticast(Ipv6Address addr) const {
  NS_LOG_FUNCTION(this << addr);

  Mac48Address pseudoAddress = BuildPseudoMacAddress(
      m_mac->GetPanId(), Mac16Address::GetMulticast(addr));

  return pseudoAddress;
}

bool LrWpanNetDevice::IsBridge() const {
  NS_LOG_FUNCTION(this);
  return false;
}

bool LrWpanNetDevice::IsPointToPoint() const {
  NS_LOG_FUNCTION(this);
  return false;
}
...
Ptr<Node> LrWpanNetDevice::GetNode() const {
  NS_LOG_FUNCTION(this);
  return m_node;
}

void LrWpanNetDevice::SetNode(Ptr<Node> node) {
  NS_LOG_FUNCTION(this);
  m_node = node;
  CompleteConfig();
}

bool LrWpanNetDevice::NeedsArp() const {
  NS_LOG_FUNCTION(this);
  return true;
}

void LrWpanNetDevice::SetReceiveCallback(ReceiveCallback cb) {
  NS_LOG_FUNCTION(this);
...
void LrWpanNetDevice::McpsDataIndication(McpsDataIndicationParams params,
  ...
  if (params.m_dstAddrMode == SHORT_ADDR) {
    m_receiveCallback(
        this, pkt, 0,
        BuildPseudoMacAddress(params.m_srcPanId, params.m_srcAddr));
  } else {
    m_receiveCallback(this, pkt, 0, params.m_srcExtAddr);
  }
}

bool LrWpanNetDevice::SupportsSendFrom() const {
  NS_LOG_FUNCTION_NOARGS();
  return false;
}

Mac48Address
LrWpanNetDevice::BuildPseudoMacAddress(uint16_t panId,
...
int64_t LrWpanNetDevice::AssignStreams(int64_t stream) {
