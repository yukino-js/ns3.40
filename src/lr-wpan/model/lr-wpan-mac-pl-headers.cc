...
TypeId BeaconPayloadHeader::GetTypeId() {
...
TypeId BeaconPayloadHeader::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t BeaconPayloadHeader::GetSerializedSize() const {
  uint32_t size = 0;
...
void BeaconPayloadHeader::Serialize(Buffer::Iterator start) const {
...
uint32_t BeaconPayloadHeader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
void BeaconPayloadHeader::Print(std::ostream &os) const {
...
void BeaconPayloadHeader::SetSuperframeSpecField(SuperframeField sf) {
  m_superframeField = sf;
}

void BeaconPayloadHeader::SetGtsFields(GtsFields gtsFields) {
  m_gtsFields = gtsFields;
}

void BeaconPayloadHeader::SetPndAddrFields(PendingAddrFields pndAddrFields) {
  m_pndAddrFields = pndAddrFields;
}

SuperframeField BeaconPayloadHeader::GetSuperframeSpecField() const {
  return m_superframeField;
}

GtsFields BeaconPayloadHeader::GetGtsFields() const { return m_gtsFields; }

PendingAddrFields BeaconPayloadHeader::GetPndAddrFields() const {
...
TypeId CommandPayloadHeader::GetTypeId() {
...
TypeId CommandPayloadHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
void CommandPayloadHeader::Print(std::ostream &os) const {
  ...
  case ASSOCIATION_REQ:
    os << "| Device Type FFD | = " << m_capabilityInfo.IsDeviceTypeFfd()
       ...
       << m_capabilityInfo.IsPowSrcAvailable() << "| Receiver on when Idle | = "
       << m_capabilityInfo.IsReceiverOnWhenIdle()
...
void CommandPayloadHeader::SetCommandFrameType(MacCommand macCommand) {
  m_cmdFrameId = macCommand;
}

void CommandPayloadHeader::SetCapabilityField(CapabilityField cap) {
  NS_ASSERT(m_cmdFrameId == ASSOCIATION_REQ);
  m_capabilityInfo = cap;
}

void CommandPayloadHeader::SetCoordShortAddr(Mac16Address addr) {
  NS_ASSERT(m_cmdFrameId == COOR_REALIGN);
  m_coordShortAddr = addr;
}

void CommandPayloadHeader::SetChannel(uint8_t channel) {
  NS_ASSERT(m_cmdFrameId == COOR_REALIGN);
  m_logCh = channel;
}

void CommandPayloadHeader::SetPage(uint8_t page) {
  NS_ASSERT(m_cmdFrameId == COOR_REALIGN);
  m_logChPage = page;
}
...
void CommandPayloadHeader::SetAssociationStatus(AssocStatus status) {
  NS_ASSERT(m_cmdFrameId == ASSOCIATION_RESP);
  m_assocStatus = status;
}

Mac16Address CommandPayloadHeader::GetShortAddr() const { return m_shortAddr; }
...
CapabilityField CommandPayloadHeader::GetCapabilityField() const {
  NS_ASSERT(m_cmdFrameId == ASSOCIATION_REQ);
  return m_capabilityInfo;
}

Mac16Address CommandPayloadHeader::GetCoordShortAddr() const {
  NS_ASSERT(m_cmdFrameId == COOR_REALIGN);
  return m_coordShortAddr;
}

uint8_t CommandPayloadHeader::GetChannel() const {
  NS_ASSERT(m_cmdFrameId == COOR_REALIGN);
  return m_logCh;
}

uint8_t CommandPayloadHeader::GetPage() const {
  NS_ASSERT(m_cmdFrameId == COOR_REALIGN);
  return m_logChPage;
}
