...
LrWpanCsmaCa::~LrWpanCsmaCa() { m_mac = nullptr; }
...
void LrWpanCsmaCa::SetMac(Ptr<LrWpanMac> mac) { m_mac = mac; }

Ptr<LrWpanMac> LrWpanCsmaCa::GetMac() const { return m_mac; }

void LrWpanCsmaCa::SetSlottedCsmaCa() {
  NS_LOG_FUNCTION(this);
  m_isSlotted = true;
}

void LrWpanCsmaCa::SetUnSlottedCsmaCa() {
  NS_LOG_FUNCTION(this);
  m_isSlotted = false;
}

bool LrWpanCsmaCa::IsSlottedCsmaCa() const {
  NS_LOG_FUNCTION(this);
  return (m_isSlotted);
}

bool LrWpanCsmaCa::IsUnSlottedCsmaCa() const {
  NS_LOG_FUNCTION(this);
  return (!m_isSlotted);
}
...
void LrWpanCsmaCa::SetMacMinBE(uint8_t macMinBE) {
  ...
  NS_ASSERT_MSG(macMinBE <= m_macMaxBE,
                "MacMinBE (" << macMinBE << ") should be <= MacMaxBE ("
                             << m_macMaxBE << ")");
  m_macMinBE = macMinBE;
}

uint8_t LrWpanCsmaCa::GetMacMinBE() const {
  NS_LOG_FUNCTION(this);
  return m_macMinBE;
}
...
uint8_t LrWpanCsmaCa::GetMacMaxBE() const {
  NS_LOG_FUNCTION(this);
  return m_macMaxBE;
}
...
uint8_t LrWpanCsmaCa::GetMacMaxCSMABackoffs() const {
  NS_LOG_FUNCTION(this);
  return m_macMaxCSMABackoffs;
}

Time LrWpanCsmaCa::GetTimeToNextSlot() const {
  NS_LOG_FUNCTION(this);
  ...
  uint64_t symbolRate = (uint64_t)m_mac->GetPhy()->GetDataOrSymbolRate(
      false); // symbols per second
  Time timeAtBoundary;
  ...
  if (m_coorDest) {
    ...
    Time beaconTime [[maybe_unused]] =
        Seconds((double)m_mac->m_rxBeaconSymbols / symbolRate);
    Time elapsedCap [[maybe_unused]] = elapsedSuperframe - beaconTime;
    NS_LOG_DEBUG("Elapsed incoming CAP symbols: "
                 << (elapsedCap.GetSeconds() * symbolRate) << " ("
  ...
  NS_LOG_DEBUG("Elapsed Superframe symbols: "
               << elapsedSuperframeSymbols << " ("
...
void LrWpanCsmaCa::Start() {
  ...
  if (IsSlottedCsmaCa()) {
    ...
    m_randomBackoffEvent = Simulator::Schedule(
        backoffBoundary, &LrWpanCsmaCa::RandomBackoffDelay, this);
  } else {
    m_BE = m_macMinBE;
...
void LrWpanCsmaCa::RandomBackoffDelay() {
  ...
  symbolRate = (uint64_t)m_mac->GetPhy()->GetDataOrSymbolRate(
      false); // symbols per second
  ...
  if (m_randomBackoffPeriodsLeft == 0 || IsUnSlottedCsmaCa()) {
    m_randomBackoffPeriodsLeft =
        (uint64_t)m_random->GetValue(0, upperBound + 1);
  }

  randomBackoff = Seconds(
      (double)(m_randomBackoffPeriodsLeft * lrwpan::aUnitBackoffPeriod) /
      symbolRate);
  ...
  if (IsUnSlottedCsmaCa()) {
    ...
    NS_LOG_DEBUG("Backoff periods left in CAP: "
                 << ((timeLeftInCap.GetSeconds() * symbolRate) /
                     lrwpan::aUnitBackoffPeriod)
                 << " (" << (timeLeftInCap.GetSeconds() * symbolRate)
                 << " symbols or " << timeLeftInCap.As(Time::S) << ")");

    if (randomBackoff >= timeLeftInCap) {
      uint64_t usedBackoffs =
          (double)(timeLeftInCap.GetSeconds() * symbolRate) /
          lrwpan::aUnitBackoffPeriod;
      m_randomBackoffPeriodsLeft -= usedBackoffs;
      NS_LOG_DEBUG("No time in CAP to complete backoff delay, deferring to the "
                   "next CAP");
      m_endCapEvent = Simulator::Schedule(
          timeLeftInCap, &LrWpanCsmaCa::DeferCsmaTimeout, this);
    } else {
      m_canProceedEvent =
...
Time LrWpanCsmaCa::GetTimeLeftInCap() {
  Time currentTime;
  uint64_t capSymbols;
  Time endCapTime;
  uint64_t activeSlot;
  ...
  if (m_coorDest) { // Take Incoming frame reference
    activeSlot = m_mac->m_incomingSuperframeDuration / 16;
    capSymbols = activeSlot * (m_mac->m_incomingFnlCapSlot + 1);
    endCapTime =
        m_mac->m_macBeaconRxTime + Seconds((double)capSymbols / symbolRate);
  } else { // Take Outgoing frame reference
    activeSlot = m_mac->m_superframeDuration / 16;
    capSymbols = activeSlot * (m_mac->m_fnlCapSlot + 1);
    endCapTime =
        m_mac->m_macBeaconTxTime + Seconds((double)capSymbols / symbolRate);
  }
...
void LrWpanCsmaCa::CanProceed() {
  ...
  NS_LOG_DEBUG("Total required transaction: "
  ...
  if (transactionTime > timeLeftInCap) {
    ...
    NS_LOG_DEBUG("Symbols left in CAP: "
                 << (timeLeftInCap.GetSeconds() * symbolRate) << " ("
                 << timeLeftInCap.As(Time::S) << ")");

    m_endCapEvent = Simulator::Schedule(timeLeftInCap,
                                        &LrWpanCsmaCa::DeferCsmaTimeout, this);
  } else {
...
void LrWpanCsmaCa::DeferCsmaTimeout() {
  NS_LOG_FUNCTION(this);
...
void LrWpanCsmaCa::SetLrWpanMacTransCostCallback(LrWpanMacTransCostCallback c) {
  NS_LOG_FUNCTION(this);
...
void LrWpanCsmaCa::SetBatteryLifeExtension(bool batteryLifeExtension) {
...
uint8_t LrWpanCsmaCa::GetNB() const { return m_NB; }

bool LrWpanCsmaCa::GetBatteryLifeExtension() const { return m_macBattLifeExt; }
