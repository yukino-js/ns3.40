...
public:
  ...
  enum MacCommand {
    ...
    DISASSOCIATION_NOTIF =
        0x03,              //!< Disassociation notification (RFD true: TX, Rx)
...
private:
  MacCommand
      m_cmdFrameId; //!< The command Frame Identifier (Used by all commands)
  ...
  uint8_t
      m_logChPage; //!< The channel page number (Coordinator realigment command)
  AssocStatus
      m_assocStatus; //!< Association Status (Association Response Command)
};
