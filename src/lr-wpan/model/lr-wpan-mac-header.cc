...
LrWpanMacHeader::LrWpanMacType LrWpanMacHeader::GetType() const {
...
bool LrWpanMacHeader::IsSecEnable() const { return (m_fctrlSecU == 1); }

bool LrWpanMacHeader::IsFrmPend() const { return (m_fctrlFrmPending == 1); }

bool LrWpanMacHeader::IsAckReq() const { return (m_fctrlAckReq == 1); }

bool LrWpanMacHeader::IsPanIdComp() const { return (m_fctrlPanIdComp == 1); }

uint8_t LrWpanMacHeader::GetFrmCtrlRes() const { return (m_fctrlReserved); }

uint8_t LrWpanMacHeader::GetDstAddrMode() const { return m_fctrlDstAddrMode; }

uint8_t LrWpanMacHeader::GetFrameVer() const { return m_fctrlFrmVer; }

uint8_t LrWpanMacHeader::GetSrcAddrMode() const { return m_fctrlSrcAddrMode; }

uint8_t LrWpanMacHeader::GetSeqNum() const { return (m_SeqNum); }

uint16_t LrWpanMacHeader::GetDstPanId() const { return (m_addrDstPanId); }

Mac16Address LrWpanMacHeader::GetShortDstAddr() const {
  return (m_addrShortDstAddr);
}

Mac64Address LrWpanMacHeader::GetExtDstAddr() const {
  return (m_addrExtDstAddr);
}

uint16_t LrWpanMacHeader::GetSrcPanId() const { return (m_addrSrcPanId); }

Mac16Address LrWpanMacHeader::GetShortSrcAddr() const {
  return (m_addrShortSrcAddr);
}

Mac64Address LrWpanMacHeader::GetExtSrcAddr() const {
...
uint32_t LrWpanMacHeader::GetFrmCounter() const { return (m_auxFrmCntr); }

uint8_t LrWpanMacHeader::GetSecLevel() const { return (m_secctrlSecLevel); }

uint8_t LrWpanMacHeader::GetKeyIdMode() const { return (m_secctrlKeyIdMode); }

uint8_t LrWpanMacHeader::GetSecCtrlReserved() const {
  return (m_secctrlReserved);
}

uint32_t LrWpanMacHeader::GetKeyIdSrc32() const { return (m_auxKeyIdKeySrc32); }

uint64_t LrWpanMacHeader::GetKeyIdSrc64() const { return (m_auxKeyIdKeySrc64); }

uint8_t LrWpanMacHeader::GetKeyIdIndex() const { return (m_auxKeyIdKeyIndex); }

bool LrWpanMacHeader::IsBeacon() const {
  return (m_fctrlFrmType == LRWPAN_MAC_BEACON);
}

bool LrWpanMacHeader::IsData() const {
  return (m_fctrlFrmType == LRWPAN_MAC_DATA);
}

bool LrWpanMacHeader::IsAcknowledgment() const {
  return (m_fctrlFrmType == LRWPAN_MAC_ACKNOWLEDGMENT);
}

bool LrWpanMacHeader::IsCommand() const {
  return (m_fctrlFrmType == LRWPAN_MAC_COMMAND);
}

void LrWpanMacHeader::SetType(LrWpanMacType wpanMacType) {
...
void LrWpanMacHeader::SetSecDisable() { m_fctrlSecU = 0; }

void LrWpanMacHeader::SetFrmPend() { m_fctrlFrmPending = 1; }

void LrWpanMacHeader::SetNoFrmPend() { m_fctrlFrmPending = 0; }

void LrWpanMacHeader::SetAckReq() { m_fctrlAckReq = 1; }

void LrWpanMacHeader::SetNoAckReq() { m_fctrlAckReq = 0; }

void LrWpanMacHeader::SetPanIdComp() { m_fctrlPanIdComp = 1; }

void LrWpanMacHeader::SetNoPanIdComp() { m_fctrlPanIdComp = 0; }

void LrWpanMacHeader::SetFrmCtrlRes(uint8_t res) { m_fctrlReserved = res; }

void LrWpanMacHeader::SetDstAddrMode(uint8_t addrMode) {
  m_fctrlDstAddrMode = addrMode;
}

void LrWpanMacHeader::SetFrameVer(uint8_t ver) { m_fctrlFrmVer = ver; }

void LrWpanMacHeader::SetSrcAddrMode(uint8_t addrMode) {
  m_fctrlSrcAddrMode = addrMode;
}

void LrWpanMacHeader::SetSeqNum(uint8_t seqNum) { m_SeqNum = seqNum; }
...
TypeId LrWpanMacHeader::GetTypeId() {
...
TypeId LrWpanMacHeader::GetInstanceTypeId() const { return GetTypeId(); }

void LrWpanMacHeader::Print(std::ostream &os) const {
