...
std::complex<double> JakesProcess::Oscillator::GetValueAt(Time at) const {
  return (m_amplitude * std::cos(at.GetSeconds() * m_omega + m_phase));
}

NS_OBJECT_ENSURE_REGISTERED(JakesProcess);

TypeId JakesProcess::GetTypeId() {
  static TypeId tid =
...
void JakesProcess::SetPropagationLossModel(
    Ptr<const PropagationLossModel> propagationModel) {
...
void JakesProcess::SetNOscillators(unsigned int nOscillators) {
  m_nOscillators = nOscillators;
}

void JakesProcess::SetDopplerFrequencyHz(double dopplerFrequencyHz) {
...
JakesProcess::~JakesProcess() { m_oscillators.clear(); }

void JakesProcess::DoDispose() {
  m_uniformVariable = nullptr;
  m_jakes = nullptr;
}

std::complex<double> JakesProcess::GetComplexGain() const {
...
double JakesProcess::GetChannelGainDb() const {
  std::complex<double> complexGain = GetComplexGain();
  return (10 * std::log10((std::pow(complexGain.real(), 2) +
                           std::pow(complexGain.imag(), 2)) /
                          2));
}
