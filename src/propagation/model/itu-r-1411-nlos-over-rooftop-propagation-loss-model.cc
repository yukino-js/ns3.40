...
TypeId ItuR1411NlosOverRooftopPropagationLossModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ItuR1411NlosOverRooftopPropagationLossModel")
          ...
          .AddAttribute(
              ...
              MakeEnumChecker(SmallCity, "Small", MediumCity, "Medium",
                              LargeCity, "Large"))
          .AddAttribute(
              "RooftopLevel", "The height of the rooftop level in meters",
              DoubleValue(20.0),
              MakeDoubleAccessor(&ItuR1411NlosOverRooftopPropagationLossModel::
                                     m_rooftopHeight),
...
double ItuR1411NlosOverRooftopPropagationLossModel::GetLoss(
  ...
  double hb = (a->GetPosition().z > b->GetPosition().z ? a->GetPosition().z
                                                       : b->GetPosition().z);
  double hm = (a->GetPosition().z < b->GetPosition().z ? a->GetPosition().z
                                                       : b->GetPosition().z);
  ...
  double Lmsd = 0.0;
  NS_LOG_LOGIC(this << " build " << m_buildingsExtend << " ds " << ds
                    << " roof " << m_rooftopHeight << " hb " << hb << " lambda "
                    << m_lambda);
  if (ds < m_buildingsExtend) {
    ...
    if (fmhz > 2000) {
      kf = -8;
    } else if ((m_environment == UrbanEnvironment) &&
               (m_citySize == LargeCity)) {
      kf = -4 + 0.7 * (fmhz / 925.0 - 1);
    } else {
      kf = -4 + 1.5 * (fmhz / 925.0 - 1);
    }

    Lmsd = Lbsh + ka + kd * std::log10(distance / 1000.0) +
           kf * std::log10(fmhz) - 9.0 * std::log10(m_buildingSeparation);
  } else {
    double theta = std::atan(Dhb / m_buildingSeparation);
    double rho =
        std::sqrt(Dhb * Dhb + m_buildingSeparation * m_buildingSeparation);
    double Qm = 0.0;
    if ((hb > m_rooftopHeight - 1.0) && (hb < m_rooftopHeight + 1.0)) {
      ...
      Qm =
          2.35 *
          pow(Dhb / distance * std::sqrt(m_buildingSeparation / m_lambda), 0.9);
    } else {
      Qm = m_buildingSeparation / (2 * M_PI * distance) *
           std::sqrt(m_lambda / rho) * (1 / theta - (1 / (2 * M_PI + theta)));
    }
  ...
  NS_LOG_LOGIC(this << " Lbf " << Lbf << " Lrts " << Lrts << " Dhm" << Dhm
                    << " Lmsd " << Lmsd);
  double loss = 0.0;
  if (Lrts + Lmsd > 0) {
    loss = Lbf + Lrts + Lmsd;
...
void ItuR1411NlosOverRooftopPropagationLossModel::SetFrequency(double freq) {
  m_frequency = freq;
  m_lambda = 299792458.0 / freq;
}

double ItuR1411NlosOverRooftopPropagationLossModel::DoCalcRxPower(
