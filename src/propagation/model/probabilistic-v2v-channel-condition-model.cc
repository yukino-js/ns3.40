...
TypeId ProbabilisticV2vUrbanChannelConditionModel::GetTypeId() {
  static TypeId tid =
...
double ProbabilisticV2vUrbanChannelConditionModel::ComputePnlos(
  ...
  case VehicleDensity::LOW:
    pNlosv = std::min(
        1.0, std::max(0.0, 1 / (0.0396 * distance2D) *
                               exp(-(log(distance2D) - 5.2718) *
                                   (log(distance2D) - 5.2718) / 3.4827)));
    break;
  case VehicleDensity::MEDIUM:
    pNlosv = std::min(
        1.0, std::max(0.0, 1 / (0.0312 * distance2D) *
                               exp(-(log(distance2D) - 5.0063) *
                                   (log(distance2D) - 5.0063) / 2.4544)));
    break;
  case VehicleDensity::HIGH:
    pNlosv = std::min(
        1.0, std::max(0.0, 1 / (0.0242 * distance2D) *
                               exp(-(log(distance2D) - 5.0115) *
                                   (log(distance2D) - 5.0115) / 2.2092)));
    break;
  default:
    NS_FATAL_ERROR("Undefined density, choose between Low, Medium and High");
  }
...
TypeId ProbabilisticV2vHighwayChannelConditionModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ProbabilisticV2vHighwayChannelConditionModel")
          ...
          .AddAttribute(
              ...
              "It can be set to Low, Medium or High.",
              EnumValue(VehicleDensity::LOW),
              MakeEnumAccessor(&ProbabilisticV2vHighwayChannelConditionModel::
                                   m_densityHighway),
              MakeEnumChecker(VehicleDensity::LOW, "Low",
                              VehicleDensity::MEDIUM, "Medium",
                              VehicleDensity::HIGH, "High"));
...
double ProbabilisticV2vHighwayChannelConditionModel::ComputePlos(
  ...
  double pLos = std::min(1.0, std::max(0.0, aLos * distance2D * distance2D +
                                                bLos * distance2D + cLos));
...
double ProbabilisticV2vHighwayChannelConditionModel::ComputePnlos(
  ...
  double pNlos = std::min(1.0, std::max(0.0, aNlos * pow(distance2D, 2) +
                                                 bNlos * distance2D + cNlos));
