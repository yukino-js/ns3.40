...
TypeId ThreeGppPropagationLossModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThreeGppPropagationLossModel")
          .SetParent<PropagationLossModel>()
          .SetGroupName("Propagation")
          .AddAttribute(
              "Frequency", "The centre frequency in Hz.", DoubleValue(500.0e6),
          ...
          .AddAttribute("ShadowingEnabled", "Enable/disable shadowing.",
                        BooleanValue(true),
                        MakeBooleanAccessor(
                            &ThreeGppPropagationLossModel::m_shadowingEnabled),
                        MakeBooleanChecker())
          .AddAttribute(
              "ChannelConditionModel",
              "Pointer to the channel condition model.", PointerValue(),
              MakePointerAccessor(
                  &ThreeGppPropagationLossModel::SetChannelConditionModel,
          ...
          .AddAttribute(
              ...
              BooleanValue(false),
              MakeBooleanAccessor(
                  &ThreeGppPropagationLossModel::m_enforceRanges),
              MakeBooleanChecker())
          .AddAttribute(
              "BuildingPenetrationLossesEnabled",
              "Enable/disable Building Penetration Losses.", BooleanValue(true),
              MakeBooleanAccessor(
                  &ThreeGppPropagationLossModel::m_buildingPenLossesEnabled),
              MakeBooleanChecker());
  return tid;
}
...
void ThreeGppPropagationLossModel::DoDispose() {
  m_channelConditionModel->Dispose();
  m_channelConditionModel = nullptr;
  m_shadowingMap.clear();
}

void ThreeGppPropagationLossModel::SetChannelConditionModel(
    Ptr<ChannelConditionModel> model) {
  NS_LOG_FUNCTION(this);
...
void ThreeGppPropagationLossModel::SetFrequency(double f) {
  NS_LOG_FUNCTION(this);
...
double ThreeGppPropagationLossModel::GetFrequency() const {
  NS_LOG_FUNCTION(this);
  return m_frequency;
}

bool ThreeGppPropagationLossModel::IsO2iLowPenetrationLoss(
    Ptr<const ChannelCondition> cond) const {
  return DoIsO2iLowPenetrationLoss(cond);
}

double ThreeGppPropagationLossModel::DoCalcRxPower(double txPowerDbm,
                                                   Ptr<MobilityModel> a,
...
double ThreeGppPropagationLossModel::GetLoss(Ptr<ChannelCondition> cond,
                                             double distance2d,
                                             double distance3d, double hUt,
                                             double hBs) const {
  NS_LOG_FUNCTION(this);
...
double ThreeGppPropagationLossModel::GetO2iLowPenetrationLoss(
...
double ThreeGppPropagationLossModel::GetO2iHighPenetrationLoss(
    Ptr<MobilityModel> a, Ptr<MobilityModel> b,
    ChannelCondition::LosConditionValue cond) const {
  NS_LOG_FUNCTION(this);
...
bool ThreeGppPropagationLossModel::DoIsO2iLowPenetrationLoss(
    Ptr<const ChannelCondition> cond) const {
  if (cond->GetO2iLowHighCondition() ==
      ChannelCondition::O2iLowHighConditionValue::LOW) {
    return true;
  } else if (cond->GetO2iLowHighCondition() ==
...
double ThreeGppPropagationLossModel::GetLossNlosv(double distance2D,
                                                  double distance3D, double hUt,
                                                  double hBs) const {
  NS_LOG_FUNCTION(this);
...
double ThreeGppPropagationLossModel::GetShadowing(
    Ptr<MobilityModel> a, Ptr<MobilityModel> b,
    ChannelCondition::LosConditionValue cond) const {
  NS_LOG_FUNCTION(this);
  ...
  if (notFound || newCondition) {
    ...
    double R = exp(-1 * displacement.GetLength() /
                   GetShadowingCorrelationDistance(cond));
    shadowingValue = R * it->second.m_shadowing +
                     sqrt(1 - R * R) * m_normRandomVariable->GetValue() *
...
int64_t ThreeGppPropagationLossModel::DoAssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this);
...
double ThreeGppPropagationLossModel::Calculate2dDistance(Vector a, Vector b) {
  double x = a.x - b.x;
  double y = a.y - b.y;
...
Vector ThreeGppPropagationLossModel::GetVectorDifference(Ptr<MobilityModel> a,
                                                         Ptr<MobilityModel> b) {
...
TypeId ThreeGppRmaPropagationLossModel::GetTypeId() {
  static TypeId tid =
...
double ThreeGppRmaPropagationLossModel::GetLossLos(double distance2D,
                                                   double distance3D,
                                                   double hUt,
                                                   double hBs) const {
  NS_LOG_FUNCTION(this);
...
double ThreeGppRmaPropagationLossModel::GetLossNlos(double distance2D,
                                                    double distance3D,
                                                    double hUt,
                                                    double hBs) const {
  NS_LOG_FUNCTION(this);
  ...
  double plNlos = 161.04 - 7.1 * log10(m_w) + 7.5 * log10(m_h) -
                  ...
                  20.0 * log10(m_frequency / 1e9) -
                  (3.2 * pow(log10(11.75 * hUt), 2) - 4.97);
...
double ThreeGppRmaPropagationLossModel::GetShadowingStd(
    Ptr<MobilityModel> a, Ptr<MobilityModel> b,
    ChannelCondition::LosConditionValue cond) const {
  NS_LOG_FUNCTION(this);
  double shadowingStd;
...
double ThreeGppRmaPropagationLossModel::GetShadowingCorrelationDistance(
    ChannelCondition::LosConditionValue cond) const {
  NS_LOG_FUNCTION(this);
...
double ThreeGppRmaPropagationLossModel::GetBpDistance(double frequency,
                                                      double hA, double hB) {
...
TypeId ThreeGppUmaPropagationLossModel::GetTypeId() {
...
double ThreeGppUmaPropagationLossModel::GetBpDistance(double hUt, double hBs,
...
double ThreeGppUmaPropagationLossModel::GetLossLos(double distance2D,
                                                   double distance3D,
                                                   double hUt,
                                                   double hBs) const {
  NS_LOG_FUNCTION(this);
...
double ThreeGppUmaPropagationLossModel::GetLossNlos(double distance2D,
                                                    double distance3D,
                                                    double hUt,
                                                    double hBs) const {
  NS_LOG_FUNCTION(this);
...
double ThreeGppUmaPropagationLossModel::GetShadowingCorrelationDistance(
    ChannelCondition::LosConditionValue cond) const {
  NS_LOG_FUNCTION(this);
  double correlationDistance;
...
int64_t ThreeGppUmaPropagationLossModel::DoAssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this);
...
TypeId ThreeGppUmiStreetCanyonPropagationLossModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThreeGppUmiStreetCanyonPropagationLossModel")
          .SetParent<ThreeGppPropagationLossModel>()
          .SetGroupName("Propagation")
...
double ThreeGppUmiStreetCanyonPropagationLossModel::GetLossLos(
...
double ThreeGppUmiStreetCanyonPropagationLossModel::GetLossNlos(
    double distance2D, double distance3D, double hUt, double hBs) const {
  NS_LOG_FUNCTION(this);
...
TypeId ThreeGppIndoorOfficePropagationLossModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThreeGppIndoorOfficePropagationLossModel")
          .SetParent<ThreeGppPropagationLossModel>()
          .SetGroupName("Propagation")
...
double ThreeGppIndoorOfficePropagationLossModel::GetO2iDistance2dIn() const {
  return 0;
}
...
double ThreeGppIndoorOfficePropagationLossModel::GetLossNlos(double distance2D,
                                                             double distance3D,
                                                             double hUt,
                                                             double hBs) const {
  NS_LOG_FUNCTION(this);
