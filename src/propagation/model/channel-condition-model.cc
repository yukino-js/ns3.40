...
TypeId ChannelCondition::GetTypeId() {
...
ChannelCondition::LosConditionValue ChannelCondition::GetLosCondition() const {
  return m_losCondition;
}

void ChannelCondition::SetLosCondition(LosConditionValue cond) {
  m_losCondition = cond;
}

ChannelCondition::O2iConditionValue ChannelCondition::GetO2iCondition() const {
  return m_o2iCondition;
}

void ChannelCondition::SetO2iCondition(O2iConditionValue o2iCondition) {
  m_o2iCondition = o2iCondition;
}
...
void ChannelCondition::SetO2iLowHighCondition(
    O2iLowHighConditionValue o2iLowHighCondition) {
  m_o2iLowHighCondition = o2iLowHighCondition;
}

bool ChannelCondition::IsLos() const {
  return (m_losCondition == ChannelCondition::LOS);
}

bool ChannelCondition::IsNlos() const {
  return (m_losCondition == ChannelCondition::NLOS);
}

bool ChannelCondition::IsNlosv() const {
  return (m_losCondition == ChannelCondition::NLOSv);
}

bool ChannelCondition::IsO2i() const {
  return (m_o2iCondition == ChannelCondition::O2I);
}

bool ChannelCondition::IsO2o() const {
  return (m_o2iCondition == ChannelCondition::O2O);
}

bool ChannelCondition::IsI2i() const {
  return (m_o2iCondition == ChannelCondition::I2I);
}

bool ChannelCondition::IsEqual(LosConditionValue losCondition,
...
TypeId AlwaysLosChannelConditionModel::GetTypeId() {
...
TypeId NeverLosChannelConditionModel::GetTypeId() {
...
TypeId NeverLosVehicleChannelConditionModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::NeverLosVehicleChannelConditionModel")
...
TypeId ThreeGppChannelConditionModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThreeGppChannelConditionModel")
          ...
          .AddAttribute("O2iThreshold",
                        ...
                        DoubleValue(0.0),
                        MakeDoubleAccessor(
                            &ThreeGppChannelConditionModel::m_o2iThreshold),
          ...
          .AddAttribute(
              ...
              DoubleValue(1.0),
              MakeDoubleAccessor(
                  &ThreeGppChannelConditionModel::m_o2iLowLossThreshold),
              MakeDoubleChecker<double>(0, 1))
          .AddAttribute(
              ...
              "otherwise it is O2I.",
              BooleanValue(false),
              MakeBooleanAccessor(&ThreeGppChannelConditionModel::
...
Ptr<ChannelCondition> ThreeGppChannelConditionModel::GetChannelCondition(
    Ptr<const MobilityModel> a, Ptr<const MobilityModel> b) const {
  ...
  if (notFound || update) {
    ...
    const_cast<ThreeGppChannelConditionModel *>(this)
        ->m_channelConditionMap[key] = mapItem;
  }
...
Ptr<ChannelCondition> ThreeGppChannelConditionModel::ComputeChannelCondition(
    Ptr<const MobilityModel> a, Ptr<const MobilityModel> b) const {
...
int64_t ThreeGppChannelConditionModel::AssignStreams(int64_t stream) {
...
double ThreeGppChannelConditionModel::Calculate2dDistance(const Vector &a,
                                                          const Vector &b) {
  double x = a.x - b.x;
  double y = a.y - b.y;
...
TypeId ThreeGppRmaChannelConditionModel::GetTypeId() {
...
TypeId ThreeGppUmaChannelConditionModel::GetTypeId() {
...
double ThreeGppUmaChannelConditionModel::ComputePlos(
  ...
  if (distance2D <= 18.0) {
    ...
    pLos = (18.0 / distance2D +
            exp(-distance2D / 63.0) * (1.0 - 18.0 / distance2D)) *
           (1.0 + c * 5.0 / 4.0 * pow(distance2D / 100.0, 3.0) *
                      exp(-distance2D / 150.0));
  }
...
TypeId ThreeGppUmiStreetCanyonChannelConditionModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThreeGppUmiStreetCanyonChannelConditionModel")
          .SetParent<ThreeGppChannelConditionModel>()
          .SetGroupName("Propagation")
...
double ThreeGppUmiStreetCanyonChannelConditionModel::ComputePlos(
  ...
  if (distance2D <= 18.0) {
    ...
    pLos =
        18.0 / distance2D + exp(-distance2D / 36.0) * (1.0 - 18.0 / distance2D);
  }
...
TypeId ThreeGppIndoorMixedOfficeChannelConditionModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThreeGppIndoorMixedOfficeChannelConditionModel")
          .SetParent<ThreeGppChannelConditionModel>()
          .SetGroupName("Propagation")
...
TypeId ThreeGppIndoorOpenOfficeChannelConditionModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThreeGppIndoorOpenOfficeChannelConditionModel")
          .SetParent<ThreeGppChannelConditionModel>()
          .SetGroupName("Propagation")
