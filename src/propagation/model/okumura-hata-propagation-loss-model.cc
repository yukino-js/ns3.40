...
TypeId OkumuraHataPropagationLossModel::GetTypeId() {
  static TypeId tid =
...
double OkumuraHataPropagationLossModel::GetLoss(Ptr<MobilityModel> a,
  ...
  if (m_frequency <= 1.500e9) {
    ...
    NS_LOG_INFO(this << " logf " << 26.16 * log_fMhz << " loga " << log_aHeight
                     << " X " << ((44.9 - (6.55 * log_hb)) * std::log10(distKm))
                     << " logb " << log_bHeight);
...
double OkumuraHataPropagationLossModel::DoCalcRxPower(
    double txPowerDbm, Ptr<MobilityModel> a, Ptr<MobilityModel> b) const {
  return (txPowerDbm - GetLoss(a, b));
}

int64_t OkumuraHataPropagationLossModel::DoAssignStreams(int64_t stream) {
  return 0;
}
