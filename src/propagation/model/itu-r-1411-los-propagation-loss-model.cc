...
TypeId ItuR1411LosPropagationLossModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ItuR1411LosPropagationLossModel")
          .SetParent<PropagationLossModel>()
          .SetGroupName("Propagation")
          .AddConstructor<ItuR1411LosPropagationLossModel>()
          .AddAttribute("Frequency", "The propagation frequency in Hz",
                        DoubleValue(2160e6),
                        MakeDoubleAccessor(
...
double ItuR1411LosPropagationLossModel::GetLoss(Ptr<MobilityModel> a,
...
void ItuR1411LosPropagationLossModel::SetFrequency(double freq) {
  NS_ASSERT(freq > 0.0);
  m_lambda = 299792458.0 / freq;
}

double ItuR1411LosPropagationLossModel::DoCalcRxPower(
    double txPowerDbm, Ptr<MobilityModel> a, Ptr<MobilityModel> b) const {
  return (txPowerDbm - GetLoss(a, b));
}

int64_t ItuR1411LosPropagationLossModel::DoAssignStreams(int64_t stream) {
  return 0;
}
} // namespace ns3
