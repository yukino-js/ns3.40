...
Ipv6ListRoutingHelper::Ipv6ListRoutingHelper(const Ipv6ListRoutingHelper &o) {
  for (auto i = o.m_list.begin(); i != o.m_list.end(); ++i) {
    m_list.emplace_back(const_cast<const Ipv6RoutingHelper *>(i->first->Copy()),
                        i->second);
  }
}

Ipv6ListRoutingHelper *Ipv6ListRoutingHelper::Copy() const {
  return new Ipv6ListRoutingHelper(*this);
}

void Ipv6ListRoutingHelper::Add(const Ipv6RoutingHelper &routing,
                                int16_t priority) {
  m_list.emplace_back(const_cast<const Ipv6RoutingHelper *>(routing.Copy()),
                      priority);
}

Ptr<Ipv6RoutingProtocol> Ipv6ListRoutingHelper::Create(Ptr<Node> node) const {
