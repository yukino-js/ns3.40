...
NeighborCacheHelper::NeighborCacheHelper() { NS_LOG_FUNCTION(this); }

NeighborCacheHelper::~NeighborCacheHelper() { NS_LOG_FUNCTION(this); }

void NeighborCacheHelper::PopulateNeighborCache() {
...
void NeighborCacheHelper::PopulateNeighborCache(Ptr<Channel> channel) const {
  ...
  for (std::size_t i = 0; i < channel->GetNDevices(); ++i) {
    ...
    if (node->GetObject<Ipv4>()) {
      ipv4InterfaceIndex =
          node->GetObject<Ipv4>()->GetInterfaceForDevice(netDevice);
    }
    int32_t ipv6InterfaceIndex = -1;
    if (node->GetObject<Ipv6>()) {
      ipv6InterfaceIndex =
          node->GetObject<Ipv6>()->GetInterfaceForDevice(netDevice);
    }
    ...
    for (std::size_t j = 0; j < channel->GetNDevices(); ++j) {
      ...
      if (neighborNode->GetObject<Ipv4>()) {
        ipv4NeighborInterfaceIndex =
            neighborNode->GetObject<Ipv4>()->GetInterfaceForDevice(
                neighborDevice);
      }
      int32_t ipv6NeighborInterfaceIndex = -1;
      if (neighborNode->GetObject<Ipv6>()) {
        ipv6NeighborInterfaceIndex =
            neighborNode->GetObject<Ipv6>()->GetInterfaceForDevice(
                neighborDevice);
      }
      ...
      if (neighborDevice != netDevice) {
        if (ipv4InterfaceIndex != -1) {
          Ptr<Ipv4Interface> ipv4Interface =
              node->GetObject<Ipv4L3Protocol>()->GetInterface(
                  ipv4InterfaceIndex);
        ...
        if (ipv6InterfaceIndex != -1) {
          Ptr<Ipv6Interface> ipv6Interface =
              node->GetObject<Ipv6L3Protocol>()->GetInterface(
                  ipv6InterfaceIndex);
...
void NeighborCacheHelper::PopulateNeighborCache(
    const NetDeviceContainer &c) const {
  NS_LOG_FUNCTION(this);
...
void NeighborCacheHelper::PopulateNeighborCache(
    const Ipv4InterfaceContainer &c) const {
  NS_LOG_FUNCTION(this);
  for (uint32_t i = 0; i < c.GetN(); ++i) {
    ...
    Ptr<Ipv4Interface> ipv4Interface =
        DynamicCast<Ipv4L3Protocol>(ipv4)->GetInterface(index);
    if (ipv4Interface) {
...
void NeighborCacheHelper::PopulateNeighborCache(
    const Ipv6InterfaceContainer &c) const {
  NS_LOG_FUNCTION(this);
  for (uint32_t i = 0; i < c.GetN(); ++i) {
    ...
    uint32_t index = returnValue.second;
    Ptr<Ipv6Interface> ipv6Interface =
        DynamicCast<Ipv6L3Protocol>(ipv6)->GetInterface(index);
    if (ipv6Interface) {
...
void NeighborCacheHelper::PopulateNeighborEntriesIpv4(
    Ptr<Ipv4Interface> ipv4Interface,
  ...
  if (m_dynamicNeighborCache) {
    ipv4Interface->RemoveAddressCallback(MakeCallback(
        &NeighborCacheHelper::UpdateCacheByIpv4AddressRemoved, this));
    if (m_globalNeighborCache) {
      ipv4Interface->AddAddressCallback(MakeCallback(
          &NeighborCacheHelper::UpdateCacheByIpv4AddressAdded, this));
    }
  }
...
void NeighborCacheHelper::PopulateNeighborEntriesIpv6(
    Ptr<Ipv6Interface> ipv6Interface,
    Ptr<Ipv6Interface> neighborDeviceInterface) const {
  uint32_t netDeviceAddresses = ipv6Interface->GetNAddresses();
  uint32_t neighborDeviceAddresses = neighborDeviceInterface->GetNAddresses();
  if (m_dynamicNeighborCache) {
    ipv6Interface->RemoveAddressCallback(MakeCallback(
        &NeighborCacheHelper::UpdateCacheByIpv6AddressRemoved, this));
    if (m_globalNeighborCache) {
      ipv6Interface->AddAddressCallback(MakeCallback(
          &NeighborCacheHelper::UpdateCacheByIpv6AddressAdded, this));
    }
  }
  for (uint32_t n = 0; n < netDeviceAddresses; ++n) {
...
void NeighborCacheHelper::AddEntry(Ptr<Ipv4Interface> netDeviceInterface,
...
void NeighborCacheHelper::AddEntry(Ptr<Ipv6Interface> netDeviceInterface,
...
void NeighborCacheHelper::FlushAutoGenerated() const {
  NS_LOG_FUNCTION(this);
  for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i) {
    Ptr<Node> node = NodeList::GetNode(i);
    for (uint32_t j = 0; j < node->GetNDevices(); ++j) {
      Ptr<NetDevice> netDevice = node->GetDevice(j);
      int32_t ipv4InterfaceIndex =
          node->GetObject<Ipv4>()->GetInterfaceForDevice(netDevice);
      int32_t ipv6InterfaceIndex =
          node->GetObject<Ipv6>()->GetInterfaceForDevice(netDevice);
      if (ipv4InterfaceIndex != -1) {
        Ptr<Ipv4Interface> ipv4Interface =
...
void NeighborCacheHelper::UpdateCacheByIpv4AddressRemoved(
    const Ptr<Ipv4Interface> interface,
...
void NeighborCacheHelper::UpdateCacheByIpv4AddressAdded(
    const Ptr<Ipv4Interface> interface,
    const Ipv4InterfaceAddress ifAddr) const {
  NS_LOG_FUNCTION(this);
  Ptr<NetDevice> netDevice = interface->GetDevice();
...
void NeighborCacheHelper::UpdateCacheByIpv6AddressRemoved(
    const Ptr<Ipv6Interface> interface,
...
void NeighborCacheHelper::UpdateCacheByIpv6AddressAdded(
    const Ptr<Ipv6Interface> interface,
    const Ipv6InterfaceAddress ifAddr) const {
  NS_LOG_FUNCTION(this);
  Ptr<NetDevice> netDevice = interface->GetDevice();
...
void NeighborCacheHelper::SetDynamicNeighborCache(bool enable) {
  NS_LOG_FUNCTION(this);
