...
RipNgHelper::RipNgHelper() { m_factory.SetTypeId("ns3::RipNg"); }
...
RipNgHelper *RipNgHelper::Copy() const { return new RipNgHelper(*this); }

Ptr<Ipv6RoutingProtocol> RipNgHelper::Create(Ptr<Node> node) const {
...
void RipNgHelper::Set(std::string name, const AttributeValue &value) {
  m_factory.Set(name, value);
}

int64_t RipNgHelper::AssignStreams(NodeContainer c, int64_t stream) {
...
void RipNgHelper::SetDefaultRouter(Ptr<Node> node, Ipv6Address nextHop,
                                   uint32_t interface) {
  Ptr<Ipv6> ipv6 = node->GetObject<Ipv6>();
  NS_ASSERT_MSG(ipv6, "Ipv6 not installed on node");
  Ptr<Ipv6RoutingProtocol> proto = ipv6->GetRoutingProtocol();
...
void RipNgHelper::ExcludeInterface(Ptr<Node> node, uint32_t interface) {
  auto it = m_interfaceExclusions.find(node);
...
void RipNgHelper::SetInterfaceMetric(Ptr<Node> node, uint32_t interface,
                                     uint8_t metric) {
