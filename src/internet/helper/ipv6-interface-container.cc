...
Ipv6InterfaceContainer::Iterator Ipv6InterfaceContainer::Begin() const {
  return m_interfaces.begin();
}

Ipv6InterfaceContainer::Iterator Ipv6InterfaceContainer::End() const {
  return m_interfaces.end();
}

uint32_t Ipv6InterfaceContainer::GetN() const { return m_interfaces.size(); }
...
std::pair<Ptr<Ipv6>, uint32_t> Ipv6InterfaceContainer::Get(uint32_t i) const {
...
void Ipv6InterfaceContainer::SetDefaultRouteInAllNodes(uint32_t router) {
  ...
  for (other = 0; other < m_interfaces.size(); other++) {
    if (other != router) {
      ...
      NS_ASSERT_MSG(routing, "Default router setup failed because no "
...
void Ipv6InterfaceContainer::SetDefaultRouteInAllNodes(
...
void Ipv6InterfaceContainer::SetDefaultRoute(uint32_t i,
...
Ipv6Address Ipv6InterfaceContainer::GetLinkLocalAddress(Ipv6Address address) {
  ...
  for (uint32_t i = 0; i < ipv6->GetNAddresses(m_interfaces[nodeIndex].second);
       i++) {
    Ipv6InterfaceAddress iAddress;
