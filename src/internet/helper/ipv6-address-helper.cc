...
Ipv6AddressHelper::Ipv6AddressHelper(Ipv6Address network, Ipv6Prefix prefix,
  ...
  NS_ASSERT_MSG(m_network == network.CombinePrefix(prefix),
                "Ipv6AddressHelper: network address and prefix mismatch: "
                    << m_network << " " << m_prefix);

  NS_ASSERT_MSG(base.CombinePrefix(prefix) == Ipv6Address::GetZero(),
                "Ipv6AddressHelper: base address and prefix mismatch: "
                    << base << " " << m_prefix);
}
...
void Ipv6AddressHelper::SetBase(Ipv6Address network, Ipv6Prefix prefix,
  ...
  NS_ASSERT_MSG(
      m_network == network.CombinePrefix(prefix),
      "Ipv6AddressHelper::SetBase(): network address and prefix mismatch: "
          << m_network << " " << m_prefix);

  NS_ASSERT_MSG(
      base.CombinePrefix(prefix) == Ipv6Address::GetZero(),
      "Ipv6AddressHelper::SetBase(): base address and prefix mismatch: "
          << base << " " << m_prefix);
}
...
Ipv6Address Ipv6AddressHelper::NewAddress() {
  ...
  NS_ASSERT_MSG(
      m_address.CombinePrefix(m_prefix) == Ipv6Address::GetZero(),
...
void Ipv6AddressHelper::NewNetwork() {
  ...
  uint16_t sum[16];
  for (uint8_t index = 0; index < 16; index++) {
    sum[index] = static_cast<uint16_t>(netBuf[index]) +
                 static_cast<uint16_t>(addBuf[index]);
  ...
  for (uint32_t i = 0; i < c.GetN(); ++i) {
    ...
    if (withConfiguration.at(i)) {
      Ipv6InterfaceAddress ipv6Addr = Ipv6InterfaceAddress(
          NewAddress(device->GetAddress()), Ipv6Prefix(64), onLink.at(i));
    ...
    if (tc && !DynamicCast<LoopbackNetDevice>(device) &&
      ...
      if (ndqi) {
        ...
        TrafficControlHelper tcHelper =
            TrafficControlHelper::Default(nTxQueues);
