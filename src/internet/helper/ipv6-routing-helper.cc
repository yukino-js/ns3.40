...
void Ipv6RoutingHelper::PrintRoutingTableAllAt(Time printTime,
...
void Ipv6RoutingHelper::PrintRoutingTableAllEvery(
    Time printInterval, Ptr<OutputStreamWrapper> stream, Time::Unit unit) {
  for (uint32_t i = 0; i < NodeList::GetNNodes(); i++) {
    Ptr<Node> node = NodeList::GetNode(i);
    Simulator::Schedule(printInterval, &Ipv6RoutingHelper::PrintEvery,
                        printInterval, node, stream, unit);
  }
}

void Ipv6RoutingHelper::PrintRoutingTableAt(Time printTime, Ptr<Node> node,
                                            Ptr<OutputStreamWrapper> stream,
                                            Time::Unit unit) {
  Simulator::Schedule(printTime, &Ipv6RoutingHelper::Print, node, stream, unit);
}

void Ipv6RoutingHelper::PrintRoutingTableEvery(Time printInterval,
                                               Ptr<Node> node,
                                               Ptr<OutputStreamWrapper> stream,
                                               Time::Unit unit) {
  Simulator::Schedule(printInterval, &Ipv6RoutingHelper::PrintEvery,
...
void Ipv6RoutingHelper::Print(Ptr<Node> node, Ptr<OutputStreamWrapper> stream,
                              Time::Unit unit) {
...
void Ipv6RoutingHelper::PrintEvery(Time printInterval, Ptr<Node> node,
                                   Ptr<OutputStreamWrapper> stream,
                                   Time::Unit unit) {
  Ptr<Ipv6> ipv6 = node->GetObject<Ipv6>();
  if (ipv6) {
