...
Ipv4ListRoutingHelper::Ipv4ListRoutingHelper(const Ipv4ListRoutingHelper &o) {
  for (auto i = o.m_list.begin(); i != o.m_list.end(); ++i) {
    m_list.emplace_back(const_cast<const Ipv4RoutingHelper *>(i->first->Copy()),
                        i->second);
  }
}

Ipv4ListRoutingHelper *Ipv4ListRoutingHelper::Copy() const {
  return new Ipv4ListRoutingHelper(*this);
}

void Ipv4ListRoutingHelper::Add(const Ipv4RoutingHelper &routing,
                                int16_t priority) {
  m_list.emplace_back(const_cast<const Ipv4RoutingHelper *>(routing.Copy()),
                      priority);
}

Ptr<Ipv4RoutingProtocol> Ipv4ListRoutingHelper::Create(Ptr<Node> node) const {
