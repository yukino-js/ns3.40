...
void Ipv4AddressHelper::SetBase(const Ipv4Address network, const Ipv4Mask mask,
                                const Ipv4Address address) {
  NS_LOG_FUNCTION_NOARGS();
...
Ipv4Address Ipv4AddressHelper::NewAddress() {
  //
...
Ipv4Address Ipv4AddressHelper::NewNetwork() {
  NS_LOG_FUNCTION_NOARGS();
  ++m_network;
  m_address = m_base;
...
Ipv4InterfaceContainer Ipv4AddressHelper::Assign(const NetDeviceContainer &c) {
  NS_LOG_FUNCTION_NOARGS();
  ...
  for (uint32_t i = 0; i < c.GetN(); ++i) {
    ...
    NS_ASSERT_MSG(
        node, "Ipv4AddressHelper::Assign(): NetDevice is not not associated "
              "with any node -> fail");
...
uint32_t Ipv4AddressHelper::NumAddressBits(uint32_t maskbits) const {
  NS_LOG_FUNCTION_NOARGS();
