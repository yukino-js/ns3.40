...
RipHelper::RipHelper() { m_factory.SetTypeId("ns3::Rip"); }
...
RipHelper *RipHelper::Copy() const { return new RipHelper(*this); }

Ptr<Ipv4RoutingProtocol> RipHelper::Create(Ptr<Node> node) const {
...
void RipHelper::Set(std::string name, const AttributeValue &value) {
  m_factory.Set(name, value);
}

int64_t RipHelper::AssignStreams(NodeContainer c, int64_t stream) {
...
void RipHelper::SetDefaultRouter(Ptr<Node> node, Ipv4Address nextHop,
                                 uint32_t interface) {
  Ptr<Ipv4> ipv4 = node->GetObject<Ipv4>();
  NS_ASSERT_MSG(ipv4, "Ipv4 not installed on node");
  Ptr<Ipv4RoutingProtocol> proto = ipv4->GetRoutingProtocol();
...
void RipHelper::ExcludeInterface(Ptr<Node> node, uint32_t interface) {
  auto it = m_interfaceExclusions.find(node);
...
void RipHelper::SetInterfaceMetric(Ptr<Node> node, uint32_t interface,
                                   uint8_t metric) {
