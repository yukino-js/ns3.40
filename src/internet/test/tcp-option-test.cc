...
void TcpOptionWSTestCase::DoRun() {
  TestSerialize();
  TestDeserialize();
}

void TcpOptionWSTestCase::TestSerialize() {
...
void TcpOptionWSTestCase::TestDeserialize() {
  TcpOptionWinScale opt;
...
void TcpOptionTSTestCase::DoRun() {
...
void TcpOptionTSTestCase::TestSerialize() {
  TcpOptionTS opt;

  opt.SetTimestamp(m_timestamp);
  opt.SetEcho(m_echo);

  NS_TEST_EXPECT_MSG_EQ(m_timestamp, opt.GetTimestamp(),
...
void TcpOptionTSTestCase::TestDeserialize() {
  TcpOptionTS opt;
...
public:
  TcpOptionTestSuite() : TestSuite("tcp-option", UNIT) {
    ...
    AddTestCase(new TcpOptionTSTestCase(
