...
void Ipv4RipTest::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket->Bind(InetSocketAddress(Ipv4Address("10.0.2.2"), 1234)), 0,
      "trivial");
...
void Ipv4RipCountToInfinityTest::DoSendData(Ptr<Socket> socket,
                                            std::string to) {
  Address realTo = InetSocketAddress(Ipv4Address(to.c_str()), 1234);
  NS_TEST_EXPECT_MSG_EQ(socket->SendTo(Create<Packet>(123), 0, realTo), 123,
                        "100");
...
void Ipv4RipCountToInfinityTest::SendData(Ptr<Socket> socket, std::string to) {
  m_receivedPacket = Create<Packet>();
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(60),
                                 &Ipv4RipCountToInfinityTest::DoSendData, this,
                                 socket, to);
  Simulator::Stop(Seconds(66));
  Simulator::Run();
}
...
void Ipv4RipCountToInfinityTest::DoRun() {
  ...
  rxSocket->SetRecvCallback(
      MakeCallback(&Ipv4RipCountToInfinityTest::ReceivePkt, this));
...
Ipv4RipSplitHorizonStrategyTest::Ipv4RipSplitHorizonStrategyTest(
    Rip::SplitHorizonType_e strategy)
...
void Ipv4RipSplitHorizonStrategyTest::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket->Bind(InetSocketAddress(Ipv4Address("224.0.0.9"), 520)), 0,
      "trivial");
  rxSocket->SetRecvCallback(
