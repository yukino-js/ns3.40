...
void Ipv6ForwardingTest::DoSendData(Ptr<Socket> socket, std::string to) {
...
void Ipv6ForwardingTest::SendData(Ptr<Socket> socket, std::string to) {
  m_receivedPacket = Create<Packet>();
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(0),
                                 &Ipv6ForwardingTest::DoSendData, this, socket,
                                 to);
  Simulator::Run();
}
...
void Ipv6ForwardingTest::DoRun() {
  ...
  txNode->GetObject<Icmpv6L4Protocol>()->SetAttribute("DAD",
                                                      BooleanValue(false));
  fwNode->GetObject<Icmpv6L4Protocol>()->SetAttribute("DAD",
                                                      BooleanValue(false));
  ...
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket->Bind(Inet6SocketAddress(Ipv6Address("2001:1::2"), 1234)), 0,
      "trivial");
  rxSocket->SetRecvCallback(
      MakeCallback(&Ipv6ForwardingTest::ReceivePkt, this));
