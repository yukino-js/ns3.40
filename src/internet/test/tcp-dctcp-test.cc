...
TcpDctcpCodePointsTest::TcpDctcpCodePointsTest(uint8_t testCase,
    ...
    : TcpGeneralTest(desc), m_senderSent(0), m_receiverSent(0),
      m_senderReceived(0), m_testCase(testCase) {}
...
void TcpDctcpCodePointsTest::Rx(const Ptr<const Packet> p, const TcpHeader &h,
                                SocketWho who) {
...
void TcpDctcpCodePointsTest::ConfigureProperties() {
  TcpGeneralTest::ConfigureProperties();
  SetUseEcn(SENDER, TcpSocketState::On);
  SetUseEcn(RECEIVER, TcpSocketState::On);
}

void TcpDctcpCodePointsTest::ConfigureEnvironment() {
...
TypeId TcpDctcpCongestedRouter::GetTypeId() {
...
void TcpDctcpCongestedRouter::ReTxTimeout() { TcpSocketBase::ReTxTimeout(); }

void TcpDctcpCongestedRouter::SetTestCase(uint8_t testCase) {
  m_testCase = testCase;
}

uint32_t TcpDctcpCongestedRouter::SendDataPacket(SequenceNumber32 seq,
                                                 uint32_t maxSize,
                                                 bool withAck) {
  ...
  if (GetIpTos()) {
    ...
    if (m_testCase == 3 && m_dataPacketSent == 1 && !isRetransmission) {
      ...
      if (m_tcb->m_ecnState != TcpSocketState::ECN_DISABLED &&
          (GetIpTos() & 0x3) == 0 && !isRetransmission) {
  ...
  if (IsManualIpv6Tclass()) {
    ...
    if (m_testCase == 3 && m_dataPacketSent == 1 && !isRetransmission) {
      ...
      if (m_tcb->m_ecnState != TcpSocketState::ECN_DISABLED &&
          (GetIpv6Tclass() & 0x3) == 0 && !isRetransmission) {
  ...
  if (m_retxEvent.IsExpired()) {
    ...
    m_retxEvent =
        Simulator::Schedule(m_rto, &TcpDctcpCongestedRouter::ReTxTimeout, this);
  }
...
Ptr<TcpSocketBase> TcpDctcpCongestedRouter::Fork() {
...
TcpDctcpCodePointsTest::CreateSenderSocket(Ptr<Node> node) {
  if (m_testCase == 2) {
    ...
    Ptr<TcpDctcpCongestedRouter> socket =
        DynamicCast<TcpDctcpCongestedRouter>(CreateSocket(
            node, TcpDctcpCongestedRouter::GetTypeId(), TcpDctcp::GetTypeId()));
...
public:
  TcpDctcpTestSuite() : TestSuite("tcp-dctcp-test", UNIT) {
    AddTestCase(new TcpDctcpToLinuxReno(
    ...
    AddTestCase(new TcpDctcpCodePointsTest(
                    1, "ECT Test : Check if ECT is set on Syn, Syn+Ack, "
                       "Ack and Data packets for DCTCP packets"),
                TestCase::QUICK);
    AddTestCase(new TcpDctcpCodePointsTest(
                    2, "ECT Test : Check if ECT is not set on Syn, Syn+Ack and "
                       "Ack but set on "
                       "Data packets for non-DCTCP but ECN enabled traffic"),
                TestCase::QUICK);
    AddTestCase(new TcpDctcpCodePointsTest(
                    3, "ECE Functionality Test: ECE should only be sent by "
