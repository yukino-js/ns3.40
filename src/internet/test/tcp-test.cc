...
static std::string Name(std::string str, uint32_t totalStreamSize,
                        uint32_t sourceWriteSize, uint32_t serverReadSize,
                        uint32_t serverWriteSize, uint32_t sourceReadSize,
                        bool useIpv6) {
  std::ostringstream oss;
  oss << str << " total=" << totalStreamSize
      << " sourceWrite=" << sourceWriteSize << " sourceRead=" << sourceReadSize
      << " serverRead=" << serverReadSize << " serverWrite=" << serverWriteSize
      << " useIpv6=" << useIpv6;
  return oss.str();
}
...
TcpTestCase::TcpTestCase(uint32_t totalStreamSize, uint32_t sourceWriteSize,
    ...
    : TestCase(Name("Send string data from client to server and back",
                    totalStreamSize, sourceWriteSize, serverReadSize,
                    serverWriteSize, sourceReadSize, useIpv6)),
      m_totalBytes(totalStreamSize), m_sourceWriteSize(sourceWriteSize),
      m_sourceReadSize(sourceReadSize), m_serverWriteSize(serverWriteSize),
      m_serverReadSize(serverReadSize), m_useIpv6(useIpv6) {}

void TcpTestCase::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(m_currentSourceTxBytes, m_totalBytes,
                        "Source sent all bytes");
  NS_TEST_EXPECT_MSG_EQ(m_currentServerRxBytes, m_totalBytes,
                        "Server received all bytes");
  NS_TEST_EXPECT_MSG_EQ(m_currentSourceRxBytes, m_totalBytes,
                        "Source received all bytes");
  NS_TEST_EXPECT_MSG_EQ(
      memcmp(m_sourceTxPayload, m_serverRxPayload, m_totalBytes), 0,
      "Server received expected data buffers");
  NS_TEST_EXPECT_MSG_EQ(
      memcmp(m_sourceTxPayload, m_sourceRxPayload, m_totalBytes), 0,
      "Source received back expected data buffers");
}

void TcpTestCase::DoTeardown() {
...
void TcpTestCase::ServerHandleRecv(Ptr<Socket> sock) {
  while (sock->GetRxAvailable() > 0) {
    ...
    if (!p && sock->GetErrno() != Socket::ERROR_NOTERROR) {
      NS_FATAL_ERROR("Server could not read stream at byte "
                     << m_currentServerRxBytes);
    }
    NS_TEST_EXPECT_MSG_EQ(
        (m_currentServerRxBytes + p->GetSize() <= m_totalBytes), true,
...
void TcpTestCase::ServerHandleSend(Ptr<Socket> sock, uint32_t available) {
  while (sock->GetTxAvailable() > 0 &&
         m_currentServerTxBytes < m_currentServerRxBytes) {
    ...
    Ptr<Packet> p =
        Create<Packet>(&m_serverRxPayload[m_currentServerTxBytes], toSend);
...
void TcpTestCase::SourceHandleSend(Ptr<Socket> sock, uint32_t available) {
  while (sock->GetTxAvailable() > 0 && m_currentSourceTxBytes < m_totalBytes) {
    ...
    Ptr<Packet> p =
        Create<Packet>(&m_sourceTxPayload[m_currentSourceTxBytes], toSend);
...
void TcpTestCase::SourceHandleRecv(Ptr<Socket> sock) {
  while (sock->GetRxAvailable() > 0 && m_currentSourceRxBytes < m_totalBytes) {
    ...
    Ptr<Packet> p = sock->Recv(toRead, 0);
    if (!p && sock->GetErrno() != Socket::ERROR_NOTERROR) {
      NS_FATAL_ERROR("Source could not read stream at byte "
                     << m_currentSourceRxBytes);
    }
    NS_TEST_EXPECT_MSG_EQ(
        (m_currentSourceRxBytes + p->GetSize() <= m_totalBytes), true,
...
Ptr<SimpleNetDevice> TcpTestCase::AddSimpleNetDevice(Ptr<Node> node,
  ...
  Ipv4InterfaceAddress ipv4Addr =
      Ipv4InterfaceAddress(Ipv4Address(ipaddr), Ipv4Mask(netmask));
  ipv4->AddAddress(ndid, ipv4Addr);
  ipv4->SetUp(ndid);
  return dev;
}

void TcpTestCase::SetupDefaultSim() {
