...
void UdpSocketLoopbackTest::DoRun() {
  ...
  rxSocket->SetRecvCallback(
      MakeCallback(&UdpSocketLoopbackTest::ReceivePkt, this));
...
void Udp6SocketLoopbackTest::DoRun() {
  Ptr<Node> rxNode = CreateObject<Node>();
  InternetStackHelper internet;
  internet.Install(rxNode);
  ...
  rxSocket->SetRecvCallback(
      MakeCallback(&Udp6SocketLoopbackTest::ReceivePkt, this));
...
void UdpSocketImplTest::ReceivePkt2(Ptr<Socket> socket) {
  ...
  NS_TEST_ASSERT_MSG_EQ(
      availableData, m_receivedPacket2->GetSize(),
      "ReceivedPacket size is not equal to the Rx buffer size");
}
...
void UdpSocketImplTest::SentPkt(Ptr<const QueueDiscItem> item) {
  ...
  Address addr;
  m_sentPacket = Create<Ipv4QueueDiscItem>(ipv4Item->GetPacket()->Copy(), addr,
                                           0, ipv4Item->GetHeader());
}
...
uint32_t UdpSocketImplTest::GetPriority() {
  ...
  NS_TEST_EXPECT_MSG_EQ(found, true,
                        "the packet should carry a SocketPriorityTag");
  return static_cast<uint32_t>(priorityTag.GetPriority());
}

void UdpSocketImplTest::DoSendDataTo(Ptr<Socket> socket, std::string to) {
  Address realTo = InetSocketAddress(Ipv4Address(to.c_str()), 1234);
  NS_TEST_EXPECT_MSG_EQ(socket->SendTo(Create<Packet>(123), 0, realTo), 123,
                        "100");
}

void UdpSocketImplTest::SendDataTo(Ptr<Socket> socket, std::string to) {
  m_receivedPacket = Create<Packet>();
  m_receivedPacket2 = Create<Packet>();
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(0),
                                 &UdpSocketImplTest::DoSendDataTo, this, socket,
                                 to);
  Simulator::Run();
}

void UdpSocketImplTest::DoSendData(Ptr<Socket> socket) {
  NS_TEST_EXPECT_MSG_EQ(socket->Send(Create<Packet>(123), 0), 123, "100");
}

void UdpSocketImplTest::SendData(Ptr<Socket> socket) {
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(0),
...
void UdpSocketImplTest::DoRun() {
  ...
  Ptr<Socket> rxSocket = rxSocketFactory->CreateSocket();
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket->Bind(InetSocketAddress(Ipv4Address("10.0.0.1"), 1234)), 0,
      "trivial");
  rxSocket->SetRecvCallback(MakeCallback(&UdpSocketImplTest::ReceivePkt, this));

  Ptr<Socket> rxSocket2 = rxSocketFactory->CreateSocket();
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket2->Bind(InetSocketAddress(Ipv4Address("10.0.1.1"), 1234)), 0,
      "trivial");
  rxSocket2->SetRecvCallback(
      MakeCallback(&UdpSocketImplTest::ReceivePkt2, this));
  ...
  NS_TEST_EXPECT_MSG_EQ(
      m_receivedPacket->GetSize(), 0,
      "first socket should not receive it (it is bound specifically to the "
      "first interface's address");
  NS_TEST_EXPECT_MSG_EQ(
      m_receivedPacket2->GetSize(), 0,
  ...
  rxSocket2->SetRecvCallback(
      MakeCallback(&UdpSocketImplTest::ReceivePkt2, this));
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket2->Bind(InetSocketAddress(Ipv4Address("0.0.0.0"), 1234)), 0,
      "trivial");
  ...
  NS_TEST_EXPECT_MSG_EQ(
      err, -1, "socket GetPeerName() should fail when socket is not connected");
  ...
  NS_TEST_EXPECT_MSG_EQ(
      err, 0, "socket GetPeerName() should succeed when socket is connected");
  NS_TEST_EXPECT_MSG_EQ(
      peerAddress, peer,
...
void Udp6SocketImplTest::ReceivePacket(Ptr<Socket> socket, Ptr<Packet> packet,
                                       const Address &from) {
  m_receivedPacket = packet;
}

void Udp6SocketImplTest::ReceivePacket2(Ptr<Socket> socket, Ptr<Packet> packet,
                                        const Address &from) {
...
void Udp6SocketImplTest::DoSendDataTo(Ptr<Socket> socket, std::string to) {
  Address realTo = Inet6SocketAddress(Ipv6Address(to.c_str()), 1234);
  NS_TEST_EXPECT_MSG_EQ(socket->SendTo(Create<Packet>(123), 0, realTo), 123,
                        "200");
}

void Udp6SocketImplTest::SendDataTo(Ptr<Socket> socket, std::string to) {
  m_receivedPacket = Create<Packet>();
  m_receivedPacket2 = Create<Packet>();
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(0),
                                 &Udp6SocketImplTest::DoSendDataTo, this,
                                 socket, to);
  Simulator::Run();
}
...
void Udp6SocketImplTest::DoRun() {
  ...
  txNode->GetObject<Icmpv6L4Protocol>()->SetAttribute("DAD",
                                                      BooleanValue(false));
  rxNode->GetObject<Icmpv6L4Protocol>()->SetAttribute("DAD",
                                                      BooleanValue(false));
  ...
  ifIndex = ipv6->GetInterfaceForDevice(device);
  ipv6Addr =
      Ipv6InterfaceAddress(Ipv6Address("2001:0100:1::1"), Ipv6Prefix(64));
  ipv6->AddAddress(ifIndex, ipv6Addr);
  ipv6->SetUp(ifIndex);
  ...
  ifIndex = ipv6->GetInterfaceForDevice(device);
  ipv6Addr =
      Ipv6InterfaceAddress(Ipv6Address("2001:0100:1::2"), Ipv6Prefix(64));
  ipv6->AddAddress(ifIndex, ipv6Addr);
  ipv6->SetUp(ifIndex);
  ...
  NS_TEST_EXPECT_MSG_EQ(
      ...
      "trivial");
  rxSocket->SetRecvCallback(
      MakeCallback(&Udp6SocketImplTest::ReceivePkt, this));

  Ptr<Socket> rxSocket2 = rxSocketFactory->CreateSocket();
  rxSocket2->SetRecvCallback(
      MakeCallback(&Udp6SocketImplTest::ReceivePkt2, this));
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket2->Bind(Inet6SocketAddress(Ipv6Address("2001:0100:1::1"), 1234)),
      0, "trivial");
  ...
  rxSocket2->SetRecvCallback(
      MakeCallback(&Udp6SocketImplTest::ReceivePkt2, this));
  NS_TEST_EXPECT_MSG_EQ(
      rxSocket2->Bind(Inet6SocketAddress(Ipv6Address("::"), 1234)), 0,
      "trivial");
