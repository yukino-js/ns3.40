...
void Ipv6L3ProtocolTestCase::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ(
      num, 3, "Number of addresses should be 3??"); /* 2 global addresses +
                                                       link-local ones */

  num = interface2->GetNAddresses();
  NS_TEST_ASSERT_MSG_EQ(
      num, 2, "1 global addresses + link-local ones"); /* 1 global addresses +
                                                          link-local ones */
  ...
  index = ipv6->GetInterfaceForAddress(
      "2001:ffff:5678:9000::1"); /* address we just remove */
...
static IPv6L3ProtocolTestSuite
    g_ipv6protocolTestSuite; //!< Static variable for test initialization
