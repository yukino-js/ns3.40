...
TcpFastRetrTest::TcpFastRetrTest(TypeId typeId, uint32_t seqToKill,
                                 const std::string &msg)
    : TcpGeneralTest(msg), m_pktDropped(false), m_pktWasDropped(false),
      m_seqToKill(seqToKill), m_dupAckReceived(0), m_sndNextExpSeq(0),
      m_rcvNextExpAck(1), m_countRetr(0), m_bytesRcvButNotAcked(0) {
  m_congControlTypeId = typeId;
}

void TcpFastRetrTest::ConfigureProperties() {
  TcpGeneralTest::ConfigureProperties();
  SetInitialSsThresh(SENDER, 0);
}

void TcpFastRetrTest::ConfigureEnvironment() {
  TcpGeneralTest::ConfigureEnvironment();
  SetAppPktCount(100);
}

Ptr<ErrorModel> TcpFastRetrTest::CreateSenderErrorModel() { return nullptr; }

Ptr<ErrorModel> TcpFastRetrTest::CreateReceiverErrorModel() {
  ...
  m_errorModel->SetDropCallback(
      MakeCallback(&TcpFastRetrTest::PktDropped, this));

  return m_errorModel;
}

Ptr<TcpSocketMsgBase> TcpFastRetrTest::CreateSenderSocket(Ptr<Node> node) {
...
void TcpFastRetrTest::Tx(const Ptr<const Packet> p, const TcpHeader &h,
                         SocketWho who) {
  if (who == SENDER) {
...
void TcpFastRetrTest::RcvAck(const Ptr<const TcpSocketState> tcb,
    ...
    if (h.GetAckNumber().GetValue() < m_seqToKill) {
      ...
      if (GetDupAckCount(SENDER) == 0 &&
        ...
        NS_TEST_ASSERT_MSG_EQ(
            tcb->m_congState.Get(), TcpSocketState::CA_RECOVERY,
...
void TcpFastRetrTest::ProcessedAck(const Ptr<const TcpSocketState> tcb,
                                   const TcpHeader &h, SocketWho who) {
  NS_LOG_FUNCTION(this << tcb << h << who);

  if (who == SENDER) {
    if (m_previousAck == h.GetAckNumber() &&
        h.GetAckNumber().GetValue() < 50002) {
      m_dupAckReceived++;
...
void TcpFastRetrTest::AfterRTOExpired(const Ptr<const TcpSocketState> tcb,
                                      SocketWho who) {
  NS_TEST_ASSERT_MSG_EQ(true, false, "RTO isn't expected here");
}

void TcpFastRetrTest::CongStateTrace(
    const TcpSocketState::TcpCongState_t oldValue,
    const TcpSocketState::TcpCongState_t newValue) {
  NS_LOG_FUNCTION(this << oldValue << newValue);

  if (oldValue == TcpSocketState::CA_OPEN &&
      newValue == TcpSocketState::CA_DISORDER) {
...
void TcpFastRetrTest::PktDropped(const Ipv4Header &ipH, const TcpHeader &tcpH,
                                 Ptr<const Packet> p) {
...
void TcpFastRetrTest::FinalChecks() {
