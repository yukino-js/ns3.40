...
void TcpHeaderGetSetTestCase::DoRun() {
  ...
  for (uint32_t i = 0; i < 1000; ++i) {
    ...
    NS_TEST_ASSERT_MSG_EQ(
        destinationPort, copyHeader.GetDestinationPort(),
        "Different destination port found in deserialized header");
    NS_TEST_ASSERT_MSG_EQ(
        sequenceNumber, copyHeader.GetSequenceNumber(),
    ...
    NS_TEST_ASSERT_MSG_EQ(
        urgentPointer, copyHeader.GetUrgentPointer(),
...
void TcpHeaderWithRFC793OptionTestCase::DoRun() {
  OneOptionAtTime();
  CheckNoPadding();
  CheckCorrectDeserialize();
}

void TcpHeaderWithRFC793OptionTestCase::CheckCorrectDeserialize() {
  TcpHeader source;
...
void TcpHeaderWithRFC793OptionTestCase::CheckNoPadding() {
  {
    TcpOptionNOP oNop1;
    TcpOptionNOP oNop2;
    TcpOptionNOP oNop3;
...
void TcpHeaderWithRFC793OptionTestCase::OneOptionAtTime() {
  {
    TcpOptionEnd oEnd;
    TcpHeader header;
    Buffer buffer;
...
void TcpHeaderFlagsToString::DoRun() {
  std::string str;
  std::string target;
  str = TcpHeader::FlagsToString(0x0);
  target = "";
  NS_TEST_ASSERT_MSG_EQ(str, target,
...
public:
  TcpHeaderTestSuite() : TestSuite("tcp-header", UNIT) {
    AddTestCase(new TcpHeaderGetSetTestCase("GetSet test cases"),
                TestCase::QUICK);
    AddTestCase(
        new TcpHeaderWithRFC793OptionTestCase("Test for options in RFC 793"),
        TestCase::QUICK);
