...
void TcpBytesInFlightTest::Rx(const Ptr<const Packet> p, const TcpHeader &h,
  ...
  if (who == RECEIVER) {
    ...
    if (h.GetAckNumber() > m_lastAckRecv) { // New ack
      uint32_t diff = h.GetAckNumber() - m_lastAckRecv;
      NS_LOG_DEBUG("Recv ACK=" << h.GetAckNumber());

      if (m_dupAckRecv > 0) { // Previously we got some ACKs
        if (h.GetAckNumber() >=
            m_greatestSeqSent) { // This an ACK which acknowledge all the window
      ...
      if ((h.GetFlags() & TcpHeader::FIN) != 0 ||
          m_guessedBytesInFlight + 1 ==
              diff) { // received the ACK for the FIN (which includes 1 spurious
                      // byte)
        diff -= 1;
      }
...
void TcpBytesInFlightTest::Tx(const Ptr<const Packet> p, const TcpHeader &h,
  ...
  if (who == SENDER) {
    ...
    if (m_greatestSeqSent <=
        h.GetSequenceNumber()) { // This is not a retransmission
