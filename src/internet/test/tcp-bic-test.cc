...
TcpBicIncrementTest::TcpBicIncrementTest(uint32_t cWnd, uint32_t segmentSize,
    ...
    : TestCase(name), m_cWnd(cWnd), m_segmentSize(segmentSize),
      m_ssThresh(ssThresh), m_segmentsAcked(segmentsAcked),
      m_lastMaxCwnd(lastMaxCwnd) {}

void TcpBicIncrementTest::DoRun() {
...
void TcpBicIncrementTest::ExecuteTest() {
...
uint32_t TcpBicIncrementTest::Update(Ptr<TcpSocketState> tcb) {
...
TcpBicDecrementTest::TcpBicDecrementTest(uint32_t cWnd, uint32_t segmentSize,
                                         ...
                                         const std::string &name)
    : TestCase(name), m_cWnd(cWnd), m_segmentSize(segmentSize),
      m_fastConvergence(fastConvergence), m_lastMaxCwnd(lastMaxCwnd) {}

void TcpBicDecrementTest::DoRun() {
  m_state = CreateObject<TcpSocketState>();
...
void TcpBicDecrementTest::ExecuteTest() {
  Ptr<TcpBic> cong = CreateObject<TcpBic>();
  cong->m_lastMaxCwnd = m_lastMaxCwnd;
...
public:
  TcpBicTestSuite() : TestSuite("tcp-bic-test", UNIT) {
    ...
    AddTestCase(
        ...
        TestCase::QUICK);
    AddTestCase(
        new TcpBicDecrementTest(5 * 1446, 1446, false, 10,
                                "Bic decrement test: not in fast convergence & "
                                "cwnd less than lowWindow"),
        TestCase::QUICK);
    AddTestCase(
        new TcpBicDecrementTest(15 * 1446, 1446, false, 10,
                                "Bic decrement test: not in fast convergence & "
