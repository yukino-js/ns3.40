...
TcpGeneralTest::TcpGeneralTest(const std::string &desc)
    : TestCase(desc), m_congControlTypeId(TcpNewReno::GetTypeId()),
      m_recoveryTypeId(TcpClassicRecovery::GetTypeId()),
      m_remoteAddr(Ipv4Address::GetAny(), 4477) {
  NS_LOG_FUNCTION(this << desc);
}

TcpGeneralTest::~TcpGeneralTest() { NS_LOG_FUNCTION_NOARGS(); }
...
void TcpGeneralTest::SendPacket(Ptr<Socket> socket, uint32_t pktSize,
                                uint32_t pktCount, Time pktInterval) {
  NS_LOG_FUNCTION(this << " " << pktSize << " " << pktCount << " "
                       << pktInterval.GetSeconds());
  if (pktCount > 0) {
    socket->Send(Create<Packet>(pktSize));
    Simulator::Schedule(pktInterval, &TcpGeneralTest::SendPacket, this, socket,
...
void TcpGeneralTest::ConfigureProperties() {
  NS_LOG_FUNCTION(this);
...
void TcpGeneralTest::DoRun() {
  ...
  senderDev->GetQueue()->TraceConnect(
      "Drop", "SENDER", MakeCallback(&TcpGeneralTest::QueueDropCb, this));
  senderDev->TraceConnect("PhyRxDrop", "sender",
                          MakeCallback(&TcpGeneralTest::PhyDropCb, this));

  receiverDev->SetMtu(m_mtu);
  receiverDev->GetQueue()->TraceConnect(
      "Drop", "RECEIVER", MakeCallback(&TcpGeneralTest::QueueDropCb, this));
  ...
  m_receiverSocket->SetRecvCallback(
      MakeCallback(&TcpGeneralTest::ReceivePacket, this));
  ...
  m_receiverSocket->SetCloseCallbacks(
      MakeCallback(&TcpGeneralTest::NormalCloseCb, this),
      MakeCallback(&TcpGeneralTest::ErrorCloseCb, this));
  m_receiverSocket->SetRcvAckCb(MakeCallback(&TcpGeneralTest::RcvAckCb, this));
  m_receiverSocket->SetProcessedAckCb(
      MakeCallback(&TcpGeneralTest::ProcessedAckCb, this));
  m_receiverSocket->SetAfterRetransmitCb(
      MakeCallback(&TcpGeneralTest::AfterRetransmitCb, this));
  ...
  m_receiverSocket->TraceConnectWithoutContext(
      "Tx", MakeCallback(&TcpGeneralTest::TxPacketCb, this));
  m_receiverSocket->TraceConnectWithoutContext(
      "Rx", MakeCallback(&TcpGeneralTest::RxPacketCb, this));
  ...
  m_senderSocket->SetCloseCallbacks(
      MakeCallback(&TcpGeneralTest::NormalCloseCb, this),
      MakeCallback(&TcpGeneralTest::ErrorCloseCb, this));
  m_senderSocket->SetRcvAckCb(MakeCallback(&TcpGeneralTest::RcvAckCb, this));
  m_senderSocket->SetProcessedAckCb(
      MakeCallback(&TcpGeneralTest::ProcessedAckCb, this));
  m_senderSocket->SetAfterRetransmitCb(
      MakeCallback(&TcpGeneralTest::AfterRetransmitCb, this));
  m_senderSocket->SetBeforeRetransmitCb(
      MakeCallback(&TcpGeneralTest::BeforeRetransmitCb, this));
  m_senderSocket->SetDataSentCallback(
      MakeCallback(&TcpGeneralTest::DataSentCb, this));
  m_senderSocket->SetUpdateRttHistoryCb(
      MakeCallback(&TcpGeneralTest::UpdateRttHistoryCb, this));
  m_senderSocket->TraceConnectWithoutContext(
      "CongestionWindow", MakeCallback(&TcpGeneralTest::CWndTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "CongestionWindowInflated",
      MakeCallback(&TcpGeneralTest::CWndInflTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "SlowStartThreshold", MakeCallback(&TcpGeneralTest::SsThreshTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "CongState", MakeCallback(&TcpGeneralTest::CongStateTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "Tx", MakeCallback(&TcpGeneralTest::TxPacketCb, this));
  m_senderSocket->TraceConnectWithoutContext(
      "Rx", MakeCallback(&TcpGeneralTest::RxPacketCb, this));
  m_senderSocket->TraceConnectWithoutContext(
      "RTT", MakeCallback(&TcpGeneralTest::RttTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "BytesInFlight", MakeCallback(&TcpGeneralTest::BytesInFlightTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "RTO", MakeCallback(&TcpGeneralTest::RtoTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "NextTxSequence", MakeCallback(&TcpGeneralTest::NextTxSeqTrace, this));
  m_senderSocket->TraceConnectWithoutContext(
      "HighestSequence",
  ...
  Simulator::ScheduleWithContext(
      nodes.Get(0)->GetId(), m_startTime, &TcpGeneralTest::SendPacket, this,
...
Ptr<TcpSocketMsgBase> TcpGeneralTest::CreateSocket(Ptr<Node> node,
  ...
  Ptr<TcpSocketMsgBase> socket =
      DynamicCast<TcpSocketMsgBase>(socketFactory.Create());
  Ptr<TcpCongestionOps> algo =
      congestionAlgorithmFactory.Create<TcpCongestionOps>();
  Ptr<TcpRecoveryOps> recovery =
      recoveryAlgorithmFactory.Create<TcpRecoveryOps>();
...
Ptr<ErrorModel> TcpGeneralTest::CreateSenderErrorModel() { return nullptr; }

Ptr<ErrorModel> TcpGeneralTest::CreateReceiverErrorModel() { return nullptr; }
...
void TcpGeneralTest::QueueDropCb(std::string context, Ptr<const Packet> p) {
  if (context == "SENDER") {
    QueueDrop(SENDER);
...
void TcpGeneralTest::DataSentCb(Ptr<Socket> socket, uint32_t size) {
  if (socket->GetNode() == m_receiverSocket->GetNode()) {
...
void TcpGeneralTest::Tx(const Ptr<const Packet> p, const TcpHeader &h,
                        SocketWho who) {
  NS_LOG_FUNCTION(this << p << h << who);
}

void TcpGeneralTest::Rx(const Ptr<const Packet> p, const TcpHeader &h,
                        SocketWho who) {
  NS_LOG_FUNCTION(this << p << h << who);
}

void TcpGeneralTest::RcvAckCb(const Ptr<const Packet> p, const TcpHeader &h,
                              const Ptr<const TcpSocketBase> tcp) {
  if (tcp->GetNode() == m_receiverSocket->GetNode()) {
...
void TcpGeneralTest::TxPacketCb(const Ptr<const Packet> p, const TcpHeader &h,
                                const Ptr<const TcpSocketBase> tcp) {
  if (tcp->GetNode() == m_receiverSocket->GetNode()) {
    Tx(p, h, RECEIVER);
  } else if (tcp->GetNode() == m_senderSocket->GetNode()) {
...
void TcpGeneralTest::RxPacketCb(const Ptr<const Packet> p, const TcpHeader &h,
                                const Ptr<const TcpSocketBase> tcp) {
  if (tcp->GetNode() == m_receiverSocket->GetNode()) {
    Rx(p, h, RECEIVER);
  } else if (tcp->GetNode() == m_senderSocket->GetNode()) {
...
void TcpGeneralTest::ProcessedAckCb(Ptr<const Packet> p, const TcpHeader &h,
...
uint32_t TcpGeneralTest::GetInitialSsThresh(SocketWho who) {
  if (who == SENDER) {
    ...
    return DynamicCast<TcpSocketMsgBase>(m_receiverSocket)
        ->GetInitialSSThresh();
  } else {
    NS_FATAL_ERROR("Not defined");
  }
...
void TcpGeneralTest::SetSegmentSize(SocketWho who, uint32_t segmentSize) {
  if (who == SENDER) {
...
void TcpGeneralTest::SetInitialCwnd(SocketWho who, uint32_t initialCwnd) {
  if (who == SENDER) {
...
void TcpGeneralTest::SetDelAckMaxCount(SocketWho who, uint32_t count) {
  if (who == SENDER) {
...
void TcpGeneralTest::SetUseEcn(SocketWho who, TcpSocketState::UseEcn_t useEcn) {
  if (who == SENDER) {
...
void TcpGeneralTest::SetPacingStatus(SocketWho who, bool pacing) {
  if (who == SENDER) {
...
void TcpGeneralTest::SetPaceInitialWindow(SocketWho who, bool paceWindow) {
  if (who == SENDER) {
...
TypeId TcpSocketMsgBase::GetTypeId() {
...
Ptr<TcpSocketBase> TcpSocketMsgBase::Fork() {
  return CopyObject<TcpSocketMsgBase>(this);
}

void TcpSocketMsgBase::SetRcvAckCb(AckManagementCb cb) {
  NS_ASSERT(!cb.IsNull());
  m_rcvAckCb = cb;
}

void TcpSocketMsgBase::SetProcessedAckCb(AckManagementCb cb) {
  NS_ASSERT(!cb.IsNull());
  m_processedAckCb = cb;
}

void TcpSocketMsgBase::SetAfterRetransmitCb(RetrCb cb) {
  NS_ASSERT(!cb.IsNull());
  m_afterRetrCallback = cb;
}

void TcpSocketMsgBase::SetBeforeRetransmitCb(RetrCb cb) {
  NS_ASSERT(!cb.IsNull());
  m_beforeRetrCallback = cb;
}

void TcpSocketMsgBase::ReceivedAck(Ptr<Packet> packet,
...
void TcpSocketMsgBase::ReTxTimeout() {
  m_beforeRetrCallback(m_tcb, this);
  TcpSocketBase::ReTxTimeout();
  m_afterRetrCallback(m_tcb, this);
}

void TcpSocketMsgBase::SetForkCb(Callback<void, Ptr<TcpSocketMsgBase>> cb) {
  NS_ASSERT(!cb.IsNull());
  m_forkCb = cb;
}

void TcpSocketMsgBase::SetUpdateRttHistoryCb(UpdateRttCallback cb) {
  NS_ASSERT(!cb.IsNull());
  m_updateRttCb = cb;
}

void TcpSocketMsgBase::UpdateRttHistory(const SequenceNumber32 &seq,
                                        uint32_t sz, bool isRetransmission) {
...
void TcpSocketMsgBase::CompleteFork(Ptr<Packet> p, const TcpHeader &tcpHeader,
...
TypeId TcpSocketSmallAcks::GetTypeId() {
...
void TcpSocketSmallAcks::SendEmptyPacket(uint8_t flags) {
  ...
  if (hasSyn || hasFin) {
    ...
    if (m_bytesLeftToBeAcked == 0 &&
        m_tcb->m_rxBuffer->NextRxSequence() > m_lastAckedSeq) {
      m_bytesLeftToBeAcked = m_tcb->m_rxBuffer->NextRxSequence().GetValue() -
                             m_lastAckedSeq.GetValue();
      m_bytesLeftToBeAcked -= m_bytesToAck;
      NS_LOG_DEBUG("Setting m_bytesLeftToBeAcked to " << m_bytesLeftToBeAcked);
    } else if (m_bytesLeftToBeAcked > 0 &&
               m_tcb->m_rxBuffer->NextRxSequence() > m_lastAckedSeq) {
      m_bytesLeftToBeAcked -= m_bytesToAck;
      NS_LOG_DEBUG("Decrementing m_bytesLeftToBeAcked to "
                   << m_bytesLeftToBeAcked);
    }

    NS_LOG_LOGIC("Acking up to "
                 << ackSeq << " remaining bytes: " << m_bytesLeftToBeAcked);
...
Ptr<TcpSocketBase> TcpSocketSmallAcks::Fork() {
