...
void TcpVenoTest::AdditiveIncrease(Ptr<TcpSocketState> state, uint32_t diff,
                                   UintegerValue beta) {
  if (m_cWnd < m_ssThresh) { // Slow start
    NewReno_SlowStart(state, 1);
  } else {                   // Congestion avoidance
    if (diff < beta.Get()) { // Increase cwnd by 1 every RTT when bandwidth is
                             // not fully utilized
      NewReno_CongestionAvoidance(state, 1);
    } else { // Increase cwnd by 1 every other RTT when bandwidth is fully
             // utilized
      if (m_inc) {
        NewReno_CongestionAvoidance(state, 1);
        m_inc = false;
...
static TcpVenoTestSuite
    g_tcpVenoTest; //!< Static variable for test initialization
