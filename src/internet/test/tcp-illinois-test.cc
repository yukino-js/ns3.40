...
TcpIllinoisTest::TcpIllinoisTest(uint32_t cWnd, uint32_t ssThresh,
                                 uint32_t segmentSize, uint32_t cntRtt,
                                 Time maxRtt, uint32_t segmentsAcked,
                                 SequenceNumber32 nextTxSeq,
                                 SequenceNumber32 lastAckedSeq,
                                 const std::string &name)
    : TestCase(name), m_cWnd(cWnd), m_ssThresh(ssThresh),
      m_segmentSize(segmentSize), m_baseRtt(MilliSeconds(100)),
      m_maxRtt(maxRtt), m_segmentsAcked(segmentsAcked), m_nextTxSeq(nextTxSeq),
      m_lastAckedSeq(lastAckedSeq), m_alpha(0.0), m_beta(0.0), m_cntRtt(cntRtt),
      m_sumRtt(0), m_rttAbove(false), m_rttLow(0), m_ackCnt(0) {}
...
Time TcpIllinoisTest::CalculateMaxDelay() { return (m_maxRtt - m_baseRtt); }

Time TcpIllinoisTest::CalculateAvgDelay() {
  return (m_sumRtt / m_cntRtt - m_baseRtt);
}

void TcpIllinoisTest::CalculateAlpha(Ptr<TcpIllinois> cong, double da,
                                     double dm) {
...
void TcpIllinoisTest::CalculateBeta(Ptr<TcpIllinois> cong, double da,
                                    double dm) {
  DoubleValue betaMin;
  ...
  if (da <= d2) {
    ...
    m_beta = (betaMin.Get() * d3 - betaMax.Get() * d2 +
              (betaMax.Get() - betaMin.Get()) * da) /
             (d3 - d2);
  }
...
public:
  TcpIllinoisTestSuite() : TestSuite("tcp-illinois-test", UNIT) {
    AddTestCase(new TcpIllinoisTest(
                    38 * 1446, 40 * 1446, 1446, 2, MilliSeconds(105), 2,
                    SequenceNumber32(2893), SequenceNumber32(5785),
                    "Illinois test on cWnd and ssThresh when in slow start"),
                TestCase::QUICK);
    AddTestCase(
        new TcpIllinoisTest(60 * 346, 40 * 346, 346, 2, MilliSeconds(100), 2,
                            SequenceNumber32(2893), SequenceNumber32(5785),
                            "Illinois test on cWnd and ssThresh when avg "
                            "queueing delay is at minimum"),
        TestCase::QUICK);
    AddTestCase(
        new TcpIllinoisTest(38 * 1446, 40 * 1446, 1446, 5, MilliSeconds(110), 2,
                            SequenceNumber32(2893), SequenceNumber32(5785),
                            "Illinois test on cWnd and ssThresh when avg "
                            "queueing delay is at maximum"),
        TestCase::QUICK);
    AddTestCase(new TcpIllinoisTest(
                    40 * 1446, 38 * 1446, 1446, 2, MilliSeconds(105), 55,
                    SequenceNumber32(2893), SequenceNumber32(5785),
