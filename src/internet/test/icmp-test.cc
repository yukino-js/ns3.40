...
void IcmpEchoReplyTestCase::SendData(Ptr<Socket> socket, Ipv4Address dst) {
...
void IcmpEchoReplyTestCase::ReceivePkt(Ptr<Socket> socket) {
  Address from;
  ...
  Ipv4Header ipv4;
  p->RemoveHeader(ipv4);
  NS_TEST_EXPECT_MSG_EQ(ipv4.GetProtocol(), 1,
                        " The received Packet is not an ICMP packet");
...
void IcmpEchoReplyTestCase::DoRun() {
  NodeContainer n;
  n.Create(2);
...
void IcmpTimeExceedTestCase::SendData(Ptr<Socket> socket, Ipv4Address dst) {
  Ptr<Packet> p = Create<Packet>();
  Icmpv4Echo echo;
  echo.SetSequenceNumber(1);
  echo.SetIdentifier(0);
...
void IcmpTimeExceedTestCase::ReceivePkt(Ptr<Socket> socket) {
  Address from;
  Ptr<Packet> p = socket->RecvFrom(0xffffffff, 0, from);
  m_receivedPacket = p->Copy();
  ...
  NS_TEST_EXPECT_MSG_EQ(
      icmp.GetType(), Icmpv4Header::ICMPV4_TIME_EXCEEDED,
      "The received packet is not a ICMPV4_TIME_EXCEEDED packet ");
}

void IcmpTimeExceedTestCase::DoRun() {
  NodeContainer n;
  NodeContainer n0n1;
  NodeContainer n1n2;
  n.Create(3);
...
void IcmpV6EchoReplyTestCase::SendData(Ptr<Socket> socket, Ipv6Address dst) {
  Ptr<Packet> p = Create<Packet>();
...
void IcmpV6EchoReplyTestCase::ReceivePkt(Ptr<Socket> socket) {
  Address from;
...
void IcmpV6EchoReplyTestCase::DoRun() {
  NodeContainer n;
  n.Create(2);
  ...
  Ptr<Socket> socket;
  socket = Socket::CreateSocket(
      n.Get(0), TypeId::LookupByName("ns3::Ipv6RawSocketFactory"));
  socket->SetAttribute("Protocol", UintegerValue(Ipv6Header::IPV6_ICMPV6));
  socket->SetRecvCallback(
      MakeCallback(&IcmpV6EchoReplyTestCase::ReceivePkt, this));
  ...
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(0),
                                 &IcmpV6EchoReplyTestCase::SendData, this,
                                 socket, i.GetAddress(1, 1));
  Simulator::Run();
...
void IcmpV6TimeExceedTestCase::SendData(Ptr<Socket> socket, Ipv6Address dst) {
  Ptr<Packet> p = Create<Packet>();
  Icmpv6Echo echo(true);
  echo.SetSeq(1);
  echo.SetId(0XB1ED);
...
void IcmpV6TimeExceedTestCase::ReceivePkt(Ptr<Socket> socket) {
  Address from;
  Ptr<Packet> p = socket->RecvFrom(from);
  Ptr<Packet> pkt = p->Copy();
...
void IcmpV6TimeExceedTestCase::DoRun() {
  NodeContainer n;
  NodeContainer n0n1;
  NodeContainer n1n2;
  n.Create(3);
  ...
  socket->SetAttribute("Protocol", UintegerValue(Ipv6Header::IPV6_ICMPV6));
  socket->SetRecvCallback(
      MakeCallback(&IcmpV6TimeExceedTestCase::ReceivePkt, this));
  ...
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(0),
                                 &IcmpV6TimeExceedTestCase::SendData, this,
                                 socket, interfaces2.GetAddress(1, 1));
  Simulator::Run();
