...
RipNg::RipNg()
    : m_ipv6(nullptr), m_splitHorizonStrategy(RipNg::POISON_REVERSE),
      m_initialized(false) {
      ...
      TypeId("ns3::RipNg")
          ...
          .AddAttribute("SplitHorizon", "Split Horizon strategy.",
                        ...
                        MakeEnumChecker(RipNg::NO_SPLIT_HORIZON,
                                        "NoSplitHorizon", RipNg::SPLIT_HORIZON,
                                        "SplitHorizon", RipNg::POISON_REVERSE,
                                        "PoisonReverse"))
          .AddAttribute(
...
void RipNg::DoInitialize() {
  ...
  m_nextUnsolicitedUpdate =
      Simulator::Schedule(delay, &RipNg::SendUnsolicitedRouteUpdate, this);
    ...
    for (uint32_t j = 0; j < m_ipv6->GetNAddresses(i); j++) {
      ...
      if (address.GetScope() == Ipv6InterfaceAddress::LINKLOCAL &&
        ...
        Inet6SocketAddress local =
            Inet6SocketAddress(address.GetAddress(), RIPNG_PORT);
      ...
      } else if (m_ipv6->GetAddress(i, j).GetScope() ==
                 Ipv6InterfaceAddress::GLOBAL) {
        addedGlobal = true;
      }
    }
  }
  ...
  if (addedGlobal) {
    Time delay =
        Seconds(m_rng->GetValue(m_minTriggeredUpdateDelay.GetSeconds(),
                                m_maxTriggeredUpdateDelay.GetSeconds()));
    m_nextTriggeredUpdate =
        Simulator::Schedule(delay, &RipNg::DoSendRouteUpdate, this, false);
  }

  delay = Seconds(m_rng->GetValue(0.01, m_startupDelay.GetSeconds()));
  m_nextTriggeredUpdate =
      Simulator::Schedule(delay, &RipNg::SendRouteRequest, this);

  Ipv6RoutingProtocol::DoInitialize();
}

Ptr<Ipv6Route> RipNg::RouteOutput(Ptr<Packet> p, const Ipv6Header &header,
                                  Ptr<NetDevice> oif,
...
bool RipNg::RouteInput(Ptr<const Packet> p, const Ipv6Header &header,
...
void RipNg::NotifyInterfaceUp(uint32_t i) {
  ...
  for (uint32_t j = 0; j < m_ipv6->GetNAddresses(i); j++) {
    ...
    Ipv6Address networkAddress =
        address.GetAddress().CombinePrefix(networkMask);
  ...
  for (uint32_t j = 0; j < m_ipv6->GetNAddresses(i); j++) {
    ...
    if (address.GetScope() == Ipv6InterfaceAddress::LINKLOCAL &&
        !sendSocketFound && activeInterface) {
...
void RipNg::NotifyRemoveAddress(uint32_t interface,
...
void RipNg::NotifyAddRoute(Ipv6Address dst, Ipv6Prefix mask,
...
void RipNg::PrintRoutingTable(Ptr<OutputStreamWrapper> stream,
...
Ptr<Ipv6Route> RipNg::Lookup(Ipv6Address dst, bool setSource,
  ...
  for (auto it = m_routes.begin(); it != m_routes.end(); it++) {
    ...
    if (j->GetRouteStatus() == RipNgRoutingTableEntry::RIPNG_VALID) {
      ...
      NS_LOG_LOGIC("Searching for route to " << dst << ", mask length "
                                             << maskLen);
...
void RipNg::AddNetworkRouteTo(Ipv6Address network, Ipv6Prefix networkPrefix,
                              Ipv6Address nextHop, uint32_t interface,
                              Ipv6Address prefixToUse) {
  NS_LOG_FUNCTION(this << network << networkPrefix << nextHop
                       << interface << prefixToUse);

  if (nextHop.IsLinkLocal()) {
    NS_LOG_WARN("Ripng::AddNetworkRouteTo - Next hop should be link-local");
  }

  auto route = new RipNgRoutingTableEntry(network, networkPrefix, nextHop,
                                          interface, prefixToUse);
...
void RipNg::InvalidateRoute(RipNgRoutingTableEntry *route) {
  ...
  for (auto it = m_routes.begin(); it != m_routes.end(); it++) {
    if (it->first == route) {
      ...
      it->second = Simulator::Schedule(m_garbageCollectionDelay,
                                       &RipNg::DeleteRoute, this, route);
      return;
    }
  }
...
void RipNg::Receive(Ptr<Socket> socket) {
  ...
  if (hdr.GetCommand() == RipNgHeader::RESPONSE) {
    ...
    NS_LOG_LOGIC(
        "Ignoring message with unknown command: " << int(hdr.GetCommand()));
  }
}

void RipNg::HandleRequests(RipNgHeader requestHdr, Ipv6Address senderAddress,
                           uint16_t senderPort, uint32_t incomingInterface,
                           uint8_t hopLimit) {
  NS_LOG_FUNCTION(this << senderAddress << int(senderPort) << incomingInterface
  ...
  if (rtes.size() == 1 && senderAddress.IsLinkLocal()) {
    if (rtes.begin()->GetPrefix() == Ipv6Address::GetAny() &&
      ...
      if (m_interfaceExclusions.find(incomingInterface) ==
        ...
        uint16_t maxRte = (mtu - Ipv6Header().GetSerializedSize() -
        ...
        for (auto rtIter = m_routes.begin(); rtIter != m_routes.end();
             rtIter++) {
          bool splitHorizoning =
          ...
          bool isGlobal =
              (rtDestAddr.GetScope() == Ipv6InterfaceAddress::GLOBAL);
          bool isDefaultRoute =
          ...
          if ((isGlobal || isDefaultRoute) &&
              (rtIter->first->GetRouteStatus() ==
               RipNgRoutingTableEntry::RIPNG_VALID)) {
            RipNgRte rte;
          ...
          if (hdr.GetRteNumber() == maxRte) {
            ...
            sendingSocket->SendTo(
                p, 0, Inet6SocketAddress(senderAddress, RIPNG_PORT));
            p->RemoveHeader(hdr);
            hdr.ClearRtes();
          }
        }
        if (hdr.GetRteNumber() > 0) {
          p->AddHeader(hdr);
          NS_LOG_DEBUG("SendTo: " << *p);
          sendingSocket->SendTo(p, 0,
                                Inet6SocketAddress(senderAddress, RIPNG_PORT));
        }
      }
    }
  } else {
...
void RipNg::HandleResponses(RipNgHeader hdr, Ipv6Address senderAddress,
                            uint32_t incomingInterface, uint8_t hopLimit) {
  NS_LOG_FUNCTION(this << senderAddress << incomingInterface << int(hopLimit)
                       << hdr);

  if (m_interfaceExclusions.find(incomingInterface) !=
      m_interfaceExclusions.end()) {
    NS_LOG_LOGIC("Ignoring an update message from an excluded interface: "
                 << incomingInterface);
    return;
  }

  if (!senderAddress.IsLinkLocal()) {
    NS_LOG_LOGIC("Ignoring an update message from a non-link-local source: "
                 << senderAddress);
    return;
  }

  if (hopLimit != 255) {
    NS_LOG_LOGIC("Ignoring an update message with suspicious hop count: "
                 << int(hopLimit));
    return;
  }
  ...
  for (auto iter = rtes.begin(); iter != rtes.end(); iter++) {
    ...
    if (iter->GetPrefix().IsLocalhost() || iter->GetPrefix().IsLinkLocal() ||
      ...
      NS_LOG_LOGIC("Ignoring an update message with wrong prefixes: "
                   << iter->GetPrefix());
      return;
    }
  }
  ...
  for (auto iter = rtes.begin(); iter != rtes.end(); iter++) {
    ...
    if (m_interfaceMetrics.find(incomingInterface) !=
        m_interfaceMetrics.end()) {
      ...
      if (it->first->GetDestNetwork() == rteAddr &&
        ...
        if (rteMetric < it->first->GetRouteMetric()) {
          if (senderAddress != it->first->GetGateway()) {
            auto route = new RipNgRoutingTableEntry(
          ...
          it->second.Cancel();
          it->second = Simulator::Schedule(
              m_timeoutDelay, &RipNg::InvalidateRoute, this, it->first);
          changed = true;
          ...
          if (senderAddress == it->first->GetGateway()) {
            ...
            if (Simulator::GetDelayLeft(it->second) < m_timeoutDelay / 2) {
              ...
              it->second.Cancel();
              it->second = Simulator::Schedule(
                  m_timeoutDelay, &RipNg::InvalidateRoute, this, route);
              changed = true;
            }
          }
        } else if (rteMetric > it->first->GetRouteMetric() &&
    ...
    if (!found && rteMetric != m_linkDown) {
      ...
      EventId invalidateEvent = Simulator::Schedule(
          m_timeoutDelay, &RipNg::InvalidateRoute, this, route);
      ...
      for (auto rtIter = m_routes.begin(); rtIter != m_routes.end(); rtIter++) {
        ...
        if (hdr.GetRteNumber() == maxRte) {
          ...
          iter->first->SendTo(p, 0,
                              Inet6SocketAddress(RIPNG_ALL_NODE, RIPNG_PORT));
          p->RemoveHeader(hdr);
          hdr.ClearRtes();
        }
      }
...
RipNgRoutingTableEntry::RipNgRoutingTableEntry(Ipv6Address network,
                                               Ipv6Prefix networkPrefix,
                                               uint32_t interface)
    : Ipv6RoutingTableEntry(Ipv6RoutingTableEntry::CreateNetworkRouteTo(
          network, networkPrefix, interface)),
      m_tag(0), m_metric(0), m_status(RIPNG_INVALID), m_changed(false) {}
...
uint16_t RipNgRoutingTableEntry::GetRouteTag() const { return m_tag; }
...
uint8_t RipNgRoutingTableEntry::GetRouteMetric() const { return m_metric; }
...
bool RipNgRoutingTableEntry::IsRouteChanged() const { return m_changed; }

std::ostream &operator<<(std::ostream &os, const RipNgRoutingTableEntry &rte) {
  os << static_cast<const Ipv6RoutingTableEntry &>(rte);
  os << ", metric: " << int(rte.GetRouteMetric())
     << ", tag: " << int(rte.GetRouteTag());
