...
public:
  ...
  TracedValue<uint32_t> m_cWndInfl{
      0}; //!< Inflated congestion window trace (used only for backward
  ...
  TracedValue<TcpCongState_t> m_congState{
      CA_OPEN}; //!< State in the Congestion state machine

  TracedValue<EcnState_t> m_ecnState{
      ECN_DISABLED}; //!< Current ECN State, represented as combination of
                     //!< EcnState values

  TracedValue<SequenceNumber32> m_highTxMark{
      0}; //!< Highest seqno ever sent, regardless of ReTx
  ...
  uint32_t m_rcvTimestampEchoReply{
      0}; //!< Sender Timestamp echoed by the receiver
  ...
  bool m_paceInitialWindow{
      false}; //!< Enable/Disable pacing for the initial window

  Time m_minRtt{
      Time::Max()}; //!< Minimum RTT observed throughout the connection
