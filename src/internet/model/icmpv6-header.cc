...
Icmpv6Header::Icmpv6Header()
    : m_calcChecksum(true), m_checksum(0), m_type(0), m_code(0) {
  NS_LOG_FUNCTION(this);
}

Icmpv6Header::~Icmpv6Header() { NS_LOG_FUNCTION(this); }
...
void Icmpv6Header::CalculatePseudoHeaderChecksum(Ipv6Address src,
                                                 Ipv6Address dst,
                                                 uint16_t length,
                                                 uint8_t protocol) {
...
void Icmpv6NS::Print(std::ostream &os) const {
  ...
  os << "( type = " << (uint32_t)GetType()
     << " (NS) code = " << (uint32_t)GetCode() << " target = " << m_target
...
Icmpv6NA::~Icmpv6NA() { NS_LOG_FUNCTION(this); }
...
Icmpv6RA::~Icmpv6RA() { NS_LOG_FUNCTION(this); }
...
Icmpv6RS::~Icmpv6RS() { NS_LOG_FUNCTION(this); }
...
Icmpv6Redirection::~Icmpv6Redirection() { NS_LOG_FUNCTION(this); }
...
void Icmpv6Redirection::Print(std::ostream &os) const {
  NS_LOG_FUNCTION(this << &os);
  os << "( type = " << (uint32_t)GetType()
     << " (Redirection) code = " << (uint32_t)GetCode()
...
Icmpv6Echo::Icmpv6Echo(bool request) {
  ...
  SetType(request ? Icmpv6Header::ICMPV6_ECHO_REQUEST
...
Icmpv6Echo::~Icmpv6Echo() { NS_LOG_FUNCTION(this); }
...
Icmpv6TooBig::~Icmpv6TooBig() { NS_LOG_FUNCTION(this); }
...
Icmpv6TimeExceeded::~Icmpv6TimeExceeded() { NS_LOG_FUNCTION(this); }
...
Icmpv6ParameterError::~Icmpv6ParameterError() { NS_LOG_FUNCTION(this); }
...
Icmpv6OptionHeader::~Icmpv6OptionHeader() { NS_LOG_FUNCTION(this); }
...
Icmpv6OptionMtu::~Icmpv6OptionMtu() { NS_LOG_FUNCTION(this); }
...
void Icmpv6OptionMtu::Print(std::ostream &os) const {
  NS_LOG_FUNCTION(this << &os);
  os << "( type = " << (uint32_t)GetType()
     << " length = " << (uint32_t)GetLength() << " MTU = " << m_mtu << ")";
}
...
void Icmpv6OptionPrefixInformation::Print(std::ostream &os) const {
  NS_LOG_FUNCTION(this << &os);
  os << "( type = " << (uint32_t)GetType()
     << " length = " << (uint32_t)GetLength() << " prefix " << m_prefix << ")";
}
...
void Icmpv6OptionLinkLayerAddress::Print(std::ostream &os) const {
  NS_LOG_FUNCTION(this << &os);
  os << "( type = " << (uint32_t)GetType()
     << " length = " << (uint32_t)GetLength() << " L2 Address = " << m_addr
     << ")";
}
