...
ArpQueueDiscItem::~ArpQueueDiscItem() { NS_LOG_FUNCTION(this); }
...
const ArpHeader &ArpQueueDiscItem::GetHeader() const { return m_header; }
...
void ArpQueueDiscItem::Print(std::ostream &os) const {
  if (!m_headerAdded) {
    os << m_header << " ";
  }
  os << GetPacket() << " "
...
bool ArpQueueDiscItem::Mark() {
  NS_LOG_FUNCTION(this);
  return false;
}

uint32_t ArpQueueDiscItem::Hash(uint32_t perturbation) const {
