...
public:
  ...
  struct TcpRateSample {
    ...
    bool m_isAppLimited{
        false}; //!< Indicates whether the rate sample is application-limited
    Time m_interval{Seconds(0.0)}; //!< The length of the sampling interval
    int32_t m_delivered{0}; //!< The amount of data marked as delivered over the
                            //!< sampling interval
    uint32_t m_priorDelivered{
        0}; //!< The delivered count of the most recent packet delivered
    Time m_priorTime{Seconds(
        0.0)}; //!< The delivered time of the most recent packet delivered
    ...
    uint32_t m_priorInFlight{
        0}; //!< The value if bytes in flight prior to last received ack
    uint32_t m_ackedSacked{
        0}; //!< The amount of data acked and sacked in the last received ack
  ...
  struct TcpRateConnection {
    ...
    Time m_deliveredTime{
        Seconds(0)}; //!< Simulator time when m_delivered was last updated
    ...
    uint32_t m_txItemDelivered{
        0}; //!< The value of delivered when the acked item was sent
    int32_t m_rateDelivered{0}; //!< The amount of data delivered considered to
                                //!< calculate delivery rate.
    Time m_rateInterval{Seconds(
        0)}; //!< The value of interval considered to calculate delivery rate.
    bool m_rateAppLimited{
        false}; //!< Was sample was taken when data is app limited?
  };
};
...
private:
  ...
  TracedCallback<const TcpRateSample &>
      m_rateSampleTrace; //!< Rate Sample trace
};
