...
Ipv6QueueDiscItem::~Ipv6QueueDiscItem() { NS_LOG_FUNCTION(this); }
...
const Ipv6Header &Ipv6QueueDiscItem::GetHeader() const { return m_header; }
...
void Ipv6QueueDiscItem::Print(std::ostream &os) const {
  if (!m_headerAdded) {
    os << m_header << " ";
  }
  os << GetPacket() << " "
...
bool Ipv6QueueDiscItem::GetUint8Value(QueueItem::Uint8Values field,
                                      uint8_t &value) const {
  bool ret = false;
...
uint32_t Ipv6QueueDiscItem::Hash(uint32_t perturbation) const {
