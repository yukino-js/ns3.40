...
public:
  ...
  int Bind(
      const Address &address) override; // ... endpoint of specific addr or port
  int Connect(const Address &address)
      override;          // Setup endpoint and call ProcessAction() to connect
  ...
  int ShutdownSend()
      override; // Assert the m_shutdownSend flag to prevent send to network
  int ShutdownRecv()
      override; // Assert the m_shutdownRecv flag to prevent forward to app
  int Send(Ptr<Packet> p,
           uint32_t flags) override; // Call by app to send data to network
  int SendTo(Ptr<Packet> p, uint32_t flags,
             const Address &toAddress)
      override; // Same as Send(), toAddress is insignificant
  Ptr<Packet>
  Recv(uint32_t maxSize,
  ...
  uint32_t GetRxAvailable() const
      override; // Available-to-read data size, i.e. value of m_rxAvailable
  int GetSockName(
      Address &address) const override; // Return local addr:port in address
  int GetPeerName(Address &address) const override;
  void BindToNetDevice(
      Ptr<NetDevice> netdevice) override; // NetDevice with my m_endPoint
...
protected:
  ...
  uint32_t m_delAckMaxCount{
      0}; //!< Number of packet to fire an ACK before delay timeout
  ...
  uint32_t m_dataRetrCount{
      0}; //!< Count of remaining data retransmission attempts
  ...
  Time m_clockGranularity{
      Seconds(0.001)};                 //!< Clock Granularity used in RTO calcs
  ...
  bool m_recoverActive{
      false}; //!< Whether "m_recover" has been set/activated
  ...
  TracedValue<SequenceNumber32> m_ecnCWRSeq{
      0}; //!< Sequence number of the last sent CWR
};
