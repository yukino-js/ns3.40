...
Ipv4Header::Ipv4Header()
    : m_calcChecksum(false), m_payloadSize(0), m_identification(0), m_tos(0),
      m_ttl(0), m_protocol(0), m_flags(0), m_fragmentOffset(0), m_checksum(0),
      m_goodChecksum(true), m_headerSize(5 * 4) {}

void Ipv4Header::EnableChecksum() {
  NS_LOG_FUNCTION(this);
  m_calcChecksum = true;
}

void Ipv4Header::SetPayloadSize(uint16_t size) {
  NS_LOG_FUNCTION(this << size);
  m_payloadSize = size;
}

uint16_t Ipv4Header::GetPayloadSize() const {
  NS_LOG_FUNCTION(this);
...
void Ipv4Header::SetTos(uint8_t tos) {
...
std::string Ipv4Header::DscpTypeToString(DscpType dscp) const {
  NS_LOG_FUNCTION(this << dscp);
  switch (dscp) {
  case DscpDefault:
    return "Default";
...
std::string Ipv4Header::EcnTypeToString(EcnType ecn) const {
  NS_LOG_FUNCTION(this << ecn);
  switch (ecn) {
  case ECN_NotECT:
    return "Not-ECT";
...
uint8_t Ipv4Header::GetTos() const {
  NS_LOG_FUNCTION(this);
  return m_tos;
}

void Ipv4Header::SetMoreFragments() {
  NS_LOG_FUNCTION(this);
  m_flags |= MORE_FRAGMENTS;
}

void Ipv4Header::SetLastFragment() {
  NS_LOG_FUNCTION(this);
  m_flags &= ~MORE_FRAGMENTS;
}

bool Ipv4Header::IsLastFragment() const {
  NS_LOG_FUNCTION(this);
  return !(m_flags & MORE_FRAGMENTS);
}

void Ipv4Header::SetDontFragment() {
  NS_LOG_FUNCTION(this);
  m_flags |= DONT_FRAGMENT;
}

void Ipv4Header::SetMayFragment() {
  NS_LOG_FUNCTION(this);
  m_flags &= ~DONT_FRAGMENT;
}

bool Ipv4Header::IsDontFragment() const {
  NS_LOG_FUNCTION(this);
...
void Ipv4Header::SetSource(Ipv4Address source) {
  NS_LOG_FUNCTION(this << source);
  m_source = source;
}

Ipv4Address Ipv4Header::GetSource() const {
  NS_LOG_FUNCTION(this);
  return m_source;
}

void Ipv4Header::SetDestination(Ipv4Address dst) {
  NS_LOG_FUNCTION(this << dst);
  m_destination = dst;
}

Ipv4Address Ipv4Header::GetDestination() const {
  NS_LOG_FUNCTION(this);
  return m_destination;
}

bool Ipv4Header::IsChecksumOk() const {
  NS_LOG_FUNCTION(this);
  return m_goodChecksum;
}

TypeId Ipv4Header::GetTypeId() {
...
TypeId Ipv4Header::GetInstanceTypeId() const {
  NS_LOG_FUNCTION(this);
  return GetTypeId();
}
...
void Ipv4Header::Print(std::ostream &os) const {
  ...
  os << "tos 0x" << std::hex << m_tos << std::dec << " "
     ...
     << "flags [" << flags << "] "
     << "length: " << (m_payloadSize + 5 * 4) << " " << m_source << " > "
     << m_destination;
}
...
void Ipv4Header::Serialize(Buffer::Iterator start) const {
...
uint32_t Ipv4Header::Deserialize(Buffer::Iterator start) {
  NS_LOG_FUNCTION(this << &start);
  Buffer::Iterator i = start;
