...
public:
  ...
  struct RateInformation {
    uint64_t m_delivered{
        0}; //!< Connection's delivered data at the time the packet was sent
    Time m_deliveredTime{Time::Max()}; //!< Connection's delivered time at the
                                       //!< time the packet was sent
    Time m_firstSent{Time::Max()}; //!< Connection's first sent time at the time
                                   //!< the packet was sent
    bool m_isAppLimited{
        false}; //!< Connection's app limited at the time the packet was sent
  };
...
private:
  ...
  SequenceNumber32 m_startSeq{
      0}; //!< Sequence number of the item (if transmitted)
