...
TypeId TcpOptionTS::GetInstanceTypeId() const { return GetTypeId(); }

void TcpOptionTS::Print(std::ostream &os) const {
  os << m_timestamp << ";" << m_echo;
}

uint32_t TcpOptionTS::GetSerializedSize() const { return 10; }
...
uint8_t TcpOptionTS::GetKind() const { return TcpOption::TS; }

uint32_t TcpOptionTS::GetTimestamp() const { return m_timestamp; }

uint32_t TcpOptionTS::GetEcho() const { return m_echo; }

void TcpOptionTS::SetTimestamp(uint32_t ts) { m_timestamp = ts; }

void TcpOptionTS::SetEcho(uint32_t ts) { m_echo = ts; }
...
Time TcpOptionTS::ElapsedTimeFromTsValue(uint32_t echoTime) {
