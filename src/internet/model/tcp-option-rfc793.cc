...
TypeId TcpOptionEnd::GetInstanceTypeId() const { return GetTypeId(); }

void TcpOptionEnd::Print(std::ostream &os) const { os << "EOL"; }

uint32_t TcpOptionEnd::GetSerializedSize() const { return 1; }

void TcpOptionEnd::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
  i.WriteU8(GetKind());
}

uint32_t TcpOptionEnd::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
TypeId TcpOptionNOP::GetInstanceTypeId() const { return GetTypeId(); }

void TcpOptionNOP::Print(std::ostream &os) const { os << "NOP"; }

uint32_t TcpOptionNOP::GetSerializedSize() const { return 1; }

void TcpOptionNOP::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
  i.WriteU8(GetKind());
}

uint32_t TcpOptionNOP::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
TypeId TcpOptionMSS::GetInstanceTypeId() const { return GetTypeId(); }

void TcpOptionMSS::Print(std::ostream &os) const { os << "MSS:" << m_mss; }

uint32_t TcpOptionMSS::GetSerializedSize() const { return 4; }
...
uint8_t TcpOptionMSS::GetKind() const { return TcpOption::MSS; }

uint16_t TcpOptionMSS::GetMSS() const { return m_mss; }

void TcpOptionMSS::SetMSS(uint16_t mss) { m_mss = mss; }
