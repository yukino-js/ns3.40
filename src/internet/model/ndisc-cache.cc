...
NdiscCache::NdiscCache() { NS_LOG_FUNCTION(this); }
...
NdiscCache::Entry::Entry(NdiscCache *nd)
    : m_ndCache(nd), m_waiting(), m_router(false),
      m_nudTimer(Timer::CANCEL_ON_DESTROY),
      m_lastReachabilityConfirmation(Seconds(0.0)), m_nsRetransmit(0) {
  NS_LOG_FUNCTION(this);
}
...
void NdiscCache::RemoveAutoGeneratedEntries() {
  NS_LOG_FUNCTION(this);
