...
private:
  ...
  ListOfNodeExit_t
      m_ecmpRootExits; //!< store the multiple root's exits for supporting ECMP
  typedef std::list<SPFVertex *>
      ListOfSPFVertex_t;        //!< container of SPFVertexes
  ListOfSPFVertex_t m_parents;  //!< parent list
  ListOfSPFVertex_t m_children; //!< Children list
  bool m_vertexProcessed; //!< Flag to note whether vertex has been processed in
                          //!< stage two of SPF computation
...
private:
  ...
  LSDBMap_t
      m_database; //!< database of IPv4 addresses / Link State Advertisements
