...
Rip::Rip()
    : m_ipv4(nullptr), m_splitHorizonStrategy(Rip::POISON_REVERSE),
      m_initialized(false) {
...
void Rip::DoInitialize() {
  ...
  m_nextUnsolicitedUpdate =
      Simulator::Schedule(delay, &Rip::SendUnsolicitedRouteUpdate, this);
    ...
    for (uint32_t j = 0; j < m_ipv4->GetNAddresses(i); j++) {
      ...
      if (address.GetScope() != Ipv4InterfaceAddress::HOST && activeInterface) {
        ...
        InetSocketAddress local =
            InetSocketAddress(address.GetLocal(), RIP_PORT);
      ...
      } else if (m_ipv4->GetAddress(i, j).GetScope() ==
  ...
  if (addedGlobal) {
    ...
    m_nextTriggeredUpdate =
        Simulator::Schedule(delay, &Rip::DoSendRouteUpdate, this, false);
  }

  delay = Seconds(m_rng->GetValue(0.01, m_startupDelay.GetSeconds()));
  m_nextTriggeredUpdate =
      Simulator::Schedule(delay, &Rip::SendRouteRequest, this);

  Ipv4RoutingProtocol::DoInitialize();
}

Ptr<Ipv4Route> Rip::RouteOutput(Ptr<Packet> p, const Ipv4Header &header,
                                Ptr<NetDevice> oif,
...
bool Rip::RouteInput(Ptr<const Packet> p, const Ipv4Header &header,
...
void Rip::NotifyInterfaceUp(uint32_t i) {
  ...
  for (uint32_t j = 0; j < m_ipv4->GetNAddresses(i); j++) {
    ...
    if (address.GetScope() != Ipv4InterfaceAddress::HOST && !sendSocketFound &&
        activeInterface) {
...
void Rip::NotifyRemoveAddress(uint32_t interface,
...
void Rip::PrintRoutingTable(Ptr<OutputStreamWrapper> stream,
...
Ptr<Ipv4Route> Rip::Lookup(Ipv4Address dst, bool setSource,
...
void Rip::AddNetworkRouteTo(Ipv4Address network, Ipv4Mask networkPrefix,
...
void Rip::Receive(Ptr<Socket> socket) {
  ...
  if (hdr.GetCommand() == RipHeader::RESPONSE) {
    NS_LOG_LOGIC("The message is a Response from "
                 << senderAddr.GetIpv4() << ":" << senderAddr.GetPort());
...
void Rip::HandleRequests(RipHeader requestHdr, Ipv4Address senderAddress,
  ...
  if (rtes.size() == 1) {
    if (rtes.begin()->GetPrefix() == Ipv4Address::GetAny() &&
      ...
      if (m_interfaceExclusions.find(incomingInterface) ==
        ...
        for (auto rtIter = m_routes.begin(); rtIter != m_routes.end();
          ...
          bool isGlobal =
              (rtDestAddr.GetScope() == Ipv4InterfaceAddress::GLOBAL);
          bool isDefaultRoute =
          ...
          if ((isGlobal || isDefaultRoute) &&
              (rtIter->first->GetRouteStatus() ==
               RipRoutingTableEntry::RIP_VALID)) {
            RipRte rte;
          ...
          if (hdr.GetRteNumber() == maxRte) {
            ...
            sendingSocket->SendTo(p, 0,
                                  InetSocketAddress(senderAddress, RIP_PORT));
            p->RemoveHeader(hdr);
            hdr.ClearRtes();
          }
        }
    ...
    p->AddHeader(hdr);
    NS_LOG_DEBUG("SendTo: " << *p);
    m_multicastRecvSocket->SendTo(p, 0,
                                  InetSocketAddress(senderAddress, senderPort));
  }
}

void Rip::HandleResponses(RipHeader hdr, Ipv4Address senderAddress,
  ...
  for (auto iter = rtes.begin(); iter != rtes.end(); iter++) {
      ...
      if (it->first->GetDestNetwork() == rteAddr &&
        ...
        if (rteMetric < it->first->GetRouteMetric()) {
          if (senderAddress != it->first->GetGateway()) {
            auto route = new RipRoutingTableEntry(
          ...
          it->second = Simulator::Schedule(
              m_timeoutDelay, &Rip::InvalidateRoute, this, it->first);
          changed = true;
          ...
          if (senderAddress == it->first->GetGateway()) {
            ...
            if (Simulator::GetDelayLeft(it->second) < m_timeoutDelay / 2) {
              ...
              it->second.Cancel();
              it->second = Simulator::Schedule(
                  m_timeoutDelay, &Rip::InvalidateRoute, this, route);
              changed = true;
            }
          }
        } else if (rteMetric > it->first->GetRouteMetric() &&
    ...
    if (!found && rteMetric != m_linkDown) {
      ...
      auto route = new RipRoutingTableEntry(rteAddr, rtePrefixMask,
...
void Rip::DoSendRouteUpdate(bool periodic) {
  ...
  for (auto iter = m_unicastSocketList.begin();
    ...
    if (m_interfaceExclusions.find(interface) == m_interfaceExclusions.end()) {
      ...
      uint16_t maxRte =
          (mtu - Ipv4Header().GetSerializedSize() -
      ...
      for (auto rtIter = m_routes.begin(); rtIter != m_routes.end(); rtIter++) {
        ...
        for (uint32_t index = 0; index < m_ipv4->GetNAddresses(interface);
             index++) {
        ...
        if ((isGlobal || isDefaultRoute) &&
            (periodic || rtIter->first->IsRouteChanged()) && !sameNetwork) {
          RipRte rte;
          rte.SetPrefix(rtIter->first->GetDestNetwork());
          rte.SetSubnetMask(rtIter->first->GetDestNetworkMask());
          if (m_splitHorizonStrategy == POISON_REVERSE && splitHorizoning) {
...
RipRoutingTableEntry::RipRoutingTableEntry(Ipv4Address network,
                                           ...
                                           Ipv4Address nextHop,
                                           uint32_t interface)
    : Ipv4RoutingTableEntry(Ipv4RoutingTableEntry::CreateNetworkRouteTo(
          network, networkPrefix, nextHop, interface)),
      m_tag(0), m_metric(0), m_status(RIP_INVALID), m_changed(false) {}
...
uint16_t RipRoutingTableEntry::GetRouteTag() const { return m_tag; }
...
uint8_t RipRoutingTableEntry::GetRouteMetric() const { return m_metric; }
...
bool RipRoutingTableEntry::IsRouteChanged() const { return m_changed; }
