...
TypeId TcpOption::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TcpOption").SetParent<Object>().SetGroupName("Internet");
  return tid;
}

TypeId TcpOption::GetInstanceTypeId() const { return GetTypeId(); }
...
TypeId TcpOptionUnknown::GetInstanceTypeId() const { return GetTypeId(); }

void TcpOptionUnknown::Print(std::ostream &os) const { os << "Unknown option"; }

uint32_t TcpOptionUnknown::GetSerializedSize() const { return m_size; }
...
uint32_t TcpOptionUnknown::Deserialize(Buffer::Iterator start) {
  ...
  m_kind = i.ReadU8();
  NS_LOG_WARN("Trying to Deserialize an Unknown Option of Kind "
              << int(m_kind));
...
uint8_t TcpOptionUnknown::GetKind() const { return m_kind; }
