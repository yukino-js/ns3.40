...
std::string TcpHeader::FlagsToString(uint8_t flags,
                                     const std::string &delimiter) {
  static const char *flagNames[8] = {"FIN", "SYN", "RST", "PSH",
                                     "ACK", "URG", "ECE", "CWR"};
...
void TcpHeader::EnableChecksums() { m_calcChecksum = true; }

void TcpHeader::SetSourcePort(uint16_t port) { m_sourcePort = port; }

void TcpHeader::SetDestinationPort(uint16_t port) { m_destinationPort = port; }
...
void TcpHeader::SetFlags(uint8_t flags) { m_flags = flags; }
...
uint16_t TcpHeader::GetSourcePort() const { return m_sourcePort; }

uint16_t TcpHeader::GetDestinationPort() const { return m_destinationPort; }

SequenceNumber32 TcpHeader::GetSequenceNumber() const {
  return m_sequenceNumber;
}

SequenceNumber32 TcpHeader::GetAckNumber() const { return m_ackNumber; }

uint8_t TcpHeader::GetLength() const { return m_length; }

uint8_t TcpHeader::GetOptionLength() const { return m_optionsLen; }

uint8_t TcpHeader::GetMaxOptionLength() const { return m_maxOptionsLen; }

uint8_t TcpHeader::GetFlags() const { return m_flags; }

uint16_t TcpHeader::GetWindowSize() const { return m_windowSize; }

uint16_t TcpHeader::GetUrgentPointer() const { return m_urgentPointer; }

void TcpHeader::InitializeChecksum(const Ipv4Address &source,
...
void TcpHeader::InitializeChecksum(const Ipv6Address &source,
...
void TcpHeader::InitializeChecksum(const Address &source,
...
bool TcpHeader::IsChecksumOk() const { return m_goodChecksum; }

TypeId TcpHeader::GetTypeId() {
...
TypeId TcpHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
uint32_t TcpHeader::Deserialize(Buffer::Iterator start) {
  ...
  if (optionLen > m_maxOptionsLen) {
    NS_LOG_ERROR("Illegal TCP option length " << optionLen
                                              << "; options discarded");
    return 20;
  }
  while (optionLen) {
    ...
    if (TcpOption::IsKindKnown(kind)) {
      ...
      NS_LOG_WARN("Option kind " << static_cast<int>(kind)
                                 << " unknown, skipping.");
    }
...
uint8_t TcpHeader::CalculateHeaderLength() const {
  uint32_t len = 20;
...
bool TcpHeader::AppendOption(Ptr<const TcpOption> option) {
...
const TcpHeader::TcpOptionList &TcpHeader::GetOptionList() const {
  return m_options;
}

Ptr<const TcpOption> TcpHeader::GetOption(uint8_t kind) const {
  for (auto i = m_options.begin(); i != m_options.end(); ++i) {
    if ((*i)->GetKind() == kind) {
      return (*i);
    }
...
bool TcpHeader::HasOption(uint8_t kind) const {
  for (auto i = m_options.begin(); i != m_options.end(); ++i) {
    if ((*i)->GetKind() == kind) {
      return true;
    }
...
bool operator==(const TcpHeader &lhs, const TcpHeader &rhs) {
  return (lhs.m_sourcePort == rhs.m_sourcePort &&
          ...
          lhs.m_ackNumber == rhs.m_ackNumber && lhs.m_flags == rhs.m_flags &&
