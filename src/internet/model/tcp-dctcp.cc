...
TypeId TcpDctcp::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TcpDctcp")
          ...
          .AddAttribute("DctcpAlphaOnInit", "Initial alpha value",
                        DoubleValue(1.0),
...
std::string TcpDctcp::GetName() const { return "TcpDctcp"; }
...
TcpDctcp::TcpDctcp(const TcpDctcp &sock)
    : TcpLinuxReno(sock), m_ackedBytesEcn(sock.m_ackedBytesEcn),
      m_ackedBytesTotal(sock.m_ackedBytesTotal),
      m_priorRcvNxt(sock.m_priorRcvNxt),
      m_priorRcvNxtFlag(sock.m_priorRcvNxtFlag), m_alpha(sock.m_alpha),
      m_nextSeq(sock.m_nextSeq), m_nextSeqFlag(sock.m_nextSeqFlag),
...
TcpDctcp::~TcpDctcp() { NS_LOG_FUNCTION(this); }
...
void TcpDctcp::Init(Ptr<TcpSocketState> tcb) {
...
void TcpDctcp::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
                         const Time &rtt) {
...
void TcpDctcp::InitializeDctcpAlpha(double alpha) {
  NS_LOG_FUNCTION(this << alpha);
  NS_ABORT_MSG_IF(m_initialized, "DCTCP has already been initialized");
  m_alpha = alpha;
}

void TcpDctcp::Reset(Ptr<TcpSocketState> tcb) {
  NS_LOG_FUNCTION(this << tcb);
  m_nextSeq = tcb->m_nextTxSequence;
  m_ackedBytesEcn = 0;
  m_ackedBytesTotal = 0;
...
void TcpDctcp::CeState0to1(Ptr<TcpSocketState> tcb) {
  NS_LOG_FUNCTION(this << tcb);
...
void TcpDctcp::CeState1to0(Ptr<TcpSocketState> tcb) {
  NS_LOG_FUNCTION(this << tcb);
  if (m_ceState && m_delayedAckReserved && m_priorRcvNxtFlag) {
    SequenceNumber32 tmpRcvNxt;
    /* Save current NextRxSequence. */
...
void TcpDctcp::UpdateAckReserved(Ptr<TcpSocketState> tcb,
...
void TcpDctcp::CwndEvent(Ptr<TcpSocketState> tcb,
                         const TcpSocketState::TcpCAEvent_t event) {
  NS_LOG_FUNCTION(this << tcb << event);
  switch (event) {
