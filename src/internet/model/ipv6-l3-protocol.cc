...
TypeId Ipv6L3Protocol::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ipv6L3Protocol")
          ...
          .AddAttribute(
              "SendIcmpv6Redirect",
              "Send the ICMPv6 Redirect when appropriate.", BooleanValue(true),
...
Ipv6L3Protocol::Ipv6L3Protocol() : m_nInterfaces(0) {
  ...
  Ptr<Ipv6RawSocketFactoryImpl> rawFactoryImpl =
      CreateObject<Ipv6RawSocketFactoryImpl>();
...
Ipv6L3Protocol::~Ipv6L3Protocol() { NS_LOG_FUNCTION(this); }
...
void Ipv6L3Protocol::SetRoutingProtocol(
...
int32_t Ipv6L3Protocol::GetInterfaceForPrefix(Ipv6Address address,
...
void Ipv6L3Protocol::AddAutoconfiguredAddress(
    uint32_t interface, Ipv6Address network, Ipv6Prefix mask, uint8_t flags,
    uint32_t validTime, uint32_t preferredTime, Ipv6Address defaultRouter) {
  NS_LOG_FUNCTION(this << interface << network << mask << (uint32_t)flags
  ...
  if (!defaultRouter.IsAny()) {
    GetRoutingProtocol()->NotifyAddRoute(Ipv6Address::GetAny(),
                                         Ipv6Prefix((uint8_t)0), defaultRouter,
                                         interface, network);
  }
  ...
  if (flags & Icmpv6OptionPrefixInformation::AUTADDRCONF) /* auto flag */
  {
    ...
    Ptr<Ipv6AutoconfiguredPrefix> aPrefix =
        CreateObject<Ipv6AutoconfiguredPrefix>(m_node, interface, network, mask,
...
void Ipv6L3Protocol::RemoveAutoconfiguredAddress(uint32_t interface,
                                                 Ipv6Address network,
                                                 Ipv6Prefix mask,
  ...
  Ipv6Address addressToFind =
      Ipv6Address::MakeAutoconfiguredAddress(addr, network);
...
bool Ipv6L3Protocol::AddAddress(uint32_t i, Ipv6InterfaceAddress address,
  ...
  if (addOnLinkRoute) {
    ...
    GetRoutingProtocol()->NotifyAddRoute(networkAddress, networkMask,
                                         Ipv6Address::GetZero(), i);
  }
  return ret;
}
...
Ipv6InterfaceAddress Ipv6L3Protocol::GetAddress(uint32_t i,
...
Ipv6Address Ipv6L3Protocol::SourceAddressSelection(uint32_t interface,
                                                   Ipv6Address dest) {
...
void Ipv6L3Protocol::Insert(Ptr<IpL4Protocol> protocol) {
  ...
  if (m_protocols.find(key) != m_protocols.end()) {
    NS_LOG_WARN("Overwriting default protocol "
...
void Ipv6L3Protocol::Insert(Ptr<IpL4Protocol> protocol,
  ...
  L4ListKey_t key =
      std::make_pair(protocol->GetProtocolNumber(), interfaceIndex);
  if (m_protocols.find(key) != m_protocols.end()) {
...
Ptr<IpL4Protocol> Ipv6L3Protocol::GetProtocol(int protocolNumber,
...
Ptr<Icmpv6L4Protocol> Ipv6L3Protocol::GetIcmpv6() const {
  NS_LOG_FUNCTION(this);
  Ptr<IpL4Protocol> protocol =
      GetProtocol(Icmpv6L4Protocol::GetStaticProtocolNumber());
...
void Ipv6L3Protocol::Send(Ptr<Packet> packet, Ipv6Address source,
...
void Ipv6L3Protocol::Receive(Ptr<NetDevice> device, Ptr<const Packet> p,
  ...
  NS_ASSERT_MSG(
      GetInterfaceForDevice(device) != -1,
  ...
  if (nextHeader == Ipv6Header::IPV6_EXT_HOP_BY_HOP) {
    ...
    if (ipv6Extension) {
      ipv6Extension->Process(packet, 0, hdr, hdr.GetDestination(),
                             (uint8_t *)nullptr, stopProcessing, isDropped,
                             dropReason);
    }
...
void Ipv6L3Protocol::CallTxTrace(const Ipv6Header &ipHeader, Ptr<Packet> packet,
...
void Ipv6L3Protocol::SendRealOut(Ptr<Ipv6Route> route, Ptr<Packet> packet,
...
void Ipv6L3Protocol::IpForward(Ptr<const NetDevice> idev,
                               Ptr<Ipv6Route> rtentry, Ptr<const Packet> p,
  ...
  if (m_sendIcmpv6Redirect && (rtentry->GetOutputDevice() == idev)) {
    ...
    Ipv6Address linkLocal =
        GetInterface(GetInterfaceForDevice(rtentry->GetOutputDevice()))
            ->GetLinkLocalAddress()
            .GetAddress();

    if (icmpv6->Lookup(target, rtentry->GetOutputDevice(), nullptr,
                       &hardwareTarget)) {
      icmpv6->SendRedirection(copy, linkLocal, src, target, dst,
                              hardwareTarget);
    } else {
...
void Ipv6L3Protocol::IpMulticastForward(Ptr<const NetDevice> idev,
...
void Ipv6L3Protocol::LocalDeliver(Ptr<const Packet> packet,
                                  const Ipv6Header &ip, uint32_t iif) {
  ...
  do {
    ...
    if (ipv6Extension) {
      ...
      nextHeaderStep =
          ipv6Extension->Process(p, nextHeaderPosition, ip, dst, &nextHeader,
      ...
      NS_ASSERT_MSG(nextHeaderStep != 0 ||
                        curHeader == Ipv6Header::IPV6_EXT_FRAGMENTATION,
...
void Ipv6L3Protocol::RouteInputError(Ptr<const Packet> p,
                                     const Ipv6Header &ipHeader,
...
Ipv6Header Ipv6L3Protocol::BuildHeader(Ipv6Address src, Ipv6Address dst,
                                       uint8_t protocol, uint16_t payloadSize,
                                       uint8_t ttl, uint8_t tclass) {
  NS_LOG_FUNCTION(this << src << dst << (uint32_t)protocol
                       << (uint32_t)payloadSize << (uint32_t)ttl
                       << (uint32_t)tclass);
  Ipv6Header hdr;
...
void Ipv6L3Protocol::RegisterExtensions() {
  ...
  Ptr<Ipv6ExtensionDemux> ipv6ExtensionDemux =
      CreateObject<Ipv6ExtensionDemux>();
  ipv6ExtensionDemux->SetNode(m_node);

  Ptr<Ipv6ExtensionHopByHop> hopbyhopExtension =
      CreateObject<Ipv6ExtensionHopByHop>();
  hopbyhopExtension->SetNode(m_node);
  Ptr<Ipv6ExtensionDestination> destinationExtension =
      CreateObject<Ipv6ExtensionDestination>();
  destinationExtension->SetNode(m_node);
  Ptr<Ipv6ExtensionFragment> fragmentExtension =
      CreateObject<Ipv6ExtensionFragment>();
...
void Ipv6L3Protocol::RegisterOptions() {
  ...
  Ptr<Ipv6OptionJumbogram> jumbogramOption =
      CreateObject<Ipv6OptionJumbogram>();
  jumbogramOption->SetNode(m_node);
  Ptr<Ipv6OptionRouterAlert> routerAlertOption =
      CreateObject<Ipv6OptionRouterAlert>();
...
void Ipv6L3Protocol::ReportDrop(Ipv6Header ipHeader, Ptr<Packet> p,
                                DropReason dropReason) {
...
bool Ipv6L3Protocol::IsRegisteredMulticastAddress(Ipv6Address address,
...
bool Ipv6L3Protocol::ReachabilityHint(uint32_t ipInterfaceIndex,
                                      Ipv6Address address) {
  ...
  if (entry->IsReachable()) {
    ...
    for (auto it = waiting.begin(); it != waiting.end(); it++) {
      ndiscCache->GetInterface()->Send(it->first, it->second,
                                       it->second.GetSource());
    }
