  ...
  static TypeId tid =
      TypeId("ns3::TcpSocketBase")
          ...
          .AddAttribute(
              ...
              TimeValue(Seconds(
                  1.0)), // RFC 6298 says min RTO=1 sec, but Linux uses 200ms.
              // See
          ...
          .AddAttribute(
              ...
              TimeValue(MilliSeconds(
                  1)), // RFC6298 suggest to use fine clock granularity
          ...
          .AddAttribute(
              ...
              MakeEnumChecker(TcpSocketState::Off, "Off", TcpSocketState::On,
                              "On", TcpSocketState::AcceptOnly, "AcceptOnly"))
...
TypeId TcpSocketBase::GetInstanceTypeId() const {
...
TcpSocketBase::TcpSocketBase() : TcpSocket() {
  ...
  m_tcb->m_sendEmptyPacketCallback =
      MakeCallback(&TcpSocketBase::SendEmptyPacket, this);
  ...
  ok = m_tcb->TraceConnectWithoutContext(
      "CongestionWindow", MakeCallback(&TcpSocketBase::UpdateCwnd, this));
  NS_ASSERT(ok == true);
  ...
  ok = m_tcb->TraceConnectWithoutContext(
      "SlowStartThreshold", MakeCallback(&TcpSocketBase::UpdateSsThresh, this));
  NS_ASSERT(ok == true);

  ok = m_tcb->TraceConnectWithoutContext(
      "CongState", MakeCallback(&TcpSocketBase::UpdateCongState, this));
  NS_ASSERT(ok == true);

  ok = m_tcb->TraceConnectWithoutContext(
      "EcnState", MakeCallback(&TcpSocketBase::UpdateEcnState, this));
  NS_ASSERT(ok == true);
  ...
  ok = m_tcb->TraceConnectWithoutContext(
      "HighestSequence", MakeCallback(&TcpSocketBase::UpdateHighTxMark, this));
  NS_ASSERT(ok == true);

  ok = m_tcb->TraceConnectWithoutContext(
      "BytesInFlight", MakeCallback(&TcpSocketBase::UpdateBytesInFlight, this));
  NS_ASSERT(ok == true);

  ok = m_tcb->TraceConnectWithoutContext(
      "RTT", MakeCallback(&TcpSocketBase::UpdateRtt, this));
  NS_ASSERT(ok == true);
}
...
TcpSocketBase::TcpSocketBase(const TcpSocketBase &sock)
    : TcpSocket(sock),
      ...
      m_dupAckCount(sock.m_dupAckCount), m_delAckCount(0),
      m_delAckMaxCount(sock.m_delAckMaxCount), m_noDelay(sock.m_noDelay),
      m_synCount(sock.m_synCount), m_synRetries(sock.m_synRetries),
      m_dataRetrCount(sock.m_dataRetrCount), m_dataRetries(sock.m_dataRetries),
      m_rto(sock.m_rto), m_minRto(sock.m_minRto),
      m_clockGranularity(sock.m_clockGranularity),
      m_delAckTimeout(sock.m_delAckTimeout),
      m_persistTimeout(sock.m_persistTimeout), m_cnTimeout(sock.m_cnTimeout),
      m_endPoint(nullptr), m_endPoint6(nullptr), m_node(sock.m_node),
      m_tcp(sock.m_tcp), m_state(sock.m_state), m_errno(sock.m_errno),
      m_closeNotified(sock.m_closeNotified),
      m_closeOnEmpty(sock.m_closeOnEmpty), m_shutdownSend(sock.m_shutdownSend),
      m_shutdownRecv(sock.m_shutdownRecv), m_connected(sock.m_connected),
      m_msl(sock.m_msl), m_maxWinSize(sock.m_maxWinSize),
      m_bytesAckedNotProcessed(sock.m_bytesAckedNotProcessed),
      m_rWnd(sock.m_rWnd), m_highRxMark(sock.m_highRxMark),
      m_highRxAckMark(sock.m_highRxAckMark), m_sackEnabled(sock.m_sackEnabled),
      m_winScalingEnabled(sock.m_winScalingEnabled),
      m_rcvWindShift(sock.m_rcvWindShift), m_sndWindShift(sock.m_sndWindShift),
      m_timestampEnabled(sock.m_timestampEnabled),
      m_timestampToEcho(sock.m_timestampToEcho), m_recover(sock.m_recover),
      m_recoverActive(sock.m_recoverActive), m_retxThresh(sock.m_retxThresh),
      m_limitedTx(sock.m_limitedTx),
      m_isFirstPartialAck(sock.m_isFirstPartialAck), m_txTrace(sock.m_txTrace),
      m_rxTrace(sock.m_rxTrace), m_pacingTimer(Timer::CANCEL_ON_DESTROY),
      m_ecnEchoSeq(sock.m_ecnEchoSeq), m_ecnCESeq(sock.m_ecnCESeq),
...
void TcpSocketBase::SetRtt(Ptr<RttEstimator> rtt) { m_rtt = rtt; }

/* Inherit from Socket class: Returns error code */
Socket::SocketErrno TcpSocketBase::GetErrno() const { return m_errno; }
...
Ptr<Node> TcpSocketBase::GetNode() const { return m_node; }
...
void TcpSocketBase::SetInitialSSThresh(uint32_t threshold) {
  NS_ABORT_MSG_UNLESS((m_state == CLOSED) ||
                          threshold == m_tcb->m_initialSsThresh,
...
uint32_t TcpSocketBase::GetInitialCwnd() const { return m_tcb->m_initialCWnd; }
...
int TcpSocketBase::Close() {
  ...
  if (m_tcb->m_rxBuffer->Size() != 0) {
    NS_LOG_WARN(
        "Socket "
        << this << " << unread rx data during close.  Sending reset."
...
int TcpSocketBase::Send(Ptr<Packet> p, uint32_t flags) {
  ...
  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT) {
    ...
    m_rateOps->CalculateAppLimited(
        m_tcb->m_cWnd, m_tcb->m_bytesInFlight, m_tcb->m_segmentSize,
    ...
    NS_LOG_LOGIC("txBufSize=" << m_txBuffer->Size() << " state "
                              << TcpStateName[m_state]);
    if ((m_state == ESTABLISHED || m_state == CLOSE_WAIT) &&
        AvailableWindow() > 0) { // Try to send the data out: Add a little step
...
int TcpSocketBase::SendTo(Ptr<Packet> p, uint32_t flags,
...
Ptr<Packet> TcpSocketBase::RecvFrom(uint32_t maxSize, uint32_t flags,
                                    Address &fromAddress) {
  ...
  if (packet && packet->GetSize() != 0) {
    if (m_endPoint != nullptr) {
      fromAddress = InetSocketAddress(m_endPoint->GetPeerAddress(),
                                      m_endPoint->GetPeerPort());
    } else if (m_endPoint6 != nullptr) {
      fromAddress = Inet6SocketAddress(m_endPoint6->GetPeerAddress(),
...
int TcpSocketBase::DoConnect() {
  ...
  if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT ||
      m_state == LAST_ACK ||
      m_state ==
          CLOSE_WAIT) { // send a SYN packet and change state into SYN_SENT
    ...
    m_tcb->m_ecnState =
        TcpSocketState::ECN_DISABLED; // because sender is not yet aware about
             ...
             TIME_WAIT) { // In states SYN_RCVD, ESTABLISHED, FIN_WAIT_1,
...
/* Tell if a sequence number range is out side the range that my rx buffer can
    accept */
bool TcpSocketBase::OutOfRange(SequenceNumber32 head,
  ...
  if (m_state == LAST_ACK || m_state == CLOSING ||
      m_state == CLOSE_WAIT) { // In LAST_ACK and CLOSING states, it only wait
                               // for an ACK and the
...
void TcpSocketBase::ForwardUp(Ptr<Packet> packet, Ipv4Header header,
                              uint16_t port,
  ...
  Address toAddress =
      InetSocketAddress(header.GetDestination(), m_endPoint->GetLocalPort());
...
void TcpSocketBase::ForwardUp6(Ptr<Packet> packet, Ipv6Header header,
                               uint16_t port,
...
void TcpSocketBase::DoForwardUp(Ptr<Packet> packet, const Address &fromAddress,
  ...
  if (m_rWnd.Get() == 0 &&
      m_persistEvent.IsExpired()) { // Zero window: Enter persist state to send
                                    // 1 byte to probe
    NS_LOG_LOGIC(this << " Enter zerowindow persist state");
    NS_LOG_LOGIC(
        this << " Cancelled ReTxTimeout event which was set to expire at "
             << (Simulator::Now() + Simulator::GetDelayLeft(m_retxEvent))
                    .GetSeconds());
    m_retxEvent.Cancel();
    NS_LOG_LOGIC("Schedule persist timeout at time "
                 << Simulator::Now().GetSeconds() << " to expire at time "
                 << (Simulator::Now() + m_persistTimeout).GetSeconds());
    m_persistEvent = Simulator::Schedule(m_persistTimeout,
                                         &TcpSocketBase::PersistTimeout, this);
  ...
  case CLOSED:
    ...
    if ((tcpHeader.GetFlags() & ~(TcpHeader::PSH | TcpHeader::URG)) !=
        TcpHeader::RST) { // Since m_endPoint is not configured yet, we cannot
                          // use SendRST here
      TcpHeader h;
  ...
  if (m_rWnd.Get() != 0 &&
      m_persistEvent.IsRunning()) { // persist probes end, the other end has
...
void TcpSocketBase::ProcessEstablished(Ptr<Packet> packet,
  ...
  if (tcpflags == TcpHeader::ACK) {
    if (tcpHeader.GetAckNumber() < m_txBuffer->HeadSequence()) {
      ...
      NS_LOG_WARN("Ignored ack of " << tcpHeader.GetAckNumber() << " SND.UNA = "
  ...
  } else { // Received RST or the TCP flags is invalid, in either case,
           // terminate this socket
    if (tcpflags !=
        TcpHeader::RST) { // this must be an invalid flag, send reset
...
void TcpSocketBase::ReadOptions(const TcpHeader &tcpHeader,
...
void TcpSocketBase::EnterCwr(uint32_t currentDelivered) {
  ...
  if (!m_congestionControl->HasCongControl()) {
    ...
    m_recoveryOps->EnterRecovery(m_tcb, m_dupAckCount, UnAckDataCount(),
                                 currentDelivered);
...
void TcpSocketBase::ReceivedAck(Ptr<Packet> packet,
                                const TcpHeader &tcpHeader) {
  NS_LOG_FUNCTION(this << tcpHeader);
  ...
  if ((ackNumber > oldHeadSequence) && (ackNumber < m_recover) &&
    ...
    for (uint32_t i = 0; i < segAcked; i++) {
      if (m_txBuffer->IsRetransmittedDataAcked(ackNumber -
                                               (i * m_tcb->m_segmentSize))) {
  ...
  auto currentDelivered = static_cast<uint32_t>(
  ...
  if (m_congestionControl->HasCongControl()) {
    ...
    auto rateSample = m_rateOps->GenerateSample(currentDelivered, lost, false,
                                                priorInFlight, m_tcb->m_minRtt);
...
void TcpSocketBase::ProcessAck(const SequenceNumber32 &ackNumber,
  ...
  if (ackNumber == oldHeadSequence && ackNumber == m_tcb->m_highTxMark) {
    ...
    // of RecoveryPoint.
      ...
      NS_LOG_DEBUG(" Cong Control Called, cWnd=" << m_tcb->m_cWnd << " ssTh="
                                                 << m_tcb->m_ssThresh);
      if (!m_sackEnabled) {
      ...
      // phase.
        ...
        segsAcked = static_cast<uint32_t>(ackNumber - oldHeadSequence) /
                    m_tcb->m_segmentSize;
        m_congestionControl->PktsAcked(m_tcb, segsAcked, m_tcb->m_lastRtt);
        m_congestionControl->CwndEvent(m_tcb,
                                       TcpSocketState::CA_EVENT_COMPLETE_CWR);
      ...
      if (m_tcb->m_congState == TcpSocketState::CA_OPEN) {
        ...
        NS_LOG_LOGIC("Congestion control called: "
                     << " cWnd: " << m_tcb->m_cWnd << " ssTh: "
                     << m_tcb->m_ssThresh << " segsAcked: " << segsAcked);
...
void TcpSocketBase::ProcessListen(Ptr<Packet> packet,
  ...
  Simulator::ScheduleNow(&TcpSocketBase::CompleteFork, newSock, packet,
                         tcpHeader, fromAddress, toAddress);
}

/* Received a packet upon SYN_SENT */
void TcpSocketBase::ProcessSynSent(Ptr<Packet> packet,
                                   const TcpHeader &tcpHeader) {
  NS_LOG_FUNCTION(this << tcpHeader);
  ...
  if (tcpflags == 0) { // Bare data, accept it and move to ESTABLISHED state.
    ...
    Simulator::ScheduleNow(&TcpSocketBase::ConnectionSucceeded, this);
             ...
             !(tcpflags & TcpHeader::ACK)) { // Received SYN, move to SYN_RCVD
    ...
    if (!(tcpflags & TcpHeader::RST)) { // When (1) rx of FIN+ACK; (2) rx of
...
void TcpSocketBase::ProcessSynRcvd(Ptr<Packet> packet,
  ...
  if (tcpflags == 0 ||
    ...
    if (tcpHeader.GetSequenceNumber() ==
        m_tcb->m_rxBuffer
            ->NextRxSequence()) { // In-sequence FIN before connection complete.
    ...
    if (tcpflags != TcpHeader::RST) { // When (1) rx of SYN+ACK; (2) rx of FIN;
                                      // (3) rx of bad flags
      NS_LOG_LOGIC("Illegal flag " << TcpHeader::FlagsToString(tcpflags)
                                   << " received. Reset packet is sent.");
      if (m_endPoint) {
...
void TcpSocketBase::ProcessWait(Ptr<Packet> packet,
                                const TcpHeader &tcpHeader) {
  NS_LOG_FUNCTION(this << tcpHeader);
  ...
  if (packet->GetSize() > 0 &&
      !(tcpflags & TcpHeader::ACK)) { // Bare data, accept it
    ReceivedData(packet, tcpHeader);
  } else if (tcpflags ==
             TcpHeader::ACK) { // Process the ACK, and if in FIN_WAIT_1,
    ...
    if (m_state == FIN_WAIT_1 && m_txBuffer->Size() == 0 &&
      ...
      m_state = FIN_WAIT_2;
             ...
             tcpflags == (TcpHeader::FIN |
                          TcpHeader::ACK)) { // Got FIN, respond with ACK and
                                             // move to next state
    ...
    m_tcb->m_rxBuffer->SetFinSequence(tcpHeader.GetSequenceNumber());
             ...
             tcpflags ==
                 (TcpHeader::SYN |
                  TcpHeader::ACK)) { // Duplicated SYN or SYN+ACK, possibly due
...
void TcpSocketBase::ProcessClosing(Ptr<Packet> packet,
                                   const TcpHeader &tcpHeader) {
  NS_LOG_FUNCTION(this << tcpHeader);
  ...
  if (tcpflags == TcpHeader::ACK) {
    if (tcpHeader.GetSequenceNumber() ==
        m_tcb->m_rxBuffer
            ->NextRxSequence()) { // This ACK corresponds to the FIN sent
      TimeWait();
    }
  } else { // CLOSING state means simultaneous close, i.e. no one is sending
           // data to
    // anyone. If anything other than ACK is received, respond with a reset.
    if (tcpflags == TcpHeader::FIN ||
        tcpflags ==
            (TcpHeader::FIN | TcpHeader::ACK)) { // FIN from the peer as well.
                                                 // We can close immediately.
      SendEmptyPacket(TcpHeader::ACK);
    } else if (tcpflags != TcpHeader::RST) { // Receive of SYN or SYN+ACK or bad
                                             // flags or pure data
      NS_LOG_LOGIC("Illegal flag " << TcpHeader::FlagsToString(tcpflags)
                                   << " received. Reset packet is sent.");
      SendRST();
...
void TcpSocketBase::ProcessLastAck(Ptr<Packet> packet,
                                   const TcpHeader &tcpHeader) {
  NS_LOG_FUNCTION(this << tcpHeader);
  ...
  if (tcpflags == 0) {
    ...
    if (tcpHeader.GetSequenceNumber() ==
        m_tcb->m_rxBuffer
            ->NextRxSequence()) { // This ACK corresponds to the FIN sent. This
                                  // socket closed peacefully.
      CloseAndNotify();
    }
  } else if (tcpflags == TcpHeader::FIN) { // Received FIN again, the peer
                                           // probably lost the FIN+ACK
    SendEmptyPacket(TcpHeader::FIN | TcpHeader::ACK);
  } else if (tcpflags == (TcpHeader::FIN | TcpHeader::ACK) ||
             tcpflags == TcpHeader::RST) {
    CloseAndNotify();
...
void TcpSocketBase::PeerClose(Ptr<Packet> p, const TcpHeader &tcpHeader) {
  ...
  NS_LOG_LOGIC("Accepted FIN at seq " << tcpHeader.GetSequenceNumber() +
                                             SequenceNumber32(p->GetSize()));
...
void TcpSocketBase::DoPeerClose() {
  ...
  if (m_shutdownSend) { // The application declares that it would not sent any
  ...
  if (m_state == LAST_ACK) {
    ...
    Time lastRto = m_rtt->GetEstimate() +
                   Max(m_clockGranularity, m_rtt->GetVariation() * 4);
    m_lastAckEvent =
        Simulator::Schedule(lastRto, &TcpSocketBase::LastAckTimeout, this);
  }
}
...
void TcpSocketBase::SendEmptyPacket(uint8_t flags) {
  ...
  if (flags & TcpHeader::FIN) {
    flags |= TcpHeader::ACK;
  } else if (m_state == FIN_WAIT_1 || m_state == LAST_ACK ||
             m_state == CLOSING) {
    ++s;
  }
  ...
  if (hasSyn) {
    if (m_winScalingEnabled) { // The window scaling option is set only on SYN
                               // packets
  ...
  if (flags &
    ...
    NS_LOG_INFO("Sending a pure ACK, acking seq "
                << m_tcb->m_rxBuffer->NextRxSequence());
  }
  ...
  if (m_retxEvent.IsExpired() && (hasSyn || hasFin) &&
    ...
    m_retxEvent = Simulator::Schedule(m_rto, &TcpSocketBase::SendEmptyPacket,
                                      this, flags);
  }
}
...
void TcpSocketBase::CompleteFork(Ptr<Packet> p [[maybe_unused]],
                                 const TcpHeader &h, const Address &fromAddress,
                                 const Address &toAddress) {
  NS_LOG_FUNCTION(this << p << h << fromAddress << toAddress);
  // Get port and address from peer (connecting host)
  if (InetSocketAddress::IsMatchingType(toAddress)) {
    m_endPoint =
        m_tcp->Allocate(GetBoundNetDevice(),
  ...
  m_tcb->m_rxBuffer->SetNextRxSequence(h.GetSequenceNumber() +
                                       SequenceNumber32(1));
...
void TcpSocketBase::AddSocketTags(const Ptr<Packet> &p) const {
  ...
  if (IsManualIpv6Tclass()) {
    ...
    if (m_tcb->m_ecnState != TcpSocketState::ECN_DISABLED &&
        !CheckNoEcn(GetIpv6Tclass())) {
      ipTclassTag.SetTclass(
          MarkEcnCodePoint(GetIpv6Tclass(), m_tcb->m_ectCodePoint));
    } else {
      // Set the last received ipTos
...
uint32_t TcpSocketBase::SendDataPacket(SequenceNumber32 seq, uint32_t maxSize,
                                       bool withAck) {
...
void TcpSocketBase::UpdateRttHistory(const SequenceNumber32 &seq, uint32_t sz,
  ...
  if (!isRetransmission) { // This is the next expected one, just log at end
    ...
    for (auto i = m_history.begin(); i != m_history.end(); ++i) {
      if ((seq >= i->seq) &&
          (seq < (i->seq + SequenceNumber32(i->count)))) { // Found it
        i->retx = true;
        i->count =
            ((seq + SequenceNumber32(sz)) - i->seq); // And update count in hist
        break;
      }
    }
  }
...
uint32_t TcpSocketBase::SendPendingData(bool withAck) {
  ...
  if (m_endPoint == nullptr && m_endPoint6 == nullptr) {
    NS_LOG_INFO("TcpSocketBase::SendPendingData: No endpoint; m_shutdownSend="
                << m_shutdownSend);
  ...
  while (availableWindow > 0) {
    if (IsPacingEnabled()) {
      ...
      if (m_pacingTimer.IsRunning()) {
        NS_LOG_INFO("Skipping Packet due to pacing"
                    << m_pacingTimer.GetDelayLeft());
        break;
      }
    ...
    bool enableRule3 =
        m_sackEnabled && m_tcb->m_congState == TcpSocketState::CA_RECOVERY;
    if (!m_txBuffer->NextSeg(&next, &nextHigh, enableRule3)) {
      ...
      if (availableWindow < m_tcb->m_segmentSize &&
          availableData > availableWindow) {
      ...
      if (!m_noDelay && UnAckDataCount() > 0 &&
          availableData < m_tcb->m_segmentSize) {
      ...
      NS_LOG_LOGIC(" rxwin "
                   << m_rWnd << " segsize " << m_tcb->m_segmentSize
                   << " highestRxAck " << m_txBuffer->HeadSequence()
                   << " pd->Size " << m_txBuffer->Size() << " pd->SFS "
                   << m_txBuffer->SizeFromSequence(m_tcb->m_nextTxSequence));

      NS_LOG_DEBUG("cWnd: " << m_tcb->m_cWnd << " total unAck: "
                            << UnAckDataCount() << " sent seq "
                            << m_tcb->m_nextTxSequence << " size " << sz);
...
uint16_t TcpSocketBase::AdvertisedWindowSize(bool scale) const {
  ...
  if (m_tcb->m_rxBuffer->GotFin()) {
    ...
    NS_ASSERT_MSG(m_tcb->m_rxBuffer->MaxRxSequence() -
                          m_tcb->m_rxBuffer->NextRxSequence() >=
                      0,
...
void TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader &tcpHeader) {
  ...
  if (!m_tcb->m_rxBuffer->Add(
          p, tcpHeader)) { // Insert failed: No data or RX buffer full
    if (m_tcb->m_ecnState == TcpSocketState::ECN_CE_RCVD ||
        m_tcb->m_ecnState == TcpSocketState::ECN_SENDING_ECE) {
      SendEmptyPacket(TcpHeader::ACK | TcpHeader::ECE);
      NS_LOG_DEBUG(TcpSocketState::EcnStateName[m_tcb->m_ecnState]
  ...
  if (expectedSeq <
      m_tcb->m_rxBuffer->NextRxSequence()) { // NextRxSeq advanced, we have
    ...
    if (m_tcb->m_rxBuffer->Finished() &&
        (tcpHeader.GetFlags() & TcpHeader::FIN) == 0) {
      DoPeerClose();
      return;
    }
  }
  ...
  if (m_tcb->m_rxBuffer->Size() > m_tcb->m_rxBuffer->Available() ||
      m_tcb->m_rxBuffer->NextRxSequence() >
          expectedSeq + p->GetSize()) { // A gap exists in the buffer, or we
...
void TcpSocketBase::EstimateRtt(const TcpHeader &tcpHeader) {
  ...
  if (!m_history.empty()) {
    ...
    if (!h.retx && ackSeq >= (h.seq + SequenceNumber32(
                                          h.count))) { // Ok to use this sample
...
void TcpSocketBase::ReTxTimeout() {
  ...
  if (m_txBuffer->Size() == 0) {
    if (m_state == FIN_WAIT_1 ||
        m_state == CLOSING) { // Must have lost FIN, re-send
      SendEmptyPacket(TcpHeader::FIN);
    }
    return;
  }
  ...
  if (m_state <= ESTABLISHED &&
      m_txBuffer->HeadSequence() >= m_tcb->m_highTxMark &&
      m_txBuffer->Size() == 0) {
  ...
  if (m_tcb->m_congState != TcpSocketState::CA_LOSS ||
      !m_txBuffer->IsHeadRetransmitted()) {
    m_tcb->m_ssThresh =
        m_congestionControl->GetSsThresh(m_tcb, inFlightBeforeRto);
  }
  ...
  NS_LOG_DEBUG("RTO. Reset cwnd to "
               << m_tcb->m_cWnd << ", ssthresh to " << m_tcb->m_ssThresh
...
void TcpSocketBase::PersistTimeout() {
  ...
  m_persistTimeout = std::min(
      Seconds(60), Time(2 * m_persistTimeout)); // max persist timeout = 60s
  Ptr<Packet> p =
      m_txBuffer->CopyFromSequence(1, m_tcb->m_nextTxSequence)->GetPacketCopy();
...
uint32_t TcpSocketBase::GetSegSize() const { return m_tcb->m_segmentSize; }
...
Time TcpSocketBase::GetConnTimeout() const { return m_cnTimeout; }
...
uint32_t TcpSocketBase::GetSynRetries() const { return m_synRetries; }
...
Time TcpSocketBase::GetDelAckTimeout() const { return m_delAckTimeout; }
...
uint32_t TcpSocketBase::GetDelAckMaxCount() const { return m_delAckMaxCount; }
...
bool TcpSocketBase::GetTcpNoDelay() const { return m_noDelay; }
...
Time TcpSocketBase::GetPersistTimeout() const { return m_persistTimeout; }
...
bool TcpSocketBase::GetAllowBroadcast() const { return false; }
...
void TcpSocketBase::ProcessOptionWScale(const Ptr<const TcpOption> option) {
  ...
  Ptr<const TcpOptionWinScale> ws =
      DynamicCast<const TcpOptionWinScale>(option);
...
void TcpSocketBase::ProcessOptionSackPermitted(
    const Ptr<const TcpOption> option) {
  NS_LOG_FUNCTION(this << option);

  Ptr<const TcpOptionSackPermitted> s =
      DynamicCast<const TcpOptionSackPermitted>(option);
...
void TcpSocketBase::AddOptionSack(TcpHeader &header) {
  ...
  uint8_t optionLenAvail =
      header.GetMaxOptionLength() - header.GetOptionLength();
  ...
  for (auto i = sackList.begin(); allowedSackBlocks > 0 && i != sackList.end();
       ++i) {
...
Time TcpSocketBase::GetMinRto() const { return m_minRto; }
...
Time TcpSocketBase::GetClockGranularity() const { return m_clockGranularity; }

Ptr<TcpTxBuffer> TcpSocketBase::GetTxBuffer() const { return m_txBuffer; }
...
void TcpSocketBase::UpdatePacingRateTrace(DataRate oldValue,
...
void TcpSocketBase::UpdateCongState(
...
void TcpSocketBase::UpdateBytesInFlight(uint32_t oldValue,
...
void TcpSocketBase::UpdatePacingRate() {
  ...
  if (m_tcb->m_cWnd < m_tcb->m_ssThresh / 2) {
    NS_LOG_DEBUG("Pacing according to slow start factor; "
                 << m_tcb->m_cWnd << " " << m_tcb->m_ssThresh);
    factor = static_cast<double>(m_tcb->m_pacingSsRatio) / 100;
  } else {
    NS_LOG_DEBUG("Pacing according to congestion avoidance factor; "
                 << m_tcb->m_cWnd << " " << m_tcb->m_ssThresh);
  ...
  DataRate pacingRate(
      (std::max(m_tcb->m_cWnd, m_tcb->m_bytesInFlight) * 8 * factor) /
      lastRtt.GetSeconds());
  if (pacingRate < m_tcb->m_maxPacingRate) {
    ...
    NS_LOG_DEBUG(
        "Pacing capped by max pacing rate: " << m_tcb->m_maxPacingRate);
    m_tcb->m_pacingRate = m_tcb->m_maxPacingRate;
  }
}
...
uint32_t TcpSocketBase::GetRWnd() const { return m_rWnd.Get(); }
