  ...
  static TypeId tid =
      TypeId("ns3::Icmpv6L4Protocol")
          ...
          .AddAttribute("RsInitialRetransmissionTime",
                        ...
                        TimeValue(Seconds(4)),
                        MakeTimeAccessor(
                            &Icmpv6L4Protocol::m_rsInitialRetransmissionTime),
                        MakeTimeChecker())
          .AddAttribute(
          ...
          .AddAttribute(
              ...
              MakeUintegerAccessor(
                  &Icmpv6L4Protocol::m_rsMaxRetransmissionCount),
          ...
          .AddAttribute(
              ...
              TimeValue(Seconds(0)),
              MakeTimeAccessor(
                  &Icmpv6L4Protocol::m_rsMaxRetransmissionDuration),
              MakeTimeChecker());
  return tid;
}
...
Icmpv6L4Protocol::~Icmpv6L4Protocol() { NS_LOG_FUNCTION(this); }
...
IpL4Protocol::RxStatus Icmpv6L4Protocol::Receive(Ptr<Packet> packet,
...
IpL4Protocol::RxStatus Icmpv6L4Protocol::Receive(Ptr<Packet> packet,
  ...
  NS_LOG_FUNCTION(this << packet << header.GetSource()
  ...
  case Icmpv6Header::ICMPV6_ND_REDIRECTION:
    HandleRedirection(p, header.GetSource(), header.GetDestination(),
                      interface);
    break;
  case Icmpv6Header::ICMPV6_ECHO_REQUEST:
    HandleEchoRequest(p, header.GetSource(), header.GetDestination(),
                      interface);
    break;
  ...
  case Icmpv6Header::ICMPV6_ERROR_PACKET_TOO_BIG:
    HandlePacketTooBig(p, header.GetSource(), header.GetDestination(),
                       interface);
    break;
  case Icmpv6Header::ICMPV6_ERROR_TIME_EXCEEDED:
    HandleTimeExceeded(p, header.GetSource(), header.GetDestination(),
                       interface);
    break;
  case Icmpv6Header::ICMPV6_ERROR_PARAMETER_ERROR:
    HandleParameterError(p, header.GetSource(), header.GetDestination(),
                         interface);
    break;
  default:
...
void Icmpv6L4Protocol::Forward(Ipv6Address source, Icmpv6Header icmp,
  ...
  if (nextHeader != Icmpv6L4Protocol::PROT_NUMBER) {
    ...
    if (l4) {
      l4->ReceiveIcmp(source, ipHeader.GetHopLimit(), icmp.GetType(),
                      icmp.GetCode(), info, ipHeader.GetSource(),
                      ipHeader.GetDestination(), payload);
    }
  }
}

void Icmpv6L4Protocol::HandleEchoRequest(Ptr<Packet> packet,
...
void Icmpv6L4Protocol::HandleRA(Ptr<Packet> packet, const Ipv6Address &src,
                                const Ipv6Address &dst,
                                Ptr<Ipv6Interface> interface) {
  NS_LOG_FUNCTION(this << packet << src << dst << interface);
...
void Icmpv6L4Protocol::ReceiveLLA(Icmpv6OptionLinkLayerAddress lla,
                                  const Ipv6Address &src,
                                  const Ipv6Address &dst,
                                  Ptr<Ipv6Interface> interface) {
...
void Icmpv6L4Protocol::HandleRS(Ptr<Packet> packet, const Ipv6Address &src,
                                const Ipv6Address &dst,
                                Ptr<Ipv6Interface> interface) {
  NS_LOG_FUNCTION(this << packet << src << dst << interface);
  Ptr<Ipv6L3Protocol> ipv6 = m_node->GetObject<Ipv6L3Protocol>();
...
void Icmpv6L4Protocol::HandleNS(Ptr<Packet> packet, const Ipv6Address &src,
                                const Ipv6Address &dst,
                                Ptr<Ipv6Interface> interface) {
  NS_LOG_FUNCTION(this << packet << src << dst << interface);
  Icmpv6NS nsHeader("::");
...
Icmpv6L4Protocol::ForgeRS(Ipv6Address src, Ipv6Address dst,
                          Address hardwareAddress) {
  ...
  rs.CalculatePseudoHeaderChecksum(
      src, dst, p->GetSize() + rs.GetSerializedSize(), PROT_NUMBER);
  p->AddHeader(rs);
...
NdiscCache::Ipv6PayloadHeaderPair
Icmpv6L4Protocol::ForgeEchoRequest(Ipv6Address src, Ipv6Address dst,
  ...
  req.CalculatePseudoHeaderChecksum(
      src, dst, p->GetSize() + req.GetSerializedSize(), PROT_NUMBER);
  p->AddHeader(req);
...
void Icmpv6L4Protocol::HandleNA(Ptr<Packet> packet, const Ipv6Address &src,
                                const Ipv6Address &dst,
                                Ptr<Ipv6Interface> interface) {
  NS_LOG_FUNCTION(this << packet << src << dst << interface);
  Icmpv6NA naHeader;
...
void Icmpv6L4Protocol::HandleRedirection(Ptr<Packet> packet,
  ...
  if (redirTarget == redirDestination) {
    ipv6->GetRoutingProtocol()->NotifyAddRoute(
        redirDestination, Ipv6Prefix(128), Ipv6Address("::"),
        ipv6->GetInterfaceForAddress(dst));
  } else {
    uint32_t ifIndex = ipv6->GetInterfaceForAddress(dst);
    ipv6->GetRoutingProtocol()->NotifyAddRoute(
        redirDestination, Ipv6Prefix(128), redirTarget, ifIndex);
  }
}

void Icmpv6L4Protocol::HandleDestinationUnreachable(
    Ptr<Packet> p, const Ipv6Address &src, const Ipv6Address &dst,
    Ptr<Ipv6Interface> interface) {
...
void Icmpv6L4Protocol::HandleTimeExceeded(Ptr<Packet> p, const Ipv6Address &src,
                                          const Ipv6Address &dst,
                                          Ptr<Ipv6Interface> interface) {
  NS_LOG_FUNCTION(this << *p << src << dst << interface);
  Ptr<Packet> pkt = p->Copy();
...
void Icmpv6L4Protocol::HandlePacketTooBig(Ptr<Packet> p, const Ipv6Address &src,
                                          const Ipv6Address &dst,
                                          Ptr<Ipv6Interface> interface) {
  NS_LOG_FUNCTION(this << *p << src << dst << interface);
  Ptr<Packet> pkt = p->Copy();
...
void Icmpv6L4Protocol::HandleParameterError(Ptr<Packet> p,
                                            const Ipv6Address &src,
                                            const Ipv6Address &dst,
                                            Ptr<Ipv6Interface> interface) {
  NS_LOG_FUNCTION(this << *p << src << dst << interface);
...
void Icmpv6L4Protocol::SendMessage(Ptr<Packet> packet, Ipv6Address src,
                                   Ipv6Address dst, uint8_t ttl) {
...
void Icmpv6L4Protocol::DelayedSendMessage(Ptr<Packet> packet, Ipv6Address src,
                                          Ipv6Address dst, uint8_t ttl) {
  NS_LOG_FUNCTION(this << packet << src << dst << (uint32_t)ttl);
  SendMessage(packet, src, dst, ttl);
}

void Icmpv6L4Protocol::SendMessage(Ptr<Packet> packet, Ipv6Address dst,
                                   Icmpv6Header &icmpv6Hdr, uint8_t ttl) {
  ...
  if (route) {
    ...
    icmpv6Hdr.CalculatePseudoHeaderChecksum(
        src, dst, packet->GetSize() + icmpv6Hdr.GetSerializedSize(),
        PROT_NUMBER);
...
void Icmpv6L4Protocol::SendNA(Ipv6Address src, Ipv6Address dst,
  ...
  p->AddHeader(llOption);
  na.CalculatePseudoHeaderChecksum(
      src, dst, p->GetSize() + na.GetSerializedSize(), PROT_NUMBER);
  p->AddHeader(na);

  SendMessage(p, src, dst, 255);
}

void Icmpv6L4Protocol::SendEchoReply(Ipv6Address src, Ipv6Address dst,
                                     uint16_t id, uint16_t seq,
                                     Ptr<Packet> data) {
  NS_LOG_FUNCTION(this << src << dst << id << seq << data);
  Ptr<Packet> p = data->Copy();
  ...
  reply.CalculatePseudoHeaderChecksum(
      src, dst, p->GetSize() + reply.GetSerializedSize(), PROT_NUMBER);
  p->AddHeader(reply);
  SendMessage(p, src, dst, 64);
}

void Icmpv6L4Protocol::SendNS(Ipv6Address src, Ipv6Address dst,
  ...
  p->AddHeader(llOption);
  ns.CalculatePseudoHeaderChecksum(
      src, dst, p->GetSize() + ns.GetSerializedSize(), PROT_NUMBER);
  p->AddHeader(ns);
  if (!dst.IsMulticast()) {
    SendMessage(p, src, dst, 255);
  } else {
    ...
    Simulator::Schedule(Time(MilliSeconds(m_solicitationJitter->GetValue())),
                        &Icmpv6L4Protocol::DelayedSendMessage, this, p, src,
                        dst, 255);
  }
}

void Icmpv6L4Protocol::SendRS(Ipv6Address src, Ipv6Address dst,
                              Address hardwareAddress) {
  NS_LOG_FUNCTION(this << src << dst << hardwareAddress);
  Ptr<Packet> p = Create<Packet>();
  Icmpv6RS rs;
  ...
  if (!dst.IsMulticast()) {
    ...
    if (m_rsRetransmissionCount == 0) {
      ...
      if (rsTimeout > m_rsMaxRetransmissionTime) {
        rsTimeout = m_rsMaxRetransmissionTime *
                    (1 + m_rsRetransmissionJitter->GetValue());
      }
    }
    m_rsPrevRetransmissionTimeout = rsTimeout;
    Simulator::Schedule(rsDelay, &Icmpv6L4Protocol::DelayedSendMessage, this, p,
                        src, dst, 255);
    m_handleRsTimeoutEvent = Simulator::Schedule(
        rsDelay + m_rsPrevRetransmissionTimeout,
        &Icmpv6L4Protocol::HandleRsTimeout, this, src, dst, hardwareAddress);
  }
}

void Icmpv6L4Protocol::HandleRsTimeout(Ipv6Address src, Ipv6Address dst,
                                       Address hardwareAddress) {
  NS_LOG_FUNCTION(this << src << dst << hardwareAddress);
  ...
  if (m_rsMaxRetransmissionDuration != Time(0) &&
      Simulator::Now() - m_rsInitialRetransmissionTime >
...
void Icmpv6L4Protocol::SendErrorDestinationUnreachable(
    Ptr<Packet> malformedPacket, Ipv6Address dst, uint8_t code) {
...
void Icmpv6L4Protocol::SendErrorTooBig(Ptr<Packet> malformedPacket,
                                       Ipv6Address dst, uint32_t mtu) {
...
void Icmpv6L4Protocol::SendErrorTimeExceeded(Ptr<Packet> malformedPacket,
                                             Ipv6Address dst, uint8_t code) {
  NS_LOG_FUNCTION(this << malformedPacket << dst
...
void Icmpv6L4Protocol::SendErrorParameterError(Ptr<Packet> malformedPacket,
                                               Ipv6Address dst, uint8_t code,
                                               uint32_t ptr) {
...
void Icmpv6L4Protocol::SendRedirection(Ptr<Packet> redirectedPacket,
                                       Ipv6Address src, Ipv6Address dst,
                                       Ipv6Address redirTarget,
                                       Ipv6Address redirDestination,
                                       Address redirHardwareTarget) {
  NS_LOG_FUNCTION(this << redirectedPacket << dst << redirTarget
                       << redirDestination << redirHardwareTarget);
  uint32_t llaSize = 0;
  Ptr<Packet> p = Create<Packet>();
  uint32_t redirectedPacketSize = redirectedPacket->GetSize();
  Icmpv6OptionLinkLayerAddress llOption(false);

  NS_LOG_LOGIC("Send Redirection ( to " << dst << " target " << redirTarget
  ...
  if (redirectedPacketSize <= (1280 - 56 - llaSize)) {
    ...
    Ptr<Packet> fragment =
        redirectedPacket->CreateFragment(0, 1280 - 56 - llaSize);
  ...
  redirectionHeader.CalculatePseudoHeaderChecksum(
      src, dst, p->GetSize() + redirectionHeader.GetSerializedSize(),
      PROT_NUMBER);
...
NdiscCache::Ipv6PayloadHeaderPair
Icmpv6L4Protocol::ForgeNA(Ipv6Address src, Ipv6Address dst,
                          Address *hardwareAddress, uint8_t flags) {
...
Ptr<NdiscCache> Icmpv6L4Protocol::CreateCache(Ptr<NetDevice> device,
                                              Ptr<Ipv6Interface> interface) {
...
bool Icmpv6L4Protocol::Lookup(Ipv6Address dst, Ptr<NetDevice> device,
...
bool Icmpv6L4Protocol::Lookup(Ptr<Packet> p, const Ipv6Header &ipHeader,
                              Ipv6Address dst, Ptr<NetDevice> device,
                              Ptr<NdiscCache> cache,
                              Address *hardwareDestination) {
  NS_LOG_FUNCTION(this << p << ipHeader << dst << device << cache
                       << hardwareDestination);
...
void Icmpv6L4Protocol::FunctionDadTimeout(Ipv6Interface *interface,
                                          Ipv6Address addr) {
...
void Icmpv6L4Protocol::SetDownTarget(
    IpL4Protocol::DownTargetCallback callback) {
  NS_LOG_FUNCTION(this << &callback);
}

void Icmpv6L4Protocol::SetDownTarget6(
    IpL4Protocol::DownTargetCallback6 callback) {
  NS_LOG_FUNCTION(this << &callback);
  m_downTarget = callback;
}

IpL4Protocol::DownTargetCallback Icmpv6L4Protocol::GetDownTarget() const {
  NS_LOG_FUNCTION(this);
  return IpL4Protocol::DownTargetCallback();
}

IpL4Protocol::DownTargetCallback6 Icmpv6L4Protocol::GetDownTarget6() const {
  NS_LOG_FUNCTION(this);
  return m_downTarget;
}

uint8_t Icmpv6L4Protocol::GetMaxMulticastSolicit() const {
  return m_maxMulticastSolicit;
}

uint8_t Icmpv6L4Protocol::GetMaxUnicastSolicit() const {
  return m_maxUnicastSolicit;
}

Time Icmpv6L4Protocol::GetReachableTime() const { return m_reachableTime; }

Time Icmpv6L4Protocol::GetRetransmissionTime() const {
  return m_retransmissionTime;
}

Time Icmpv6L4Protocol::GetDelayFirstProbe() const { return m_delayFirstProbe; }
