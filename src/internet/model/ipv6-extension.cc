...
TypeId Ipv6Extension::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ipv6Extension")
          ...
          .AddAttribute(
              "ExtensionNumber", "The IPv6 extension number.", UintegerValue(0),
...
Ptr<Node> Ipv6Extension::GetNode() const { return m_node; }
...
uint8_t Ipv6Extension::ProcessOptions(Ptr<Packet> &packet, uint8_t offset,
  ...
  Ptr<Icmpv6L4Protocol> icmpv6 =
      GetNode()->GetObject<Ipv6L3Protocol>()->GetIcmpv6();

  Ptr<Packet> p = packet->Copy();
  p->RemoveAtStart(offset);

  Ptr<Ipv6OptionDemux> ipv6OptionDemux =
      GetNode()->GetObject<Ipv6OptionDemux>();
    ...
    if (!ipv6Option) {
      ...
      case 3:
        ...
        if (!ipv6Header.GetDestination().IsMulticast()) {
          icmpv6->SendErrorParameterError(
              malformedPacket, ipv6Header.GetSource(),
              Icmpv6Header::ICMPV6_UNKNOWN_OPTION, offset + processedSize);
        }
        optionLength = 0;
        isDropped = true;
        stopProcessing = true;
      ...
      optionLength = ipv6Option->Process(packet, offset + processedSize,
                                         ipv6Header, isDropped);
    }
...
int64_t Ipv6Extension::AssignStreams(int64_t stream) {
...
uint8_t Ipv6ExtensionHopByHop::GetExtensionNumber() const { return EXT_NUMBER; }

uint8_t Ipv6ExtensionHopByHop::Process(Ptr<Packet> &packet, uint8_t offset,
                                       const Ipv6Header &ipv6Header,
                                       Ipv6Address dst, uint8_t *nextHeader,
                                       bool &stopProcessing, bool &isDropped,
                                       Ipv6L3Protocol::DropReason &dropReason) {
  ...
  offset += processedSize;
  uint8_t length =
      hopbyhopHeader.GetLength() - hopbyhopHeader.GetOptionsOffset();

  processedSize +=
      ProcessOptions(packet, offset, length, ipv6Header, dst, nextHeader,
...
uint8_t Ipv6ExtensionDestination::Process(
    Ptr<Packet> &packet, uint8_t offset, const Ipv6Header &ipv6Header,
  ...
  offset += processedSize;
  uint8_t length =
      destinationHeader.GetLength() - destinationHeader.GetOptionsOffset();
  ...
  static TypeId tid =
      TypeId("ns3::Ipv6ExtensionFragment")
          ...
          .AddAttribute(
              ...
              MakeTimeAccessor(
                  &Ipv6ExtensionFragment::m_fragmentExpirationTimeout),
              MakeTimeChecker());
  return tid;
}
...
uint8_t Ipv6ExtensionFragment::GetExtensionNumber() const { return EXT_NUMBER; }
...
uint8_t Ipv6ExtensionFragment::Process(Ptr<Packet> &packet, uint8_t offset,
  ...
  if (it == m_fragments.end()) {
    ...
    NS_LOG_DEBUG("Insert new fragment key: src: "
                 << src << " IP hdr id " << identification
                 << " m_fragments.size() " << m_fragments.size() << " offset "
                 << fragmentOffset);
  ...
  NS_LOG_DEBUG("Add fragment with IP hdr id " << identification << " offset "
                                              << fragmentOffset);
  ...
  if (fragments->IsEntire()) {
    ...
    NS_LOG_DEBUG("Finished fragment with IP hdr id "
                 << fragmentKey.second << " erase timeout, m_fragments.size(): "
                 << m_fragments.size());
...
void Ipv6ExtensionFragment::GetFragments(
    Ptr<Packet> packet, Ipv6Header ipv6Header, uint32_t maxFragmentSize,
  ...
  Ptr<Ipv6ExtensionDemux> extensionDemux =
      GetNode()->GetObject<Ipv6ExtensionDemux>();
  ...
  while (moreHeader) {
    if (nextHeader == Ipv6Header::IPV6_EXT_HOP_BY_HOP) {
      ...
      unfragmentablePart.emplace_back(hopbyhopHeader,
      ...
      unfragmentablePart.emplace_back(routingHeader,
      ...
      unfragmentablePart.emplace_back(destinationHeader,
                                      Ipv6Header::IPV6_EXT_DESTINATION);
      unfragmentablePartSize += extensionHeaderLength;
    }
  }
  ...
  uint32_t maxFragmentablePartSize = maxFragmentSize - ipv6HeaderSize -
                                     unfragmentablePartSize -
                                     fragmentHeaderSize;
  ...
  do {
    ...
    Ptr<Packet> fragment =
        p->CreateFragment(offset, currentFragmentablePartSize);
    offset += currentFragmentablePartSize;

    fragment->AddHeader(fragmentHeader);

    for (auto it = unfragmentablePart.begin(); it != unfragmentablePart.end();
         it++) {
...
void Ipv6ExtensionFragment::HandleFragmentsTimeout(FragmentKey_t fragmentKey,
  ...
  if (packet && packet->GetSize() > 8) {
    ...
    icmp->SendErrorTimeExceeded(p, ipHeader.GetSource(),
                                Icmpv6Header::ICMPV6_FRAGTIME);
  }
...
Ipv6ExtensionFragment::SetTimeout(FragmentKey_t key, Ipv6Header ipHeader) {
  ...
  if (m_timeoutEventList.empty()) {
    NS_LOG_DEBUG(
        "Scheduling timeout for IP hdr id "
        << key.second << " at time "
        << (Simulator::Now() + m_fragmentExpirationTimeout).GetSeconds());
    m_timeoutEvent =
        Simulator::Schedule(m_fragmentExpirationTimeout,
  ...
  m_timeoutEventList.emplace_back(
      Simulator::Now() + m_fragmentExpirationTimeout, key, ipHeader);
...
void Ipv6ExtensionFragment::HandleTimeout() {
  ...
  NS_LOG_DEBUG("Scheduling later HandleTimeout at "
               << (now + difference).GetSeconds());
  m_timeoutEvent = Simulator::Schedule(
      difference, &Ipv6ExtensionFragment::HandleTimeout, this);
}

Ipv6ExtensionFragment::Fragments::Fragments() : m_moreFragment(false) {}

Ipv6ExtensionFragment::Fragments::~Fragments() {}

void Ipv6ExtensionFragment::Fragments::AddFragment(Ptr<Packet> fragment,
  ...
  m_packetFragments.insert(
      it, std::pair<Ptr<Packet>, uint16_t>(fragment, fragmentOffset));
}

void Ipv6ExtensionFragment::Fragments::SetUnfragmentablePart(
    Ptr<Packet> unfragmentablePart) {
...
bool Ipv6ExtensionFragment::Fragments::IsEntire() const {
  ...
  if (ret) {
    ...
    for (auto it = m_packetFragments.begin(); it != m_packetFragments.end();
         it++) {
...
void Ipv6ExtensionFragment::Fragments::SetTimeoutIter(
...
uint8_t Ipv6ExtensionRouting::GetExtensionNumber() const { return EXT_NUMBER; }

uint8_t Ipv6ExtensionRouting::GetTypeRouting() const { return 0; }
...
uint8_t Ipv6ExtensionRouting::Process(Ptr<Packet> &packet, uint8_t offset,
                                      const Ipv6Header &ipv6Header,
                                      Ipv6Address dst, uint8_t *nextHeader,
                                      bool &stopProcessing, bool &isDropped,
                                      Ipv6L3Protocol::DropReason &dropReason) {
  ...
  return ipv6ExtensionRouting->Process(packet, offset, ipv6Header, dst,
                                       (uint8_t *)nullptr, stopProcessing,
      ...
      TypeId("ns3::Ipv6ExtensionRoutingDemux")
          ...
          .AddAttribute(
              ...
              MakeObjectVectorAccessor(
                  &Ipv6ExtensionRoutingDemux::m_extensionsRouting),
...
void Ipv6ExtensionRoutingDemux::DoDispose() {
  NS_LOG_FUNCTION(this);
  for (auto it = m_extensionsRouting.begin(); it != m_extensionsRouting.end();
       it++) {
    (*it)->Dispose();
    *it = nullptr;
  }
...
void Ipv6ExtensionRoutingDemux::Insert(
...
void Ipv6ExtensionRoutingDemux::Remove(
    Ptr<Ipv6ExtensionRouting> extensionRouting) {
  NS_LOG_FUNCTION(this << extensionRouting);
...
uint8_t Ipv6ExtensionLooseRouting::Process(
    Ptr<Packet> &packet, uint8_t offset, const Ipv6Header &ipv6Header,
    Ipv6Address dst, uint8_t *nextHeader, bool &stopProcessing, bool &isDropped,
    Ipv6L3Protocol::DropReason &dropReason) {
  NS_LOG_FUNCTION(this << packet << offset << ipv6Header << dst << nextHeader
  ...
  if (hopLimit <= 1) {
    ...
    icmpv6->SendErrorTimeExceeded(malformedPacket, srcAddress,
...
uint8_t Ipv6ExtensionESP::GetExtensionNumber() const { return EXT_NUMBER; }

uint8_t Ipv6ExtensionESP::Process(Ptr<Packet> &packet, uint8_t offset,
...
uint8_t Ipv6ExtensionAH::GetExtensionNumber() const { return EXT_NUMBER; }

uint8_t Ipv6ExtensionAH::Process(Ptr<Packet> &packet, uint8_t offset,
                                 const Ipv6Header &ipv6Header, Ipv6Address dst,
                                 uint8_t *nextHeader, bool &stopProcessing,
                                 bool &isDropped,
                                 Ipv6L3Protocol::DropReason &dropReason) {
