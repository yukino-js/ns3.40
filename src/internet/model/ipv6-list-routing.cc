...
TypeId Ipv6ListRouting::GetTypeId() {
...
Ipv6ListRouting::Ipv6ListRouting() : m_ipv6(nullptr) { NS_LOG_FUNCTION(this); }

Ipv6ListRouting::~Ipv6ListRouting() { NS_LOG_FUNCTION(this); }

void Ipv6ListRouting::DoDispose() {
...
Ptr<Ipv6Route> Ipv6ListRouting::RouteOutput(Ptr<Packet> p,
  ...
  for (auto i = m_routingProtocols.begin(); i != m_routingProtocols.end();
       i++) {
    NS_LOG_LOGIC("Checking protocol " << (*i).second->GetInstanceTypeId()
                                      << " with priority " << (*i).first);
    NS_LOG_LOGIC("Requesting source address for destination "
...
bool Ipv6ListRouting::RouteInput(Ptr<const Packet> p, const Ipv6Header &header,
  ...
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
    if ((*rprotoIter)
            .second->RouteInput(p, header, idev, ucb, mcb, lcb, nullEcb)) {
      return true;
    }
  }
...
void Ipv6ListRouting::NotifyInterfaceUp(uint32_t interface) {
...
void Ipv6ListRouting::NotifyInterfaceDown(uint32_t interface) {
  NS_LOG_FUNCTION(this << interface);
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
...
void Ipv6ListRouting::NotifyAddAddress(uint32_t interface,
...
void Ipv6ListRouting::NotifyRemoveAddress(uint32_t interface,
                                          Ipv6InterfaceAddress address) {
  NS_LOG_FUNCTION(this << interface << address);
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
...
void Ipv6ListRouting::NotifyAddRoute(Ipv6Address dst, Ipv6Prefix mask,
  ...
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
    (*rprotoIter)
        .second->NotifyAddRoute(dst, mask, nextHop, interface, prefixToUse);
  }
}

void Ipv6ListRouting::NotifyRemoveRoute(Ipv6Address dst, Ipv6Prefix mask,
                                        Ipv6Address nextHop, uint32_t interface,
                                        Ipv6Address prefixToUse) {
  NS_LOG_FUNCTION(this << dst << mask << nextHop << interface);
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
    (*rprotoIter)
        .second->NotifyRemoveRoute(dst, mask, nextHop, interface, prefixToUse);
  }
}

void Ipv6ListRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream,
                                        Time::Unit unit) const {
  NS_LOG_FUNCTION(this);

  *stream->GetStream() << "Node: " << m_ipv6->GetObject<Node>()->GetId()
                       << ", Time: " << Now().As(unit) << ", Local time: "
                       << m_ipv6->GetObject<Node>()->GetLocalTime().As(unit)
                       << ", Ipv6ListRouting table" << std::endl;
  for (auto i = m_routingProtocols.begin(); i != m_routingProtocols.end();
       i++) {
    *stream->GetStream() << "  Priority: " << (*i).first
                         << " Protocol: " << (*i).second->GetInstanceTypeId()
                         << std::endl;
    (*i).second->PrintRoutingTable(stream, unit);
  }
}

void Ipv6ListRouting::SetIpv6(Ptr<Ipv6> ipv6) {
...
void Ipv6ListRouting::AddRoutingProtocol(
    Ptr<Ipv6RoutingProtocol> routingProtocol, int16_t priority) {
...
uint32_t Ipv6ListRouting::GetNRoutingProtocols() const {
  NS_LOG_FUNCTION(this);
...
Ipv6ListRouting::GetRoutingProtocol(uint32_t index, int16_t &priority) const {
  ...
  if (index >= m_routingProtocols.size()) {
    NS_FATAL_ERROR("Ipv6ListRouting::GetRoutingProtocol ():  index "
                   << index << " out of range");
  }
  uint32_t i = 0;
  for (auto rprotoIter = m_routingProtocols.begin();
...
bool Ipv6ListRouting::Compare(const Ipv6RoutingProtocolEntry &a,
