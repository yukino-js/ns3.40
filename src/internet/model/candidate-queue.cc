...
std::ostream &operator<<(std::ostream &os, const CandidateQueue &q) {
  ...
  for (auto iter = list.begin(); iter != list.end(); iter++) {
    os << "<" << (*iter)->GetVertexId() << ", "
       << (*iter)->GetDistanceFromRoot() << ", " << (*iter)->GetVertexType()
       << ">" << std::endl;
  }
...
CandidateQueue::CandidateQueue() : m_candidates() { NS_LOG_FUNCTION(this); }
...
void CandidateQueue::Push(SPFVertex *vNew) {
  NS_LOG_FUNCTION(this << vNew);

  auto i = std::upper_bound(m_candidates.begin(), m_candidates.end(), vNew,
                            &CandidateQueue::CompareSPFVertex);
  m_candidates.insert(i, vNew);
}

SPFVertex *CandidateQueue::Pop() {
  NS_LOG_FUNCTION(this);
...
SPFVertex *CandidateQueue::Top() const {
  NS_LOG_FUNCTION(this);
  if (m_candidates.empty()) {
    return nullptr;
  }
...
uint32_t CandidateQueue::Size() const {
  NS_LOG_FUNCTION(this);
  return m_candidates.size();
}

SPFVertex *CandidateQueue::Find(const Ipv4Address addr) const {
  NS_LOG_FUNCTION(this);
 ...
 */
bool CandidateQueue::CompareSPFVertex(const SPFVertex *v1,
                                      const SPFVertex *v2) {
