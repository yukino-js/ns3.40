...
TypeId TcpWestwoodPlus::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TcpWestwoodPlus")
          ...
          .AddAttribute(
              ...
              MakeEnumChecker(TcpWestwoodPlus::NONE, "None",
                              TcpWestwoodPlus::TUSTIN, "Tustin"))
          .AddTraceSource(
...
TcpWestwoodPlus::TcpWestwoodPlus()
    : TcpNewReno(), m_currentBW(0), m_lastSampleBW(0), m_lastBW(0),
      m_ackedSegments(0), m_IsCount(false), m_lastAck(0) {
  NS_LOG_FUNCTION(this);
}

TcpWestwoodPlus::TcpWestwoodPlus(const TcpWestwoodPlus &sock)
    : TcpNewReno(sock), m_currentBW(sock.m_currentBW),
      m_lastSampleBW(sock.m_lastSampleBW), m_lastBW(sock.m_lastBW),
      m_fType(sock.m_fType), m_IsCount(sock.m_IsCount) {
  NS_LOG_FUNCTION(this);
...
void TcpWestwoodPlus::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t packetsAcked,
                                const Time &rtt) {
  ...
  if (!(rtt.IsZero() || m_IsCount)) {
    ...
    m_bwEstimateEvent =
        Simulator::Schedule(rtt, &TcpWestwoodPlus::EstimateBW, this, rtt, tcb);
  }
}

void TcpWestwoodPlus::EstimateBW(const Time &rtt, Ptr<TcpSocketState> tcb) {
  NS_LOG_FUNCTION(this);

  NS_ASSERT(!rtt.IsZero());

  m_currentBW =
      DataRate(m_ackedSegments * tcb->m_segmentSize * 8.0 / rtt.GetSeconds());
  m_IsCount = false;
  ...
  if (m_fType == TcpWestwoodPlus::TUSTIN) {
    ...
    m_currentBW = (m_lastBW * ALPHA) +
                  (((sample_bwe + m_lastSampleBW) * 0.5) * (1 - ALPHA));
...
uint32_t TcpWestwoodPlus::GetSsThresh(Ptr<const TcpSocketState> tcb,
                                      uint32_t bytesInFlight [[maybe_unused]]) {
  uint32_t ssThresh =
      static_cast<uint32_t>((m_currentBW * tcb->m_minRtt) / 8.0);
...
Ptr<TcpCongestionOps> TcpWestwoodPlus::Fork() {
