...
TypeId ArpL3Protocol::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ArpL3Protocol")
          .SetParent<Object>()
          .AddConstructor<ArpL3Protocol>()
          .SetGroupName("Internet")
          .AddAttribute("CacheList", "The list of ARP caches",
                        ObjectVectorValue(),
...
ArpL3Protocol::ArpL3Protocol() : m_tc(nullptr) { NS_LOG_FUNCTION(this); }

ArpL3Protocol::~ArpL3Protocol() { NS_LOG_FUNCTION(this); }

int64_t ArpL3Protocol::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_requestJitter->SetStream(stream);
  return 1;
}

void ArpL3Protocol::SetNode(Ptr<Node> node) {
  NS_LOG_FUNCTION(this << node);
  m_node = node;
}

void ArpL3Protocol::SetTrafficControl(Ptr<TrafficControlLayer> tc) {
...
Ptr<ArpCache> ArpL3Protocol::CreateCache(Ptr<NetDevice> device,
  ...
  cache->SetArpRequestCallback(
      MakeCallback(&ArpL3Protocol::SendArpRequest, this));
  m_cacheList.push_back(cache);
  return cache;
}

Ptr<ArpCache> ArpL3Protocol::FindCache(Ptr<NetDevice> device) {
...
void ArpL3Protocol::Receive(Ptr<NetDevice> device, Ptr<const Packet> p,
                            uint16_t protocol, const Address &from,
                            const Address &to,
                            NetDevice::PacketType packetType) {
  NS_LOG_FUNCTION(this << device << p->GetSize() << protocol << from << to
                       << packetType);
  ...
  for (uint32_t i = 0; i < cache->GetInterface()->GetNAddresses(); i++) {
    if (arp.IsRequest() &&
      ...
      SendArpReply(cache, arp.GetDestinationIpv4Address(),
                   arp.GetSourceIpv4Address(), arp.GetSourceHardwareAddress());
      break;
    } else if (arp.IsReply() &&
               arp.GetDestinationIpv4Address() ==
                   cache->GetInterface()->GetAddress(i).GetLocal() &&
...
bool ArpL3Protocol::Lookup(Ptr<Packet> packet, const Ipv4Header &ipHeader,
    ...
    if (entry->IsExpired()) {
      if (entry->IsDead()) {
        NS_LOG_LOGIC("node=" << m_node->GetId() << ", dead entry for "
                             << destination << " expired -- send arp request");
        ...
        Simulator::Schedule(Time(MilliSeconds(m_requestJitter->GetValue())),
                            &ArpL3Protocol::SendArpRequest, this, cache,
                            destination);
      } else if (entry->IsAlive()) {
        NS_LOG_LOGIC("node=" << m_node->GetId() << ", alive entry for "
                             << destination << " expired -- send arp request");
        entry->MarkWaitReply(ArpCache::Ipv4PayloadHeaderPair(packet, ipHeader));
        Simulator::Schedule(Time(MilliSeconds(m_requestJitter->GetValue())),
                            &ArpL3Protocol::SendArpRequest, this, cache,
      ...
      if (entry->IsDead()) {
        ...
        NS_LOG_LOGIC("node=" << m_node->GetId() << ", alive entry for "
                             << destination << " valid -- send");
        *hardwareDestination = entry->GetMacAddress();
        return true;
      } else if (entry->IsWaitReply()) {
        NS_LOG_LOGIC("node=" << m_node->GetId() << ", wait reply for "
        ...
        NS_LOG_LOGIC("node=" << m_node->GetId() << ", permanent for "
                             << destination << "valid -- send");
        *hardwareDestination = entry->GetMacAddress();
        return true;
      } else {
...
void ArpL3Protocol::SendArpRequest(Ptr<const ArpCache> cache, Ipv4Address to) {
  ...
  Ipv4Address source =
      ipv4->SelectSourceAddress(device, to, Ipv4InterfaceAddress::GLOBAL);
  NS_LOG_LOGIC("ARP: sending request from node "
               << m_node->GetId() << " || src: " << device->GetAddress()
               << " / " << source << " || dst: " << device->GetBroadcast()
               << " / " << to);
  arp.SetRequest(device->GetAddress(), source, device->GetBroadcast(), to);
  NS_ASSERT(m_tc);
  m_tc->Send(device, Create<ArpQueueDiscItem>(packet, device->GetBroadcast(),
                                              PROT_NUMBER, arp));
}

void ArpL3Protocol::SendArpReply(Ptr<const ArpCache> cache, Ipv4Address myIp,
  ...
  m_tc->Send(cache->GetDevice(),
             Create<ArpQueueDiscItem>(packet, toMac, PROT_NUMBER, arp));
}
