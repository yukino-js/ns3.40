...
TypeId IpL4Protocol::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::IpL4Protocol")
          .SetParent<Object>()
          .SetGroupName("Internet")
          .AddAttribute("ProtocolNumber", "The IP protocol number.",
                        TypeId::ATTR_GET, IntegerValue(0),
                        MakeIntegerAccessor(&IpL4Protocol::GetProtocolNumber),
                        MakeIntegerChecker<int>(0, 255));
  return tid;
}

IpL4Protocol::~IpL4Protocol() { NS_LOG_FUNCTION(this); }

void IpL4Protocol::ReceiveIcmp(Ipv4Address icmpSource, uint8_t icmpTtl,
  ...
  NS_LOG_FUNCTION(this << icmpSource << static_cast<uint32_t>(icmpTtl)
                       ...
                       << payloadSource << payloadDestination << payload);
}

void IpL4Protocol::ReceiveIcmp(Ipv6Address icmpSource, uint8_t icmpTtl,
                               uint8_t icmpType, uint8_t icmpCode,
