...
UdpL4Protocol::~UdpL4Protocol() { NS_LOG_FUNCTION(this); }
...
void UdpL4Protocol::NotifyNewAggregate() {
  ...
  if (!m_node) {
    if (node && (ipv4 || ipv6)) {
      this->SetNode(node);
      Ptr<UdpSocketFactoryImpl> udpFactory =
...
int UdpL4Protocol::GetProtocolNumber() const { return PROT_NUMBER; }
...
Ptr<Socket> UdpL4Protocol::CreateSocket() {
  NS_LOG_FUNCTION(this);
...
Ipv4EndPoint *UdpL4Protocol::Allocate() {
  NS_LOG_FUNCTION(this);
  return m_endPoints->Allocate();
}

Ipv4EndPoint *UdpL4Protocol::Allocate(Ipv4Address address) {
  NS_LOG_FUNCTION(this << address);
  return m_endPoints->Allocate(address);
}

Ipv4EndPoint *UdpL4Protocol::Allocate(Ptr<NetDevice> boundNetDevice,
                                      uint16_t port) {
...
void UdpL4Protocol::DeAllocate(Ipv4EndPoint *endPoint) {
  NS_LOG_FUNCTION(this << endPoint);
  m_endPoints->DeAllocate(endPoint);
}

Ipv6EndPoint *UdpL4Protocol::Allocate6() {
  NS_LOG_FUNCTION(this);
  return m_endPoints6->Allocate();
}

Ipv6EndPoint *UdpL4Protocol::Allocate6(Ipv6Address address) {
  NS_LOG_FUNCTION(this << address);
  return m_endPoints6->Allocate(address);
}

Ipv6EndPoint *UdpL4Protocol::Allocate6(Ptr<NetDevice> boundNetDevice,
                                       uint16_t port) {
  NS_LOG_FUNCTION(this << boundNetDevice << port);
...
Ipv6EndPoint *UdpL4Protocol::Allocate6(Ptr<NetDevice> boundNetDevice,
                                       Ipv6Address address, uint16_t port) {
  NS_LOG_FUNCTION(this << boundNetDevice << address << port);
...
Ipv6EndPoint *UdpL4Protocol::Allocate6(Ptr<NetDevice> boundNetDevice,
  ...
  NS_LOG_FUNCTION(this << boundNetDevice << localAddress << localPort
                       << peerAddress << peerPort);
  return m_endPoints6->Allocate(boundNetDevice, localAddress, localPort,
                                peerAddress, peerPort);
}

void UdpL4Protocol::DeAllocate(Ipv6EndPoint *endPoint) {
  NS_LOG_FUNCTION(this << endPoint);
  m_endPoints6->DeAllocate(endPoint);
}

void UdpL4Protocol::ReceiveIcmp(Ipv4Address icmpSource, uint8_t icmpTtl,
...
void UdpL4Protocol::ReceiveIcmp(Ipv6Address icmpSource, uint8_t icmpTtl,
                                uint8_t icmpType, uint8_t icmpCode,
...
IpL4Protocol::RxStatus UdpL4Protocol::Receive(Ptr<Packet> packet,
  ...
  if (endPoints.empty()) {
    if (this->GetObject<Ipv6>()) {
      NS_LOG_LOGIC("  No Ipv4 endpoints matched on UdpL4Protocol, trying Ipv6 "
                   << this);
      Ptr<Ipv6Interface> fakeInterface;
      Ipv6Header ipv6Header;
      Ipv6Address src = Ipv6Address::MakeIpv4MappedAddress(header.GetSource());
      Ipv6Address dst =
          Ipv6Address::MakeIpv4MappedAddress(header.GetDestination());
...
void UdpL4Protocol::Send(Ptr<Packet> packet, Ipv4Address saddr,
...
void UdpL4Protocol::Send(Ptr<Packet> packet, Ipv6Address saddr,
                         Ipv6Address daddr, uint16_t sport, uint16_t dport) {
  NS_LOG_FUNCTION(this << packet << saddr << daddr << sport << dport);
...
void UdpL4Protocol::SetDownTarget(IpL4Protocol::DownTargetCallback callback) {
  NS_LOG_FUNCTION(this);
  m_downTarget = callback;
}

IpL4Protocol::DownTargetCallback UdpL4Protocol::GetDownTarget() const {
  return m_downTarget;
}

void UdpL4Protocol::SetDownTarget6(IpL4Protocol::DownTargetCallback6 callback) {
  NS_LOG_FUNCTION(this);
  m_downTarget6 = callback;
}

IpL4Protocol::DownTargetCallback6 UdpL4Protocol::GetDownTarget6() const {
  return m_downTarget6;
}

bool UdpL4Protocol::RemoveSocket(Ptr<UdpSocketImpl> socket) {
