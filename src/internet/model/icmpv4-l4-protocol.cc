...
void Icmpv4L4Protocol::SetNode(Ptr<Node> node) {
  ...
  if (!m_node) {
    ...
    if (node) {
      ...
      if (ipv4 && m_downTarget.IsNull()) {
        this->SetNode(node);
        ipv4->Insert(this);
        Ptr<Ipv4RawSocketFactoryImpl> rawFactory =
...
uint16_t Icmpv4L4Protocol::GetStaticProtocolNumber() {
  NS_LOG_FUNCTION_NOARGS();
  return PROT_NUMBER;
}

int Icmpv4L4Protocol::GetProtocolNumber() const {
  NS_LOG_FUNCTION(this);
  return PROT_NUMBER;
}

void Icmpv4L4Protocol::SendMessage(Ptr<Packet> packet, Ipv4Address dest,
...
void Icmpv4L4Protocol::SendMessage(Ptr<Packet> packet, Ipv4Address source,
                                   Ipv4Address dest, uint8_t type, uint8_t code,
                                   Ptr<Ipv4Route> route) {
...
void Icmpv4L4Protocol::SendDestUnreachFragNeeded(Ipv4Header header,
                                                 Ptr<const Packet> orgData,
                                                 uint16_t nextHopMtu) {
  NS_LOG_FUNCTION(this << header << *orgData << nextHopMtu);
  SendDestUnreach(header, orgData,
                  Icmpv4DestinationUnreachable::ICMPV4_FRAG_NEEDED, nextHopMtu);
}

void Icmpv4L4Protocol::SendDestUnreachPort(Ipv4Header header,
                                           Ptr<const Packet> orgData) {
  NS_LOG_FUNCTION(this << header << *orgData);
  SendDestUnreach(header, orgData,
                  Icmpv4DestinationUnreachable::ICMPV4_PORT_UNREACHABLE, 0);
}

void Icmpv4L4Protocol::SendDestUnreach(Ipv4Header header,
                                       Ptr<const Packet> orgData, uint8_t code,
                                       uint16_t nextHopMtu) {
...
void Icmpv4L4Protocol::SendTimeExceededTtl(Ipv4Header header,
                                           Ptr<const Packet> orgData,
                                           bool isFragment) {
  NS_LOG_FUNCTION(this << header << *orgData);
  Ptr<Packet> p = Create<Packet>();
...
void Icmpv4L4Protocol::HandleEcho(Ptr<Packet> p, Icmpv4Header header,
  ...
  SendMessage(reply, destination, source, Icmpv4Header::ICMPV4_ECHO_REPLY, 0,
              nullptr);
}

void Icmpv4L4Protocol::Forward(Ipv4Address source, Icmpv4Header icmp,
  ...
  if (l4) {
    l4->ReceiveIcmp(source, ipHeader.GetTtl(), icmp.GetType(), icmp.GetCode(),
                    info, ipHeader.GetSource(), ipHeader.GetDestination(),
                    payload);
  }
}

void Icmpv4L4Protocol::HandleDestUnreach(Ptr<Packet> p, Icmpv4Header icmp,
                                         Ipv4Address source,
...
void Icmpv4L4Protocol::HandleTimeExceeded(Ptr<Packet> p, Icmpv4Header icmp,
                                          Ipv4Address source,
                                          Ipv4Address destination) {
  NS_LOG_FUNCTION(this << p << icmp << source << destination);
...
void Icmpv4L4Protocol::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_node = nullptr;
...
void Icmpv4L4Protocol::SetDownTarget(
    IpL4Protocol::DownTargetCallback callback) {
  NS_LOG_FUNCTION(this << &callback);
  m_downTarget = callback;
}

void Icmpv4L4Protocol::SetDownTarget6(
    IpL4Protocol::DownTargetCallback6 callback) {
  NS_LOG_FUNCTION(this << &callback);
}

IpL4Protocol::DownTargetCallback Icmpv4L4Protocol::GetDownTarget() const {
  NS_LOG_FUNCTION(this);
  return m_downTarget;
}

IpL4Protocol::DownTargetCallback6 Icmpv4L4Protocol::GetDownTarget6() const {
  NS_LOG_FUNCTION(this);
