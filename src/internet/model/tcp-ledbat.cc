...
TypeId TcpLedbat::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TcpLedbat")
          .SetParent<TcpNewReno>()
          .AddConstructor<TcpLedbat>()
          .SetGroupName("Internet")
          .AddAttribute("TargetDelay", "Targeted Queue Delay",
                        TimeValue(MilliSeconds(100)),
                        MakeTimeAccessor(&TcpLedbat::m_target),
                        MakeTimeChecker())
          .AddAttribute("BaseHistoryLen", "Number of Base delay samples",
                        UintegerValue(10),
                        MakeUintegerAccessor(&TcpLedbat::m_baseHistoLen),
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute("NoiseFilterLen", "Number of Current delay samples",
                        UintegerValue(4),
          ...
          .AddAttribute(
              "SSParam", "Possibility of Slow Start", EnumValue(DO_SLOWSTART),
...
void TcpLedbat::InitCircBuf(OwdCircBuf &buffer) {
  NS_LOG_FUNCTION(this);
  buffer.buffer.clear();
  buffer.min = 0;
}
...
TcpLedbat::~TcpLedbat() { NS_LOG_FUNCTION(this); }

Ptr<TcpCongestionOps> TcpLedbat::Fork() { return CopyObject<TcpLedbat>(this); }

std::string TcpLedbat::GetName() const { return "TcpLedbat"; }

uint32_t TcpLedbat::MinCircBuf(OwdCircBuf &b) {
...
void TcpLedbat::IncreaseWindow(Ptr<TcpSocketState> tcb,
                               uint32_t segmentsAcked) {
  NS_LOG_FUNCTION(this << tcb << segmentsAcked);
  if (tcb->m_cWnd.Get() <= tcb->m_segmentSize) {
    m_flag |= LEDBAT_CAN_SS;
  }
  if (m_doSs == DO_SLOWSTART && tcb->m_cWnd <= tcb->m_ssThresh &&
      (m_flag & LEDBAT_CAN_SS)) {
...
void TcpLedbat::CongestionAvoidance(Ptr<TcpSocketState> tcb,
                                    uint32_t segmentsAcked) {
  NS_LOG_FUNCTION(this << tcb << segmentsAcked);
  ...
  offset *= m_gain;
  m_sndCwndCnt =
      static_cast<int32_t>(offset * segmentsAcked * tcb->m_segmentSize);
  double inc =
      (m_sndCwndCnt * 1.0) / (m_target.GetMilliSeconds() * tcb->m_cWnd.Get());
...
void TcpLedbat::AddDelay(OwdCircBuf &cb, uint32_t owd, uint32_t maxlen) {
