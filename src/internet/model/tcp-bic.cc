  ...
  static TypeId tid =
      TypeId("ns3::TcpBic")
          ...
          .AddAttribute("Beta", "Beta for multiplicative decrease",
                        DoubleValue(0.8), MakeDoubleAccessor(&TcpBic::m_beta),
...
TcpBic::TcpBic()
    : TcpCongestionOps(), m_cWndCnt(0), m_lastMaxCwnd(0), m_lastCwnd(0),
...
TcpBic::TcpBic(const TcpBic &sock)
    : TcpCongestionOps(sock), m_fastConvergence(sock.m_fastConvergence),
      m_beta(sock.m_beta), m_maxIncr(sock.m_maxIncr), m_lowWnd(sock.m_lowWnd),
      m_smoothPart(sock.m_smoothPart), m_cWndCnt(sock.m_cWndCnt),
      m_lastMaxCwnd(sock.m_lastMaxCwnd), m_lastCwnd(sock.m_lastCwnd),
      m_epochStart(sock.m_epochStart), m_b(sock.m_b) {
  NS_LOG_FUNCTION(this);
}

void TcpBic::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) {
...
uint32_t TcpBic::Update(Ptr<TcpSocketState> tcb) {
...
std::string TcpBic::GetName() const { return "TcpBic"; }

uint32_t TcpBic::GetSsThresh(Ptr<const TcpSocketState> tcb,
  ...
  if (segCwnd < m_lowWnd) {
    ...
    ssThresh = static_cast<uint32_t>(std::max(segCwnd * m_beta, 2.0) *
                                     tcb->m_segmentSize);
...
Ptr<TcpCongestionOps> TcpBic::Fork() { return CopyObject<TcpBic>(this); }
