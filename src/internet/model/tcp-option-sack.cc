...
TypeId TcpOptionSack::GetTypeId() {
...
TypeId TcpOptionSack::GetInstanceTypeId() const { return GetTypeId(); }

void TcpOptionSack::Print(std::ostream &os) const {
...
uint32_t TcpOptionSack::GetSerializedSize() const {
...
uint32_t TcpOptionSack::Deserialize(Buffer::Iterator start) {
  NS_LOG_FUNCTION(this);
  Buffer::Iterator i = start;
...
uint8_t TcpOptionSack::GetKind() const { return TcpOption::SACK; }

void TcpOptionSack::AddSackBlock(SackBlock s) {
  NS_LOG_FUNCTION(this);
  m_sackList.push_back(s);
}

uint32_t TcpOptionSack::GetNumSackBlocks() const {
  NS_LOG_FUNCTION(this);
  NS_LOG_LOGIC("Number of SACK blocks appended: " << m_sackList.size());
  return static_cast<uint32_t>(m_sackList.size());
}

void TcpOptionSack::ClearSackList() { m_sackList.clear(); }

TcpOptionSack::SackList TcpOptionSack::GetSackList() const {
  NS_LOG_FUNCTION(this);
  return m_sackList;
}
...
std::ostream &operator<<(std::ostream &os, const TcpOptionSack &sackOption) {
  std::stringstream ss;
  ss << "{";
  for (auto it = sackOption.m_sackList.begin();
       it != sackOption.m_sackList.end(); ++it) {
    ss << *it;
  }
  ss << "}";
  os << ss.str();
