...
Ipv4QueueDiscItem::~Ipv4QueueDiscItem() { NS_LOG_FUNCTION(this); }
...
const Ipv4Header &Ipv4QueueDiscItem::GetHeader() const { return m_header; }

void Ipv4QueueDiscItem::AddHeader() {
  NS_LOG_FUNCTION(this);

  NS_ASSERT_MSG(!m_headerAdded,
...
void Ipv4QueueDiscItem::Print(std::ostream &os) const {
  if (!m_headerAdded) {
    os << m_header << " ";
  }
  os << GetPacket() << " "
...
bool Ipv4QueueDiscItem::GetUint8Value(QueueItem::Uint8Values field,
                                      uint8_t &value) const {
  bool ret = false;
...
uint32_t Ipv4QueueDiscItem::Hash(uint32_t perturbation) const {
