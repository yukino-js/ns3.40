...
SequenceNumber32 TcpRxBuffer::MaxRxSequence() const {
  if (m_gotFin) { // No data allowed beyond FIN
    return m_finSeq;
  } else if (!m_data.empty() &&
             m_nextRxSeq >
                 m_data.begin()
                     ->first) { // No data allowed beyond Rx window allowed
...
bool TcpRxBuffer::Add(Ptr<Packet> p, const TcpHeader &tcph) {
  ...
  while (i != m_data.end() && i->first <= tailSeq) {
    ...
    if (lastByteSeq > headSeq) {
      if (i->first > headSeq &&
          lastByteSeq < tailSeq) { // Rare case: Existing packet is embedded
                                   // fully in the new packet
        m_size -= i->second->GetSize();
        m_data.erase(i++);
        continue;
      }
      if (i->first <= headSeq) { // Incoming head is overlapped
        headSeq = lastByteSeq;
      }
      if (lastByteSeq >= tailSeq) { // Incoming tail is overlapped
        tailSeq = i->first;
      }
    }
    ++i;
  ...
  if (m_gotFin && m_nextRxSeq == m_finSeq) { // Account for the FIN packet
    ++m_nextRxSeq;
  };
  return true;
}
...
Ptr<Packet> TcpRxBuffer::Extract(uint32_t maxSize) {
  ...
  Ptr<Packet> outPkt =
      Create<Packet>(); // The packet that contains all the data to return
  BufIterator i;
  while (extractSize) { // Check the buffered data for delivery
    i = m_data.begin();
    ...
    if (pktSize <= extractSize) { // Whole packet is extracted
      outPkt->AddAtEnd(i->second);
      m_data.erase(i);
      m_size -= pktSize;
      m_availBytes -= pktSize;
      extractSize -= pktSize;
    } else { // Partial is extracted and done
