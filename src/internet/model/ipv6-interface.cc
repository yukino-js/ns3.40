...
TypeId Ipv6Interface::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ipv6Interface").SetParent<Object>().SetGroupName("Internet");
  return tid;
}

Ipv6Interface::Ipv6Interface()
    : m_ifup(false), m_forwarding(true), m_metric(1), m_node(nullptr),
      m_device(nullptr), m_tc(nullptr), m_ndCache(nullptr), m_curHopLimit(0),
      m_baseReachableTime(0), m_reachableTime(0), m_retransTimer(0) {
...
void Ipv6Interface::SetTrafficControl(Ptr<TrafficControlLayer> tc) {
...
bool Ipv6Interface::AddAddress(Ipv6InterfaceAddress iface) {
  ...
  if (!addr.IsAny()) {
    ...
    Ipv6Address solicited =
        Ipv6Address::MakeSolicitedAddress(iface.GetAddress());
    ...
    if (!addr.IsAny() || !addr.IsLocalhost()) {
      ...
      if (icmpv6) {
        if (icmpv6->IsAlwaysDad()) {
          Simulator::Schedule(Seconds(0.), &Icmpv6L4Protocol::DoDAD, icmpv6,
                              addr, this);
          Simulator::Schedule(icmpv6->GetDadTimeout(),
                              &Icmpv6L4Protocol::FunctionDadTimeout, icmpv6,
                              this, addr);
        } else {
...
void Ipv6Interface::Send(Ptr<Packet> p, const Ipv6Header &hdr,
  ...
  if (m_device->NeedsArp()) {
    ...
    if (dest.IsMulticast()) {
      ...
      found = icmpv6->Lookup(p, hdr, dest, GetDevice(), m_ndCache,
                             &hardwareDestination);
    }
    ...
    if (found) {
      ...
      m_tc->Send(m_device,
                 Create<Ipv6QueueDiscItem>(p, hardwareDestination,
                                           Ipv6L3Protocol::PROT_NUMBER, hdr));
    }
  } else {
    NS_LOG_LOGIC("Doesn't need NDISC");
    m_tc->Send(m_device,
               Create<Ipv6QueueDiscItem>(p, m_device->GetBroadcast(),
                                         Ipv6L3Protocol::PROT_NUMBER, hdr));
  }
}
...
void Ipv6Interface::SetState(Ipv6Address address,
...
void Ipv6Interface::RemoveAddressCallback(
...
void Ipv6Interface::AddAddressCallback(
    Callback<void, Ptr<Ipv6Interface>, Ipv6InterfaceAddress>
        addAddressCallback) {
