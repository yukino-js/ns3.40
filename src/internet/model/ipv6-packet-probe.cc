...
TypeId Ipv6PacketProbe::GetTypeId() {
  static TypeId tid =
...
Ipv6PacketProbe::~Ipv6PacketProbe() { NS_LOG_FUNCTION(this); }

void Ipv6PacketProbe::SetValue(Ptr<const Packet> packet, Ptr<Ipv6> ipv6,
                               uint32_t interface) {
...
void Ipv6PacketProbe::SetValueByPath(std::string path, Ptr<const Packet> packet,
...
bool Ipv6PacketProbe::ConnectByObject(std::string traceSource,
                                      Ptr<Object> obj) {
...
void Ipv6PacketProbe::ConnectByPath(std::string path) {
  NS_LOG_FUNCTION(this << path);
  NS_LOG_DEBUG("Name of probe to search for in config database: " << path);
  Config::ConnectWithoutContext(
      path, MakeCallback(&ns3::Ipv6PacketProbe::TraceSink, this));
}

void Ipv6PacketProbe::TraceSink(Ptr<const Packet> packet, Ptr<Ipv6> ipv6,
                                uint32_t interface) {
  NS_LOG_FUNCTION(this << packet << ipv6 << interface);
  if (IsEnabled()) {
    m_packet = packet;
