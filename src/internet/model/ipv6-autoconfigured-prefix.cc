...
Ipv6AutoconfiguredPrefix::Ipv6AutoconfiguredPrefix(
    Ptr<Node> node, uint32_t interface, Ipv6Address prefix, Ipv6Prefix mask,
    uint32_t preferredLifeTime, uint32_t validLifeTime, Ipv6Address router) {
...
uint32_t Ipv6AutoconfiguredPrefix::GetInterface() const { return m_interface; }
...
void Ipv6AutoconfiguredPrefix::MarkPreferredTime() { m_preferred = true; }
...
void Ipv6AutoconfiguredPrefix::StartPreferredTimer() {
  if (m_preferredLifeTime != 0xffffffff) {
    ...
    m_preferredTimer.SetFunction(
        &Ipv6AutoconfiguredPrefix::FunctionPreferredTimeout, this);
...
void Ipv6AutoconfiguredPrefix::StartValidTimer() {
  if (m_validLifeTime != 0xffffffff) {
    ...
    m_validTimer.SetFunction(&Ipv6AutoconfiguredPrefix::FunctionValidTimeout,
                             this);
...
void Ipv6AutoconfiguredPrefix::RemoveMe() {
  NS_LOG_INFO("The prefix " << m_prefix << " will be removed on interface "
                            << m_interface);
  Ptr<Ipv6L3Protocol> ipv6 = m_node->GetObject<Ipv6L3Protocol>();
  ipv6->RemoveAutoconfiguredAddress(m_interface, m_prefix, m_mask,
                                    m_defaultGatewayRouter);
}

void Ipv6AutoconfiguredPrefix::SetPreferred() { m_preferred = true; }
...
uint32_t Ipv6AutoconfiguredPrefix::GetId() const { return m_id; }

bool Ipv6AutoconfiguredPrefix::IsPreferred() const { return m_preferred; }

bool Ipv6AutoconfiguredPrefix::IsValid() const { return m_valid; }

Ipv6Address Ipv6AutoconfiguredPrefix::GetPrefix() const { return m_prefix; }

void Ipv6AutoconfiguredPrefix::SetPrefix(Ipv6Address prefix) {
  m_prefix = prefix;
}

Ipv6Prefix Ipv6AutoconfiguredPrefix::GetMask() const { return m_mask; }

void Ipv6AutoconfiguredPrefix::SetMask(Ipv6Prefix mask) { m_mask = mask; }
