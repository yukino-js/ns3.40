...
TypeId Ipv4GlobalRouting::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ipv4GlobalRouting")
          ...
          .AddAttribute(
              "FlowEcmpRouting",
              "Set to true if flows are randomly routed among ECMP; set to "
              "false for "
              "using only one route consistently",
              BooleanValue(false),
...
Ipv4GlobalRouting::~Ipv4GlobalRouting() { NS_LOG_FUNCTION(this); }
...
uint32_t Ipv4GlobalRouting::GetNRoutes() const {
  NS_LOG_FUNCTION(this);
  uint32_t n = 0;
  n += m_hostRoutes.size();
...
Ipv4RoutingTableEntry *Ipv4GlobalRouting::GetRoute(uint32_t index) const {
...
void Ipv4GlobalRouting::RemoveRoute(uint32_t index) {
    ...
    for (auto i = m_hostRoutes.begin(); i != m_hostRoutes.end(); i++) {
      if (tmp == index) {
        ...
        NS_LOG_LOGIC("Done removing host route "
                     << index << "; host route remaining size = "
                     << m_hostRoutes.size());
        return;
      }
      tmp++;
  ...
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    if (tmp == index) {
      ...
      NS_LOG_LOGIC("Done removing network route "
                   << index << "; network route remaining size = "
  ...
  for (auto k = m_ASexternalRoutes.begin(); k != m_ASexternalRoutes.end();
       k++) {
    if (tmp == index) {
      NS_LOG_LOGIC("Removing route "
                   << index << "; size = " << m_ASexternalRoutes.size());
      delete *k;
...
void Ipv4GlobalRouting::DoDispose() {
  NS_LOG_FUNCTION(this);
  for (auto i = m_hostRoutes.begin(); i != m_hostRoutes.end();
...
bool Ipv4GlobalRouting::RouteInput(
