...
Callback<void, TcpTxItem *> TcpTxBuffer::m_nullCb =
    MakeNullCallback<void, TcpTxItem *>();

TypeId TcpTxBuffer::GetTypeId() {
  static TypeId tid =
...
SequenceNumber32 TcpTxBuffer::HeadSequence() const { return m_firstByteSeq; }

SequenceNumber32 TcpTxBuffer::TailSequence() const {
  return m_firstByteSeq + SequenceNumber32(m_size);
}

uint32_t TcpTxBuffer::Size() const { return m_size; }

uint32_t TcpTxBuffer::MaxBufferSize() const { return m_maxBuffer; }

void TcpTxBuffer::SetMaxBufferSize(uint32_t n) { m_maxBuffer = n; }

bool TcpTxBuffer::IsSackEnabled() const { return m_sackEnabled; }

void TcpTxBuffer::SetSackEnabled(bool enabled) { m_sackEnabled = enabled; }

uint32_t TcpTxBuffer::Available() const { return m_maxBuffer - m_size; }

void TcpTxBuffer::SetDupAckThresh(uint32_t dupAckThresh) {
  m_dupAckThresh = dupAckThresh;
}

void TcpTxBuffer::SetSegmentSize(uint32_t segmentSize) {
  m_segmentSize = segmentSize;
}

uint32_t TcpTxBuffer::GetRetransmitsCount() const { return m_retrans; }

uint32_t TcpTxBuffer::GetLost() const { return m_lostOut; }

uint32_t TcpTxBuffer::GetSacked() const { return m_sackedOut; }

void TcpTxBuffer::SetHeadSequence(const SequenceNumber32 &seq) {
...
bool TcpTxBuffer::Add(Ptr<Packet> p) {
  ...
  NS_LOG_LOGIC("Try to append "
               << p->GetSize() << " bytes to window starting at "
...
uint32_t TcpTxBuffer::SizeFromSequence(const SequenceNumber32 &seq) const {
  ...
  NS_LOG_ERROR("Requested a sequence beyond our space ("
               << seq << " > " << lastSeq << "). Returning 0 for convenience.");
  return 0;
}

TcpTxItem *TcpTxBuffer::CopyFromSequence(uint32_t numBytes,
  ...
  if (m_firstByteSeq + m_sentSize >= seq + s) {
    ...
    NS_LOG_DEBUG("Returning already sent item " << *outItem << " from "
                                                << *this);
...
TcpTxItem *TcpTxBuffer::GetNewSegment(uint32_t numBytes) {
  NS_LOG_FUNCTION(this << numBytes);

  SequenceNumber32 startOfAppList = m_firstByteSeq + m_sentSize;

  NS_LOG_INFO("AppList start at " << startOfAppList
                                  << ", sentSize = " << m_sentSize
...
TcpTxItem *TcpTxBuffer::GetTransmittedSegment(uint32_t numBytes,
                                              const SequenceNumber32 &seq) {
  NS_LOG_FUNCTION(this << numBytes << seq);
  ...
  TcpTxItem *item =
      GetPacketFromList(m_sentList, m_firstByteSeq, s, seq, &listEdited);
...
void TcpTxBuffer::SplitItems(TcpTxItem *t1, TcpTxItem *t2,
                             uint32_t size) const {
  ...
  NS_LOG_INFO("Split of size " << size << " result: t1 " << *t1 << " t2 "
                               << *t2);
}

TcpTxItem *TcpTxBuffer::GetPacketFromList(PacketList &list,
  ...
  while (it != list.end()) {
    ...
    NS_ASSERT_MSG(
        list != m_sentList || currentItem->m_startSeq >= m_firstByteSeq,
...
void TcpTxBuffer::RemoveFromCounts(TcpTxItem *item, uint32_t size) {
...
bool TcpTxBuffer::IsRetransmittedDataAcked(const SequenceNumber32 &ack) const {
  NS_LOG_FUNCTION(this);
  for (const auto &it : m_sentList) {
    TcpTxItem *item = it;
    Ptr<Packet> p = item->m_packet;
    if (item->m_startSeq + p->GetSize() == ack && !item->m_sacked &&
        item->m_retrans) {
      return true;
    }
  }
...
void TcpTxBuffer::DiscardUpTo(const SequenceNumber32 &seq,
  ...
  while (m_size > 0 && offset > 0) {
    ...
    if (offset >= pktSize) { // This packet is behind the seqnum. Remove this
      ...
      NS_LOG_INFO("Removed " << *item << " lost: " << m_lostOut << " retrans: "
                             << m_retrans << " sacked: " << m_sackedOut
      ...
      NS_LOG_INFO("Fragmented one packet by size "
                  << offset << ", new size=" << pktSize << " resulting item is "
                  << *item << " status: " << *this);
      break;
    }
  }
...
uint32_t TcpTxBuffer::Update(const TcpOptionSack::SackList &list,
                             const Callback<void, TcpTxItem *> &sackedCb) {
  NS_LOG_FUNCTION(this);
  NS_LOG_INFO("Updating scoreboard, got " << list.size()
                                          << " blocks to analyze");
  ...
  if (bytesSacked > 0) {
    NS_ASSERT_MSG(m_highestSack.first != m_sentList.end(),
                  "Buffer status: " << *this);
    UpdateLostCount();
  }
...
void TcpTxBuffer::UpdateLostCount() {
  NS_LOG_FUNCTION(this);
  uint32_t sacked = 0;
  SequenceNumber32 beginOfCurrentPacket = m_highestSack.second;
  if (m_highestSack.first == m_sentList.end()) {
    NS_LOG_INFO("Status before the update: "
                << *this << ", will start from the latest sent item");
  } else {
...
bool TcpTxBuffer::IsLost(const SequenceNumber32 &seq) const {
  NS_LOG_FUNCTION(this << seq);
...
bool TcpTxBuffer::NextSeg(SequenceNumber32 *seq, SequenceNumber32 *seqHigh,
  ...
  if (SizeFromSequence(m_firstByteSeq + m_sentSize) > 0) {
    if (m_sentSize <= m_rWndCallback()) {
      ...
      *seqHigh = *seq + std::min<uint32_t>(m_segmentSize,
                                           (m_rWndCallback() - m_sentSize));
      return true;
    } else {
...
uint32_t TcpTxBuffer::BytesInFlight() const {
...
uint32_t TcpTxBuffer::BytesInFlightRFC() const {
  ...
  NS_ASSERT_MSG(totalSize == m_sentSize,
                "Sent size counted: " << totalSize << " " << m_sentSize
                                      << *this);

  return size;
}

bool TcpTxBuffer::IsLostRFC(const SequenceNumber32 &seq,
  ...
  for (it = segment; it != m_sentList.end(); ++it) {
    ...
    if (item->m_sacked) {
      NS_LOG_INFO("Segment "
                  << *item << " found to be SACKed while checking for " << seq);
      ++count;
      bytes += current->GetSize();
      if ((count >= m_dupAckThresh) ||
          (bytes > (m_dupAckThresh - 1) * m_segmentSize)) {
        NS_LOG_INFO("seq=" << seq
...
void TcpTxBuffer::ResetRenoSack() {
  NS_LOG_FUNCTION(this);
...
void TcpTxBuffer::SetRWndCallback(Callback<uint32_t> rWndCallback) {
  NS_LOG_FUNCTION(this);
  m_rWndCallback = rWndCallback;
}

void TcpTxBuffer::ResetSentList() {
  NS_LOG_FUNCTION(this);
  TcpTxItem *item;
...
void TcpTxBuffer::ResetLastSegmentSent() {
  NS_LOG_FUNCTION(this);
  if (!m_sentList.empty()) {
...
void TcpTxBuffer::SetSentListLost(bool resetSack) {
  NS_LOG_FUNCTION(this);
  m_retrans = 0;
...
bool TcpTxBuffer::IsHeadRetransmitted() const {
  NS_LOG_FUNCTION(this);
...
void TcpTxBuffer::DeleteRetransmittedFlagFromHead() {
  NS_LOG_FUNCTION(this);
...
void TcpTxBuffer::ConsistencyCheck() const {
  ...
  NS_ASSERT_MSG(lost == m_lostOut,
                " Counted lost: " << lost << " stored lost: " << m_lostOut);
  NS_ASSERT_MSG(retrans == m_retrans,
                " Counted retrans: " << retrans
...
std::ostream &operator<<(std::ostream &os, const TcpTxBuffer &tcpTxBuf) {
  ...
  Ptr<const Packet> p;
  for (auto it = tcpTxBuf.m_sentList.begin(); it != tcpTxBuf.m_sentList.end();
       ++it) {
    p = (*it)->GetPacket();
    ss << "{";
    (*it)->Print(ss);
  ...
  for (auto it = tcpTxBuf.m_appList.begin(); it != tcpTxBuf.m_appList.end();
       ++it) {
