...
TcpIllinois::TcpIllinois()
    : TcpNewReno(), m_sumRtt(Time(0)), m_cntRtt(0), m_baseRtt(Time::Max()),
      m_maxRtt(Time::Min()), m_endSeq(0), m_rttAbove(false), m_rttLow(0),
      m_alphaMin(0.3), m_alphaMax(10.0), m_alphaBase(1.0), m_alpha(m_alphaMax),
      m_betaMin(0.125), m_betaMax(0.5), m_betaBase(0.5), m_beta(m_betaBase),
      m_winThresh(15), m_theta(5), m_ackCnt(0) {
  NS_LOG_FUNCTION(this);
}

TcpIllinois::TcpIllinois(const TcpIllinois &sock)
    : TcpNewReno(sock), m_sumRtt(sock.m_sumRtt), m_cntRtt(sock.m_cntRtt),
      m_baseRtt(sock.m_baseRtt), m_maxRtt(sock.m_maxRtt),
      m_endSeq(sock.m_endSeq), m_rttAbove(sock.m_rttAbove),
      m_rttLow(sock.m_rttLow), m_alphaMin(sock.m_alphaMin),
      m_alphaMax(sock.m_alphaMax), m_alphaBase(sock.m_alphaBase),
      m_alpha(sock.m_alpha), m_betaMin(sock.m_betaMin),
      m_betaMax(sock.m_betaMax), m_betaBase(sock.m_betaBase),
      m_beta(sock.m_beta), m_winThresh(sock.m_winThresh), m_theta(sock.m_theta),
      m_ackCnt(sock.m_ackCnt) {
  NS_LOG_FUNCTION(this);
}

TcpIllinois::~TcpIllinois() { NS_LOG_FUNCTION(this); }

void TcpIllinois::RecalcParam(uint32_t cWnd) {
...
void TcpIllinois::CongestionStateSet(
...
void TcpIllinois::IncreaseWindow(Ptr<TcpSocketState> tcb,
  ...
  if (tcb->m_cWnd < tcb->m_ssThresh) {
    ...
    if (segCwnd != oldCwnd) {
      ...
      NS_LOG_INFO("In CongAvoid, updated to cwnd "
                  << tcb->m_cWnd << " ssthresh " << tcb->m_ssThresh);
    }
  }
}

void TcpIllinois::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t packetsAcked,
                            const Time &rtt) {
...
uint32_t TcpIllinois::GetSsThresh(Ptr<const TcpSocketState> tcb,
                                  uint32_t bytesInFlight) {
  NS_LOG_FUNCTION(this << tcb << bytesInFlight);

  uint32_t segBytesInFlight = bytesInFlight / tcb->m_segmentSize;
  uint32_t ssThresh =
      static_cast<uint32_t>(std::max(2.0, (1.0 - m_beta) * segBytesInFlight));
...
void TcpIllinois::CalculateAlpha(double da, double dm) {
  ...
  if (da <= d1) {
    ...
    m_alpha = (dm * m_alphaMax) /
              (dm + (da * (m_alphaMax - m_alphaMin)) / m_alphaMin);
  }

  NS_LOG_INFO("Updated to alpha = " << m_alpha);
}

void TcpIllinois::CalculateBeta(double da, double dm) {
  NS_LOG_FUNCTION(this << da << dm);
...
Time TcpIllinois::CalculateAvgDelay() const {
  NS_LOG_FUNCTION(this);

  return (m_sumRtt / m_cntRtt - m_baseRtt);
}

Time TcpIllinois::CalculateMaxDelay() const {
  NS_LOG_FUNCTION(this);

  return (m_maxRtt - m_baseRtt);
}

void TcpIllinois::Reset(const SequenceNumber32 &nextTxSequence) {
...
Ptr<TcpCongestionOps> TcpIllinois::Fork() {
  NS_LOG_FUNCTION(this);

  return CopyObject<TcpIllinois>(this);
}

std::string TcpIllinois::GetName() const {
  NS_LOG_FUNCTION(this);
