...
TypeId Ipv4StaticRouting::GetTypeId() {
...
void Ipv4StaticRouting::AddNetworkRouteTo(Ipv4Address network,
...
void Ipv4StaticRouting::AddHostRouteTo(Ipv4Address dest, Ipv4Address nextHop,
                                       uint32_t interface, uint32_t metric) {
...
void Ipv4StaticRouting::AddHostRouteTo(Ipv4Address dest, uint32_t interface,
                                       uint32_t metric) {
  NS_LOG_FUNCTION(this << dest << " " << interface << " " << metric);
  AddNetworkRouteTo(dest, Ipv4Mask::GetOnes(), interface, metric);
}

void Ipv4StaticRouting::SetDefaultRoute(Ipv4Address nextHop, uint32_t interface,
                                        uint32_t metric) {
...
uint32_t Ipv4StaticRouting::GetNMulticastRoutes() const {
  NS_LOG_FUNCTION(this);
...
bool Ipv4StaticRouting::RemoveMulticastRoute(Ipv4Address origin,
                                             Ipv4Address group,
...
void Ipv4StaticRouting::RemoveMulticastRoute(uint32_t index) {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto i = m_multicastRoutes.begin(); i != m_multicastRoutes.end(); i++) {
    if (tmp == index) {
...
bool Ipv4StaticRouting::LookupRoute(const Ipv4RoutingTableEntry &route,
                                    uint32_t metric) {
...
Ptr<Ipv4Route> Ipv4StaticRouting::LookupStatic(Ipv4Address dest,
                                               Ptr<NetDevice> oif) {
...
Ptr<Ipv4MulticastRoute> Ipv4StaticRouting::LookupStatic(Ipv4Address origin,
                                                        Ipv4Address group,
                                                        uint32_t interface) {
...
uint32_t Ipv4StaticRouting::GetNRoutes() const {
  NS_LOG_FUNCTION(this);
...
Ipv4RoutingTableEntry Ipv4StaticRouting::GetRoute(uint32_t index) const {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    if (tmp == index) {
...
uint32_t Ipv4StaticRouting::GetMetric(uint32_t index) const {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    if (tmp == index) {
...
void Ipv4StaticRouting::RemoveRoute(uint32_t index) {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    if (tmp == index) {
...
Ptr<Ipv4Route> Ipv4StaticRouting::RouteOutput(Ptr<Packet> p,
...
bool Ipv4StaticRouting::RouteInput(
...
Ipv4StaticRouting::~Ipv4StaticRouting() { NS_LOG_FUNCTION(this); }

void Ipv4StaticRouting::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void Ipv4StaticRouting::NotifyAddAddress(uint32_t interface,
...
void Ipv4StaticRouting::NotifyRemoveAddress(uint32_t interface,
                                            Ipv4InterfaceAddress address) {
  NS_LOG_FUNCTION(this << interface << " " << address.GetLocal());
  if (!m_ipv4->IsUp(interface)) {
    return;
...
void Ipv4StaticRouting::SetIpv4(Ptr<Ipv4> ipv4) {
