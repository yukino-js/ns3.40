...
UdpSocketImpl::UdpSocketImpl()
    : m_endPoint(nullptr), m_endPoint6(nullptr), m_node(nullptr),
      m_udp(nullptr), m_errno(ERROR_NOTERROR), m_shutdownSend(false),
      m_shutdownRecv(false), m_connected(false), m_rxAvailable(0) {
...
void UdpSocketImpl::SetNode(Ptr<Node> node) {
  NS_LOG_FUNCTION(this << node);
  m_node = node;
}

void UdpSocketImpl::SetUdp(Ptr<UdpL4Protocol> udp) {
  NS_LOG_FUNCTION(this << udp);
  m_udp = udp;
}

Socket::SocketErrno UdpSocketImpl::GetErrno() const {
  NS_LOG_FUNCTION(this);
  return m_errno;
}

Socket::SocketType UdpSocketImpl::GetSocketType() const {
  return NS3_SOCK_DGRAM;
}

Ptr<Node> UdpSocketImpl::GetNode() const {
  NS_LOG_FUNCTION(this);
  return m_node;
}

void UdpSocketImpl::Destroy() {
  NS_LOG_FUNCTION(this);
  if (m_udp) {
...
int UdpSocketImpl::Bind(const Address &address) {
...
int UdpSocketImpl::ShutdownSend() {
  NS_LOG_FUNCTION(this);
  m_shutdownSend = true;
  return 0;
}

int UdpSocketImpl::ShutdownRecv() {
  NS_LOG_FUNCTION(this);
  m_shutdownRecv = true;
  if (m_endPoint) {
...
int UdpSocketImpl::Connect(const Address &address) {
  NS_LOG_FUNCTION(this << address);
  if (InetSocketAddress::IsMatchingType(address)) {
    InetSocketAddress transport = InetSocketAddress::ConvertFrom(address);
...
int UdpSocketImpl::DoSend(Ptr<Packet> p) {
  ...
  if (Ipv4Address::IsMatchingType(m_defaultAddress)) {
    return DoSendTo(p, Ipv4Address::ConvertFrom(m_defaultAddress),
                    m_defaultPort, GetIpTos());
  } else if (Ipv6Address::IsMatchingType(m_defaultAddress)) {
    return DoSendTo(p, Ipv6Address::ConvertFrom(m_defaultAddress),
                    m_defaultPort);
  }
...
int UdpSocketImpl::DoSendTo(Ptr<Packet> p, Ipv4Address dest, uint16_t port,
  ...
  if (m_ipMulticastTtl != 0 && dest.IsMulticast()) {
    ...
    p->AddPacketTag(tag);
  } else if (IsManualIpTtl() && GetIpTtl() != 0 && !dest.IsMulticast() &&
             !dest.IsBroadcast()) {
    SocketIpTtlTag tag;
    tag.SetTtl(GetIpTtl());
    p->AddPacketTag(tag);
...
int UdpSocketImpl::DoSendTo(Ptr<Packet> p, Ipv6Address dest, uint16_t port) {
  ...
  if (m_ipMulticastTtl != 0 && dest.IsMulticast()) {
    ...
    p->AddPacketTag(tag);
  } else if (IsManualIpv6HopLimit() && GetIpv6HopLimit() != 0 &&
             !dest.IsMulticast()) {
    SocketIpv6HopLimitTag tag;
...
int UdpSocketImpl::SendTo(Ptr<Packet> p, uint32_t flags,
...
Ptr<Packet> UdpSocketImpl::Recv(uint32_t maxSize, uint32_t flags) {
...
Ptr<Packet> UdpSocketImpl::RecvFrom(uint32_t maxSize, uint32_t flags,
                                    Address &fromAddress) {
  NS_LOG_FUNCTION(this << maxSize << flags);
...
int UdpSocketImpl::GetSockName(Address &address) const {
  NS_LOG_FUNCTION(this << address);
  if (m_endPoint != nullptr) {
...
int UdpSocketImpl::GetPeerName(Address &address) const {
  NS_LOG_FUNCTION(this << address);
...
void UdpSocketImpl::BindToNetDevice(Ptr<NetDevice> netdevice) {
...
void UdpSocketImpl::ForwardUp(Ptr<Packet> packet, Ipv4Header header,
                              uint16_t port,
...
void UdpSocketImpl::ForwardUp6(Ptr<Packet> packet, Ipv6Header header,
                               uint16_t port,
...
void UdpSocketImpl::ForwardIcmp(Ipv4Address icmpSource, uint8_t icmpTtl,
...
void UdpSocketImpl::ForwardIcmp6(Ipv6Address icmpSource, uint8_t icmpTtl,
                                 uint8_t icmpType, uint8_t icmpCode,
                                 uint32_t icmpInfo) {
  NS_LOG_FUNCTION(this << icmpSource << (uint32_t)icmpTtl << (uint32_t)icmpType
                       << (uint32_t)icmpCode << icmpInfo);
...
void UdpSocketImpl::SetRcvBufSize(uint32_t size) { m_rcvBufSize = size; }

uint32_t UdpSocketImpl::GetRcvBufSize() const { return m_rcvBufSize; }

void UdpSocketImpl::SetIpMulticastTtl(uint8_t ipTtl) {
  m_ipMulticastTtl = ipTtl;
}

uint8_t UdpSocketImpl::GetIpMulticastTtl() const { return m_ipMulticastTtl; }

void UdpSocketImpl::SetIpMulticastIf(int32_t ipIf) { m_ipMulticastIf = ipIf; }

int32_t UdpSocketImpl::GetIpMulticastIf() const { return m_ipMulticastIf; }

void UdpSocketImpl::SetIpMulticastLoop(bool loop) { m_ipMulticastLoop = loop; }

bool UdpSocketImpl::GetIpMulticastLoop() const { return m_ipMulticastLoop; }

void UdpSocketImpl::SetMtuDiscover(bool discover) { m_mtuDiscover = discover; }

bool UdpSocketImpl::GetMtuDiscover() const { return m_mtuDiscover; }

bool UdpSocketImpl::SetAllowBroadcast(bool allowBroadcast) {
  m_allowBroadcast = allowBroadcast;
  return true;
}

bool UdpSocketImpl::GetAllowBroadcast() const { return m_allowBroadcast; }

void UdpSocketImpl::Ipv6JoinGroup(Ipv6Address address,
