...
TypeId TcpHtcp::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TcpHtcp")
          ...
          .AddAttribute("DefaultBackoff", "The default AIMD backoff factor",
                        DoubleValue(0.5),
                        MakeDoubleAccessor(&TcpHtcp::m_defaultBackoff),
                        MakeDoubleChecker<double>(0, 1))
          .AddAttribute("ThroughputRatio", "Threshold value for updating beta",
                        DoubleValue(0.2),
...
std::string TcpHtcp::GetName() const { return "TcpHtcp"; }

TcpHtcp::TcpHtcp()
    : TcpNewReno(), m_alpha(0), m_beta(0), m_delta(0), m_lastCon(0),
      m_minRtt(Time::Max()), m_maxRtt(Time::Min()), m_throughput(0),
      m_lastThroughput(0), m_dataSent(0) {
...
TcpHtcp::TcpHtcp(const TcpHtcp &sock)
    : TcpNewReno(sock), m_alpha(sock.m_alpha), m_beta(sock.m_beta),
      ...
      m_throughputRatio(sock.m_throughputRatio), m_delta(sock.m_delta),
      m_deltaL(sock.m_deltaL), m_lastCon(sock.m_lastCon),
      m_minRtt(sock.m_minRtt), m_maxRtt(sock.m_maxRtt),
      m_throughput(sock.m_throughput), m_lastThroughput(sock.m_lastThroughput),
      m_dataSent(sock.m_dataSent) {
  NS_LOG_FUNCTION(this);
}

TcpHtcp::~TcpHtcp() { NS_LOG_FUNCTION(this); }
...
void TcpHtcp::CongestionAvoidance(Ptr<TcpSocketState> tcb,
  ...
  if (segmentsAcked > 0) {
    double adder = static_cast<double>(
        ((tcb->m_segmentSize * tcb->m_segmentSize) + (tcb->m_cWnd * m_alpha)) /
        tcb->m_cWnd);
