...
TypeId Ipv4ListRouting::GetTypeId() {
...
Ipv4ListRouting::Ipv4ListRouting() : m_ipv4(nullptr) { NS_LOG_FUNCTION(this); }

Ipv4ListRouting::~Ipv4ListRouting() { NS_LOG_FUNCTION(this); }

void Ipv4ListRouting::DoDispose() {
...
void Ipv4ListRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream,
...
void Ipv4ListRouting::DoInitialize() {
  NS_LOG_FUNCTION(this);
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
...
Ptr<Ipv4Route> Ipv4ListRouting::RouteOutput(Ptr<Packet> p,
                                            const Ipv4Header &header,
                                            Ptr<NetDevice> oif,
                                            Socket::SocketErrno &sockerr) {
  NS_LOG_FUNCTION(this << p << header.GetDestination() << header.GetSource()
                       << oif << sockerr);
  Ptr<Ipv4Route> route;
...
bool Ipv4ListRouting::RouteInput(Ptr<const Packet> p, const Ipv4Header &header,
  ...
  if (retVal) {
    NS_LOG_LOGIC("Address " << header.GetDestination()
                            << " is a match for local delivery");
  ...
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
    if ((*rprotoIter)
            .second->RouteInput(p, header, idev, ucb, mcb, downstreamLcb,
                                ecb)) {
...
void Ipv4ListRouting::NotifyInterfaceUp(uint32_t interface) {
...
void Ipv4ListRouting::NotifyInterfaceDown(uint32_t interface) {
  NS_LOG_FUNCTION(this << interface);
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
...
void Ipv4ListRouting::NotifyAddAddress(uint32_t interface,
...
void Ipv4ListRouting::NotifyRemoveAddress(uint32_t interface,
                                          Ipv4InterfaceAddress address) {
  NS_LOG_FUNCTION(this << interface << address);
  for (auto rprotoIter = m_routingProtocols.begin();
       rprotoIter != m_routingProtocols.end(); rprotoIter++) {
...
void Ipv4ListRouting::SetIpv4(Ptr<Ipv4> ipv4) {
...
void Ipv4ListRouting::AddRoutingProtocol(
    Ptr<Ipv4RoutingProtocol> routingProtocol, int16_t priority) {
...
uint32_t Ipv4ListRouting::GetNRoutingProtocols() const {
  NS_LOG_FUNCTION(this);
...
Ipv4ListRouting::GetRoutingProtocol(uint32_t index, int16_t &priority) const {
  ...
  if (index >= m_routingProtocols.size()) {
    NS_FATAL_ERROR("Ipv4ListRouting::GetRoutingProtocol():  index "
                   << index << " out of range");
  }
  uint32_t i = 0;
  for (auto rprotoIter = m_routingProtocols.begin();
...
bool Ipv4ListRouting::Compare(const Ipv4RoutingProtocolEntry &a,
