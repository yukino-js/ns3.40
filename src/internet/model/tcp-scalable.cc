...
TcpScalable::TcpScalable()
    : TcpNewReno(), m_ackCnt(0), m_aiFactor(50), m_mdFactor(0.125) {
  NS_LOG_FUNCTION(this);
}

TcpScalable::TcpScalable(const TcpScalable &sock)
    : TcpNewReno(sock), m_ackCnt(sock.m_ackCnt), m_aiFactor(sock.m_aiFactor),
      m_mdFactor(sock.m_mdFactor) {
  NS_LOG_FUNCTION(this);
}

TcpScalable::~TcpScalable() { NS_LOG_FUNCTION(this); }

Ptr<TcpCongestionOps> TcpScalable::Fork() {
  return CopyObject<TcpScalable>(this);
}

void TcpScalable::CongestionAvoidance(Ptr<TcpSocketState> tcb,
...
std::string TcpScalable::GetName() const { return "TcpScalable"; }

uint32_t TcpScalable::GetSsThresh(Ptr<const TcpSocketState> tcb,
  ...
  NS_LOG_DEBUG("Calculated b(w) = " << b << " resulting (in segment) ssThresh="
                                    << ssThresh);
