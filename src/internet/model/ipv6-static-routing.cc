...
Ipv6StaticRouting::~Ipv6StaticRouting() { NS_LOG_FUNCTION(this); }
...
void Ipv6StaticRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream,
...
void Ipv6StaticRouting::AddHostRouteTo(Ipv6Address dst, Ipv6Address nextHop,
                                       uint32_t interface,
  ...
  AddNetworkRouteTo(dst, Ipv6Prefix::GetOnes(), nextHop, interface, prefixToUse,
                    metric);
}
...
void Ipv6StaticRouting::AddNetworkRouteTo(Ipv6Address network,
...
void Ipv6StaticRouting::AddNetworkRouteTo(
...
void Ipv6StaticRouting::SetDefaultRoute(Ipv6Address nextHop, uint32_t interface,
                                        Ipv6Address prefixToUse,
                                        uint32_t metric) {
...
void Ipv6StaticRouting::AddMulticastRoute(
  ...
  *route = Ipv6MulticastRoutingTableEntry::CreateMulticastRoute(
...
void Ipv6StaticRouting::SetDefaultMulticastRoute(uint32_t outputInterface) {
  ...
  *route = Ipv6RoutingTableEntry::CreateNetworkRouteTo(network, networkMask,
                                                       outputInterface);
...
bool Ipv6StaticRouting::RemoveMulticastRoute(Ipv6Address origin,
                                             Ipv6Address group,
...
bool Ipv6StaticRouting::HasNetworkDest(Ipv6Address network,
  ...
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    ...
    if (prefix.IsMatch(network, entry) &&
        rtentry->GetInterface() == interfaceIndex) {
      return true;
    }
  }
...
bool Ipv6StaticRouting::LookupRoute(const Ipv6RoutingTableEntry &route,
                                    uint32_t metric) {
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    Ipv6RoutingTableEntry *rtentry = j->first;
...
Ptr<Ipv6Route> Ipv6StaticRouting::LookupStatic(Ipv6Address dst,
...
Ptr<Ipv6MulticastRoute> Ipv6StaticRouting::LookupStatic(Ipv6Address origin,
                                                        Ipv6Address group,
                                                        uint32_t interface) {
...
void Ipv6StaticRouting::RemoveRoute(Ipv6Address network, Ipv6Prefix prefix,
                                    uint32_t ifIndex, Ipv6Address prefixToUse) {
...
Ptr<Ipv6Route> Ipv6StaticRouting::RouteOutput(Ptr<Packet> p,
...
bool Ipv6StaticRouting::RouteInput(
    Ptr<const Packet> p, const Ipv6Header &header, Ptr<const NetDevice> idev,
  ...
  if (dst.IsMulticast()) {
    ...
    Ptr<Ipv6MulticastRoute> mrtentry =
        LookupStatic(header.GetSource(), header.GetDestination(),
...
void Ipv6StaticRouting::NotifyAddAddress(uint32_t interface,
...
void Ipv6StaticRouting::NotifyRemoveAddress(uint32_t interface,
                                            Ipv6InterfaceAddress address) {
  if (!m_ipv6->IsUp(interface)) {
    return;
  }
...
void Ipv6StaticRouting::NotifyAddRoute(Ipv6Address dst, Ipv6Prefix mask,
...
void Ipv6StaticRouting::NotifyRemoveRoute(Ipv6Address dst, Ipv6Prefix mask,
                                          Ipv6Address nextHop,
                                          uint32_t interface,
                                          Ipv6Address prefixToUse) {
  ...
  if (dst != Ipv6Address::GetZero()) {
    for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end();) {
      ...
      if (dst == entry && prefix == mask &&
          rtentry->GetInterface() == interface) {
        delete j->first;
        j = m_networkRoutes.erase(j);
      } else {
        ++j;
