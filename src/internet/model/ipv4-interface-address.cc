...
Ipv4InterfaceAddress::Ipv4InterfaceAddress(const Ipv4InterfaceAddress &o)
    : m_local(o.m_local), m_mask(o.m_mask), m_broadcast(o.m_broadcast),
      m_scope(o.m_scope), m_secondary(o.m_secondary) {
  NS_LOG_FUNCTION(this << &o);
}

void Ipv4InterfaceAddress::SetLocal(Ipv4Address local) {
  NS_LOG_FUNCTION(this << local);
  m_local = local;
}

void Ipv4InterfaceAddress::SetAddress(Ipv4Address address) {
  SetLocal(address);
}

Ipv4Address Ipv4InterfaceAddress::GetLocal() const {
  NS_LOG_FUNCTION(this);
  return m_local;
}

Ipv4Address Ipv4InterfaceAddress::GetAddress() const { return GetLocal(); }

void Ipv4InterfaceAddress::SetMask(Ipv4Mask mask) {
  NS_LOG_FUNCTION(this << mask);
  m_mask = mask;
}

Ipv4Mask Ipv4InterfaceAddress::GetMask() const {
  NS_LOG_FUNCTION(this);
  return m_mask;
}

void Ipv4InterfaceAddress::SetBroadcast(Ipv4Address broadcast) {
  NS_LOG_FUNCTION(this << broadcast);
  m_broadcast = broadcast;
}

Ipv4Address Ipv4InterfaceAddress::GetBroadcast() const {
  NS_LOG_FUNCTION(this);
  return m_broadcast;
}

void Ipv4InterfaceAddress::SetScope(
...
bool Ipv4InterfaceAddress::IsSecondary() const {
  NS_LOG_FUNCTION(this);
  return m_secondary;
}

void Ipv4InterfaceAddress::SetSecondary() {
  NS_LOG_FUNCTION(this);
  m_secondary = true;
}

void Ipv4InterfaceAddress::SetPrimary() {
  NS_LOG_FUNCTION(this);
  m_secondary = false;
}
