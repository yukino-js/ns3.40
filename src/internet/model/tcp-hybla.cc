...
TcpHybla::~TcpHybla() { NS_LOG_FUNCTION(this); }

void TcpHybla::RecalcParam(const Ptr<TcpSocketState> &tcb) {
  NS_LOG_FUNCTION(this);

  m_rho = std::max(
      (double)tcb->m_minRtt.GetMilliSeconds() / m_rRtt.GetMilliSeconds(), 1.0);

  NS_ASSERT(m_rho > 0.0);
  NS_LOG_DEBUG("Calculated rho=" << m_rho);
}

void TcpHybla::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
                         const Time &rtt) {
...
uint32_t TcpHybla::SlowStart(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) {
  ...
  if (segmentsAcked >= 1) {
    ...
    NS_LOG_INFO("In SlowStart, updated to cwnd "
...
void TcpHybla::CongestionAvoidance(Ptr<TcpSocketState> tcb,
...
Ptr<TcpCongestionOps> TcpHybla::Fork() { return CopyObject<TcpHybla>(this); }

std::string TcpHybla::GetName() const { return "TcpHybla"; }
