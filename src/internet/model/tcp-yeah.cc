...
TcpYeah::TcpYeah()
    : TcpNewReno(), m_alpha(80), m_gamma(1), m_delta(3), m_epsilon(1), m_phy(8),
      m_rho(16), m_zeta(50), m_stcpAiFactor(100), m_stcp(nullptr),
      m_baseRtt(Time::Max()), m_minRtt(Time::Max()), m_cntRtt(0),
      m_doingYeahNow(true), m_begSndNxt(0), m_lastQ(0), m_doingRenoNow(0),
      m_renoCount(2), m_fastCount(0) {
...
TcpYeah::TcpYeah(const TcpYeah &sock)
    : TcpNewReno(sock), m_alpha(sock.m_alpha), m_gamma(sock.m_gamma),
      m_delta(sock.m_delta), m_epsilon(sock.m_epsilon), m_phy(sock.m_phy),
      m_rho(sock.m_rho), m_zeta(sock.m_zeta),
      m_stcpAiFactor(sock.m_stcpAiFactor), m_baseRtt(sock.m_baseRtt),
      m_minRtt(sock.m_minRtt), m_cntRtt(sock.m_cntRtt),
      m_doingYeahNow(sock.m_doingYeahNow), m_begSndNxt(sock.m_begSndNxt),
      m_lastQ(sock.m_lastQ), m_doingRenoNow(sock.m_doingRenoNow),
      m_renoCount(sock.m_renoCount), m_fastCount(sock.m_fastCount) {
  NS_LOG_FUNCTION(this);
  m_stcp = CopyObject(sock.m_stcp);
}

TcpYeah::~TcpYeah() { NS_LOG_FUNCTION(this); }

Ptr<TcpCongestionOps> TcpYeah::Fork() { return CopyObject<TcpYeah>(this); }

void TcpYeah::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
                        const Time &rtt) {
...
void TcpYeah::EnableYeah(const SequenceNumber32 &nextTxSequence) {
...
void TcpYeah::DisableYeah() {
  NS_LOG_FUNCTION(this);

  m_doingYeahNow = false;
}

void TcpYeah::CongestionStateSet(
...
void TcpYeah::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) {
...
std::string TcpYeah::GetName() const { return "TcpYeah"; }

uint32_t TcpYeah::GetSsThresh(Ptr<const TcpSocketState> tcb,
