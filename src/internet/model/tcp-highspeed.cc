...
TcpHighSpeed::TcpHighSpeed(const TcpHighSpeed &sock)
    : TcpNewReno(sock), m_ackCnt(sock.m_ackCnt) {
  NS_LOG_FUNCTION(this);
}

TcpHighSpeed::~TcpHighSpeed() { NS_LOG_FUNCTION(this); }

Ptr<TcpCongestionOps> TcpHighSpeed::Fork() {
...
uint32_t TcpHighSpeed::TableLookupA(uint32_t w) {
  if (w <= 38) {
    return 1;
  } else if (w <= 118) {
    return 2;
