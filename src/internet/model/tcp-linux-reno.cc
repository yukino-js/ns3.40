...
TcpLinuxReno::TcpLinuxReno() : TcpCongestionOps() { NS_LOG_FUNCTION(this); }
...
uint32_t TcpLinuxReno::SlowStart(Ptr<TcpSocketState> tcb,
...
void TcpLinuxReno::CongestionAvoidance(Ptr<TcpSocketState> tcb,
                                       uint32_t segmentsAcked) {
  NS_LOG_FUNCTION(this << tcb << segmentsAcked);
  ...
  NS_LOG_DEBUG("w in segments " << w << " m_cWndCnt " << m_cWndCnt
  ...
  NS_LOG_DEBUG("At end of CongestionAvoidance(), m_cWnd: "
               << tcb->m_cWnd << " m_cWndCnt: " << m_cWndCnt);
}

void TcpLinuxReno::IncreaseWindow(Ptr<TcpSocketState> tcb,
                                  uint32_t segmentsAcked) {
  NS_LOG_FUNCTION(this << tcb << segmentsAcked);
...
std::string TcpLinuxReno::GetName() const { return "TcpLinuxReno"; }

uint32_t TcpLinuxReno::GetSsThresh(Ptr<const TcpSocketState> state,
...
Ptr<TcpCongestionOps> TcpLinuxReno::Fork() {
