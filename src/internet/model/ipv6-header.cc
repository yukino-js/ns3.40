...
Ipv6Header::Ipv6Header()
    : m_trafficClass(0), m_flowLabel(1), m_payloadLength(0), m_nextHeader(0),
      m_hopLimit(0) {
  SetSource(Ipv6Address("::"));
  SetDestination(Ipv6Address("::"));
}

void Ipv6Header::SetTrafficClass(uint8_t traffic) { m_trafficClass = traffic; }

uint8_t Ipv6Header::GetTrafficClass() const { return m_trafficClass; }

void Ipv6Header::SetFlowLabel(uint32_t flow) { m_flowLabel = flow; }

uint32_t Ipv6Header::GetFlowLabel() const { return m_flowLabel; }

void Ipv6Header::SetPayloadLength(uint16_t len) { m_payloadLength = len; }

uint16_t Ipv6Header::GetPayloadLength() const { return m_payloadLength; }

void Ipv6Header::SetNextHeader(uint8_t next) { m_nextHeader = next; }

uint8_t Ipv6Header::GetNextHeader() const { return m_nextHeader; }

void Ipv6Header::SetHopLimit(uint8_t limit) { m_hopLimit = limit; }

uint8_t Ipv6Header::GetHopLimit() const { return m_hopLimit; }

void Ipv6Header::SetSource(Ipv6Address src) { m_sourceAddress = src; }

Ipv6Address Ipv6Header::GetSource() const { return m_sourceAddress; }

void Ipv6Header::SetDestination(Ipv6Address dst) { m_destinationAddress = dst; }

Ipv6Address Ipv6Header::GetDestination() const { return m_destinationAddress; }
...
TypeId Ipv6Header::GetInstanceTypeId() const { return GetTypeId(); }
...
uint32_t Ipv6Header::GetSerializedSize() const { return 10 * 4; }
