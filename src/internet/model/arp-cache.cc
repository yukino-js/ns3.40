...
ArpCache::~ArpCache() { NS_LOG_FUNCTION(this); }

void ArpCache::DoDispose() {
  NS_LOG_FUNCTION(this);
  Flush();
  m_device = nullptr;
  m_interface = nullptr;
...
void ArpCache::SetDevice(Ptr<NetDevice> device, Ptr<Ipv4Interface> interface) {
  NS_LOG_FUNCTION(this << device << interface);
  m_device = device;
  m_interface = interface;
}

Ptr<NetDevice> ArpCache::GetDevice() const {
  NS_LOG_FUNCTION(this);
  return m_device;
}

Ptr<Ipv4Interface> ArpCache::GetInterface() const {
  NS_LOG_FUNCTION(this);
  return m_interface;
}

void ArpCache::SetAliveTimeout(Time aliveTimeout) {
  NS_LOG_FUNCTION(this << aliveTimeout);
  m_aliveTimeout = aliveTimeout;
}

void ArpCache::SetDeadTimeout(Time deadTimeout) {
...
Time ArpCache::GetAliveTimeout() const {
  NS_LOG_FUNCTION(this);
  return m_aliveTimeout;
}

Time ArpCache::GetDeadTimeout() const {
  NS_LOG_FUNCTION(this);
  return m_deadTimeout;
}

Time ArpCache::GetWaitReplyTimeout() const {
  NS_LOG_FUNCTION(this);
  return m_waitReplyTimeout;
}

void ArpCache::SetArpRequestCallback(
...
void ArpCache::StartWaitReplyTimer() {
  NS_LOG_FUNCTION(this);
  if (!m_waitReplyTimer.IsRunning()) {
    NS_LOG_LOGIC("Starting WaitReplyTimer at " << Simulator::Now() << " for "
                                               << m_waitReplyTimeout);
    m_waitReplyTimer = Simulator::Schedule(
        m_waitReplyTimeout, &ArpCache::HandleWaitReplyTimeout, this);
  }
}

void ArpCache::HandleWaitReplyTimeout() {
  NS_LOG_FUNCTION(this);
  ...
  for (auto i = m_arpCache.begin(); i != m_arpCache.end(); i++) {
    ...
    if (entry != nullptr && entry->IsWaitReply()) {
      if (entry->GetRetries() < m_maxRetries) {
        NS_LOG_LOGIC(
            "node=" << m_device->GetNode()->GetId() << ", ArpWaitTimeout for "
  ...
  if (restartWaitReplyTimer) {
    NS_LOG_LOGIC("Restarting WaitReplyTimer at "
...
void ArpCache::PrintArpCache(Ptr<OutputStreamWrapper> stream) {
...
void ArpCache::RemoveAutoGeneratedEntries() {
  NS_LOG_FUNCTION(this);
...
std::list<ArpCache::Entry *> ArpCache::LookupInverse(Address to) {
...
ArpCache::Entry *ArpCache::Lookup(Ipv4Address to) {
  NS_LOG_FUNCTION(this << to);
...
ArpCache::Entry *ArpCache::Add(Ipv4Address to) {
  NS_LOG_FUNCTION(this << to);
...
void ArpCache::Remove(ArpCache::Entry *entry) {
...
ArpCache::Entry::Entry(ArpCache *arp)
    : m_arp(arp), m_state(ALIVE), m_retries(0) {
  NS_LOG_FUNCTION(this << arp);
}

bool ArpCache::Entry::IsDead() {
  NS_LOG_FUNCTION(this);
  return (m_state == DEAD);
}

bool ArpCache::Entry::IsAlive() {
  NS_LOG_FUNCTION(this);
  return (m_state == ALIVE);
}

bool ArpCache::Entry::IsWaitReply() {
  NS_LOG_FUNCTION(this);
  return (m_state == WAIT_REPLY);
}

bool ArpCache::Entry::IsPermanent() {
  NS_LOG_FUNCTION(this);
  return (m_state == PERMANENT);
}

bool ArpCache::Entry::IsAutoGenerated() {
  NS_LOG_FUNCTION(this);
  return (m_state == STATIC_AUTOGENERATED);
}

void ArpCache::Entry::MarkDead() {
  NS_LOG_FUNCTION(this);
...
void ArpCache::Entry::MarkAlive(Address macAddress) {
...
void ArpCache::Entry::MarkPermanent() {
...
void ArpCache::Entry::MarkAutoGenerated() {
  NS_LOG_FUNCTION(this << m_macAddress);
  NS_ASSERT(!m_macAddress.IsInvalid());
...
bool ArpCache::Entry::UpdateWaitReply(Ipv4PayloadHeaderPair waiting) {
...
void ArpCache::Entry::MarkWaitReply(Ipv4PayloadHeaderPair waiting) {
  NS_LOG_FUNCTION(this << waiting.first);
...
Address ArpCache::Entry::GetMacAddress() const {
  NS_LOG_FUNCTION(this);
  return m_macAddress;
}

void ArpCache::Entry::SetMacAddress(Address macAddress) {
  NS_LOG_FUNCTION(this);
  m_macAddress = macAddress;
}

Ipv4Address ArpCache::Entry::GetIpv4Address() const {
  NS_LOG_FUNCTION(this);
  return m_ipv4Address;
}

void ArpCache::Entry::SetIpv4Address(Ipv4Address destination) {
  NS_LOG_FUNCTION(this << destination);
  m_ipv4Address = destination;
}

Time ArpCache::Entry::GetTimeout() const {
  NS_LOG_FUNCTION(this);
  switch (m_state) {
...
bool ArpCache::Entry::IsExpired() const {
  NS_LOG_FUNCTION(this);
...
ArpCache::Ipv4PayloadHeaderPair ArpCache::Entry::DequeuePending() {
  NS_LOG_FUNCTION(this);
  if (m_pending.empty()) {
    Ipv4Header h;
...
void ArpCache::Entry::ClearPendingPacket() {
  NS_LOG_FUNCTION(this);
  m_pending.clear();
}

void ArpCache::Entry::UpdateSeen() {
  NS_LOG_FUNCTION(this);
  m_lastSeen = Simulator::Now();
}

uint32_t ArpCache::Entry::GetRetries() const {
  NS_LOG_FUNCTION(this);
  return m_retries;
}

void ArpCache::Entry::IncrementRetries() {
  NS_LOG_FUNCTION(this);
  m_retries++;
  UpdateSeen();
}

void ArpCache::Entry::ClearRetries() {
  NS_LOG_FUNCTION(this);
  m_retries = 0;
}
