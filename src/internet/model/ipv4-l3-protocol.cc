...
TypeId Ipv4L3Protocol::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ipv4L3Protocol")
          ...
          .AddAttribute(
              ...
              MakeTimeAccessor(&Ipv4L3Protocol::m_expire), MakeTimeChecker())
          .AddAttribute(
              "PurgeExpiredPeriod",
...
Ipv4L3Protocol::~Ipv4L3Protocol() { NS_LOG_FUNCTION(this); }
...
Ptr<Socket> Ipv4L3Protocol::CreateRawSocket() {
  NS_LOG_FUNCTION(this);
...
Ptr<Ipv4RoutingProtocol> Ipv4L3Protocol::GetRoutingProtocol() const {
  NS_LOG_FUNCTION(this);
  return m_routingProtocol;
}

void Ipv4L3Protocol::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void Ipv4L3Protocol::SetupLoopback() {
  NS_LOG_FUNCTION(this);
...
uint32_t Ipv4L3Protocol::GetNInterfaces() const {
  NS_LOG_FUNCTION(this);
...
void Ipv4L3Protocol::Receive(Ptr<NetDevice> device, Ptr<const Packet> p,
  ...
  NS_ASSERT_MSG(m_routingProtocol,
                "Need a routing protocol object to process packets");
  if (!m_routingProtocol->RouteInput(packet, ipHeader, device, m_ucb, m_mcb,
                                     m_lcb, m_ecb)) {
...
Ptr<Icmpv4L4Protocol> Ipv4L3Protocol::GetIcmp() const {
  NS_LOG_FUNCTION(this);
  ...
  if (ad.IsBroadcast() || ad.IsMulticast()) {
    ...
    for (uint32_t ifaceIndex = 0; ifaceIndex < GetNInterfaces(); ifaceIndex++) {
      for (uint32_t j = 0; j < GetNAddresses(ifaceIndex); j++) {
        ...
        NS_LOG_LOGIC("Testing address " << ad
...
void Ipv4L3Protocol::DecreaseIdentification(Ipv4Address source,
...
Ipv4Header Ipv4L3Protocol::BuildHeader(Ipv4Address source,
                                       ...
                                       bool mayFragment) {
  NS_LOG_FUNCTION(this << source << destination << (uint16_t)protocol
                       << payloadSize << (uint16_t)ttl << (uint16_t)tos
                       << mayFragment);
  Ipv4Header ipHeader;
...
void Ipv4L3Protocol::SendRealOut(Ptr<Ipv4Route> route, Ptr<Packet> packet,
  ...
  NS_LOG_LOGIC("Send via NetDevice ifIndex "
               << outDev->GetIfIndex() << " ipv4InterfaceIndex " << interface);
  ...
  if (outInterface->IsUp()) {
    ...
    if (packet->GetSize() + ipHeader.GetSerializedSize() >
      ...
      DoFragmentation(packet, ipHeader, outInterface->GetDevice()->GetMtu(),
                      listFragments);
...
Ipv4Address Ipv4L3Protocol::SelectSourceAddress(
...
bool Ipv4L3Protocol::GetIpForward() const {
  NS_LOG_FUNCTION(this);
  return m_ipForward;
}
...
bool Ipv4L3Protocol::GetWeakEsModel() const {
  NS_LOG_FUNCTION(this);
...
void Ipv4L3Protocol::DoFragmentation(
  ...
  NS_ASSERT_MSG(
      (ipv4Header.GetSerializedSize() == 5 * 4),
  ...
  do {
    ...
    NS_LOG_LOGIC("Fragment creation - " << offset << ", "
...
bool Ipv4L3Protocol::ProcessFragment(Ptr<Packet> &packet, Ipv4Header &ipHeader,
  ...
  NS_LOG_LOGIC("Adding fragment - Size: " << packet->GetSize() << " - Offset: "
...
Ipv4L3Protocol::Fragments::Fragments() : m_moreFragment(false) {
  NS_LOG_FUNCTION(this);
}
...
void Ipv4L3Protocol::Fragments::SetTimeoutIter(FragmentsTimeoutsListI_t iter) {
  m_timeoutIter = iter;
}
...
void Ipv4L3Protocol::HandleFragmentsTimeout(FragmentKey_t key,
                                            Ipv4Header &ipHeader,
                                            uint32_t iif) {
...
bool Ipv4L3Protocol::UpdateDuplicate(Ptr<const Packet> p,
                                     const Ipv4Header &header) {
...
void Ipv4L3Protocol::RemoveDuplicates() {
  NS_LOG_FUNCTION(this);
...
Ipv4L3Protocol::FragmentsTimeoutsListI_t
Ipv4L3Protocol::SetTimeout(FragmentKey_t key, Ipv4Header ipHeader,
                           uint32_t iif) {
  ...
  if (m_timeoutEventList.empty()) {
    m_timeoutEvent = Simulator::Schedule(m_fragmentExpirationTimeout,
                                         &Ipv4L3Protocol::HandleTimeout, this);
  }
...
void Ipv4L3Protocol::HandleTimeout() {
  ...
  m_timeoutEvent =
      Simulator::Schedule(difference, &Ipv4L3Protocol::HandleTimeout, this);
}
