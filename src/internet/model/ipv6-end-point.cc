...
Ipv6EndPoint::Ipv6EndPoint(Ipv6Address addr, uint16_t port)
    : m_localAddr(addr), m_localPort(port), m_peerAddr(Ipv6Address::GetAny()),
      m_peerPort(0), m_rxEnabled(true) {}
...
Ipv6Address Ipv6EndPoint::GetLocalAddress() const { return m_localAddr; }

void Ipv6EndPoint::SetLocalAddress(Ipv6Address addr) { m_localAddr = addr; }

uint16_t Ipv6EndPoint::GetLocalPort() const { return m_localPort; }

void Ipv6EndPoint::SetLocalPort(uint16_t port) { m_localPort = port; }

Ipv6Address Ipv6EndPoint::GetPeerAddress() const { return m_peerAddr; }

uint16_t Ipv6EndPoint::GetPeerPort() const { return m_peerPort; }

void Ipv6EndPoint::BindToNetDevice(Ptr<NetDevice> netdevice) {
  m_boundnetdevice = netdevice;
}

Ptr<NetDevice> Ipv6EndPoint::GetBoundNetDevice() const {
...
void Ipv6EndPoint::SetRxCallback(
...
void Ipv6EndPoint::SetRxEnabled(bool enabled) { m_rxEnabled = enabled; }

bool Ipv6EndPoint::IsRxEnabled() const { return m_rxEnabled; }
