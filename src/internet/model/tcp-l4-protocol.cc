...
TcpL4Protocol::~TcpL4Protocol() { NS_LOG_FUNCTION(this); }
...
void TcpL4Protocol::NotifyNewAggregate() {
  ...
  if (!m_node) {
    if (node && (ipv4 || ipv6)) {
      this->SetNode(node);
      Ptr<TcpSocketFactoryImpl> tcpFactory =
          CreateObject<TcpSocketFactoryImpl>();
...
int TcpL4Protocol::GetProtocolNumber() const { return PROT_NUMBER; }
...
TcpL4Protocol::Receive(Ptr<Packet> packet, const Ipv4Header &incomingIpHeader,
  ...
  endPoints = m_endPoints->Lookup(
      ...
      incomingIpHeader.GetSource(), incomingTcpHeader.GetSourcePort(),
      incomingInterface);

  if (endPoints.empty()) {
    if (this->GetObject<Ipv6L3Protocol>()) {
      NS_LOG_LOGIC("  No Ipv4 endpoints matched on TcpL4Protocol, trying Ipv6 "
                   << this);
      ...
      dst =
          Ipv6Address::MakeIpv4MappedAddress(incomingIpHeader.GetDestination());
  ...
  (*endPoints.begin())
      ->ForwardUp(packet, incomingIpHeader, incomingTcpHeader.GetSourcePort(),
                  incomingInterface);
...
void TcpL4Protocol::SendPacketV4(Ptr<Packet> packet, const TcpHeader &outgoing,
  ...
  NS_LOG_LOGIC("TcpL4Protocol "
               << this << " sending seq " << outgoing.GetSequenceNumber()
...
void TcpL4Protocol::SendPacketV6(Ptr<Packet> packet, const TcpHeader &outgoing,
  ...
  if (ipv6) {
    ...
    if (ipv6->GetRoutingProtocol()) {
      route =
          ipv6->GetRoutingProtocol()->RouteOutput(packet, header, oif, errno_);
    } else {
...
void TcpL4Protocol::SendPacket(Ptr<Packet> pkt, const TcpHeader &outgoing,
