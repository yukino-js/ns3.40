  ...
  static TypeId tid =
      TypeId("ns3::SixLowPanNetDevice")
          ...
          .AddAttribute("EtherType",
                        ...
                        MakeUintegerChecker<uint16_t>())
          .AddAttribute("UseMeshUnder", "Use a mesh-under routing protocol.",
                        BooleanValue(false),
...
SixLowPanNetDevice::SixLowPanNetDevice()
    : m_node(nullptr), m_netDevice(nullptr), m_ifIndex(0) {
...
void SixLowPanNetDevice::SetNetDevice(Ptr<NetDevice> device) {
  ...
  m_node->RegisterProtocolHandler(
      MakeCallback(&SixLowPanNetDevice::ReceiveFromDevice, this), protocolType,
      device, false);
}
...
void SixLowPanNetDevice::ReceiveFromDevice(
    Ptr<NetDevice> incomingPort, Ptr<const Packet> packet, uint16_t protocol,
  ...
  NS_LOG_DEBUG("Dispatches: " << int(dispatchRawVal) << " - "
                              << int(dispatchVal));
  ...
  if (hasMesh) {
    ...
    NS_ABORT_MSG_IF(!Mac16Address::IsMatchingType(meshHdr.GetFinalDst()),
                    ...
                    "final destinations: "
                        << meshHdr.GetFinalDst());
    NS_ABORT_MSG_IF(
        !Mac48Address::IsMatchingType(m_netDevice->GetAddress()),
    ...
    if (meshHdr.GetFinalDst() != Get16MacFrom48Mac(m_netDevice->GetAddress()) ||
      ...
      } else if (meshHdr.GetOriginator() ==
                 Get16MacFrom48Mac(m_netDevice->GetAddress())) {
  ...
  case SixLowPanDispatch::LOWPAN_IPHC:
    ...
    if (DecompressLowPanIphc(copyPkt, realSrc, realDst)) {
      m_dropTrace(DROP_SATETFUL_DECOMPRESSION_PROBLEM, copyPkt, this,
                  GetIfIndex());
    } else {
      isPktDecompressed = true;
    }
  ...
  if (!m_promiscRxCallback.IsNull()) {
    m_promiscRxCallback(this, copyPkt, Ipv6L3Protocol::PROT_NUMBER, realSrc,
                        realDst, packetType);
  }
...
Ptr<Channel> SixLowPanNetDevice::GetChannel() const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT_MSG(m_netDevice, "Sixlowpan: can't find any lower-layer protocol "
                                 << m_netDevice);
...
bool SixLowPanNetDevice::DoSend(Ptr<Packet> packet, const Address &src,
  ...
  if (m_useIphc) {
    ...
    origHdrSize +=
        CompressLowPanIphc(packet, m_netDevice->GetAddress(), destination);
  } else {
    NS_LOG_LOGIC("Compressing packet using HC1");
    origHdrSize +=
        CompressLowPanHc1(packet, m_netDevice->GetAddress(), destination);
  }
  ...
  if (pktSize > m_netDevice->GetMtu()) {
    ...
    if (doSendFrom) {
      NS_LOG_DEBUG("SixLowPanNetDevice::SendFrom " << m_node->GetId() << " "
                                                   << *packet);
      ret = m_netDevice->SendFrom(packet, src, destination, protocolNumber);
    } else {
      NS_LOG_DEBUG("SixLowPanNetDevice::Send " << m_node->GetId() << " "
                                               << *packet);
...
uint32_t SixLowPanNetDevice::CompressLowPanHc1(Ptr<Packet> packet,
  ...
  if (packet->PeekHeader(ipHeader) != 0) {
    ...
    Ipv6Address mySrcAddr =
        Ipv6Address::MakeAutoconfiguredLinkLocalAddress(src);

    NS_LOG_LOGIC("Checking source compression: " << mySrcAddr << " - "
                                                 << srcAddr);
    ...
    Ipv6Address myDstAddr =
        Ipv6Address::MakeAutoconfiguredLinkLocalAddress(dst);

    NS_LOG_LOGIC("Checking destination compression: " << myDstAddr << " - "
                                                      << dstAddr);
...
void SixLowPanNetDevice::DecompressLowPanHc1(Ptr<Packet> packet,
  ...
  case SixLowPanHc1::HC1_PIIC:
    ...
    ipHeader.SetSource(
        Ipv6Address::MakeAutoconfiguredAddress(src, Ipv6Address(address)));
    break;
  ...
  case SixLowPanHc1::HC1_PIIC:
    ...
    ipHeader.SetDestination(
        Ipv6Address::MakeAutoconfiguredAddress(dst, Ipv6Address(address)));
    break;
  case SixLowPanHc1::HC1_PCII:
    interface = encoding.GetDstInterface();
    address[0] = 0xfe;
...
uint32_t SixLowPanNetDevice::CompressLowPanIphc(Ptr<Packet> packet,
                                                const Address &src,
                                                const Address &dst) {
  NS_LOG_FUNCTION(this << *packet << src << dst);
  ...
  if (packet->PeekHeader(ipHeader) != 0) {
      ...
      if (!iphcHeader.GetM()) {
        ...
        if (FindUnicastCompressionContext(dstAddr, dstContextId)) {
          ...
          if (Ipv6Address::MakeAutoconfiguredAddress(
            ...
            Ipv6Address cleanedAddr = CleanPrefix(
                dstAddr, m_contextTable[dstContextId].contextPrefix);
    ...
    NS_LOG_DEBUG("IPHC Compression - IPHC header size = "
                 << iphcHeader.GetSerializedSize());
...
bool SixLowPanNetDevice::CanCompressLowPanNhc(uint8_t nextHeader) {
  bool ret = false;
...
bool SixLowPanNetDevice::DecompressLowPanIphc(Ptr<Packet> packet,
                                              const Address &src,
                                              const Address &dst) {
  NS_LOG_FUNCTION(this << *packet << src << dst);
  ...
  if (encoding.GetSac()) {
    ...
    if (encoding.GetSam() == SixLowPanIphc::HC_INLINE) {
      ...
      if (m_contextTable.find(contextId) == m_contextTable.end()) {
        NS_LOG_LOGIC("Unknown Source compression context ("
                     << +contextId << "), dropping packet");
        return true;
      }
      if (m_contextTable[contextId].validLifetime < Simulator::Now()) {
        NS_LOG_LOGIC("Expired Source compression context ("
                     << +contextId << "), dropping packet");
        return true;
      }
  ...
  if (encoding.GetDac()) {
    ...
    if (m_contextTable.find(contextId) == m_contextTable.end()) {
      NS_LOG_LOGIC("Unknown Destination compression context ("
                   << +contextId << "), dropping packet");
      return true;
    }
    if (m_contextTable[contextId].validLifetime < Simulator::Now()) {
      NS_LOG_LOGIC("Expired Destination compression context ("
                   << +contextId << "), dropping packet");
      return true;
    }
  ...
  if (encoding.GetNh()) {
    ...
    if (dispatchVal == SixLowPanDispatch::LOWPAN_UDPNHC) {
      ...
      DecompressLowPanUdpNhc(packet, ipHeader.GetSource(),
...
uint32_t SixLowPanNetDevice::CompressLowPanNhc(Ptr<Packet> packet,
                                               uint8_t headerType,
                                               const Address &src,
                                               const Address &dst) {
  ...
  NS_LOG_DEBUG(
      "NHC Compression - NHC header size = " << nhcHeader.GetSerializedSize());
...
std::pair<uint8_t, bool> SixLowPanNetDevice::DecompressLowPanNhc(
    Ptr<Packet> packet, const Address &src, const Address &dst,
  ...
  case SixLowPanNhcExtension::EID_HOPBYHOP_OPTIONS_H:
    ...
    if (encoding.GetNh()) {
      ...
      if (dispatchVal == SixLowPanDispatch::LOWPAN_UDPNHC) {
        ...
        blobData[0] =
            DecompressLowPanNhc(packet, src, dst, srcAddress, dstAddress).first;
      }
    } else {
  ...
  case SixLowPanNhcExtension::EID_IPv6_H:
    ...
    if (DecompressLowPanIphc(packet, src, dst)) {
      m_dropTrace(DROP_SATETFUL_DECOMPRESSION_PROBLEM, packet, this,
                  GetIfIndex());
...
uint32_t SixLowPanNetDevice::CompressLowPanUdpNhc(Ptr<Packet> packet,
                                                  bool omitChecksum) {
  ...
  NS_ASSERT_MSG(packet->PeekHeader(udpHeader) != 0,
  ...
  NS_LOG_DEBUG("UDP_NHC Compression - UDP_NHC header size = "
...
void SixLowPanNetDevice::DecompressLowPanUdpNhc(Ptr<Packet> packet,
                                                Ipv6Address saddr,
                                                Ipv6Address daddr) {
  NS_LOG_FUNCTION(this << *packet);
...
void SixLowPanNetDevice::DoFragmentation(
    Ptr<Packet> packet, uint32_t origPacketSize, uint32_t origHdrSize,
  ...
  do {
    ...
    if (size > 0) {
      ...
      NS_LOG_LOGIC("Fragment created - " << offset << ", "
                                         << fragment->GetSize());
...
bool SixLowPanNetDevice::ProcessFragment(Ptr<Packet> &packet,
  ...
  if (isFirst) {
    ...
    NS_LOG_DEBUG("Dispatches: " << int(dispatchRawValFrag1) << " - "
  ...
  if (it == m_fragments.end()) {
    ...
    if (m_fragmentReassemblyListSize &&
      ...
      std::list<Ptr<Packet>> storedFragments =
          m_fragments[oldestKey]->GetFragments();
      for (auto fragIter = storedFragments.begin();
  ...
  if (fragments->IsEntire()) {
    ...
    NS_LOG_LOGIC("Rebuilt packet. Size " << packet->GetSize() << " - "
                                         << *packet);
...
SixLowPanNetDevice::Fragments::~Fragments() { NS_LOG_FUNCTION(this); }
...
std::list<Ptr<Packet>> SixLowPanNetDevice::Fragments::GetFragments() const {
...
void SixLowPanNetDevice::Fragments::SetTimeoutIter(
    FragmentsTimeoutsListI_t iter) {
  m_timeoutIter = iter;
}
...
void SixLowPanNetDevice::HandleFragmentsTimeout(FragmentKey_t key,
                                                uint32_t iif) {
  NS_LOG_FUNCTION(this);
...
SixLowPanNetDevice::SetTimeout(FragmentKey_t key, uint32_t iif) {
  if (m_timeoutEventList.empty()) {
    m_timeoutEvent = Simulator::Schedule(
        m_fragmentExpirationTimeout, &SixLowPanNetDevice::HandleTimeout, this);
  }
  m_timeoutEventList.emplace_back(
      Simulator::Now() + m_fragmentExpirationTimeout, key, iif);
...
void SixLowPanNetDevice::HandleTimeout() {
  ...
  while (!m_timeoutEventList.empty() &&
         std::get<0>(*m_timeoutEventList.begin()) == now) {
  ...
  m_timeoutEvent =
      Simulator::Schedule(difference, &SixLowPanNetDevice::HandleTimeout, this);
}

void SixLowPanNetDevice::AddContext(uint8_t contextId, Ipv6Prefix contextPrefix,
                                    bool compressionAllowed,
                                    Time validLifetime) {
  NS_LOG_FUNCTION(
      this << +contextId << Ipv6Address::GetOnes().CombinePrefix(contextPrefix)
  ...
  if (validLifetime == Time(0)) {
    NS_LOG_LOGIC("Context (" << +contextId
...
bool SixLowPanNetDevice::GetContext(uint8_t contextId,
...
bool SixLowPanNetDevice::FindUnicastCompressionContext(Ipv6Address address,
                                                       uint8_t &contextId) {
  NS_LOG_FUNCTION(this << address);
  ...
  for (const auto &iter : m_contextTable) {
    ...
    if (context.compressionAllowed &&
        context.validLifetime > Simulator::Now()) {
      if (address.HasPrefix(context.contextPrefix)) {
        NS_LOG_LOGIC("Found context " << +contextId << " "
                                      << Ipv6Address::GetOnes().CombinePrefix(
...
bool SixLowPanNetDevice::FindMulticastCompressionContext(Ipv6Address address,
                                                         uint8_t &contextId) {
  NS_LOG_FUNCTION(this << address);
...
Ipv6Address SixLowPanNetDevice::CleanPrefix(Ipv6Address address,
                                            Ipv6Prefix prefix) {
  uint8_t addressBytes[16];
  address.GetBytes(addressBytes);
