...
void SixlowpanFragmentationTest::StartServer(Ptr<Node> serverNode) {
  if (!m_socketServer) {
    ...
    Inet6SocketAddress local =
        Inet6SocketAddress(Ipv6Address("2001:0100::1"), 9);
...
void SixlowpanFragmentationTest::SetFill(uint8_t *fill, uint32_t fillSize,
                                         uint32_t dataSize) {
...
void SixlowpanFragmentationTest::DoRun() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    Simulator::ScheduleWithContext(
        m_socketClient->GetNode()->GetId(), Seconds(0),
