...
template <typename T> NixVectorHelper<T>::NixVectorHelper() {
  std::string name;
  if constexpr (IsIpv4) {
    name = "Ipv4";
  } else {
...
NixVectorHelper<T>::NixVectorHelper(const NixVectorHelper<T> &o)
  ...
  static_assert(
      (std::is_same_v<Ipv4RoutingHelper, T> ||
       std::is_same_v<Ipv6RoutingHelper, T>),
      "Template parameter is not Ipv4RoutingHelper or Ipv6Routing Helper");
}

template <typename T> NixVectorHelper<T> *NixVectorHelper<T>::Copy() const {
...
template <typename T>
void NixVectorHelper<T>::PrintRoutingPathAt(Time printTime, Ptr<Node> source,
                                            IpAddress dest,
                                            Ptr<OutputStreamWrapper> stream,
                                            Time::Unit unit) {
  Simulator::Schedule(printTime, &NixVectorHelper<T>::PrintRoute, source, dest,
                      stream, unit);
}

template <typename T>
void NixVectorHelper<T>::PrintRoute(Ptr<Node> source, IpAddress dest,
                                    Ptr<OutputStreamWrapper> stream,
                                    Time::Unit unit) {
