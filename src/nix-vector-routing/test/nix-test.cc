...
void NixVectorRoutingTest::DoSendDatav4(Ptr<Socket> socket, Ipv4Address to) {
  Address realTo = InetSocketAddress(to, 1234);
  socket->SendTo(Create<Packet>(123), 0, realTo);
}

void NixVectorRoutingTest::DoSendDatav6(Ptr<Socket> socket, Ipv6Address to) {
  Address realTo = Inet6SocketAddress(to, 1234);
  socket->SendTo(Create<Packet>(123), 0, realTo);
}

void NixVectorRoutingTest::SendData(Time delay, Ptr<Socket> socket,
                                    Ipv4Address to) {
  m_receivedPacket = Create<Packet>();
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), delay,
                                 &NixVectorRoutingTest::DoSendDatav4, this,
                                 socket, to);
}
...
void NixVectorRoutingTest::SendData(Time delay, Ptr<Socket> socket,
  ...
  m_receivedPacket = Create<Packet>();
  Simulator::ScheduleWithContext(socket->GetNode()->GetId(), delay,
                                 &NixVectorRoutingTest::DoSendDatav6, this,
                                 socket, to);
}
...
void NixVectorRoutingTest::DoRun() {
  ...
  Ptr<OutputStreamWrapper> routingStream1v4 =
      Create<OutputStreamWrapper>(&stringStream1v4);
  std::ostringstream stringStream1v6;
  Ptr<OutputStreamWrapper> routingStream1v6 =
      Create<OutputStreamWrapper>(&stringStream1v6);
  std::ostringstream stringStream2v4;
  Ptr<OutputStreamWrapper> cacheStreamv4 =
      Create<OutputStreamWrapper>(&stringStream2v4);
  std::ostringstream stringStream2v6;
  Ptr<OutputStreamWrapper> cacheStreamv6 =
      Create<OutputStreamWrapper>(&stringStream2v6);
  std::ostringstream stringStream3v4;
  Ptr<OutputStreamWrapper> routingStream3v4 =
      Create<OutputStreamWrapper>(&stringStream3v4);
  ...
  NS_TEST_EXPECT_MSG_EQ(
      rxSocketv4->Bind(InetSocketAddress(iCiDstv4.GetAddress(1), 1234)), 0,
      "trivial");
  NS_TEST_EXPECT_MSG_EQ(
      rxSocketv6->Bind(Inet6SocketAddress(iCiDstv6.GetAddress(1, 1), 1234)), 0,
      "trivial");
  rxSocketv4->SetRecvCallback(
      MakeCallback(&NixVectorRoutingTest::ReceivePkt, this));
  rxSocketv6->SetRecvCallback(
      MakeCallback(&NixVectorRoutingTest::ReceivePkt, this));

  Ptr<SocketFactory> txSocketFactory =
      nSrcnA.Get(0)->GetObject<UdpSocketFactory>();
  ...
  ipv4NixRouting.PrintRoutingPathAt(Seconds(3), nSrcnA.Get(0),
                                    iCiDstv4.GetAddress(1), routingStream1v4);
  ...
  ipv4NixRouting.PrintRoutingPathAt(Seconds(9), nSrcnA.Get(0),
                                    iCiDstv4.GetAddress(1), routingStream3v4);
  ipv6NixRouting.PrintRoutingPathAt(
  ...
  NS_TEST_EXPECT_MSG_EQ(m_receivedPacketSizes[1], 123,
                        "IPv6 Nix-Vector Routing should work.");
  NS_TEST_EXPECT_MSG_EQ(
  ...
  const std::string p_nSrcnAnCnDstv4 =
      ...
      "(Node 4)\n\n";
  NS_TEST_EXPECT_MSG_EQ(stringStream1v4.str(), p_nSrcnAnCnDstv4,
                        "Routing Path is incorrect.");
  ...
  const std::string p_nSrcnAnCnDstv6 =
      ...
      "(Node 4)\n\n";
  NS_TEST_EXPECT_MSG_EQ(stringStream1v6.str(), p_nSrcnAnCnDstv6,
                        "Routing Path is incorrect.");
  ...
  const std::string p_nSrcnAnBnCnDstv4 =
      ...
      "10.1.3.1                 (Node 3)  ---->   10.1.3.2                 "
      "(Node 4)\n\n";
  NS_TEST_EXPECT_MSG_EQ(stringStream3v4.str(), p_nSrcnAnBnCnDstv4,
                        "Routing Path is incorrect.");
  ...
  const std::string p_nSrcnAnBnCnDstv6 =
      ...
      "fe80::200:ff:fe00:7      (Node 3)  ---->   2001:3::200:ff:fe00:8    "
      "(Node 4)\n\n";
  NS_TEST_EXPECT_MSG_EQ(stringStream3v6.str(), p_nSrcnAnBnCnDstv6,
                        "Routing Path is incorrect.");

  const std::string emptyCaches =
      ...
      "NixCache:\n"
      "IpRouteCache:\n\n";
  NS_TEST_EXPECT_MSG_EQ(stringStream2v4.str(), emptyCaches,
                        "The caches should have been empty.");
  NS_TEST_EXPECT_MSG_EQ(stringStream2v6.str(), emptyCaches,
                        "The caches should have been empty.");
