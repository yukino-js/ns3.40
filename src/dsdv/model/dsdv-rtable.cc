...
RoutingTableEntry::RoutingTableEntry(Ptr<NetDevice> dev, Ipv4Address dst,
                                     uint32_t seqNo, Ipv4InterfaceAddress iface,
                                     uint32_t hops, Ipv4Address nextHop,
                                     Time lifetime, Time settlingTime,
                                     bool areChanged)
    : m_seqNo(seqNo), m_hops(hops), m_lifeTime(lifetime), m_iface(iface),
      m_flag(VALID), m_settlingTime(settlingTime),
...
uint32_t RoutingTable::RoutingTableSize() { return m_ipv4AddressEntry.size(); }
...
void RoutingTable::Purge(
  ...
  for (auto i = m_ipv4AddressEntry.begin(); i != m_ipv4AddressEntry.end();) {
    ...
    if (i->second.GetLifeTime() > m_holddownTime && (i->second.GetHop() > 0)) {
      for (auto j = m_ipv4AddressEntry.begin();
