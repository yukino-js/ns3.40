...
public:
  ...
  RoutingTableEntry(Ptr<NetDevice> dev = nullptr,
                    Ipv4Address dst = Ipv4Address(), uint32_t seqNo = 0,
                    Ipv4InterfaceAddress iface = Ipv4InterfaceAddress(),
                    uint32_t hops = 0, Ipv4Address nextHop = Ipv4Address(),
   ...
   */
  void SetSeqNo(uint32_t sequenceNumber) { m_seqNo = sequenceNumber; }
   ...
   */
  void SetHop(uint32_t hopCount) { m_hops = hopCount; }

  /**
   * Get hop
   * \returns the hop count
   */
  uint32_t GetHop() const { return m_hops; }
   ...
   */
  Time GetLifeTime() const { return (Simulator::Now() - m_lifeTime); }

  /**
   * Set settling time
   * \param settlingTime the settling time
   */
  void SetSettlingTime(Time settlingTime) { m_settlingTime = settlingTime; }

  /**
   * Get settling time
   * \returns the settling time
   */
  Time GetSettlingTime() const { return (m_settlingTime); }
   ...
   */
  bool GetEntriesChanged() const { return m_entriesChanged; }
...
public:
  ...
  void GetListOfDestinationWithNextHop(
      Ipv4Address nxtHp, std::map<Ipv4Address, RoutingTableEntry> &dstList);
  /**
   ...
   */
  Time Getholddowntime() const { return m_holddownTime; }

  /**
   * Set hold down time (time until an invalid route may be deleted)
   * \param t the hold down time
   */
  void Setholddowntime(Time t) { m_holddownTime = t; }
