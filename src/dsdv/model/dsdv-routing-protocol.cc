...
struct DeferredRouteOutputTag : public Tag {
  ...
  void Serialize(TagBuffer i) const override { i.WriteU32(oif); }

  void Deserialize(TagBuffer i) override { oif = i.ReadU32(); }
...
void RoutingProtocol::SetEnableBufferFlag(bool f) { EnableBuffering = f; }

bool RoutingProtocol::GetEnableBufferFlag() const { return EnableBuffering; }

void RoutingProtocol::SetWSTFlag(bool f) { EnableWST = f; }

bool RoutingProtocol::GetWSTFlag() const { return EnableWST; }

void RoutingProtocol::SetEnableRAFlag(bool f) { EnableRouteAggregation = f; }

bool RoutingProtocol::GetEnableRAFlag() const { return EnableRouteAggregation; }
...
RoutingProtocol::RoutingProtocol()
    : m_routingTable(), m_advRoutingTable(), m_queue(),
...
Ptr<Ipv4Route> RoutingProtocol::RouteOutput(Ptr<Packet> p,
  ...
  for (auto rmItr = removedAddresses.begin(); rmItr != removedAddresses.end();
       ++rmItr) {
...
bool RoutingProtocol::RouteInput(Ptr<const Packet> p, const Ipv4Header &header,
  ...
  if (m_routingTable.LookupRoute(dst, toDst)) {
    ...
    if (m_routingTable.LookupRoute(toDst.GetNextHop(), ne)) {
      ...
      NS_LOG_LOGIC(m_mainAddress
                   << " is forwarding packet " << p->GetUid() << " to " << dst
...
Ptr<Ipv4Route> RoutingProtocol::LoopbackRoute(const Ipv4Header &hdr,
  ...
  NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(),
                "Valid DSDV source address not found");
  rt->SetGateway(Ipv4Address("127.0.0.1"));
  rt->SetOutputDevice(m_lo);
  return rt;
}

void RoutingProtocol::RecvDsdv(Ptr<Socket> socket) {
  ...
  NS_LOG_FUNCTION(m_mainAddress
                  << " received dsdv packet of size: " << packetSize
  ...
  for (; packetSize > 0; packetSize = packetSize - 12) {
    ...
    NS_LOG_DEBUG("Received a DSDV packet from "
                 << sender << " to " << receiver
                 << ". Details are: Destination: " << dsdvHeader.GetDst()
                 << ", Seq No: " << dsdvHeader.GetDstSeqno()
    ...
    if (!permanentTableVerifier) {
      ...
      if (dsdvHeader.GetDstSeqno() % 2 != 1) {
        ...
        if (sender == advTableEntry.GetNextHop()) {
          ...
          advTableEntry.SetEntriesChanged(true);
          m_advRoutingTable.Update(advTableEntry);
          for (auto i = dstsWithNextHopSrc.begin();
          ...
          NS_LOG_DEBUG(dsdvHeader.GetDst()
                       << " : Discard this link break update as it "
...
void RoutingProtocol::SendTriggeredUpdate() {
  ...
  for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j) {
    ...
    if (packet->GetSize() >= 12) {
      ...
      m_routingTable.LookupRoute(m_ipv4->GetAddress(1, 0).GetBroadcast(),
                                 temp2);
...
void RoutingProtocol::Send(Ptr<Ipv4Route> route, Ptr<const Packet> packet,
                           const Ipv4Header &header) {
  Ptr<Ipv4L3Protocol> l3 = m_ipv4->GetObject<Ipv4L3Protocol>();
  NS_ASSERT(l3);
  Ptr<Packet> p = packet->Copy();
  l3->Send(p, route->GetSource(), header.GetDestination(), header.GetProtocol(),
           route);
}

void RoutingProtocol::Drop(Ptr<const Packet> packet, const Ipv4Header &header,
                           Socket::SocketErrno err) {
  NS_LOG_DEBUG(m_mainAddress << " drop packet " << packet->GetUid() << " to "
                             << header.GetDestination() << " from queue. Error "
                             << err);
}
    ...
    if (m_queue.Find(rt.GetDestination())) {
      if (rt.GetHop() == 1) {
        ...
        NS_LOG_LOGIC("A route exists from "
                     ...
                     << route->GetDestination() << " via " << rt.GetNextHop());
      }
...
void RoutingProtocol::SendPacketFromQueue(Ipv4Address dst,
                                          Ptr<Ipv4Route> route) {
  NS_LOG_DEBUG(m_mainAddress << " is sending a queued packet to destination "
                             << dst);
...
Time RoutingProtocol::GetSettlingTime(Ipv4Address address) {
  ...
  if (EnableWST) {
    if (mainrt.GetSettlingTime() == Seconds(0)) {
      ...
      NS_LOG_DEBUG("Route SettlingTime: "
                   << mainrt.GetSettlingTime().As(Time::S)
