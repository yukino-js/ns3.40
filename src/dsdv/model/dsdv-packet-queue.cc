...
namespace dsdv {
uint32_t PacketQueue::GetSize() {
  Purge();
  return m_queue.size();
}

bool PacketQueue::Enqueue(QueueEntry &entry) {
  NS_LOG_FUNCTION("Enqueuing packet destined for"
...
void PacketQueue::DropPacketWithDst(Ipv4Address dst) {
  ...
  auto new_end =
      std::remove_if(m_queue.begin(), m_queue.end(), [&](const QueueEntry &en) {
...
uint32_t PacketQueue::GetCountForPacketsWithDst(Ipv4Address dst) {
  uint32_t count = 0;
  for (auto i = m_queue.begin(); i != m_queue.end(); ++i) {
    if (i->GetIpv4Header().GetDestination() == dst) {
      count++;
