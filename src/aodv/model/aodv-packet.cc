...
TypeId TypeHeader::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t TypeHeader::GetSerializedSize() const { return 1; }
...
RreqHeader::RreqHeader(uint8_t flags, uint8_t reserved, uint8_t hopCount,
    ...
    : m_flags(flags), m_reserved(reserved), m_hopCount(hopCount),
      m_requestID(requestID), m_dst(dst), m_dstSeqNo(dstSeqNo),
      m_origin(origin), m_originSeqNo(originSeqNo) {}
...
uint32_t RreqHeader::GetSerializedSize() const { return 23; }
...
std::ostream &operator<<(std::ostream &os, const RreqHeader &h) {
  h.Print(os);
  return os;
}
...
bool RreqHeader::GetGratuitousRrep() const { return (m_flags & (1 << 5)); }
...
bool RreqHeader::GetDestinationOnly() const { return (m_flags & (1 << 4)); }
...
bool RreqHeader::GetUnknownSeqno() const { return (m_flags & (1 << 3)); }
...
RrepHeader::RrepHeader(uint8_t prefixSize, uint8_t hopCount, Ipv4Address dst,
    ...
    : m_flags(0), m_prefixSize(prefixSize), m_hopCount(hopCount), m_dst(dst),
...
uint32_t RrepHeader::GetSerializedSize() const { return 19; }
...
void RrepHeader::SetLifeTime(Time t) { m_lifeTime = t.GetMilliSeconds(); }
...
bool RrepHeader::GetAckRequired() const { return (m_flags & (1 << 6)); }
...
std::ostream &operator<<(std::ostream &os, const RrepHeader &h) {
  h.Print(os);
  return os;
}
...
uint32_t RrepAckHeader::GetSerializedSize() const { return 1; }
...
std::ostream &operator<<(std::ostream &os, const RrepAckHeader &h) {
  h.Print(os);
  return os;
}
...
void RerrHeader::Serialize(Buffer::Iterator i) const {
  ...
  for (auto j = m_unreachableDstSeqNo.begin(); j != m_unreachableDstSeqNo.end();
       ++j) {
    WriteTo(i, (*j).first);
...
std::ostream &operator<<(std::ostream &os, const RerrHeader &h) {
  h.Print(os);
  return os;
}
} // namespace aodv
