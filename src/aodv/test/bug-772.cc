...
Bug772ChainTest::Bug772ChainTest(const char *const prefix,
    ...
    : TestCase("Bug 772 UDP and TCP chain regression test"), m_nodes(nullptr),
      m_prefix(prefix), m_proto(proto), m_time(t), m_size(size), m_step(120),
      m_port(9), m_receivedPackets(0) {}

Bug772ChainTest::~Bug772ChainTest() { delete m_nodes; }
...
void Bug772ChainTest::HandleRead(Ptr<Socket> socket) { m_receivedPackets++; }
...
void Bug772ChainTest::CreateDevices() {
  ...
  wifi.SetRemoteStationManager(
      "ns3::ConstantRateWifiManager", "DataMode", StringValue("OfdmRate6Mbps"),
  ...
  m_sendSocket->Connect(
      InetSocketAddress(interfaces.GetAddress(m_size - 1), m_port));
  m_sendSocket->SetAllowBroadcast(true);
  Simulator::ScheduleWithContext(m_sendSocket->GetNode()->GetId(), Seconds(1.0),
                                 &Bug772ChainTest::SendData, this,
                                 m_sendSocket);

  m_recvSocket = Socket::CreateSocket(m_nodes->Get(m_size - 1),
                                      TypeId::LookupByName(m_proto));
  m_recvSocket->Bind(InetSocketAddress(Ipv4Address::GetAny(), m_port));
  m_recvSocket->Listen();
  m_recvSocket->ShutdownSend();
  m_recvSocket->SetRecvCallback(
      MakeCallback(&Bug772ChainTest::HandleRead, this));
}
