...
TypeId PointToPointChannel::GetTypeId() {
  static TypeId tid =
...
void PointToPointChannel::Attach(Ptr<PointToPointNetDevice> device) {
...
bool PointToPointChannel::TransmitStart(Ptr<const Packet> p,
...
std::size_t PointToPointChannel::GetNDevices() const {
  NS_LOG_FUNCTION_NOARGS();
  return m_nDevices;
}
...
Ptr<NetDevice> PointToPointChannel::GetDevice(std::size_t i) const {
  NS_LOG_FUNCTION_NOARGS();
  return GetPointToPointDevice(i);
}

Time PointToPointChannel::GetDelay() const { return m_delay; }

Ptr<PointToPointNetDevice> PointToPointChannel::GetSource(uint32_t i) const {
  return m_link[i].m_src;
}
...
bool PointToPointChannel::IsInitialized() const {
  NS_ASSERT(m_link[0].m_state != INITIALIZING);
  NS_ASSERT(m_link[1].m_state != INITIALIZING);
  return true;
}
