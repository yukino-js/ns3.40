...
void ClientRx(Ptr<const Packet> packet, const Address &address) {
  NS_LOG_INFO("Client received a packet of " << packet->GetSize()
                                             << " bytes from " << address);
}
...
void ClientPageReceived(Ptr<const ThreeGppHttpClient> client, const Time &time,
                        uint32_t numObjects, uint32_t numBytes) {
  NS_LOG_INFO("Client " << client << " has received a page that took "
                        << time.As(Time::MS) << " ms to load with "
                        << numObjects << " objects and " << numBytes
                        << " bytes.");
}
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("SimulationTime", "Length of simulation in seconds.",
               simTimeSec);
  cmd.Parse(argc, argv);
  ...
  httpClient->TraceConnectWithoutContext(
      "RxEmbeddedObject", MakeCallback(&ClientEmbeddedObjectReceived));
