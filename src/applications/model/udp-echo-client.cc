...
TypeId UdpEchoClient::GetTypeId() {
  static TypeId tid =
...
void UdpEchoClient::SetRemote(Address ip, uint16_t port) {
  NS_LOG_FUNCTION(this << ip << port);
  m_peerAddress = ip;
  m_peerPort = port;
}

void UdpEchoClient::SetRemote(Address addr) {
  NS_LOG_FUNCTION(this << addr);
  m_peerAddress = addr;
}

void UdpEchoClient::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void UdpEchoClient::StopApplication() {
  NS_LOG_FUNCTION(this);
...
void UdpEchoClient::SetDataSize(uint32_t dataSize) {
...
uint32_t UdpEchoClient::GetDataSize() const {
  NS_LOG_FUNCTION(this);
  return m_size;
}

void UdpEchoClient::SetFill(std::string fill) {
...
void UdpEchoClient::SetFill(uint8_t fill, uint32_t dataSize) {
...
void UdpEchoClient::SetFill(uint8_t *fill, uint32_t fillSize,
                            uint32_t dataSize) {
...
void UdpEchoClient::ScheduleTransmit(Time dt) {
...
void UdpEchoClient::Send() {
  ...
  if (Ipv4Address::IsMatchingType(m_peerAddress)) {
    ...
    NS_LOG_INFO(
        "At time " << Simulator::Now().As(Time::S) << " client sent " << m_size
                   << " bytes to "
...
void UdpEchoClient::HandleRead(Ptr<Socket> socket) {
