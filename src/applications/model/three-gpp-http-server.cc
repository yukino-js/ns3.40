...
ThreeGppHttpServer::ThreeGppHttpServer()
    : m_state(NOT_STARTED), m_initialSocket(nullptr),
  ...
  NS_LOG_INFO(this << " MTU size for this server application is " << m_mtuSize
                   << " bytes.");
}
      ...
      TypeId("ns3::ThreeGppHttpServer")
          ...
          .AddTraceSource(
              ...
              MakeTraceSourceAccessor(
                  &ThreeGppHttpServer::m_connectionEstablishedTrace),
          ...
          .AddTraceSource("EmbeddedObject",
                          ...
                          MakeTraceSourceAccessor(
                              &ThreeGppHttpServer::m_embeddedObjectTrace),
                          "ns3::HttpServer::HttpObjectCallback")
          .AddTraceSource(
          ...
          .AddTraceSource(
              ...
              MakeTraceSourceAccessor(
                  &ThreeGppHttpServer::m_stateTransitionTrace),
              "ns3::Application::StateTransitionCallback");
  return tid;
}

void ThreeGppHttpServer::SetMtuSize(uint32_t mtuSize) {
  NS_LOG_FUNCTION(this << mtuSize);
  m_mtuSize = mtuSize;
}

Ptr<Socket> ThreeGppHttpServer::GetSocket() const { return m_initialSocket; }

ThreeGppHttpServer::State_t ThreeGppHttpServer::GetState() const {
  return m_state;
}

std::string ThreeGppHttpServer::GetStateString() const {
...
void ThreeGppHttpServer::DoDispose() {
  NS_LOG_FUNCTION(this);
  ...
  if (m_state == NOT_STARTED) {
    ...
    if (!m_initialSocket) {
      ...
      if (Ipv4Address::IsMatchingType(m_localAddress)) {
        ...
        const Inet6SocketAddress inet6Socket =
            Inet6SocketAddress(ipv6, m_localPort);
    ...
    m_initialSocket->SetSendCallback(
...
void ThreeGppHttpServer::NormalCloseCallback(Ptr<Socket> socket) {
...
void ThreeGppHttpServer::ErrorCloseCallback(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
...
void ThreeGppHttpServer::ReceivedDataCallback(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
  ...
  while ((packet = socket->RecvFrom(from))) {
    ...
    case ThreeGppHttpHeader::MAIN_OBJECT:
      ...
      m_txBuffer->RecordNextServe(
          ...
          Simulator::Schedule(processingDelay,
                              &ThreeGppHttpServer::ServeNewMainObject, this,
                              socket),
    ...
    case ThreeGppHttpHeader::EMBEDDED_OBJECT:
      ...
      m_txBuffer->RecordNextServe(
          socket,
          Simulator::Schedule(processingDelay,
                              &ThreeGppHttpServer::ServeNewEmbeddedObject, this,
                              socket),
          httpHeader.GetClientTs());
      break;
...
void ThreeGppHttpServer::SendCallback(Ptr<Socket> socket,
  ...
  if (!m_txBuffer->IsBufferEmpty(socket)) {
    const uint32_t txBufferSize [[maybe_unused]] =
...
void ThreeGppHttpServer::ServeNewMainObject(Ptr<Socket> socket) {
  ...
  NS_LOG_INFO(this << " Main object to be served is " << objectSize
                   << " bytes.");
...
void ThreeGppHttpServer::ServeNewEmbeddedObject(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);

  const uint32_t objectSize = m_httpVariables->GetEmbeddedObjectSize();
  NS_LOG_INFO(this << " Embedded object to be served is " << objectSize
                   << " bytes.");
...
uint32_t ThreeGppHttpServer::ServeFromTxBuffer(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
  ...
  if (firstPartOfObject) {
    ...
    NS_LOG_INFO(
        this << " Created packet " << packet << " of " << packetSize
             << " bytes."
...
bool ThreeGppHttpServerTxBuffer::IsSocketAvailable(Ptr<Socket> socket) const {
  auto it = m_txBuffer.find(socket);
  return (it != m_txBuffer.end());
}

void ThreeGppHttpServerTxBuffer::AddSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
...
void ThreeGppHttpServerTxBuffer::RemoveSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
  ...
  if (!Simulator::IsExpired(it->second.nextServe)) {
    NS_LOG_INFO(
        this << " Canceling a serving event which is due in "
...
void ThreeGppHttpServerTxBuffer::CloseSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
...
void ThreeGppHttpServerTxBuffer::CloseAllSockets() {
  NS_LOG_FUNCTION(this);
...
bool ThreeGppHttpServerTxBuffer::IsBufferEmpty(Ptr<Socket> socket) const {
  auto it = m_txBuffer.find(socket);
  NS_ASSERT_MSG(it != m_txBuffer.end(),
                "Socket " << socket << " cannot be found.");
...
Time ThreeGppHttpServerTxBuffer::GetClientTs(Ptr<Socket> socket) const {
  auto it = m_txBuffer.find(socket);
  NS_ASSERT_MSG(it != m_txBuffer.end(),
                "Socket " << socket << " cannot be found.");
...
uint32_t ThreeGppHttpServerTxBuffer::GetBufferSize(Ptr<Socket> socket) const {
  auto it = m_txBuffer.find(socket);
  NS_ASSERT_MSG(it != m_txBuffer.end(),
                "Socket " << socket << " cannot be found.");
...
bool ThreeGppHttpServerTxBuffer::HasTxedPartOfObject(Ptr<Socket> socket) const {
  auto it = m_txBuffer.find(socket);
  NS_ASSERT_MSG(it != m_txBuffer.end(),
                "Socket " << socket << " cannot be found");
...
void ThreeGppHttpServerTxBuffer::WriteNewObject(
    Ptr<Socket> socket, ThreeGppHttpHeader::ContentType_t contentType,
    uint32_t objectSize) {
...
void ThreeGppHttpServerTxBuffer::RecordNextServe(Ptr<Socket> socket,
...
void ThreeGppHttpServerTxBuffer::DepleteBufferSize(Ptr<Socket> socket,
                                                   uint32_t amount) {
...
void ThreeGppHttpServerTxBuffer::PrepareClose(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
  auto it = m_txBuffer.find(socket);
  NS_ASSERT_MSG(it != m_txBuffer.end(),
                "Socket " << socket << " cannot be found.");
