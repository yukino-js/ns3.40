...
PacketSink::~PacketSink() { NS_LOG_FUNCTION(this); }
...
Ptr<Socket> PacketSink::GetListeningSocket() const {
  NS_LOG_FUNCTION(this);
  return m_socket;
}

std::list<Ptr<Socket>> PacketSink::GetAcceptedSockets() const {
  NS_LOG_FUNCTION(this);
  return m_socketList;
}
...
void PacketSink::PacketReceived(const Ptr<Packet> &p, const Address &from,
