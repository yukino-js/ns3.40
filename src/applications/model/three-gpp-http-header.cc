...
ThreeGppHttpHeader::ThreeGppHttpHeader()
    : Header(), m_contentType(NOT_SET), m_contentLength(0), m_clientTs(0),
      m_serverTs(0) {
...
TypeId ThreeGppHttpHeader::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t ThreeGppHttpHeader::GetSerializedSize() const { return 2 + 4 + 8 + 8; }

void ThreeGppHttpHeader::Serialize(Buffer::Iterator start) const {
...
uint32_t ThreeGppHttpHeader::Deserialize(Buffer::Iterator start) {
  NS_LOG_FUNCTION(this << &start);
...
void ThreeGppHttpHeader::Print(std::ostream &os) const {
...
std::string ThreeGppHttpHeader::ToString() const {
  NS_LOG_FUNCTION(this);
...
void ThreeGppHttpHeader::SetContentType(
...
ThreeGppHttpHeader::ContentType_t ThreeGppHttpHeader::GetContentType() const {
  ContentType_t ret;
...
void ThreeGppHttpHeader::SetContentLength(uint32_t contentLength) {
  NS_LOG_FUNCTION(this << contentLength);
  m_contentLength = contentLength;
}

uint32_t ThreeGppHttpHeader::GetContentLength() const {
  return m_contentLength;
}

void ThreeGppHttpHeader::SetClientTs(Time clientTs) {
  NS_LOG_FUNCTION(this << clientTs.As(Time::S));
  m_clientTs = clientTs.GetTimeStep();
}

Time ThreeGppHttpHeader::GetClientTs() const { return TimeStep(m_clientTs); }

void ThreeGppHttpHeader::SetServerTs(Time serverTs) {
  NS_LOG_FUNCTION(this << serverTs.As(Time::S));
  m_serverTs = serverTs.GetTimeStep();
}

Time ThreeGppHttpHeader::GetServerTs() const { return TimeStep(m_serverTs); }
