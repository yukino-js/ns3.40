...
TypeId ApplicationPacketProbe::GetTypeId() {
  static TypeId tid =
...
ApplicationPacketProbe::~ApplicationPacketProbe() { NS_LOG_FUNCTION(this); }

void ApplicationPacketProbe::SetValue(Ptr<const Packet> packet,
...
void ApplicationPacketProbe::SetValueByPath(std::string path,
...
bool ApplicationPacketProbe::ConnectByObject(std::string traceSource,
                                             Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << traceSource << obj);
  NS_LOG_DEBUG(
      "Name of probe (if any) in names database: " << Names::FindPath(obj));
  bool connected = obj->TraceConnectWithoutContext(
      traceSource, MakeCallback(&ns3::ApplicationPacketProbe::TraceSink, this));
  return connected;
}

void ApplicationPacketProbe::ConnectByPath(std::string path) {
  NS_LOG_FUNCTION(this << path);
  NS_LOG_DEBUG("Name of probe to search for in config database: " << path);
  Config::ConnectWithoutContext(
      path, MakeCallback(&ns3::ApplicationPacketProbe::TraceSink, this));
}

void ApplicationPacketProbe::TraceSink(Ptr<const Packet> packet,
                                       const Address &address) {
  NS_LOG_FUNCTION(this << packet << address);
  if (IsEnabled()) {
    m_packet = packet;
