...
TypeId UdpTraceClient::GetTypeId() {
  static TypeId tid =
...
void UdpTraceClient::SetRemote(Address ip, uint16_t port) {
...
void UdpTraceClient::SetRemote(Address addr) {
  NS_LOG_FUNCTION(this << addr);
  m_entries.clear();
  m_peerAddress = addr;
}

void UdpTraceClient::SetTraceFile(std::string traceFile) {
...
void UdpTraceClient::SetMaxPacketSize(uint16_t maxPacketSize) {
...
void UdpTraceClient::DoDispose() {
  NS_LOG_FUNCTION(this);
  Application::DoDispose();
}

void UdpTraceClient::LoadTrace(std::string filename) {
...
void UdpTraceClient::LoadDefaultTrace() {
  NS_LOG_FUNCTION(this);
  uint32_t prevTime = 0;
  for (uint32_t i = 0; i < (sizeof(g_defaultEntries) / sizeof(TraceEntry));
       i++) {
...
void UdpTraceClient::SendPacket(uint32_t size) {
...
void UdpTraceClient::Send() {
  ...
  if (!cycled || m_traceLoop) {
    m_sendEvent = Simulator::Schedule(MilliSeconds(entry->timeToSend),
                                      &UdpTraceClient::Send, this);
  }
}

void UdpTraceClient::SetTraceLoop(bool traceLoop) { m_traceLoop = traceLoop; }
