...
UdpServer::~UdpServer() { NS_LOG_FUNCTION(this); }
...
uint32_t UdpServer::GetLost() const {
  NS_LOG_FUNCTION(this);
  return m_lossCounter.GetLost();
}

uint64_t UdpServer::GetReceived() const {
  NS_LOG_FUNCTION(this);
  return m_received;
}

void UdpServer::DoDispose() {
  NS_LOG_FUNCTION(this);
    ...
    if (packet->GetSize() > 0) {
      ...
      if (InetSocketAddress::IsMatchingType(from)) {
        NS_LOG_INFO("TraceDelay: RX "
                    ...
                    << " Sequence Number: " << currentSequenceNumber << " Uid: "
                    << packet->GetUid() << " TXtime: " << seqTs.GetTs()
