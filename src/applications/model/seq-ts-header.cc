...
SeqTsHeader::SeqTsHeader() : m_seq(0), m_ts(Simulator::Now().GetTimeStep()) {
...
uint32_t SeqTsHeader::GetSeq() const {
  NS_LOG_FUNCTION(this);
  return m_seq;
}

Time SeqTsHeader::GetTs() const {
  NS_LOG_FUNCTION(this);
  return TimeStep(m_ts);
}

TypeId SeqTsHeader::GetTypeId() {
...
TypeId SeqTsHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
uint32_t SeqTsHeader::GetSerializedSize() const {
  NS_LOG_FUNCTION(this);
  return 4 + 8;
}
...
uint32_t SeqTsHeader::Deserialize(Buffer::Iterator start) {
  NS_LOG_FUNCTION(this << &start);
  Buffer::Iterator i = start;
  m_seq = i.ReadNtohU32();
  m_ts = i.ReadNtohU64();
