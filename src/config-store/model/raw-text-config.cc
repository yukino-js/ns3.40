...
RawTextConfigSave::RawTextConfigSave() : m_os(nullptr) {
...
void RawTextConfigSave::SetFilename(std::string filename) {
  NS_LOG_FUNCTION(this << filename);
  m_os = new std::ofstream();
  m_os->open(filename, std::ios::out);
}

void RawTextConfigSave::Default() {
  NS_LOG_FUNCTION(this);

  class RawTextDefaultIterator : public AttributeDefaultIterator {
  public:
    RawTextDefaultIterator(std::ostream *os) { m_os = os; }
  ...
  private:
    void StartVisitTypeId(std::string name) override { m_typeId = name; }
...
void RawTextConfigSave::Global() {
  NS_LOG_FUNCTION(this);
  for (auto i = GlobalValue::Begin(); i != GlobalValue::End(); ++i) {
    StringValue value;
    (*i)->GetValue(value);
    NS_LOG_LOGIC("Saving " << (*i)->GetName());
    *m_os << "global " << (*i)->GetName() << " \"" << value.Get() << "\""
          << std::endl;
  }
}

void RawTextConfigSave::Attributes() {
  NS_LOG_FUNCTION(this);

  class RawTextAttributeIterator : public AttributeIterator {
  public:
    RawTextAttributeIterator(std::ostream *os) : m_os(os) {}
...
RawTextConfigLoad::RawTextConfigLoad() : m_is(nullptr) {
  NS_LOG_FUNCTION(this);
}
...
void RawTextConfigLoad::SetFilename(std::string filename) {
  NS_LOG_FUNCTION(this << filename);
  m_is = new std::ifstream();
  m_is->open(filename, std::ios::in);
}

std::string RawTextConfigLoad::Strip(std::string value) {
...
void RawTextConfigLoad::Attributes() {
  NS_LOG_FUNCTION(this);
  m_is->clear();
  m_is->seekg(0);
  std::string type;
...
bool RawTextConfigLoad::ParseLine(const std::string &line, std::string &type,
