...
TypeId ConfigStore::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ConfigStore")
          .SetParent<ObjectBase>()
          .SetGroupName("ConfigStore")
          .AddAttribute(
              "Mode", "Configuration mode", EnumValue(ConfigStore::NONE),
              MakeEnumAccessor(&ConfigStore::SetMode),
              MakeEnumChecker(ConfigStore::NONE, "None", ConfigStore::LOAD,
                              "Load", ConfigStore::SAVE, "Save"))
...
TypeId ConfigStore::GetInstanceTypeId() const { return GetTypeId(); }
...
void ConfigStore::SetMode(Mode mode) {
  NS_LOG_FUNCTION(this << mode);
  m_mode = mode;
}

void ConfigStore::SetFileFormat(FileFormat format) {
  NS_LOG_FUNCTION(this << format);
  m_fileFormat = format;
}

void ConfigStore::SetFilename(std::string filename) {
  NS_LOG_FUNCTION(this << filename);
  m_filename = filename;
}

void ConfigStore::SetSaveDeprecated(bool saveDeprecated) {
  NS_LOG_FUNCTION(this << saveDeprecated);
  m_saveDeprecated = saveDeprecated;
}

void ConfigStore::ConfigureAttributes() {
  NS_LOG_FUNCTION(this);
  m_file->Attributes();
}

void ConfigStore::ConfigureDefaults() {
  NS_LOG_FUNCTION(this);
  m_file->Default();
  m_file->Global();
}
