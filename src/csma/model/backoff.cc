...
void Backoff::ResetBackoffTime() { m_numBackoffRetries = 0; }

bool Backoff::MaxRetriesReached() const {
  return (m_numBackoffRetries >= m_maxRetries);
}

void Backoff::IncrNumRetries() { m_numBackoffRetries++; }

int64_t Backoff::AssignStreams(int64_t stream) {
