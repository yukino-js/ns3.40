...
int32_t CsmaChannel::Attach(Ptr<CsmaNetDevice> device) {
...
bool CsmaChannel::Reattach(Ptr<CsmaNetDevice> device) {
  NS_LOG_FUNCTION(this << device);
  NS_ASSERT(device);
...
bool CsmaChannel::Detach(uint32_t deviceId) {
  NS_LOG_FUNCTION(this << deviceId);
...
bool CsmaChannel::Detach(Ptr<CsmaNetDevice> device) {
  NS_LOG_FUNCTION(this << device);
  NS_ASSERT(device);
...
bool CsmaChannel::TransmitStart(Ptr<const Packet> p, uint32_t srcId) {
...
bool CsmaChannel::IsActive(uint32_t deviceId) {
  return (m_deviceList[deviceId].active);
}

bool CsmaChannel::TransmitEnd() {
...
void CsmaChannel::PropagationCompleteEvent() {
...
uint32_t CsmaChannel::GetNumActDevices() {
  int numActDevices = 0;
  for (auto it = m_deviceList.begin(); it < m_deviceList.end(); it++) {
    if (it->active) {
      numActDevices++;
...
std::size_t CsmaChannel::GetNDevices() const { return m_deviceList.size(); }

Ptr<CsmaNetDevice> CsmaChannel::GetCsmaDevice(std::size_t i) const {
  return m_deviceList[i].devicePtr;
}

int32_t CsmaChannel::GetDeviceNum(Ptr<CsmaNetDevice> device) {
  int i = 0;
  for (auto it = m_deviceList.begin(); it < m_deviceList.end(); it++) {
    if (it->devicePtr == device) {
      if (it->active) {
...
bool CsmaChannel::IsBusy() { return m_state != IDLE; }

DataRate CsmaChannel::GetDataRate() { return m_bps; }

Time CsmaChannel::GetDelay() { return m_delay; }

WireState CsmaChannel::GetState() { return m_state; }

Ptr<NetDevice> CsmaChannel::GetDevice(std::size_t i) const {
  return GetCsmaDevice(i);
}

CsmaDeviceRec::CsmaDeviceRec() { active = false; }
...
bool CsmaDeviceRec::IsActive() const { return active; }
