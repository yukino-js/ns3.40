...
void Ns3WimaxCsParamTlvTestCase::DoRun() {
  IpcsClassifierRecord classifier(
      Ipv4Address("10.0.0.0"), Ipv4Mask("255.0.0.0"), Ipv4Address("11.0.0.0"),
      Ipv4Mask("255.0.0.0"), 1000, 1100, 3000, 3100, 17, 1);
    ...
    for (auto iter = sfVecValue->Begin(); iter != sfVecValue->End(); ++iter) {
      if ((*iter)->GetType() == SfVectorTlvValue::IPV4_CS_Parameters) {
        ...
        NS_TEST_ASSERT_MSG_EQ(!classifier.CheckMatch(Ipv4Address("10.1.1.1"),
                                                     ...
                                                     1050, 3050, 17),
                              false, "The classifier address did not match.");
        ...
        NS_TEST_ASSERT_MSG_EQ(classifier.CheckMatch(Ipv4Address("11.1.1.1"),
                                                    ...
                                                    1050, 3050, 17),
                              false, "The classifier addresses matched.");
        NS_TEST_ASSERT_MSG_EQ(classifier.CheckMatch(Ipv4Address("10.1.1.1"),
                                                    Ipv4Address("16.1.1.1"),
                                                    1050, 3050, 8),
                              false, "The classifier addresses matched.");
...
void Ns3WimaxSfTlvTestCase::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ(sfRecv.GetDirection(), ServiceFlow::SF_DIRECTION_DOWN,
                        "The sfRecv had the wrong direction.");
  NS_TEST_ASSERT_MSG_EQ(sfRecv.GetSfid(), 100,
                        "The sfRecv had the wrong sfid.");
  NS_TEST_ASSERT_MSG_EQ(sfRecv.GetCsSpecification(), ServiceFlow::IPV4,
  ...
  NS_TEST_ASSERT_MSG_EQ(sfRecv.GetMaximumLatency(), 10,
                        "The sfRecv had the wrong maximum latency.");
  NS_TEST_ASSERT_MSG_EQ(sfRecv.GetMaxTrafficBurst(), 1000,
                        "The sfRecv had the wrong maximum traffic burst.");
  NS_TEST_ASSERT_MSG_EQ(sfRecv.GetTrafficPriority(), 1,
...
Ns3WimaxTlvTestSuite::Ns3WimaxTlvTestSuite() : TestSuite("wimax-tlv", UNIT) {
