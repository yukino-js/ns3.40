...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("scheduler", "type of scheduler to use with the netdevices",
               schedType);
  ...
  Ptr<WimaxNetDevice> dev =
      wimax.Install(bsNodes.Get(0), WimaxHelper::DEVICE_TYPE_BASE_STATION,
                    WimaxHelper::SIMPLE_PHY_TYPE_OFDM, channel, scheduler);
  ...
  for (int i = 0; i < nbSS; i++) {
    ...
    SSPosition[i]->SetAttribute("PositionAllocator",
                                PointerValue(SSPosAllocator[i]));
    SSPosition[i]->SetAttribute(
  ...
  csmaStreamer_ASN.SetChannelAttribute("DataRate",
  ...
  NetDeviceContainer LAN_ASN_STREAMER_Devs =
  ...
  IpcsClassifierRecord MulticastClassifier(
      Ipv4Address("0.0.0.0"), Ipv4Mask("0.0.0.0"), multicastGroup,
      Ipv4Mask("255.255.255.255"), 0, 65000, multicast_port, multicast_port, 17,
      1);

  ServiceFlow MulticastServiceFlow =
      wimax.CreateServiceFlow(ServiceFlow::SF_DIRECTION_DOWN,
                              ServiceFlow::SF_TYPE_UGS, MulticastClassifier);

  bs->GetServiceFlowManager()->AddMulticastServiceFlow(
      MulticastServiceFlow, WimaxPhy::MODULATION_TYPE_QPSK_12);
