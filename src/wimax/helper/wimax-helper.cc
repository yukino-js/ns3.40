...
WimaxHelper::WimaxHelper() : m_channel(nullptr) {}
...
void WimaxHelper::EnableAsciiForConnection(Ptr<OutputStreamWrapper> os,
  ...
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
      << "/$ns3::" << netdevice << "/" << connection << "/TxQueue/Enqueue";
  Config::Connect(
      oss.str(),
  ...
  oss.str("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
      << "/$ns3::" << netdevice << "/" << connection << "/TxQueue/Dequeue";
  Config::Connect(
      oss.str(),
  ...
  oss.str("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
      << "/$ns3::" << netdevice << "/" << connection << "/TxQueue/Drop";
  Config::Connect(
      oss.str(),
  ...
  case SIMPLE_PHY_TYPE_OFDM:
    ...
    if (!m_channel) {
      m_channel = CreateObject<SimpleOfdmWimaxChannel>(
          SimpleOfdmWimaxChannel::COST231_PROPAGATION);
    }
    break;
  default:
...
void WimaxHelper::SetPropagationLossModel(
    SimpleOfdmWimaxChannel::PropModel propagationModel) {
  if (!m_channel) {
    m_channel = CreateObject<SimpleOfdmWimaxChannel>();
  }
  m_channel->GetObject<SimpleOfdmWimaxChannel>()->SetPropagationModel(
      propagationModel);
}

Ptr<WimaxPhy> WimaxHelper::CreatePhy(PhyType phyType, char *SNRTraceFilePath,
                                     bool activateLoss) {
  Ptr<WimaxPhy> phy;
...
WimaxHelper::Install(NodeContainer c, NetDeviceType deviceType, PhyType phyType,
  ...
  for (auto i = c.Begin(); i != c.End(); i++) {
    ...
    Ptr<WimaxPhy> phy =
        CreatePhyWithoutChannel(phyType, (char *)"dummy", false);
    Ptr<WimaxNetDevice> device;
    Ptr<UplinkScheduler> uplinkScheduler = CreateUplinkScheduler(schedulerType);
    Ptr<BSScheduler> bsScheduler = CreateBSScheduler(schedulerType);
...
void WimaxHelper::EnableLogComponents() {
...
void WimaxHelper::AsciiTxEvent(Ptr<OutputStreamWrapper> stream,
                               std::string path, Ptr<const Packet> packet,
...
void WimaxHelper::EnableAsciiInternal(Ptr<OutputStreamWrapper> stream,
  ...
  if (!stream) {
    ...
    Config::Connect(oss.str(),
                    MakeBoundCallback(&WimaxHelper::AsciiRxEvent, theStream));
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
    ...
    if (ssNetDev) {
      ...
      SubscriberStationNetDevice::AsciiTraceCallback DropCb = MakeBoundCallback(
          &AsciiTraceHelper::DefaultDropSinkWithContext, theStream);
  ...
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
      << "/$ns3::WimaxNetDevice/Rx";
  Config::Connect(oss.str(),
                  MakeBoundCallback(&WimaxHelper::AsciiRxEvent, stream));

  oss.str("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
      << "/$ns3::WimaxNetDevice/Tx";
  Config::Connect(oss.str(),
                  MakeBoundCallback(&WimaxHelper::AsciiTxEvent, stream));
...
void WimaxHelper::EnablePcapInternal(std::string prefix, Ptr<NetDevice> nd,
  ...
  phy->TraceConnectWithoutContext("Tx",
                                  MakeBoundCallback(&PcapSniffTxRxEvent, file));
  phy->TraceConnectWithoutContext("Rx",
                                  MakeBoundCallback(&PcapSniffTxRxEvent, file));
}

int64_t WimaxHelper::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  return m_channel->AssignStreams(stream);
}

int64_t WimaxHelper::AssignStreams(NetDeviceContainer c, int64_t stream) {
