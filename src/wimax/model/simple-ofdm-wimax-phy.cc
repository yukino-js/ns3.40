...
TypeId SimpleOfdmWimaxPhy::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::SimpleOfdmWimaxPhy")
          ...
          .AddAttribute("TxPower", "Transmission power (dB).", DoubleValue(30),
                        MakeDoubleAccessor(&SimpleOfdmWimaxPhy::SetTxPower,
                                           &SimpleOfdmWimaxPhy::GetTxPower),
                        MakeDoubleChecker<double>())

          .AddAttribute("G", "This is the ratio of CP time to useful time.",
                        DoubleValue(0.25),
                        MakeDoubleAccessor(&SimpleOfdmWimaxPhy::DoSetGValue,
                                           &SimpleOfdmWimaxPhy::DoGetGValue),
                        MakeDoubleChecker<double>())

          .AddAttribute("TxGain", "Transmission gain (dB).", DoubleValue(0),
                        MakeDoubleAccessor(&SimpleOfdmWimaxPhy::SetTxGain,
                                           &SimpleOfdmWimaxPhy::GetTxGain),
                        MakeDoubleChecker<double>())

          .AddAttribute("RxGain", "Reception gain (dB).", DoubleValue(0),
                        MakeDoubleAccessor(&SimpleOfdmWimaxPhy::SetRxGain,
                                           &SimpleOfdmWimaxPhy::GetRxGain),
                        MakeDoubleChecker<double>())

          .AddAttribute("Nfft", "FFT size", UintegerValue(256),
...
void SimpleOfdmWimaxPhy::InitSimpleOfdmWimaxPhy() {
  m_fecBlockSize = 0;
  m_nrFecBlocksSent = 0;
  m_dataRateBpsk12 = 0;
  m_dataRateQpsk12 = 0;
...
void SimpleOfdmWimaxPhy::ActivateLoss(bool loss) {
  m_snrToBlockErrorRateManager->ActivateLoss(loss);
}

void SimpleOfdmWimaxPhy::SetSNRToBlockErrorRateTracesPath(char *tracesPath) {
  m_snrToBlockErrorRateManager->SetTraceFilePath(tracesPath);
  m_snrToBlockErrorRateManager->ReLoadTraces();
}

uint32_t SimpleOfdmWimaxPhy::GetBandwidth() const {
  return WimaxPhy::GetChannelBandwidth();
}

void SimpleOfdmWimaxPhy::SetBandwidth(uint32_t BW) {
  WimaxPhy::SetChannelBandwidth(BW);
}

double SimpleOfdmWimaxPhy::GetTxPower() const { return m_txPower; }

void SimpleOfdmWimaxPhy::SetTxPower(double txPower) { m_txPower = txPower; }

double SimpleOfdmWimaxPhy::GetNoiseFigure() const { return m_noiseFigure; }

void SimpleOfdmWimaxPhy::SetNoiseFigure(double noiseFigure) {
  m_noiseFigure = noiseFigure;
}

void SimpleOfdmWimaxPhy::DoDispose() {
...
void SimpleOfdmWimaxPhy::DoAttach(Ptr<WimaxChannel> channel) {
  GetChannel()->Attach(this);
}

void SimpleOfdmWimaxPhy::Send(SendParams *params) {
...
WimaxPhy::PhyType SimpleOfdmWimaxPhy::GetPhyType() const {
  return WimaxPhy::simpleOfdmWimaxPhy;
}

void SimpleOfdmWimaxPhy::Send(Ptr<PacketBurst> burst,
...
void SimpleOfdmWimaxPhy::StartSendDummyFecBlock(
    bool isFirstBlock, WimaxPhy::ModulationType modulationType,
    uint8_t direction) {
  ...
  channel->Send(m_blockTime, m_currentBurstSize, this, isFirstBlock,
                isLastFecBlock, GetTxFrequency(), modulationType, direction,
                m_txPower, m_currentBurst);

  m_nrRemainingBlocksToSend--;
  Simulator::Schedule(m_blockTime, &SimpleOfdmWimaxPhy::EndSendFecBlock, this,
                      modulationType, direction);
}

void SimpleOfdmWimaxPhy::EndSendFecBlock(
...
void SimpleOfdmWimaxPhy::EndSend() { SetState(PHY_STATE_IDLE); }

void SimpleOfdmWimaxPhy::StartReceive(uint32_t burstSize, bool isFirstBlock,
                                      uint64_t frequency,
                                      WimaxPhy::ModulationType modulationType,
                                      uint8_t direction, double rxPower,
                                      Ptr<PacketBurst> burst) {
  bool drop = false;
  double Nwb = -114 + m_noiseFigure +
               10 * std::log(GetBandwidth() / 1000000000.0) / 2.303;
  double SNR = rxPower - Nwb;

  SNRToBlockErrorRateRecord *record =
      m_snrToBlockErrorRateManager->GetSNRToBlockErrorRateRecord(
          SNR, modulationType);
  ...
  NS_LOG_INFO("PHY: Receive rxPower=" << rxPower << ", Nwb=" << Nwb
                                      << ", SNR=" << SNR
                                      << ", Modulation=" << modulationType
                                      << ", BlockErrorRate=" << blockErrorRate
  ...
  case PHY_STATE_IDLE:
    if (frequency == GetRxFrequency()) {
      ...
      Simulator::Schedule(m_blockTime, &SimpleOfdmWimaxPhy::EndReceiveFecBlock,
                          this, burstSize, modulationType, direction, drop,
                          burst);
...
void SimpleOfdmWimaxPhy::EndReceiveFecBlock(
    uint32_t burstSize, WimaxPhy::ModulationType modulationType,
  ...
  if ((uint32_t)m_nrReceivedFecBlocks * m_blockSize ==
      burstSize * 8 + m_paddingBits) {
    NotifyRxEnd(burst);
    if (m_nbErroneousBlock == 0) {
      Simulator::Schedule(Seconds(0), &SimpleOfdmWimaxPhy::EndReceive, this,
                          burst);
    } else {
      NotifyRxDrop(burst);
    }
...
void SimpleOfdmWimaxPhy::EndReceive(Ptr<const PacketBurst> burst) {
  Ptr<PacketBurst> b = burst->Copy();
  GetReceiveCallback()(b);
  m_traceRx(burst);
}

Bvec SimpleOfdmWimaxPhy::ConvertBurstToBits(Ptr<const PacketBurst> burst) {
...
void SimpleOfdmWimaxPhy::CreateFecBlocks(
...
Bvec SimpleOfdmWimaxPhy::RecreateBuffer() {
  ...
  for (uint32_t j = 0; j < m_nrBlocks; j++) {
    ...
    buffer.insert(buffer.begin() + i, tmpRecFecBlock.begin(),
                  tmpRecFecBlock.end());
...
void SimpleOfdmWimaxPhy::GetModulationFecParams(
    WimaxPhy::ModulationType modulationType, uint8_t &bitsPerSymbol,
...
uint32_t SimpleOfdmWimaxPhy::CalculateDataRate(
...
uint32_t SimpleOfdmWimaxPhy::DoGetDataRate(
    WimaxPhy::ModulationType modulationType) const {
  switch (modulationType) {
  case MODULATION_TYPE_BPSK_12:
...
Time SimpleOfdmWimaxPhy::GetBlockTransmissionTime(
    WimaxPhy::ModulationType modulationType) const {
  return Seconds((double)GetFecBlockSize(modulationType) /
...
uint64_t SimpleOfdmWimaxPhy::DoGetNrSymbols(
    uint32_t size, WimaxPhy::ModulationType modulationType) const {
  Time transmissionTime = Seconds((double)(GetNrBlocks(size, modulationType) *
                                           GetFecBlockSize(modulationType)) /
                                  DoGetDataRate(modulationType));
  return (uint64_t)std::ceil(transmissionTime.GetSeconds() /
                             GetSymbolDuration().GetSeconds());
}

uint64_t SimpleOfdmWimaxPhy::DoGetNrBytes(
    uint32_t symbols, WimaxPhy::ModulationType modulationType) const {
  Time transmissionTime = Seconds(symbols * GetSymbolDuration().GetSeconds());
  return (uint64_t)std::floor(
      (transmissionTime.GetSeconds() * DoGetDataRate(modulationType)) / 8);
}

uint32_t SimpleOfdmWimaxPhy::GetFecBlockSize(
    WimaxPhy::ModulationType modulationType) const {
...
void SimpleOfdmWimaxPhy::SetBlockParameters(
  ...
  NS_ASSERT_MSG(static_cast<uint32_t>(m_nrBlocks * m_blockSize) >=
                    (burstSize * 8),
...
void SimpleOfdmWimaxPhy::DoSetPhyParameters() {
  ...
  uint32_t symbolsPerFrame =
      lrint(GetFrameDuration().GetSeconds() / symbolDuration.GetSeconds());
  SetSymbolsPerFrame(symbolsPerFrame);
}

void SimpleOfdmWimaxPhy::DoSetNfft(uint16_t nfft) { m_nfft = nfft; }
...
double SimpleOfdmWimaxPhy::DoGetGValue() const { return m_g; }

void SimpleOfdmWimaxPhy::DoSetGValue(double g) { m_g = g; }

void SimpleOfdmWimaxPhy::SetTxGain(double txGain) { m_txGain = txGain; }

void SimpleOfdmWimaxPhy::SetRxGain(double txRain) { m_rxGain = txRain; }

double SimpleOfdmWimaxPhy::GetTxGain() const { return m_txGain; }

double SimpleOfdmWimaxPhy::GetRxGain() const { return m_rxGain; }

std::string SimpleOfdmWimaxPhy::GetTraceFilePath() const {
  return (m_snrToBlockErrorRateManager->GetTraceFilePath());
}

void SimpleOfdmWimaxPhy::SetTraceFilePath(std::string path) {
  m_snrToBlockErrorRateManager->SetTraceFilePath((char *)path.c_str());
  m_snrToBlockErrorRateManager->LoadTraces();
}

void SimpleOfdmWimaxPhy::NotifyTxBegin(Ptr<PacketBurst> burst) {
  m_phyTxBeginTrace(burst);
}

void SimpleOfdmWimaxPhy::NotifyTxEnd(Ptr<PacketBurst> burst) {
  m_phyTxEndTrace(burst);
}

void SimpleOfdmWimaxPhy::NotifyTxDrop(Ptr<PacketBurst> burst) {
  m_phyTxDropTrace(burst);
}

void SimpleOfdmWimaxPhy::NotifyRxBegin(Ptr<PacketBurst> burst) {
  m_phyRxBeginTrace(burst);
}

void SimpleOfdmWimaxPhy::NotifyRxEnd(Ptr<PacketBurst> burst) {
  m_phyRxEndTrace(burst);
}

void SimpleOfdmWimaxPhy::NotifyRxDrop(Ptr<PacketBurst> burst) {
  m_phyRxDropTrace(burst);
}

int64_t SimpleOfdmWimaxPhy::AssignStreams(int64_t stream) {
