...
TypeId UplinkSchedulerRtps::GetTypeId() {
...
std::list<OfdmUlMapIe> UplinkSchedulerRtps::GetUplinkAllocations() const {
  return m_uplinkAllocations;
}

void UplinkSchedulerRtps::GetChannelDescriptorsToUpdate(bool &updateDcd,
                                                        bool &updateUcd,
                                                        bool &sendDcd,
                                                        bool &sendUcd) {
...
uint32_t UplinkSchedulerRtps::CalculateAllocationStartTime() {
  return GetBs()->GetNrDlSymbols() * GetBs()->GetPhy()->GetPsPerSymbol() +
         GetBs()->GetTtg();
}

void UplinkSchedulerRtps::AddUplinkAllocation(OfdmUlMapIe &ulMapIe,
...
void UplinkSchedulerRtps::Schedule() {
  m_uplinkAllocations.clear();
  SetIsIrIntrvlAllocated(false);
  SetIsInvIrIntrvlAllocated(false);
  ...
  if (availableSymbols) {
    for (auto iter = ssRecords->begin(); iter != ssRecords->end(); ++iter) {
      ...
      if (!ssRecord->GetPollForRanging() &&
        ...
        ServiceBandwidthRequests(ssRecord, ServiceFlow::SF_TYPE_NRTPS, ulMapIe,
                                 modulationType, symbolsToAllocation,
                                 availableSymbols);
...
void UplinkSchedulerRtps::ServiceUnsolicitedGrants(
...
void UplinkSchedulerRtps::ServiceBandwidthRequests(
    const SSRecord *ssRecord, ServiceFlow::SchedulingType schedulingType,
    OfdmUlMapIe &ulMapIe, const WimaxPhy::ModulationType modulationType,
    uint32_t &symbolsToAllocation, uint32_t &availableSymbols) {
  std::vector<ServiceFlow *> serviceFlows =
...
bool UplinkSchedulerRtps::ServiceBandwidthRequests(
...
void UplinkSchedulerRtps::ULSchedulerRTPSConnection(
    uint32_t &symbolsToAllocation, uint32_t &availableSymbols) {
    ...
    if (!ssRecord->GetPollForRanging() &&
      ...
      for (auto iter2 = serviceFlows.begin(); iter2 != serviceFlows.end();
        ...
        if (requiredBandwidth > 0) {
          ...
          allocSizeBytes = requiredBandwidth;
          allocSizeSymbols_[nbAllocation] = GetBs()->GetPhy()->GetNrSymbols(
              allocSizeBytes, modulationType_[nbAllocation]);
          totAllocSizeSymbols += allocSizeSymbols_[nbAllocation];

          NS_LOG_INFO("\t\tUL Scheduler for CID = "
  ...
  for (int i = 0; i < nbAllocation; i++) {
    AddUplinkAllocation(ulMapIe_[i], allocSizeSymbols_[i], symbolsToAllocation,
                        availableSymbols);
    allocSizeBytes =
        GetBs()->GetPhy()->GetNrBytes(allocSizeSymbols_[i], modulationType_[i]);
    NS_LOG_INFO("\t\tUpdateGrantedBandwidth for " << i << " = "
                                                  << allocSizeBytes);
...
void UplinkSchedulerRtps::AllocateInitialRangingInterval(
    uint32_t &symbolsToAllocation, uint32_t &availableSymbols) {
...
void UplinkSchedulerRtps::SetupServiceFlow(SSRecord *ssRecord,
...
void UplinkSchedulerRtps::InitOnce() {}

void UplinkSchedulerRtps::ProcessBandwidthRequest(
    const BandwidthRequestHeader &bwRequestHdr) {}

void UplinkSchedulerRtps::OnSetRequestedBandwidth(ServiceFlowRecord *sfr) {
