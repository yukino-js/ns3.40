...
void ServiceFlowManager::DoDispose() {
...
void ServiceFlowManager::AddServiceFlow(ServiceFlow *serviceFlow) {
...
ServiceFlow *ServiceFlowManager::DoClassify(Ipv4Address srcAddress,
  ...
  for (auto iter = m_serviceFlows->begin(); iter != m_serviceFlows->end();
    ...
    if ((*iter)->GetDirection() == dir) {
      if ((*iter)->CheckClassifierMatch(srcAddress, dstAddress, srcPort,
                                        dstPort, proto)) {
        return (*iter);
      }
    }
...
ServiceFlow *ServiceFlowManager::GetServiceFlow(uint32_t sfid) const {
  for (auto iter = m_serviceFlows->begin(); iter != m_serviceFlows->end();
       ++iter) {
...
ServiceFlow *ServiceFlowManager::GetServiceFlow(Cid cid) const {
  for (auto iter = m_serviceFlows->begin(); iter != m_serviceFlows->end();
       ++iter) {
...
std::vector<ServiceFlow *> ServiceFlowManager::GetServiceFlows(
...
bool ServiceFlowManager::AreServiceFlowsAllocated(
    std::vector<ServiceFlow *> serviceFlowVector) {
  for (auto iter = serviceFlowVector.begin(); iter != serviceFlowVector.end();
       ++iter) {
...
ServiceFlow *ServiceFlowManager::GetNextServiceFlowToAllocate() {
  for (auto iter = m_serviceFlows->begin(); iter != m_serviceFlows->end();
       ++iter) {
    if (!(*iter)->GetIsEnabled()) {
      return (*iter);
...
uint32_t ServiceFlowManager::GetNrServiceFlows() const {
