...
void SNRToBlockErrorRateManager::ClearRecords() {
  for (int i = 0; i < 7; i++) {
    for (auto iter = m_recordModulation[i]->begin();
         iter != m_recordModulation[i]->end(); ++iter) {
      if (*iter) {
        delete (*iter);
        (*iter) = 0;
      }
...
void SNRToBlockErrorRateManager::ActivateLoss(bool loss) {
  m_activateLoss = loss;
}

void SNRToBlockErrorRateManager::LoadTraces() {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (!traceFile.good()) {
      NS_LOG_INFO("Unable to load " << traceFilePath.str()
                                    << "!! Loading default traces...");
      LoadDefaultTraces();
      return;
    }
    while (traceFile.good()) {
      traceFile >> snrValue >> bitErrorRate >> burstErrorRate >> sigma2 >> I1 >>
          I2;
      auto record = new SNRToBlockErrorRateRecord(
          snrValue, bitErrorRate, burstErrorRate, sigma2, I1, I2);
...
void SNRToBlockErrorRateManager::LoadDefaultTraces() {
  double snrValue;
  double bitErrorRate;
  double burstErrorRate;
  double sigma2;
  ...
  for (unsigned int j = 0; j < sizeof(modulation0[0]) / sizeof(double); j++) {
    ...
    I1 = modulation0[4][j];
    I2 = modulation0[5][j];
    auto record = new SNRToBlockErrorRateRecord(snrValue, bitErrorRate,
                                                burstErrorRate, sigma2, I1, I2);
...
void SNRToBlockErrorRateManager::ReLoadTraces() {
  double snrValue;
  double bitErrorRate;
  double burstErrorRate;
  double sigma2;
...
void SNRToBlockErrorRateManager::SetTraceFilePath(char *traceFilePath) {
  m_traceFilePath = traceFilePath;
}

std::string SNRToBlockErrorRateManager::GetTraceFilePath() {
  return m_traceFilePath;
}

double SNRToBlockErrorRateManager::GetBlockErrorRate(double SNR,
                                                     uint8_t modulation) {
  if (!m_activateLoss) {
    return 0;
  }
  ...
  double intervalSize =
      (record->at(i)->GetSNRValue() - record->at(i - 1)->GetSNRValue());
...
SNRToBlockErrorRateManager::GetSNRToBlockErrorRateRecord(double SNR,
  ...
  double BlcER = coeff2 * (record->at(i - 1)->GetBlockErrorRate()) +
                 coeff1 * (record->at(i)->GetBlockErrorRate());
  double sigma2 = coeff2 * (record->at(i - 1)->GetSigma2()) +
                  coeff1 * (record->at(i)->GetSigma2());
  double I1 =
      coeff2 * (record->at(i - 1)->GetI1()) + coeff1 * (record->at(i)->GetI1());
  double I2 =
      coeff2 * (record->at(i - 1)->GetI2()) + coeff1 * (record->at(i)->GetI2());

  auto SNRToBlockErrorRate =
