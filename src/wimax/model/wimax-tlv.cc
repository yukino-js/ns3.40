...
TypeId Tlv::GetInstanceTypeId() const { return GetTypeId(); }

void Tlv::Print(std::ostream &os) const {
  os << "TLV type = " << (uint32_t)m_type
...
TlvValue *Tlv::CopyValue() const { return m_value->Copy(); }
...
uint32_t Tlv::GetSerializedSize() const {
  return 1 + GetSizeOfLen() + m_value->GetSerializedSize();
}

uint8_t Tlv::GetSizeOfLen() const {
  uint8_t sizeOfLen = 1;
...
void Tlv::Serialize(Buffer::Iterator i) const {
  i.WriteU8(m_type);
...
uint8_t Tlv::GetType() const { return m_type; }

uint64_t Tlv::GetLength() const { return m_length; }

TlvValue *Tlv::PeekValue() { return m_value; }
...
uint32_t VectorTlvValue::GetSerializedSize() const {
  uint32_t size = 0;
  for (auto iter = m_tlvList->begin(); iter != m_tlvList->end(); ++iter) {
...
void VectorTlvValue::Serialize(Buffer::Iterator i) const {
  for (auto iter = m_tlvList->begin(); iter != m_tlvList->end(); ++iter) {
    (*iter)->Serialize(i);
...
VectorTlvValue::Iterator VectorTlvValue::Begin() const {
  return m_tlvList->begin();
}

VectorTlvValue::Iterator VectorTlvValue::End() const {
...
uint32_t SfVectorTlvValue::Deserialize(Buffer::Iterator i, uint64_t valueLen) {
...
uint32_t U8TlvValue::GetSerializedSize() const { return 1; }

void U8TlvValue::Serialize(Buffer::Iterator i) const { i.WriteU8(m_value); }

uint32_t U8TlvValue::Deserialize(Buffer::Iterator i, uint64_t valueLen) {
  return Deserialize(i);
}

uint32_t U8TlvValue::Deserialize(Buffer::Iterator i) {
  m_value = i.ReadU8();
  return 1;
}

uint8_t U8TlvValue::GetValue() const { return m_value; }

U8TlvValue *U8TlvValue::Copy() const {
...
uint32_t U16TlvValue::GetSerializedSize() const { return 2; }

void U16TlvValue::Serialize(Buffer::Iterator i) const {
  i.WriteHtonU16(m_value);
}

uint32_t U16TlvValue::Deserialize(Buffer::Iterator i, uint64_t valueLen) {
  return Deserialize(i);
}

uint32_t U16TlvValue::Deserialize(Buffer::Iterator i) {
  m_value = i.ReadNtohU16();
  return 2;
}

uint16_t U16TlvValue::GetValue() const { return m_value; }

U16TlvValue *U16TlvValue::Copy() const {
...
uint32_t U32TlvValue::GetSerializedSize() const { return 4; }

void U32TlvValue::Serialize(Buffer::Iterator i) const {
  i.WriteHtonU32(m_value);
}

uint32_t U32TlvValue::Deserialize(Buffer::Iterator i, uint64_t valueLen) {
  return Deserialize(i);
}

uint32_t U32TlvValue::Deserialize(Buffer::Iterator i) {
  m_value = i.ReadNtohU32();
  return 4;
}

uint32_t U32TlvValue::GetValue() const { return m_value; }

U32TlvValue *U32TlvValue::Copy() const {
...
CsParamVectorTlvValue *CsParamVectorTlvValue::Copy() const {
...
uint32_t ClassificationRuleVectorTlvValue::Deserialize(Buffer::Iterator i,
                                                       uint64_t valueLength) {
  uint64_t serializedSize = 0;
  uint8_t lenSize = 0;
  uint8_t type = 0;
...
uint32_t TosTlvValue::GetSerializedSize() const { return 3; }

void TosTlvValue::Serialize(Buffer::Iterator i) const {
  i.WriteU8(m_low);
  i.WriteU8(m_high);
  i.WriteU8(m_mask);
}

uint32_t TosTlvValue::Deserialize(Buffer::Iterator i, uint64_t valueLength) {
  m_low = i.ReadU8();
  m_high = i.ReadU8();
  m_mask = i.ReadU8();
  return 3;
...
uint8_t TosTlvValue::GetLow() const { return m_low; }

uint8_t TosTlvValue::GetHigh() const { return m_high; }

uint8_t TosTlvValue::GetMask() const { return m_mask; }

TosTlvValue *TosTlvValue::Copy() const {
...
uint32_t PortRangeTlvValue::Deserialize(Buffer::Iterator i,
                                        uint64_t valueLength) {
  uint64_t len = 0;
...
void PortRangeTlvValue::Add(uint16_t portLow, uint16_t portHigh) {
  PortRange tmp;
  tmp.PortLow = portLow;
...
PortRangeTlvValue::Iterator PortRangeTlvValue::Begin() const {
  return m_portRange->begin();
}

PortRangeTlvValue::Iterator PortRangeTlvValue::End() const {
  return m_portRange->end();
}

PortRangeTlvValue *PortRangeTlvValue::Copy() const {
...
uint32_t ProtocolTlvValue::GetSerializedSize() const {
  return m_protocol->size();
}

void ProtocolTlvValue::Serialize(Buffer::Iterator i) const {
  for (auto iter = m_protocol->begin(); iter != m_protocol->end(); ++iter) {
    i.WriteU8((*iter));
  }
}

uint32_t ProtocolTlvValue::Deserialize(Buffer::Iterator i,
                                       uint64_t valueLength) {
  uint64_t len = 0;
  while (len < valueLength) {
    Add(i.ReadU8());
...
void ProtocolTlvValue::Add(uint8_t protocol) {
  m_protocol->push_back(protocol);
}

ProtocolTlvValue::Iterator ProtocolTlvValue::Begin() const {
  return m_protocol->begin();
}

ProtocolTlvValue::Iterator ProtocolTlvValue::End() const {
  return m_protocol->end();
}

ProtocolTlvValue *ProtocolTlvValue::Copy() const {
...
uint32_t Ipv4AddressTlvValue::Deserialize(Buffer::Iterator i,
                                          uint64_t valueLength) {
  uint64_t len = 0;
  while (len < valueLength) {
...
void Ipv4AddressTlvValue::Add(Ipv4Address address, Ipv4Mask mask) {
  m_ipv4Addr->push_back({address, mask});
}

Ipv4AddressTlvValue::Iterator Ipv4AddressTlvValue::Begin() const {
  return m_ipv4Addr->begin();
}

Ipv4AddressTlvValue::Iterator Ipv4AddressTlvValue::End() const {
  return m_ipv4Addr->end();
}

Ipv4AddressTlvValue *Ipv4AddressTlvValue::Copy() const {
