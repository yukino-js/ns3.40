...
private:
  ...
  uint16_t m_pTxIrMax; ///< maximum transmit signal strength for initial ranging
                       ///< calculated by SS

  uint8_t m_initRangOppNumber; ///< Initial Ranging opportunity (1–255) in which
                               ///< SS transmitted the RNG_REQ
  uint8_t m_contentionRangingRetries; ///< contention ranging retries
  uint32_t
      m_rngReqFrameNumber; ///< frame number in which SS sent RNG_REQ message
  RngReq m_rngreq;         ///< rng request

  uint8_t m_dlChnlNr;   ///< indicates the channel/frequency currently the SS is
                        ///< scanning
  uint64_t m_frequency; ///< frequency on which it is currently operating, i.e.,
                        ///< where scanning was successful
