...
WimaxMacQueue::QueueElement::QueueElement()
    : m_packet(Create<Packet>()), m_hdrType(MacHeaderType()),
      m_hdr(GenericMacHeader()), m_timeStamp(Seconds(0)),
      m_fragmentation(false), m_fragmentNumber(0), m_fragmentOffset(0) {}

WimaxMacQueue::QueueElement::QueueElement(Ptr<Packet> packet,
                                          const MacHeaderType &hdrType,
                                          const GenericMacHeader &hdr,
                                          Time timeStamp)
    : m_packet(packet), m_hdrType(hdrType), m_hdr(hdr), m_timeStamp(timeStamp),
      m_fragmentation(false), m_fragmentNumber(0), m_fragmentOffset(0) {}
...
TypeId WimaxMacQueue::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WimaxMacQueue")
          .SetParent<Object>()
          .SetGroupName("Wimax")
          .AddAttribute("MaxSize", "Maximum size", UintegerValue(1024),
          ...
          .AddTraceSource(
              ...
              "ns3::Packet::TracedCallback")
          .AddTraceSource("Drop", "Drop trace",
...
WimaxMacQueue::WimaxMacQueue()
    : m_maxSize(0), m_bytes(0), m_nrDataPackets(0), m_nrRequestPackets(0) {}
...
void WimaxMacQueue::SetMaxSize(uint32_t maxSize) { m_maxSize = maxSize; }

uint32_t WimaxMacQueue::GetMaxSize() const { return m_maxSize; }

bool WimaxMacQueue::Enqueue(Ptr<Packet> packet, const MacHeaderType &hdrType,
                            const GenericMacHeader &hdr) {
...
Ptr<Packet> WimaxMacQueue::Dequeue(MacHeaderType::HeaderType packetType) {
  if (!IsEmpty()) {
    ...
    if (!element.m_fragmentation) {
      ...
      Ptr<Packet> fragment =
          packet->CreateFragment(fragmentOffset, fragmentSize);
...
Ptr<Packet> WimaxMacQueue::Dequeue(MacHeaderType::HeaderType packetType,
  ...
  if (!IsEmpty()) {
    ...
    NS_LOG_INFO("\t Create a fragment"
                ...
                << std::endl);
    Ptr<Packet> fragment =
        packet->CreateFragment(fragmentOffset, maxFragmentSize);
...
Ptr<Packet> WimaxMacQueue::Peek(GenericMacHeader &hdr, Time &timeStamp) const {
  if (!IsEmpty()) {
    QueueElement element = m_queue.front();
    hdr = element.m_hdr;
...
Ptr<Packet> WimaxMacQueue::Peek(MacHeaderType::HeaderType packetType) const {
  if (!IsEmpty()) {
    QueueElement element = Front(packetType);
    Ptr<Packet> packet = element.m_packet->Copy();
...
Ptr<Packet> WimaxMacQueue::Peek(MacHeaderType::HeaderType packetType,
...
uint32_t WimaxMacQueue::GetSize() const { return m_queue.size(); }

uint32_t WimaxMacQueue::GetNBytes() const { return m_bytes; }
...
void WimaxMacQueue::Pop(MacHeaderType::HeaderType packetType) {
  QueueElement element;
...
bool WimaxMacQueue::IsEmpty() const { return m_queue.empty(); }

bool WimaxMacQueue::IsEmpty(MacHeaderType::HeaderType packetType) const {
...
const WimaxMacQueue::PacketQueue &WimaxMacQueue::GetPacketQueue() const {
  return m_queue;
}
...
WimaxMacQueue::GetFirstPacketHdrSize(MacHeaderType::HeaderType packetType) {
  ...
  if (element.m_hdrType.GetType() == MacHeaderType::HEADER_TYPE_GENERIC) {
    ...
    NS_LOG_INFO(
        "\t\t\t m_hdr.GetSerializedSize=" << element.m_hdr.GetSerializedSize());
  }
...
uint32_t WimaxMacQueue::GetFirstPacketRequiredByte(
    MacHeaderType::HeaderType packetType) {
...
void WimaxMacQueue::SetFragmentation(MacHeaderType::HeaderType packetType) {
  for (auto iter = m_queue.begin(); iter != m_queue.end(); ++iter) {
...
void WimaxMacQueue::SetFragmentNumber(MacHeaderType::HeaderType packetType) {
  for (auto iter = m_queue.begin(); iter != m_queue.end(); ++iter) {
    if (iter->m_hdrType.GetType() == packetType) {
...
void WimaxMacQueue::SetFragmentOffset(MacHeaderType::HeaderType packetType,
                                      uint32_t offset) {
  for (auto iter = m_queue.begin(); iter != m_queue.end(); ++iter) {
    if (iter->m_hdrType.GetType() == packetType) {
...
void WimaxMacQueue::QueueElement::SetFragmentation() { m_fragmentation = true; }

void WimaxMacQueue::QueueElement::SetFragmentNumber() { m_fragmentNumber++; }

void WimaxMacQueue::QueueElement::SetFragmentOffset(uint32_t offset) {
