...
void BsServiceFlowManager::DoDispose() { ServiceFlowManager::DoDispose(); }

void BsServiceFlowManager::SetMaxDsaRspRetries(uint8_t maxDsaRspRetries) {
  m_maxDsaRspRetries = maxDsaRspRetries;
}

uint8_t BsServiceFlowManager::GetMaxDsaRspRetries() const {
  return m_maxDsaRspRetries;
}

EventId BsServiceFlowManager::GetDsaAckTimeoutEvent() const {
  return m_dsaAckTimeoutEvent;
}

void BsServiceFlowManager::AddServiceFlow(ServiceFlow *serviceFlow) {
  ServiceFlowManager::AddServiceFlow(serviceFlow);
}

ServiceFlow *BsServiceFlowManager::GetServiceFlow(uint32_t sfid) const {
  return ServiceFlowManager::GetServiceFlow(sfid);
}

ServiceFlow *BsServiceFlowManager::GetServiceFlow(Cid cid) const {
  return ServiceFlowManager::GetServiceFlow(cid);
}

std::vector<ServiceFlow *> BsServiceFlowManager::GetServiceFlows(
    ServiceFlow::SchedulingType schedulingType) const {
  return ServiceFlowManager::GetServiceFlows(schedulingType);
}

DsaRsp BsServiceFlowManager::CreateDsaRsp(const ServiceFlow *serviceFlow,
...
void BsServiceFlowManager::ScheduleDsaRsp(ServiceFlow *serviceFlow, Cid cid) {
  ...
  p->AddHeader(
      ManagementMessageType(ManagementMessageType::MESSAGE_TYPE_DSA_RSP));
  ...
  m_dsaAckTimeoutEvent = Simulator::Schedule(
      bs->GetIntervalT8(), &BsServiceFlowManager::ScheduleDsaRsp, this,
      serviceFlow, cid);
  m_device->Enqueue(p, MacHeaderType(),
                    bs->GetConnection(ssRecord->GetPrimaryCid()));
}

ServiceFlow *BsServiceFlowManager::ProcessDsaReq(const DsaReq &dsaReq,
                                                 Cid cid) {
  ...
  if (ssRecord->GetSfTransactionId() != 0) {
    ...
    NS_LOG_INFO("BsServiceFlowManager: Creating a new Service flow: SFID = "
                << serviceFlow->GetSfid()
                << " CID = " << serviceFlow->GetCid());
  }
  return serviceFlow;
}

void BsServiceFlowManager::AddMulticastServiceFlow(
    ServiceFlow sf, WimaxPhy::ModulationType modulation) {
...
void BsServiceFlowManager::AllocateServiceFlows(const DsaReq &dsaReq, Cid cid) {
...
void BsServiceFlowManager::ProcessDsaAck(const DsaAck &dsaAck, Cid cid) {
  Ptr<BaseStationNetDevice> bs = m_device->GetObject<BaseStationNetDevice>();
  SSRecord *ssRecord = bs->GetSSManager()->GetSSRecord(cid);
