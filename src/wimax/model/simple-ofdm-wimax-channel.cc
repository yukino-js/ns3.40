...
void SimpleOfdmWimaxChannel::SetPropagationModel(PropModel propModel) {
  switch (propModel) {
  case RANDOM_PROPAGATION:
    m_loss = CreateObject<RandomPropagationLossModel>();
    break;
...
void SimpleOfdmWimaxChannel::DoAttach(Ptr<WimaxPhy> phy) {
  Ptr<SimpleOfdmWimaxPhy> o_phy = phy->GetObject<SimpleOfdmWimaxPhy>();
  m_phyList.push_back(o_phy);
}

std::size_t SimpleOfdmWimaxChannel::DoGetNDevices() const {
  return m_phyList.size();
}

Ptr<NetDevice> SimpleOfdmWimaxChannel::DoGetDevice(std::size_t index) const {
  std::size_t j = 0;
...
void SimpleOfdmWimaxChannel::Send(Time BlockTime, uint32_t burstSize,
                                  Ptr<WimaxPhy> phy, bool isFirstBlock,
                                  bool isLastBlock, uint64_t frequency,
  ...
  for (auto iter = m_phyList.begin(); iter != m_phyList.end(); ++iter) {
    ...
    if (phy != *iter) {
      double distance = 0;
      receiverMobility =
          (*iter)->GetDevice()->GetNode()->GetObject<MobilityModel>();
      ...
      param =
          new SimpleOfdmSendParam(burstSize, isFirstBlock, frequency,
                                  modulationType, direction, rxPowerDbm, burst);
...
void SimpleOfdmWimaxChannel::EndSendDummyBlock(Ptr<SimpleOfdmWimaxPhy> rxphy,
                                               SimpleOfdmSendParam *param) {
  rxphy->StartReceive(param->GetBurstSize(), param->GetIsFirstBlock(),
                      param->GetFrequency(), param->GetModulationType(),
                      param->GetDirection(), param->GetRxPowerDbm(),
                      param->GetBurst());
  delete param;
}

int64_t SimpleOfdmWimaxChannel::AssignStreams(int64_t stream) {
