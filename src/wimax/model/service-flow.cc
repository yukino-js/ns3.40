...
ServiceFlow::ServiceFlow()
    : m_sfid(0), m_direction(SF_DIRECTION_DOWN), m_type(SF_TYPE_PROVISIONED),
      m_connection(nullptr), m_isEnabled(false),
      m_record(new ServiceFlowRecord()) {
  InitValues();
  m_isMulticast = false;
  m_modulationType = WimaxPhy::MODULATION_TYPE_QPSK_12;
...
ServiceFlow::ServiceFlow(uint32_t sfid, Direction direction,
...
ServiceFlow::ServiceFlow(Tlv tlv) {
  ...
  for (auto iter = param->Begin(); iter != param->End(); ++iter) {
    ...
    case SfVectorTlvValue::Service_Flow_Scheduling_Type: {
      m_schedulingType =
          (ServiceFlow::SchedulingType)((U8TlvValue *)((*iter)->PeekValue()))
              ->GetValue();
      break;
    }
...
void ServiceFlow::SetDirection(Direction direction) { m_direction = direction; }

ServiceFlow::Direction ServiceFlow::GetDirection() const { return m_direction; }

void ServiceFlow::SetType(Type type) { m_type = type; }

ServiceFlow::Type ServiceFlow::GetType() const { return m_type; }

void ServiceFlow::SetConnection(Ptr<WimaxConnection> connection) {
  m_connection = connection;
  m_connection->SetServiceFlow(this);
}

Ptr<WimaxConnection> ServiceFlow::GetConnection() const { return m_connection; }

void ServiceFlow::SetIsEnabled(bool isEnabled) { m_isEnabled = isEnabled; }

bool ServiceFlow::GetIsEnabled() const { return m_isEnabled; }

void ServiceFlow::SetRecord(ServiceFlowRecord *record) { m_record = record; }

ServiceFlowRecord *ServiceFlow::GetRecord() const { return m_record; }

Ptr<WimaxMacQueue> ServiceFlow::GetQueue() const {
  if (!m_connection) {
    return nullptr;
  }
...
ServiceFlow::SchedulingType ServiceFlow::GetSchedulingType() const {
  return m_schedulingType;
}

bool ServiceFlow::HasPackets() const {
  if (!m_connection) {
    return false;
  }
...
bool ServiceFlow::HasPackets(MacHeaderType::HeaderType packetType) const {
  if (!m_connection) {
    return false;
  }
...
void ServiceFlow::PrintQoSParameters() const {}

// ==============================================================================

uint32_t ServiceFlow::GetSfid() const { return m_sfid; }

uint16_t ServiceFlow::GetCid() const {
  if (!m_connection) {
    return 0;
  }
...
std::string ServiceFlow::GetServiceClassName() const {
  return m_serviceClassName;
}

uint8_t ServiceFlow::GetQosParamSetType() const { return m_qosParamSetType; }

uint8_t ServiceFlow::GetTrafficPriority() const { return m_trafficPriority; }

uint32_t ServiceFlow::GetMaxSustainedTrafficRate() const {
  return m_maxSustainedTrafficRate;
}

uint32_t ServiceFlow::GetMaxTrafficBurst() const { return m_maxTrafficBurst; }

uint32_t ServiceFlow::GetMinReservedTrafficRate() const {
  return m_minReservedTrafficRate;
}

uint32_t ServiceFlow::GetMinTolerableTrafficRate() const {
...
uint32_t ServiceFlow::GetRequestTransmissionPolicy() const {
  return m_requestTransmissionPolicy;
}

uint32_t ServiceFlow::GetToleratedJitter() const { return m_toleratedJitter; }

uint32_t ServiceFlow::GetMaximumLatency() const { return m_maximumLatency; }

uint8_t ServiceFlow::GetFixedversusVariableSduIndicator() const {
  return m_fixedversusVariableSduIndicator;
}

uint8_t ServiceFlow::GetSduSize() const { return m_sduSize; }

uint16_t ServiceFlow::GetTargetSAID() const { return m_targetSAID; }

uint8_t ServiceFlow::GetArqEnable() const { return m_arqEnable; }

uint16_t ServiceFlow::GetArqWindowSize() const { return m_arqWindowSize; }

uint16_t ServiceFlow::GetArqRetryTimeoutTx() const {
  return m_arqRetryTimeoutTx;
}

uint16_t ServiceFlow::GetArqRetryTimeoutRx() const {
  return m_arqRetryTimeoutRx;
}

uint16_t ServiceFlow::GetArqBlockLifeTime() const { return m_arqBlockLifeTime; }

uint16_t ServiceFlow::GetArqSyncLoss() const { return m_arqSyncLoss; }

uint8_t ServiceFlow::GetArqDeliverInOrder() const {
  return m_arqDeliverInOrder;
}

uint16_t ServiceFlow::GetArqPurgeTimeout() const { return m_arqPurgeTimeout; }

uint16_t ServiceFlow::GetArqBlockSize() const { return m_arqBlockSize; }

ServiceFlow::CsSpecification ServiceFlow::GetCsSpecification() const {
  return m_csSpecification;
}

CsParameters ServiceFlow::GetConvergenceSublayerParam() const {
  return m_convergenceSublayerParam;
}

uint16_t ServiceFlow::GetUnsolicitedGrantInterval() const {
  return m_unsolicitedGrantInterval;
}

uint16_t ServiceFlow::GetUnsolicitedPollingInterval() const {
  return m_unsolicitedPollingInterval;
}

bool ServiceFlow::GetIsMulticast() const { return m_isMulticast; }

WimaxPhy::ModulationType ServiceFlow::GetModulation() const {
  return m_modulationType;
}

// ==============================================================================

void ServiceFlow::SetSfid(uint32_t sfid) { m_sfid = sfid; }

void ServiceFlow::SetServiceClassName(std::string name) {
  m_serviceClassName = name;
}

void ServiceFlow::SetQosParamSetType(uint8_t type) { m_qosParamSetType = type; }

void ServiceFlow::SetTrafficPriority(uint8_t priority) {
  m_trafficPriority = priority;
}

void ServiceFlow::SetMaxSustainedTrafficRate(uint32_t maxSustainedRate) {
  m_maxSustainedTrafficRate = maxSustainedRate;
}

void ServiceFlow::SetMaxTrafficBurst(uint32_t maxTrafficBurst) {
  m_maxTrafficBurst = maxTrafficBurst;
}

void ServiceFlow::SetMinReservedTrafficRate(uint32_t minResvRate) {
  m_minReservedTrafficRate = minResvRate;
}

void ServiceFlow::SetMinTolerableTrafficRate(uint32_t minJitter) {
  m_minTolerableTrafficRate = minJitter;
}

void ServiceFlow::SetServiceSchedulingType(
    ServiceFlow::SchedulingType schedType) {
  m_schedulingType = schedType;
}

void ServiceFlow::SetRequestTransmissionPolicy(uint32_t policy) {
  m_requestTransmissionPolicy = policy;
}

void ServiceFlow::SetToleratedJitter(uint32_t jitter) {
  m_toleratedJitter = jitter;
}

void ServiceFlow::SetMaximumLatency(uint32_t MaximumLatency) {
  m_maximumLatency = MaximumLatency;
}

void ServiceFlow::SetFixedversusVariableSduIndicator(uint8_t sduIndicator) {
  m_fixedversusVariableSduIndicator = sduIndicator;
}

void ServiceFlow::SetSduSize(uint8_t sduSize) { m_sduSize = sduSize; }

void ServiceFlow::SetTargetSAID(uint16_t targetSaid) {
  m_targetSAID = targetSaid;
}

void ServiceFlow::SetArqEnable(uint8_t arqEnable) { m_arqEnable = arqEnable; }

void ServiceFlow::SetArqWindowSize(uint16_t arqWindowSize) {
  m_arqWindowSize = arqWindowSize;
}

void ServiceFlow::SetArqRetryTimeoutTx(uint16_t timeout) {
  m_arqRetryTimeoutTx = timeout;
}

void ServiceFlow::SetArqRetryTimeoutRx(uint16_t timeout) {
  m_arqRetryTimeoutRx = timeout;
}

void ServiceFlow::SetArqBlockLifeTime(uint16_t lifeTime) {
  m_arqBlockLifeTime = lifeTime;
}

void ServiceFlow::SetArqSyncLoss(uint16_t syncLoss) {
  m_arqSyncLoss = syncLoss;
}

void ServiceFlow::SetArqDeliverInOrder(uint8_t inOrder) {
  m_arqDeliverInOrder = inOrder;
}

void ServiceFlow::SetArqPurgeTimeout(uint16_t timeout) {
  m_arqPurgeTimeout = timeout;
}

void ServiceFlow::SetArqBlockSize(uint16_t size) { m_arqBlockSize = size; }

void ServiceFlow::SetCsSpecification(ServiceFlow::CsSpecification spec) {
  m_csSpecification = spec;
}

void ServiceFlow::SetConvergenceSublayerParam(CsParameters csparam) {
  m_convergenceSublayerParam = csparam;
}

void ServiceFlow::SetUnsolicitedGrantInterval(
    uint16_t unsolicitedGrantInterval) {
  m_unsolicitedGrantInterval = unsolicitedGrantInterval;
}

void ServiceFlow::SetUnsolicitedPollingInterval(
    uint16_t unsolicitedPollingInterval) {
  m_unsolicitedPollingInterval = unsolicitedPollingInterval;
}

void ServiceFlow::SetIsMulticast(bool isMulticast) {
  m_isMulticast = isMulticast;
}

void ServiceFlow::SetModulation(WimaxPhy::ModulationType modulationType) {
...
void ServiceFlow::CopyParametersFrom(ServiceFlow sf) {
...
ServiceFlow &ServiceFlow::operator=(const ServiceFlow &o) {
  m_sfid = o.GetSfid();
...
char *ServiceFlow::GetSchedulingTypeStr() const {
...
Tlv ServiceFlow::ToTlv() const {
  ...
  tmpSfVector.Add(Tlv(SfVectorTlvValue::QoS_Parameter_Set_Type, 1,
                      U8TlvValue(m_qosParamSetType)));
  tmpSfVector.Add(Tlv(SfVectorTlvValue::Traffic_Priority, 1,
  ...
  tmpSfVector.Add(Tlv(SfVectorTlvValue::Maximum_Traffic_Burst, 4,
  ...
  tmpSfVector.Add(Tlv(SfVectorTlvValue::Service_Flow_Scheduling_Type, 1,
  ...
  tmpSfVector.Add(Tlv(SfVectorTlvValue::Tolerated_Jitter, 4,
                      U32TlvValue(m_toleratedJitter)));
  tmpSfVector.Add(
      Tlv(SfVectorTlvValue::Maximum_Latency, 4, U32TlvValue(m_maximumLatency)));
  tmpSfVector.Add(
      Tlv(SfVectorTlvValue::Fixed_length_versus_Variable_length_SDU_Indicator,
          1, U8TlvValue(m_fixedversusVariableSduIndicator)));
  tmpSfVector.Add(Tlv(SfVectorTlvValue::SDU_Size, 1, U8TlvValue(m_sduSize)));
  tmpSfVector.Add(
      Tlv(SfVectorTlvValue::Target_SAID, 2, U16TlvValue(m_targetSAID)));
  tmpSfVector.Add(Tlv(SfVectorTlvValue::CS_Specification, 1,
                      U8TlvValue(m_csSpecification)));
  tmpSfVector.Add(m_convergenceSublayerParam.ToTlv());
  if (m_direction == SF_DIRECTION_UP) {
    return Tlv(Tlv::UPLINK_SERVICE_FLOW, tmpSfVector.GetSerializedSize(),
               tmpSfVector);
  } else {
    return Tlv(Tlv::DOWNLINK_SERVICE_FLOW, tmpSfVector.GetSerializedSize(),
               tmpSfVector);
  }
}

bool ServiceFlow::CheckClassifierMatch(Ipv4Address srcAddress,
                                       Ipv4Address dstAddress, uint16_t srcPort,
                                       uint16_t dstPort, uint8_t proto) const {
  return m_convergenceSublayerParam.GetPacketClassifierRule().CheckMatch(
      srcAddress, dstAddress, srcPort, dstPort, proto);
}
} // namespace ns3
