...
MacHeaderType::MacHeaderType() : m_type(0) {}

MacHeaderType::MacHeaderType(uint8_t type) : m_type(type) {}

MacHeaderType::~MacHeaderType() {}

void MacHeaderType::SetType(uint8_t type) { m_type = type; }

uint8_t MacHeaderType::GetType() const { return m_type; }

std::string MacHeaderType::GetName() const { return "MAC Header Type"; }

TypeId MacHeaderType::GetTypeId() {
...
TypeId MacHeaderType::GetInstanceTypeId() const { return GetTypeId(); }

void MacHeaderType::Print(std::ostream &os) const {
  os << " header type = " << (uint32_t)m_type;
}

uint32_t MacHeaderType::GetSerializedSize() const { return 0; }
...
GenericMacHeader::GenericMacHeader()
    : m_ht(0), m_ec(0), m_type(0), m_ci(0), m_eks(0), m_len(0), m_cid(Cid()) {
  m_esf = 0;
  m_hcs = 0;
  m_rsv1 = 0;
  c_hcs = 0;
...
void GenericMacHeader::SetHt(uint8_t ht) { m_ht = ht; }

void GenericMacHeader::SetEc(uint8_t ec) { m_ec = ec; }

void GenericMacHeader::SetType(uint8_t type) { m_type = type; }

void GenericMacHeader::SetCi(uint8_t ci) { m_ci = ci; }

void GenericMacHeader::SetEks(uint8_t eks) { m_eks = eks; }

void GenericMacHeader::SetLen(uint16_t len) { m_len = len; }

void GenericMacHeader::SetCid(Cid cid) { m_cid = cid; }

void GenericMacHeader::SetHcs(uint8_t hcs) { m_hcs = hcs; }

uint8_t GenericMacHeader::GetHt() const { return m_ht; }

uint8_t GenericMacHeader::GetEc() const { return m_ec; }

uint8_t GenericMacHeader::GetType() const { return m_type; }

uint8_t GenericMacHeader::GetCi() const { return m_ci; }

uint8_t GenericMacHeader::GetEks() const { return m_eks; }

uint16_t GenericMacHeader::GetLen() const { return m_len; }

Cid GenericMacHeader::GetCid() const { return m_cid; }

uint8_t GenericMacHeader::GetHcs() const { return m_hcs; }

std::string GenericMacHeader::GetName() const { return "Generic Mac Header"; }
...
void GenericMacHeader::Serialize(Buffer::Iterator start) const {
  ...
  headerBuffer[0] =
      ((m_ht << 7) & 0x80) | ((m_ec << 6) & 0x40) | (m_type & 0x3F);
  headerBuffer[1] = ((m_esf << 7) & 0x80) | ((m_ci << 6) & 0x40) |
                    ((m_eks << 4) & 0x30) | ((m_rsv1 << 3) & 0x08) |
...
BandwidthRequestHeader::BandwidthRequestHeader()
    : m_ht(1), m_ec(0), m_type(0), m_br(0), m_cid(Cid()), m_hcs(0) {}

BandwidthRequestHeader::~BandwidthRequestHeader() {}

void BandwidthRequestHeader::SetHt(uint8_t ht) { m_ht = ht; }

void BandwidthRequestHeader::SetEc(uint8_t ec) { m_ec = ec; }

void BandwidthRequestHeader::SetType(uint8_t type) { m_type = type; }

void BandwidthRequestHeader::SetBr(uint32_t br) { m_br = br; }

void BandwidthRequestHeader::SetCid(Cid cid) { m_cid = cid; }

void BandwidthRequestHeader::SetHcs(uint8_t hcs) { m_hcs = hcs; }

uint8_t BandwidthRequestHeader::GetHt() const { return m_ht; }

uint8_t BandwidthRequestHeader::GetEc() const { return m_ec; }

uint8_t BandwidthRequestHeader::GetType() const { return m_type; }

uint32_t BandwidthRequestHeader::GetBr() const { return m_br; }

Cid BandwidthRequestHeader::GetCid() const { return m_cid; }

uint8_t BandwidthRequestHeader::GetHcs() const { return m_hcs; }
...
TypeId BandwidthRequestHeader::GetInstanceTypeId() const { return GetTypeId(); }

void BandwidthRequestHeader::Print(std::ostream &os) const {
  os << " ec (encryption control) = " << (uint32_t)m_ec
     << ", type = " << (uint32_t)m_type << ", br (bandwidth request) = " << m_br
     << ", cid = ";
  m_cid.GetIdentifier();
...
GrantManagementSubheader::GrantManagementSubheader()
    : m_si(0), m_pm(0), m_pbr(0) {}

GrantManagementSubheader::~GrantManagementSubheader() {}

void GrantManagementSubheader::SetSi(uint8_t si) { m_si = si; }

void GrantManagementSubheader::SetPm(uint8_t pm) { m_pm = pm; }

void GrantManagementSubheader::SetPbr(uint16_t pbr) { m_pbr = pbr; }

uint8_t GrantManagementSubheader::GetSi() const { return m_si; }

uint8_t GrantManagementSubheader::GetPm() const { return m_pm; }

uint16_t GrantManagementSubheader::GetPbr() const { return m_pbr; }
...
void GrantManagementSubheader::Print(std::ostream &os) const {
  os << " si (slip indicator) = " << (uint32_t)m_si
...
FragmentationSubheader::FragmentationSubheader() : m_fc(0), m_fsn(0) {}

FragmentationSubheader::~FragmentationSubheader() {}

void FragmentationSubheader::SetFc(uint8_t fc) { m_fc = fc; }

void FragmentationSubheader::SetFsn(uint8_t fsn) { m_fsn = fsn; }

uint8_t FragmentationSubheader::GetFc() const { return m_fc; }

uint8_t FragmentationSubheader::GetFsn() const { return m_fsn; }

std::string FragmentationSubheader::GetName() const {
  return "Fragmentation Subheader";
}

TypeId FragmentationSubheader::GetTypeId() {
...
TypeId FragmentationSubheader::GetInstanceTypeId() const { return GetTypeId(); }

void FragmentationSubheader::Print(std::ostream &os) const {
  os << " fc (fragment control) = " << (uint32_t)m_fc
     << ", fsn (fragmentation sequence number) = " << (uint32_t)m_fsn << "\n";
}

uint32_t FragmentationSubheader::GetSerializedSize() const { return 2; }

void FragmentationSubheader::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
  i.WriteU8(m_fc);
  i.WriteU8(m_fsn);
}

uint32_t FragmentationSubheader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  m_fc = i.ReadU8();
  m_fsn = i.ReadU8();
