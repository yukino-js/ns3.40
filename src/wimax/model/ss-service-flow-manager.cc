...
SsServiceFlowManager::SsServiceFlowManager(
    Ptr<SubscriberStationNetDevice> device)
    : m_device(device), m_maxDsaReqRetries(100), m_dsaReq(DsaReq()),
      m_dsaAck(DsaAck()), m_currentTransactionId(0), m_transactionIdIndex(1),
      m_dsaReqRetries(0), m_pendingServiceFlow(nullptr) {}
...
void SsServiceFlowManager::DoDispose() { ServiceFlowManager::DoDispose(); }

void SsServiceFlowManager::SetMaxDsaReqRetries(uint8_t maxDsaReqRetries) {
  m_maxDsaReqRetries = maxDsaReqRetries;
}

uint8_t SsServiceFlowManager::GetMaxDsaReqRetries() const {
  return m_maxDsaReqRetries;
}

EventId SsServiceFlowManager::GetDsaRspTimeoutEvent() const {
  return m_dsaRspTimeoutEvent;
}

EventId SsServiceFlowManager::GetDsaAckTimeoutEvent() const {
  return m_dsaAckTimeoutEvent;
}

void SsServiceFlowManager::AddServiceFlow(ServiceFlow serviceFlow) {
...
void SsServiceFlowManager::InitiateServiceFlows() {
...
DsaReq SsServiceFlowManager::CreateDsaReq(const ServiceFlow *serviceFlow) {
  DsaReq dsaReq;
...
Ptr<Packet> SsServiceFlowManager::CreateDsaAck() {
  DsaAck dsaAck;
  ...
  p->AddHeader(dsaAck);
  p->AddHeader(
      ManagementMessageType(ManagementMessageType::MESSAGE_TYPE_DSA_ACK));
  return p;
}

void SsServiceFlowManager::ScheduleDsaReq(const ServiceFlow *serviceFlow) {
  Ptr<Packet> p = Create<Packet>();
  DsaReq dsaReq;
  ...
  m_dsaReqRetries++;
  p->AddHeader(
      ManagementMessageType(ManagementMessageType::MESSAGE_TYPE_DSA_REQ));

  if (m_dsaRspTimeoutEvent.IsRunning()) {
    Simulator::Cancel(m_dsaRspTimeoutEvent);
  }

  m_dsaRspTimeoutEvent = Simulator::Schedule(
      ss->GetIntervalT7(), &SsServiceFlowManager::ScheduleDsaReq, this,
      serviceFlow);

  m_device->Enqueue(p, MacHeaderType(), ss->GetPrimaryConnection());
}

void SsServiceFlowManager::ProcessDsaRsp(const DsaRsp &dsaRsp) {
  Ptr<SubscriberStationNetDevice> ss =
      m_device->GetObject<SubscriberStationNetDevice>();
