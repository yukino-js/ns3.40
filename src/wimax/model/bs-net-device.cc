...
TypeId BaseStationNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::BaseStationNetDevice")
          ...
          .AddAttribute("BSScheduler", "Downlink Scheduler for BS",
                        PointerValue(),
          ...
          .AddAttribute("InitialRangInterval",
                        ...
                        MakeTimeAccessor(
                            &BaseStationNetDevice::GetInitialRangingInterval,
          ...
          .AddAttribute(
              ...
              MakeUintegerAccessor(
                  &BaseStationNetDevice::GetMaxRangingCorrectionRetries,
          ...
          .AddAttribute(
              "UplinkScheduler",
              "The uplink scheduler attached to this device.", PointerValue(),
          ...
          .AddTraceSource(
              ...
              MakeTraceSourceAccessor(
                  &BaseStationNetDevice::m_bsPromiscRxTrace),
              "ns3::Packet::TracedCallback")
...
void BaseStationNetDevice::DoDispose() {
  delete m_cidFactory;
...
void BaseStationNetDevice::SetBsClassifier(Ptr<IpcsClassifier> bsc) {
  m_bsClassifier = bsc;
}

Ptr<IpcsClassifier> BaseStationNetDevice::GetBsClassifier() const {
  return m_bsClassifier;
}

void BaseStationNetDevice::SetInitialRangingInterval(Time initialRangInterval) {
  m_initialRangInterval = initialRangInterval;
}

Time BaseStationNetDevice::GetInitialRangingInterval() const {
  return m_initialRangInterval;
}

void BaseStationNetDevice::SetDcdInterval(Time dcdInterval) {
  m_dcdInterval = dcdInterval;
}

Time BaseStationNetDevice::GetDcdInterval() const { return m_dcdInterval; }

void BaseStationNetDevice::SetUcdInterval(Time ucdInterval) {
  m_ucdInterval = ucdInterval;
}

Time BaseStationNetDevice::GetUcdInterval() const { return m_ucdInterval; }

void BaseStationNetDevice::SetIntervalT8(Time interval) {
  m_intervalT8 = interval;
}

Time BaseStationNetDevice::GetIntervalT8() const { return m_intervalT8; }

void BaseStationNetDevice::SetMaxRangingCorrectionRetries(
    uint8_t maxRangCorrectionRetries) {
  m_maxRangCorrectionRetries = maxRangCorrectionRetries;
}

uint8_t BaseStationNetDevice::GetMaxRangingCorrectionRetries() const {
...
uint8_t BaseStationNetDevice::GetMaxInvitedRangRetries() const {
  return m_maxInvitedRangRetries;
}

void BaseStationNetDevice::SetRangReqOppSize(uint8_t rangReqOppSize) {
  m_rangReqOppSize = rangReqOppSize;
}

uint8_t BaseStationNetDevice::GetRangReqOppSize() const {
  return m_rangReqOppSize;
}

void BaseStationNetDevice::SetBwReqOppSize(uint8_t bwReqOppSize) {
  m_bwReqOppSize = bwReqOppSize;
}

uint8_t BaseStationNetDevice::GetBwReqOppSize() const { return m_bwReqOppSize; }

void BaseStationNetDevice::SetNrDlSymbols(uint32_t nrDlSymbols) {
  m_nrDlSymbols = nrDlSymbols;
}

uint32_t BaseStationNetDevice::GetNrDlSymbols() const { return m_nrDlSymbols; }

void BaseStationNetDevice::SetNrUlSymbols(uint32_t nrUlSymbols) {
  m_nrUlSymbols = nrUlSymbols;
}

uint32_t BaseStationNetDevice::GetNrUlSymbols() const { return m_nrUlSymbols; }

uint32_t BaseStationNetDevice::GetNrDcdSent() const { return m_nrDcdSent; }

uint32_t BaseStationNetDevice::GetNrUcdSent() const { return m_nrUcdSent; }

Time BaseStationNetDevice::GetDlSubframeStartTime() const {
  return m_dlSubframeStartTime;
}

Time BaseStationNetDevice::GetUlSubframeStartTime() const {
  return m_ulSubframeStartTime;
}

uint8_t BaseStationNetDevice::GetRangingOppNumber() const {
  return m_rangingOppNumber;
}

Ptr<SSManager> BaseStationNetDevice::GetSSManager() const {
  return m_ssManager;
}

void BaseStationNetDevice::SetSSManager(Ptr<SSManager> ssm) {
  m_ssManager = ssm;
}

Ptr<BsServiceFlowManager> BaseStationNetDevice::GetServiceFlowManager() const {
  return m_serviceFlowManager;
}

void BaseStationNetDevice::SetServiceFlowManager(
    Ptr<BsServiceFlowManager> sfm) {
  m_serviceFlowManager = sfm;
}

Ptr<UplinkScheduler> BaseStationNetDevice::GetUplinkScheduler() const {
  return m_uplinkScheduler;
}

void BaseStationNetDevice::SetUplinkScheduler(Ptr<UplinkScheduler> uls) {
  m_uplinkScheduler = uls;
}

Ptr<BSLinkManager> BaseStationNetDevice::GetLinkManager() const {
  return m_linkManager;
}

void BaseStationNetDevice::SetLinkManager(Ptr<BSLinkManager> lm) {
  m_linkManager = lm;
}

void BaseStationNetDevice::SetBSScheduler(Ptr<BSScheduler> bss) {
  m_scheduler = bss;
}

Ptr<BSScheduler> BaseStationNetDevice::GetBSScheduler() const {
  return m_scheduler;
}

Time BaseStationNetDevice::GetPsDuration() const { return m_psDuration; }

Time BaseStationNetDevice::GetSymbolDuration() const {
  return m_symbolDuration;
}

void BaseStationNetDevice::Start() {
  SetReceiveCallback();
...
void BaseStationNetDevice::EndDlSubFrame() {
  m_nrDlFrames++;
...
void BaseStationNetDevice::StartUlSubFrame() {
...
void BaseStationNetDevice::EndUlSubFrame() {
  m_nrUlFrames++;
...
void BaseStationNetDevice::EndFrame() { StartFrame(); }

bool BaseStationNetDevice::DoSend(Ptr<Packet> packet,
...
bool BaseStationNetDevice::Enqueue(Ptr<Packet> packet,
                                   const MacHeaderType &hdrType,
                                   Ptr<WimaxConnection> connection) {
  NS_ASSERT_MSG(connection, "BS: Can not enqueue packet on the selected "
...
void BaseStationNetDevice::DoReceive(Ptr<Packet> packet) {
  ...
  if (gnrcMacHdr.GetHt() == MacHeaderType::HEADER_TYPE_GENERIC) {
    ...
    } else // transport connection
    {
      ...
      if (!fragmentation) {
        ...
        NS_LOG_INFO("FRAG_DEBUG: BS DoReceive, the Packet is a fragment"
                    << std::endl);
...
void BaseStationNetDevice::CreateMapMessages() {
  Ptr<Packet> dlmap;
  Ptr<Packet> ulmap;
  bool sendDcd = false;
  bool sendUcd = false;
  ...
  if (m_nrSsRegistered == currentNrSsRegistered) {
    m_uplinkScheduler->GetChannelDescriptorsToUpdate(updateDcd, updateUcd,
                                                     sendDcd, sendUcd);
  } else {
...
void BaseStationNetDevice::CreateDescriptorMessages(bool sendDcd,
                                                    bool sendUcd) {
  Ptr<Packet> dcd;
  Ptr<Packet> ucd;
...
void BaseStationNetDevice::SendBursts() {
  ...
  while (!downlinkBursts->empty()) {
    ...
    Simulator::Schedule(txTime, &WimaxNetDevice::ForwardDown, this, burst,
                        modulationType);
...
Ptr<Packet> BaseStationNetDevice::CreateDlMap() {
  m_nrDlAllocations = 0;
  ...
  for (auto iter = downlinkBursts->begin(); iter != downlinkBursts->end();
       ++iter) {
  ...
  p->AddHeader(dlmap);
  p->AddHeader(
      ManagementMessageType(ManagementMessageType::MESSAGE_TYPE_DL_MAP));
  return p;
}

Ptr<Packet> BaseStationNetDevice::CreateDcd() {
  Dcd dcd;
...
Ptr<Packet> BaseStationNetDevice::CreateUlMap() {
  m_ulAllocationNumber = 0;
  m_rangingOppNumber = 0;
  m_nrUlAllocations = 0;
  ...
  ulmap.SetAllocationStartTime(
      m_uplinkScheduler->CalculateAllocationStartTime());

  std::list<OfdmUlMapIe> uplinkAllocations =
      m_uplinkScheduler->GetUplinkAllocations();

  for (auto iter = uplinkAllocations.begin(); iter != uplinkAllocations.end();
       ++iter) {
  ...
  p->AddHeader(ulmap);
  p->AddHeader(
      ManagementMessageType(ManagementMessageType::MESSAGE_TYPE_UL_MAP));
  return p;
}
...
Ptr<Packet> BaseStationNetDevice::CreateUcd() {
  ...
  chnlEncodings.SetRangReqOppSize(m_rangReqOppSize *
...
void BaseStationNetDevice::SetDlBurstProfiles(Dcd *dcd) {
  for (int i = 0; i < GetBurstProfileManager()->GetNrBurstProfilesToDefine();
       ++i) {
...
void BaseStationNetDevice::SetUlBurstProfiles(Ucd *ucd) {
  for (int i = 0; i < GetBurstProfileManager()->GetNrBurstProfilesToDefine();
       ++i) {
...
Ptr<WimaxConnection> BaseStationNetDevice::GetConnection(Cid cid) {
  Ptr<WimaxConnection> connection = nullptr;
...
void BaseStationNetDevice::MarkUplinkAllocations() {
  ...
  for (auto iter = uplinkAllocations.begin(); iter != uplinkAllocations.end();
    ...
    MarkUplinkAllocationEnd(
        (symbolsToAllocation + uplinkAllocation.GetDuration()) *
            m_symbolDuration,
        uplinkAllocation.GetCid(), uplinkAllocation.GetUiuc());
  }
}

void BaseStationNetDevice::MarkUplinkAllocationStart(Time allocationStartTime) {
  Simulator::Schedule(allocationStartTime,
                      &BaseStationNetDevice::UplinkAllocationStart, this);
}

void BaseStationNetDevice::MarkUplinkAllocationEnd(Time allocationEndTime,
                                                   Cid cid, uint8_t uiuc) {
  Simulator::Schedule(allocationEndTime,
                      &BaseStationNetDevice::UplinkAllocationEnd, this, cid,
                      uiuc);
}

void BaseStationNetDevice::UplinkAllocationStart() {
  m_ulAllocationNumber++;

  NS_LOG_DEBUG("--UL allocation "
               << (uint32_t)m_ulAllocationNumber
               << " started : " << Simulator::Now().As(Time::S));
}

void BaseStationNetDevice::UplinkAllocationEnd(Cid cid, uint8_t uiuc) {
  NS_LOG_DEBUG("--UL allocation "
               << (uint32_t)m_ulAllocationNumber
...
void BaseStationNetDevice::MarkRangingOppStart(Time rangingOppStartTime) {
  Simulator::Schedule(rangingOppStartTime,
                      &BaseStationNetDevice::RangingOppStart, this);
}

void BaseStationNetDevice::RangingOppStart() {
  m_rangingOppNumber++;
