...
IpcsClassifierRecord::IpcsClassifierRecord(
    Ipv4Address SrcAddress, Ipv4Mask SrcMask, Ipv4Address DstAddress,
...
void IpcsClassifierRecord::AddSrcAddr(Ipv4Address srcAddress,
                                      Ipv4Mask srcMask) {
  Ipv4Addr tmp;
...
void IpcsClassifierRecord::AddDstAddr(Ipv4Address dstAddress,
                                      Ipv4Mask dstMask) {
  Ipv4Addr tmp;
...
void IpcsClassifierRecord::AddSrcPortRange(uint16_t srcPortLow,
...
void IpcsClassifierRecord::AddDstPortRange(uint16_t dstPortLow,
...
void IpcsClassifierRecord::AddProtocol(uint8_t proto) {
  m_protocol.push_back(proto);
}

void IpcsClassifierRecord::SetPriority(uint8_t prio) { m_priority = prio; }

void IpcsClassifierRecord::SetCid(uint16_t cid) { m_cid = cid; }

void IpcsClassifierRecord::SetIndex(uint16_t index) { m_index = index; }

uint16_t IpcsClassifierRecord::GetIndex() const { return m_index; }

uint16_t IpcsClassifierRecord::GetCid() const { return m_cid; }

uint8_t IpcsClassifierRecord::GetPriority() const { return m_priority; }

bool IpcsClassifierRecord::CheckMatchSrcAddr(Ipv4Address srcAddress) const {
...
bool IpcsClassifierRecord::CheckMatchDstAddr(Ipv4Address dstAddress) const {
...
bool IpcsClassifierRecord::CheckMatchSrcPort(uint16_t port) const {
  for (auto iter = m_srcPortRange.begin(); iter != m_srcPortRange.end();
       ++iter) {
    NS_LOG_INFO("src port check match: pkt=" << port << " cls= ["
                                             << (*iter).PortLow << " TO "
                                             << (*iter).PortHigh << "]");
...
bool IpcsClassifierRecord::CheckMatchDstPort(uint16_t port) const {
  for (auto iter = m_dstPortRange.begin(); iter != m_dstPortRange.end();
       ++iter) {
    NS_LOG_INFO("dst port check match: pkt=" << port << " cls= ["
                                             << (*iter).PortLow << " TO "
                                             << (*iter).PortHigh << "]");
    if (port >= (*iter).PortLow && port <= (*iter).PortHigh) {
      return true;
...
bool IpcsClassifierRecord::CheckMatchProtocol(uint8_t proto) const {
...
bool IpcsClassifierRecord::CheckMatch(Ipv4Address srcAddress,
                                      Ipv4Address dstAddress, uint16_t srcPort,
                                      uint16_t dstPort, uint8_t proto) const {
  return (CheckMatchProtocol(proto) && CheckMatchDstPort(dstPort) &&
          CheckMatchSrcPort(srcPort) && CheckMatchDstAddr(dstAddress) &&
          CheckMatchSrcAddr(srcAddress));
}

Tlv IpcsClassifierRecord::ToTlv() const {
  ...
  ClassVectVal.Add(Tlv(ClassificationRuleVectorTlvValue::Priority, 1,
                       U8TlvValue(m_priority)));
  ClassVectVal.Add(Tlv(ClassificationRuleVectorTlvValue::Protocol,
  ...
  ClassVectVal.Add(
      Tlv(ClassificationRuleVectorTlvValue::Index, 2, U16TlvValue(1)));
