...
namespace ns3 {
CsParameters::CsParameters() { m_classifierDscAction = CsParameters::ADD; }
  ...
  for (auto iter = param->Begin(); iter != param->End(); ++iter) {
    ...
    case CsParamVectorTlvValue::Classifier_DSC_Action: {
      m_classifierDscAction =
          (CsParameters::Action)((U8TlvValue *)((*iter)->PeekValue()))
              ->GetValue();
      break;
    }
...
void CsParameters::SetClassifierDscAction(CsParameters::Action action) {
  m_classifierDscAction = action;
}

void CsParameters::SetPacketClassifierRule(
    IpcsClassifierRecord packetClassifierRule) {
  m_packetClassifierRule = packetClassifierRule;
}

CsParameters::Action CsParameters::GetClassifierDscAction() const {
  return m_classifierDscAction;
}

IpcsClassifierRecord CsParameters::GetPacketClassifierRule() const {
  return m_packetClassifierRule;
}

Tlv CsParameters::ToTlv() const {
  CsParamVectorTlvValue tmp;
  tmp.Add(Tlv(CsParamVectorTlvValue::Classifier_DSC_Action, 1,
              U8TlvValue(m_classifierDscAction)));
  tmp.Add(m_packetClassifierRule.ToTlv());
  return Tlv(SfVectorTlvValue::IPV4_CS_Parameters, tmp.GetSerializedSize(),
             tmp);
}
} // namespace ns3
