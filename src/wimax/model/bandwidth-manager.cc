...
TypeId BandwidthManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::BandwidthManager").SetParent<Object>().SetGroupName("Wimax");
  return tid;
}

BandwidthManager::BandwidthManager(Ptr<WimaxNetDevice> device)
    : m_device(device), m_nrBwReqsSent(0) {}

BandwidthManager::~BandwidthManager() {}

void BandwidthManager::DoDispose() { m_device = nullptr; }

uint32_t
BandwidthManager::CalculateAllocationSize(const SSRecord *ssRecord,
  ...
  case ServiceFlow::SF_TYPE_UGS: {
    if ((currentTime - serviceFlow->GetRecord()->GetGrantTimeStamp())
            .GetMilliSeconds() >= serviceFlow->GetUnsolicitedGrantInterval()) {
...
ServiceFlow *BandwidthManager::SelectFlowForRequest(uint32_t &bytesToRequest) {
  Ptr<Packet> packet;
...
void BandwidthManager::SendBandwidthRequest(uint8_t uiuc,
  ...
  ss->Enqueue(packet, MacHeaderType(MacHeaderType::HEADER_TYPE_BANDWIDTH),
  ...
  ss->SendBurst(uiuc, allocationSize, serviceFlow->GetConnection(),
                MacHeaderType::HEADER_TYPE_BANDWIDTH);
}

void BandwidthManager::ProcessBandwidthRequest(
    const BandwidthRequestHeader &bwRequestHdr) {
  Ptr<BaseStationNetDevice> bs = m_device->GetObject<BaseStationNetDevice>();

  ServiceFlow *serviceFlow = bs->GetConnectionManager()
                                 ->GetConnection(bwRequestHdr.GetCid())
                                 ->GetServiceFlow();
  if (bwRequestHdr.GetType() ==
      (uint8_t)BandwidthRequestHeader::HEADER_TYPE_INCREMENTAL) {
...
uint32_t BandwidthManager::GetSymbolsPerFrameAllocated() {
  Ptr<BaseStationNetDevice> bs = m_device->GetObject<BaseStationNetDevice>();
  ...
  for (auto iter1 = ssRecords->begin(); iter1 != ssRecords->end(); ++iter1) {
    ...
    for (auto iter2 = ssServiceFlows.begin(); iter2 != ssServiceFlows.end();
         ++iter2) {
