...
TypeId IpcsClassifier::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::IpcsClassifier").SetParent<Object>().SetGroupName("Wimax");
  return tid;
}
...
ServiceFlow *IpcsClassifier::Classify(Ptr<const Packet> packet,
  ...
  NS_LOG_INFO("Classifing packet: src_addr="
              << source_address << " dst_addr=" << dest_address
              << " src_port=" << sourcePort << " dst_port=" << destPort
              << " proto=" << (uint16_t)protocol);
  return (sfm->DoClassify(source_address, dest_address, sourcePort, destPort,
                          protocol, dir));
}
