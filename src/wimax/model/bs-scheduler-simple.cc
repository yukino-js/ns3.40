...
void BSSchedulerSimple::AddDownlinkBurst(
    Ptr<const WimaxConnection> connection, uint8_t diuc,
...
void BSSchedulerSimple::Schedule() {
  ...
  while (SelectConnection(connection)) {
    ...
    if (connection->GetType() == Cid::TRANSPORT ||
        connection->GetType() == Cid::MULTICAST) {
      schedulingType =
          (ServiceFlow::SchedulingType)connection->GetSchedulingType();
    }
...
bool BSSchedulerSimple::SelectConnection(Ptr<WimaxConnection> &connection) {
  ...
  if (GetBs()->GetBroadcastConnection()->HasPackets()) {
    ...
    for (auto iter1 = connections.begin(); iter1 != connections.end();
         ++iter1) {
      if ((*iter1)->HasPackets()) {
    ...
    for (auto iter2 = serviceFlows.begin(); iter2 != serviceFlows.end();
      ...
      if ((*iter2)->HasPackets() &&
        ...
        NS_LOG_INFO("Return UGS SF: CID = " << (*iter2)->GetCid() << "SFID = "
                                            << (*iter2)->GetSfid());
        return true;
      }
    }
    ...
    for (auto iter2 = serviceFlows.begin(); iter2 != serviceFlows.end();
      ...
      if ((*iter2)->HasPackets() &&
        ...
        serviceFlowRecord->SetDlTimeStamp(currentTime);
        connection = (*iter2)->GetConnection();
        NS_LOG_INFO("Return RTPS SF: CID = " << (*iter2)->GetCid() << "SFID = "
                                             << (*iter2)->GetSfid());
        return true;
      }
    }
