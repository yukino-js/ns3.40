...
TypeId SSManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::SSManager").SetParent<Object>().SetGroupName("Wimax");
  return tid;
}

SSManager::SSManager() { m_ssRecords = new std::vector<SSRecord *>(); }
...
SSRecord *SSManager::CreateSSRecord(const Mac48Address &macAddress) {
  auto ssRecord = new SSRecord(macAddress);
  m_ssRecords->push_back(ssRecord);
  return ssRecord;
}

SSRecord *SSManager::GetSSRecord(const Mac48Address &macAddress) const {
  for (auto iter = m_ssRecords->begin(); iter != m_ssRecords->end(); ++iter) {
...
SSRecord *SSManager::GetSSRecord(Cid cid) const {
  for (auto iter1 = m_ssRecords->begin(); iter1 != m_ssRecords->end();
       ++iter1) {
    SSRecord *ssRecord = *iter1;
    if (ssRecord->GetBasicCid() == cid || ssRecord->GetPrimaryCid() == cid) {
      return ssRecord;
    } else {
      std::vector<ServiceFlow *> sf =
...
std::vector<SSRecord *> *SSManager::GetSSRecords() const { return m_ssRecords; }

bool SSManager::IsInRecord(const Mac48Address &macAddress) const {
  for (auto iter = m_ssRecords->begin(); iter != m_ssRecords->end(); ++iter) {
    if ((*iter)->GetMacAddress() == macAddress) {
      return true;
    }
...
bool SSManager::IsRegistered(const Mac48Address &macAddress) const {
...
void SSManager::DeleteSSRecord(Cid cid) {
  for (auto iter1 = m_ssRecords->begin(); iter1 != m_ssRecords->end();
       ++iter1) {
    SSRecord *ssRecord = *iter1;
    if (ssRecord->GetBasicCid() == cid || ssRecord->GetPrimaryCid() == cid) {
...
Mac48Address SSManager::GetMacAddress(Cid cid) const {
  return GetSSRecord(cid)->GetMacAddress();
}

uint32_t SSManager::GetNSSs() const { return m_ssRecords->size(); }

uint32_t SSManager::GetNRegisteredSSs() const {
  uint32_t nrSS = 0;
  for (auto iter = m_ssRecords->begin(); iter != m_ssRecords->end(); ++iter) {
