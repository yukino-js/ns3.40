...
ConnectionManager::ConnectionManager() : m_cidFactory(nullptr) {}

void ConnectionManager::DoDispose() {}

ConnectionManager::~ConnectionManager() {}

void ConnectionManager::SetCidFactory(CidFactory *cidFactory) {
  m_cidFactory = cidFactory;
}

void ConnectionManager::AllocateManagementConnections(SSRecord *ssRecord,
                                                      RngRsp *rngrsp) {
...
Ptr<WimaxConnection> ConnectionManager::CreateConnection(Cid::Type type) {
  Cid cid;
  switch (type) {
  case Cid::BASIC:
  case Cid::MULTICAST:
...
void ConnectionManager::AddConnection(Ptr<WimaxConnection> connection,
                                      Cid::Type type) {
  switch (type) {
  case Cid::BASIC:
...
Ptr<WimaxConnection> ConnectionManager::GetConnection(Cid cid) {
  for (auto iter = m_basicConnections.begin(); iter != m_basicConnections.end();
       ++iter) {
    if ((*iter)->GetCid() == cid) {
      return *iter;
    }
  ...
  for (auto iter = m_primaryConnections.begin();
       iter != m_primaryConnections.end(); ++iter) {
    if ((*iter)->GetCid() == cid) {
      return *iter;
    }
  }

  for (auto iter = m_transportConnections.begin();
       iter != m_transportConnections.end(); ++iter) {
    if ((*iter)->GetCid() == cid) {
      return *iter;
    }
  }
...
uint32_t ConnectionManager::GetNPackets(
    Cid::Type type, ServiceFlow::SchedulingType schedulingType) const {
  uint32_t nrPackets = 0;

  switch (type) {
  case Cid::BASIC: {
    for (auto iter = m_basicConnections.begin();
         iter != m_basicConnections.end(); ++iter) {
...
bool ConnectionManager::HasPackets() const {
  for (auto iter = m_basicConnections.begin(); iter != m_basicConnections.end();
       ++iter) {
    if ((*iter)->HasPackets()) {
      return true;
