...
TypeId BSLinkManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::BSLinkManager").SetParent<Object>().SetGroupName("Wimax");
  return tid;
}

BSLinkManager::BSLinkManager(Ptr<BaseStationNetDevice> bs)
    : m_bs(bs), m_signalQuality(10),
      m_signalQualityThreshold(10) // arbitrary value
{
  tries = 0;
}

BSLinkManager::~BSLinkManager() { m_bs = nullptr; }

uint8_t BSLinkManager::CalculateRangingOppsToAllocate() {
 ...
 */
void BSLinkManager::ProcessRangingRequest(Cid cid, RngReq rngreq) {
  ...
  if (m_bs->GetUplinkScheduler()->GetIsInvIrIntrvlAllocated()) {
    ...
    irIntervalBoundary =
        m_bs->GetUlSubframeStartTime() +
        Seconds(m_bs->GetUplinkScheduler()->GetNrIrOppsAllocated() *
                m_bs->GetRangReqOppSize() *
                m_bs->GetSymbolDuration().GetSeconds());
  }
  ...
  if (Simulator::Now() >= m_bs->GetUlSubframeStartTime() &&
      Simulator::Now() < irIntervalBoundary) {
    PerformRanging(cid, rngreq);
  }
}

void BSLinkManager::PerformRanging(Cid cid, RngReq rngreq) {
  RngRsp rngrsp;
  ...
  if (!decodable) {
    ...
    rngrsp.SetRangStatus(
        WimaxNetDevice::RANGING_STATUS_CONTINUE); // see Figure 64
...
void BSLinkManager::PerformInitialRanging(Cid cid, RngReq *rngreq,
                                          RngRsp *rngrsp) {
  ...
  if (isOldSS) {
    ...
    m_bs->GetConnectionManager()->AllocateManagementConnections(ssRecord,
                                                                rngrsp);

    WimaxPhy::ModulationType modulationType;
    uint8_t diuc = m_bs->GetBurstProfileManager()->GetBurstProfileForSS(
...
void BSLinkManager::PerformInvitedRanging(Cid cid, RngRsp *rngrsp) {
  ...
  if (IsRangingAcceptable()) {
    ...
    if (ssRecord->GetRangingCorrectionRetries() ==
...
void BSLinkManager::VerifyInvitedRanging(Cid cid, uint8_t uiuc) {
  if (uiuc == OfdmUlBurstProfile::UIUC_INITIAL_RANGING) {
    ...
    if (ssRecord->GetInvitedRangRetries() > 0) {
      ...
      if (ssRecord->GetInvitedRangRetries() ==
...
void BSLinkManager::SetParametersToAdjust(RngRsp *rngrsp) {
...
void BSLinkManager::AbortRanging(Cid cid, RngRsp *rngrsp, SSRecord *ssRecord,
                                 bool isOldSS) {
...
void BSLinkManager::AcceptRanging(Cid cid, RngRsp *rngrsp, SSRecord *ssRecord) {
...
void BSLinkManager::ContinueRanging(Cid cid, RngRsp *rngrsp,
                                    SSRecord *ssRecord) {
...
void BSLinkManager::ScheduleRngRspMessage(Cid cid, RngRsp *rngrsp) {
  ...
  p->AddHeader(*rngrsp);
  p->AddHeader(
      ManagementMessageType(ManagementMessageType::MESSAGE_TYPE_RNG_RSP));

  m_bs->Enqueue(p, MacHeaderType(), m_bs->GetConnection(cid));
}

void BSLinkManager::DeallocateCids(Cid cid) {
  // if necessary, delete entire connections or simply set CIDs to 0
}

uint64_t BSLinkManager::SelectDlChannel() {
...
bool BSLinkManager::IsRangingAcceptable() {
