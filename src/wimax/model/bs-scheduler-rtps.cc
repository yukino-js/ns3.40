...
TypeId BSSchedulerRtps::GetTypeId() {
...
void BSSchedulerRtps::AddDownlinkBurst(Ptr<const WimaxConnection> connection,
                                       uint8_t diuc,
  ...
  NS_LOG_INFO("BS scheduler, burst size: "
              << burst->GetSize() << " bytes"
              << ", pkts: " << burst->GetNPackets() << ", connection: "
              << connection->GetTypeStr() << ", CID: " << connection->GetCid());
  if (connection->GetType() == Cid::TRANSPORT) {
    NS_LOG_INFO(
        ", SFID: " << connection->GetServiceFlow()->GetSfid() << ", service: "
                   << connection->GetServiceFlow()->GetSchedulingTypeStr());
  }
  NS_LOG_INFO(", modulation: " << modulationType
                               << ", DIUC: " << (uint32_t)diuc);
...
void BSSchedulerRtps::Schedule() {
  ...
  if (!m_downlinkBursts->empty()) {
    NS_LOG_DEBUG(
        ...
        << m_downlinkBursts->size() << ", symbols left: " << availableSymbols
        << std::endl
        << "BS scheduler, queues:"
        << " IR "
        ...
        << GetBs()->GetConnectionManager()->GetNPackets(
               Cid::BASIC, ServiceFlow::SF_TYPE_NONE)
        << " primary "
        << GetBs()->GetConnectionManager()->GetNPackets(
               Cid::PRIMARY, ServiceFlow::SF_TYPE_NONE)
        << " transport "
        << GetBs()->GetConnectionManager()->GetNPackets(
               Cid::TRANSPORT, ServiceFlow::SF_TYPE_ALL));
  }
}
...
BSSchedulerRtps::CreateUgsBurst(ServiceFlow *serviceFlow,
  ...
  while (serviceFlow->HasPackets()) {
    uint32_t FirstPacketSize =
        connection->GetQueue()->GetFirstPacketRequiredByte(
...
bool BSSchedulerRtps::SelectConnection(Ptr<WimaxConnection> &connection) {
  return false;
}

void BSSchedulerRtps::BSSchedulerBroadcastConnection(
    uint32_t &availableSymbols) {
  ...
  while (GetBs()->GetBroadcastConnection()->HasPackets() &&
         availableSymbols > 0) {
...
void BSSchedulerRtps::BSSchedulerInitialRangingConnection(
    uint32_t &availableSymbols) {
  Ptr<WimaxConnection> connection;
  WimaxPhy::ModulationType modulationType = WimaxPhy::MODULATION_TYPE_BPSK_12;
  uint8_t diuc = OfdmDlBurstProfile::DIUC_BURST_PROFILE_1;
  ...
  while (GetBs()->GetInitialRangingConnection()->HasPackets() &&
         availableSymbols > 0) {
...
void BSSchedulerRtps::BSSchedulerBasicConnection(uint32_t &availableSymbols) {
  Ptr<WimaxConnection> connection;
  WimaxPhy::ModulationType modulationType = WimaxPhy::MODULATION_TYPE_BPSK_12;
  uint8_t diuc = OfdmDlBurstProfile::DIUC_BURST_PROFILE_1;
  uint32_t nrSymbolsRequired = 0;
  ...
  for (auto iter = connections.begin(); iter != connections.end(); ++iter) {
    while ((*iter)->HasPackets() && availableSymbols > 0) {
      ...
      modulationType = GetBs()
                           ->GetSSManager()
                           ->GetSSRecord(connection->GetCid())
                           ->GetModulationType();
      diuc = GetBs()->GetBurstProfileManager()->GetBurstProfile(
          modulationType, WimaxNetDevice::DIRECTION_DOWNLINK);
...
void BSSchedulerRtps::BSSchedulerPrimaryConnection(uint32_t &availableSymbols) {
  Ptr<WimaxConnection> connection;
  WimaxPhy::ModulationType modulationType = WimaxPhy::MODULATION_TYPE_BPSK_12;
  uint8_t diuc = 0;
  uint32_t nrSymbolsRequired = 0;
...
void BSSchedulerRtps::BSSchedulerUGSConnection(uint32_t &availableSymbols) {
  Ptr<WimaxConnection> connection;
  WimaxPhy::ModulationType modulationType = WimaxPhy::MODULATION_TYPE_BPSK_12;
  uint8_t diuc;
  uint32_t nrSymbolsRequired = 0;
  ...
  serviceFlows = GetBs()->GetServiceFlowManager()->GetServiceFlows(
...
void BSSchedulerRtps::BSSchedulerRTPSConnection(uint32_t &availableSymbols) {
  Ptr<WimaxConnection> connection;
  GenericMacHeader hdr;
  Ptr<Packet> packet;
  Ptr<PacketBurst> burst = Create<PacketBurst>();
  ...
  for (auto iter2 = serviceFlows.begin(); iter2 != serviceFlows.end();
    ...
    if ((*iter2)->HasPackets()) {
      ...
      diuc_[nbConnection] = GetBs()->GetBurstProfileManager()->GetBurstProfile(
          modulationType_[nbConnection], WimaxNetDevice::DIRECTION_DOWNLINK);

      dataToSend = rtPSConnection[nbConnection]
                       ->GetQueue()
      ...
      symbolsRequired[nbConnection] = GetBs()->GetPhy()->GetNrSymbols(
          dataToSend, modulationType_[nbConnection]);
    ...
    while (symbolsRequired[i] > 0) {
      ...
      if (symbolsForPacketTransmission > symbolsRequired[i] &&
        ...
        break;
                 ...
                 CheckForFragmentation(rtPSConnection[i], symbolsRequired[i],
                                       modulationType_[i])) {
        uint32_t availableByte = GetBs()->GetPhy()->GetNrBytes(
            symbolsRequired[i], modulationType_[i]);
        packet = rtPSConnection[i]->Dequeue(MacHeaderType::HEADER_TYPE_GENERIC,
                                            availableByte);
        symbolsRequired[i] = 0;
      } else {
      ...
      NS_ASSERT_MSG(
          hdr.GetCid() == rtPSConnection[i]->GetCid(),
...
void BSSchedulerRtps::BSSchedulerNRTPSConnection(uint32_t &availableSymbols) {
  Ptr<WimaxConnection> connection;
  WimaxPhy::ModulationType modulationType = WimaxPhy::MODULATION_TYPE_BPSK_12;
  uint8_t diuc = 0;
  uint32_t nrSymbolsRequired = 0;
...
void BSSchedulerRtps::BSSchedulerBEConnection(uint32_t &availableSymbols) {
  Ptr<WimaxConnection> connection;
  WimaxPhy::ModulationType modulationType = WimaxPhy::MODULATION_TYPE_BPSK_12;
  uint8_t diuc = 0;
  uint32_t nrSymbolsRequired = 0;
