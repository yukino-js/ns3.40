...
TypeId SSScheduler::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::SSScheduler").SetParent<Object>().SetGroupName("Wimax");
  return tid;
}

SSScheduler::SSScheduler(Ptr<SubscriberStationNetDevice> ss)
    : m_ss(ss), m_pollMe(false) {}

SSScheduler::~SSScheduler() {}

void SSScheduler::DoDispose() { m_ss = nullptr; }

void SSScheduler::SetPollMe(bool pollMe) { m_pollMe = pollMe; }

bool SSScheduler::GetPollMe() const { return m_pollMe; }

Ptr<PacketBurst> SSScheduler::Schedule(uint16_t availableSymbols,
  ...
  while (connection && connection->HasPackets(packetType)) {
    ...
    uint32_t requiredByte =
        connection->GetQueue()->GetFirstPacketRequiredByte(packetType);
    ...
    if (availableByte >= requiredByte) {
      ...
      if (connection->GetType() == Cid::TRANSPORT) {
        ...
        uint32_t headerSize =
            connection->GetQueue()->GetFirstPacketHdrSize(packetType);
...
Ptr<WimaxConnection> SSScheduler::SelectConnection() {
  ...
  serviceFlows = m_ss->GetServiceFlowManager()->GetServiceFlows(
  ...
  serviceFlows =
      m_ss->GetServiceFlowManager()->GetServiceFlows(ServiceFlow::SF_TYPE_BE);
  for (auto iter = serviceFlows.begin(); iter != serviceFlows.end(); ++iter) {
    if ((*iter)->HasPackets(MacHeaderType::HEADER_TYPE_GENERIC)) {
