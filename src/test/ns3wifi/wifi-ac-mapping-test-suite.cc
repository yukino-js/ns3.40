...
WifiAcMappingTest::WifiAcMappingTest(uint8_t tos, uint8_t expectedQueue)
    : TestCase("User priority to Access Category mapping test. Checks that "
      ...
      m_tos(tos), m_expectedQueue(expectedQueue) {
...
void WifiAcMappingTest::PacketEnqueuedInQueueDisc(
    uint8_t tos, uint16_t *count, Ptr<const QueueDiscItem> item) {
  uint8_t val;
...
void WifiAcMappingTest::DoRun() {
  ...
  Ptr<QueueDisc> root =
      ap.Get(0)->GetObject<TrafficControlLayer>()->GetRootQueueDiscOnDevice(
          apDev.Get(0));
  ...
  ptr.Get<QosTxop>()->GetWifiMacQueue()->TraceConnectWithoutContext(
      "Enqueue",
      MakeBoundCallback(&WifiAcMappingTest::PacketEnqueuedInWifiMacQueue, m_tos,
                        +m_WifiMacQueueCount));
