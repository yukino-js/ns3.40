...
const uint32_t PCAP_LINK_TYPE =
    1187373557; //!< Some large random number -- we use to verify data was
...
Ns3TcpLossTestCase::Ns3TcpLossTestCase()
    : TestCase(
      ...
      m_testCase(0), m_totalTxBytes(200000), m_currentTxBytes(0),
...
Ns3TcpLossTestCase::Ns3TcpLossTestCase(std::string tcpModel, uint32_t testCase)
    : TestCase("Check the behaviour of TCP upon packet losses"),
      m_testCase(testCase), m_totalTxBytes(200000), m_currentTxBytes(0),
      m_writeVectors(WRITE_VECTORS), m_writeResults(WRITE_PCAP),
      m_writeLogging(WRITE_LOGGING), m_needToClose(true), m_tcpModel(tcpModel) {
}

void Ns3TcpLossTestCase::DoSetup() {
...
void Ns3TcpLossTestCase::DoTeardown() { m_pcapFile.Close(); }

void Ns3TcpLossTestCase::Ipv4L3Tx(std::string, Ptr<const Packet> packet,
                                  Ptr<Ipv4>, uint32_t) {
  //
...
void Ns3TcpLossTestCase::CwndTracer(uint32_t oldval, uint32_t newval) {
  if (m_writeLogging) {
...
void Ns3TcpLossTestCase::WriteUntilBufferFull(Ptr<Socket> localSocket,
                                              uint32_t) {
  while (m_currentTxBytes < m_totalTxBytes) {
    ...
    NS_ASSERT(
        amountSent >
        0); // Given GetTxAvailable() non-zero, amountSent should not be zero
...
void Ns3TcpLossTestCase::StartFlow(Ptr<Socket> localSocket,
  ...
  localSocket->SetSendCallback(
      MakeCallback(&Ns3TcpLossTestCase::WriteUntilBufferFull, this));
  WriteUntilBufferFull(localSocket, localSocket->GetTxAvailable());
}

void Ns3TcpLossTestCase::DoRun() {
  // Network topology
  //
  ...
  Ptr<Socket> localSocket =
      Socket::CreateSocket(s1r1.Get(0), TcpSocketFactory::GetTypeId());
  localSocket->Bind();
  ...
  case 4:
    ...
    break;
  default:
    NS_FATAL_ERROR("Program fatal error: loss value " << m_testCase
                                                      << " not supported.");
    break;
  }
