...
MeshHelper::MeshHelper()
    : m_nInterfaces(1), m_spreadChannelPolicy(ZERO_CHANNEL), m_stack(nullptr),
      m_standard(WIFI_STANDARD_80211a) {}

MeshHelper::~MeshHelper() { m_stack = nullptr; }

void MeshHelper::SetSpreadInterfaceChannels(ChannelPolicy policy) {
  m_spreadChannelPolicy = policy;
}

void MeshHelper::SetNumberOfInterfaces(uint32_t nInterfaces) {
  m_nInterfaces = nInterfaces;
}

NetDeviceContainer MeshHelper::Install(const WifiPhyHelper &phyHelper,
...
void MeshHelper::SetStandard(WifiStandard standard) { m_standard = standard; }

Ptr<WifiNetDevice> MeshHelper::CreateInterface(const WifiPhyHelper &phyHelper,
                                               Ptr<Node> node,
  ...
  Ptr<MeshWifiInterfaceMac> mac =
      macObjectFactory.Create<MeshWifiInterfaceMac>();
  NS_ASSERT(mac);
  mac->SetSsid(Ssid());
...
void MeshHelper::Report(const ns3::Ptr<ns3::NetDevice> &device,
                        std::ostream &os) {
  NS_ASSERT(m_stack);
  Ptr<MeshPointDevice> mp = device->GetObject<MeshPointDevice>();
  NS_ASSERT(mp);
  std::vector<Ptr<NetDevice>> ifaces = mp->GetInterfaces();
  os << "<MeshPointDevice time=\"" << Simulator::Now().GetSeconds()
     << "\" address=\"" << Mac48Address::ConvertFrom(mp->GetAddress())
     << "\">\n";
  m_stack->Report(mp, os);
  os << "</MeshPointDevice>\n";
}

void MeshHelper::ResetStats(const ns3::Ptr<ns3::NetDevice> &device) {
  NS_ASSERT(m_stack);
  Ptr<MeshPointDevice> mp = device->GetObject<MeshPointDevice>();
  NS_ASSERT(mp);
...
int64_t MeshHelper::AssignStreams(NetDeviceContainer c, int64_t stream) {
...
void MeshHelper::EnableLogComponents() {
