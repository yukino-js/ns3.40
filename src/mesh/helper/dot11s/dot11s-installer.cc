...
TypeId Dot11sStack::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Dot11sStack")
          .SetParent<MeshStack>()
          .SetGroupName("Mesh")
...
bool Dot11sStack::InstallStack(Ptr<MeshPointDevice> mp) {
  ...
  hwmp->SetNeighboursCallback(
      MakeCallback(&PeerManagementProtocol::GetPeers, PeekPointer(pmp)));
  return true;
}

void Dot11sStack::Report(const Ptr<MeshPointDevice> mp, std::ostream &os) {
  mp->Report(os);
  ...
  for (auto i = ifaces.begin(); i != ifaces.end(); ++i) {
    ...
    Ptr<MeshWifiInterfaceMac> mac =
        device->GetMac()->GetObject<MeshWifiInterfaceMac>();
    NS_ASSERT(mac);
    mac->Report(os);
  }
...
void Dot11sStack::ResetStats(const Ptr<MeshPointDevice> mp) {
  mp->ResetStats();
