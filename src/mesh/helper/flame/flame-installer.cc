...
void FlameStack::DoDispose() {}

bool FlameStack::InstallStack(Ptr<MeshPointDevice> mp) {
  Ptr<FlameProtocol> flame = CreateObject<FlameProtocol>();
  return flame->Install(mp);
}

void FlameStack::Report(const Ptr<MeshPointDevice> mp, std::ostream &os) {
  mp->Report(os);
...
void FlameStack::ResetStats(const Ptr<MeshPointDevice> mp) {
  mp->ResetStats();
