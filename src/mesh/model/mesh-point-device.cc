...
TypeId MeshPointDevice::GetTypeId() {
  static TypeId tid =
...
void MeshPointDevice::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void MeshPointDevice::ReceiveFromDevice(Ptr<NetDevice> incomingPort,
  ...
  if (dst48.IsGroup()) {
    ...
    if (m_routingProtocol->RemoveRoutingStuff(incomingPort->GetIfIndex(), src48,
                                              dst48, packet_copy,
                                              realProtocol)) {
      ...
      NS_LOG_DEBUG("Forwarding broadcast from "
                   << src48 << " to " << dst48 << " with delay "
                   << forwardingDelay.As(Time::US));
      Simulator::Schedule(forwardingDelay, &MeshPointDevice::Forward, this,
                          incomingPort, packet, protocol, src48, dst48);
    }
    return;
  }
  if (dst48 == m_address) {
    ...
    Time forwardingDelay = GetForwardingDelay();
    Simulator::Schedule(forwardingDelay, &MeshPointDevice::Forward, this,
                        incomingPort, packet->Copy(), protocol, src48, dst48);
...
void MeshPointDevice::Forward(Ptr<NetDevice> inport, Ptr<const Packet> packet,
  ...
  bool result = m_routingProtocol->RequestRoute(
      inport->GetIfIndex(), src, dst, packet, protocol,
      MakeCallback(&MeshPointDevice::DoSend, this));
  if (!result) {
    NS_LOG_DEBUG("Request to forward packet " << packet << " to destination "
                                              << dst
                                              << " failed; dropping packet");
  }
}

void MeshPointDevice::SetIfIndex(const uint32_t index) {
  NS_LOG_FUNCTION(this);
  m_ifIndex = index;
}

uint32_t MeshPointDevice::GetIfIndex() const {
  NS_LOG_FUNCTION(this);
  return m_ifIndex;
}

Ptr<Channel> MeshPointDevice::GetChannel() const {
  NS_LOG_FUNCTION(this);
  return m_channel;
}

Address MeshPointDevice::GetAddress() const {
  NS_LOG_FUNCTION(this);
  return m_address;
}

void MeshPointDevice::SetAddress(Address a) {
  NS_LOG_FUNCTION(this);
  NS_LOG_WARN("Manual changing mesh point address can cause routing errors.");
  m_address = Mac48Address::ConvertFrom(a);
}

bool MeshPointDevice::SetMtu(const uint16_t mtu) {
  NS_LOG_FUNCTION(this);
  m_mtu = mtu;
  return true;
}

uint16_t MeshPointDevice::GetMtu() const {
  NS_LOG_FUNCTION(this);
  return m_mtu;
}

bool MeshPointDevice::IsLinkUp() const {
  NS_LOG_FUNCTION(this);
  return true;
}
...
bool MeshPointDevice::IsBroadcast() const {
  NS_LOG_FUNCTION(this);
  return true;
}

Address MeshPointDevice::GetBroadcast() const {
  NS_LOG_FUNCTION(this);
  return Mac48Address("ff:ff:ff:ff:ff:ff");
}

bool MeshPointDevice::IsMulticast() const {
  NS_LOG_FUNCTION(this);
  return true;
}

Address MeshPointDevice::GetMulticast(Ipv4Address multicastGroup) const {
  NS_LOG_FUNCTION(this << multicastGroup);
  Mac48Address multicast = Mac48Address::GetMulticast(multicastGroup);
  return multicast;
}

bool MeshPointDevice::IsPointToPoint() const {
  NS_LOG_FUNCTION(this);
  return false;
}

bool MeshPointDevice::IsBridge() const {
  NS_LOG_FUNCTION(this);
  return false;
}

bool MeshPointDevice::Send(Ptr<Packet> packet, const Address &dest,
                           uint16_t protocolNumber) {
  NS_LOG_FUNCTION(this);
  const Mac48Address dst48 = Mac48Address::ConvertFrom(dest);
  return m_routingProtocol->RequestRoute(
      m_ifIndex, m_address, dst48, packet, protocolNumber,
      MakeCallback(&MeshPointDevice::DoSend, this));
}

bool MeshPointDevice::SendFrom(Ptr<Packet> packet, const Address &src,
                               const Address &dest, uint16_t protocolNumber) {
  NS_LOG_FUNCTION(this);
  const Mac48Address src48 = Mac48Address::ConvertFrom(src);
  const Mac48Address dst48 = Mac48Address::ConvertFrom(dest);
  return m_routingProtocol->RequestRoute(
      m_ifIndex, src48, dst48, packet, protocolNumber,
      MakeCallback(&MeshPointDevice::DoSend, this));
}

Ptr<Node> MeshPointDevice::GetNode() const {
  NS_LOG_FUNCTION(this);
  return m_node;
}

void MeshPointDevice::SetNode(Ptr<Node> node) {
  NS_LOG_FUNCTION(this);
  m_node = node;
}

bool MeshPointDevice::NeedsArp() const {
  NS_LOG_FUNCTION(this);
  return true;
}

void MeshPointDevice::SetReceiveCallback(NetDevice::ReceiveCallback cb) {
  NS_LOG_FUNCTION(this);
  m_rxCallback = cb;
}

void MeshPointDevice::SetPromiscReceiveCallback(
...
Ptr<NetDevice> MeshPointDevice::GetInterface(uint32_t n) const {
...
std::vector<Ptr<NetDevice>> MeshPointDevice::GetInterfaces() const {
  return m_ifaces;
}

void MeshPointDevice::AddInterface(Ptr<NetDevice> iface) {
  ...
  Ptr<MeshWifiInterfaceMac> ifaceMac =
...
void MeshPointDevice::SetRoutingProtocol(Ptr<MeshL2RoutingProtocol> protocol) {
  ...
  NS_ASSERT_MSG(
      PeekPointer(protocol->GetMeshPoint()) == this,
      "Routing protocol must be installed on mesh point to be useful.");
  m_routingProtocol = protocol;
}

Ptr<MeshL2RoutingProtocol> MeshPointDevice::GetRoutingProtocol() const {
  NS_LOG_FUNCTION(this);
  return m_routingProtocol;
}

void MeshPointDevice::DoSend(bool success, Ptr<Packet> packet, Mac48Address src,
                             Mac48Address dst, uint16_t protocol,
                             uint32_t outIface) {
  NS_LOG_FUNCTION(this << success << packet << src << dst << protocol
                       << outIface);
  if (!success) {
...
MeshPointDevice::Statistics::Statistics()
    : unicastData(0), unicastDataBytes(0), broadcastData(0),
      broadcastDataBytes(0) {
  NS_LOG_FUNCTION(this);
}

void MeshPointDevice::Report(std::ostream &os) const {
  NS_LOG_FUNCTION(this);
...
void MeshPointDevice::ResetStats() {
  NS_LOG_FUNCTION(this);
...
int64_t MeshPointDevice::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_forwardingRandomVariable->SetStream(stream);
  return 1;
}

Time MeshPointDevice::GetForwardingDelay() const {
