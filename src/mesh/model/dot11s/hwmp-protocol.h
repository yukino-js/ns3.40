...
private:
  ...
  struct QueuedPacket {
    ...
    uint32_t inInterface; ///< incoming device interface ID. (if packet has come
...
private:
  ...
  uint8_t m_unicastPerrThreshold; //!< Maximum number of PERR receivers, when we
                                  //!< send a PERR as a chain of unicasts
  uint8_t m_unicastPreqThreshold; //!< Maximum number of PREQ receivers, when we
                                  //!< send a PREQ as a chain of unicasts
  uint8_t
      m_unicastDataThreshold; //!< Maximum number of broadcast receivers, when
  ...
  Callback<std::vector<Mac48Address>, uint32_t>
      m_neighboursCallback; ///< neighbors callback
};
} // namespace dot11s
} // namespace ns3
#endif
