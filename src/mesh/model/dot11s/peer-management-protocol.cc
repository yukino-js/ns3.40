...
TypeId PeerManagementProtocol::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::dot11s::PeerManagementProtocol")
          ...
          .AddAttribute(
              "MaxBeaconShiftValue",
              "Maximum number of TUs for beacon shifting", UintegerValue(15),
...
PeerManagementProtocol::PeerManagementProtocol()
    : m_lastAssocId(0), m_lastLocalLinkId(1), m_enableBca(true),
      m_maxBeaconShift(15) {
...
bool PeerManagementProtocol::Install(Ptr<MeshPointDevice> mp) {
...
PeerManagementProtocol::GetBeaconTimingElement(uint32_t interface) {
  ...
  for (auto i = iface->second.begin(); i != iface->second.end(); i++) {
    ...
    retval->AddNeighboursTimingElementUnit(
        (*i)->GetLocalAid(), (*i)->GetLastBeacon(), (*i)->GetBeaconInterval());
  }
  return retval;
}

void PeerManagementProtocol::ReceiveBeacon(uint32_t interface,
  ...
  if (!peerLink) {
    if (ShouldSendOpen(interface, peerAddress)) {
      peerLink =
          InitiateLink(interface, peerAddress, Mac48Address::GetBroadcast());
...
void PeerManagementProtocol::ReceivePeerLinkFrame(
    uint32_t interface, Mac48Address peerAddress,
  ...
  if (peerManagementElement.SubtypeIsConfirm()) {
    peerLink->ConfirmAccept(peerManagementElement.GetLocalLinkId(),
                            peerManagementElement.GetPeerLinkId(), aid,
                            meshConfig, peerMeshPointAddress);
  }
...
void PeerManagementProtocol::ConfigurationMismatch(uint32_t interface,
                                                   Mac48Address peerAddress) {
  Ptr<PeerLink> peerLink = FindPeerLink(interface, peerAddress);
  if (peerLink) {
...
void PeerManagementProtocol::TransmissionFailure(uint32_t interface,
                                                 Mac48Address peerAddress) {
  NS_LOG_DEBUG("transmission failed between "
               << GetAddress() << " and " << peerAddress
...
void PeerManagementProtocol::TransmissionSuccess(uint32_t interface,
                                                 Mac48Address peerAddress) {
  NS_LOG_DEBUG("transmission success " << GetAddress() << " and "
                                       << peerAddress);
  Ptr<PeerLink> peerLink = FindPeerLink(interface, peerAddress);
  if (peerLink) {
...
Ptr<PeerLink> PeerManagementProtocol::FindPeerLink(uint32_t interface,
                                                   Mac48Address peerAddress) {
  auto iface = m_peerLinks.find(interface);
  NS_ASSERT(iface != m_peerLinks.end());
  for (auto i = iface->second.begin(); i != iface->second.end(); i++) {
...
void PeerManagementProtocol::SetPeerLinkStatusCallback(
...
std::vector<Ptr<PeerLink>> PeerManagementProtocol::GetPeerLinks() const {
...
bool PeerManagementProtocol::IsActiveLink(uint32_t interface,
                                          Mac48Address peerAddress) {
  Ptr<PeerLink> peerLink = FindPeerLink(interface, peerAddress);
  if (peerLink) {
...
bool PeerManagementProtocol::ShouldSendOpen(uint32_t interface,
                                            Mac48Address peerAddress) const {
  return (m_stats.linksTotal < m_maxNumberOfPeerLinks);
}

bool PeerManagementProtocol::ShouldAcceptOpen(uint32_t interface,
                                              Mac48Address peerAddress,
...
void PeerManagementProtocol::CheckBeaconCollisions(uint32_t interface) {
  if (!GetBeaconCollisionAvoidance()) {
    return;
  }
  auto iface = m_peerLinks.find(interface);
  ...
  if ((lastBeacon == m_lastBeacon.end()) ||
...
void PeerManagementProtocol::ShiftOwnBeacon(uint32_t interface) {
  int shift = 0;
  do {
...
Time PeerManagementProtocol::TuToTime(int x) { return MicroSeconds(x * 1024); }

int PeerManagementProtocol::TimeToTu(Time x) {
  return (int)(x.GetMicroSeconds() / 1024);
}

void PeerManagementProtocol::NotifyLinkOpen(Mac48Address peerMp,
...
void PeerManagementProtocol::NotifyLinkClose(Mac48Address peerMp,
                                             Mac48Address peerIface,
                                             Mac48Address myIface,
                                             uint32_t interface) {
...
void PeerManagementProtocol::PeerLinkStatus(uint32_t interface,
                                            Mac48Address peerAddress,
                                            Mac48Address peerMeshPointAddress,
  ...
  auto plugin = m_plugins.find(interface);
  NS_ASSERT(plugin != m_plugins.end());
  NS_LOG_DEBUG("Link between me:"
               << m_address << " my interface:" << plugin->second->GetAddress()
  ...
  if ((nstate == PeerLink::ESTAB) && (ostate != PeerLink::ESTAB)) {
    NotifyLinkOpen(peerMeshPointAddress, peerAddress,
                   plugin->second->GetAddress(), interface);
  }
  if ((ostate == PeerLink::ESTAB) && (nstate != PeerLink::ESTAB)) {
    NotifyLinkClose(peerMeshPointAddress, peerAddress,
                    plugin->second->GetAddress(), interface);
  }
...
uint8_t PeerManagementProtocol::GetNumberOfLinks() const {
  return m_stats.linksTotal;
}

Ptr<IeMeshId> PeerManagementProtocol::GetMeshId() const {
  NS_ASSERT(m_meshId);
  return m_meshId;
}

void PeerManagementProtocol::SetMeshId(std::string s) {
  m_meshId = Create<IeMeshId>(s);
}

Mac48Address PeerManagementProtocol::GetAddress() { return m_address; }

void PeerManagementProtocol::NotifyBeaconSent(uint32_t interface,
                                              Time beaconInterval) {
  m_lastBeacon[interface] = Simulator::Now();
  Simulator::Schedule(beaconInterval - TuToTime(m_maxBeaconShift + 1),
                      &PeerManagementProtocol::CheckBeaconCollisions, this,
                      interface);
...
void PeerManagementProtocol::Statistics::Print(std::ostream &os) const {
  os << "<Statistics "
        "linksTotal=\""
     << linksTotal
     << "\" "
...
void PeerManagementProtocol::Report(std::ostream &os) const {
...
int64_t PeerManagementProtocol::AssignStreams(int64_t stream) {
...
void PeerManagementProtocol::SetBeaconCollisionAvoidance(bool enable) {
  m_enableBca = enable;
}

bool PeerManagementProtocol::GetBeaconCollisionAvoidance() const {
  return m_enableBca;
}
} // namespace dot11s
} // namespace ns3
