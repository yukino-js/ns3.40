...
PeerManagementProtocolMac::PeerManagementProtocolMac(
    uint32_t interface, Ptr<PeerManagementProtocol> protocol) {
  m_ifIndex = interface;
  m_protocol = protocol;
}
...
void PeerManagementProtocolMac::SetParent(Ptr<MeshWifiInterfaceMac> parent) {
  m_parent = parent;
  m_parent->TraceConnectWithoutContext(
      "DroppedMpdu", MakeCallback(&PeerManagementProtocolMac::TxError, this));
  m_parent->TraceConnectWithoutContext(
      "AckedMpdu", MakeCallback(&PeerManagementProtocolMac::TxOk, this));
}

void PeerManagementProtocolMac::TxError(WifiMacDropReason reason,
                                        Ptr<const WifiMpdu> mpdu) {
  m_protocol->TransmissionFailure(m_ifIndex, mpdu->GetHeader().GetAddr1());
}

void PeerManagementProtocolMac::TxOk(Ptr<const WifiMpdu> mpdu) {
...
bool PeerManagementProtocolMac::Receive(Ptr<Packet> const_packet,
  ...
  if (header.IsBeacon()) {
    ...
    Ptr<IeMeshId> meshId =
        DynamicCast<IeMeshId>(elements.FindFirst(IE_MESH_ID));
  ...
  if (header.IsAction()) {
    ...
    if (actionValue.selfProtectedAction == WifiActionHeader::PEER_LINK_OPEN) {
    ...
    } else if (actionValue.selfProtectedAction ==
      ...
      NS_FATAL_ERROR("Unknown Self-protected Action type: "
...
bool PeerManagementProtocolMac::UpdateOutcomingFrame(Ptr<Packet> packet,
...
void PeerManagementProtocolMac::UpdateBeacon(MeshWifiBeacon &beacon) const {
  if (m_protocol->GetBeaconCollisionAvoidance()) {
    Ptr<IeBeaconTiming> beaconTiming =
        m_protocol->GetBeaconTimingElement(m_ifIndex);
...
void PeerManagementProtocolMac::SendPeerLinkManagementFrame(
...
Mac48Address PeerManagementProtocolMac::GetAddress() const {
  if (m_parent) {
...
void PeerManagementProtocolMac::SetBeaconShift(Time shift) {
  if (shift != Seconds(0)) {
    m_stats.beaconShift++;
  }
...
PeerManagementProtocolMac::Statistics::Statistics()
    : txOpen(0), txConfirm(0), txClose(0), rxOpen(0), rxConfirm(0), rxClose(0),
      dropped(0), brokenMgt(0), txMgt(0), txMgtBytes(0), rxMgt(0),
      rxMgtBytes(0), beaconShift(0) {}

void PeerManagementProtocolMac::Statistics::Print(std::ostream &os) const {
  os << "<Statistics "
        "txOpen=\""
     << txOpen << "\"" << std::endl
...
void PeerManagementProtocolMac::Report(std::ostream &os) const {
...
void PeerManagementProtocolMac::ResetStats() { m_stats = Statistics(); }

uint32_t PeerManagementProtocolMac::GetLinkMetric(Mac48Address peerAddress) {
  return m_parent->GetLinkMetric(peerAddress);
}

int64_t PeerManagementProtocolMac::AssignStreams(int64_t stream) {
