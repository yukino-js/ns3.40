...
HwmpRtable::HwmpRtable() { DeleteProactivePath(); }

HwmpRtable::~HwmpRtable() {}

void HwmpRtable::DoDispose() { m_routes.clear(); }

void HwmpRtable::AddReactivePath(Mac48Address destination,
                                 Mac48Address retransmitter, uint32_t interface,
                                 uint32_t metric, Time lifetime,
                                 uint32_t seqnum) {
...
void HwmpRtable::AddProactivePath(uint32_t metric, Mac48Address root,
                                  Mac48Address retransmitter,
                                  uint32_t interface, Time lifetime,
                                  uint32_t seqnum) {
  NS_LOG_FUNCTION(this << metric << root << retransmitter
                       << interface << lifetime << seqnum);
  m_root.root = root;
...
void HwmpRtable::AddPrecursor(Mac48Address destination,
                              uint32_t precursorInterface,
                              Mac48Address precursorAddress, Time lifetime) {
  NS_LOG_FUNCTION(this << destination << precursorInterface << precursorAddress
                       << lifetime);
  Precursor precursor;
...
void HwmpRtable::DeleteProactivePath(Mac48Address root) {
...
void HwmpRtable::DeleteReactivePath(Mac48Address destination) {
...
HwmpRtable::LookupReactiveExpired(Mac48Address destination) {
  ...
  return LookupResult(i->second.retransmitter, i->second.interface,
...
HwmpRtable::LookupResult HwmpRtable::LookupProactiveExpired() {
  ...
  return LookupResult(m_root.retransmitter, m_root.interface, m_root.metric,
                      m_root.seqnum, m_root.whenExpire - Simulator::Now());
}
...
HwmpRtable::PrecursorList HwmpRtable::GetPrecursors(Mac48Address destination) {
  NS_LOG_FUNCTION(this << destination);
  // We suppose that no duplicates here can be
  PrecursorList retval;
  auto route = m_routes.find(destination);
  if (route != m_routes.end()) {
    for (auto i = route->second.precursors.begin();
...
bool HwmpRtable::LookupResult::operator==(
    const HwmpRtable::LookupResult &o) const {
  return (retransmitter == o.retransmitter && ifIndex == o.ifIndex &&
          metric == o.metric && seqnum == o.seqnum);
}

HwmpRtable::LookupResult::LookupResult(Mac48Address r, uint32_t i, uint32_t m,
                                       uint32_t s, Time l)
...
bool HwmpRtable::LookupResult::IsValid() const {
  return !(retransmitter == Mac48Address::GetBroadcast() &&
           ifIndex == INTERFACE_ANY && metric == MAX_METRIC && seqnum == 0);
}
} // namespace dot11s
} // namespace ns3
