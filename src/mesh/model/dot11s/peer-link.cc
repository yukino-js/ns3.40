...
PeerLink::PeerLink()
    : m_peerAddress(Mac48Address::GetBroadcast()),
      ...
      m_peerLinkId(0), m_assocId(0), m_peerAssocId(0), m_lastBeacon(Seconds(0)),
      m_beaconInterval(Seconds(0)), m_packetFail(0), m_state(IDLE),
      m_retryCounter(0), m_maxPacketFail(3) {
  NS_LOG_FUNCTION(this);
}

PeerLink::~PeerLink() {}

void PeerLink::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_retryTimer.Cancel();
...
void PeerLink::SetPeerAddress(Mac48Address macaddr) { m_peerAddress = macaddr; }

void PeerLink::SetPeerMeshPointAddress(Mac48Address macaddr) {
  m_peerMeshPointAddress = macaddr;
}

void PeerLink::SetInterface(uint32_t interface) { m_interface = interface; }

void PeerLink::SetLocalLinkId(uint16_t id) { m_localLinkId = id; }

void PeerLink::SetLocalAid(uint16_t aid) { m_assocId = aid; }

void PeerLink::SetBeaconInformation(Time lastBeacon, Time beaconInterval) {
...
void PeerLink::MLMESetSignalStatusCallback(PeerLink::SignalStatusCallback cb) {
  m_linkStatusCallback = cb;
}

void PeerLink::BeaconLoss() {
  NS_LOG_FUNCTION(this);
  StateMachine(CNCL);
}

void PeerLink::TransmissionSuccess() { m_packetFail = 0; }

void PeerLink::TransmissionFailure() {
  NS_LOG_FUNCTION(this);
  m_packetFail++;
...
void PeerLink::SetBeaconTimingElement(IeBeaconTiming beaconTiming) {
  m_beaconTiming = beaconTiming;
}

Mac48Address PeerLink::GetPeerAddress() const { return m_peerAddress; }

uint16_t PeerLink::GetLocalAid() const { return m_assocId; }

uint16_t PeerLink::GetPeerAid() const { return m_peerAssocId; }

Time PeerLink::GetLastBeacon() const { return m_lastBeacon; }

Time PeerLink::GetBeaconInterval() const { return m_beaconInterval; }

IeBeaconTiming PeerLink::GetBeaconTimingElement() const {
...
void PeerLink::MLMEActivePeerLinkOpen() { StateMachine(ACTOPN); }

void PeerLink::MLMEPeeringRequestReject() {
  StateMachine(REQ_RJCT, REASON11S_PEERING_CANCELLED);
}

void PeerLink::Close(uint16_t localLinkId, uint16_t peerLinkId,
...
void PeerLink::OpenReject(uint16_t localLinkId, IeConfiguration conf,
...
void PeerLink::ConfirmAccept(uint16_t localLinkId, uint16_t peerLinkId,
...
void PeerLink::ConfirmReject(uint16_t localLinkId, uint16_t peerLinkId,
...
bool PeerLink::LinkIsEstab() const { return (m_state == ESTAB); }

bool PeerLink::LinkIsIdle() const { return (m_state == IDLE); }

void PeerLink::SetMacPlugin(Ptr<PeerManagementProtocolMac> plugin) {
  m_macPlugin = plugin;
}
...
void PeerLink::ClearRetryTimer() { m_retryTimer.Cancel(); }

void PeerLink::ClearConfirmTimer() { m_confirmTimer.Cancel(); }

void PeerLink::ClearHoldingTimer() { m_holdingTimer.Cancel(); }
...
void PeerLink::SendPeerLinkClose(PmpReasonCode reasoncode) {
  ...
  m_macPlugin->SendPeerLinkManagementFrame(m_peerAddress,
                                           m_peerMeshPointAddress, m_assocId,
...
void PeerLink::SetHoldingTimer() {
  ...
  m_holdingTimer = Simulator::Schedule(m_dot11MeshHoldingTimeout,
                                       &PeerLink::HoldingTimeout, this);
}
...
void PeerLink::SetRetryTimer() {
  ...
  m_retryTimer = Simulator::Schedule(m_dot11MeshRetryTimeout,
                                     &PeerLink::RetryTimeout, this);
}
...
void PeerLink::SetConfirmTimer() {
  ...
  m_confirmTimer = Simulator::Schedule(m_dot11MeshConfirmTimeout,
                                       &PeerLink::ConfirmTimeout, this);
}

void PeerLink::ConfirmTimeout() { StateMachine(TOC); }

void PeerLink::Report(std::ostream &os) const {
  if (m_state != ESTAB) {
    return;
  }
