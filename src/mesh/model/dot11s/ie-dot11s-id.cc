...
WifiInformationElementId IeMeshId::ElementId() const { return IE_MESH_ID; }

bool IeMeshId::IsEqual(const IeMeshId &o) const {
  uint8_t i = 0;
...
bool IeMeshId::IsBroadcast() const { return m_meshId[0] == 0; }

char *IeMeshId::PeekString() const { return (char *)m_meshId; }

uint16_t IeMeshId::GetInformationFieldSize() const {
  uint8_t size = 0;
...
void IeMeshId::SerializeInformationField(Buffer::Iterator i) const {
  uint8_t size = 0;
  while (m_meshId[size] != 0 && size < 32) {
...
uint16_t IeMeshId::DeserializeInformationField(Buffer::Iterator start,
                                               uint16_t length) {
...
void IeMeshId::Print(std::ostream &os) const {
