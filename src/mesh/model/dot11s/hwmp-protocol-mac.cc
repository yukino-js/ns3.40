...
void HwmpProtocolMac::SetParent(Ptr<MeshWifiInterfaceMac> parent) {
  NS_LOG_FUNCTION(this << parent);
  m_parent = parent;
}

bool HwmpProtocolMac::ReceiveData(Ptr<Packet> packet,
  ...
  if ((destination == Mac48Address::GetBroadcast()) &&
    ...
    NS_LOG_DEBUG("Dropping frame; source " << source << " dest " << destination
                                           << " seqno "
...
bool HwmpProtocolMac::ReceiveAction(Ptr<Packet> packet,
                                    const WifiMacHeader &header) {
  NS_LOG_FUNCTION(this << packet << header);
  m_stats.rxMgt++;
...
bool HwmpProtocolMac::Receive(Ptr<Packet> packet, const WifiMacHeader &header) {
  NS_LOG_FUNCTION(this << packet << header);
  if (header.IsData()) {
...
bool HwmpProtocolMac::UpdateOutcomingFrame(Ptr<Packet> packet,
...
WifiActionHeader HwmpProtocolMac::GetWifiActionHeader() {
  WifiActionHeader actionHdr;
...
void HwmpProtocolMac::SendPreq(IePreq preq) {
...
void HwmpProtocolMac::SendPreqVector(std::vector<IePreq> preq) {
  NS_LOG_FUNCTION(this);
...
void HwmpProtocolMac::RequestDestination(Mac48Address dst,
...
void HwmpProtocolMac::SendMyPreq() {
  NS_LOG_FUNCTION(this);
...
void HwmpProtocolMac::ForwardPerr(
    std::vector<HwmpProtocol::FailedDestination> failedDestinations,
  ...
  MeshInformationElementVector elements;
  for (auto i = failedDestinations.begin(); i != failedDestinations.end();
       i++) {
    if (!perr->IsFull()) {
...
void HwmpProtocolMac::InitiatePerr(
    std::vector<HwmpProtocol::FailedDestination> failedDestinations,
    std::vector<Mac48Address> receivers) {
  NS_LOG_FUNCTION(this);
  ...
  // check it at any other place
  {
    for (auto i = receivers.begin(); i != receivers.end(); i++) {
      ...
      for (auto j = m_myPerr.receivers.begin(); j != m_myPerr.receivers.end();
           j++) {
        if ((*i) == (*j)) {
          should_add = false;
        }
  ...
  }
  {
    for (auto i = failedDestinations.begin(); i != failedDestinations.end();
         i++) {
      bool should_add = true;
      for (auto j = m_myPerr.destinations.begin();
...
void HwmpProtocolMac::SendMyPerr() {
  NS_LOG_FUNCTION(this);
  if (m_perrTimer.IsRunning()) {
    return;
  }
  m_perrTimer = Simulator::Schedule(m_protocol->GetPerrMinInterval(),
                                    &HwmpProtocolMac::SendMyPerr, this);
  ForwardPerr(m_myPerr.destinations, m_myPerr.receivers);
  m_myPerr.destinations.clear();
  m_myPerr.receivers.clear();
}

uint32_t HwmpProtocolMac::GetLinkMetric(Mac48Address peerAddress) const {
  return m_parent->GetLinkMetric(peerAddress);
}

uint16_t HwmpProtocolMac::GetChannelId() const {
...
HwmpProtocolMac::Statistics::Statistics()
    : txPreq(0), rxPreq(0), txPrep(0), rxPrep(0), txPerr(0), rxPerr(0),
      txMgt(0), txMgtBytes(0), rxMgt(0), rxMgtBytes(0), txData(0),
      txDataBytes(0), rxData(0), rxDataBytes(0) {}

void HwmpProtocolMac::Statistics::Print(std::ostream &os) const {
  os << "<Statistics "
        "txPreq= \""
     << txPreq << "\"" << std::endl
...
void HwmpProtocolMac::Report(std::ostream &os) const {
...
void HwmpProtocolMac::ResetStats() {
  NS_LOG_FUNCTION(this);
  m_stats = Statistics();
}

int64_t HwmpProtocolMac::AssignStreams(int64_t stream) {
