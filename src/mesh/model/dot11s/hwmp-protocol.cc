  ...
  static TypeId tid =
      TypeId("ns3::dot11s::HwmpProtocol")
          ...
          .AddAttribute(
              ...
              MakeTimeChecker())
          .AddAttribute("MaxTtl", "Initial value of Time To Live field",
                        UintegerValue(32),
          ...
          .AddAttribute(
              ...
              MakeUintegerChecker<uint8_t>(1))
          .AddAttribute("DoFlag", "Destination only HWMP flag",
                        BooleanValue(false),
...
HwmpProtocol::HwmpProtocol()
    : m_dataSeqno(1), m_hwmpSeqno(1), m_preqId(0),
      m_rtable(CreateObject<HwmpRtable>()), m_randomStart(Seconds(0.1)),
      m_maxQueueSize(255), m_dot11MeshHWMPmaxPREQretries(3),
      ...
      m_dot11MeshHWMPrannInterval(MicroSeconds(1024 * 5000)), m_isRoot(false),
      m_maxTtl(32), m_unicastPerrThreshold(32), m_unicastPreqThreshold(1),
      m_unicastDataThreshold(1), m_doFlag(false), m_rfFlag(false) {
  NS_LOG_FUNCTION(this);
  m_coefficient = CreateObject<UniformRandomVariable>();
}

HwmpProtocol::~HwmpProtocol() { NS_LOG_FUNCTION(this); }

void HwmpProtocol::DoInitialize() {
  NS_LOG_FUNCTION(this);
  m_coefficient->SetAttribute("Max", DoubleValue(m_randomStart.GetSeconds()));
  if (m_isRoot) {
    Time randomStart = Seconds(m_coefficient->GetValue());
    m_proactivePreqTimer = Simulator::Schedule(
        randomStart, &HwmpProtocol::SendProactivePreq, this);
  }
}

void HwmpProtocol::DoDispose() {
  NS_LOG_FUNCTION(this);
  ...
  if (destination == Mac48Address::GetBroadcast()) {
    ...
    for (auto plugin = m_interfaces.begin(); plugin != m_interfaces.end();
      ...
      for (auto i = receivers.begin(); i != receivers.end(); i++) {
        ...
        routeReply(true, packetCopy, source, destination, protocolType,
                   plugin->first);
      }
    }
  } else {
    return ForwardUnicast(sourceIface, source, destination, packet,
                          protocolType, routeReply, tag.GetTtl());
  }
  return true;
}

bool HwmpProtocol::RemoveRoutingStuff(uint32_t fromIface,
...
bool HwmpProtocol::ForwardUnicast(uint32_t sourceIface,
                                  const Mac48Address source,
                                  const Mac48Address destination,
                                  Ptr<Packet> packet, uint16_t protocolType,
                                  RouteReplyCallback routeReply, uint32_t ttl) {
  NS_LOG_FUNCTION(this << sourceIface << source << destination << packet
                       << protocolType << ttl);
  NS_ASSERT(destination != Mac48Address::GetBroadcast());
  HwmpRtable::LookupResult result = m_rtable->LookupReactive(destination);
  NS_LOG_DEBUG("Requested src = " << source << ", dst = " << destination
                                  << ", I am " << GetAddress()
...
void HwmpProtocol::ReceivePreq(IePreq preq, Mac48Address from,
  ...
  if ((freshInfo) ||
      ((m_rtable->LookupReactive(preq.GetOriginatorAddress()).retransmitter ==
       ...
       (m_rtable->LookupReactive(preq.GetOriginatorAddress()).metric >
        preq.GetMetric()))) {
  ...
  if ((m_rtable->LookupReactive(fromMp).retransmitter ==
       Mac48Address::GetBroadcast()) ||
  ...
  for (auto i = destinations.begin(); i != destinations.end(); i++) {
    if ((*i)->GetDestinationAddress() == Mac48Address::GetBroadcast()) {
      ...
      if (!preq.IsNeedNotPrep()) {
        SendPrep(GetAddress(), preq.GetOriginatorAddress(), from, (uint32_t)0,
                 preq.GetOriginatorSeqNumber(), GetNextHwmpSeqno(),
                 preq.GetLifetime(), interface);
      }
      break;
    }
    ...
    if ((!((*i)->IsDo())) &&
      ...
      if ((lifetime > 0) &&
        ...
        SendPrep((*i)->GetDestinationAddress(), preq.GetOriginatorAddress(),
                 from, result.metric, preq.GetOriginatorSeqNumber(),
                 result.seqnum, lifetime, interface);
  ...
  for (auto i = m_interfaces.begin(); i != m_interfaces.end(); i++) {
    ...
    Simulator::Schedule(forwardingDelay, &HwmpProtocolMac::SendPreq, i->second,
                        preq);
  }
}

void HwmpProtocol::ReceivePrep(IePrep prep, Mac48Address from,
                               uint32_t interface, Mac48Address fromMp,
                               uint32_t metric) {
  NS_LOG_FUNCTION(this << from << interface << fromMp << metric);
  ...
  if ((freshInfo) ||
      (((m_rtable->LookupReactive(prep.GetOriginatorAddress())).retransmitter ==
        Mac48Address::GetBroadcast()) ||
       ((m_rtable->LookupReactive(prep.GetOriginatorAddress())).metric >
        prep.GetMetric()))) {
    m_rtable->AddReactivePath(
    ...
    if (result.retransmitter != Mac48Address::GetBroadcast()) {
      m_rtable->AddPrecursor(prep.GetOriginatorAddress(), interface,
                             result.retransmitter, result.lifetime);
    }
  ...
  if (prep.GetDestinationAddress() == GetAddress()) {
    NS_LOG_DEBUG("I am " << GetAddress() << ", resolved "
                         << prep.GetOriginatorAddress());
    return;
  }
  if (result.retransmitter == Mac48Address::GetBroadcast()) {
  ...
  Time forwardingDelay = GetMeshPoint()->GetForwardingDelay();
  NS_LOG_DEBUG("Forwarding PREP from " << from << " with delay "
                                       << forwardingDelay.As(Time::US));
  Simulator::Schedule(forwardingDelay, &HwmpProtocolMac::SendPrep,
                      prep_sender->second, prep, result.retransmitter);
}

void HwmpProtocol::ReceivePerr(std::vector<FailedDestination> destinations,
...
void HwmpProtocol::SendPrep(Mac48Address src, Mac48Address dst,
                            Mac48Address retransmitter, uint32_t initMetric,
                            uint32_t originatorDsn, uint32_t destinationSN,
...
bool HwmpProtocol::Install(Ptr<MeshPointDevice> mp) {
...
void HwmpProtocol::PeerLinkStatus(Mac48Address meshPointAddress,
                                  Mac48Address peerAddress, uint32_t interface,
                                  bool status) {
  NS_LOG_FUNCTION(this << meshPointAddress << peerAddress
                       << interface << status);
  if (status) {
    return;
  }
  ...
  NS_LOG_DEBUG(destinations.size()
               << " failed destinations for peer address " << peerAddress);
  InitiatePathError(MakePathError(destinations));
}

void HwmpProtocol::SetNeighboursCallback(
    Callback<std::vector<Mac48Address>, uint32_t> cb) {
  m_neighboursCallback = cb;
}

bool HwmpProtocol::DropDataFrame(uint32_t seqno, Mac48Address source) {
...
void HwmpProtocol::ForwardPathError(PathError perr) {
  ...
  for (auto i = m_interfaces.begin(); i != m_interfaces.end(); i++) {
    ...
    Simulator::Schedule(forwardingDelay, &HwmpProtocolMac::ForwardPerr,
                        i->second, perr.destinations, receivers_for_interface);
...
std::vector<Mac48Address> HwmpProtocol::GetPreqReceivers(uint32_t interface) {
...
void HwmpProtocol::ReactivePathResolved(Mac48Address dst) {
  NS_LOG_FUNCTION(this << dst);
  ...
  while (packet.pkt) {
    ...
    packet.reply(true, packet.pkt, packet.src, packet.dst, packet.protocol,
                 result.ifIndex);
...
bool HwmpProtocol::ShouldSendPreq(Mac48Address dst) {
  NS_LOG_FUNCTION(this << dst);
  auto i = m_preqTimeouts.find(dst);
  if (i == m_preqTimeouts.end()) {
...
void HwmpProtocol::RetryPathDiscovery(Mac48Address dst, uint8_t numOfRetry) {
...
void HwmpProtocol::SendProactivePreq() {
  ...
  m_proactivePreqTimer =
      Simulator::Schedule(m_dot11MeshHWMPpathToRootInterval,
                          &HwmpProtocol::SendProactivePreq, this);
}

bool HwmpProtocol::GetDoFlag() const { return m_doFlag; }

bool HwmpProtocol::GetRfFlag() const { return m_rfFlag; }

Time HwmpProtocol::GetPreqMinInterval() {
  return m_dot11MeshHWMPpreqMinInterval;
}

Time HwmpProtocol::GetPerrMinInterval() {
  return m_dot11MeshHWMPperrMinInterval;
}

uint8_t HwmpProtocol::GetMaxTtl() const { return m_maxTtl; }
...
uint8_t HwmpProtocol::GetUnicastPerrThreshold() const {
...
void HwmpProtocol::Report(std::ostream &os) const {
  os << "<Hwmp "
        "address=\""
     << m_address << "\"" << std::endl
     << "maxQueueSize=\"" << m_maxQueueSize << "\"" << std::endl
     << "Dot11MeshHWMPmaxPREQretries=\""
     << (uint16_t)m_dot11MeshHWMPmaxPREQretries << "\"" << std::endl
     << "Dot11MeshHWMPnetDiameterTraversalTime=\""
     << m_dot11MeshHWMPnetDiameterTraversalTime.GetSeconds() << "\""
     << std::endl
     << "Dot11MeshHWMPpreqMinInterval=\""
     << m_dot11MeshHWMPpreqMinInterval.GetSeconds() << "\"" << std::endl
     << "Dot11MeshHWMPperrMinInterval=\""
     << m_dot11MeshHWMPperrMinInterval.GetSeconds() << "\"" << std::endl
     << "Dot11MeshHWMPactiveRootTimeout=\""
     << m_dot11MeshHWMPactiveRootTimeout.GetSeconds() << "\"" << std::endl
     << "Dot11MeshHWMPactivePathTimeout=\""
     << m_dot11MeshHWMPactivePathTimeout.GetSeconds() << "\"" << std::endl
     << "Dot11MeshHWMPpathToRootInterval=\""
     << m_dot11MeshHWMPpathToRootInterval.GetSeconds() << "\"" << std::endl
     << "Dot11MeshHWMPrannInterval=\""
     << m_dot11MeshHWMPrannInterval.GetSeconds() << "\"" << std::endl
...
int64_t HwmpProtocol::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_coefficient->SetStream(stream);
  return 1;
}

Ptr<HwmpRtable> HwmpProtocol::GetRoutingTable() const { return m_rtable; }

HwmpProtocol::QueuedPacket::QueuedPacket()
    : pkt(nullptr), protocol(0), inInterface(0) {}
} // namespace dot11s
} // namespace ns3
