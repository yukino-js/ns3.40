...
void IeBeaconTimingUnit::SetAid(uint8_t aid) { m_aid = aid; }

void IeBeaconTimingUnit::SetLastBeacon(uint16_t lastBeacon) {
  m_lastBeacon = lastBeacon;
}

void IeBeaconTimingUnit::SetBeaconInterval(uint16_t beaconInterval) {
  m_beaconInterval = beaconInterval;
}

uint8_t IeBeaconTimingUnit::GetAid() const { return m_aid; }

uint16_t IeBeaconTimingUnit::GetLastBeacon() const { return m_lastBeacon; }

uint16_t IeBeaconTimingUnit::GetBeaconInterval() const {
 ...
 * IeBeaconTiming
 *******************************************/
WifiInformationElementId IeBeaconTiming::ElementId() const {
...
void IeBeaconTiming::AddNeighboursTimingElementUnit(uint16_t aid,
                                                    Time last_beacon,
...
void IeBeaconTiming::DelNeighboursTimingElementUnit(uint16_t aid,
                                                    Time last_beacon,
                                                    Time beacon_interval) {
  for (auto i = m_neighbours.begin(); i != m_neighbours.end(); i++) {
    if (((*i)->GetAid() == AidToU8(aid)) &&
...
void IeBeaconTiming::ClearTimingElement() {
...
uint16_t IeBeaconTiming::GetInformationFieldSize() const {
  return (5 * m_numOfUnits);
}

void IeBeaconTiming::Print(std::ostream &os) const {
  os << "BeaconTiming=(Number of units=" << (uint16_t)m_numOfUnits;
  for (auto j = m_neighbours.begin(); j != m_neighbours.end(); j++) {
    os << "(AID=" << (uint16_t)(*j)->GetAid()
...
void IeBeaconTiming::SerializeInformationField(Buffer::Iterator i) const {
  for (auto j = m_neighbours.begin(); j != m_neighbours.end(); j++) {
    i.WriteU8((*j)->GetAid());
...
uint16_t IeBeaconTiming::DeserializeInformationField(Buffer::Iterator start,
                                                     uint16_t length) {
...
uint16_t IeBeaconTiming::TimestampToU16(Time t) {
  return ((uint16_t)((t.GetMicroSeconds() >> 8) & 0xffff));
};

uint16_t IeBeaconTiming::BeaconIntervalToU16(Time t) {
  return ((uint16_t)(t.GetMicroSeconds() >> 10) & 0xffff);
}

uint8_t IeBeaconTiming::AidToU8(uint16_t x) { return (uint8_t)(x & 0xff); }
...
bool IeBeaconTiming::operator==(const WifiInformationElement &a) const {
  try {
    ...
    for (unsigned int i = 0; i < m_neighbours.size(); i++) {
      if (!(*PeekPointer(m_neighbours[i]) ==
            *PeekPointer(aa.m_neighbours[i]))) {
        return false;
      }
    }
    return true;
