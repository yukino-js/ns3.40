...
TypeId MeshInformationElementVector::GetTypeId() {
...
TypeId MeshInformationElementVector::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t MeshInformationElementVector::GetSerializedSize() const {
  return GetSize();
}

void MeshInformationElementVector::Serialize(Buffer::Iterator start) const {
  for (auto i = m_elements.begin(); i != m_elements.end(); i++) {
    start = (*i)->Serialize(start);
  }
}

uint32_t MeshInformationElementVector::Deserialize(Buffer::Iterator start) {
  NS_FATAL_ERROR(
...
void MeshInformationElementVector::Print(std::ostream &os) const {
  for (auto i = m_elements.begin(); i != m_elements.end(); i++) {
    os << "(";
    (*i)->Print(os);
    os << ")";
...
MeshInformationElementVector::Iterator MeshInformationElementVector::Begin() {
  return m_elements.begin();
}

MeshInformationElementVector::Iterator MeshInformationElementVector::End() {
  return m_elements.end();
}

bool MeshInformationElementVector::AddInformationElement(
...
uint32_t MeshInformationElementVector::GetSize() const {
  uint32_t size = 0;
  for (auto i = m_elements.begin(); i != m_elements.end(); i++) {
...
bool MeshInformationElementVector::operator==(
    const MeshInformationElementVector &a) const {
