...
void FlameProtocolMac::SetParent(Ptr<MeshWifiInterfaceMac> parent) {
  m_parent = parent;
}

bool FlameProtocolMac::Receive(Ptr<Packet> packet,
...
bool FlameProtocolMac::UpdateOutcomingFrame(Ptr<Packet> packet,
...
uint16_t FlameProtocolMac::GetChannelId() const {
  return m_parent->GetFrequencyChannel();
}

FlameProtocolMac::Statistics::Statistics()
    : txUnicast(0), txBroadcast(0), txBytes(0), rxUnicast(0), rxBroadcast(0),
      rxBytes(0) {}

void FlameProtocolMac::Statistics::Print(std::ostream &os) const {
  os << "<Statistics "
        "txUnicast=\""
     << txUnicast
     << "\" "
...
void FlameProtocolMac::Report(std::ostream &os) const {
...
void FlameProtocolMac::ResetStats() { m_stats = Statistics(); }
