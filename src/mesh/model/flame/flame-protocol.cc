...
TypeId FlameTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t FlameTag::GetSerializedSize() const { return 12; }

void FlameTag::Serialize(TagBuffer i) const {
  uint8_t buf[6];
  receiver.CopyTo(buf);
  for (int j = 0; j < 6; j++) {
    i.WriteU8(buf[j]);
...
void FlameTag::Deserialize(TagBuffer i) {
  uint8_t buf[6];
  for (int j = 0; j < 6; j++) {
    buf[j] = i.ReadU8();
  }
...
void FlameTag::Print(std::ostream &os) const {
...
FlameProtocol::FlameProtocol()
    : m_address(Mac48Address()), m_broadcastInterval(Seconds(5)),
      m_lastBroadcast(Seconds(0)), m_maxCost(32), m_myLastSeqno(1),
      m_rtable(CreateObject<FlameRtable>()) {}

FlameProtocol::~FlameProtocol() {}

void FlameProtocol::DoDispose() {
  m_interfaces.clear();
  m_rtable = nullptr;
  m_mp = nullptr;
}

bool FlameProtocol::RequestRoute(uint32_t sourceIface,
...
bool FlameProtocol::RemoveRoutingStuff(uint32_t fromIface,
                                       const Mac48Address source,
                                       const Mac48Address destination,
                                       Ptr<Packet> packet,
                                       uint16_t &protocolType) {
...
bool FlameProtocol::Install(Ptr<MeshPointDevice> mp) {
  m_mp = mp;
...
Mac48Address FlameProtocol::GetAddress() { return m_address; }

bool FlameProtocol::HandleDataFrame(uint16_t seqno, Mac48Address source,
  ...
  m_rtable->AddPath(source, receiver, fromInterface, flameHdr.GetCost(),
                    flameHdr.GetSeqno());
  return false;
}
...
void FlameProtocol::Report(std::ostream &os) const {
  os << "<Flame "
        "address=\""
     << m_address << "\"" << std::endl
