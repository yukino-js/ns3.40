...
TypeId FlameRtable::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::flame::FlameRtable")
          .SetParent<Object>()
          .SetGroupName("Mesh")
          .AddConstructor<FlameRtable>()
          .AddAttribute("Lifetime", "The lifetime of the routing entry",
...
FlameRtable::FlameRtable() : m_lifetime(Seconds(120)) {}

FlameRtable::~FlameRtable() {}

void FlameRtable::DoDispose() { m_routes.clear(); }

void FlameRtable::AddPath(const Mac48Address destination,
...
FlameRtable::LookupResult FlameRtable::Lookup(Mac48Address destination) {
  auto i = m_routes.find(destination);
  if (i == m_routes.end()) {
    return LookupResult();
  }
  ...
  return LookupResult(i->second.retransmitter, i->second.interface,
                      i->second.cost, i->second.seqnum);
}

bool FlameRtable::LookupResult::operator==(
    const FlameRtable::LookupResult &o) const {
...
bool FlameRtable::LookupResult::IsValid() const {
  return !(retransmitter == Mac48Address::GetBroadcast() &&
           ifIndex == INTERFACE_ANY && cost == MAX_COST && seqnum == 0);
}
