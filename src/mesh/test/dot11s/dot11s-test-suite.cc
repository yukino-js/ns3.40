...
void MeshHeaderTest::DoRun() {
  {
    ...
    NS_TEST_ASSERT_MSG_EQ(
        a, b, "Mesh header roundtrip serialization works, 3 addresses");
  }
  {
    MeshHeader a;
    a.SetAddressExt(2);
    ...
    packet->RemoveHeader(b);
    NS_TEST_ASSERT_MSG_EQ(
        a, b, "Mesh header roundtrip serialization works, 2 addresses");
  }
  {
    MeshHeader a;
    a.SetAddressExt(1);
    ...
    packet->RemoveHeader(b);
    NS_TEST_ASSERT_MSG_EQ(
        a, b, "Mesh header roundtrip serialization works, 1 address");
  }
}
...
HwmpRtableTest::HwmpRtableTest()
    : TestCase("HWMP routing table"), dst("01:00:00:01:00:01"),
      hop("01:00:00:01:00:03"), iface(8010), metric(10), seqnum(1),
      expire(Seconds(10)) {
...
void HwmpRtableTest::TestLookup() {
  ...
  NS_TEST_EXPECT_MSG_EQ(table->LookupProactive().IsValid(), false,
                        "Proactive lookup works");
}
...
void HwmpRtableTest::TestExpire() {
  ...
  NS_TEST_EXPECT_MSG_EQ((table->LookupProactiveExpired() == correct), true,
...
void HwmpRtableTest::TestPrecursorFind() {
  ...
  NS_TEST_EXPECT_MSG_EQ(precursors.size(), precursorList.size(),
                        "Precursors size works");
  for (unsigned i = 0; i < precursors.size(); i++) {
    NS_TEST_EXPECT_MSG_EQ(precursorList[i].first, iface,
                          "Precursors lookup works");
    NS_TEST_EXPECT_MSG_EQ(precursorList[i].second, precursors[i],
                          "Precursors lookup works");
  }
}

void HwmpRtableTest::DoRun() {
  ...
  Simulator::Schedule(expire + Seconds(3), &HwmpRtableTest::TestPrecursorFind,
                      this);
...
void PeerLinkFrameStartTest::DoRun() {
  {
    PeerLinkOpenStart a;
...
Dot11sTestSuite::Dot11sTestSuite() : TestSuite("devices-mesh-dot11s", UNIT) {
