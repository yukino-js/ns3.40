...
HwmpDoRfRegressionTest::HwmpDoRfRegressionTest()
    : TestCase("HWMP target flags regression test"), m_nodes(nullptr),
      m_time(Seconds(5)), m_sentPktsCounterA(0), m_sentPktsCounterB(0),
      m_sentPktsCounterC(0) {}

HwmpDoRfRegressionTest::~HwmpDoRfRegressionTest() { delete m_nodes; }
...
void HwmpDoRfRegressionTest::InstallApplications() {
  ...
  m_clientSocketA->SetRecvCallback(
      MakeCallback(&HwmpDoRfRegressionTest::HandleReadClient, this));
  ...
  m_clientSocketB->SetRecvCallback(
      MakeCallback(&HwmpDoRfRegressionTest::HandleReadClient, this));
  Simulator::ScheduleWithContext(
  ...
  m_clientSocketB->SetRecvCallback(
      MakeCallback(&HwmpDoRfRegressionTest::HandleReadClient, this));
  Simulator::ScheduleWithContext(
      m_clientSocketB->GetNode()->GetId(), Seconds(2.4),
...
void HwmpDoRfRegressionTest::SendDataA(Ptr<Socket> socket) {
  if ((Simulator::Now() < m_time) && (m_sentPktsCounterA < 300)) {
    ...
    Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(1.0),
                                   &HwmpDoRfRegressionTest::SendDataA, this,
                                   socket);
  }
}

void HwmpDoRfRegressionTest::SendDataB(Ptr<Socket> socket) {
  if ((Simulator::Now() < m_time) && (m_sentPktsCounterA < 300)) {
    socket->Send(Create<Packet>(100));
    m_sentPktsCounterB++;
    Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(1.0),
                                   &HwmpDoRfRegressionTest::SendDataB, this,
                                   socket);
  }
}

void HwmpDoRfRegressionTest::SendDataC(Ptr<Socket> socket) {
  if ((Simulator::Now() < m_time) && (m_sentPktsCounterA < 300)) {
    socket->Send(Create<Packet>(100));
    m_sentPktsCounterC++;
    Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(1.0),
                                   &HwmpDoRfRegressionTest::SendDataC, this,
                                   socket);
  }
}

void HwmpDoRfRegressionTest::HandleReadServer(Ptr<Socket> socket) {
  Ptr<Packet> packet;
  Address from;
...
void HwmpDoRfRegressionTest::HandleReadClient(Ptr<Socket> socket) {
  Ptr<Packet> packet;
  Address from;
  while ((packet = socket->RecvFrom(from))) {
  }
