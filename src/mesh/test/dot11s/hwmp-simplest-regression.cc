...
HwmpSimplestRegressionTest::HwmpSimplestRegressionTest()
    : TestCase("Simplest HWMP regression test"), m_nodes(nullptr),
      m_time(Seconds(15)), m_sentPktsCounter(0) {}

HwmpSimplestRegressionTest::~HwmpSimplestRegressionTest() { delete m_nodes; }

void HwmpSimplestRegressionTest::DoRun() {
...
void HwmpSimplestRegressionTest::CreateNodes() {
...
void HwmpSimplestRegressionTest::ResetPosition() {
...
void HwmpSimplestRegressionTest::CheckResults() {
  for (int i = 0; i < 2; ++i) {
    NS_PCAP_TEST_EXPECT_EQ(PREFIX << "-" << i << "-1.pcap");
  }
}

void HwmpSimplestRegressionTest::SendData(Ptr<Socket> socket) {
  if ((Simulator::Now() < m_time) && (m_sentPktsCounter < 300)) {
    ...
    Simulator::ScheduleWithContext(socket->GetNode()->GetId(), Seconds(0.05),
                                   &HwmpSimplestRegressionTest::SendData, this,
                                   socket);
  }
}

void HwmpSimplestRegressionTest::HandleReadServer(Ptr<Socket> socket) {
  Ptr<Packet> packet;
  Address from;
...
void HwmpSimplestRegressionTest::HandleReadClient(Ptr<Socket> socket) {
  Ptr<Packet> packet;
  Address from;
  while ((packet = socket->RecvFrom(from))) {
  }
