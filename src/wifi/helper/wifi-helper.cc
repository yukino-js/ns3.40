...
void WifiPhyHelper::Set(std::string name, const AttributeValue &v) {
  for (auto &phy : m_phys) {
    phy.Set(name, v);
  }
}

void WifiPhyHelper::Set(uint8_t linkId, std::string name,
                        const AttributeValue &v) {
  m_phys.at(linkId).Set(name, v);
}

void WifiPhyHelper::DisablePreambleDetectionModel() {
  for (auto &preambleDetectionModel : m_preambleDetectionModel) {
    preambleDetectionModel.SetTypeId(TypeId());
  }
}

void WifiPhyHelper::PcapSniffTxEvent(Ptr<PcapFileWrapper> file,
...
void WifiPhyHelper::PcapSniffRxEvent(Ptr<PcapFileWrapper> file,
                                     Ptr<const Packet> packet,
                                     uint16_t channelFreqMhz,
                                     WifiTxVector txVector, MpduInfo aMpdu,
...
void WifiPhyHelper::GetRadiotapHeader(RadiotapHeader &header,
  ...
  if (txVector.GetMode(staId).GetModulationClass() == WIFI_MOD_CLASS_HT) {
    ...
    header.SetMcsFields(mcsKnown, mcsFlags,
                        txVector.GetMode(staId).GetMcsValue());
  }
  ...
  if (txVector.GetMode(staId).GetModulationClass() == WIFI_MOD_CLASS_VHT) {
    ...
    header.SetVhtFields(vhtKnown, vhtFlags, vhtBandwidth, vhtMcsNss, vhtCoding,
                        vhtGroupId, vhtPartialAid);
  }
  ...
  if (txVector.GetMode(staId).GetModulationClass() == WIFI_MOD_CLASS_HE) {
    ...
    if (preamble == WIFI_PREAMBLE_HE_MU || preamble == WIFI_PREAMBLE_HE_TB) {
      ...
      data2 |=
          (((!txVector.GetHeMuUserInfo(staId).ru.GetPrimary80MHz()) << 15) &
           0x8000);
    }
...
void WifiPhyHelper::SetPcapDataLinkType(SupportedPcapDataLinkTypes dlt) {
  switch (dlt) {
  case DLT_IEEE802_11:
    m_pcapDlt = PcapHelper::DLT_IEEE802_11;
    return;
...
PcapHelper::DataLinkType WifiPhyHelper::GetPcapDataLinkType() const {
  return m_pcapDlt;
}

void WifiPhyHelper::EnablePcapInternal(std::string prefix, Ptr<NetDevice> nd,
                                       bool promiscuous,
...
void WifiPhyHelper::EnableAsciiInternal(Ptr<OutputStreamWrapper> stream,
  ...
  Config::Connect(oss.str(),
                  MakeBoundCallback(&AsciiPhyTransmitSinkWithContext, stream));
}
...
WifiHelper::WifiHelper()
    : m_standard(WIFI_STANDARD_80211ax),
      m_selectQueueCallback(&SelectQueueByDSField), m_enableFlowControl(true) {
...
void WifiHelper::SetStandard(WifiStandard standard) { m_standard = standard; }

void WifiHelper::DisableFlowControl() { m_enableFlowControl = false; }

void WifiHelper::SetSelectQueueCallback(SelectQueueCallback f) {
  m_selectQueueCallback = f;
}

NetDeviceContainer WifiHelper::Install(const WifiPhyHelper &phyHelper,
  ...
  for (auto i = first; i != last; ++i) {
    ...
    if ((m_standard >= WIFI_STANDARD_80211ax) &&
        (m_obssPdAlgorithm.IsTypeIdSet())) {
      Ptr<ObssPdAlgorithm> obssPdAlgorithm =
          m_obssPdAlgorithm.Create<ObssPdAlgorithm>();
    ...
    if (m_enableFlowControl) {
      ...
      if (qosSupported.Get()) {
        ...
        for (auto &ac : {AC_BE, AC_BK, AC_VI, AC_VO}) {
          ...
          ndqi->GetTxQueue(static_cast<std::size_t>(ac))
              ->ConnectQueueTraces(wmq);
        }
...
NetDeviceContainer WifiHelper::Install(const WifiPhyHelper &phy,
...
void WifiHelper::EnableLogComponents() {
...
int64_t WifiHelper::AssignStreams(NetDeviceContainer c, int64_t stream) {
