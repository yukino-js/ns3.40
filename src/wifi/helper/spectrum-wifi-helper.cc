...
void SpectrumWifiPhyHelper::SetChannel(const Ptr<SpectrumChannel> channel) {
  AddChannel(channel);
}

void SpectrumWifiPhyHelper::SetChannel(const std::string &channelName) {
  AddChannel(channelName);
}

void SpectrumWifiPhyHelper::AddChannel(const Ptr<SpectrumChannel> channel,
                                       const FrequencyRange &freqRange) {
  m_channels[freqRange] = channel;
  AddWifiBandwidthFilter(channel);
}

void SpectrumWifiPhyHelper::AddChannel(const std::string &channelName,
                                       const FrequencyRange &freqRange) {
  Ptr<SpectrumChannel> channel = Names::Find<SpectrumChannel>(channelName);
  AddChannel(channel, freqRange);
}

void SpectrumWifiPhyHelper::AddWifiBandwidthFilter(
  ...
  while (p && !found) {
    if (DynamicCast<const WifiBandwidthFilter>(p)) {
      NS_LOG_DEBUG("Found existing WifiBandwidthFilter for channel "
                   << channel);
      found = true;
    } else {
      NS_LOG_DEBUG("Found different SpectrumTransmitFilter for channel "
                   << channel);
      p = p->GetNext();
    }
  }
...
void SpectrumWifiPhyHelper::AddPhyToFreqRangeMapping(
    uint8_t linkId, const FrequencyRange &freqRange) {
...
void SpectrumWifiPhyHelper::ResetPhyToFreqRangeMapping() {
...
SpectrumWifiPhyHelper::Create(Ptr<Node> node, Ptr<WifiNetDevice> device) const {
  ...
  for (std::size_t i = 0; i < m_phys.size(); i++) {
    ...
    phy->SetChannelSwitchedCallback(
        MakeCallback(&SpectrumWifiPhyHelper::SpectrumChannelSwitched, this)
            .Bind(phy));
...
void SpectrumWifiPhyHelper::SpectrumChannelSwitched(
