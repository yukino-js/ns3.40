...
Ptr<YansWifiChannel> YansWifiChannelHelper::Create() const {
...
int64_t YansWifiChannelHelper::AssignStreams(Ptr<YansWifiChannel> c,
                                             int64_t stream) {
...
void YansWifiPhyHelper::SetChannel(std::string channelName) {
...
YansWifiPhyHelper::Create(Ptr<Node> node, Ptr<WifiNetDevice> device) const {
  ...
  Ptr<InterferenceHelper> interference =
      m_interferenceHelper.Create<InterferenceHelper>();
  ...
  if (m_preambleDetectionModel.front().IsTypeIdSet()) {
    auto preambleDetection =
        m_preambleDetectionModel.front().Create<PreambleDetectionModel>();
