...
void WifiTest::SendOnePacket(Ptr<WifiNetDevice> dev) {
  Ptr<Packet> p = Create<Packet>();
  dev->Send(p, dev->GetBroadcast(), 1);
}

void WifiTest::CreateOne(Vector pos, Ptr<YansWifiChannel> channel) {
  ...
  if (mac->GetTypeOfStation() == STA) {
    StaticCast<StaWifiMac>(mac)->SetAssocManager(
        CreateObject<WifiDefaultAssocManager>());
  }
...
void WifiTest::RunOne() {
  Ptr<YansWifiChannel> channel = CreateObject<YansWifiChannel>();
  Ptr<PropagationDelayModel> propDelay =
      m_propDelay.Create<PropagationDelayModel>();
  Ptr<PropagationLossModel> propLoss =
...
void InterferenceHelperSequenceTest::SendOnePacket(Ptr<WifiNetDevice> dev) {
  Ptr<Packet> p = Create<Packet>(1000);
  dev->Send(p, dev->GetBroadcast(), 1);
}

void InterferenceHelperSequenceTest::SwitchCh(Ptr<WifiNetDevice> dev) {
  Ptr<WifiPhy> p = dev->GetPhy();
  p->SetOperatingChannel(
      WifiPhy::ChannelTuple{40, 0, (int)(WIFI_PHY_BAND_5GHZ), 0});
}

Ptr<Node>
InterferenceHelperSequenceTest::CreateOne(Vector pos,
...
void InterferenceHelperSequenceTest::DoRun() {
  m_mac.SetTypeId("ns3::AdhocWifiMac");
  m_propDelay.SetTypeId("ns3::ConstantSpeedPropagationDelayModel");
  m_manager.SetTypeId("ns3::ConstantRateWifiManager");
  ...
  Ptr<PropagationDelayModel> propDelay =
      m_propDelay.Create<PropagationDelayModel>();
  Ptr<MatrixPropagationLossModel> propLoss =
  ...
  propLoss->SetLoss(senderB->GetObject<MobilityModel>(),
                    rxOnly->GetObject<MobilityModel>(), 0, true);
...
void DcfImmediateAccessBroadcastTestCase::SendOnePacket(
...
void DcfImmediateAccessBroadcastTestCase::DoRun() {
  ...
  Ptr<ConstantPositionMobilityModel> txMobility =
  ...
  txPhy->TraceConnectWithoutContext(
      "PhyTxBegin",
      MakeCallback(&DcfImmediateAccessBroadcastTestCase::NotifyPhyTxBegin,
                   this));
  ...
  Simulator::Schedule(Seconds(1.0),
                      &DcfImmediateAccessBroadcastTestCase::SendOnePacket, this,
                      txDev);
...
Bug730TestCase::Bug730TestCase()
    : TestCase("Test case for Bug 730"), m_received(0) {}

Bug730TestCase::~Bug730TestCase() {}

void Bug730TestCase::Receive(std::string context, Ptr<const Packet> p,
                             const Address &adr) {
...
QosFragmentationTestCase::QosFragmentationTestCase()
    : TestCase("Test case for fragmentation with QoS stations"), m_received(0),
      m_fragments(0) {}

QosFragmentationTestCase::~QosFragmentationTestCase() {}

void QosFragmentationTestCase::Receive(std::string context, Ptr<const Packet> p,
                                       const Address &adr) {
  if (p->GetSize() == 1400) {
    m_received++;
  }
...
void QosFragmentationTestCase::Transmit(std::string context,
                                        Ptr<const Packet> p, double power) {
  WifiMacHeader hdr;
  p->PeekHeader(hdr);
  if (hdr.IsQosData()) {
...
void QosFragmentationTestCase::DoRun() {
  NodeContainer wifiStaNode;
  wifiStaNode.Create(1);
  ...
  NS_TEST_ASSERT_MSG_EQ(m_fragments, 4,
...
void SetChannelFrequencyTest::DoRun() {
  NodeContainer wifiStaNode;
  wifiStaNode.Create(1);
  NodeContainer wifiApNode;
  wifiApNode.Create(1);
  ...
  }
  {
    ...
    staDevice = wifi.Install(phy, macSta, wifiStaNode.Get(0));
    phySta = GetYansWifiPhyPtr(staDevice);
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelNumber(), 42,
                          "802.11ac configuration");
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelWidth(), 80,
                          "802.11ac configuration");
    NS_TEST_ASSERT_MSG_EQ(phySta->GetFrequency(), 5210,
                          "802.11ac configuration");
  }
  {
    // case 7:
  ...
  }
  {
    ...
    staDevice = wifi.Install(phy, macSta, wifiStaNode.Get(0));
    phySta = GetYansWifiPhyPtr(staDevice);
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelNumber(), 7,
  ...
  }
  {
    ...
    staDevice = wifi.Install(phy, macSta, wifiStaNode.Get(0));
    phySta = GetYansWifiPhyPtr(staDevice);
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelNumber(), 172,
                          "802.11p 10Mhz configuration");
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelWidth(), 10,
                          "802.11p 10Mhz configuration");
  ...
  }
  {
    ...
    staDevice = wifi.Install(phy, macSta, wifiStaNode.Get(0));
    phySta = GetYansWifiPhyPtr(staDevice);
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelNumber(), 171,
                          "802.11p 5Mhz configuration");
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelWidth(), 5,
                          "802.11p 5Mhz configuration");
    NS_TEST_ASSERT_MSG_EQ(phySta->GetFrequency(), 5860,
                          "802.11p 5Mhz configuration");
    phy.Set("ChannelSettings",
  ...
  }
  {
    ...
    staDevice = wifi.Install(phy, macSta, wifiStaNode.Get(0));
    phySta = GetYansWifiPhyPtr(staDevice);
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelNumber(), 44,
  ...
  }
  {
    ...
    NS_TEST_ASSERT_MSG_EQ(phySta->GetChannelWidth(), 20,
                          "802.11 5GHz configuration");
    NS_TEST_ASSERT_MSG_EQ(phySta->GetFrequency(), 5500,
                          "802.11 5GHz configuration");
  }
  {
    // case 18:
...
void Bug2222TestCase::DoRun() {
  ...
  Ptr<PacketSocketClient> clientLowPriority =
  ...
  Ptr<PacketSocketClient> clientHighPriority =
      CreateObject<PacketSocketClient>();
  ...
  NS_TEST_ASSERT_MSG_EQ(m_countInternalCollisions, 1,
...
Bug2843TestCase::Bug2843TestCase()
    : TestCase("Test case for Bug 2843"), m_channelWidth(20) {}
...
void Bug2843TestCase::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ(
      std::get<2>(m_distinctTuples[0]), 193,
  ...
  NS_TEST_ASSERT_MSG_EQ(
      std::get<2>(m_distinctTuples[1]), 385,
...
Bug2831TestCase::Bug2831TestCase()
    : TestCase("Test case for Bug 2831"), m_assocReqCount(0),
      m_assocRespCount(0), m_countOperationalChannelWidth20(0),
      m_countOperationalChannelWidth40(0) {}

Bug2831TestCase::~Bug2831TestCase() {}

void Bug2831TestCase::ChangeSupportedChannelWidth() {
  m_apPhy->SetOperatingChannel(
      WifiPhy::ChannelTuple{38, 40, (int)(WIFI_PHY_BAND_5GHZ), 0});
  m_staPhy->SetOperatingChannel(
      WifiPhy::ChannelTuple{38, 40, (int)(WIFI_PHY_BAND_5GHZ), 0});
}

void Bug2831TestCase::RxCallback(std::string context, Ptr<const Packet> p,
                                 RxPowerWattPerChannelBand rxPowersW) {
...
void Bug2831TestCase::DoRun() {
  ...
  Ptr<PropagationDelayModel> propagationDelay =
      propDelay.Create<PropagationDelayModel>();
  ...
  m_apPhy->SetOperatingChannel(
      WifiPhy::ChannelTuple{36, 20, (int)(WIFI_PHY_BAND_5GHZ), 0});
  ...
  StaticCast<StaWifiMac>(staMac)->SetAssocManager(
      CreateObject<WifiDefaultAssocManager>());
  ...
  Simulator::Schedule(Seconds(1.0),
                      &Bug2831TestCase::ChangeSupportedChannelWidth, this);

  Simulator::Stop(Seconds(3.0));
  Simulator::Run();
  Simulator::Destroy();

  NS_TEST_ASSERT_MSG_EQ(m_assocReqCount, 2,
                        "Second Association request not received");
  NS_TEST_ASSERT_MSG_EQ(m_assocRespCount, 2,
                        "Second Association response not received");
  NS_TEST_ASSERT_MSG_EQ(
      m_countOperationalChannelWidth20, 10,
      "Incorrect operational channel width before channel change");
  NS_TEST_ASSERT_MSG_EQ(
      m_countOperationalChannelWidth40, 20,
...
void StaWifiMacScanningTestCase::TurnBeaconGenerationOn(Ptr<Node> apNode) {
  Ptr<WifiNetDevice> netDevice =
      DynamicCast<WifiNetDevice>(apNode->GetDevice(0));
...
NodeContainer StaWifiMacScanningTestCase::Setup(bool nearestApBeaconGeneration,
  ...
  mac.SetType("ns3::ApWifiMac", "BeaconGeneration",
...
void StaWifiMacScanningTestCase::DoRun() {
  {
    ...
    NS_TEST_ASSERT_MSG_EQ(m_associatedApBssid, nearestApAddr,
  ...
  m_associatedApBssid = Mac48Address();
  {
    ...
    Mac48Address secondNearestApAddr =
        DynamicCast<WifiNetDevice>(nodes.Get(1)->GetDevice(0))
            ->GetMac()
            ->GetAddress();

    Simulator::Schedule(Seconds(0.1), &StaWifiMacScanningTestCase::TurnApOff,
                        this, nearestAp);
...
Bug2470TestCase::Bug2470TestCase()
    : TestCase("Test case for Bug 2470"), m_receivedNormalMpduCount(0),
      m_receivedAmpduCount(0), m_failedActionCount(0),
      m_addbaEstablishedCount(0), m_addbaPendingCount(0),
      m_addbaRejectedCount(0), m_addbaNoReplyCount(0), m_addbaResetCount(0) {}

Bug2470TestCase::~Bug2470TestCase() {}

void Bug2470TestCase::AddbaStateChangedCallback(
    std::string context, Time t, Mac48Address recipient, uint8_t tid,
    OriginatorBlockAckAgreement::State state) {
  switch (state) {
...
void Bug2470TestCase::TxCallback(Ptr<ListErrorModel> rxErrorModel,
                                 std::string context, WifiConstPsduMap psduMap,
...
void Bug2470TestCase::RxCallback(std::string context, Ptr<const Packet> p,
                                 uint16_t channelFreqMhz, WifiTxVector txVector,
...
void Bug2470TestCase::RxErrorCallback(std::string context, Ptr<const Packet> p,
                                      double snr) {
  Ptr<Packet> packet = p->Copy();
  WifiMacHeader hdr;
  packet->RemoveHeader(hdr);
...
void Bug2470TestCase::RunSubtest(TypeOfStation rcvErrorType) {
  ...
  Simulator::Schedule(Seconds(0.5), &Bug2470TestCase::SendPacketBurst, this, 1,
                      apDevice.Get(0), staDevice.Get(0)->GetAddress());
  Simulator::Schedule(Seconds(0.5) + MicroSeconds(5),
                      &Bug2470TestCase::SendPacketBurst, this, 4,
                      apDevice.Get(0), staDevice.Get(0)->GetAddress());
  Simulator::Schedule(Seconds(0.8), &Bug2470TestCase::SendPacketBurst, this, 1,
                      apDevice.Get(0), staDevice.Get(0)->GetAddress());
...
void Bug2470TestCase::DoRun() {
  {
    ...
    NS_TEST_ASSERT_MSG_EQ(m_addbaPendingCount, 2,
                          "Incorrect number of times the ADDBA state machine "
                          "was in pending state on subtest 1");
    NS_TEST_ASSERT_MSG_EQ(m_addbaRejectedCount, 0,
                          "Incorrect number of times the ADDBA state machine "
                          "was in rejected state on subtest 1");
    NS_TEST_ASSERT_MSG_EQ(m_addbaNoReplyCount, 1,
                          "Incorrect number of times the ADDBA state machine "
                          "was in no_reply state on subtest 1");
    NS_TEST_ASSERT_MSG_EQ(m_addbaResetCount, 1,
                          "Incorrect number of times the ADDBA state machine "
...
Issue40TestCase::Issue40TestCase()
    : TestCase("Test case for issue #40"), m_rxCount(0), m_txCount(0),
...
void Issue40TestCase::RunOne(bool useAmpdu) {
  ...
  Ptr<WaypointMobilityModel> staWaypointMobility =
      DynamicCast<WaypointMobilityModel>(
          wifiStaNode.Get(0)->GetObject<MobilityModel>());
  staWaypointMobility->AddWaypoint(
      Waypoint(Seconds(1.0), Vector(10.0, 0.0, 0.0)));
  staWaypointMobility->AddWaypoint(
  ...
  if (useAmpdu) {
    ...
    ptr.Get<QosTxop>()->SetAttribute("UseExplicitBarAfterMissedBlockAck",
                                     BooleanValue(false));
  }
  ...
  Simulator::Schedule(Seconds(2.1), &Issue40TestCase::SendPackets, this,
                      useAmpdu ? 2 : 1, apDevice.Get(0),
                      staDevice.Get(0)->GetAddress());
  Simulator::Schedule(Seconds(2.2), &Issue40TestCase::SendPackets, this,
                      useAmpdu ? 2 : 1, apDevice.Get(0),
                      staDevice.Get(0)->GetAddress());
  Simulator::Schedule(Seconds(2.3), &Issue40TestCase::SendPackets, this,
                      useAmpdu ? 2 : 1, apDevice.Get(0),
                      staDevice.Get(0)->GetAddress());
  Simulator::Schedule(Seconds(2.4), &Issue40TestCase::SendPackets, this,
                      useAmpdu ? 2 : 1, apDevice.Get(0),
                      staDevice.Get(0)->GetAddress());
  Simulator::Schedule(Seconds(2.5), &Issue40TestCase::SendPackets, this,
                      useAmpdu ? 2 : 1, apDevice.Get(0),
                      staDevice.Get(0)->GetAddress());
  ...
  NS_TEST_ASSERT_MSG_EQ(m_txCount, (useAmpdu ? 14 : 7),
  ...
  NS_TEST_ASSERT_MSG_EQ(m_txMacFinalDataFailedCount, 1,
...
void Issue169TestCase::TxCallback(std::string context, WifiConstPsduMap psdus,
                                  WifiTxVector txVector, double txPowerW) {
...
void IdealRateManagerChannelWidthTest::ChangeChannelWidth(
...
void IdealRateManagerChannelWidthTest::CheckLastSelectedMode(
    WifiMode expectedMode) {
  NS_TEST_ASSERT_MSG_EQ(m_txMode, expectedMode,
                        "Last selected WifiMode "
                            << m_txMode << " does not match expected WifiMode "
                            << expectedMode);
}
...
void IdealRateManagerChannelWidthTest::DoRun() {
  ...
  Config::Connect(
      "/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/$ns3::WifiPhy/"
      "PhyTxPsduBegin",
...
void IdealRateManagerMimoTest::DoRun() {
  ...
  Simulator::Schedule(Seconds(9.9),
                      &IdealRateManagerMimoTest::SetApMimoSettings, this, 1, 1);
  Simulator::Schedule(
      Seconds(9.9), &IdealRateManagerMimoTest::SetStaMimoSettings, this, 1, 1);
  Simulator::Schedule(Seconds(10.0), &IdealRateManagerMimoTest::SendPacket,
                      this, apDevice.Get(0), staDevice.Get(0)->GetAddress());
...
bool HeRuMcsDataRateTestCase::CheckDataRate(HeRu::RuType ruType,
                                            std::string mcs, uint8_t nss,
                                            uint16_t guardInterval,
                                            uint16_t expectedDataRate) {
  uint16_t approxWidth = HeRu::GetBandwidth(ruType);
  WifiMode mode(mcs);
  uint64_t dataRate =
      round(mode.GetDataRate(approxWidth, guardInterval, nss) / 100000.0);
...
WifiMgtHeaderTest::WifiMgtHeaderTest()
    : HeaderSerializationTestCase(
          "Check (de)serialization of a test management header") {}
...
void WifiMgtHeaderTest::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(frame.Get<Ssid>().size(), 0,
                        "Expected no Ssid IE to be included");
  ...
  NS_TEST_EXPECT_MSG_EQ(frame.Get<SupportedRates>().has_value(), true,
                        "Expected a SupportedRates IE to be included");
  NS_TEST_EXPECT_MSG_EQ(frame.Get<ExtendedSupportedRatesIE>().has_value(), true,
  ...
  NS_TEST_EXPECT_MSG_EQ(frame.Get<ExtendedSupportedRatesIE>().has_value(), true,
                        "Expected an ExtendedSupportedRatesIE to be included");
  NS_TEST_EXPECT_MSG_EQ(frame.Get<Ssid>().size(), 1,
                        "Expected one Ssid IE to be included");
  NS_TEST_EXPECT_MSG_EQ(std::string(frame.Get<Ssid>().front().PeekString()),
                        "Ssid One", "Incorrect SSID");
  ...
  NS_TEST_EXPECT_MSG_EQ(frame.Get<ExtendedSupportedRatesIE>().has_value(), true,
                        "Expected an ExtendedSupportedRatesIE to be included");
  NS_TEST_EXPECT_MSG_EQ(frame.Get<Ssid>().size(), 2,
                        "Expected two Ssid IEs to be included");
  NS_TEST_EXPECT_MSG_EQ(std::string(frame.Get<Ssid>().front().PeekString()),
                        "Ssid One", "Incorrect first SSID");
  NS_TEST_EXPECT_MSG_EQ(std::string(frame.Get<Ssid>().back().PeekString()),
                        "Ssid Two", "Incorrect second SSID");
