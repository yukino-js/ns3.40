...
private:
  ...
  typedef std::pair<uint64_t, uint64_t>
      ExpectedGrant; //!< the expected grant typedef
  typedef std::list<ExpectedGrant>
      ExpectedGrants; //!< the collection of expected grants typedef
  ...
  typedef std::list<ExpectedBackoff>
      ExpectedBackoffs; //!< expected backoffs typedef
...
private:
  ...
  typedef std::vector<Ptr<TxopTest<TxopType>>>
      TxopTests; //!< the TXOP tests typedef

  Ptr<FrameExchangeManagerStub<TxopType>>
      m_feManager; //!< the Frame Exchange Manager stubbed
  Ptr<ChannelAccessManagerStub>
...
void ChannelAccessManagerTest<TxopType>::StartTest(uint64_t slotTime,
  ...
  m_phy->ConfigureStandard(
      WIFI_STANDARD_80211ac); // required to use 160 MHz channels
