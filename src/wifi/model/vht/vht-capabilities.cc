...
VhtCapabilities::VhtCapabilities()
    : m_maxMpduLength(0), m_supportedChannelWidthSet(0), m_rxLdpc(0),
      m_shortGuardIntervalFor80Mhz(0), m_shortGuardIntervalFor160Mhz(0),
      m_txStbc(0), m_rxStbc(0), m_suBeamformerCapable(0),
      m_suBeamformeeCapable(0), m_beamformeeStsCapable(0),
      m_numberOfSoundingDimensions(0), m_muBeamformerCapable(0),
      m_muBeamformeeCapable(0), m_vhtTxopPs(0), m_htcVhtCapable(0),
      m_maxAmpduLengthExponent(0), m_vhtLinkAdaptationCapable(0),
      m_rxAntennaPatternConsistency(0), m_txAntennaPatternConsistency(0),
...
WifiInformationElementId VhtCapabilities::ElementId() const {
  return IE_VHT_CAPABILITIES;
}

void VhtCapabilities::Print(std::ostream &os) const {
  os << "VHT Capabilities=" << GetVhtCapabilitiesInfo() << "|"
...
uint16_t VhtCapabilities::DeserializeInformationField(Buffer::Iterator start,
                                                      uint16_t length) {
...
void VhtCapabilities::SetVhtCapabilitiesInfo(uint32_t ctrl) {
...
uint32_t VhtCapabilities::GetVhtCapabilitiesInfo() const {
  uint32_t val = 0;
...
void VhtCapabilities::SetSupportedMcsAndNssSet(uint64_t ctrl) {
  uint16_t n;
...
uint64_t VhtCapabilities::GetSupportedMcsAndNssSet() const {
  uint64_t val = 0;
  uint16_t n;
  for (uint8_t i = 0; i < 8; i++) {
    n = i * 2;
  ...
  val |= (static_cast<uint64_t>(m_rxHighestSupportedLongGuardIntervalDataRate) &
          0x1fff)
         << 16;
  for (uint8_t i = 0; i < 8; i++) {
    n = (i * 2) + 32;
  ...
  val |= (static_cast<uint64_t>(m_txHighestSupportedLongGuardIntervalDataRate) &
          0x1fff)
         << 48;
  return val;
}

void VhtCapabilities::SetMaxMpduLength(uint16_t length) {
...
void VhtCapabilities::SetSupportedChannelWidthSet(uint8_t channelWidthSet) {
  m_supportedChannelWidthSet = channelWidthSet;
}

void VhtCapabilities::SetRxLdpc(uint8_t rxLdpc) { m_rxLdpc = rxLdpc; }

void VhtCapabilities::SetShortGuardIntervalFor80Mhz(
    uint8_t shortGuardInterval) {
  m_shortGuardIntervalFor80Mhz = shortGuardInterval;
}

void VhtCapabilities::SetShortGuardIntervalFor160Mhz(
    uint8_t shortGuardInterval) {
  m_shortGuardIntervalFor160Mhz = shortGuardInterval;
}

void VhtCapabilities::SetRxStbc(uint8_t rxStbc) { m_rxStbc = rxStbc; }

void VhtCapabilities::SetTxStbc(uint8_t txStbc) { m_txStbc = txStbc; }

void VhtCapabilities::SetMaxAmpduLength(uint32_t maxampdulength) {
...
bool VhtCapabilities::IsSupportedRxMcs(uint8_t mcs) const {
  NS_ASSERT(mcs >= 0 && mcs <= 9);
  if (mcs <= 7) {
    return true;
  }
...
void VhtCapabilities::SetRxHighestSupportedLgiDataRate(
    uint16_t supportedDatarate) {
  m_rxHighestSupportedLongGuardIntervalDataRate = supportedDatarate;
}

void VhtCapabilities::SetTxHighestSupportedLgiDataRate(
    uint16_t supportedDatarate) {
  m_txHighestSupportedLongGuardIntervalDataRate = supportedDatarate;
}

uint16_t VhtCapabilities::GetMaxMpduLength() const {
...
uint8_t VhtCapabilities::GetSupportedChannelWidthSet() const {
  return m_supportedChannelWidthSet;
}

uint8_t VhtCapabilities::GetRxLdpc() const { return m_rxLdpc; }

uint8_t VhtCapabilities::GetRxStbc() const { return m_rxStbc; }

uint8_t VhtCapabilities::GetTxStbc() const { return m_txStbc; }

uint32_t VhtCapabilities::GetMaxAmpduLength() const {
...
uint16_t VhtCapabilities::GetRxHighestSupportedLgiDataRate() const {
