...
VhtPhy::~VhtPhy() { NS_LOG_FUNCTION(this); }

void VhtPhy::BuildModeList() {
...
const PhyEntity::PpduFormats &VhtPhy::GetPpduFormats() const {
...
WifiMode VhtPhy::GetHtSigMode() const {
...
Time VhtPhy::GetDuration(WifiPpduField field,
                         const WifiTxVector &txVector) const {
  switch (field) {
  case WIFI_PPDU_FIELD_SIG_A:
...
Ptr<WifiPpdu> VhtPhy::BuildPpdu(const WifiConstPsduMap &psdus,
...
PhyEntity::PhyFieldRxStatus VhtPhy::DoEndReceiveField(WifiPpduField field,
                                                      Ptr<Event> event) {
...
PhyEntity::PhyFieldRxStatus VhtPhy::EndReceiveSig(Ptr<Event> event,
                                                  WifiPpduField field) {
  NS_LOG_FUNCTION(this << *event << field);
  SnrPer snrPer = GetPhyHeaderSnrPer(field, event);
  NS_LOG_DEBUG(field << ": SNR(dB)=" << RatioToDb(snrPer.snr)
                     << ", PER=" << snrPer.per);
...
WifiPhyRxfailureReason VhtPhy::GetFailureReason(WifiPpduField field) const {
  switch (field) {
  case WIFI_PPDU_FIELD_SIG_A:
...
PhyEntity::PhyFieldRxStatus VhtPhy::ProcessSig(Ptr<Event> event,
...
void VhtPhy::InitializeModes() {
  for (uint8_t i = 0; i < 10; ++i) {
    GetVhtMcs(i);
  }
}

WifiMode VhtPhy::GetVhtMcs(uint8_t index) {
#define CASE(x)                                                                \
  case x:                                                                      \
    return GetVhtMcs##x();
...
WifiMode VhtPhy::CreateVhtMcs(uint8_t index) {
...
WifiCodeRate VhtPhy::GetCodeRate(uint8_t mcsValue) {
  switch (mcsValue) {
  case 8:
...
uint16_t VhtPhy::GetConstellationSize(uint8_t mcsValue) {
  switch (mcsValue) {
  case 8:
  case 9:
    return 256;
...
uint64_t VhtPhy::GetPhyRate(uint8_t mcsValue, uint16_t channelWidth,
...
uint64_t VhtPhy::GetDataRate(uint8_t mcsValue, uint16_t channelWidth,
                             uint16_t guardInterval, uint8_t nss) {
  NS_ASSERT(guardInterval == 800 || guardInterval == 400);
  NS_ASSERT(nss <= 8);
  NS_ASSERT_MSG(IsCombinationAllowed(mcsValue, channelWidth, nss),
                "VHT MCS " << +mcsValue << " forbidden at " << channelWidth
                           << " MHz when NSS is " << +nss);
...
uint16_t VhtPhy::GetUsableSubcarriers(uint16_t channelWidth) {
  switch (channelWidth) {
  case 80:
    return 234;
  case 160:
...
uint64_t VhtPhy::GetNonHtReferenceRate(uint8_t mcsValue) {
  WifiCodeRate codeRate = GetCodeRate(mcsValue);
  uint16_t constellationSize = GetConstellationSize(mcsValue);
  return CalculateNonHtReferenceRate(codeRate, constellationSize);
}

uint64_t VhtPhy::CalculateNonHtReferenceRate(WifiCodeRate codeRate,
...
bool VhtPhy::IsAllowed(const WifiTxVector &txVector) {
  return IsCombinationAllowed(txVector.GetMode().GetMcsValue(),
                              txVector.GetChannelWidth(), txVector.GetNss());
}

bool VhtPhy::IsCombinationAllowed(uint8_t mcsValue, uint16_t channelWidth,
                                  uint8_t nss) {
...
uint32_t VhtPhy::GetMaxPsduSize() const { return 4692480; }

double VhtPhy::GetCcaThreshold(const Ptr<const WifiPpdu> ppdu,
  ...
  if (ppdu) {
    ...
    NS_ASSERT_MSG(it != std::end(thresholds),
                  "Invalid channel width " << ppduBw);
    return it->second;
  } else {
    const auto it = channelTypeToScalingFactorDbm.find(channelType);
    NS_ASSERT_MSG(it != std::end(channelTypeToScalingFactorDbm),
                  "Invalid channel list type");
...
VhtPhy::GetCcaIndication(const Ptr<const WifiPpdu> ppdu) {
  ...
  for (auto secondaryWidth : secondaryWidthsToCheck) {
    ...
    delayUntilCcaEnd =
        GetDelayUntilCcaEnd(ccaThresholdDbm, GetSecondaryBand(secondaryWidth));
    if (delayUntilCcaEnd.IsStrictlyPositive()) {
