...
VhtPpdu::VhtPpdu(Ptr<const WifiPsdu> psdu, const WifiTxVector &txVector,
...
void VhtPpdu::SetPhyHeaders(const WifiTxVector &txVector, Time ppduDuration) {
...
void VhtPpdu::SetLSigHeader(LSigHeader &lSig, Time ppduDuration) const {
  uint16_t length =
...
void VhtPpdu::SetVhtSigHeader(VhtSigHeader &vhtSig,
...
WifiTxVector VhtPpdu::DoGetTxVector() const {
...
void VhtPpdu::SetTxVectorFromPhyHeaders(WifiTxVector &txVector,
...
Time VhtPpdu::GetTxDuration() const {
...
Ptr<WifiPpdu> VhtPpdu::Copy() const {
  return Ptr<WifiPpdu>(new VhtPpdu(*this), false);
}

WifiPpduType VhtPpdu::GetType() const {
  return (m_preamble == WIFI_PREAMBLE_VHT_MU) ? WIFI_PPDU_TYPE_DL_MU
                                              : WIFI_PPDU_TYPE_SU;
}

VhtPpdu::VhtSigHeader::VhtSigHeader()
    : m_bw(0), m_nsts(0), m_sgi(0), m_sgi_disambiguation(0), m_suMcs(0),
      m_mu(false) {}

void VhtPpdu::VhtSigHeader::SetMuFlag(bool mu) { m_mu = mu; }

void VhtPpdu::VhtSigHeader::SetChannelWidth(uint16_t channelWidth) {
  if (channelWidth == 160) {
    m_bw = 3;
...
uint16_t VhtPpdu::VhtSigHeader::GetChannelWidth() const {
  if (m_bw == 3) {
    return 160;
  } else if (m_bw == 2) {
    return 80;
...
void VhtPpdu::VhtSigHeader::SetNStreams(uint8_t nStreams) {
  NS_ASSERT(nStreams <= 8);
  m_nsts = (nStreams - 1);
}

uint8_t VhtPpdu::VhtSigHeader::GetNStreams() const { return (m_nsts + 1); }

void VhtPpdu::VhtSigHeader::SetShortGuardInterval(bool sgi) {
  m_sgi = sgi ? 1 : 0;
}

bool VhtPpdu::VhtSigHeader::GetShortGuardInterval() const { return m_sgi; }

void VhtPpdu::VhtSigHeader::SetShortGuardIntervalDisambiguation(
    bool disambiguation) {
  m_sgi_disambiguation = disambiguation ? 1 : 0;
}

bool VhtPpdu::VhtSigHeader::GetShortGuardIntervalDisambiguation() const {
  return m_sgi_disambiguation;
}

void VhtPpdu::VhtSigHeader::SetSuMcs(uint8_t mcs) {
  NS_ASSERT(mcs <= 9);
  m_suMcs = mcs;
}

uint8_t VhtPpdu::VhtSigHeader::GetSuMcs() const { return m_suMcs; }
