...
public:
  ...
  using ChannelTuple =
      std::tuple<uint8_t /* channel number */, uint16_t /* channel width */,
                 int /* WifiPhyBand */,
...
protected:
  ...
  std::map<std::pair<uint64_t /* UID*/, WifiPreamble>, Ptr<Event>>
      m_currentPreambleEvents; //!< store event associated to a PPDU (that has a
                               //!< unique ID and preamble combination) whose
...
private:
   ...
   * \see class CallBackTraceSource
   */
  TracedCallback<WifiConstPsduMap, WifiTxVector, double /* TX power (W) */>
   ...
   */
  TracedCallback<Ptr<const Packet>, uint16_t /* frequency (MHz) */,
                 WifiTxVector, MpduInfo, SignalNoiseDbm, uint16_t /* STA-ID*/>
   ...
   */
  TracedCallback<Ptr<const Packet>, uint16_t /* frequency (MHz) */,
                 WifiTxVector, MpduInfo, uint16_t /* STA-ID*/>
  ...
  double m_ccaEdThresholdW; //!< Clear channel assessment (CCA) energy detection
                            //!< (ED) threshold in watts
  double m_ccaSensitivityThresholdW; //!< Clear channel assessment (CCA)
                                     //!< modulation and coding rate sensitivity
                                     //!< threshold in watts
  ...
  bool m_powerRestricted; //!< Flag whether transmit power is restricted by OBSS
                          //!< PD SR
  ...
  double m_txPowerMaxMimo; //!< MIMO maximum transmit power due to OBSS PD SR
                           //!< power restriction (dBm)
  bool m_channelAccessRequested; //!< Flag if channels access has been requested
                                 //!< (used for OBSS_PD SR)
  ...
  Callback<void>
      m_capabilitiesChangedCallback; //!< Callback when PHY capabilities changed
};
