...
TypeId ErrorRateModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ErrorRateModel").SetParent<Object>().SetGroupName("Wifi");
  return tid;
}

double ErrorRateModel::CalculateSnr(const WifiTxVector &txVector,
                                    double ber) const {
  ...
  while (high - low > precision) {
    ...
    if ((1 - GetChunkSuccessRate(txVector.GetMode(), txVector, middle, 1)) >
        ber) {
      low = middle;
    } else {
      high = middle;
...
double ErrorRateModel::GetChunkSuccessRate(
  ...
  if (mode.GetModulationClass() == WIFI_MOD_CLASS_DSSS ||
    ...
    return DoGetChunkSuccessRate(mode, txVector, snr, nbits, numRxAntennas,
                                 field, staId);
  }
  return 0;
}

bool ErrorRateModel::IsAwgn() const { return true; }

int64_t ErrorRateModel::AssignStreams(int64_t stream) {
