...
TypeId WifiPhyStateHelper::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WifiPhyStateHelper")
          ...
          .AddTraceSource(
              "RxError", "A packet has been received unsuccessfuly.",
...
WifiPhyStateHelper::WifiPhyStateHelper()
    : m_sleeping(false), m_isOff(false), m_endTx(Seconds(0)),
      m_endRx(Seconds(0)), m_endCcaBusy(Seconds(0)), m_endSwitching(Seconds(0)),
      m_startTx(Seconds(0)), m_startRx(Seconds(0)), m_startCcaBusy(Seconds(0)),
      m_startSwitching(Seconds(0)), m_startSleep(Seconds(0)),
      m_previousStateChangeTime(Seconds(0)) {
  NS_LOG_FUNCTION(this);
}

void WifiPhyStateHelper::SetReceiveOkCallback(RxOkCallback callback) {
  m_rxOkCallback = callback;
}

void WifiPhyStateHelper::SetReceiveErrorCallback(RxErrorCallback callback) {
  m_rxErrorCallback = callback;
}

void WifiPhyStateHelper::RegisterListener(WifiPhyListener *listener) {
  m_listeners.push_back(listener);
}

void WifiPhyStateHelper::UnregisterListener(WifiPhyListener *listener) {
...
bool WifiPhyStateHelper::IsStateIdle() const {
  return (GetState() == WifiPhyState::IDLE);
}

bool WifiPhyStateHelper::IsStateCcaBusy() const {
  return (GetState() == WifiPhyState::CCA_BUSY);
}

bool WifiPhyStateHelper::IsStateRx() const {
  return (GetState() == WifiPhyState::RX);
}

bool WifiPhyStateHelper::IsStateTx() const {
  return (GetState() == WifiPhyState::TX);
}

bool WifiPhyStateHelper::IsStateSwitching() const {
  return (GetState() == WifiPhyState::SWITCHING);
}

bool WifiPhyStateHelper::IsStateSleep() const {
  return (GetState() == WifiPhyState::SLEEP);
}

bool WifiPhyStateHelper::IsStateOff() const {
  return (GetState() == WifiPhyState::OFF);
}

Time WifiPhyStateHelper::GetDelayUntilIdle() const {
  Time retval;
...
Time WifiPhyStateHelper::GetLastRxStartTime() const { return m_startRx; }

Time WifiPhyStateHelper::GetLastRxEndTime() const { return m_endRx; }

WifiPhyState WifiPhyStateHelper::GetState() const {
  if (m_isOff) {
...
void WifiPhyStateHelper::NotifyTxStart(Time duration, double txPowerDbm) {
  NS_LOG_FUNCTION(this);
...
void WifiPhyStateHelper::NotifyRxStart(Time duration) {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
...
void WifiPhyStateHelper::NotifyRxEndOk() {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
...
void WifiPhyStateHelper::NotifyRxEndError() {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
...
void WifiPhyStateHelper::NotifyCcaBusyStart(
...
void WifiPhyStateHelper::NotifySwitchingStart(Time duration) {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
...
void WifiPhyStateHelper::NotifySleep() {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
...
void WifiPhyStateHelper::NotifyOff() {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
    listener->NotifyOff();
  }
...
void WifiPhyStateHelper::NotifyWakeup() {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
...
void WifiPhyStateHelper::NotifyOn() {
  NS_LOG_FUNCTION(this);
  for (const auto &listener : m_listeners) {
    listener->NotifyOn();
  }
...
void WifiPhyStateHelper::LogPreviousIdleAndCcaBusyStates() {
  NS_LOG_FUNCTION(this);
  Time now = Simulator::Now();
  WifiPhyState state = GetState();
  if (state == WifiPhyState::CCA_BUSY) {
    Time ccaStart =
        std::max({m_endRx, m_endTx, m_startCcaBusy, m_endSwitching});
    ...
    if (m_endCcaBusy > m_endRx && m_endCcaBusy > m_endSwitching &&
        m_endCcaBusy > m_endTx) {
      Time ccaBusyStart =
          std::max({m_endTx, m_endRx, m_startCcaBusy, m_endSwitching});
...
void WifiPhyStateHelper::SwitchToTx(Time txDuration, WifiConstPsduMap psdus,
                                    double txPowerDbm,
...
void WifiPhyStateHelper::SwitchToChannelSwitching(Time switchingDuration) {
...
void WifiPhyStateHelper::NotifyRxMpdu(Ptr<const WifiPsdu> psdu,
...
void WifiPhyStateHelper::NotifyRxPsduSucceeded(
...
void WifiPhyStateHelper::NotifyRxPsduFailed(Ptr<const WifiPsdu> psdu,
                                            double snr) {
...
void WifiPhyStateHelper::SwitchFromRxEndOk() {
  NS_LOG_FUNCTION(this);
...
void WifiPhyStateHelper::SwitchFromRxEndError() {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(m_endRx == Simulator::Now());
  NotifyRxEndError();
  DoSwitchFromRx();
...
void WifiPhyStateHelper::DoSwitchFromRx() {
  NS_LOG_FUNCTION(this);
  Time now = Simulator::Now();
  m_stateLogger(m_startRx, now - m_startRx, WifiPhyState::RX);
  m_previousStateChangeTime = now;
...
void WifiPhyStateHelper::SwitchMaybeToCcaBusy(
    Time duration, WifiChannelListType channelType,
    const std::vector<Time> &per20MhzDurations) {
...
void WifiPhyStateHelper::SwitchToSleep() {
  NS_LOG_FUNCTION(this);
  Time now = Simulator::Now();
  switch (GetState()) {
  case WifiPhyState::IDLE:
...
void WifiPhyStateHelper::SwitchFromSleep() {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(IsStateSleep());
  Time now = Simulator::Now();
...
void WifiPhyStateHelper::SwitchFromRxAbort(uint16_t operatingWidth) {
  ...
  if (operatingWidth >= 40) {
    std::fill_n(std::back_inserter(per20MhzDurations), (operatingWidth / 20),
                Seconds(0));
  }
...
void WifiPhyStateHelper::SwitchToOff() {
  NS_LOG_FUNCTION(this);
  Time now = Simulator::Now();
  switch (GetState()) {
  case WifiPhyState::RX:
...
void WifiPhyStateHelper::SwitchFromOff() {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(IsStateOff());
  Time now = Simulator::Now();
  m_previousStateChangeTime = now;
