...
WifiTxTimer::WifiTxTimer()
    : m_timeoutEvent(), m_reason(NOT_RUNNING), m_impl(nullptr),
      m_end(Seconds(0)) {}
...
void WifiTxTimer::Reschedule(const Time &delay) {
...
void WifiTxTimer::Expire() {
  ...
  if (m_end == now) {
    ...
    m_timeoutEvent =
        Simulator::Schedule(m_end - now, &WifiTxTimer::Expire, this);
  }
}

WifiTxTimer::Reason WifiTxTimer::GetReason() const {
  NS_ASSERT(IsRunning());
  return m_reason;
}

std::string WifiTxTimer::GetReasonString(Reason reason) const {
...
bool WifiTxTimer::IsRunning() const { return m_timeoutEvent.IsRunning(); }

void WifiTxTimer::Cancel() {
...
void WifiTxTimer::GotResponseFrom(const Mac48Address &from) {
  m_staExpectResponseFrom.erase(from);
}

const std::set<Mac48Address> &WifiTxTimer::GetStasExpectedToRespond() const {
  return m_staExpectResponseFrom;
}

Time WifiTxTimer::GetDelayLeft() const { return m_end - Simulator::Now(); }

void WifiTxTimer::SetMpduResponseTimeoutCallback(
    MpduResponseTimeout callback) const {
  m_mpduResponseTimeoutCallback = callback;
}

void WifiTxTimer::FeedTraceSource(Ptr<WifiMpdu> item, WifiTxVector txVector) {
  if (!m_mpduResponseTimeoutCallback.IsNull()) {
    m_mpduResponseTimeoutCallback(m_reason, item, txVector);
  }
}

void WifiTxTimer::SetPsduResponseTimeoutCallback(
    PsduResponseTimeout callback) const {
  m_psduResponseTimeoutCallback = callback;
}

void WifiTxTimer::FeedTraceSource(Ptr<WifiPsdu> psdu, WifiTxVector txVector) {
  if (!m_psduResponseTimeoutCallback.IsNull()) {
    m_psduResponseTimeoutCallback(m_reason, psdu, txVector);
  }
}

void WifiTxTimer::SetPsduMapResponseTimeoutCallback(
    PsduMapResponseTimeout callback) const {
  m_psduMapResponseTimeoutCallback = callback;
}

void WifiTxTimer::FeedTraceSource(WifiPsduMap *psduMap,
                                  std::size_t nTotalStations) {
  if (!m_psduMapResponseTimeoutCallback.IsNull()) {
    m_psduMapResponseTimeoutCallback(m_reason, psduMap,
                                     &m_staExpectResponseFrom, nTotalStations);
  }
}
