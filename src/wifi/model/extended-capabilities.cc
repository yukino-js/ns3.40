...
ExtendedCapabilities::ExtendedCapabilities()
    : m_20_40_bssCoexistenceManagementSupport(0), m_extendedChannelSwitching(0),
      m_psmpCapability(0), m_spsmpSupport(0), m_event(0), m_diagnostics(0),
      m_multicastDiagnostics(0), m_locationTracking(0), m_fms(0),
      m_proxyArpService(0), m_collocatedInterferenceReporting(0),
      m_civicLocation(0), m_geospatialLocation(0), m_tfs(0), m_wnmSleepMode(0),
      m_timBroadcast(0), m_bssTransition(0), m_qosTrafficCapability(0),
      m_acStationCount(0), m_multipleBssid(0), m_timingMeasurement(0),
      m_channelUsage(0), m_ssidList(0), m_dms(0), m_utcTsfOffset(0),
      m_tpuBufferStaSupport(0), m_tdlsPeerPsmSupport(0),
      m_tdlsChannelSwitching(0), m_interworking(0), m_qosMap(0), m_ebr(0),
      m_sspnInterface(0), m_msgcfCapability(0), m_tdlsSupport(0),
      m_tdlsProhibited(0), m_tdlsChannelSwitchingProhibited(0),
      m_rejectUnadmittedFrame(0), m_serviceIntervalGranularity(0),
      m_identifierLocation(0), m_uapsdCoexistence(0), m_wnmNotification(0),
      m_qabCapability(0), m_utf8Ssid(0), m_qmfActivated(0),
      m_qmfReconfigurationActivated(0), m_robustAvStreaming(0),
      m_advancedGcr(0), m_meshGcr(0), m_scs(0), m_qloadReport(0),
      m_alternateEdca(0), m_unprotectedTxopNegotiation(0),
      m_protectedTxopNegotiation(0), m_protectedQloadReport(0),
      m_tdlsWiderBandwidth(0), m_operatingModeNotification(0),
      m_maxNumberOfMsdusInAmsdu(0), m_htSupported(0), m_vhtSupported(0) {}

WifiInformationElementId ExtendedCapabilities::ElementId() const {
  return IE_EXTENDED_CAPABILITIES;
}

void ExtendedCapabilities::Print(std::ostream &os) const {
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte1() const {
  uint8_t val = 0;
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte2() const {
  uint8_t val = 0;
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte3() const {
  uint8_t val = 0;
  val |= m_tfs & 0x01;
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte4() const {
  uint8_t val = 0;
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte5() const {
  uint8_t val = 0;
  val |= m_qosMap & 0x01;
  val |= (m_ebr & 0x01) << 1;
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte6() const {
  uint8_t val = 0;
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte7() const {
  uint8_t val = 0;
  val |= m_utf8Ssid & 0x01;
...
uint8_t ExtendedCapabilities::GetExtendedCapabilitiesByte8() const {
  uint8_t val = 0;
...
uint16_t
ExtendedCapabilities::DeserializeInformationField(Buffer::Iterator start,
                                                  uint16_t length) {
