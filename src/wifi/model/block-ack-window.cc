...
BlockAckWindow::BlockAckWindow() : m_winStart(0), m_head(0) {}

void BlockAckWindow::Init(uint16_t winStart, uint16_t winSize) {
...
void BlockAckWindow::Reset(uint16_t winStart) {
  Init(winStart, m_window.size());
}

uint16_t BlockAckWindow::GetWinStart() const { return m_winStart; }

uint16_t BlockAckWindow::GetWinEnd() const {
  return (m_winStart + m_window.size() - 1) % SEQNO_SPACE_SIZE;
}

std::size_t BlockAckWindow::GetWinSize() const { return m_window.size(); }

std::vector<bool>::reference BlockAckWindow::At(std::size_t distance) {
...
void BlockAckWindow::Advance(std::size_t count) {
