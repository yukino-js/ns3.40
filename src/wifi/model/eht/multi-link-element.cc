...
uint8_t CommonInfoBasicMle::Deserialize(Buffer::Iterator start,
                                        uint16_t presence) {
  ...
  NS_ABORT_MSG_IF(count != length, "Common Info Length ("
                                       << +length
                                       << ") differs "
...
uint8_t CommonInfoBasicMle::EncodeEmlsrPaddingDelay(Time delay) {
...
Time CommonInfoBasicMle::DecodeEmlsrPaddingDelay(uint8_t value) {
...
uint8_t CommonInfoBasicMle::EncodeEmlsrTransitionDelay(Time delay) {
  auto delayUs = delay.GetMicroSeconds();
...
Time CommonInfoBasicMle::DecodeEmlsrTransitionDelay(uint8_t value) {
...
WifiInformationElementId MultiLinkElement::ElementId() const {
  return IE_EXTENSION;
}

WifiInformationElementId MultiLinkElement::ElementIdExt() const {
  return IE_EXT_MULTI_LINK_ELEMENT;
}

MultiLinkElement::Variant MultiLinkElement::GetVariant() const {
  return static_cast<Variant>(m_commonInfo.index());
}

void MultiLinkElement::SetVariant(Variant variant) {
  NS_ABORT_MSG_IF(GetVariant() != UNSET,
                  "Multi-Link Element variant already set");
...
CommonInfoBasicMle &MultiLinkElement::GetCommonInfoBasic() {
  return std::get<BASIC_VARIANT>(m_commonInfo);
}

const CommonInfoBasicMle &MultiLinkElement::GetCommonInfoBasic() const {
  return std::get<BASIC_VARIANT>(m_commonInfo);
}

void MultiLinkElement::SetMldMacAddress(Mac48Address address) {
  std::get<BASIC_VARIANT>(m_commonInfo).m_mldMacAddress = address;
}

Mac48Address MultiLinkElement::GetMldMacAddress() const {
  return std::get<BASIC_VARIANT>(m_commonInfo).m_mldMacAddress;
}

void MultiLinkElement::SetLinkIdInfo(uint8_t linkIdInfo) {
  std::get<BASIC_VARIANT>(m_commonInfo).m_linkIdInfo = (linkIdInfo & 0x0f);
}

bool MultiLinkElement::HasLinkIdInfo() const {
  return std::get<BASIC_VARIANT>(m_commonInfo).m_linkIdInfo.has_value();
}

uint8_t MultiLinkElement::GetLinkIdInfo() const {
  return std::get<BASIC_VARIANT>(m_commonInfo).m_linkIdInfo.value();
}

void MultiLinkElement::SetBssParamsChangeCount(uint8_t count) {
  std::get<BASIC_VARIANT>(m_commonInfo).m_bssParamsChangeCount = count;
}

bool MultiLinkElement::HasBssParamsChangeCount() const {
  return std::get<BASIC_VARIANT>(m_commonInfo)
      .m_bssParamsChangeCount.has_value();
}

uint8_t MultiLinkElement::GetBssParamsChangeCount() const {
  return std::get<BASIC_VARIANT>(m_commonInfo).m_bssParamsChangeCount.value();
}

void MultiLinkElement::SetMediumSyncDelayTimer(Time delay) {
  int64_t delayUs = delay.GetMicroSeconds();
  NS_ABORT_MSG_IF(delayUs % 32 != 0,
                  "Delay must be a multiple of 32 microseconds");
  delayUs /= 32;

  auto &mediumSyncDelayInfo =
      std::get<BASIC_VARIANT>(m_commonInfo).m_mediumSyncDelayInfo;
...
Time MultiLinkElement::GetMediumSyncDelayTimer() const {
  return MicroSeconds((std::get<BASIC_VARIANT>(m_commonInfo)
                           .m_mediumSyncDelayInfo.value()
                           .mediumSyncDuration) *
                      32);
}

void MultiLinkElement::SetMediumSyncOfdmEdThreshold(int8_t threshold) {
  NS_ABORT_MSG_IF(threshold < -72 || threshold > -62,
...
int8_t MultiLinkElement::GetMediumSyncOfdmEdThreshold() const {
  return (std::get<BASIC_VARIANT>(m_commonInfo)
              .m_mediumSyncDelayInfo.value()
...
void MultiLinkElement::SetMediumSyncMaxNTxops(uint8_t nTxops) {
  NS_ASSERT(nTxops > 0);
  nTxops--;
...
uint8_t MultiLinkElement::GetMediumSyncMaxNTxops() const {
  return (std::get<BASIC_VARIANT>(m_commonInfo)
              .m_mediumSyncDelayInfo.value()
              .mediumSyncMaxNTxops) +
         1;
...
bool MultiLinkElement::HasMediumSyncDelayInfo() const {
  return std::get<BASIC_VARIANT>(m_commonInfo)
      .m_mediumSyncDelayInfo.has_value();
}

void MultiLinkElement::SetEmlsrSupported(bool supported) {
  auto &emlCapabilities =
      std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities;
...
void MultiLinkElement::SetEmlsrPaddingDelay(Time delay) {
  auto &emlCapabilities =
      std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities;
  if (!emlCapabilities.has_value()) {
    emlCapabilities = CommonInfoBasicMle::EmlCapabilities{};
  ...
  emlCapabilities->emlsrPaddingDelay =
      CommonInfoBasicMle::EncodeEmlsrPaddingDelay(delay);
}

void MultiLinkElement::SetEmlsrTransitionDelay(Time delay) {
  auto &emlCapabilities =
      std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities;
  if (!emlCapabilities.has_value()) {
    emlCapabilities = CommonInfoBasicMle::EmlCapabilities{};
  ...
  emlCapabilities->emlsrTransitionDelay =
      CommonInfoBasicMle::EncodeEmlsrTransitionDelay(delay);
}

void MultiLinkElement::SetTransitionTimeout(Time timeout) {
  auto &emlCapabilities =
      std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities;
  if (!emlCapabilities.has_value()) {
    emlCapabilities = CommonInfoBasicMle::EmlCapabilities{};
  ...
  if (timeoutUs == 0) {
    ...
    NS_ABORT_MSG_IF(i > 10,
                    "Value not allowed (" << timeout.As(Time::US) << ")");
  }
}

bool MultiLinkElement::HasEmlCapabilities() const {
  return std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities.has_value();
}

bool MultiLinkElement::IsEmlsrSupported() const {
  return std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities->emlsrSupport;
}

Time MultiLinkElement::GetEmlsrPaddingDelay() const {
  auto &emlCapabilities =
      std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities;
  NS_ASSERT(emlCapabilities);
  return CommonInfoBasicMle::DecodeEmlsrPaddingDelay(
      emlCapabilities->emlsrPaddingDelay);
}

Time MultiLinkElement::GetEmlsrTransitionDelay() const {
  auto &emlCapabilities =
      std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities;
  NS_ASSERT(emlCapabilities);
  return CommonInfoBasicMle::DecodeEmlsrTransitionDelay(
      emlCapabilities->emlsrTransitionDelay);
}

Time MultiLinkElement::GetTransitionTimeout() const {
  auto &emlCapabilities =
      std::get<BASIC_VARIANT>(m_commonInfo).m_emlCapabilities;
  NS_ASSERT(emlCapabilities);
...
MultiLinkElement::PerStaProfileSubelement::PerStaProfileSubelement(
    Variant variant)
    : m_variant(variant), m_staControl(0) {}
...
uint8_t MultiLinkElement::PerStaProfileSubelement::GetLinkId() const {
  return static_cast<uint8_t>(m_staControl & 0x000f);
}

void MultiLinkElement::PerStaProfileSubelement::SetCompleteProfile() {
  m_staControl |= 0x0010;
}

bool MultiLinkElement::PerStaProfileSubelement::IsCompleteProfileSet() const {
  return (m_staControl & 0x0010) != 0;
}

void MultiLinkElement::PerStaProfileSubelement::SetStaMacAddress(
...
bool MultiLinkElement::PerStaProfileSubelement::HasStaMacAddress() const {
...
void MultiLinkElement::PerStaProfileSubelement::SetAssocRequest(
...
bool MultiLinkElement::PerStaProfileSubelement::HasAssocRequest() const {
  return std::holds_alternative<std::unique_ptr<MgtAssocRequestHeader>>(
      m_staProfile);
}

bool MultiLinkElement::PerStaProfileSubelement::HasReassocRequest() const {
  return std::holds_alternative<std::unique_ptr<MgtReassocRequestHeader>>(
      m_staProfile);
}
...
void MultiLinkElement::PerStaProfileSubelement::SetAssocResponse(
    const MgtAssocResponseHeader &assoc) {
...
bool MultiLinkElement::PerStaProfileSubelement::HasAssocResponse() const {
  return std::holds_alternative<std::unique_ptr<MgtAssocResponseHeader>>(
      m_staProfile);
}
...
MultiLinkElement::PerStaProfileSubelement::GetInformationFieldSize() const {
  ...
  auto staProfileSize = [&](auto &&frame) {
    ...
    if constexpr (std::is_same_v<T, std::monostate>) {
      ...
      NS_ASSERT_MSG(std::holds_alternative<std::reference_wrapper<const U>>(
                        m_containingFrame),
                    "Containing frame type and frame type in Per-STA Profile "
                    "do not match");
      const auto &containing =
          std::get<std::reference_wrapper<const U>>(m_containingFrame);
...
void MultiLinkElement::PerStaProfileSubelement::SerializeInformationField(
...
uint16_t MultiLinkElement::PerStaProfileSubelement::DeserializeInformationField(
  ...
  auto staProfileDeserialize = [&](auto &&frame) {
    ...
    if constexpr (!std::is_same_v<T, std::monostate>) {
      ...
      U assoc;
      count +=
          assoc.DeserializeFromPerStaProfile(i, length - count, frame.get());
...
void MultiLinkElement::AddPerStaProfileSubelement() {
  auto variant = GetVariant();
  NS_ABORT_IF(variant == UNSET);
  m_perStaProfileSubelements.emplace_back(variant);
}

std::size_t MultiLinkElement::GetNPerStaProfileSubelements() const {
...
uint16_t MultiLinkElement::DeserializeInformationField(Buffer::Iterator start,
                                                       uint16_t length) {
  Buffer::Iterator i = start;
  uint16_t count = 0;
  ...
  while (count < length) {
    ...
    case PER_STA_PROFILE_SUBELEMENT_ID: {
      ...
      auto &perStaProfile =
          GetPerStaProfile(GetNPerStaProfileSubelements() - 1);
