...
EhtPhy::~EhtPhy() { NS_LOG_FUNCTION(this); }

void EhtPhy::BuildModeList() {
...
WifiMode EhtPhy::GetSigMode(WifiPpduField field,
...
WifiMode EhtPhy::GetSigBMode(const WifiTxVector &txVector) const {
  if (txVector.IsDlMu()) {
...
Time EhtPhy::GetDuration(WifiPpduField field,
                         const WifiTxVector &txVector) const {
  switch (field) {
  case WIFI_PPDU_FIELD_U_SIG:
...
uint32_t EhtPhy::GetSigBSize(const WifiTxVector &txVector) const {
  if (ns3::IsDlMu(txVector.GetPreambleType()) &&
      ns3::IsEht(txVector.GetPreambleType())) {
    return EhtPpdu::GetEhtSigFieldSize(
        ...
        txVector.GetRuAllocation(
            m_wifiPhy
                ...
                : 0),
        txVector.GetEhtPpduType(), txVector.IsSigBCompression(),
...
Time EhtPhy::CalculateNonHeDurationForHeTb(const WifiTxVector &txVector) const {
...
Time EhtPhy::CalculateNonHeDurationForHeMu(const WifiTxVector &txVector) const {
  Time duration = GetDuration(WIFI_PPDU_FIELD_PREAMBLE, txVector) +
                  GetDuration(WIFI_PPDU_FIELD_NON_HT_HEADER, txVector) +
                  GetDuration(WIFI_PPDU_FIELD_U_SIG, txVector) +
...
const PhyEntity::PpduFormats &EhtPhy::GetPpduFormats() const {
  return m_ehtPpduFormats;
}

Ptr<WifiPpdu> EhtPhy::BuildPpdu(const WifiConstPsduMap &psdus,
                                const WifiTxVector &txVector,
                                Time ppduDuration) {
  NS_LOG_FUNCTION(this << psdus << txVector << ppduDuration);
  return Create<EhtPpdu>(psdus, txVector, m_wifiPhy->GetOperatingChannel(),
                         ppduDuration, ObtainNextUid(txVector),
                         HePpdu::PSD_NON_HE_PORTION);
}

PhyEntity::PhyFieldRxStatus EhtPhy::DoEndReceiveField(WifiPpduField field,
                                                      Ptr<Event> event) {
...
PhyEntity::PhyFieldRxStatus EhtPhy::ProcessSig(Ptr<Event> event,
...
WifiPhyRxfailureReason EhtPhy::GetFailureReason(WifiPpduField field) const {
  switch (field) {
  case WIFI_PPDU_FIELD_U_SIG:
...
void EhtPhy::InitializeModes() {
  for (uint8_t i = 0; i <= 13; ++i) {
    GetEhtMcs(i);
  }
}

WifiMode EhtPhy::GetEhtMcs(uint8_t index) {
#define CASE(x)                                                                \
  case x:                                                                      \
    return GetEhtMcs##x();
...
WifiMode EhtPhy::CreateEhtMcs(uint8_t index) {
...
WifiCodeRate EhtPhy::GetCodeRate(uint8_t mcsValue) {
  switch (mcsValue) {
  case 12:
...
uint16_t EhtPhy::GetConstellationSize(uint8_t mcsValue) {
  switch (mcsValue) {
  case 12:
    [[fallthrough]];
  case 13:
...
uint64_t EhtPhy::GetPhyRate(uint8_t mcsValue, uint16_t channelWidth,
...
uint64_t EhtPhy::GetDataRate(uint8_t mcsValue, uint16_t channelWidth,
                             uint16_t guardInterval, uint8_t nss) {
...
uint64_t EhtPhy::GetNonHtReferenceRate(uint8_t mcsValue) {
  WifiCodeRate codeRate = GetCodeRate(mcsValue);
  uint16_t constellationSize = GetConstellationSize(mcsValue);
  return CalculateNonHtReferenceRate(codeRate, constellationSize);
}

uint64_t EhtPhy::CalculateNonHtReferenceRate(WifiCodeRate codeRate,
