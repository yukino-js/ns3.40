...
void EhtOperation::Print(std::ostream &os) const {
  os << "EHT Operation=" << +m_params.opInfoPresent << "|"
     << +m_params.disabledSubchBmPresent << "|" << +m_params.defaultPeDur << "|"
     << +m_params.grpBuIndLimit << "|" << +m_params.grpBuExp << "|[";
...
void EhtOperation::EhtOpParams::Serialize(Buffer::Iterator &start) const {
  uint8_t val = opInfoPresent | (disabledSubchBmPresent << 1) |
                (defaultPeDur << 2) | (grpBuIndLimit << 3) | (grpBuExp << 4);
  start.WriteU8(val);
}

uint16_t EhtOperation::EhtOpParams::Deserialize(Buffer::Iterator start) {
...
void EhtOperation::EhtBasicMcsNssSet::Serialize(Buffer::Iterator &start) const {
  uint32_t val =
      GetMaxNss(maxRxNss, 0, 7) | (GetMaxNss(maxTxNss, 0, 7) << 4) |
      (GetMaxNss(maxRxNss, 8, 9) << 8) | (GetMaxNss(maxTxNss, 8, 9) << 12) |
      (GetMaxNss(maxRxNss, 10, 11) << 16) |
      (GetMaxNss(maxTxNss, 10, 11) << 20) |
...
uint16_t EhtOperation::EhtOpInfo::Deserialize(Buffer::Iterator start,
...
EhtOperation::EhtOperation() {
  m_mcsNssSet.maxRxNss.assign(WIFI_EHT_MAX_MCS_INDEX + 1,
                              WIFI_DEFAULT_EHT_MAX_NSS);
  m_mcsNssSet.maxTxNss.assign(WIFI_EHT_MAX_MCS_INDEX + 1,
                              WIFI_DEFAULT_EHT_MAX_NSS);
}

WifiInformationElementId EhtOperation::ElementId() const {
  return IE_EXTENSION;
}

WifiInformationElementId EhtOperation::ElementIdExt() const {
...
void EhtOperation::SetMaxRxNss(uint8_t maxNss, uint8_t mcsStart,
                               uint8_t mcsEnd) {
  NS_ASSERT(mcsStart <= mcsEnd);
  NS_ASSERT((mcsStart >= 0) && (mcsEnd <= WIFI_EHT_MAX_MCS_INDEX));
  NS_ASSERT((maxNss >= 1) && (maxNss <= WIFI_EHT_MAX_NSS_CONFIGURABLE));
...
void EhtOperation::SetMaxTxNss(uint8_t maxNss, uint8_t mcsStart,
                               uint8_t mcsEnd) {
  NS_ASSERT(mcsStart <= mcsEnd);
  NS_ASSERT((mcsStart >= 0) && (mcsEnd <= WIFI_EHT_MAX_MCS_INDEX));
  NS_ASSERT((maxNss >= 1) && (maxNss <= WIFI_EHT_MAX_NSS_CONFIGURABLE));
...
void EhtOperation::SerializeInformationField(Buffer::Iterator start) const {
...
uint16_t EhtOperation::DeserializeInformationField(Buffer::Iterator start,
                                                   uint16_t length) {
  auto i = start;
  ...
  NS_ABORT_MSG_IF(count != length, "EHT Operation Length ("
                                       << +length
                                       << ") differs "
