...
std::size_t WifiAddressHash::operator()(const Mac48Address &address) const {
  uint8_t buffer[6];
...
WifiAc::WifiAc(uint8_t lowTid, uint8_t highTid)
    : m_lowTid(lowTid), m_highTid(highTid) {}

uint8_t WifiAc::GetLowTid() const { return m_lowTid; }

uint8_t WifiAc::GetHighTid() const { return m_highTid; }

uint8_t WifiAc::GetOtherTid(uint8_t tid) const {
  if (tid == m_lowTid) {
    return m_highTid;
  }
  if (tid == m_highTid) {
...
bool operator<(AcIndex left, AcIndex right) { return !(left >= right); }
