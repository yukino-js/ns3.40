...
TypeId WifiRemoteStationManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WifiRemoteStationManager")
          ...
          .AddAttribute("RtsCtsThreshold",
                        ...
                        MakeUintegerAccessor(
                            &WifiRemoteStationManager::SetRtsCtsThreshold),
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute(
...
WifiRemoteStationManager::WifiRemoteStationManager()
    : m_useNonErpProtection(false), m_useNonHtProtection(false),
      m_shortPreambleEnabled(false), m_shortSlotTimeEnabled(false) {
  NS_LOG_FUNCTION(this);
}

WifiRemoteStationManager::~WifiRemoteStationManager() { NS_LOG_FUNCTION(this); }

void WifiRemoteStationManager::DoDispose() {
  NS_LOG_FUNCTION(this);
  Reset();
}
...
int64_t WifiRemoteStationManager::AssignStreams(int64_t stream) {
...
void WifiRemoteStationManager::SetShortPreambleEnabled(bool enable) {
  NS_LOG_FUNCTION(this << enable);
  m_shortPreambleEnabled = enable;
}

void WifiRemoteStationManager::SetShortSlotTimeEnabled(bool enable) {
  NS_LOG_FUNCTION(this << enable);
  m_shortSlotTimeEnabled = enable;
}

bool WifiRemoteStationManager::GetShortSlotTimeEnabled() const {
  return m_shortSlotTimeEnabled;
}

bool WifiRemoteStationManager::GetShortPreambleEnabled() const {
  return m_shortPreambleEnabled;
}

bool WifiRemoteStationManager::GetHtSupported() const {
  return bool(m_wifiPhy->GetDevice()->GetHtConfiguration());
}

bool WifiRemoteStationManager::GetVhtSupported() const {
  return (m_wifiPhy->GetDevice()->GetVhtConfiguration() &&
          m_wifiPhy->GetPhyBand() != WIFI_PHY_BAND_2_4GHZ);
}

bool WifiRemoteStationManager::GetHeSupported() const {
  return bool(m_wifiPhy->GetDevice()->GetHeConfiguration());
}

bool WifiRemoteStationManager::GetEhtSupported() const {
  return bool(m_wifiPhy->GetDevice()->GetEhtConfiguration());
}

bool WifiRemoteStationManager::GetLdpcSupported() const {
  if (GetHtSupported()) {
...
bool WifiRemoteStationManager::GetShortGuardIntervalSupported() const {
  if (GetHtSupported()) {
    Ptr<HtConfiguration> htConfiguration =
        m_wifiPhy->GetDevice()->GetHtConfiguration();
    NS_ASSERT(htConfiguration); // If HT is supported, we should have a HT
...
uint16_t WifiRemoteStationManager::GetGuardInterval() const {
  uint16_t gi = 0;
  if (GetHeSupported()) {
...
uint32_t WifiRemoteStationManager::GetFragmentationThreshold() const {
  return DoGetFragmentationThreshold();
}

void WifiRemoteStationManager::AddSupportedPhyPreamble(
    Mac48Address address, bool isShortPreambleSupported) {
  NS_LOG_FUNCTION(this << address << isShortPreambleSupported);
  NS_ASSERT(!address.IsGroup());
  LookupState(address)->m_shortPreamble = isShortPreambleSupported;
}

void WifiRemoteStationManager::AddSupportedErpSlotTime(
    Mac48Address address, bool isShortSlotTimeSupported) {
...
void WifiRemoteStationManager::AddAllSupportedMcs(Mac48Address address) {
  NS_LOG_FUNCTION(this << address);
  NS_ASSERT(!address.IsGroup());
  auto state = LookupState(address);
...
void WifiRemoteStationManager::RemoveAllSupportedMcs(Mac48Address address) {
  NS_LOG_FUNCTION(this << address);
  NS_ASSERT(!address.IsGroup());
...
bool WifiRemoteStationManager::GetShortPreambleSupported(
    Mac48Address address) const {
  return LookupState(address)->m_shortPreamble;
}

bool WifiRemoteStationManager::GetShortSlotTimeSupported(
    Mac48Address address) const {
  return LookupState(address)->m_shortSlotTime;
}

bool WifiRemoteStationManager::GetQosSupported(Mac48Address address) const {
...
bool WifiRemoteStationManager::IsAssocRefused(Mac48Address address) const {
  if (address.IsGroup()) {
    return false;
  }
...
void WifiRemoteStationManager::RecordAssocRefused(Mac48Address address) {
  NS_ASSERT(!address.IsGroup());
...
WifiRemoteStationManager::GetAssociationId(Mac48Address remoteAddress) const {
  ...
  if (!remoteAddress.IsGroup() &&
      (state = LookupState(remoteAddress))->m_state ==
          WifiRemoteStationState::GOT_ASSOC_TX_OK) {
    return state->m_aid;
  }
  return SU_STA_ID;
...
uint16_t
WifiRemoteStationManager::GetStaId(Mac48Address address,
...
bool WifiRemoteStationManager::IsInPsMode(const Mac48Address &address) const {
  return LookupState(address)->m_isInPsMode;
}

void WifiRemoteStationManager::SetPsMode(const Mac48Address &address,
                                         bool isInPsMode) {
...
std::optional<Mac48Address> WifiRemoteStationManager::GetAffiliatedStaAddress(
    const Mac48Address &mldAddress) const {
...
WifiRemoteStationManager::GetDataTxVector(const WifiMacHeader &header,
  ...
  if (!header.IsMgt() && address.IsGroup()) {
    ...
    v.SetGuardInterval(
        ConvertGuardIntervalToNanoSeconds(mode, m_wifiPhy->GetDevice()));
  ...
  if (header.IsMgt()) {
    ...
    txVector.SetPreambleType(GetPreambleForTransmission(
        mgtMode.GetModulationClass(), GetShortPreambleEnabled()));
...
WifiTxVector WifiRemoteStationManager::GetCtsToSelfTxVector() {
  ...
  return WifiTxVector(
      defaultMode, GetDefaultTxPowerLevel(), defaultPreamble,
      ConvertGuardIntervalToNanoSeconds(defaultMode, m_wifiPhy->GetDevice()),
      GetNumberOfAntennas(), 1, 0, m_wifiPhy->GetTxBandwidth(defaultMode),
      false);
}
...
WifiTxVector
WifiRemoteStationManager::GetCtsTxVector(Mac48Address to,
                                         WifiMode rtsTxMode) const {
  NS_ASSERT(!to.IsGroup());
  WifiMode ctsMode = GetControlAnswerMode(rtsTxMode);
  WifiTxVector v;
  v.SetMode(ctsMode);
  v.SetPreambleType(GetPreambleForTransmission(ctsMode.GetModulationClass(),
                                               GetShortPreambleEnabled()));
...
void WifiRemoteStationManager::AdjustTxVectorForIcf(
...
WifiTxVector WifiRemoteStationManager::GetAckTxVector(
    Mac48Address to, const WifiTxVector &dataTxVector) const {
  NS_ASSERT(!to.IsGroup());
  WifiMode ackMode =
      GetControlAnswerMode(dataTxVector.GetMode(GetStaId(to, dataTxVector)));
  WifiTxVector v;
  v.SetMode(ackMode);
  v.SetPreambleType(GetPreambleForTransmission(ackMode.GetModulationClass(),
                                               GetShortPreambleEnabled()));
  v.SetTxPowerLevel(GetDefaultTxPowerLevel());
...
WifiTxVector WifiRemoteStationManager::GetBlockAckTxVector(
    Mac48Address to, const WifiTxVector &dataTxVector) const {
  NS_ASSERT(!to.IsGroup());
  ...
  v.SetPreambleType(GetPreambleForTransmission(
      blockAckMode.GetModulationClass(), GetShortPreambleEnabled()));
  v.SetTxPowerLevel(GetDefaultTxPowerLevel());
...
void WifiRemoteStationManager::ReportRtsFailed(const WifiMacHeader &header) {
...
void WifiRemoteStationManager::ReportDataFailed(Ptr<const WifiMpdu> mpdu) {
  NS_LOG_FUNCTION(this << *mpdu);
  NS_ASSERT(!mpdu->GetHeader().GetAddr1().IsGroup());
  AcIndex ac = QosUtilsMapTidToAc(
      (mpdu->GetHeader().IsQosData()) ? mpdu->GetHeader().GetQosTid() : 0);
...
void WifiRemoteStationManager::ReportRtsOk(const WifiMacHeader &header,
...
void WifiRemoteStationManager::ReportDataOk(Ptr<const WifiMpdu> mpdu,
                                            double ackSnr, WifiMode ackMode,
                                            double dataSnr,
                                            WifiTxVector dataTxVector) {
  NS_LOG_FUNCTION(this << *mpdu << ackSnr << ackMode << dataSnr
                       << dataTxVector);
...
void WifiRemoteStationManager::ReportFinalRtsFailed(
...
void WifiRemoteStationManager::ReportFinalDataFailed(Ptr<const WifiMpdu> mpdu) {
  NS_LOG_FUNCTION(this << *mpdu);
  NS_ASSERT(!mpdu->GetHeader().GetAddr1().IsGroup());
...
void WifiRemoteStationManager::ReportRxOk(Mac48Address address,
  ...
  DoReportRxOk(station, rxSignalInfo.snr,
               txVector.GetMode(GetStaId(address, txVector)));
  station->m_rssiAndUpdateTimePair =
      std::make_pair(rxSignalInfo.rssi, Simulator::Now());
}

void WifiRemoteStationManager::ReportAmpduTxStatus(Mac48Address address,
...
bool WifiRemoteStationManager::NeedRts(const WifiMacHeader &header,
                                       uint32_t size) {
...
void WifiRemoteStationManager::SetUseNonErpProtection(bool enable) {
  NS_LOG_FUNCTION(this << enable);
  m_useNonErpProtection = enable;
}

bool WifiRemoteStationManager::GetUseNonErpProtection() const {
  return m_useNonErpProtection;
}

void WifiRemoteStationManager::SetUseNonHtProtection(bool enable) {
  NS_LOG_FUNCTION(this << enable);
  m_useNonHtProtection = enable;
}

bool WifiRemoteStationManager::GetUseNonHtProtection() const {
  return m_useNonHtProtection;
}

bool WifiRemoteStationManager::NeedRetransmission(Ptr<const WifiMpdu> mpdu) {
  NS_LOG_FUNCTION(this << *mpdu);
  NS_ASSERT(!mpdu->GetHeader().GetAddr1().IsGroup());
  AcIndex ac = QosUtilsMapTidToAc(
      (mpdu->GetHeader().IsQosData()) ? mpdu->GetHeader().GetQosTid() : 0);
  ...
  return DoNeedRetransmission(Lookup(mpdu->GetHeader().GetAddr1()),
                              mpdu->GetPacket(), normally);
}

bool WifiRemoteStationManager::NeedFragmentation(Ptr<const WifiMpdu> mpdu) {
  NS_LOG_FUNCTION(this << *mpdu);
...
uint32_t WifiRemoteStationManager::DoGetFragmentationThreshold() const {
...
uint32_t WifiRemoteStationManager::GetNFragments(Ptr<const WifiMpdu> mpdu) {
  ...
  NS_LOG_DEBUG("WifiRemoteStationManager::GetNFragments returning "
               << nFragments);
  return nFragments;
}

uint32_t WifiRemoteStationManager::GetFragmentSize(Ptr<const WifiMpdu> mpdu,
  ...
  if (fragmentNumber == nFragment - 1) {
    uint32_t lastFragmentSize =
        ...
        (fragmentNumber * (GetFragmentationThreshold() -
                           mpdu->GetHeader().GetSize() - WIFI_MAC_FCS_LENGTH));
...
uint32_t WifiRemoteStationManager::GetFragmentOffset(Ptr<const WifiMpdu> mpdu,
                                                     uint32_t fragmentNumber) {
  NS_LOG_FUNCTION(this << *mpdu << fragmentNumber);
  NS_ASSERT(!mpdu->GetHeader().GetAddr1().IsGroup());
  ...
  uint32_t fragmentOffset =
      fragmentNumber * (GetFragmentationThreshold() -
                        mpdu->GetHeader().GetSize() - WIFI_MAC_FCS_LENGTH);
  NS_LOG_DEBUG("WifiRemoteStationManager::GetFragmentOffset returning "
               << fragmentOffset);
  return fragmentOffset;
}

bool WifiRemoteStationManager::IsLastFragment(Ptr<const WifiMpdu> mpdu,
                                              uint32_t fragmentNumber) {
  NS_LOG_FUNCTION(this << *mpdu << fragmentNumber);
  NS_ASSERT(!mpdu->GetHeader().GetAddr1().IsGroup());
  bool isLast = fragmentNumber == (GetNFragments(mpdu) - 1);
  NS_LOG_DEBUG("WifiRemoteStationManager::IsLastFragment returning "
               << std::boolalpha << isLast);
  return isLast;
}

uint8_t WifiRemoteStationManager::GetDefaultTxPowerLevel() const {
...
WifiRemoteStationManager::Lookup(Mac48Address address) const {
  ...
  const_cast<WifiRemoteStationManager *>(this)->m_stations.insert(
      {address, station});
  return station;
}

void WifiRemoteStationManager::SetAssociationId(Mac48Address remoteAddress,
                                                uint16_t aid) {
  NS_LOG_FUNCTION(this << remoteAddress << aid);
  LookupState(remoteAddress)->m_aid = aid;
}

void WifiRemoteStationManager::SetQosSupport(Mac48Address from,
                                             bool qosSupported) {
  NS_LOG_FUNCTION(this << from << qosSupported);
  LookupState(from)->m_qosSupported = qosSupported;
}

void WifiRemoteStationManager::SetEmlsrEnabled(const Mac48Address &from,
                                               bool emlsrEnabled) {
...
void WifiRemoteStationManager::AddStationEhtCapabilities(
  ...
  for (const auto &mcs : m_wifiPhy->GetMcsList(WIFI_MOD_CLASS_EHT)) {
    for (uint8_t mapType = 0; mapType < EhtMcsAndNssSet::EHT_MCS_MAP_TYPE_MAX;
         ++mapType) {
      if (ehtCapabilities.GetHighestSupportedRxMcs(
              static_cast<EhtMcsAndNssSet::EhtMcsMapType>(mapType)) >=
          mcs.GetMcsValue()) {
        AddSupportedMcs(from, mcs);
      }
    }
...
void WifiRemoteStationManager::AddStationMleCommonInfo(
    Mac48Address from,
...
bool WifiRemoteStationManager::GetLdpcSupported(Mac48Address address) const {
  Ptr<const HtCapabilities> htCapabilities =
      LookupState(address)->m_htCapabilities;
  Ptr<const VhtCapabilities> vhtCapabilities =
      LookupState(address)->m_vhtCapabilities;
  Ptr<const HeCapabilities> heCapabilities =
      LookupState(address)->m_heCapabilities;
...
WifiMode WifiRemoteStationManager::GetDefaultMode() const {
  return m_defaultTxMode;
}

WifiMode WifiRemoteStationManager::GetDefaultMcs() const {
  return m_defaultTxMcs;
}

WifiMode WifiRemoteStationManager::GetDefaultModeForSta(
    const WifiRemoteStation *st) const {
...
void WifiRemoteStationManager::Reset() {
  NS_LOG_FUNCTION(this);
  m_states.clear();
...
uint8_t WifiRemoteStationManager::GetNBasicModes() const {
  return static_cast<uint8_t>(m_bssBasicRateSet.size());
}

WifiMode WifiRemoteStationManager::GetBasicMode(uint8_t i) const {
  NS_ASSERT(i < GetNBasicModes());
  return m_bssBasicRateSet[i];
}

uint32_t WifiRemoteStationManager::GetNNonErpBasicModes() const {
  uint32_t size = 0;
...
WifiMode WifiRemoteStationManager::GetNonErpBasicMode(uint8_t i) const {
...
void WifiRemoteStationManager::AddBasicMcs(WifiMode mcs) {
...
uint8_t WifiRemoteStationManager::GetNBasicMcs() const {
  return static_cast<uint8_t>(m_bssBasicMcsSet.size());
}

WifiMode WifiRemoteStationManager::GetBasicMcs(uint8_t i) const {
  NS_ASSERT(i < GetNBasicMcs());
  return m_bssBasicMcsSet[i];
}

WifiMode WifiRemoteStationManager::GetNonUnicastMode() const {
...
bool WifiRemoteStationManager::DoNeedRetransmission(WifiRemoteStation *station,
...
void WifiRemoteStationManager::DoReportAmpduTxStatus(
    WifiRemoteStation *station, uint16_t nSuccessfulMpdus,
...
WifiMode
WifiRemoteStationManager::GetSupported(const WifiRemoteStation *station,
                                       uint8_t i) const {
...
WifiMode
WifiRemoteStationManager::GetMcsSupported(const WifiRemoteStation *station,
                                          uint8_t i) const {
...
uint16_t WifiRemoteStationManager::GetChannelWidth(
    const WifiRemoteStation *station) const {
  return station->m_state->m_channelWidth;
}

bool WifiRemoteStationManager::GetShortGuardIntervalSupported(
    const WifiRemoteStation *station) const {
...
uint16_t WifiRemoteStationManager::GetGuardInterval(
    const WifiRemoteStation *station) const {
  return station->m_state->m_guardInterval;
}

bool WifiRemoteStationManager::GetAggregation(
    const WifiRemoteStation *station) const {
  return station->m_state->m_aggregation;
}

uint8_t WifiRemoteStationManager::GetNumberOfSupportedStreams(
    const WifiRemoteStation *station) const {
  Ptr<const HtCapabilities> htCapabilities = station->m_state->m_htCapabilities;
...
Ptr<WifiPhy> WifiRemoteStationManager::GetPhy() const { return m_wifiPhy; }

Ptr<WifiMac> WifiRemoteStationManager::GetMac() const { return m_wifiMac; }

uint8_t WifiRemoteStationManager::GetNSupported(
    const WifiRemoteStation *station) const {
  return static_cast<uint8_t>(station->m_state->m_operationalRateSet.size());
}

bool WifiRemoteStationManager::GetQosSupported(
    const WifiRemoteStation *station) const {
  return station->m_state->m_qosSupported;
}

bool WifiRemoteStationManager::GetHtSupported(
    const WifiRemoteStation *station) const {
  return bool(station->m_state->m_htCapabilities);
}

bool WifiRemoteStationManager::GetVhtSupported(
    const WifiRemoteStation *station) const {
  return bool(station->m_state->m_vhtCapabilities);
}

bool WifiRemoteStationManager::GetHeSupported(
    const WifiRemoteStation *station) const {
  return bool(station->m_state->m_heCapabilities);
}

bool WifiRemoteStationManager::GetEhtSupported(
    const WifiRemoteStation *station) const {
  return (bool)(station->m_state->m_ehtCapabilities);
}

bool WifiRemoteStationManager::GetEmlsrSupported(
    const WifiRemoteStation *station) const {
...
bool WifiRemoteStationManager::GetEmlsrEnabled(
    const WifiRemoteStation *station) const {
  return station->m_state->m_emlsrEnabled;
}

uint8_t WifiRemoteStationManager::GetNMcsSupported(
    const WifiRemoteStation *station) const {
  return static_cast<uint8_t>(station->m_state->m_operationalMcsSet.size());
}

uint32_t WifiRemoteStationManager::GetNNonErpSupported(
    const WifiRemoteStation *station) const {
  uint32_t size = 0;
...
uint8_t WifiRemoteStationManager::GetNMcsSupported(Mac48Address address) const {
  return static_cast<uint8_t>(LookupState(address)->m_operationalMcsSet.size());
}

bool WifiRemoteStationManager::GetDsssSupported(
    const Mac48Address &address) const {
  return (LookupState(address)->m_dsssSupported);
}

bool WifiRemoteStationManager::GetErpOfdmSupported(
    const Mac48Address &address) const {
  return (LookupState(address)->m_erpOfdmSupported);
}

bool WifiRemoteStationManager::GetOfdmSupported(
    const Mac48Address &address) const {
  return (LookupState(address)->m_ofdmSupported);
}

bool WifiRemoteStationManager::GetHtSupported(Mac48Address address) const {
  return bool(LookupState(address)->m_htCapabilities);
}

bool WifiRemoteStationManager::GetVhtSupported(Mac48Address address) const {
  return bool(LookupState(address)->m_vhtCapabilities);
}

bool WifiRemoteStationManager::GetHeSupported(Mac48Address address) const {
  return bool(LookupState(address)->m_heCapabilities);
}

bool WifiRemoteStationManager::GetEhtSupported(Mac48Address address) const {
...
uint8_t WifiRemoteStationManager::GetNumberOfAntennas() const {
  return m_wifiPhy->GetNumberOfAntennas();
}

uint8_t WifiRemoteStationManager::GetMaxNumberOfTransmitStreams() const {
  return m_wifiPhy->GetMaxSupportedTxSpatialStreams();
}

bool WifiRemoteStationManager::UseLdpcForDestination(Mac48Address dest) const {
