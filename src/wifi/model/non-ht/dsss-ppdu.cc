...
void DsssPpdu::SetPhyHeaders(const WifiTxVector &txVector, Time ppduDuration) {
  NS_LOG_FUNCTION(this << txVector);
  SetDsssHeader(m_dsssSig, txVector, ppduDuration);
}

void DsssPpdu::SetDsssHeader(DsssSigHeader &dsssSig,
...
WifiTxVector DsssPpdu::DoGetTxVector() const {
...
void DsssPpdu::SetTxVectorFromDsssHeader(WifiTxVector &txVector,
                                         const DsssSigHeader &dsssSig) const {
  txVector.SetMode(DsssPhy::GetDsssRate(dsssSig.GetRate()));
}

Time DsssPpdu::GetTxDuration() const {
...
Ptr<WifiPpdu> DsssPpdu::Copy() const {
...
uint64_t DsssPpdu::DsssSigHeader::GetRate() const {
  uint64_t rate = 0;
  switch (m_rate) {
  case 0b00001010:
    rate = 1000000;
...
void DsssPpdu::DsssSigHeader::SetLength(uint16_t length) { m_length = length; }

uint16_t DsssPpdu::DsssSigHeader::GetLength() const { return m_length; }
