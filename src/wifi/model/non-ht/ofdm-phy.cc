...
OfdmPhy::~OfdmPhy() { NS_LOG_FUNCTION(this); }
...
WifiMode OfdmPhy::GetHeaderMode(const WifiTxVector &txVector) const {
...
const PhyEntity::PpduFormats &OfdmPhy::GetPpduFormats() const {
  return m_ofdmPpduFormats;
}

Time OfdmPhy::GetDuration(WifiPpduField field,
                          const WifiTxVector &txVector) const {
  switch (field) {
...
Time OfdmPhy::GetPreambleDuration(const WifiTxVector &txVector) const {
  switch (txVector.GetChannelWidth()) {
  case 20:
  default:
...
Time OfdmPhy::GetHeaderDuration(const WifiTxVector &txVector) const {
  switch (txVector.GetChannelWidth()) {
  case 20:
  default:
...
uint8_t OfdmPhy::GetNumberServiceBits() const { return 16; }

Time OfdmPhy::GetSignalExtension(WifiPhyBand band) const {
...
PhyEntity::PhyFieldRxStatus OfdmPhy::EndReceiveHeader(Ptr<Event> event) {
  NS_LOG_FUNCTION(this << *event);
  SnrPer snrPer = GetPhyHeaderSnrPer(WIFI_PPDU_FIELD_NON_HT_HEADER, event);
  NS_LOG_DEBUG("L-SIG: SNR(dB)=" << RatioToDb(snrPer.snr)
                                 << ", PER=" << snrPer.per);
  PhyFieldRxStatus status(GetRandomValue() > snrPer.per);
  if (status.isSuccess) {
    NS_LOG_DEBUG("Received non-HT PHY header");
    if (!IsAllConfigSupported(WIFI_PPDU_FIELD_NON_HT_HEADER,
                              event->GetPpdu())) {
...
bool OfdmPhy::IsChannelWidthSupported(Ptr<const WifiPpdu> ppdu) const {
...
WifiMode OfdmPhy::GetOfdmRate(uint64_t rate, uint16_t bw) {
  switch (bw) {
  case 20:
    switch (rate) {
    case 6000000:
    ...
    default:
      NS_ABORT_MSG("Inexistent rate ("
                   << rate << " bps) requested for 11a OFDM (default)");
      return WifiMode();
    }
    break;
  case 10:
    ...
    default:
      NS_ABORT_MSG("Inexistent rate ("
                   << rate << " bps) requested for 11a OFDM (10 MHz)");
      return WifiMode();
    }
    break;
  case 5:
    ...
    default:
      NS_ABORT_MSG("Inexistent rate ("
                   << rate << " bps) requested for 11a OFDM (5 MHz)");
      return WifiMode();
    }
    break;
  default:
    NS_ABORT_MSG("Inexistent bandwidth (" << +bw
...
WifiMode OfdmPhy::CreateOfdmMode(std::string uniqueName, bool isMandatory) {
  ...
  return WifiModeFactory::CreateWifiMode(
      uniqueName, WIFI_MOD_CLASS_OFDM, isMandatory,
...
WifiCodeRate OfdmPhy::GetCodeRate(const std::string &name) {
  return m_ofdmModulationLookupTable.at(name).first;
}

uint16_t OfdmPhy::GetConstellationSize(const std::string &name) {
  return m_ofdmModulationLookupTable.at(name).second;
}

uint64_t OfdmPhy::GetPhyRate(const std::string &name, uint16_t channelWidth) {
...
uint64_t OfdmPhy::CalculatePhyRate(WifiCodeRate codeRate, uint64_t dataRate) {
...
double OfdmPhy::GetCodeRatio(WifiCodeRate codeRate) {
  switch (codeRate) {
...
uint64_t OfdmPhy::GetDataRate(const std::string &name, uint16_t channelWidth) {
  WifiCodeRate codeRate = GetCodeRate(name);
  uint16_t constellationSize = GetConstellationSize(name);
  return CalculateDataRate(codeRate, constellationSize, channelWidth);
}

uint64_t OfdmPhy::CalculateDataRate(WifiCodeRate codeRate,
                                    uint16_t constellationSize,
                                    uint16_t channelWidth) {
  return CalculateDataRate(
      GetSymbolDuration(channelWidth), GetUsableSubcarriers(),
      static_cast<uint16_t>(log2(constellationSize)), GetCodeRatio(codeRate));
}

uint64_t OfdmPhy::CalculateDataRate(Time symbolDuration,
                                    uint16_t usableSubCarriers,
                                    uint16_t numberOfBitsPerSubcarrier,
                                    double codingRate) {
  double symbolRate =
      (1e9 / static_cast<double>(symbolDuration.GetNanoSeconds()));
  return lrint(ceil(symbolRate * usableSubCarriers * numberOfBitsPerSubcarrier *
                    codingRate));
}

uint16_t OfdmPhy::GetUsableSubcarriers() { return 48; }

Time OfdmPhy::GetSymbolDuration(uint16_t channelWidth) {
  Time symbolDuration = MicroSeconds(4);
  uint8_t bwFactor = 1;
  if (channelWidth == 10) {
    bwFactor = 2;
