...
uint64_t OfdmPpdu::LSigHeader::GetRate(uint16_t channelWidth) const {
  uint64_t rate = 0;
  switch (m_rate) {
  case 0b1101:
    rate = 6000000;
...
void OfdmPpdu::LSigHeader::SetLength(uint16_t length) {
  NS_ASSERT_MSG(length < 4096, "Invalid length");
  m_length = length;
}

uint16_t OfdmPpdu::LSigHeader::GetLength() const { return m_length; }
