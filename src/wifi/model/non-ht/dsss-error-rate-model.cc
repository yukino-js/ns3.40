...
double DsssErrorRateModel::GetDsssDqpskCck5_5SuccessRate(double sinr,
                                                         uint64_t nbits) {
  NS_LOG_FUNCTION_NOARGS();
#ifdef HAVE_GSL
...
double DsssErrorRateModel::GetDsssDqpskCck11SuccessRate(double sinr,
                                                        uint64_t nbits) {
  NS_LOG_FUNCTION_NOARGS();
#ifdef HAVE_GSL
...
double IntegralFunction(double x, void *params) {
  ...
  double IntegralFunction =
      std::pow(2 * gsl_cdf_ugaussian_P(x + beta) - 1, n - 1) *
