...
TypeId CtrlBAckRequestHeader::GetTypeId() {
...
TypeId CtrlBAckRequestHeader::GetInstanceTypeId() const { return GetTypeId(); }

void CtrlBAckRequestHeader::Print(std::ostream &os) const {
  os << "TID_INFO=" << m_tidInfo << ", StartingSeq=" << std::hex
     << m_startingSeq << std::dec;
}
...
void CtrlBAckRequestHeader::Serialize(Buffer::Iterator start) const {
...
uint32_t CtrlBAckRequestHeader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
uint16_t CtrlBAckRequestHeader::GetBarControl() const {
  uint16_t res = 0;
  switch (m_barType.m_variant) {
  case BlockAckReqType::BASIC:
    break;
...
void CtrlBAckRequestHeader::SetBarControl(uint16_t bar) {
...
uint16_t CtrlBAckRequestHeader::GetStartingSequenceControl() const {
  return (m_startingSeq << 4) & 0xfff0;
}

void CtrlBAckRequestHeader::SetStartingSequenceControl(uint16_t seqControl) {
  m_startingSeq = (seqControl >> 4) & 0x0fff;
}

void CtrlBAckRequestHeader::SetHtImmediateAck(bool immediateAck) {
  m_barAckPolicy = immediateAck;
}

void CtrlBAckRequestHeader::SetType(BlockAckReqType type) { m_barType = type; }

BlockAckReqType CtrlBAckRequestHeader::GetType() const { return m_barType; }

void CtrlBAckRequestHeader::SetTidInfo(uint8_t tid) {
  m_tidInfo = static_cast<uint16_t>(tid);
}

void CtrlBAckRequestHeader::SetStartingSequence(uint16_t seq) {
  m_startingSeq = seq;
}

bool CtrlBAckRequestHeader::MustSendHtImmediateAck() const {
  return m_barAckPolicy;
}

uint8_t CtrlBAckRequestHeader::GetTidInfo() const {
  auto tid = static_cast<uint8_t>(m_tidInfo);
  return tid;
}

uint16_t CtrlBAckRequestHeader::GetStartingSequence() const {
  return m_startingSeq;
}

bool CtrlBAckRequestHeader::IsBasic() const {
  return m_barType.m_variant == BlockAckReqType::BASIC;
}

bool CtrlBAckRequestHeader::IsCompressed() const {
  return m_barType.m_variant == BlockAckReqType::COMPRESSED;
}

bool CtrlBAckRequestHeader::IsExtendedCompressed() const {
  return m_barType.m_variant == BlockAckReqType::EXTENDED_COMPRESSED;
}

bool CtrlBAckRequestHeader::IsMultiTid() const {
...
CtrlBAckResponseHeader::CtrlBAckResponseHeader()
    : m_baAckPolicy(false), m_tidInfo(0) {
  SetType(BlockAckType::BASIC);
}

CtrlBAckResponseHeader::~CtrlBAckResponseHeader() {}

TypeId CtrlBAckResponseHeader::GetTypeId() {
...
TypeId CtrlBAckResponseHeader::GetInstanceTypeId() const { return GetTypeId(); }

void CtrlBAckResponseHeader::Print(std::ostream &os) const {
  if (m_baType.m_variant != BlockAckType::MULTI_STA) {
    ...
    for (std::size_t i = 0; i < m_baInfo.size(); i++) {
      os << "{AID=" << GetAid11(i) << ", TID=" << GetTidInfo(i)
         << ", StartingSeq=0x" << std::hex << m_baInfo[i].m_startingSeq
         << std::dec << "}";
    }
  }
}
...
void CtrlBAckResponseHeader::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t CtrlBAckResponseHeader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
void CtrlBAckResponseHeader::SetHtImmediateAck(bool immediateAck) {
  m_baAckPolicy = immediateAck;
}

void CtrlBAckResponseHeader::SetType(BlockAckType type) {
  m_baType = type;
  m_baInfo.clear();
  ...
  for (auto &bitmapLen : m_baType.m_bitmapLen) {
    BaInfoInstance baInfoInstance{.m_aidTidInfo = 0,
                                  .m_startingSeq = 0,
                                  .m_bitmap =
                                      std::vector<uint8_t>(bitmapLen, 0),
                                  .m_ra = Mac48Address()};
...
BlockAckType CtrlBAckResponseHeader::GetType() const { return m_baType; }

void CtrlBAckResponseHeader::SetTidInfo(uint8_t tid, std::size_t index) {
  ...
  if (m_baType.m_variant != BlockAckType::MULTI_STA) {
    ...
    m_baInfo[index].m_aidTidInfo |=
        ((static_cast<uint16_t>(tid) & 0x000f) << 12);
  }
}

void CtrlBAckResponseHeader::SetStartingSequence(uint16_t seq,
                                                 std::size_t index) {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  NS_ASSERT(index < m_baInfo.size());
...
bool CtrlBAckResponseHeader::MustSendHtImmediateAck() const {
  return m_baAckPolicy;
}

uint8_t CtrlBAckResponseHeader::GetTidInfo(std::size_t index) const {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  NS_ASSERT(index < m_baInfo.size());
...
uint16_t CtrlBAckResponseHeader::GetStartingSequence(std::size_t index) const {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  NS_ASSERT(index < m_baInfo.size());
...
bool CtrlBAckResponseHeader::IsBasic() const {
  return m_baType.m_variant == BlockAckType::BASIC;
}

bool CtrlBAckResponseHeader::IsCompressed() const {
  return m_baType.m_variant == BlockAckType::COMPRESSED;
}

bool CtrlBAckResponseHeader::IsExtendedCompressed() const {
  return m_baType.m_variant == BlockAckType::EXTENDED_COMPRESSED;
}

bool CtrlBAckResponseHeader::IsMultiTid() const {
  return m_baType.m_variant == BlockAckType::MULTI_TID;
}

bool CtrlBAckResponseHeader::IsMultiSta() const {
  return m_baType.m_variant == BlockAckType::MULTI_STA;
}

void CtrlBAckResponseHeader::SetAid11(uint16_t aid, std::size_t index) {
...
uint16_t CtrlBAckResponseHeader::GetAid11(std::size_t index) const {
  NS_ASSERT(m_baType.m_variant == BlockAckType::MULTI_STA &&
            index < m_baInfo.size());
...
void CtrlBAckResponseHeader::SetAckType(bool type, std::size_t index) {
  NS_ASSERT(m_baType.m_variant == BlockAckType::MULTI_STA &&
            index < m_baInfo.size());
...
bool CtrlBAckResponseHeader::GetAckType(std::size_t index) const {
  NS_ASSERT(m_baType.m_variant == BlockAckType::MULTI_STA &&
            index < m_baInfo.size());
...
void CtrlBAckResponseHeader::SetUnassociatedStaAddress(const Mac48Address &ra,
                                                       std::size_t index) {
...
std::size_t CtrlBAckResponseHeader::GetNPerAidTidInfoSubfields() const {
  NS_ASSERT(m_baType.m_variant == BlockAckType::MULTI_STA);
...
uint16_t CtrlBAckResponseHeader::GetBaControl() const {
  uint16_t res = 0;
  if (m_baAckPolicy) {
    res |= 0x1;
  }
...
void CtrlBAckResponseHeader::SetBaControl(uint16_t ba) {
...
void CtrlBAckResponseHeader::SetStartingSequenceControl(uint16_t seqControl,
                                                        std::size_t index) {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  NS_ASSERT(index < m_baInfo.size());
  ...
  if (m_baType.m_variant == BlockAckType::COMPRESSED) {
    ...
    if (((seqControl >> 3) & 0x0001) == 0 &&
        ((seqControl >> 1) & 0x0003) == 0) {
      SetType({BlockAckType::COMPRESSED, {8}});
    } else if (((seqControl >> 3) & 0x0001) == 0 &&
               ((seqControl >> 1) & 0x0003) == 2) {
    ...
    if (((seqControl >> 3) & 0x0001) == 0 &&
      ...
      bitmapLen = 8;
               ...
               ((seqControl >> 1) & 0x0003) == 1) {
      bitmapLen = 16;
    } else if (((seqControl >> 3) & 0x0001) == 0 &&
               ((seqControl >> 1) & 0x0003) == 2) {
      bitmapLen = 32;
    } else if (((seqControl >> 3) & 0x0001) == 0 &&
               ((seqControl >> 1) & 0x0003) == 3) {
      bitmapLen = 4;
    } else {
      NS_FATAL_ERROR("Reserved configurations");
    }
...
Buffer::Iterator
CtrlBAckResponseHeader::SerializeBitmap(Buffer::Iterator start,
...
void CtrlBAckResponseHeader::SetReceivedPacket(uint16_t seq,
                                               std::size_t index) {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  NS_ASSERT(index < m_baInfo.size());
...
void CtrlBAckResponseHeader::SetReceivedFragment(uint16_t seq, uint8_t frag) {
  NS_ASSERT(frag < 16);
  if (!IsInBitmap(seq)) {
    return;
  }
  switch (m_baType.m_variant) {
  case BlockAckType::BASIC:
    m_baInfo[0].m_bitmap[IndexInBitmap(seq) * 2 + frag / 8] |=
        (0x01 << (frag % 8));
    break;
  case BlockAckType::COMPRESSED:
  case BlockAckType::EXTENDED_COMPRESSED:
...
bool CtrlBAckResponseHeader::IsPacketReceived(uint16_t seq,
                                              std::size_t index) const {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  NS_ASSERT(index < m_baInfo.size());
...
bool CtrlBAckResponseHeader::IsFragmentReceived(uint16_t seq,
                                                uint8_t frag) const {
  NS_ASSERT(frag < 16);
  if (!IsInBitmap(seq)) {
    return false;
  ...
  switch (m_baType.m_variant) {
  case BlockAckType::BASIC:
    return (m_baInfo[0].m_bitmap[IndexInBitmap(seq) * 2 + frag / 8] &
            (0x01 << (frag % 8))) != 0;
  case BlockAckType::COMPRESSED:
  case BlockAckType::EXTENDED_COMPRESSED:
  case BlockAckType::MULTI_STA:
...
uint16_t CtrlBAckResponseHeader::IndexInBitmap(uint16_t seq,
                                               std::size_t index) const {
  uint16_t i;
...
bool CtrlBAckResponseHeader::IsInBitmap(uint16_t seq, std::size_t index) const {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  ...
  return (seq - m_baInfo[index].m_startingSeq + SEQNO_SPACE_SIZE) %
             SEQNO_SPACE_SIZE <
         nAckedMpdus;
}
...
void CtrlBAckResponseHeader::ResetBitmap(std::size_t index) {
  NS_ASSERT_MSG(m_baType.m_variant == BlockAckType::MULTI_STA || index == 0,
                "index can only be non null for Multi-STA Block Ack");
  NS_ASSERT(index < m_baInfo.size());
...
CtrlTriggerUserInfoField::CtrlTriggerUserInfoField(TriggerFrameType triggerType,
    ...
    : m_variant(variant), m_aid12(0), m_ruAllocation(0),
      m_ulFecCodingType(false), m_ulMcs(0), m_ulDcm(false), m_ps160(true),
      m_ulTargetRssi(0), m_triggerType(triggerType),
...
void CtrlTriggerUserInfoField::Print(std::ostream &os) const {
...
Buffer::Iterator CtrlTriggerUserInfoField::Deserialize(Buffer::Iterator start) {
  NS_ABORT_MSG_IF(m_triggerType == TriggerFrameType::BFRP_TRIGGER,
                  "BFRP Trigger frame is not supported");
  NS_ABORT_MSG_IF(m_triggerType == TriggerFrameType::GCR_MU_BAR_TRIGGER,
                  "GCR-MU-BAR Trigger frame is not supported");
...
TriggerFrameType CtrlTriggerUserInfoField::GetType() const {
  return m_triggerType;
}

WifiPreamble CtrlTriggerUserInfoField::GetPreambleType() const {
  switch (m_variant) {
...
uint16_t CtrlTriggerUserInfoField::GetAid12() const { return m_aid12; }

bool CtrlTriggerUserInfoField::HasRaRuForAssociatedSta() const {
  return (m_aid12 == 0);
}

bool CtrlTriggerUserInfoField::HasRaRuForUnassociatedSta() const {
  return (m_aid12 == 2045);
}

void CtrlTriggerUserInfoField::SetRuAllocation(HeRu::RuSpec ru) {
...
HeRu::RuSpec CtrlTriggerUserInfoField::GetRuAllocation() const {
  NS_ABORT_MSG_IF(m_triggerType == TriggerFrameType::MU_RTS_TRIGGER,
...
void CtrlTriggerUserInfoField::SetMuRtsRuAllocation(uint8_t value) {
  NS_ABORT_MSG_IF(m_triggerType != TriggerFrameType::MU_RTS_TRIGGER,
...
uint8_t CtrlTriggerUserInfoField::GetMuRtsRuAllocation() const {
  NS_ABORT_MSG_IF(m_triggerType != TriggerFrameType::MU_RTS_TRIGGER,
...
void CtrlTriggerUserInfoField::SetUlFecCodingType(bool ldpc) {
  m_ulFecCodingType = ldpc;
}

bool CtrlTriggerUserInfoField::GetUlFecCodingType() const {
  return m_ulFecCodingType;
}

void CtrlTriggerUserInfoField::SetUlMcs(uint8_t mcs) {
  uint8_t maxMcs = m_variant == TriggerFrameVariant::EHT ? 13 : 11;
  NS_ABORT_MSG_IF(mcs > maxMcs, "Invalid MCS index");
  m_ulMcs = mcs;
}

uint8_t CtrlTriggerUserInfoField::GetUlMcs() const { return m_ulMcs; }

void CtrlTriggerUserInfoField::SetUlDcm(bool dcm) {
  NS_ASSERT_MSG(m_variant == TriggerFrameVariant::HE,
                "UL DCM flag only present in HE variant");
  m_ulDcm = dcm;
}

bool CtrlTriggerUserInfoField::GetUlDcm() const {
  NS_ASSERT_MSG(m_variant == TriggerFrameVariant::HE,
                "UL DCM flag only present in HE variant");
  return m_ulDcm;
}

void CtrlTriggerUserInfoField::SetSsAllocation(uint8_t startingSs,
                                               uint8_t nSs) {
  NS_ABORT_MSG_IF(m_aid12 == 0 || m_aid12 == 2045,
                  "SS Allocation subfield not present");
  NS_ABORT_MSG_IF(!startingSs || startingSs > 8,
                  "Starting SS must be from 1 to 8");
...
uint8_t CtrlTriggerUserInfoField::GetStartingSs() const {
  if (m_aid12 == 0 || m_aid12 == 2045) {
    return 1;
  }
...
uint8_t CtrlTriggerUserInfoField::GetNss() const {
  if (m_aid12 == 0 || m_aid12 == 2045) {
    return 1;
  }
...
void CtrlTriggerUserInfoField::SetRaRuInformation(uint8_t nRaRu,
                                                  bool moreRaRu) {
  NS_ABORT_MSG_IF(m_aid12 != 0 && m_aid12 != 2045,
                  "RA-RU Information subfield not present");
  NS_ABORT_MSG_IF(!nRaRu || nRaRu > 32,
                  "Number of contiguous RA-RUs must be from 1 to 32");

  m_bits26To31.raRuInformation.nRaRu = nRaRu - 1;
  m_bits26To31.raRuInformation.moreRaRu = moreRaRu;
}

uint8_t CtrlTriggerUserInfoField::GetNRaRus() const {
  NS_ABORT_MSG_IF(m_aid12 != 0 && m_aid12 != 2045,
                  "RA-RU Information subfield not present");
...
bool CtrlTriggerUserInfoField::GetMoreRaRu() const {
  NS_ABORT_MSG_IF(m_aid12 != 0 && m_aid12 != 2045,
                  "RA-RU Information subfield not present");
...
bool CtrlTriggerUserInfoField::IsUlTargetRssiMaxTxPower() const {
  return (m_ulTargetRssi == 127);
}

int8_t CtrlTriggerUserInfoField::GetUlTargetRssi() const {
  NS_ABORT_MSG_IF(m_ulTargetRssi == 127, "STA must use its max TX power");

  return static_cast<int8_t>(m_ulTargetRssi) - 110;
}

void CtrlTriggerUserInfoField::SetBasicTriggerDepUserInfo(uint8_t spacingFactor,
                                                          uint8_t tidLimit,
                                                          AcIndex prefAc) {
...
uint8_t CtrlTriggerUserInfoField::GetTidAggregationLimit() const {
  NS_ABORT_MSG_IF(m_triggerType != TriggerFrameType::BASIC_TRIGGER,
                  "Not a Basic Trigger Frame");
...
AcIndex CtrlTriggerUserInfoField::GetPreferredAc() const {
  NS_ABORT_MSG_IF(m_triggerType != TriggerFrameType::BASIC_TRIGGER,
                  "Not a Basic Trigger Frame");
...
void CtrlTriggerUserInfoField::SetMuBarTriggerDepUserInfo(
    const CtrlBAckRequestHeader &bar) {
...
CtrlTriggerHeader::CtrlTriggerHeader()
    : m_variant(TriggerFrameVariant::HE),
      m_triggerType(TriggerFrameType::BASIC_TRIGGER), m_ulLength(0),
      m_moreTF(false), m_csRequired(false), m_ulBandwidth(0), m_giAndLtfType(0),
      m_apTxPower(0), m_ulSpatialReuse(0), m_padding(0) {}

CtrlTriggerHeader::CtrlTriggerHeader(TriggerFrameType type,
...
TypeId CtrlTriggerHeader::GetTypeId() {
...
TypeId CtrlTriggerHeader::GetInstanceTypeId() const { return GetTypeId(); }

void CtrlTriggerHeader::Print(std::ostream &os) const {
...
void CtrlTriggerHeader::SetVariant(TriggerFrameVariant variant) {
...
TriggerFrameVariant CtrlTriggerHeader::GetVariant() const { return m_variant; }
...
void CtrlTriggerHeader::Serialize(Buffer::Iterator start) const {
  NS_ABORT_MSG_IF(m_triggerType == TriggerFrameType::BFRP_TRIGGER,
                  "BFRP Trigger frame is not supported");
  NS_ABORT_MSG_IF(m_triggerType == TriggerFrameType::GCR_MU_BAR_TRIGGER,
                  "GCR-MU-BAR Trigger frame is not supported");
...
uint32_t CtrlTriggerHeader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  ...
  m_variant =
      bit54and55 == 3 ? TriggerFrameVariant::HE : TriggerFrameVariant::EHT;
  m_userInfoFields.clear();
  m_padding = 0;
...
void CtrlTriggerHeader::SetType(TriggerFrameType type) { m_triggerType = type; }

TriggerFrameType CtrlTriggerHeader::GetType() const { return m_triggerType; }

const char *CtrlTriggerHeader::GetTypeString() const {
  return GetTypeString(GetType());
}

const char *CtrlTriggerHeader::GetTypeString(TriggerFrameType type) {
#define FOO(x)                                                                 \
...
bool CtrlTriggerHeader::IsBasic() const {
  return (m_triggerType == TriggerFrameType::BASIC_TRIGGER);
}

bool CtrlTriggerHeader::IsBfrp() const {
  return (m_triggerType == TriggerFrameType::BFRP_TRIGGER);
}

bool CtrlTriggerHeader::IsMuBar() const {
  return (m_triggerType == TriggerFrameType::MU_BAR_TRIGGER);
}

bool CtrlTriggerHeader::IsMuRts() const {
  return (m_triggerType == TriggerFrameType::MU_RTS_TRIGGER);
}

bool CtrlTriggerHeader::IsBsrp() const {
  return (m_triggerType == TriggerFrameType::BSRP_TRIGGER);
}

bool CtrlTriggerHeader::IsGcrMuBar() const {
  return (m_triggerType == TriggerFrameType::GCR_MU_BAR_TRIGGER);
}

bool CtrlTriggerHeader::IsBqrp() const {
  return (m_triggerType == TriggerFrameType::BQRP_TRIGGER);
}

bool CtrlTriggerHeader::IsNfrp() const {
  return (m_triggerType == TriggerFrameType::NFRP_TRIGGER);
}

void CtrlTriggerHeader::SetUlLength(uint16_t len) {
  m_ulLength = (len & 0x0fff);
}

uint16_t CtrlTriggerHeader::GetUlLength() const { return m_ulLength; }

WifiTxVector CtrlTriggerHeader::GetHeTbTxVector(uint16_t staId) const {
  NS_ABORT_MSG_IF(m_triggerType == TriggerFrameType::MU_RTS_TRIGGER,
  ...
  v.SetHeMuUserInfo(staId, {userInfoIt->GetRuAllocation(),
                            userInfoIt->GetUlMcs(), userInfoIt->GetNss()});
  return v;
}

void CtrlTriggerHeader::SetMoreTF(bool more) { m_moreTF = more; }

bool CtrlTriggerHeader::GetMoreTF() const { return m_moreTF; }

void CtrlTriggerHeader::SetCsRequired(bool cs) { m_csRequired = cs; }

bool CtrlTriggerHeader::GetCsRequired() const { return m_csRequired; }

void CtrlTriggerHeader::SetUlBandwidth(uint16_t bw) {
  switch (bw) {
  case 20:
    m_ulBandwidth = 0;
    break;
...
uint16_t CtrlTriggerHeader::GetUlBandwidth() const {
  return (1 << m_ulBandwidth) * 20;
}

void CtrlTriggerHeader::SetGiAndLtfType(uint16_t guardInterval,
                                        uint8_t ltfType) {
...
uint16_t CtrlTriggerHeader::GetGuardInterval() const {
...
uint8_t CtrlTriggerHeader::GetLtfType() const {
  if (m_giAndLtfType == 0) {
    return 1;
...
void CtrlTriggerHeader::SetUlSpatialReuse(uint16_t sr) {
  m_ulSpatialReuse = sr;
}

uint16_t CtrlTriggerHeader::GetUlSpatialReuse() const {
  return m_ulSpatialReuse;
}

void CtrlTriggerHeader::SetPaddingSize(std::size_t size) {
  NS_ABORT_MSG_IF(
      size == 1, "The Padding field, if present, shall be at least two octets");
  m_padding = size;
}
...
CtrlTriggerUserInfoField &CtrlTriggerHeader::AddUserInfoField() {
...
CtrlTriggerHeader::ConstIterator CtrlTriggerHeader::begin() const {
  return m_userInfoFields.begin();
}

CtrlTriggerHeader::ConstIterator CtrlTriggerHeader::end() const {
  return m_userInfoFields.end();
}

CtrlTriggerHeader::Iterator CtrlTriggerHeader::begin() {
  return m_userInfoFields.begin();
}

CtrlTriggerHeader::Iterator CtrlTriggerHeader::end() {
  return m_userInfoFields.end();
}

std::size_t CtrlTriggerHeader::GetNUserInfoFields() const {
...
bool CtrlTriggerHeader::IsValid() const {
