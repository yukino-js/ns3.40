...
public:
  ...
  enum MeshActionValue {
    ...
    MDA_SETUP_REPLY = 5, // Action Value:5 in Category 13: Mesh MCCA-Setup-Reply
                         // (not used so far)
    MDAOP_ADVERTISEMENT_REQUEST =
        6, // Action Value:6 in Category 13: Mesh MCCA-Advertisement-Request
           // (not used so far)
    MDAOP_ADVERTISEMENTS =
        7, // Action Value:7 in Category 13: Mesh (not used so far)
    MDAOP_SET_TEARDOWN =
        8, // Action Value:8 in Category 13: Mesh (not used so far)
    TBTT_ADJUSTMENT_REQUEST =
        9, // Action Value:9 in Category 13: Mesh (not used so far)
    TBTT_ADJUSTMENT_RESPONSE =
        10, // Action Value:10 in Category 13: Mesh (not used so far)
  };
...
private:
  ...
  uint16_t
      m_reasonCode; //!< Not used for now. Always set to 1: "Unspecified reason"
};
...
public:
  ...
  std::optional<EmlsrParamUpdate>
      m_emlsrParamUpdate{}; //!< EMLSR Parameter Update field
};
