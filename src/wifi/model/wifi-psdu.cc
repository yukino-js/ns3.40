...
WifiPsdu::WifiPsdu(Ptr<WifiMpdu> mpdu, bool isSingle) : m_isSingle(isSingle) {
...
bool WifiPsdu::IsSingle() const { return m_isSingle; }

bool WifiPsdu::IsAggregate() const {
  return (m_mpduList.size() > 1 || m_isSingle);
}

Ptr<const Packet> WifiPsdu::GetPacket() const {
...
void WifiPsdu::SetDuration(Time duration) {
...
std::set<uint8_t> WifiPsdu::GetTids() const {
  std::set<uint8_t> s;
  for (auto &mpdu : m_mpduList) {
...
WifiMacHeader::QosAckPolicy WifiPsdu::GetAckPolicyForTid(uint8_t tid) const {
...
void WifiPsdu::SetAckPolicyForTid(uint8_t tid,
                                  WifiMacHeader::QosAckPolicy policy) {
...
uint16_t WifiPsdu::GetMaxDistFromStartingSeq(uint16_t startingSeq) const {
...
uint32_t WifiPsdu::GetSize() const { return m_size; }

const WifiMacHeader &WifiPsdu::GetHeader(std::size_t i) const {
  return m_mpduList.at(i)->GetHeader();
}

WifiMacHeader &WifiPsdu::GetHeader(std::size_t i) {
  return m_mpduList.at(i)->GetHeader();
}

Ptr<const Packet> WifiPsdu::GetPayload(std::size_t i) const {
  return m_mpduList.at(i)->GetPacket();
}

Ptr<Packet> WifiPsdu::GetAmpduSubframe(std::size_t i) const {
  NS_ASSERT(i < m_mpduList.size());
  Ptr<Packet> subframe = m_mpduList.at(i)->GetProtocolDataUnit();
  subframe->AddHeader(MpduAggregator::GetAmpduSubframeHeader(
      static_cast<uint16_t>(subframe->GetSize()), m_isSingle));
...
std::size_t WifiPsdu::GetNMpdus() const { return m_mpduList.size(); }

std::vector<Ptr<WifiMpdu>>::const_iterator WifiPsdu::begin() const {
  return m_mpduList.begin();
}

std::vector<Ptr<WifiMpdu>>::iterator WifiPsdu::begin() {
  return m_mpduList.begin();
}

std::vector<Ptr<WifiMpdu>>::const_iterator WifiPsdu::end() const {
  return m_mpduList.end();
}

std::vector<Ptr<WifiMpdu>>::iterator WifiPsdu::end() {
  return m_mpduList.end();
}

void WifiPsdu::Print(std::ostream &os) const {
  os << "size=" << m_size;
  if (IsAggregate()) {
