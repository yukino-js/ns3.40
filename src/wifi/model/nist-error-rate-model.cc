...
TypeId NistErrorRateModel::GetTypeId() {
...
double NistErrorRateModel::GetQamBer(uint16_t constellationSize,
  ...
  double ber =
      ((bitsPerSymbol - 1) / (bitsPerSymbol * std::log2(bitsPerSymbol))) *
      erfc(z);
  NS_LOG_INFO(constellationSize << "-QAM: snr=" << snr << " ber=" << ber);
  return ber;
}

double NistErrorRateModel::GetFecBpskBer(double snr, uint64_t nbits,
...
double NistErrorRateModel::GetFecQpskBer(double snr, uint64_t nbits,
                                         uint8_t bValue) const {
  NS_LOG_FUNCTION(this << snr << nbits << +bValue);
...
double NistErrorRateModel::CalculatePe(double p, uint8_t bValue) const {
  ...
  if (bValue == 1) {
    ...
    pe = 1.0 / (2.0 * bValue) *
         (3.0 * std::pow(D, 6) + 70.0 * std::pow(D, 7) +
          ...
          6160.0 * std::pow(D, 10) + 27128.0 * std::pow(D, 11) +
    ...
    pe = 1.0 / (2.0 * bValue) *
         (42.0 * std::pow(D, 5) + 201.0 * std::pow(D, 6) +
          ...
          62935.0 * std::pow(D, 9) + 379644.0 * std::pow(D, 10) +
    ...
    pe = 1.0 / (2.0 * bValue) *
         (92.0 * std::pow(D, 4.0) + 528.0 * std::pow(D, 5.0) +
          ...
          792114.0 * std::pow(D, 8.0) + 7375573.0 * std::pow(D, 9.0) +
          67884974.0 * std::pow(D, 10.0) + 610875423.0 * std::pow(D, 11.0) +
          5427275376.0 * std::pow(D, 12.0) + 47664215639.0 * std::pow(D, 13.0));
  } else {
    NS_ASSERT(false);
  }
  return pe;
...
double NistErrorRateModel::GetFecQamBer(uint16_t constellationSize, double snr,
                                        uint64_t nbits, uint8_t bValue) const {
...
uint8_t NistErrorRateModel::GetBValue(WifiCodeRate codeRate) const {
  switch (codeRate) {
...
double NistErrorRateModel::DoGetChunkSuccessRate(
    WifiMode mode, const WifiTxVector &txVector, double snr, uint64_t nbits,
    uint8_t numRxAntennas, WifiPpduField field, uint16_t staId) const {
  NS_LOG_FUNCTION(this << mode << snr << nbits << +numRxAntennas << field
                       << staId);
  if (mode.GetModulationClass() >= WIFI_MOD_CLASS_ERP_OFDM) {
    if (mode.GetConstellationSize() == 2) {
      ...
      return GetFecQamBer(mode.GetConstellationSize(), snr, nbits,
