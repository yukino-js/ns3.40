...
void WifiMacQueueContainer::clear() {
  m_queues.clear();
...
WifiMacQueueContainer::insert(const_iterator pos, Ptr<WifiMpdu> item) {
  ...
  NS_ABORT_MSG_UNLESS(
      pos == m_queues[queueId].cend() || GetQueueId(pos->mpdu) == queueId,
      "pos iterator does not point to the correct container queue");
  NS_ABORT_MSG_IF(!item->IsOriginal(),
...
Ptr<WifiMpdu> WifiMacQueueContainer::GetItem(const const_iterator it) const {
  return it->mpdu;
}
...
WifiMacQueueContainer::GetNBytes(const WifiContainerQueueId &queueId) const {
  if (auto it = m_queues.find(queueId);
      it == m_queues.end() || it->second.empty()) {
    return 0;
  }
...
WifiMacQueueContainer::ExtractExpiredMpdus(
...
WifiMacQueueContainer::DoExtractExpiredMpdus(ContainerQueue &queue) const {
  std::optional<std::pair<WifiMacQueueContainer::iterator,
                          WifiMacQueueContainer::iterator>>
      ret;
