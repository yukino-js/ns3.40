...
MacTxMiddle::MacTxMiddle() : m_sequence(0) { NS_LOG_FUNCTION(this); }
...
uint16_t MacTxMiddle::GetNextSequenceNumberFor(const WifiMacHeader *hdr) {
  NS_LOG_FUNCTION(this);
  uint16_t retval;
...
uint16_t MacTxMiddle::PeekNextSequenceNumberFor(const WifiMacHeader *hdr) {
  NS_LOG_FUNCTION(this);
  uint16_t retval;
  if (hdr->IsQosData() && !hdr->GetAddr1().IsGroup()) {
    uint8_t tid = hdr->GetQosTid();
...
uint16_t MacTxMiddle::GetNextSeqNumberByTidAndAddress(uint8_t tid,
...
void MacTxMiddle::SetSequenceNumberFor(const WifiMacHeader *hdr) {
