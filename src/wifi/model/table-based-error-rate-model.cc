...
TypeId TableBasedErrorRateModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TableBasedErrorRateModel")
          ...
          .AddAttribute("FallbackErrorRateModel",
                        ...
                        MakePointerAccessor(
                            &TableBasedErrorRateModel::m_fallbackErrorModel),
...
TableBasedErrorRateModel::TableBasedErrorRateModel() { NS_LOG_FUNCTION(this); }
...
double TableBasedErrorRateModel::RoundSnr(double snr, double precision) const {
  NS_LOG_FUNCTION(this << snr);
  double multiplier = std::round(std::pow(10.0, precision));
  return std::floor(snr * multiplier + 0.5) / multiplier;
}

std::optional<uint8_t> TableBasedErrorRateModel::GetMcsForMode(WifiMode mode) {
...
double TableBasedErrorRateModel::DoGetChunkSuccessRate(
  ...
  NS_LOG_FUNCTION(this << mode << txVector << snr << nbits << +numRxAntennas
                       << field << staId);
  ...
  if (auto ret = GetMcsForMode(mode); ret.has_value()) {
    ...
    NS_LOG_DEBUG("No MCS found for mode " << mode
                                          << ": use fallback error rate model");
    return m_fallbackErrorModel->GetChunkSuccessRate(
        mode, txVector, snr, nbits, numRxAntennas, field, staId);
  }
  ...
  if (mcs >=
      (ldpc ? ERROR_TABLE_LDPC_MAX_NUM_MCS : ERROR_TABLE_BCC_MAX_NUM_MCS)) {
    NS_LOG_WARN(
        "Table missing for MCS: "
        << +mcs
  ...
  auto errorTable = (ldpc ? AwgnErrorTableLdpc1458
                          : (size < m_threshold ? AwgnErrorTableBcc32
                                                : AwgnErrorTableBcc1458));
  const auto &itVector = errorTable[mcs];
  auto itTable =
      std::find_if(itVector.cbegin(), itVector.cend(),
  ...
  uint16_t tableSize =
      (ldpc ? ERROR_TABLE_LDPC_FRAME_SIZE
