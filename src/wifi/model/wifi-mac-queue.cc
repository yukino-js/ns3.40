...
WifiMacQueue::WifiMacQueue(AcIndex ac)
    : m_ac(ac), NS_LOG_TEMPLATE_DEFINE("WifiMacQueue") {}

WifiMacQueue::~WifiMacQueue() { NS_LOG_FUNCTION_NOARGS(); }

void WifiMacQueue::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_scheduler = nullptr;
  Queue<WifiMpdu, WifiMacQueueContainer>::DoDispose();
}

AcIndex WifiMacQueue::GetAc() const { return m_ac; }

WifiMacQueue::Iterator WifiMacQueue::GetIt(Ptr<const WifiMpdu> mpdu) const {
  NS_ASSERT(mpdu->IsQueued());
  return mpdu->GetQueueIt(WmqIteratorTag());
}

Ptr<WifiMpdu> WifiMacQueue::GetOriginal(Ptr<WifiMpdu> mpdu) {
  return GetIt(mpdu)->mpdu;
}

Ptr<WifiMpdu> WifiMacQueue::GetAlias(Ptr<const WifiMpdu> mpdu, uint8_t linkId) {
  if (!mpdu->IsQueued()) {
    return nullptr;
  }
...
void WifiMacQueue::ExtractExpiredMpdus(
    const WifiContainerQueueId &queueId) const {
  NS_LOG_FUNCTION(this);
...
void WifiMacQueue::ExtractAllExpiredMpdus() const {
  NS_LOG_FUNCTION(this);
...
void WifiMacQueue::WipeAllExpiredMpdus() {
  NS_LOG_FUNCTION(this);
...
bool WifiMacQueue::TtlExceeded(Ptr<const WifiMpdu> item, const Time &now) {
...
void WifiMacQueue::SetScheduler(Ptr<WifiMacQueueScheduler> scheduler) {
  NS_LOG_FUNCTION(this << scheduler);
  m_scheduler = scheduler;
}

void WifiMacQueue::SetMaxDelay(Time delay) {
  NS_LOG_FUNCTION(this << delay);
  m_maxDelay = delay;
}

Time WifiMacQueue::GetMaxDelay() const { return m_maxDelay; }

bool WifiMacQueue::Enqueue(Ptr<WifiMpdu> item) {
...
bool WifiMacQueue::Insert(ConstIterator pos, Ptr<WifiMpdu> item) {
  NS_LOG_FUNCTION(this << *item);
...
void WifiMacQueue::DequeueIfQueued(
    const std::list<Ptr<const WifiMpdu>> &mpdus) {
  NS_LOG_FUNCTION(this);
...
Ptr<const WifiMpdu> WifiMacQueue::Peek() const { return Peek(std::nullopt); }

Ptr<WifiMpdu> WifiMacQueue::Peek(std::optional<uint8_t> linkId) const {
  NS_LOG_FUNCTION(this);
...
Ptr<WifiMpdu> WifiMacQueue::PeekByQueueId(const WifiContainerQueueId &queueId,
                                          Ptr<const WifiMpdu> item) const {
  NS_LOG_FUNCTION(this << item);
  NS_ASSERT(!item || (item->IsQueued() &&
  ...
  auto it = (item ? std::next(GetIt(item))
...
Ptr<WifiMpdu> WifiMacQueue::PeekFirstAvailable(uint8_t linkId,
                                               Ptr<const WifiMpdu> item) const {
  ...
  if (item) {
    queueId = m_scheduler->GetNext(m_ac, linkId,
                                   WifiMacQueueContainer::GetQueueId(item));
  } else {
    queueId = m_scheduler->GetNext(m_ac, linkId);
  }
...
Ptr<WifiMpdu> WifiMacQueue::Remove() { return Remove(Peek()); }

Ptr<WifiMpdu> WifiMacQueue::Remove(Ptr<const WifiMpdu> mpdu) {
...
void WifiMacQueue::Replace(Ptr<const WifiMpdu> currentItem,
...
uint32_t WifiMacQueue::GetNPackets(const WifiContainerQueueId &queueId) const {
  return GetContainer().GetQueue(queueId).size();
}

uint32_t WifiMacQueue::GetNBytes(const WifiContainerQueueId &queueId) const {
  return GetContainer().GetNBytes(queueId);
}

bool WifiMacQueue::DoEnqueue(ConstIterator pos, Ptr<WifiMpdu> item) {
  NS_LOG_FUNCTION(this << *item);
  ...
  if (Queue<WifiMpdu, WifiMacQueueContainer>::DoEnqueue(pos, item, ret)) {
    ...
    ret->ac = m_ac;
    ret->expiryTime =
        item->GetHeader().IsCtl() ? Time::Max() : Simulator::Now() + m_maxDelay;
    WmqIteratorTag tag;
...
void WifiMacQueue::DoDequeue(const std::list<ConstIterator> &iterators) {
  NS_LOG_FUNCTION(this);
...
Ptr<WifiMpdu> WifiMacQueue::DoRemove(ConstIterator pos) {
  NS_LOG_FUNCTION(this);
