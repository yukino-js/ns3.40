...
protected:
  ...
  struct ApLinkEntity : public WifiMac::LinkEntity {
    ...
    std::map<uint16_t, Mac48Address>
        staList; //!< Map of all stations currently associated
                 //!< to the AP with their association ID
    uint16_t numNonHtStations{
        0}; //!< Number of non-HT stations currently associated to the AP
    ...
    bool shortPreambleEnabled{
        false}; //!< Flag whether short preamble is enabled in the BSS
  };
  ...
  std::map<uint16_t, Mac48Address>
      m_aidToMldOrLinkAddress; //!< Maps AIDs to MLD addresses (for MLDs) or
                               //!< link addresses (in case of single link
                               //!< devices)
...
private:
  ...
  Ptr<UniformRandomVariable>
      m_beaconJitter; //!< UniformRandomVariable used to randomize the time of
                      //!< the first beacon
  ...
  TracedCallback<uint16_t /* AID */, Mac48Address>
      m_assocLogger; ///< association logger
  TracedCallback<uint16_t /* AID */, Mac48Address>
      m_deAssocLogger; ///< deassociation logger
};
