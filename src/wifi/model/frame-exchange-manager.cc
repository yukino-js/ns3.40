...
TypeId FrameExchangeManager::GetTypeId() {
...
FrameExchangeManager::FrameExchangeManager()
    : m_navEnd(Seconds(0)), m_linkId(0), m_allowedWidth(0), m_promisc(false),
      m_moreFragments(false) {
  NS_LOG_FUNCTION(this);
}

FrameExchangeManager::~FrameExchangeManager() { NS_LOG_FUNCTION_NOARGS(); }

void FrameExchangeManager::Reset() {
  NS_LOG_FUNCTION(this);
  m_txTimer.Cancel();
...
void FrameExchangeManager::DoDispose() {
  NS_LOG_FUNCTION(this);
  Reset();
...
void FrameExchangeManager::SetProtectionManager(
    Ptr<WifiProtectionManager> protectionManager) {
  NS_LOG_FUNCTION(this << protectionManager);
  m_protectionManager = protectionManager;
}

Ptr<WifiProtectionManager> FrameExchangeManager::GetProtectionManager() const {
  return m_protectionManager;
}

void FrameExchangeManager::SetAckManager(Ptr<WifiAckManager> ackManager) {
  NS_LOG_FUNCTION(this << ackManager);
  m_ackManager = ackManager;
}

Ptr<WifiAckManager> FrameExchangeManager::GetAckManager() const {
  return m_ackManager;
}

void FrameExchangeManager::SetLinkId(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
  m_linkId = linkId;
}

void FrameExchangeManager::SetWifiMac(Ptr<WifiMac> mac) {
  NS_LOG_FUNCTION(this << mac);
  m_mac = mac;
}

void FrameExchangeManager::SetMacTxMiddle(const Ptr<MacTxMiddle> txMiddle) {
  NS_LOG_FUNCTION(this << txMiddle);
  m_txMiddle = txMiddle;
}

void FrameExchangeManager::SetMacRxMiddle(const Ptr<MacRxMiddle> rxMiddle) {
  NS_LOG_FUNCTION(this << rxMiddle);
  m_rxMiddle = rxMiddle;
}

void FrameExchangeManager::SetChannelAccessManager(
...
void FrameExchangeManager::SetWifiPhy(Ptr<WifiPhy> phy) {
  ...
  m_phy->SetReceiveOkCallback(
      MakeCallback(&FrameExchangeManager::Receive, this));
}

void FrameExchangeManager::ResetPhy() {
  NS_LOG_FUNCTION(this);
  if (m_phy) {
    ...
    if (m_phy->GetState()) {
      m_phy->SetReceiveOkCallback(
          MakeNullCallback<void, Ptr<const WifiPsdu>, RxSignalInfo,
                           WifiTxVector, std::vector<bool>>());
    }
    m_phy = nullptr;
  }
}
...
Mac48Address FrameExchangeManager::GetAddress() const { return m_self; }

void FrameExchangeManager::SetBssid(Mac48Address bssid) {
  NS_LOG_FUNCTION(this << bssid);
  m_bssid = bssid;
}

Mac48Address FrameExchangeManager::GetBssid() const { return m_bssid; }
...
void FrameExchangeManager::SetAckedMpduCallback(AckedMpdu callback) {
  NS_LOG_FUNCTION(this << &callback);
  m_ackedMpduCallback = callback;
}

void FrameExchangeManager::SetPromisc() { m_promisc = true; }

bool FrameExchangeManager::IsPromisc() const { return m_promisc; }

const WifiTxTimer &FrameExchangeManager::GetWifiTxTimer() const {
  return m_txTimer;
}

void FrameExchangeManager::NotifyPacketDiscarded(Ptr<const WifiMpdu> mpdu) {
  if (!m_droppedMpduCallback.IsNull()) {
    m_droppedMpduCallback(WIFI_MAC_DROP_REACHED_RETRY_LIMIT, mpdu);
  }
}

void FrameExchangeManager::RxStartIndication(WifiTxVector txVector,
                                             Time psduDuration) {
  NS_LOG_FUNCTION(this << "PSDU reception started for "
                       << psduDuration.As(Time::US)
                       << " (txVector: " << txVector << ")");
...
bool FrameExchangeManager::StartTransmission(Ptr<Txop> dcf,
  ...
  NS_LOG_DEBUG("MPDU payload size=" << mpdu->GetPacketSize() << ", to="
...
void FrameExchangeManager::SendMpduWithProtection(Ptr<WifiMpdu> mpdu,
                                                  WifiTxParameters &txParams) {
...
void FrameExchangeManager::StartProtection(const WifiTxParameters &txParams) {
...
void FrameExchangeManager::ProtectionCompleted() {
  NS_LOG_FUNCTION(this);
...
const std::set<Mac48Address> &FrameExchangeManager::GetProtectedStas() const {
  return m_protectedStas;
}

void FrameExchangeManager::SendMpdu() {
  NS_LOG_FUNCTION(this);

  Time txDuration =
      m_phy->CalculateTxDuration(GetPsduSize(m_mpdu, m_txParams.m_txVector),
  ...
  if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::NONE) {
    ...
    m_mpdu->GetHeader().SetDuration(GetFrameDurationId(
    ...
    m_txTimer.Set(WifiTxTimer::WAIT_NORMAL_ACK, timeout,
                  {m_mpdu->GetHeader().GetAddr1()},
                  &FrameExchangeManager::NormalAckTimeout, this, m_mpdu,
...
void FrameExchangeManager::ForwardMpduDown(Ptr<WifiMpdu> mpdu,
...
void FrameExchangeManager::FinalizeMacHeader(Ptr<const WifiPsdu> psdu) {
...
void FrameExchangeManager::DequeueMpdu(Ptr<const WifiMpdu> mpdu) {
...
uint32_t FrameExchangeManager::GetPsduSize(Ptr<const WifiMpdu> mpdu,
                                           const WifiTxVector &txVector) const {
  return mpdu->GetSize();
}

void FrameExchangeManager::CalculateProtectionTime(
...
void FrameExchangeManager::CalculateAcknowledgmentTime(
  ...
  if (acknowledgment->method == WifiAcknowledgment::NONE) {
    ...
    normalAcknowledgment->acknowledgmentTime =
        m_phy->GetSifs() + m_phy->CalculateTxDuration(
                               GetAckSize(), normalAcknowledgment->ackTxVector,
                               m_phy->GetPhyBand());
  }
}

Time FrameExchangeManager::GetTxDuration(
    uint32_t ppduPayloadSize, Mac48Address receiver,
    const WifiTxParameters &txParams) const {
  return m_phy->CalculateTxDuration(ppduPayloadSize, txParams.m_txVector,
                                    m_phy->GetPhyBand());
}

void FrameExchangeManager::UpdateTxDuration(Mac48Address receiver,
                                            WifiTxParameters &txParams) const {
  txParams.m_txDuration =
      GetTxDuration(txParams.GetSize(receiver), receiver, txParams);
}

Time FrameExchangeManager::GetFrameDurationId(
  ...
  if (header.IsMoreFragments()) {
    ...
    uint32_t nextFragmentOffset =
        (header.GetFragmentNumber() + 1) * payloadSize;
    uint32_t nextFragmentSize =
        std::min(fragmentedPacket->GetSize() - nextFragmentOffset, payloadSize);
    WifiTxVector ackTxVector = GetWifiRemoteStationManager()->GetAckTxVector(
    ...
    durationId +=
        ...
        m_phy->CalculateTxDuration(GetAckSize(), ackTxVector,
                                   m_phy->GetPhyBand()) +
        m_phy->CalculateTxDuration(nextFragmentSize, txParams.m_txVector,
                                   m_phy->GetPhyBand());
  }
  return durationId;
}

Time FrameExchangeManager::GetRtsDurationId(const WifiTxVector &rtsTxVector,
                                            Time txDuration,
                                            Time response) const {
  NS_LOG_FUNCTION(this << rtsTxVector << txDuration << response);

  WifiTxVector ctsTxVector;
  ctsTxVector = GetWifiRemoteStationManager()->GetCtsTxVector(
      m_self, rtsTxVector.GetMode());
...
void FrameExchangeManager::SendRts(const WifiTxParameters &txParams) {
  ...
  NS_ASSERT(!m_txTimer.IsRunning());
  m_txTimer.Set(WifiTxTimer::WAIT_CTS, timeout, {receiver},
                &FrameExchangeManager::CtsTimeout, this, mpdu,
...
void FrameExchangeManager::DoSendCtsAfterRts(const WifiMacHeader &rtsHdr,
                                             WifiTxVector &ctsTxVector,
                                             double rtsSnr) {
...
void FrameExchangeManager::SendCtsAfterRts(const WifiMacHeader &rtsHdr,
                                           WifiMode rtsTxMode, double rtsSnr) {
  NS_LOG_FUNCTION(this << rtsHdr << rtsTxMode << rtsSnr);

  WifiTxVector ctsTxVector = GetWifiRemoteStationManager()->GetCtsTxVector(
      rtsHdr.GetAddr2(), rtsTxMode);
  DoSendCtsAfterRts(rtsHdr, ctsTxVector, rtsSnr);
}

Time FrameExchangeManager::GetCtsToSelfDurationId(
...
void FrameExchangeManager::SendCtsToSelf(const WifiTxParameters &txParams) {
  NS_LOG_FUNCTION(this << &txParams);
  ...
  NS_ASSERT(txParams.m_txDuration != Time::Min());
  cts.SetDuration(GetCtsToSelfDurationId(
      ctsToSelfProtection->ctsTxVector, txParams.m_txDuration,
      txParams.m_acknowledgment->acknowledgmentTime));

  ForwardMpduDown(Create<WifiMpdu>(Create<Packet>(), cts),
                  ctsToSelfProtection->ctsTxVector);

  Time ctsDuration = m_phy->CalculateTxDuration(
      GetCtsSize(), ctsToSelfProtection->ctsTxVector, m_phy->GetPhyBand());
  Simulator::Schedule(ctsDuration + m_phy->GetSifs(),
                      &FrameExchangeManager::ProtectionCompleted, this);
}

void FrameExchangeManager::SendNormalAck(const WifiMacHeader &hdr,
                                         const WifiTxVector &dataTxVector,
                                         double dataSnr) {
  NS_LOG_FUNCTION(this << hdr << dataTxVector << dataSnr);

  WifiTxVector ackTxVector = GetWifiRemoteStationManager()->GetAckTxVector(
      hdr.GetAddr2(), dataTxVector);
  WifiMacHeader ack;
...
Ptr<WifiMpdu> FrameExchangeManager::GetNextFragment() {
  NS_LOG_FUNCTION(this);
  ...
  return Create<WifiMpdu>(m_fragmentedPacket->CreateFragment(startOffset, size),
                          hdr);
}
...
void FrameExchangeManager::TransmissionSucceeded() {
  ...
  if (m_moreFragments) {
    ...
    Simulator::Schedule(m_phy->GetSifs(),
                        &FrameExchangeManager::StartTransmission, this, m_dcf,
                        m_allowedWidth);
    m_moreFragments = false;
  } else {
    NotifyChannelReleased(m_dcf);
...
void FrameExchangeManager::NotifyChannelReleased(Ptr<Txop> txop) {
  NS_LOG_FUNCTION(this << txop);
  txop->NotifyChannelReleased(m_linkId);
  m_protectedStas.clear();
}

void FrameExchangeManager::NormalAckTimeout(Ptr<WifiMpdu> mpdu,
...
void FrameExchangeManager::RetransmitMpduAfterMissedAck(
    Ptr<WifiMpdu> mpdu) const {
  NS_LOG_FUNCTION(this << *mpdu);
}

void FrameExchangeManager::CtsTimeout(Ptr<WifiMpdu> rts,
                                      const WifiTxVector &txVector) {
...
void FrameExchangeManager::DoCtsTimeout(Ptr<WifiPsdu> psdu) {
  NS_LOG_FUNCTION(this << *psdu);
...
void FrameExchangeManager::ReleaseSequenceNumbers(
    Ptr<const WifiPsdu> psdu) const {
  NS_LOG_FUNCTION(this << *psdu);

  NS_ASSERT_MSG(psdu->GetNMpdus() == 1,
...
void FrameExchangeManager::NotifySwitchingStartNow(Time duration) {
...
void FrameExchangeManager::NotifySleepNow() {
  NS_LOG_DEBUG("Device in sleep mode. Cancelling MAC pending events");
  Reset();
}

void FrameExchangeManager::NotifyOffNow() {
  NS_LOG_DEBUG("Device is switched off. Cancelling MAC pending events");
  Reset();
}

void FrameExchangeManager::Receive(Ptr<const WifiPsdu> psdu,
  ...
  if (addr1.IsGroup() || addr1 == m_self) {
    ...
    if (psdu->GetNMpdus() == 1) {
      ...
      ReceiveMpdu(*(psdu->begin()), rxSignalInfo, txVector,
                  perMpduStatus.empty());
    } else {
...
void FrameExchangeManager::PreProcessFrame(Ptr<const WifiPsdu> psdu,
                                           const WifiTxVector &txVector) {
  NS_LOG_FUNCTION(this << psdu << txVector);
}

void FrameExchangeManager::PostProcessFrame(Ptr<const WifiPsdu> psdu,
                                            const WifiTxVector &txVector) {
  NS_LOG_FUNCTION(this << psdu << txVector);
...
void FrameExchangeManager::UpdateNav(Ptr<const WifiPsdu> psdu,
                                     const WifiTxVector &txVector) {
  NS_LOG_FUNCTION(this << psdu << txVector);
  ...
  if (navEnd > m_navEnd) {
    ...
    if (psdu->GetHeader(0).IsRts()) {
      ...
      Time navResetDelay =
          ...
          2 * m_phy->GetSlot();
      m_navResetEvent = Simulator::Schedule(
          navResetDelay, &FrameExchangeManager::NavResetTimeout, this);
    }
  }
...
void FrameExchangeManager::NavResetTimeout() {
  NS_LOG_FUNCTION(this);
  m_navEnd = Simulator::Now();
  m_channelAccessManager->NotifyNavResetNow(Seconds(0));
}

bool FrameExchangeManager::VirtualCsMediumIdle() const {
  return m_navEnd <= Simulator::Now();
}

void FrameExchangeManager::ReceiveMpdu(Ptr<const WifiMpdu> mpdu,
                                       RxSignalInfo rxSignalInfo,
                                       const WifiTxVector &txVector,
                                       bool inAmpdu) {
  ...
  if (hdr.IsCtl()) {
    if (hdr.IsRts()) {
      ...
      if (VirtualCsMediumIdle()) {
        NS_LOG_DEBUG("Received RTS from=" << hdr.GetAddr2()
                                          << ", schedule CTS");
        Simulator::Schedule(m_phy->GetSifs(),
                            &FrameExchangeManager::SendCtsAfterRts, this, hdr,
      ...
      GetWifiRemoteStationManager()->ReportRtsOk(m_mpdu->GetHeader(), rxSnr,
      ...
      SnrTag tag;
      mpdu->GetPacket()->PeekPacketTag(tag);
      ReceivedNormalAck(m_mpdu, m_txParams.m_txVector, txVector, rxSignalInfo,
                        tag.Get());
      m_mpdu = nullptr;
    }
  } else if (hdr.IsMgt()) {
    ...
    if (hdr.GetAddr1() == m_self) {
      ...
      Simulator::Schedule(m_phy->GetSifs(),
                          &FrameExchangeManager::SendNormalAck, this, hdr,
                          txVector, rxSnr);
    }
...
void FrameExchangeManager::ReceivedNormalAck(Ptr<WifiMpdu> mpdu,
                                             const WifiTxVector &txVector,
...
void FrameExchangeManager::EndReceiveAmpdu(
