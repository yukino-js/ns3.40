...
TypeId IdealWifiManager::GetTypeId() {
  static TypeId tid =
...
IdealWifiManager::~IdealWifiManager() { NS_LOG_FUNCTION(this); }

void IdealWifiManager::SetupPhy(const Ptr<WifiPhy> phy) {
  NS_LOG_FUNCTION(this << phy);
  WifiRemoteStationManager::SetupPhy(phy);
}

uint16_t IdealWifiManager::GetChannelWidthForNonHtMode(WifiMode mode) const {
...
void IdealWifiManager::DoInitialize() {
  NS_LOG_FUNCTION(this);
  BuildSnrThresholds();
}

void IdealWifiManager::BuildSnrThresholds() {
  m_thresholds.clear();
  WifiMode mode;
        ...
        } else // VHT or HE
        {
          ...
          for (uint8_t k = 1; k <= GetPhy()->GetMaxSupportedTxSpatialStreams();
            ...
            if (mode.IsAllowed(j, k)) {
              ...
              txVector.SetNss(k);
              txVector.SetMode(mode);
              AddSnrThreshold(txVector,
                              GetPhy()->CalculateSnr(txVector, m_ber));
            } else {
...
double IdealWifiManager::GetSnrThreshold(WifiTxVector txVector) {
...
void IdealWifiManager::AddSnrThreshold(WifiTxVector txVector, double snr) {
  NS_LOG_FUNCTION(this << txVector.GetMode().GetUniqueName()
                       << txVector.GetChannelWidth() << snr);
  m_thresholds.emplace_back(snr, txVector);
}

WifiRemoteStation *IdealWifiManager::DoCreateStation() const {
  NS_LOG_FUNCTION(this);
...
void IdealWifiManager::Reset(WifiRemoteStation *station) const {
...
void IdealWifiManager::DoReportRxOk(WifiRemoteStation *station, double rxSnr,
                                    WifiMode txMode) {
  NS_LOG_FUNCTION(this << station << rxSnr << txMode);
}

void IdealWifiManager::DoReportRtsFailed(WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
}

void IdealWifiManager::DoReportDataFailed(WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
}

void IdealWifiManager::DoReportRtsOk(WifiRemoteStation *st, double ctsSnr,
...
void IdealWifiManager::DoReportDataOk(WifiRemoteStation *st, double ackSnr,
...
void IdealWifiManager::DoReportAmpduTxStatus(
...
void IdealWifiManager::DoReportFinalRtsFailed(WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  Reset(station);
}

void IdealWifiManager::DoReportFinalDataFailed(WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  Reset(station);
}

WifiTxVector IdealWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
  ...
  if ((station->m_lastSnrCached != CACHE_INITIAL_VALUE) &&
    ...
    NS_LOG_DEBUG("Using cached mode = "
                 ...
                 << station->m_lastChannelWidth << " nss " << +selectedNss);
  } else {
    if (GetHtSupported() && GetHtSupported(st)) {
      for (uint8_t i = 0; i < GetNMcsSupported(station); i++) {
        ...
        if (mode.GetModulationClass() == WIFI_MOD_CLASS_HT) {
          ...
          txVector.SetNss(nss);
          if (!txVector.IsValid() ||
              nss > std::min(GetMaxNumberOfTransmitStreams(),
          ...
          uint64_t dataRate = mode.GetDataRate(
              txVector.GetChannelWidth(), txVector.GetGuardInterval(), nss);
          ...
          for (uint8_t nss = 1;
            ...
            if (dataRate > bestRate && threshold < snr) {
              NS_LOG_DEBUG("Candidate mode = "
                           << mode.GetUniqueName() << " data rate " << dataRate
                           << " channel width " << channelWidth << " snr "
                           << snr);
              bestRate = dataRate;
              maxMode = mode;
              selectedNss = nss;
      ...
      for (uint8_t i = 0; i < GetNSupported(station); i++) {
        ...
        double threshold = GetSnrThreshold(txVector);
        uint64_t dataRate =
            mode.GetDataRate(txVector.GetChannelWidth(),
                             txVector.GetGuardInterval(), txVector.GetNss());
        NS_LOG_DEBUG("mode = " << mode.GetUniqueName() << " threshold "
  ...
  WifiTxVector bestTxVector{
      ...
      GetPreambleForTransmission(maxMode.GetModulationClass(),
...
WifiTxVector IdealWifiManager::DoGetRtsTxVector(WifiRemoteStation *st) {
  ...
  return WifiTxVector(maxMode, GetDefaultTxPowerLevel(),
                      GetPreambleForTransmission(maxMode.GetModulationClass(),
                                                 GetShortPreambleEnabled()),
                      800, GetNumberOfAntennas(), nss, 0,
                      GetChannelWidthForNonHtMode(maxMode),
                      GetAggregation(station));
}

double IdealWifiManager::GetLastObservedSnr(IdealWifiRemoteStation *station,
                                            uint16_t channelWidth,
                                            uint8_t nss) const {
  double snr = station->m_lastSnrObserved;
  if (channelWidth != station->m_lastChannelWidthObserved) {
    snr /= (static_cast<double>(channelWidth) /
            station->m_lastChannelWidthObserved);
  }
  if (nss != station->m_lastNssObserved) {
    snr /= (static_cast<double>(nss) / station->m_lastNssObserved);
  }
  NS_LOG_DEBUG("Last observed SNR is "
               << station->m_lastSnrObserved << " for channel width "
               << station->m_lastChannelWidthObserved << " and nss "
               << +station->m_lastNssObserved << "; computed SNR is " << snr
               << " for channel width " << channelWidth << " and nss " << +nss);
  return snr;
}
