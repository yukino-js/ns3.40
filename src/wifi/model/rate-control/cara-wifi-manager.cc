...
TypeId CaraWifiManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::CaraWifiManager")
          ...
          .AddAttribute(
              ...
              MakeUintegerChecker<uint32_t>())
          .AddAttribute("Timeout", "The 'timer' in the CARA algorithm",
                        UintegerValue(15),
...
CaraWifiManager::~CaraWifiManager() { NS_LOG_FUNCTION(this); }

void CaraWifiManager::DoInitialize() {
  NS_LOG_FUNCTION(this);
  if (GetHtSupported()) {
    NS_FATAL_ERROR(
...
WifiRemoteStation *CaraWifiManager::DoCreateStation() const {
  NS_LOG_FUNCTION(this);
...
void CaraWifiManager::DoReportRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

void CaraWifiManager::DoReportDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
...
void CaraWifiManager::DoReportRxOk(WifiRemoteStation *st, double rxSnr,
                                   WifiMode txMode) {
  NS_LOG_FUNCTION(this << st << rxSnr << txMode);
}

void CaraWifiManager::DoReportRtsOk(WifiRemoteStation *st, double ctsSnr,
                                    WifiMode ctsMode, double rtsSnr) {
  NS_LOG_FUNCTION(this << st << ctsSnr << ctsMode << rtsSnr);
}

void CaraWifiManager::DoReportDataOk(WifiRemoteStation *st, double ackSnr,
...
void CaraWifiManager::DoReportFinalRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

void CaraWifiManager::DoReportFinalDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

WifiTxVector CaraWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
...
bool CaraWifiManager::DoNeedRts(WifiRemoteStation *st, uint32_t size,
                                bool normally) {
