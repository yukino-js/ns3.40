...
TypeId ConstantRateWifiManager::GetTypeId() {
  static TypeId tid =
...
ConstantRateWifiManager::ConstantRateWifiManager() { NS_LOG_FUNCTION(this); }

ConstantRateWifiManager::~ConstantRateWifiManager() { NS_LOG_FUNCTION(this); }

WifiRemoteStation *ConstantRateWifiManager::DoCreateStation() const {
...
void ConstantRateWifiManager::DoReportFinalRtsFailed(
    WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
}

void ConstantRateWifiManager::DoReportFinalDataFailed(
    WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
}

WifiTxVector ConstantRateWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
                                                        uint16_t allowedWidth) {
  NS_LOG_FUNCTION(this << st << allowedWidth);
  uint8_t nss = std::min(GetMaxNumberOfTransmitStreams(),
  ...
  return WifiTxVector(
      ...
      GetPreambleForTransmission(m_dataMode.GetModulationClass(),
      ...
      GetPhy()->GetTxBandwidth(m_dataMode,
                               std::min(allowedWidth, GetChannelWidth(st))),
      GetAggregation(st));
}

WifiTxVector ConstantRateWifiManager::DoGetRtsTxVector(WifiRemoteStation *st) {
  ...
  return WifiTxVector(m_ctlMode, GetDefaultTxPowerLevel(),
                      ...
                      ConvertGuardIntervalToNanoSeconds(
                          m_ctlMode, GetShortGuardIntervalSupported(st),
                          NanoSeconds(GetGuardInterval(st))),
                      1, 1, 0,
