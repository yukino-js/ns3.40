...
struct MinstrelHtRateInfo {
  ...
  uint32_t prevNumRateAttempt; //!< Number of transmission attempts with
                               //!< previous rate.
  uint32_t prevNumRateSuccess; //!< Number of successful frames transmitted with
                               //!< previous rate.
...
struct GroupInfo {
  ...
  bool
      m_supported; //!< If the rates of this group are supported by the station.
  uint16_t m_maxTpRate; //!< The max throughput rate of this group in bps.
  uint16_t
      m_maxTpRate2; //!< The second max throughput rate of this group in bps.
...
static const uint8_t MAX_HT_STREAM_GROUPS =
    4; //!< Maximal number of groups per stream in HT (2 possible channel widths
       //!< and 2 possible GI configurations).
...
static const uint8_t MAX_HT_GROUP_RATES =
    8; //!< Number of rates (or MCS) per HT group.
static const uint8_t MAX_VHT_GROUP_RATES =
    10; //!< Number of rates (or MCS) per VHT group.
static const uint8_t MAX_HE_GROUP_RATES =
    12; //!< Number of rates (or MCS) per HE group.
...
private:
  ...
  uint32_t
      m_frameLength; //!< Frame length used to calculate modes TxTime in bytes.
  ...
  Ptr<MinstrelWifiManager>
      m_legacyManager; //!< Pointer to an instance of MinstrelWifiManager.
