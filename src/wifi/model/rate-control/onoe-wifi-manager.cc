...
TypeId OnoeWifiManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::OnoeWifiManager")
          ...
          .AddAttribute(
              "AddCreditThreshold", "Add credit threshold", UintegerValue(10),
...
OnoeWifiManager::~OnoeWifiManager() { NS_LOG_FUNCTION(this); }

void OnoeWifiManager::DoInitialize() {
  NS_LOG_FUNCTION(this);
  if (GetHtSupported()) {
    NS_FATAL_ERROR(
...
WifiRemoteStation *OnoeWifiManager::DoCreateStation() const {
  NS_LOG_FUNCTION(this);
...
void OnoeWifiManager::DoReportRxOk(WifiRemoteStation *station, double rxSnr,
                                   WifiMode txMode) {
  NS_LOG_FUNCTION(this << station << rxSnr << txMode);
}

void OnoeWifiManager::DoReportRtsFailed(WifiRemoteStation *st) {
...
void OnoeWifiManager::UpdateMode(OnoeWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  NS_LOG_DEBUG(this << " ok " << station->m_tx_ok << " err "
                    << station->m_tx_err << " retr " << station->m_tx_retr
  ...
  if (nrate != station->m_txrate) {
    ...
    station->m_tx_ok = station->m_tx_err = station->m_tx_retr =
        station->m_tx_upper = 0;
  } else if (enough) {
...
WifiTxVector OnoeWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
...
WifiTxVector OnoeWifiManager::DoGetRtsTxVector(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<OnoeWifiRemoteStation *>(st);
  uint16_t channelWidth = GetChannelWidth(station);
  if (channelWidth > 20 && channelWidth != 22) {
