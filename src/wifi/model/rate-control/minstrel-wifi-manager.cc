...
TypeId MinstrelWifiManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::MinstrelWifiManager")
          ...
          .AddAttribute("PrintStats", "Print statistics table",
                        BooleanValue(false),
                        MakeBooleanAccessor(&MinstrelWifiManager::m_printStats),
                        MakeBooleanChecker())
          .AddAttribute(
              "PrintSamples", "Print samples table", BooleanValue(false),
...
MinstrelWifiManager::MinstrelWifiManager()
    : WifiRemoteStationManager(), m_currentRate(0) {
  NS_LOG_FUNCTION(this);
  m_uniformRandomVariable = CreateObject<UniformRandomVariable>();
}

MinstrelWifiManager::~MinstrelWifiManager() { NS_LOG_FUNCTION(this); }

void MinstrelWifiManager::SetupPhy(const Ptr<WifiPhy> phy) {
  ...
  for (const auto &mode : phy->GetModeList()) {
    ...
    AddCalcTxTime(
        mode, phy->CalculateTxDuration(m_pktLen, txVector, phy->GetPhyBand()));
  }
  WifiRemoteStationManager::SetupPhy(phy);
}

void MinstrelWifiManager::SetupMac(const Ptr<WifiMac> mac) {
  NS_LOG_FUNCTION(this << mac);
  WifiRemoteStationManager::SetupMac(mac);
}

void MinstrelWifiManager::DoInitialize() {
  NS_LOG_FUNCTION(this);
  if (GetHtSupported()) {
    NS_FATAL_ERROR(
...
int64_t MinstrelWifiManager::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_uniformRandomVariable->SetStream(stream);
  return 1;
}

Time MinstrelWifiManager::GetCalcTxTime(WifiMode mode) const {
...
void MinstrelWifiManager::AddCalcTxTime(WifiMode mode, Time t) {
  NS_LOG_FUNCTION(this << mode << t);
  m_calcTxTime.insert(std::make_pair(mode, t));
}

WifiRemoteStation *MinstrelWifiManager::DoCreateStation() const {
  NS_LOG_FUNCTION(this);
...
void MinstrelWifiManager::CheckInit(MinstrelWifiRemoteStation *station) {
  ...
  if (!station->m_initialized && GetNSupported(station) > 1) {
    ...
    station->m_sampleTable =
        SampleRate(station->m_nModes, std::vector<uint8_t>(m_sampleCol));
  ...
  if (!station->m_isSampling) {
    ...
    // use second best throughput rate
             ...
             (station->m_minstrelTable[station->m_maxTpRate]
                  .adjustedRetryCount +
              station->m_minstrelTable[station->m_maxTpRate2]
                  .adjustedRetryCount)) {
      NS_LOG_DEBUG(
    ...
    // use best probability rate
             ...
             (station->m_minstrelTable[station->m_maxTpRate]
              ...
              station->m_minstrelTable[station->m_maxTpRate2]
                  .adjustedRetryCount +
              station->m_minstrelTable[station->m_maxProbRate]
                  .adjustedRetryCount)) {
...
uint32_t MinstrelWifiManager::CountRetries(MinstrelWifiRemoteStation *station) {
  if (!station->m_isSampling) {
...
uint16_t MinstrelWifiManager::FindRate(MinstrelWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  int delta =
      (station->m_totalPacketsCount * m_lookAroundRate / 100) -
...
void MinstrelWifiManager::UpdateStats(MinstrelWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  NS_LOG_DEBUG("Currently using rate: "
               << station->m_txrate << " ("
  ...
  for (uint8_t i = 0; i < station->m_nModes; i++) {
    ...
    station->m_minstrelTable[i].attemptHist +=
        station->m_minstrelTable[i].numRateAttempt;
    station->m_minstrelTable[i].prevNumRateSuccess =
        station->m_minstrelTable[i].numRateSuccess;
    station->m_minstrelTable[i].prevNumRateAttempt =
        station->m_minstrelTable[i].numRateAttempt;
  ...
  NS_LOG_DEBUG("max throughput="
               << +index_max_tp << "(" << GetSupported(station, index_max_tp)
...
void MinstrelWifiManager::DoReportRxOk(WifiRemoteStation *st, double rxSnr,
                                       WifiMode txMode) {
  NS_LOG_FUNCTION(this << st << rxSnr << txMode);
  NS_LOG_DEBUG("DoReportRxOk m_txrate="
               << static_cast<MinstrelWifiRemoteStation *>(st)->m_txrate);
}

void MinstrelWifiManager::DoReportRtsFailed(WifiRemoteStation *st) {
...
void MinstrelWifiManager::DoReportRtsOk(WifiRemoteStation *st, double ctsSnr,
                                        WifiMode ctsMode, double rtsSnr) {
  NS_LOG_FUNCTION(this << st << ctsSnr << ctsMode << rtsSnr);
}

void MinstrelWifiManager::DoReportFinalRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelWifiRemoteStation *>(st);
  UpdateRetry(station);
}

void MinstrelWifiManager::DoReportDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelWifiRemoteStation *>(st);
...
void MinstrelWifiManager::DoReportDataOk(WifiRemoteStation *st, double ackSnr,
...
void MinstrelWifiManager::DoReportFinalDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelWifiRemoteStation *>(st);
...
void MinstrelWifiManager::UpdatePacketCounters(
    MinstrelWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
void MinstrelWifiManager::UpdateRetry(MinstrelWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
WifiTxVector MinstrelWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
...
WifiTxVector MinstrelWifiManager::DoGetRtsTxVector(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelWifiRemoteStation *>(st);
  return GetRtsTxVector(station);
}

bool MinstrelWifiManager::DoNeedRetransmission(WifiRemoteStation *st,
...
void MinstrelWifiManager::RateInit(MinstrelWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  for (uint8_t i = 0; i < station->m_nModes; i++) {
    NS_LOG_DEBUG("Initializing rate index " << +i << " "
                                            << GetSupported(station, i));
    station->m_minstrelTable[i].numRateAttempt = 0;
    station->m_minstrelTable[i].numRateSuccess = 0;
    ...
    station->m_minstrelTable[i].perfectTxTime =
        GetCalcTxTime(GetSupported(station, i));
    NS_LOG_DEBUG(
    ...
    for (uint32_t retries = 2; retries < 11; retries++) {
      ...
      totalTxTimeWithGivenRetries = CalculateTimeUnicastPacket(
          station->m_minstrelTable[i].perfectTxTime, 0, retries);
      NS_LOG_DEBUG(
...
Time MinstrelWifiManager::CalculateTimeUnicastPacket(Time dataTransmissionTime,
...
void MinstrelWifiManager::InitSampleTable(MinstrelWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
void MinstrelWifiManager::PrintSampleTable(
    MinstrelWifiRemoteStation *station) const {
  uint8_t numSampleRates = station->m_nModes;
...
void MinstrelWifiManager::PrintTable(MinstrelWifiRemoteStation *station) {
  ...
  for (uint8_t i = 0; i < station->m_nModes; i++) {
    ...
    station->m_statsFile << "   " << std::setw(17) << GetSupported(station, i)
                         ...
                         << "    -----    " << std::setw(8) << tmpTh << "    "
                         << std::setw(3) << rate.ewmaProb / 180 << std::setw(3)
                         << "       ---      " << std::setw(3)
                         << rate.prob / 180 << "     " << std::setw(1)
                         << rate.adjustedRetryCount << "   " << std::setw(3)
                         << rate.prevNumRateSuccess << " " << std::setw(3)
                         << rate.prevNumRateAttempt << "   " << std::setw(9)
                         << rate.successHist << "   " << std::setw(9)
                         << rate.attemptHist << "\n";
  }
