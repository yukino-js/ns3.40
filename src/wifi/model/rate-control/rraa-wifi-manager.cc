...
TypeId RraaWifiManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::RraaWifiManager")
          ...
          .AddAttribute(
              ...
              MakeTimeAccessor(&RraaWifiManager::m_timeout), MakeTimeChecker())
...
RraaWifiManager::~RraaWifiManager() { NS_LOG_FUNCTION(this); }

void RraaWifiManager::SetupPhy(const Ptr<WifiPhy> phy) {
...
void RraaWifiManager::SetupMac(const Ptr<WifiMac> mac) {
  NS_LOG_FUNCTION(this);
  WifiRemoteStationManager::SetupMac(mac);
}

void RraaWifiManager::DoInitialize() {
  NS_LOG_FUNCTION(this);
  if (GetHtSupported()) {
    NS_FATAL_ERROR(
...
Time RraaWifiManager::GetCalcTxTime(WifiMode mode) const {
...
void RraaWifiManager::AddCalcTxTime(WifiMode mode, Time t) {
...
WifiRemoteStation *RraaWifiManager::DoCreateStation() const {
...
void RraaWifiManager::CheckInit(RraaWifiRemoteStation *station) {
...
void RraaWifiManager::InitThresholds(RraaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
void RraaWifiManager::ResetCountersBasic(RraaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  station->m_nFailed = 0;
...
uint8_t RraaWifiManager::GetMaxRate(RraaWifiRemoteStation *station) const {
  return station->m_nRate - 1;
}

void RraaWifiManager::DoReportRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

void RraaWifiManager::DoReportDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
...
void RraaWifiManager::DoReportRxOk(WifiRemoteStation *st, double rxSnr,
                                   WifiMode txMode) {
  NS_LOG_FUNCTION(this << st << rxSnr << txMode);
}

void RraaWifiManager::DoReportRtsOk(WifiRemoteStation *st, double ctsSnr,
                                    WifiMode ctsMode, double rtsSnr) {
  NS_LOG_FUNCTION(this << st << ctsSnr << ctsMode << rtsSnr);
}

void RraaWifiManager::DoReportDataOk(WifiRemoteStation *st, double ackSnr,
...
void RraaWifiManager::DoReportFinalRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

void RraaWifiManager::DoReportFinalDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

WifiTxVector RraaWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
...
WifiTxVector RraaWifiManager::DoGetRtsTxVector(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<RraaWifiRemoteStation *>(st);
  uint16_t channelWidth = GetChannelWidth(station);
  if (channelWidth > 20 && channelWidth != 22) {
...
bool RraaWifiManager::DoNeedRts(WifiRemoteStation *st, uint32_t size,
                                bool normally) {
...
void RraaWifiManager::CheckTimeout(RraaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
void RraaWifiManager::RunBasicAlgorithm(RraaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
void RraaWifiManager::ARts(RraaWifiRemoteStation *station) {
