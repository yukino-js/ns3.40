...
TypeId RrpaaWifiManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::RrpaaWifiManager")
          ...
          .AddAttribute("AckFrameLength",
                        ...
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute("Alpha", "Constant for calculating the MTL threshold.",
                        DoubleValue(1.25),
                        MakeDoubleAccessor(&RrpaaWifiManager::m_alpha),
                        MakeDoubleChecker<double>(1))
          .AddAttribute("Beta", "Constant for calculating the ORI threshold.",
                        DoubleValue(2),
                        MakeDoubleAccessor(&RrpaaWifiManager::m_beta),
                        MakeDoubleChecker<double>(1))
          .AddAttribute("Tau", "Constant for calculating the EWND size.",
                        DoubleValue(0.015),
...
RrpaaWifiManager::~RrpaaWifiManager() { NS_LOG_FUNCTION(this); }

int64_t RrpaaWifiManager::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_uniformRandomVariable->SetStream(stream);
  return 1;
}

void RrpaaWifiManager::SetupPhy(const Ptr<WifiPhy> phy) {
  ...
  for (const auto &mode : phy->GetModeList()) {
    ...
    Time ackTxTime =
        phy->CalculateTxDuration(m_ackLength, txVector, phy->GetPhyBand());
    NS_LOG_DEBUG("Calculating TX times: Mode= " << mode
...
void RrpaaWifiManager::SetupMac(const Ptr<WifiMac> mac) {
  NS_LOG_FUNCTION(this << mac);
  WifiRemoteStationManager::SetupMac(mac);
}

void RrpaaWifiManager::DoInitialize() {
  NS_LOG_FUNCTION(this);
  if (GetHtSupported()) {
    NS_FATAL_ERROR(
...
Time RrpaaWifiManager::GetCalcTxTime(WifiMode mode) const {
...
void RrpaaWifiManager::AddCalcTxTime(WifiMode mode, Time t) {
...
RrpaaWifiManager::GetThresholds(RrpaaWifiRemoteStation *station,
  ...
  for (auto i = station->m_thresholds.begin(); i != station->m_thresholds.end();
       i++) {
    if (mode == i->second) {
      return i->first;
    }
...
void RrpaaWifiManager::CheckInit(RrpaaWifiRemoteStation *station) {
  ...
  if (!station->m_initialized) {
    ...
    station->m_pdTable = RrpaaProbabilitiesTable(
        station->m_nRate, std::vector<double>(m_nPowerLevels));
...
void RrpaaWifiManager::InitThresholds(RrpaaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  for (uint8_t i = 0; i < station->m_nRate; i++) {
    ...
    if (i == station->m_nRate - 1) {
      ...
      nextCritical =
          1 - (nextTotalTxTime.GetSeconds() / totalTxTime.GetSeconds());
    ...
    mtl = nextMtl;
    NS_LOG_DEBUG(mode << " " << th.m_ewnd << " " << th.m_mtl << " "
                      << th.m_ori);
  }
}

void RrpaaWifiManager::ResetCountersBasic(RrpaaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  station->m_nFailed = 0;
...
void RrpaaWifiManager::DoReportRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

void RrpaaWifiManager::DoReportDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
...
void RrpaaWifiManager::DoReportRxOk(WifiRemoteStation *st, double rxSnr,
                                    WifiMode txMode) {
  NS_LOG_FUNCTION(this << st << rxSnr << txMode);
}

void RrpaaWifiManager::DoReportRtsOk(WifiRemoteStation *st, double ctsSnr,
                                     WifiMode ctsMode, double rtsSnr) {
  NS_LOG_FUNCTION(this << st << ctsSnr << ctsMode << rtsSnr);
}

void RrpaaWifiManager::DoReportDataOk(WifiRemoteStation *st, double ackSnr,
...
void RrpaaWifiManager::DoReportFinalRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

void RrpaaWifiManager::DoReportFinalDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
}

WifiTxVector RrpaaWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
...
WifiTxVector RrpaaWifiManager::DoGetRtsTxVector(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<RrpaaWifiRemoteStation *>(st);
  uint16_t channelWidth = GetChannelWidth(station);
  if (channelWidth > 20 && channelWidth != 22) {
...
bool RrpaaWifiManager::DoNeedRts(WifiRemoteStation *st, uint32_t size,
                                 bool normally) {
...
void RrpaaWifiManager::CheckTimeout(RrpaaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
void RrpaaWifiManager::RunBasicAlgorithm(RrpaaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  double wploss =
      (static_cast<double>(station->m_counter + station->m_nFailed) /
       thresholds.m_ewnd);
  ...
  if (bploss >= thresholds.m_mtl) {
    ...
    if (station->m_rateIndex < station->m_nRate - 1) {
      ...
      double rand = m_uniformRandomVariable->GetValue(0, 1);
      if (rand <
          station->m_pdTable[station->m_rateIndex][station->m_powerLevel - 1]) {
...
void RrpaaWifiManager::RunAdaptiveRtsAlgorithm(
    RrpaaWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
