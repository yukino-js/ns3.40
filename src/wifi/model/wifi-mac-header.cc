...
void WifiMacHeader::SetDsFrom() { m_ctrlFromDs = 1; }

void WifiMacHeader::SetDsNotFrom() { m_ctrlFromDs = 0; }

void WifiMacHeader::SetDsTo() { m_ctrlToDs = 1; }

void WifiMacHeader::SetDsNotTo() { m_ctrlToDs = 0; }

void WifiMacHeader::SetAddr1(Mac48Address address) { m_addr1 = address; }

void WifiMacHeader::SetAddr2(Mac48Address address) { m_addr2 = address; }

void WifiMacHeader::SetAddr3(Mac48Address address) { m_addr3 = address; }

void WifiMacHeader::SetAddr4(Mac48Address address) { m_addr4 = address; }

void WifiMacHeader::SetType(WifiMacType type, bool resetToDsFromDs) {
  switch (type) {
...
void WifiMacHeader::SetRawDuration(uint16_t duration) {
  NS_ASSERT(duration <= 32768);
  m_duration = duration;
}

void WifiMacHeader::SetDuration(Time duration) {
  auto duration_us = static_cast<int64_t>(
...
void WifiMacHeader::SetId(uint16_t id) { m_duration = id; }

void WifiMacHeader::SetSequenceNumber(uint16_t seq) { m_seqSeq = seq; }

void WifiMacHeader::SetFragmentNumber(uint8_t frag) { m_seqFrag = frag; }

void WifiMacHeader::SetNoMoreFragments() { m_ctrlMoreFrag = 0; }

void WifiMacHeader::SetMoreFragments() { m_ctrlMoreFrag = 1; }

void WifiMacHeader::SetOrder() { m_ctrlOrder = 1; }

void WifiMacHeader::SetNoOrder() { m_ctrlOrder = 0; }

void WifiMacHeader::SetRetry() { m_ctrlRetry = 1; }

void WifiMacHeader::SetNoRetry() { m_ctrlRetry = 0; }

void WifiMacHeader::SetQosTid(uint8_t tid) { m_qosTid = tid; }

void WifiMacHeader::SetPowerManagement() { m_ctrlPowerManagement = 1; }

void WifiMacHeader::SetNoPowerManagement() { m_ctrlPowerManagement = 0; }

void WifiMacHeader::SetQosEosp() { m_qosEosp = 1; }

void WifiMacHeader::SetQosNoEosp() { m_qosEosp = 0; }
...
void WifiMacHeader::SetQosAmsdu() { m_amsduPresent = 1; }

void WifiMacHeader::SetQosNoAmsdu() { m_amsduPresent = 0; }

void WifiMacHeader::SetQosTxopLimit(uint8_t txop) { m_qosStuff = txop; }

void WifiMacHeader::SetQosQueueSize(uint8_t size) {
  m_qosEosp = 1;
  m_qosStuff = size;
}
...
Mac48Address WifiMacHeader::GetAddr4() const { return m_addr4; }

WifiMacType WifiMacHeader::GetType() const {
  switch (m_ctrlType) {
  case TYPE_MGT:
...
bool WifiMacHeader::IsFromDs() const { return m_ctrlFromDs == 1; }

bool WifiMacHeader::IsToDs() const { return m_ctrlToDs == 1; }

bool WifiMacHeader::IsData() const { return (m_ctrlType == TYPE_DATA); }

bool WifiMacHeader::IsQosData() const {
  return (m_ctrlType == TYPE_DATA && (m_ctrlSubtype & 0x08));
}

bool WifiMacHeader::IsCtl() const { return (m_ctrlType == TYPE_CTL); }

bool WifiMacHeader::IsMgt() const { return (m_ctrlType == TYPE_MGT); }

bool WifiMacHeader::IsCfPoll() const {
  switch (GetType()) {
  case WIFI_MAC_DATA_CFPOLL:
  case WIFI_MAC_DATA_CFACK_CFPOLL:
  case WIFI_MAC_DATA_NULL_CFPOLL:
...
bool WifiMacHeader::IsCfEnd() const {
  switch (GetType()) {
  case WIFI_MAC_CTL_END:
  case WIFI_MAC_CTL_END_ACK:
    return true;
...
bool WifiMacHeader::IsCfAck() const {
  switch (GetType()) {
  case WIFI_MAC_DATA_CFACK:
  case WIFI_MAC_DATA_CFACK_CFPOLL:
  case WIFI_MAC_DATA_NULL_CFACK:
...
bool WifiMacHeader::HasData() const {
  switch (GetType()) {
  case WIFI_MAC_DATA:
  case WIFI_MAC_DATA_CFACK:
  case WIFI_MAC_DATA_CFPOLL:
...
bool WifiMacHeader::IsRts() const { return (GetType() == WIFI_MAC_CTL_RTS); }

bool WifiMacHeader::IsCts() const { return (GetType() == WIFI_MAC_CTL_CTS); }

bool WifiMacHeader::IsPsPoll() const {
  return (GetType() == WIFI_MAC_CTL_PSPOLL);
}

bool WifiMacHeader::IsAck() const { return (GetType() == WIFI_MAC_CTL_ACK); }

bool WifiMacHeader::IsAssocReq() const {
  return (GetType() == WIFI_MAC_MGT_ASSOCIATION_REQUEST);
}

bool WifiMacHeader::IsAssocResp() const {
  return (GetType() == WIFI_MAC_MGT_ASSOCIATION_RESPONSE);
}

bool WifiMacHeader::IsReassocReq() const {
  return (GetType() == WIFI_MAC_MGT_REASSOCIATION_REQUEST);
}

bool WifiMacHeader::IsReassocResp() const {
  return (GetType() == WIFI_MAC_MGT_REASSOCIATION_RESPONSE);
}

bool WifiMacHeader::IsProbeReq() const {
  return (GetType() == WIFI_MAC_MGT_PROBE_REQUEST);
}

bool WifiMacHeader::IsProbeResp() const {
  return (GetType() == WIFI_MAC_MGT_PROBE_RESPONSE);
}

bool WifiMacHeader::IsBeacon() const {
  return (GetType() == WIFI_MAC_MGT_BEACON);
}

bool WifiMacHeader::IsDisassociation() const {
  return (GetType() == WIFI_MAC_MGT_DISASSOCIATION);
}

bool WifiMacHeader::IsAuthentication() const {
  return (GetType() == WIFI_MAC_MGT_AUTHENTICATION);
}

bool WifiMacHeader::IsDeauthentication() const {
  return (GetType() == WIFI_MAC_MGT_DEAUTHENTICATION);
}

bool WifiMacHeader::IsAction() const {
  return (GetType() == WIFI_MAC_MGT_ACTION);
}

bool WifiMacHeader::IsActionNoAck() const {
  return (GetType() == WIFI_MAC_MGT_ACTION_NO_ACK);
}

bool WifiMacHeader::IsMultihopAction() const {
  return (GetType() == WIFI_MAC_MGT_MULTIHOP_ACTION);
}

bool WifiMacHeader::IsBlockAckReq() const {
  return (GetType() == WIFI_MAC_CTL_BACKREQ);
}

bool WifiMacHeader::IsBlockAck() const {
  return (GetType() == WIFI_MAC_CTL_BACKRESP);
}

bool WifiMacHeader::IsTrigger() const {
  return (GetType() == WIFI_MAC_CTL_TRIGGER);
}

uint16_t WifiMacHeader::GetRawDuration() const { return m_duration; }

Time WifiMacHeader::GetDuration() const { return MicroSeconds(m_duration); }

uint16_t WifiMacHeader::GetSequenceControl() const {
  return (m_seqSeq << 4) | m_seqFrag;
}

uint16_t WifiMacHeader::GetSequenceNumber() const { return m_seqSeq; }

uint8_t WifiMacHeader::GetFragmentNumber() const { return m_seqFrag; }

bool WifiMacHeader::IsRetry() const { return (m_ctrlRetry == 1); }

bool WifiMacHeader::IsMoreData() const { return (m_ctrlMoreData == 1); }

bool WifiMacHeader::IsMoreFragments() const { return (m_ctrlMoreFrag == 1); }

bool WifiMacHeader::IsPowerManagement() const {
  return (m_ctrlPowerManagement == 1);
}

bool WifiMacHeader::IsQosBlockAck() const {
  NS_ASSERT(IsQosData());
  return (m_qosAckPolicy == 3);
}

bool WifiMacHeader::IsQosNoAck() const {
  NS_ASSERT(IsQosData());
  return (m_qosAckPolicy == 1);
}

bool WifiMacHeader::IsQosAck() const {
  NS_ASSERT(IsQosData());
  return (m_qosAckPolicy == 0);
}

bool WifiMacHeader::IsQosEosp() const {
  NS_ASSERT(IsQosData());
  return (m_qosEosp == 1);
}

WifiMacHeader::QosAckPolicy WifiMacHeader::GetQosAckPolicy() const {
  NS_ASSERT(IsQosData());
  QosAckPolicy policy;
...
bool WifiMacHeader::IsQosAmsdu() const {
  NS_ASSERT(IsQosData());
  return (m_amsduPresent == 1);
}

uint8_t WifiMacHeader::GetQosTid() const {
  NS_ASSERT(IsQosData());
  return m_qosTid;
}

uint8_t WifiMacHeader::GetQosQueueSize() const {
  NS_ASSERT(m_qosEosp == 1);
  return m_qosStuff;
}

uint16_t WifiMacHeader::GetFrameControl() const {
  uint16_t val = 0;
...
uint16_t WifiMacHeader::GetQosControl() const {
  uint16_t val = 0;
  val |= m_qosTid;
  val |= m_qosEosp << 4;
...
void WifiMacHeader::SetFrameControl(uint16_t ctrl) {
...
void WifiMacHeader::SetSequenceControl(uint16_t seq) {
  m_seqFrag = seq & 0x0f;
  m_seqSeq = (seq >> 4) & 0x0fff;
}

void WifiMacHeader::SetQosControl(uint16_t qos) {
  m_qosTid = qos & 0x000f;
...
uint32_t WifiMacHeader::GetSize() const {
  uint32_t size = 0;
  switch (m_ctrlType) {
  case TYPE_MGT:
    size = 2 + 2 + 6 + 6 + 6 + 2;
...
const char *WifiMacHeader::GetTypeString() const {
...
TypeId WifiMacHeader::GetTypeId() {
...
TypeId WifiMacHeader::GetInstanceTypeId() const { return GetTypeId(); }

void WifiMacHeader::PrintFrameControl(std::ostream &os) const {
...
void WifiMacHeader::Print(std::ostream &os) const {
  os << GetTypeString() << " ";
  switch (GetType()) {
  case WIFI_MAC_CTL_PSPOLL:
    os << "Duration/ID=" << std::hex << m_duration << std::dec
       << ", BSSID(RA)=" << m_addr1 << ", TA=" << m_addr2;
    break;
  case WIFI_MAC_CTL_RTS:
  case WIFI_MAC_CTL_TRIGGER:
...
uint32_t WifiMacHeader::GetSerializedSize() const { return GetSize(); }
