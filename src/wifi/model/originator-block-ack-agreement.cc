...
OriginatorBlockAckAgreement::OriginatorBlockAckAgreement(Mac48Address recipient,
                                                         uint8_t tid)
    : BlockAckAgreement(recipient, tid), m_state(PENDING) {}

OriginatorBlockAckAgreement::~OriginatorBlockAckAgreement() {}

void OriginatorBlockAckAgreement::SetState(State state) { m_state = state; }

bool OriginatorBlockAckAgreement::IsPending() const {
  return m_state == PENDING;
}

bool OriginatorBlockAckAgreement::IsEstablished() const {
  return m_state == ESTABLISHED;
}

bool OriginatorBlockAckAgreement::IsRejected() const {
  return m_state == REJECTED;
}

bool OriginatorBlockAckAgreement::IsNoReply() const {
  return m_state == NO_REPLY;
}

bool OriginatorBlockAckAgreement::IsReset() const { return m_state == RESET; }
...
std::size_t OriginatorBlockAckAgreement::GetDistance(uint16_t seqNumber) const {
  return BlockAckAgreement::GetDistance(seqNumber, m_txWindow.GetWinStart());
}

void OriginatorBlockAckAgreement::InitTxWindow() {
...
void OriginatorBlockAckAgreement::NotifyTransmittedMpdu(
...
void OriginatorBlockAckAgreement::NotifyAckedMpdu(Ptr<const WifiMpdu> mpdu) {
  uint16_t mpduSeqNumber = mpdu->GetHeader().GetSequenceNumber();
  uint16_t distance = GetDistance(mpduSeqNumber);
...
void OriginatorBlockAckAgreement::NotifyDiscardedMpdu(
    Ptr<const WifiMpdu> mpdu) {
  uint16_t mpduSeqNumber = mpdu->GetHeader().GetSequenceNumber();
  uint16_t distance = GetDistance(mpduSeqNumber);
