...
HtCapabilities::HtCapabilities()
    : m_ldpc(0), m_supportedChannelWidth(0), m_smPowerSave(0), m_greenField(0),
      m_shortGuardInterval20(0), m_shortGuardInterval40(0), m_txStbc(0),
      m_rxStbc(0), m_htDelayedBlockAck(0), m_maxAmsduLength(0), m_dssMode40(0),
      m_psmpSupport(0), m_fortyMhzIntolerant(0), m_lsigProtectionSupport(0),
      m_maxAmpduLengthExponent(0), m_minMpduStartSpace(0), m_ampduReserved(0),
      m_reservedMcsSet1(0), m_rxHighestSupportedDataRate(0),
      m_reservedMcsSet2(0), m_txMcsSetDefined(0), m_txRxMcsSetUnequal(0),
      m_txMaxNSpatialStreams(0), m_txUnequalModulation(0), m_reservedMcsSet3(0),
      m_pco(0), m_pcoTransitionTime(0), m_reservedExtendedCapabilities(0),
      m_mcsFeedback(0), m_htcSupport(0), m_reverseDirectionResponder(0),
      m_reservedExtendedCapabilities2(0), m_implicitRxBfCapable(0),
      m_rxStaggeredSoundingCapable(0), m_txStaggeredSoundingCapable(0),
      m_rxNdpCapable(0), m_txNdpCapable(0), m_implicitTxBfCapable(0),
      m_calibration(0), m_explicitCsiTxBfCapable(0),
      m_explicitNoncompressedSteeringCapable(0),
      m_explicitCompressedSteeringCapable(0), m_explicitTxBfCsiFeedback(0),
      m_explicitNoncompressedBfFeedbackCapable(0),
      m_explicitCompressedBfFeedbackCapable(0), m_minimalGrouping(0),
      m_csiNBfAntennasSupported(0),
      m_noncompressedSteeringNBfAntennasSupported(0),
      m_compressedSteeringNBfAntennasSupported(0), m_csiMaxNRowsBfSupported(0),
      m_channelEstimationCapability(0), m_reservedTxBf(0),
      m_antennaSelectionCapability(0),
      m_explicitCsiFeedbackBasedTxASelCapable(0),
      m_antennaIndicesFeedbackBasedTxASelCapable(0),
      m_explicitCsiFeedbackCapable(0), m_antennaIndicesFeedbackCapable(0),
      m_rxASelCapable(0), m_txSoundingPpdusCapable(0), m_reservedASel(0) {
  for (uint8_t i = 0; i < MAX_SUPPORTED_MCS; i++) {
    m_rxMcsBitmask[i] = 0;
  }
}

WifiInformationElementId HtCapabilities::ElementId() const {
  return IE_HT_CAPABILITIES;
}

void HtCapabilities::Print(std::ostream &os) const {
  os << "HT Capabilities=" << bool(GetLdpc()) << "|"
...
void HtCapabilities::SetLdpc(uint8_t ldpc) { m_ldpc = ldpc; }
...
uint8_t HtCapabilities::GetLdpc() const { return m_ldpc; }

uint8_t HtCapabilities::GetSupportedChannelWidth() const {
  return m_supportedChannelWidth;
}

uint8_t HtCapabilities::GetShortGuardInterval20() const {
  return m_shortGuardInterval20;
}

uint16_t HtCapabilities::GetMaxAmsduLength() const {
...
uint32_t HtCapabilities::GetMaxAmpduLength() const {
...
uint8_t HtCapabilities::GetRxHighestSupportedAntennas() const {
...
uint16_t HtCapabilities::GetInformationFieldSize() const { return 26; }

uint16_t HtCapabilities::GetHtCapabilitiesInfo() const {
  uint16_t val = 0;
  val |= m_ldpc & 0x01;
...
uint8_t HtCapabilities::GetAmpduParameters() const {
  uint8_t val = 0;
...
uint64_t HtCapabilities::GetSupportedMcsSet1() const {
  uint64_t val = 0;
...
uint64_t HtCapabilities::GetSupportedMcsSet2() const {
  uint64_t val = 0;
...
uint16_t HtCapabilities::GetExtendedHtCapabilities() const {
  uint16_t val = 0;
  val |= m_pco & 0x01;
...
uint32_t HtCapabilities::GetTxBfCapabilities() const {
  uint32_t val = 0;
...
uint8_t HtCapabilities::GetAntennaSelectionCapabilities() const {
  uint8_t val = 0;
...
uint16_t HtCapabilities::DeserializeInformationField(Buffer::Iterator start,
                                                     uint16_t length) {
