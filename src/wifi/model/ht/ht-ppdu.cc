...
void HtPpdu::SetPhyHeaders(const WifiTxVector &txVector, Time ppduDuration,
                           std::size_t psduSize) {
...
void HtPpdu::SetLSigHeader(LSigHeader &lSig, Time ppduDuration) const {
...
void HtPpdu::SetHtSigHeader(HtSigHeader &htSig, const WifiTxVector &txVector,
...
WifiTxVector HtPpdu::DoGetTxVector() const {
...
void HtPpdu::SetTxVectorFromPhyHeaders(WifiTxVector &txVector,
...
Time HtPpdu::GetTxDuration() const {
  const auto &txVector = GetTxVector();
  const auto htLength = m_htSig.GetHtLength();
  NS_ASSERT(m_operatingChannel.IsSet());
  return WifiPhy::CalculateTxDuration(htLength, txVector,
                                      m_operatingChannel.GetPhyBand());
}

Ptr<WifiPpdu> HtPpdu::Copy() const {
...
void HtPpdu::HtSigHeader::SetMcs(uint8_t mcs) {
  NS_ASSERT(mcs <= 31);
  m_mcs = mcs;
}

uint8_t HtPpdu::HtSigHeader::GetMcs() const { return m_mcs; }

void HtPpdu::HtSigHeader::SetChannelWidth(uint16_t channelWidth) {
  m_cbw20_40 = (channelWidth > 20) ? 1 : 0;
}

uint16_t HtPpdu::HtSigHeader::GetChannelWidth() const {
  return m_cbw20_40 ? 40 : 20;
}

void HtPpdu::HtSigHeader::SetHtLength(uint16_t length) { m_htLength = length; }

uint16_t HtPpdu::HtSigHeader::GetHtLength() const { return m_htLength; }

void HtPpdu::HtSigHeader::SetAggregation(bool aggregation) {
  m_aggregation = aggregation ? 1 : 0;
}

bool HtPpdu::HtSigHeader::GetAggregation() const { return m_aggregation; }

void HtPpdu::HtSigHeader::SetShortGuardInterval(bool sgi) {
  m_sgi = sgi ? 1 : 0;
}

bool HtPpdu::HtSigHeader::GetShortGuardInterval() const { return m_sgi; }
