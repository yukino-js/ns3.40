  ...
  static TypeId tid =
      TypeId("ns3::QosTxop")
          ...
          .AddAttribute("UseExplicitBarAfterMissedBlockAck",
                        ...
                        BooleanValue(true),
                        MakeBooleanAccessor(
                            &QosTxop::m_useExplicitBarAfterMissedBlockAck),
                        MakeBooleanChecker())
          .AddAttribute(
          ...
          .AddAttribute(
              ...
              MakeTimeChecker())
          .AddAttribute("BlockAckManager", "The BlockAckManager object.",
                        PointerValue(),
...
QosTxop::QosTxop(AcIndex ac) : Txop(CreateObject<WifiMacQueue>(ac)), m_ac(ac) {
  ...
  m_baManager->SetBlockDestinationCallback(
      Callback<void, Mac48Address, uint8_t>(
          [this](Mac48Address recipient, uint8_t tid) {
            m_mac->GetMacQueueScheduler()->BlockQueues(
                WifiQueueBlockedReason::WAITING_ADDBA_RESP, m_ac,
                {WIFI_QOSDATA_QUEUE}, recipient,
                m_mac->GetLocalAddress(recipient), {tid});
          }));
  m_baManager->SetUnblockDestinationCallback(
      Callback<void, Mac48Address, uint8_t>(
          [this](Mac48Address recipient, uint8_t tid) {
            m_mac->GetMacQueueScheduler()->UnblockQueues(
                WifiQueueBlockedReason::WAITING_ADDBA_RESP, m_ac,
                {WIFI_QOSDATA_QUEUE}, recipient,
                m_mac->GetLocalAddress(recipient), {tid});
          }));
...
QosTxop::~QosTxop() { NS_LOG_FUNCTION(this); }

void QosTxop::DoDispose() {
  NS_LOG_FUNCTION(this);
  if (m_baManager) {
    m_baManager->Dispose();
  }
...
std::unique_ptr<Txop::LinkEntity> QosTxop::CreateLinkEntity() const {
  return std::make_unique<QosLinkEntity>();
}

QosTxop::QosLinkEntity &QosTxop::GetLink(uint8_t linkId) const {
...
void QosTxop::SetDroppedMpduCallback(DroppedMpdu callback) {
  NS_LOG_FUNCTION(this << &callback);
  Txop::SetDroppedMpduCallback(callback);
  m_baManager->SetDroppedOldMpduCallback(
      callback.Bind(WIFI_MAC_DROP_QOS_OLD_PACKET));
}

void QosTxop::SetMuCwMin(uint16_t cwMin, uint8_t linkId) {
  NS_LOG_FUNCTION(this << cwMin << +linkId);
  GetLink(linkId).muCwMin = cwMin;
}

void QosTxop::SetMuCwMax(uint16_t cwMax, uint8_t linkId) {
  NS_LOG_FUNCTION(this << cwMax << +linkId);
  GetLink(linkId).muCwMax = cwMax;
}

void QosTxop::SetMuAifsn(uint8_t aifsn, uint8_t linkId) {
  NS_LOG_FUNCTION(this << +aifsn << +linkId);
  GetLink(linkId).muAifsn = aifsn;
}

void QosTxop::SetMuEdcaTimer(Time timer, uint8_t linkId) {
  NS_LOG_FUNCTION(this << timer << +linkId);
  GetLink(linkId).muEdcaTimer = timer;
}

void QosTxop::StartMuEdcaTimerNow(uint8_t linkId) {
  ...
  if (EdcaDisabled(linkId)) {
    ...
    m_mac->GetChannelAccessManager(linkId)->DisableEdcaFor(this,
                                                           link.muEdcaTimer);
  }
}

bool QosTxop::MuEdcaTimerRunning(uint8_t linkId) const {
  auto &link = GetLink(linkId);
...
bool QosTxop::EdcaDisabled(uint8_t linkId) const {
  return (MuEdcaTimerRunning(linkId) && GetLink(linkId).muAifsn == 0);
}

uint32_t QosTxop::GetMinCw(uint8_t linkId) const {
...
uint32_t QosTxop::GetMaxCw(uint8_t linkId) const {
  if (!MuEdcaTimerRunning(linkId)) {
...
uint8_t QosTxop::GetAifsn(uint8_t linkId) const {
  if (!MuEdcaTimerRunning(linkId)) {
...
Ptr<BlockAckManager> QosTxop::GetBaManager() { return m_baManager; }

uint16_t QosTxop::GetBaBufferSize(Mac48Address address, uint8_t tid) const {
  return m_baManager->GetRecipientBufferSize(address, tid);
}

uint16_t QosTxop::GetBaStartingSequence(Mac48Address address,
                                        uint8_t tid) const {
...
bool QosTxop::UseExplicitBarAfterMissedBlockAck() const {
...
uint16_t QosTxop::GetNextSequenceNumberFor(const WifiMacHeader *hdr) {
  return m_txMiddle->GetNextSequenceNumberFor(hdr);
}

uint16_t QosTxop::PeekNextSequenceNumberFor(const WifiMacHeader *hdr) {
  return m_txMiddle->PeekNextSequenceNumberFor(hdr);
}

bool QosTxop::IsQosOldPacket(Ptr<const WifiMpdu> mpdu) {
...
Ptr<WifiMpdu> QosTxop::PeekNextMpdu(uint8_t linkId, uint8_t tid,
  ...
  auto peek = [this, &linkId, &tid, &recipient, &mpdu]() -> Ptr<WifiMpdu> {
    ...
    WifiContainerQueueId queueId(WIFI_QOSDATA_QUEUE, WIFI_UNICAST, recipient,
                                 tid);
    if (auto mask = m_mac->GetMacQueueScheduler()->GetQueueLinkMask(
            m_ac, queueId, linkId);
        !mask || mask->none()) {
...
Ptr<WifiMpdu> QosTxop::GetNextMpdu(uint8_t linkId, Ptr<WifiMpdu> peekedItem,
  ...
  auto qosFem = StaticCast<QosFrameExchangeManager>(
  ...
  if (peekedItem->GetHeader().IsQosData()) {
    ...
    if (m_mac->GetHtSupported() && !recipient.IsBroadcast() &&
      ...
      mpdu = htFem->GetMsduAggregator()->GetNextAmsdu(peekedItem, txParams,
                                                      availableTime);
    }
...
void QosTxop::AssignSequenceNumber(Ptr<WifiMpdu> mpdu) const {
  NS_LOG_FUNCTION(this << *mpdu);
...
void QosTxop::NotifyChannelAccessed(uint8_t linkId, Time txopDuration) {
...
bool QosTxop::IsTxopStarted(uint8_t linkId) const {
  auto &link = GetLink(linkId);
  NS_LOG_FUNCTION(this << !link.startTxop.IsZero());
  return (!link.startTxop.IsZero());
}

void QosTxop::NotifyChannelReleased(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
  auto &link = GetLink(linkId);

  if (link.startTxop.IsStrictlyPositive()) {
    NS_LOG_DEBUG("Terminating TXOP. Duration = " << Simulator::Now() -
                                                        link.startTxop);
...
Time QosTxop::GetRemainingTxop(uint8_t linkId) const {
  auto &link = GetLink(linkId);
...
void QosTxop::GotAddBaResponse(const MgtAddBaResponseHeader &respHdr,
                               Mac48Address recipient) {
...
void QosTxop::GotDelBaFrame(const MgtDelBaHeader *delBaHdr,
                            Mac48Address recipient) {
...
void QosTxop::NotifyOriginatorAgreementNoReply(const Mac48Address &recipient,
                                               uint8_t tid) {
  NS_LOG_FUNCTION(this << recipient << tid);
...
void QosTxop::SetBlockAckThreshold(uint8_t threshold) {
  NS_LOG_FUNCTION(this << +threshold);
  m_blockAckThreshold = threshold;
  m_baManager->SetBlockAckThreshold(threshold);
}

void QosTxop::SetBlockAckInactivityTimeout(uint16_t timeout) {
  NS_LOG_FUNCTION(this << timeout);
  m_blockAckInactivityTimeout = timeout;
}

uint8_t QosTxop::GetBlockAckThreshold() const {
  NS_LOG_FUNCTION(this);
  return m_blockAckThreshold;
}

uint16_t QosTxop::GetBlockAckInactivityTimeout() const {
...
void QosTxop::SetAddBaResponseTimeout(Time addBaResponseTimeout) {
  NS_LOG_FUNCTION(this << addBaResponseTimeout);
  m_addBaResponseTimeout = addBaResponseTimeout;
}

Time QosTxop::GetAddBaResponseTimeout() const { return m_addBaResponseTimeout; }

void QosTxop::SetFailedAddBaTimeout(Time failedAddBaTimeout) {
  NS_LOG_FUNCTION(this << failedAddBaTimeout);
  m_failedAddBaTimeout = failedAddBaTimeout;
}

Time QosTxop::GetFailedAddBaTimeout() const { return m_failedAddBaTimeout; }

bool QosTxop::IsQosTxop() const { return true; }

AcIndex QosTxop::GetAccessCategory() const { return m_ac; }
