...
TypeId YansWifiChannel::GetTypeId() {
  static TypeId tid =
...
YansWifiChannel::YansWifiChannel() { NS_LOG_FUNCTION(this); }
...
void YansWifiChannel::SetPropagationLossModel(
...
void YansWifiChannel::Send(Ptr<YansWifiPhy> sender, Ptr<const WifiPpdu> ppdu,
  ...
  for (auto i = m_phyList.begin(); i != m_phyList.end(); i++) {
    if (sender != (*i)) {
      ...
      Simulator::ScheduleWithContext(dstNode, delay, &YansWifiChannel::Receive,
                                     (*i), ppdu, rxPowerDbm);
    }
  }
}
...
std::size_t YansWifiChannel::GetNDevices() const { return m_phyList.size(); }

Ptr<NetDevice> YansWifiChannel::GetDevice(std::size_t i) const {
