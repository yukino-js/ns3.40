...
WifiMac::~WifiMac() { NS_LOG_FUNCTION(this); }

TypeId WifiMac::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WifiMac")
          .SetParent<Object>()
          .SetGroupName("Wifi")
          ...
          .AddTraceSource("TxOkHeader",
                          ...
                          TypeId::OBSOLETE, "Use the AckedMpdu trace instead.")
          .AddTraceSource(
          ...
          .AddTraceSource("PsduMapResponseTimeout",
                          ...
                          MakeTraceSourceAccessor(
                              &WifiMac::m_psduMapResponseTimeoutCallback),
                          "ns3::WifiMac::PsduMapResponseTimeoutCallback");
  return tid;
}

void WifiMac::DoInitialize() {
  NS_LOG_FUNCTION(this);
...
void WifiMac::SetTypeOfStation(TypeOfStation type) {
  NS_LOG_FUNCTION(this << type);
  m_typeOfStation = type;
}

TypeOfStation WifiMac::GetTypeOfStation() const { return m_typeOfStation; }

void WifiMac::SetDevice(const Ptr<WifiNetDevice> device) { m_device = device; }

Ptr<WifiNetDevice> WifiMac::GetDevice() const { return m_device; }

void WifiMac::SetAddress(Mac48Address address) {
  NS_LOG_FUNCTION(this << address);
  m_address = address;
}

Mac48Address WifiMac::GetAddress() const { return m_address; }

void WifiMac::SetSsid(Ssid ssid) {
  NS_LOG_FUNCTION(this << ssid);
  m_ssid = ssid;
}

Ssid WifiMac::GetSsid() const { return m_ssid; }

void WifiMac::SetBssid(Mac48Address bssid, uint8_t linkId) {
  NS_LOG_FUNCTION(this << bssid << +linkId);
  GetLink(linkId).feManager->SetBssid(bssid);
}

Mac48Address WifiMac::GetBssid(uint8_t linkId) const {
  return GetLink(linkId).feManager->GetBssid();
}

void WifiMac::SetPromisc() {
...
Ptr<QosTxop> WifiMac::GetQosTxop(uint8_t tid) const {
  return GetQosTxop(QosUtilsMapTidToAc(tid));
}

Ptr<QosTxop> WifiMac::GetVOQueue() const {
  return (m_qosSupported ? GetQosTxop(AC_VO) : nullptr);
}

Ptr<QosTxop> WifiMac::GetVIQueue() const {
  return (m_qosSupported ? GetQosTxop(AC_VI) : nullptr);
}

Ptr<QosTxop> WifiMac::GetBEQueue() const {
  return (m_qosSupported ? GetQosTxop(AC_BE) : nullptr);
}

Ptr<QosTxop> WifiMac::GetBKQueue() const {
  return (m_qosSupported ? GetQosTxop(AC_BK) : nullptr);
}

Ptr<WifiMacQueue> WifiMac::GetTxopQueue(AcIndex ac) const {
  Ptr<Txop> txop =
      (ac == AC_BE_NQOS ? m_txop : StaticCast<Txop>(GetQosTxop(ac)));
  return (txop ? txop->GetWifiMacQueue() : nullptr);
}

bool WifiMac::HasFramesToTransmit(uint8_t linkId) {
...
void WifiMac::SetMacQueueScheduler(Ptr<WifiMacQueueScheduler> scheduler) {
  m_scheduler = scheduler;
  m_scheduler->SetWifiMac(this);
}

Ptr<WifiMacQueueScheduler> WifiMac::GetMacQueueScheduler() const {
  return m_scheduler;
}
...
void WifiMac::NotifyTx(Ptr<const Packet> packet) { m_macTxTrace(packet); }

void WifiMac::NotifyTxDrop(Ptr<const Packet> packet) {
  m_macTxDropTrace(packet);
}

void WifiMac::NotifyRx(Ptr<const Packet> packet) { m_macRxTrace(packet); }

void WifiMac::NotifyPromiscRx(Ptr<const Packet> packet) {
  m_macPromiscRxTrace(packet);
}

void WifiMac::NotifyRxDrop(Ptr<const Packet> packet) {
...
void WifiMac::SetupEdcaQueue(AcIndex ac) {
  ...
  edca->SetDroppedMpduCallback(MakeCallback(
      &DroppedMpduTracedCallback::operator(), &m_droppedMpduCallback));

  m_edca.insert(std::make_pair(ac, edca));
}

void WifiMac::ConfigureContentionWindow(uint32_t cwMin, uint32_t cwMax) {
...
void WifiMac::ConfigureDcf(Ptr<Txop> dcf, uint32_t cwmin, uint32_t cwmax,
...
void WifiMac::ConfigureStandard(WifiStandard standard) {
...
void WifiMac::ConfigurePhyDependentParameters(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
...
WifiMac::SetupFrameExchangeManager(WifiStandard standard) {
  ...
  feManager->SetDroppedMpduCallback(MakeCallback(
      &DroppedMpduTracedCallback::operator(), &m_droppedMpduCallback));
...
std::unique_ptr<WifiMac::LinkEntity> WifiMac::CreateLinkEntity() const {
...
uint8_t WifiMac::GetNLinks() const { return m_links.size(); }

const std::set<uint8_t> &WifiMac::GetLinkIds() const { return m_linkIds; }

void WifiMac::UpdateLinkId(uint8_t id) {
...
std::optional<uint8_t> WifiMac::GetLinkForPhy(Ptr<const WifiPhy> phy) const {
  for (const auto &[id, link] : m_links) {
    if (link->phy == phy) {
      return id;
    }
...
std::optional<uint8_t> WifiMac::GetLinkForPhy(std::size_t phyId) const {
  NS_ABORT_UNLESS(phyId < m_device->GetNPhys());
  auto phy = m_device->GetPhy(phyId);
  return GetLinkForPhy(phy);
}

void WifiMac::SwapLinks(std::map<uint8_t, uint8_t> links) {
  NS_LOG_FUNCTION(this);
...
void WifiMac::UpdateTidToLinkMapping(const Mac48Address &mldAddr,
                                     WifiDirection dir,
  ...
  auto &mappings = (dir == WifiDirection::DOWNLINK ? m_dlTidLinkMappings
                                                   : m_ulTidLinkMappings);
...
WifiMac::GetTidToLinkMapping(Mac48Address mldAddr, WifiDirection dir) const {
  ...
  const auto &mappings = (dir == WifiDirection::DOWNLINK ? m_dlTidLinkMappings
                                                         : m_ulTidLinkMappings);
...
bool WifiMac::TidMappedOnLink(Mac48Address mldAddr, WifiDirection dir,
                              uint8_t tid, uint8_t linkId) const {
  NS_ABORT_MSG_IF(dir == WifiDirection::BOTH_DIRECTIONS,
                  "Cannot request TID-to-Link mapping for both directions");
  ...
  return std::find(linkSetIt->second.cbegin(), linkSetIt->second.cend(),
                   linkId) != linkSetIt->second.cend();
}

void WifiMac::SetWifiPhys(const std::vector<Ptr<WifiPhy>> &phys) {
  NS_LOG_FUNCTION(this);
  ResetWifiPhys();

  NS_ABORT_MSG_UNLESS(
      m_links.empty() || m_links.size() == phys.size(),
      "If links have been already created, the number of provided "
      "PHY objects ("
          << phys.size()
...
void WifiMac::SetQosSupported(bool enable) {
...
bool WifiMac::GetQosSupported() const { return m_qosSupported; }

bool WifiMac::GetErpSupported(uint8_t linkId) const {
  return GetLink(linkId).erpSupported;
}

void WifiMac::SetErpSupported(bool enable, uint8_t linkId) {
...
void WifiMac::SetDsssSupported(bool enable, uint8_t linkId) {
  NS_LOG_FUNCTION(this << enable << +linkId);
  GetLink(linkId).dsssSupported = enable;
}

bool WifiMac::GetDsssSupported(uint8_t linkId) const {
  return GetLink(linkId).dsssSupported;
}

void WifiMac::SetCtsToSelfSupported(bool enable) {
  NS_LOG_FUNCTION(this);
  m_ctsToSelfSupported = enable;
}

void WifiMac::SetShortSlotTimeSupported(bool enable) {
  NS_LOG_FUNCTION(this << enable);
  m_shortSlotTimeSupported = enable;
}

bool WifiMac::GetShortSlotTimeSupported() const {
  return m_shortSlotTimeSupported;
}

bool WifiMac::SupportsSendFrom() const { return false; }

void WifiMac::SetForwardUpCallback(ForwardUpCallback upCallback) {
  NS_LOG_FUNCTION(this);
  m_forwardUp = upCallback;
}

void WifiMac::SetLinkUpCallback(Callback<void> linkUp) {
  NS_LOG_FUNCTION(this);
  m_linkUp = linkUp;
}

void WifiMac::SetLinkDownCallback(Callback<void> linkDown) {
  NS_LOG_FUNCTION(this);
  m_linkDown = linkDown;
}

void WifiMac::ApplyTidLinkMapping(const Mac48Address &mldAddr,
                                  WifiDirection dir) {
  NS_LOG_FUNCTION(this << mldAddr);
  ...
  for (const auto &[tid, linkSet] : linkMapping) {
    ...
    if (!notMappedLinks.empty()) {
      m_scheduler->BlockQueues(WifiQueueBlockedReason::TID_NOT_MAPPED,
                               QosUtilsMapTidToAc(tid), {WIFI_QOSDATA_QUEUE},
                               mldAddr, GetAddress(), {tid}, notMappedLinks);
    }
  }
}

void WifiMac::BlockUnicastTxOnLinks(WifiQueueBlockedReason reason,
  ...
  for (const auto linkId : linkIds) {
    ...
    if (link.stationManager->GetMldAddress(address) == address &&
        linkAddr == address) {
      NS_LOG_DEBUG("Link " << +linkId
...
void WifiMac::UnblockUnicastTxOnLinks(WifiQueueBlockedReason reason,
                                      const Mac48Address &address,
                                      const std::set<uint8_t> &linkIds) {
  NS_LOG_FUNCTION(this << reason << address);
  NS_ASSERT(m_scheduler);
...
void WifiMac::ForwardUp(Ptr<const Packet> packet, Mac48Address from,
                        Mac48Address to) {
  NS_LOG_FUNCTION(this << packet << from << to);
  m_forwardUp(packet, from, to);
}

void WifiMac::Receive(Ptr<const WifiMpdu> mpdu, uint8_t linkId) {
  NS_LOG_FUNCTION(this << *mpdu << linkId);

  const WifiMacHeader *hdr = &mpdu->GetOriginal()->GetHeader();
  Mac48Address to = hdr->GetAddr1();
  Mac48Address from = hdr->GetAddr2();
  auto myAddr = hdr->IsData()
...
void WifiMac::DeaggregateAmsduAndForward(Ptr<const WifiMpdu> mpdu) {
...
Mac48Address WifiMac::DoGetLocalAddress(const Mac48Address &remoteAddr
                                        [[maybe_unused]]) const {
  return m_address;
}
...
WifiMac::GetBaAgreementEstablishedAsOriginator(Mac48Address recipient,
  ...
  auto agreement =
      GetQosTxop(tid)->GetBaManager()->GetAgreementAsOriginator(recipient, tid);
...
BlockAckType WifiMac::GetBaTypeAsOriginator(const Mac48Address &recipient,
                                            uint8_t tid) const {
  auto agreement = GetBaAgreementEstablishedAsOriginator(recipient, tid);
  NS_ABORT_MSG_IF(!agreement, "No existing Block Ack agreement with "
                                  << recipient << " TID: " << +tid);
  return agreement->get().GetBlockAckType();
}

BlockAckReqType WifiMac::GetBarTypeAsOriginator(const Mac48Address &recipient,
                                                uint8_t tid) const {
  auto agreement = GetBaAgreementEstablishedAsOriginator(recipient, tid);
  NS_ABORT_MSG_IF(!agreement, "No existing Block Ack agreement with "
                                  << recipient << " TID: " << +tid);
...
BlockAckType WifiMac::GetBaTypeAsRecipient(Mac48Address originator,
                                           uint8_t tid) const {
...
BlockAckReqType WifiMac::GetBarTypeAsRecipient(Mac48Address originator,
                                               uint8_t tid) const {
  auto agreement = GetBaAgreementEstablishedAsRecipient(originator, tid);
  NS_ABORT_MSG_IF(!agreement, "No existing Block Ack agreement with "
                                  << originator << " TID: " << +tid);
...
Ptr<HtConfiguration> WifiMac::GetHtConfiguration() const {
  return GetDevice()->GetHtConfiguration();
}

Ptr<VhtConfiguration> WifiMac::GetVhtConfiguration() const {
  return GetDevice()->GetVhtConfiguration();
}

Ptr<HeConfiguration> WifiMac::GetHeConfiguration() const {
  return GetDevice()->GetHeConfiguration();
}

Ptr<EhtConfiguration> WifiMac::GetEhtConfiguration() const {
  return GetDevice()->GetEhtConfiguration();
}

bool WifiMac::GetHtSupported() const {
  return bool(GetDevice()->GetHtConfiguration());
}

bool WifiMac::GetVhtSupported(uint8_t linkId) const {
  return (GetDevice()->GetVhtConfiguration() &&
          GetWifiPhy(linkId)->GetPhyBand() != WIFI_PHY_BAND_2_4GHZ);
}

bool WifiMac::GetHeSupported() const {
  return bool(GetDevice()->GetHeConfiguration());
}

bool WifiMac::GetEhtSupported() const {
  return bool(GetDevice()->GetEhtConfiguration());
}

bool WifiMac::GetHtSupported(const Mac48Address &address) const {
  for (const auto &[id, link] : m_links) {
...
bool WifiMac::GetVhtSupported(const Mac48Address &address) const {
  for (const auto &[id, link] : m_links) {
...
bool WifiMac::GetHeSupported(const Mac48Address &address) const {
  for (const auto &[id, link] : m_links) {
...
bool WifiMac::GetEhtSupported(const Mac48Address &address) const {
  for (const auto &[id, link] : m_links) {
...
void WifiMac::SetVoBlockAckThreshold(uint8_t threshold) {
...
void WifiMac::SetViBlockAckThreshold(uint8_t threshold) {
  NS_LOG_FUNCTION(this << +threshold);
  if (m_qosSupported) {
...
void WifiMac::SetBeBlockAckThreshold(uint8_t threshold) {
  NS_LOG_FUNCTION(this << +threshold);
  if (m_qosSupported) {
...
void WifiMac::SetBkBlockAckThreshold(uint8_t threshold) {
  NS_LOG_FUNCTION(this << +threshold);
  if (m_qosSupported) {
...
void WifiMac::SetVoBlockAckInactivityTimeout(uint16_t timeout) {
...
void WifiMac::SetViBlockAckInactivityTimeout(uint16_t timeout) {
  NS_LOG_FUNCTION(this << timeout);
  if (m_qosSupported) {
...
void WifiMac::SetBeBlockAckInactivityTimeout(uint16_t timeout) {
  NS_LOG_FUNCTION(this << timeout);
  if (m_qosSupported) {
...
void WifiMac::SetBkBlockAckInactivityTimeout(uint16_t timeout) {
  NS_LOG_FUNCTION(this << timeout);
  if (m_qosSupported) {
...
ExtendedCapabilities WifiMac::GetExtendedCapabilities() const {
  NS_LOG_FUNCTION(this);
...
HtCapabilities WifiMac::GetHtCapabilities(uint8_t linkId) const {
  NS_LOG_FUNCTION(this << +linkId);
  ...
  capabilities.SetSupportedChannelWidth(
      htConfiguration->Get40MHzOperationSupported() ? 1 : 0);
  ...
  for (const auto &mcs : phy->GetMcsList(WIFI_MOD_CLASS_HT)) {
    ...
    uint64_t dataRate =
        mcs.GetDataRate(phy->GetChannelWidth(), sgiSupported ? 400 : 800, nss);
...
VhtCapabilities WifiMac::GetVhtCapabilities(uint8_t linkId) const {
  NS_LOG_FUNCTION(this << +linkId);
  ...
  capabilities.SetShortGuardIntervalFor80Mhz((phy->GetChannelWidth() == 80) &&
                                             sgiSupported);
  capabilities.SetShortGuardIntervalFor160Mhz((phy->GetChannelWidth() == 160) &&
                                              sgiSupported);
  uint8_t maxMcs = 0;
...
HeCapabilities WifiMac::GetHeCapabilities(uint8_t linkId) const {
  NS_LOG_FUNCTION(this << +linkId);
  ...
  if (((phy->GetChannelWidth() >= 80) || GetEhtSupported()) &&
      ((phy->GetPhyBand() == WIFI_PHY_BAND_5GHZ) ||
...
EhtCapabilities WifiMac::GetEhtCapabilities(uint8_t linkId) const {
  NS_LOG_FUNCTION(this << +linkId);
  ...
  capabilities.m_phyCapabilities
      .supportRx1024And4096QamForRuSmallerThan242Tones = support4096Qam ? 1 : 0;
...
uint32_t WifiMac::GetMaxAmpduSize(AcIndex ac) const {
  uint32_t maxSize = 0;
  switch (ac) {
  case AC_BE:
...
uint16_t WifiMac::GetMaxAmsduSize(AcIndex ac) const {
  uint16_t maxSize = 0;
  switch (ac) {
  case AC_BE:
