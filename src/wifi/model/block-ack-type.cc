...
BlockAckType::BlockAckType(Variant v) : m_variant(v) {
  switch (m_variant) {
  case BASIC:
...
BlockAckType::BlockAckType() : BlockAckType(BASIC) {}

BlockAckType::BlockAckType(Variant v, std::vector<uint8_t> l)
    : m_variant(v), m_bitmapLen(l) {}

BlockAckReqType::BlockAckReqType(Variant v) : m_variant(v) {
  switch (m_variant) {
  case BASIC:
  case COMPRESSED:
  case EXTENDED_COMPRESSED:
...
BlockAckReqType::BlockAckReqType() : BlockAckReqType(BASIC) {}

BlockAckReqType::BlockAckReqType(Variant v, uint8_t nSeqControls)
    : m_variant(v), m_nSeqControls(nSeqControls) {}
