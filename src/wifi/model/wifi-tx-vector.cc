...
WifiTxVector::WifiTxVector()
    : m_txPowerLevel(1), m_preamble(WIFI_PREAMBLE_LONG), m_channelWidth(20),
      m_guardInterval(800), m_nTx(1), m_nss(1), m_ness(0), m_aggregation(false),
...
WifiTxVector::WifiTxVector(WifiMode mode, uint8_t powerLevel,
                           ...
                           bool ldpc, uint8_t bssColor, uint16_t length,
                           bool triggerResponding)
    : m_mode(mode), m_txPowerLevel(powerLevel), m_preamble(preamble),
      m_channelWidth(channelWidth), m_guardInterval(guardInterval), m_nTx(nTx),
      m_nss(nss), m_ness(ness), m_aggregation(aggregation), m_stbc(stbc),
...
WifiTxVector::WifiTxVector(const WifiTxVector &txVector)
    : m_mode(txVector.m_mode), m_txPowerLevel(txVector.m_txPowerLevel),
      m_preamble(txVector.m_preamble), m_channelWidth(txVector.m_channelWidth),
      m_guardInterval(txVector.m_guardInterval), m_nTx(txVector.m_nTx),
      m_nss(txVector.m_nss), m_ness(txVector.m_ness),
      m_aggregation(txVector.m_aggregation), m_stbc(txVector.m_stbc),
      m_ldpc(txVector.m_ldpc), m_bssColor(txVector.m_bssColor),
      ...
      m_sigBMcs(txVector.m_sigBMcs), m_ruAllocation(txVector.m_ruAllocation),
...
WifiTxVector::~WifiTxVector() { m_muUserInfos.clear(); }

bool WifiTxVector::GetModeInitialized() const { return m_modeInitialized; }

WifiMode WifiTxVector::GetMode(uint16_t staId) const {
...
WifiModulationClass WifiTxVector::GetModulationClass() const {
  NS_ABORT_MSG_IF(!m_modeInitialized,
                  "WifiTxVector mode must be set before using");
...
uint8_t WifiTxVector::GetTxPowerLevel() const { return m_txPowerLevel; }

WifiPreamble WifiTxVector::GetPreambleType() const { return m_preamble; }

uint16_t WifiTxVector::GetChannelWidth() const { return m_channelWidth; }

uint16_t WifiTxVector::GetGuardInterval() const { return m_guardInterval; }

uint8_t WifiTxVector::GetNTx() const { return m_nTx; }

uint8_t WifiTxVector::GetNss(uint16_t staId) const {
  if (IsMu()) {
    NS_ABORT_MSG_IF(staId > 2048,
                    "STA-ID should be correctly set for MU (" << staId << ")");
...
uint8_t WifiTxVector::GetNssTotal() const {
  ...
  if (IsMu()) {
    nss = std::accumulate(m_muUserInfos.cbegin(), m_muUserInfos.cend(), 0,
                          [](uint8_t prevNss, const auto &info) {
                            return prevNss + info.second.nss;
                          });
  } else {
    nss = m_nss;
  }
...
uint8_t WifiTxVector::GetNess() const { return m_ness; }

bool WifiTxVector::IsAggregation() const { return m_aggregation; }

bool WifiTxVector::IsStbc() const { return m_stbc; }

bool WifiTxVector::IsLdpc() const { return m_ldpc; }

bool WifiTxVector::IsNonHtDuplicate() const {
  return ((m_channelWidth >= 40) && !IsMu() &&
          (GetModulationClass() < WIFI_MOD_CLASS_HT));
}

void WifiTxVector::SetMode(WifiMode mode) {
  m_mode = mode;
  m_modeInitialized = true;
}

void WifiTxVector::SetMode(WifiMode mode, uint16_t staId) {
...
void WifiTxVector::SetTxPowerLevel(uint8_t powerlevel) {
  m_txPowerLevel = powerlevel;
}

void WifiTxVector::SetPreambleType(WifiPreamble preamble) {
  m_preamble = preamble;
}

void WifiTxVector::SetChannelWidth(uint16_t channelWidth) {
  m_channelWidth = channelWidth;
}

void WifiTxVector::SetGuardInterval(uint16_t guardInterval) {
  m_guardInterval = guardInterval;
}

void WifiTxVector::SetNTx(uint8_t nTx) { m_nTx = nTx; }

void WifiTxVector::SetNss(uint8_t nss) { m_nss = nss; }

void WifiTxVector::SetNss(uint8_t nss, uint16_t staId) {
  NS_ABORT_MSG_IF(!IsMu(), "Not a MU transmission");
  NS_ABORT_MSG_IF(staId > 2048, "STA-ID should be correctly set for MU");
  m_muUserInfos[staId].nss = nss;
}

void WifiTxVector::SetNess(uint8_t ness) { m_ness = ness; }

void WifiTxVector::SetAggregation(bool aggregation) {
  m_aggregation = aggregation;
}

void WifiTxVector::SetStbc(bool stbc) { m_stbc = stbc; }

void WifiTxVector::SetLdpc(bool ldpc) { m_ldpc = ldpc; }

void WifiTxVector::SetBssColor(uint8_t color) { m_bssColor = color; }

uint8_t WifiTxVector::GetBssColor() const { return m_bssColor; }

void WifiTxVector::SetLength(uint16_t length) { m_length = length; }

uint16_t WifiTxVector::GetLength() const { return m_length; }

bool WifiTxVector::IsTriggerResponding() const { return m_triggerResponding; }

void WifiTxVector::SetTriggerResponding(bool triggerResponding) {
  m_triggerResponding = triggerResponding;
}

void WifiTxVector::SetSigBMode(const WifiMode &mode) { m_sigBMcs = mode; }

WifiMode WifiTxVector::GetSigBMode() const { return m_sigBMcs; }

void WifiTxVector::SetRuAllocation(const RuAllocation &ruAlloc,
                                   uint8_t p20Index) {
...
const RuAllocation &WifiTxVector::GetRuAllocation(uint8_t p20Index) const {
...
void WifiTxVector::SetEhtPpduType(uint8_t type) {
  NS_ASSERT(IsEht(m_preamble));
  m_ehtPpduType = type;
}

uint8_t WifiTxVector::GetEhtPpduType() const { return m_ehtPpduType; }

bool WifiTxVector::IsValid() const {
...
bool WifiTxVector::IsMu() const { return IsDlMu() || IsUlMu(); }

bool WifiTxVector::IsDlMu() const {
  return ns3::IsDlMu(m_preamble) && !(IsEht(m_preamble) && m_ehtPpduType == 1);
}

bool WifiTxVector::IsUlMu() const { return ns3::IsUlMu(m_preamble); }

bool WifiTxVector::IsDlOfdma() const {
  if (!IsDlMu()) {
    return false;
  }
  if (IsEht(m_preamble)) {
...
bool WifiTxVector::IsDlMuMimo() const {
  if (!IsDlMu()) {
    return false;
  }
  if (IsEht(m_preamble)) {
...
uint8_t WifiTxVector::GetNumStasInRu(const HeRu::RuSpec &ru) const {
  return std::count_if(
      m_muUserInfos.cbegin(), m_muUserInfos.cend(),
      [&ru](const auto &info) -> bool { return (ru == info.second.ru); });
}

bool WifiTxVector::IsAllocated(uint16_t staId) const {
  return m_muUserInfos.count(staId) > 0;
}

HeRu::RuSpec WifiTxVector::GetRu(uint16_t staId) const {
  NS_ABORT_MSG_IF(!IsMu(), "RU only available for MU");
  NS_ABORT_MSG_IF(staId > 2048, "STA-ID should be correctly set for MU");
  return m_muUserInfos.at(staId).ru;
}

void WifiTxVector::SetRu(HeRu::RuSpec ru, uint16_t staId) {
  NS_ABORT_MSG_IF(!IsMu(), "RU only available for MU");
  NS_ABORT_MSG_IF(staId > 2048, "STA-ID should be correctly set for MU");
  m_muUserInfos[staId].ru = ru;
}

HeMuUserInfo WifiTxVector::GetHeMuUserInfo(uint16_t staId) const {
  NS_ABORT_MSG_IF(!IsMu(), "HE MU user info only available for MU");
  return m_muUserInfos.at(staId);
}

void WifiTxVector::SetHeMuUserInfo(uint16_t staId, HeMuUserInfo userInfo) {
  NS_ABORT_MSG_IF(!IsMu(), "HE MU user info only available for MU");
  NS_ABORT_MSG_IF(staId > 2048, "STA-ID should be correctly set for MU");
...
const WifiTxVector::HeMuUserInfoMap &WifiTxVector::GetHeMuUserInfoMap() const {
  NS_ABORT_MSG_IF(!IsMu(), "HE MU user info map only available for MU");
  return m_muUserInfos;
}

WifiTxVector::HeMuUserInfoMap &WifiTxVector::GetHeMuUserInfoMap() {
  NS_ABORT_MSG_IF(!IsMu(), "HE MU user info map only available for MU");
  m_ruAllocation.clear();
  return m_muUserInfos;
}

bool WifiTxVector::IsSigBCompression() const {
  return IsDlMuMimo() && !IsDlOfdma();
}

void WifiTxVector::SetInactiveSubchannels(
    const std::vector<bool> &inactiveSubchannels) {
...
const std::vector<bool> &WifiTxVector::GetInactiveSubchannels() const {
  return m_inactiveSubchannels;
}

void WifiTxVector::SetCenter26ToneRuIndication(
...
std::ostream &operator<<(std::ostream &os, const WifiTxVector &v) {
  ...
  os << "txpwrlvl: " << +v.GetTxPowerLevel()
     ...
     << " GI: " << v.GetGuardInterval() << " NTx: " << +v.GetNTx()
     << " Ness: " << +v.GetNess() << " MPDU aggregation: " << v.IsAggregation()
     << " STBC: " << v.IsStbc()
  ...
  if (v.IsMu()) {
    ...
    for (auto &ui : userInfoMap) {
      os << ", {STA-ID: " << ui.first << ", " << ui.second.ru
         << ", MCS: " << +ui.second.mcs << ", Nss: " << +ui.second.nss << "}";
    }
  } else {
...
bool HeMuUserInfo::operator==(const HeMuUserInfo &other) const {
...
WifiTxVector::GetUserInfoMapOrderedByRus(uint8_t p20Index) const {
  ...
  for (const auto &userInfo : m_muUserInfos) {
    ...
    if (auto it = orderedMap.find(ru); it != orderedMap.end()) {
      ...
      orderedMap.emplace(userInfo.second.ru,
                         std::set<uint16_t>{userInfo.first});
    }
  }
  return orderedMap;
}

RuAllocation WifiTxVector::DeriveRuAllocation(uint8_t p20Index) const {
...
Center26ToneRuIndication WifiTxVector::DeriveCenter26ToneRuIndication() const {
