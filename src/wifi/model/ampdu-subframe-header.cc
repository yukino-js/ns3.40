...
TypeId AmpduSubframeHeader::GetTypeId() {
...
uint32_t AmpduSubframeHeader::GetSerializedSize() const { return (2 + 1 + 1); }
...
uint32_t AmpduSubframeHeader::Deserialize(Buffer::Iterator start) {
...
void AmpduSubframeHeader::Print(std::ostream &os) const {
  os << "EOF = " << m_eof << ", length = " << m_length << ", signature = 0x"
     << std::hex << m_signature;
}

void AmpduSubframeHeader::SetLength(uint16_t length) { m_length = length; }

void AmpduSubframeHeader::SetEof(bool eof) { m_eof = eof; }

uint16_t AmpduSubframeHeader::GetLength() const { return m_length; }

bool AmpduSubframeHeader::GetEof() const { return m_eof; }

bool AmpduSubframeHeader::IsSignatureValid() const {
