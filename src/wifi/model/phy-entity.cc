...
void PhyEntity::SetOwner(Ptr<WifiPhy> wifiPhy) {
  NS_LOG_FUNCTION(this << wifiPhy);
  m_wifiPhy = wifiPhy;
  m_state = m_wifiPhy->m_state;
}

bool PhyEntity::IsModeSupported(WifiMode mode) const {
...
bool PhyEntity::HandlesMcsModes() const { return false; }

std::list<WifiMode>::const_iterator PhyEntity::begin() const {
  return m_modeList.begin();
}

std::list<WifiMode>::const_iterator PhyEntity::end() const {
  return m_modeList.end();
}

WifiMode PhyEntity::GetSigMode(WifiPpduField field,
...
WifiPpduField PhyEntity::GetNextField(WifiPpduField currentField,
  ...
  if (itPpdu != ppduFormats.end()) {
    const auto itField =
        std::find(itPpdu->second.begin(), itPpdu->second.end(), currentField);
...
Time PhyEntity::CalculatePhyPreambleAndHeaderDuration(
  ...
  for (uint8_t field = WIFI_PPDU_FIELD_PREAMBLE; field < WIFI_PPDU_FIELD_DATA;
       ++field) {
...
WifiConstPsduMap
PhyEntity::GetWifiConstPsduMap(Ptr<const WifiPsdu> psdu,
                               const WifiTxVector &txVector) const {
...
Ptr<WifiPpdu> PhyEntity::BuildPpdu(const WifiConstPsduMap &psdus,
...
PhyEntity::SnrPer PhyEntity::GetPhyHeaderSnrPer(WifiPpduField field,
  ...
  return m_wifiPhy->m_interference->CalculatePhyHeaderSnrPer(
      event, measurementChannelWidth, GetPrimaryBand(measurementChannelWidth),
      field);
}
...
Time PhyEntity::GetRemainingDurationAfterField(Ptr<const WifiPpdu> ppdu,
...
void PhyEntity::StartReceivePreamble(Ptr<const WifiPpdu> ppdu,
  ...
  case WifiPhyState::RX:
    if (m_wifiPhy->m_frameCaptureModel &&
        ...
        m_wifiPhy->m_frameCaptureModel->CaptureNewFrame(
...
void PhyEntity::DropPreambleEvent(Ptr<const WifiPpdu> ppdu,
...
void PhyEntity::ErasePreambleEvent(Ptr<const WifiPpdu> ppdu, Time rxDuration) {
...
void PhyEntity::StartReceivePayload(Ptr<Event> event) {
...
Time PhyEntity::DoStartReceivePayload(Ptr<Event> event) {
  NS_LOG_FUNCTION(this << *event);
  Ptr<const WifiPpdu> ppdu = event->GetPpdu();
...
void PhyEntity::ScheduleEndOfMpdus(Ptr<Event> event) {
  NS_LOG_FUNCTION(this << *event);
  Ptr<const WifiPpdu> ppdu = event->GetPpdu();
  ...
  MpduType mpduType = (nMpdus > 1)
                          ? FIRST_MPDU_IN_AGGREGATE
                          : (psdu->IsSingle() ? SINGLE_MPDU : NORMAL_MPDU);
  uint32_t totalAmpduSize = 0;
  double totalAmpduNumSymbols = 0.0;
  auto mpdu = psdu->begin();
  for (size_t i = 0; i < nMpdus && mpdu != psdu->end(); ++mpdu) {
    uint32_t size = (mpduType == NORMAL_MPDU) ? psdu->GetSize()
                                              : psdu->GetAmpduSubframeSize(i);
    Time mpduDuration = m_wifiPhy->GetPayloadDuration(
        size, txVector, m_wifiPhy->GetPhyBand(), mpduType, true, totalAmpduSize,
    ...
    NS_LOG_INFO("Schedule end of MPDU #"
                << i << " in " << endOfMpduDuration.As(Time::NS)
                << " (relativeStart=" << relativeStart.As(Time::NS)
                << ", mpduDuration=" << mpduDuration.As(Time::NS)
                << ", remainingAmdpuDuration="
                << remainingAmpduDuration.As(Time::NS) << ")");
    m_endOfMpduEvents.push_back(Simulator::Schedule(
...
void PhyEntity::EndOfMpdu(Ptr<Event> event, Ptr<const WifiPsdu> psdu,
                          size_t mpduIndex, Time relativeStart,
                          Time mpduDuration) {
  ...
  NS_LOG_DEBUG("Extracted MPDU #"
               << mpduIndex << ": duration: " << mpduDuration.As(Time::NS)
...
void PhyEntity::EndReceivePayload(Ptr<Event> event) {
  const auto ppdu = event->GetPpdu();
  const auto &txVector = ppdu->GetTxVector();
  ...
  double snr = m_wifiPhy->m_interference->CalculateSnr(
      event, channelWidthAndBand.first, txVector.GetNss(staId),
...
void PhyEntity::RxPayloadSucceeded(Ptr<const WifiPsdu> psdu,
                                   RxSignalInfo rxSignalInfo,
...
void PhyEntity::RxPayloadFailed(Ptr<const WifiPsdu> psdu, double snr,
                                const WifiTxVector &txVector) {
...
PhyEntity::GetReceptionStatus(Ptr<const WifiPsdu> psdu, Ptr<Event> event,
                              uint16_t staId, Time relativeMpduStart,
                              Time mpduDuration) {
  NS_LOG_FUNCTION(this << *psdu << *event << staId << relativeMpduStart
                       << mpduDuration);
  ...
  NS_LOG_DEBUG(
      "rate=" << (mode.GetDataRate(event->GetPpdu()->GetTxVector(), staId))
  ...
  if (GetRandomValue() > snrPer.per &&
      !(m_wifiPhy->m_postReceptionErrorModel &&
        m_wifiPhy->m_postReceptionErrorModel->IsCorrupt(
            psdu->GetPacket()->Copy()))) {
...
void PhyEntity::AddPreambleEvent(Ptr<Event> event) {
  NS_LOG_FUNCTION(this << *event);
  Ptr<const WifiPpdu> ppdu = event->GetPpdu();
...
void PhyEntity::HandleRxPpduWithSameContent(
    Ptr<Event> event, Ptr<const WifiPpdu> ppdu,
    RxPowerWattPerChannelBand &rxPower) {
...
void PhyEntity::NotifyInterferenceRxEndAndClear(bool reset) {
...
void PhyEntity::EndPreambleDetectionPeriod(Ptr<Event> event) {
  ...
  if (maxEvent != event) {
    ...
    m_wifiPhy->NotifyRxDrop(GetAddressedPsduInPpdu(event->GetPpdu()),
  ...
  NS_LOG_DEBUG("SNR(dB)=" << RatioToDb(snr)
                          << " at end of preamble detection period");
  ...
  if ((!m_wifiPhy->m_preambleDetectionModel && maxRxPowerW > 0.0) ||
    ...
    for (auto it = m_wifiPhy->m_currentPreambleEvents.begin();
      ...
      if (it->second != m_wifiPhy->m_currentEvent) {
        NS_LOG_DEBUG("Drop packet with UID "
                     << it->first.first << " and preamble " << it->first.second
        ...
        m_wifiPhy->NotifyRxDrop(GetAddressedPsduInPpdu(it->second->GetPpdu()),
                                reason);
    ...
    m_wifiPhy->NotifyRxBegin(
        GetAddressedPsduInPpdu(m_wifiPhy->m_currentEvent->GetPpdu()),
...
bool PhyEntity::IsConfigSupported(Ptr<const WifiPpdu> ppdu) const {
...
void PhyEntity::CancelAllEvents() {
  NS_LOG_FUNCTION(this);
...
bool PhyEntity::NoEndPreambleDetectionEvents() const {
...
void PhyEntity::AbortCurrentReception(WifiPhyRxfailureReason reason) {
...
void PhyEntity::ResetReceive(Ptr<Event> event) {
  NS_LOG_FUNCTION(this << *event);
  DoResetReceive(event);
  NS_ASSERT(!m_wifiPhy->IsStateRx());
  m_wifiPhy->m_interference->NotifyRxEnd(Simulator::Now(),
                                         m_wifiPhy->GetCurrentFrequencyRange());
  NS_ASSERT(m_endRxPayloadEvents.size() == 1 &&
            m_endRxPayloadEvents.front().IsExpired());
  m_endRxPayloadEvents.clear();
  m_wifiPhy->m_currentEvent = nullptr;
  m_wifiPhy->m_currentPreambleEvents.clear();
...
void PhyEntity::DoResetReceive(Ptr<Event> event) {
  NS_LOG_FUNCTION(this << *event);
  NS_ASSERT(event->GetEndTime() == Simulator::Now());
}

double PhyEntity::GetRandomValue() const {
  return m_wifiPhy->m_random->GetValue();
}

double PhyEntity::GetRxPowerWForPpdu(Ptr<Event> event) const {
  return event->GetRxPowerW(
      GetPrimaryBand(GetMeasurementChannelWidth(event->GetPpdu())));
}

Ptr<const Event> PhyEntity::GetCurrentEvent() const {
  return m_wifiPhy->m_currentEvent;
}

WifiSpectrumBandInfo PhyEntity::GetPrimaryBand(uint16_t bandWidth) const {
  if (m_wifiPhy->GetChannelWidth() % 20 != 0) {
    return m_wifiPhy->GetBand(bandWidth);
  }
  return m_wifiPhy->GetBand(
      bandWidth,
      m_wifiPhy->m_operatingChannel.GetPrimaryChannelIndex(bandWidth));
}

WifiSpectrumBandInfo PhyEntity::GetSecondaryBand(uint16_t bandWidth) const {
...
uint16_t PhyEntity::GetRxChannelWidth(const WifiTxVector &txVector) const {
...
Time PhyEntity::GetDelayUntilCcaEnd(double thresholdDbm,
...
uint16_t PhyEntity::GetCenterFrequencyForChannelWidth(
    const WifiTxVector &txVector) const {
...
void PhyEntity::NotifyPayloadBegin(const WifiTxVector &txVector,
                                   const Time &payloadDuration) {
  m_wifiPhy->m_phyRxPayloadBeginTrace(txVector, payloadDuration);
}

void PhyEntity::StartTx(Ptr<const WifiPpdu> ppdu) {
  NS_LOG_FUNCTION(this << ppdu);
  auto txPowerDbm =
  ...
  Transmit(ppdu->GetTxDuration(), ppdu, txPowerDbm, txPowerSpectrum,
           "transmission");
}

void PhyEntity::Transmit(Time txDuration, Ptr<const WifiPpdu> ppdu,
                         double txPowerDbm, Ptr<SpectrumValue> txPowerSpectrum,
                         const std::string &type) {
  NS_LOG_FUNCTION(this << txDuration << ppdu << txPowerDbm << type);
  NS_LOG_DEBUG("Start " << type << ": signal power before antenna gain="
                        << txPowerDbm << "dBm");
  auto txParams = Create<WifiSpectrumSignalParameters>();
  txParams->duration = txDuration;
  txParams->psd = txPowerSpectrum;
  txParams->ppdu = ppdu;
  NS_LOG_DEBUG("Starting " << type << " with power " << txPowerDbm
                           << " dBm on channel "
...
uint16_t PhyEntity::GetGuardBandwidth(uint16_t currentChannelWidth) const {
  return m_wifiPhy->GetGuardBandwidth(currentChannelWidth);
}

std::tuple<double, double, double> PhyEntity::GetTxMaskRejectionParams() const {
  return m_wifiPhy->GetTxMaskRejectionParams();
}

Time PhyEntity::CalculateTxDuration(WifiConstPsduMap psduMap,
                                    const WifiTxVector &txVector,
                                    WifiPhyBand band) const {
  NS_ASSERT(psduMap.size() == 1);
  const auto &it = psduMap.begin();
  return WifiPhy::CalculateTxDuration(it->second->GetSize(), txVector, band,
                                      it->first);
}
...
Ptr<const WifiPpdu> PhyEntity::GetRxPpduFromTxPpdu(Ptr<const WifiPpdu> ppdu) {
  return ppdu;
}
