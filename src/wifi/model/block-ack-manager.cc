...
TypeId BlockAckManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::BlockAckManager")
          ...
          .AddTraceSource("AgreementState", "The state of the ADDBA handshake",
                          MakeTraceSourceAccessor(
                              &BlockAckManager::m_originatorAgreementState),
                          "ns3::BlockAckManager::AgreementStateTracedCallback");
  return tid;
}

BlockAckManager::BlockAckManager() { NS_LOG_FUNCTION(this); }

BlockAckManager::~BlockAckManager() { NS_LOG_FUNCTION(this); }

void BlockAckManager::DoDispose() {
...
BlockAckManager::GetAgreementAsRecipient(const Mac48Address &originator,
  ...
  if (auto it = m_recipientAgreements.find({originator, tid});
...
void BlockAckManager::CreateOriginatorAgreement(
    const MgtAddBaRequestHeader &reqHdr, const Mac48Address &recipient,
    bool htSupported) {
  ...
  m_originatorAgreementState(Simulator::Now(), recipient, tid,
  ...
  m_originatorAgreements.insert_or_assign(
      {recipient, tid}, std::make_pair(std::move(agreement), PacketQueue{}));
  m_blockPackets(recipient, tid);
}

void BlockAckManager::DestroyOriginatorAgreement(const Mac48Address &recipient,
                                                 uint8_t tid) {
  NS_LOG_FUNCTION(this << recipient << +tid);
...
void BlockAckManager::UpdateOriginatorAgreement(
    const MgtAddBaResponseHeader &respHdr, const Mac48Address &recipient,
...
void BlockAckManager::CreateRecipientAgreement(
    const MgtAddBaResponseHeader &respHdr, const Mac48Address &originator,
    uint16_t startingSeq, bool htSupported, Ptr<MacRxMiddle> rxMiddle) {
  NS_LOG_FUNCTION(this << respHdr << originator << startingSeq << htSupported
                       << rxMiddle);
  uint8_t tid = respHdr.GetTid();

  RecipientBlockAckAgreement agreement(
      originator, respHdr.IsAmsduSupported(), tid, respHdr.GetBufferSize(),
      respHdr.GetTimeout(), startingSeq, htSupported);
...
void BlockAckManager::DestroyRecipientAgreement(const Mac48Address &originator,
                                                uint8_t tid) {
  NS_LOG_FUNCTION(this << originator << tid);
...
void BlockAckManager::StorePacket(Ptr<WifiMpdu> mpdu) {
  ...
  uint16_t mpduDist = agreementIt->second.first.GetDistance(
  ...
  while (it != agreementIt->second.second.rend()) {
    if (mpdu->GetHeader().GetSequenceControl() ==
        (*it)->GetHeader().GetSequenceControl()) {
      NS_LOG_DEBUG("Packet already in the queue of the BA agreement");
      return;
    }

    uint16_t dist = agreementIt->second.first.GetDistance(
        (*it)->GetHeader().GetSequenceNumber());

    if (mpduDist > dist ||
        (mpduDist == dist && mpdu->GetHeader().GetFragmentNumber() >
...
uint32_t BlockAckManager::GetNBufferedPackets(const Mac48Address &recipient,
                                              uint8_t tid) const {
  NS_LOG_FUNCTION(this << recipient << +tid);
  auto it = m_originatorAgreements.find({recipient, tid});
  if (it == m_originatorAgreements.end()) {
...
void BlockAckManager::SetBlockAckThreshold(uint8_t nPackets) {
  NS_LOG_FUNCTION(this << +nPackets);
  m_blockAckThreshold = nPackets;
}

BlockAckManager::PacketQueueI BlockAckManager::HandleInFlightMpdu(
    uint8_t linkId, PacketQueueI mpduIt, MpduStatus status,
    const OriginatorAgreementsI &it, const Time &now) {
  ...
  if (it->second.first.GetDistance(hdr.GetSequenceNumber()) >=
...
void BlockAckManager::NotifyGotAck(uint8_t linkId, Ptr<const WifiMpdu> mpdu) {
...
void BlockAckManager::NotifyMissedAck(uint8_t linkId, Ptr<WifiMpdu> mpdu) {
  NS_LOG_FUNCTION(this << linkId << *mpdu);
  NS_ASSERT(mpdu->GetHeader().IsQosData());
...
std::pair<uint16_t, uint16_t> BlockAckManager::NotifyGotBlockAck(
    uint8_t linkId, const CtrlBAckResponseHeader &blockAck,
    const Mac48Address &recipient, const std::set<uint8_t> &tids,
    size_t index) {
  NS_LOG_FUNCTION(this << linkId << blockAck << recipient << index);

  NS_ABORT_MSG_IF(blockAck.IsBasic(), "Basic Block Ack is not supported");
  NS_ABORT_MSG_IF(blockAck.IsMultiTid(),
...
void BlockAckManager::NotifyMissedBlockAck(uint8_t linkId,
...
void BlockAckManager::NotifyDiscardedMpdu(Ptr<const WifiMpdu> mpdu) {
  NS_LOG_FUNCTION(this << *mpdu);
  ...
  if (QosUtilsIsOldPacket(currStartingSeq,
                          mpdu->GetHeader().GetSequenceNumber())) {
...
void BlockAckManager::NotifyGotBlockAckRequest(const Mac48Address &originator,
                                               uint8_t tid,
                                               uint16_t startingSeq) {
...
void BlockAckManager::NotifyGotMpdu(Ptr<const WifiMpdu> mpdu) {
  NS_LOG_FUNCTION(this << *mpdu);
...
void BlockAckManager::ScheduleBar(const CtrlBAckRequestHeader &reqHdr,
                                  const WifiMacHeader &hdr) {
  NS_LOG_FUNCTION(this << reqHdr << hdr);

  uint8_t tid = reqHdr.GetTidInfo();

  WifiContainerQueueId queueId(WIFI_CTL_QUEUE, WIFI_UNICAST, hdr.GetAddr1(),
                               std::nullopt);
...
void BlockAckManager::RemoveFromSendBarIfDataQueuedList(
    const Mac48Address &recipient, uint8_t tid) {
  NS_LOG_FUNCTION(this << recipient << tid);
  m_sendBarIfDataQueued.remove({recipient, tid});
}

void BlockAckManager::InactivityTimeout(const Mac48Address &recipient,
                                        uint8_t tid) {
  NS_LOG_FUNCTION(this << recipient << +tid);
  m_blockAckInactivityTimeout(recipient, tid, true);
}

void BlockAckManager::NotifyOriginatorAgreementEstablished(
    const Mac48Address &recipient, uint8_t tid, uint16_t startingSeq) {
...
void BlockAckManager::NotifyOriginatorAgreementNoReply(
    const Mac48Address &recipient, uint8_t tid) {
  NS_LOG_FUNCTION(this << recipient << +tid);
  auto it = m_originatorAgreements.find({recipient, tid});
  NS_ASSERT(it != m_originatorAgreements.end());
...
void BlockAckManager::NotifyOriginatorAgreementReset(
    const Mac48Address &recipient, uint8_t tid) {
  NS_LOG_FUNCTION(this << recipient << +tid);
  auto it = m_originatorAgreements.find({recipient, tid});
  NS_ASSERT(it != m_originatorAgreements.end());
...
void BlockAckManager::SetQueue(const Ptr<WifiMacQueue> queue) {
  NS_LOG_FUNCTION(this << queue);
  m_queue = queue;
}

bool BlockAckManager::NeedBarRetransmission(uint8_t tid,
                                            const Mac48Address &recipient) {
  auto it = m_originatorAgreements.find({recipient, tid});
  if (it == m_originatorAgreements.end() || !it->second.first.IsEstablished()) {
...
void BlockAckManager::SetBlockAckInactivityCallback(
    Callback<void, Mac48Address, uint8_t, bool> callback) {
  NS_LOG_FUNCTION(this << &callback);
  m_blockAckInactivityTimeout = callback;
}

void BlockAckManager::SetBlockDestinationCallback(
    Callback<void, Mac48Address, uint8_t> callback) {
  NS_LOG_FUNCTION(this << &callback);
  m_blockPackets = callback;
}

void BlockAckManager::SetUnblockDestinationCallback(
    Callback<void, Mac48Address, uint8_t> callback) {
  NS_LOG_FUNCTION(this << &callback);
  m_unblockPackets = callback;
}

void BlockAckManager::SetTxOkCallback(TxOk callback) {
  m_txOkCallback = callback;
}

void BlockAckManager::SetTxFailedCallback(TxFailed callback) {
  m_txFailedCallback = callback;
}

void BlockAckManager::SetDroppedOldMpduCallback(DroppedOldMpdu callback) {
  m_droppedOldMpduCallback = callback;
}

uint16_t BlockAckManager::GetRecipientBufferSize(const Mac48Address &recipient,
                                                 uint8_t tid) const {
  uint16_t size = 0;
  auto it = m_originatorAgreements.find({recipient, tid});
  if (it != m_originatorAgreements.end()) {
...
uint16_t
BlockAckManager::GetOriginatorStartingSequence(const Mac48Address &recipient,
                                               uint8_t tid) const {
  uint16_t seqNum = 0;
  auto it = m_originatorAgreements.find({recipient, tid});
  if (it != m_originatorAgreements.end()) {
