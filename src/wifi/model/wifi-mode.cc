...
bool WifiMode::IsAllowed(uint16_t channelWidth, uint8_t nss) const {
...
bool WifiMode::IsAllowed(const WifiTxVector &txVector) const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
  return item->IsAllowedCallback(txVector);
}

uint64_t WifiMode::GetPhyRate(uint16_t channelWidth) const {
  return GetPhyRate(channelWidth, 800, 1);
}

uint64_t WifiMode::GetPhyRate(uint16_t channelWidth, uint16_t guardInterval,
                              uint8_t nss) const {
  WifiTxVector txVector;
  txVector.SetMode(WifiMode(m_uid));
  txVector.SetChannelWidth(channelWidth);
...
uint64_t WifiMode::GetPhyRate(const WifiTxVector &txVector,
...
uint64_t WifiMode::GetDataRate(uint16_t channelWidth) const {
  return GetDataRate(channelWidth, 800, 1);
}

uint64_t WifiMode::GetDataRate(const WifiTxVector &txVector,
                               uint16_t staId) const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
...
uint64_t WifiMode::GetDataRate(uint16_t channelWidth, uint16_t guardInterval,
                               uint8_t nss) const {
  NS_ASSERT(nss <= 8);
  WifiTxVector txVector;
  txVector.SetMode(WifiMode(m_uid));
...
WifiCodeRate WifiMode::GetCodeRate() const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
  return item->GetCodeRateCallback();
}

uint16_t WifiMode::GetConstellationSize() const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
...
bool WifiMode::IsMandatory() const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
  return item->isMandatory;
}

uint8_t WifiMode::GetMcsValue() const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
...
uint32_t WifiMode::GetUid() const { return m_uid; }

WifiModulationClass WifiMode::GetModulationClass() const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
  return item->modClass;
}

uint64_t WifiMode::GetNonHtReferenceRate() const {
  WifiModeFactory::WifiModeItem *item =
      WifiModeFactory::GetFactory()->Get(m_uid);
...
bool WifiMode::IsHigherCodeRate(WifiMode mode) const {
  NS_ASSERT_MSG(GetCodeRate() != WIFI_CODE_RATE_UNDEFINED,
                "Wifi Code Rate not defined");
...
WifiMode WifiModeFactory::CreateWifiMode(
    std::string uniqueName, WifiModulationClass modClass, bool isMandatory,
...
WifiMode WifiModeFactory::CreateWifiMcs(
...
WifiMode WifiModeFactory::Search(std::string name) const {
  uint32_t j = 0;
...
uint32_t WifiModeFactory::AllocateUid(std::string uniqueUid) {
  uint32_t j = 0;
  for (auto i = m_itemList.begin(); i != m_itemList.end(); i++) {
...
WifiModeFactory::WifiModeItem *WifiModeFactory::Get(uint32_t uid) {
  NS_ASSERT(uid < m_itemList.size());
  return &m_itemList[uid];
}

WifiModeFactory *WifiModeFactory::GetFactory() {
  ...
  if (isFirstTime) {
    ...
    item->GetPhyRateCallback =
        MakeNullCallback<uint64_t, const WifiTxVector &, uint16_t>();
    item->GetDataRateCallback =
        MakeNullCallback<uint64_t, const WifiTxVector &, uint16_t>();
    item->GetNonHtReferenceRateCallback = MakeNullCallback<uint64_t>();
