...
TypeId ConstantObssPdAlgorithm::GetTypeId() {
...
void ConstantObssPdAlgorithm::ConnectWifiNetDevice(
  ...
  if (phy->GetStandard() >= WIFI_STANDARD_80211be) {
    ...
    NS_ASSERT(ehtPhy);
    ehtPhy->SetEndOfHeSigACallback(
        MakeCallback(&ConstantObssPdAlgorithm::ReceiveHeSigA, this));
  }
...
void ConstantObssPdAlgorithm::ReceiveHeSigA(HeSigAParameters params) {
  ...
  if (isObss) {
    ...
    if (WToDbm(params.rssiW) < obssPdLevel) {
      NS_LOG_DEBUG("Frame is OBSS and RSSI "
                   << WToDbm(params.rssiW) << " is below OBSS-PD level of "
