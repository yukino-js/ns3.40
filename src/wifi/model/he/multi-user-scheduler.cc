...
TypeId MultiUserScheduler::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::MultiUserScheduler")
          ...
          .AddAttribute("AccessReqAc",
                        ...
                        MakeEnumChecker(AcIndex::AC_BE, "AC_BE", AcIndex::AC_VI,
                                        "AC_VI", AcIndex::AC_VO, "AC_VO",
                                        AcIndex::AC_BK, "AC_BK"))
          .AddAttribute(
              ...
              BooleanValue(true),
              MakeBooleanAccessor(
                  &MultiUserScheduler::m_restartTimerUponAccess),
              MakeBooleanChecker());
  return tid;
}
...
MultiUserScheduler::~MultiUserScheduler() { NS_LOG_FUNCTION_NOARGS(); }

void MultiUserScheduler::DoDispose() {
...
void MultiUserScheduler::NotifyNewAggregate() {
  NS_LOG_FUNCTION(this);
  if (!m_apMac) {
...
void MultiUserScheduler::DoInitialize() {
  NS_LOG_FUNCTION(this);
...
void MultiUserScheduler::SetWifiMac(Ptr<ApWifiMac> mac) {
  ...
  for (uint8_t linkId = 0; linkId < m_apMac->GetNLinks(); linkId++) {
    auto heFem = DynamicCast<HeFrameExchangeManager>(
        m_apMac->GetFrameExchangeManager(linkId));
    NS_ASSERT(heFem);
...
Ptr<HeFrameExchangeManager> MultiUserScheduler::GetHeFem(uint8_t linkId) const {
  return StaticCast<HeFrameExchangeManager>(
      m_apMac->GetFrameExchangeManager(linkId));
}
...
MultiUserScheduler::NotifyAccessGranted(Ptr<QosTxop> edca, Time availableTime,
                                        bool initialFrame,
                                        uint16_t allowedWidth, uint8_t linkId) {
...
MultiUserScheduler::DlMuInfo &MultiUserScheduler::GetDlMuInfo(uint8_t linkId) {
  ...
  for (auto &psdu : m_lastTxInfo[linkId].dlInfo.psduMap) {
    ...
    NS_ABORT_MSG_IF(
        !GetWifiRemoteStationManager(*linkId)->GetHeSupported(receiver),
...
MultiUserScheduler::UlMuInfo &MultiUserScheduler::GetUlMuInfo(uint8_t linkId) {
...
Ptr<WifiMpdu>
MultiUserScheduler::GetTriggerFrame(const CtrlTriggerHeader &trigger,
...
uint32_t MultiUserScheduler::GetMaxSizeOfQosNullAmpdu(
  ...
  for (uint8_t i = 0; i < maxNTids; i++) {
    maxSize = MpduAggregator::GetSizeIfAggregated(
        headerSize + WIFI_MAC_FCS_LENGTH, maxSize);
  }
