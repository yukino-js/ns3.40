...
HeCapabilities::HeCapabilities()
    : m_plusHtcHeSupport(0), m_twtRequesterSupport(0), m_twtResponderSupport(0),
      m_fragmentationSupport(0), m_maximumNumberOfFragmentedMsdus(0),
      m_minimumFragmentSize(0), m_triggerFrameMacPaddingDuration(0),
      m_multiTidAggregationRxSupport(0), m_heLinkAdaptation(0),
      m_allAckSupport(0), m_trsSupport(0), m_bsrSupport(0),
      m_broadcastTwtSupport(0), m_32bitBaBitmapSupport(0),
      m_muCascadeSupport(0), m_ackEnabledAggregationSupport(0),
      m_omControlSupport(0), m_ofdmaRaSupport(0), m_maxAmpduLengthExponent(0),
      m_amsduFragmentationSupport(0), m_flexibleTwtScheduleSupport(0),
      m_rxControlFrameToMultiBss(0), m_bsrpBqrpAmpduAggregation(0),
      m_qtpSupport(0), m_bqrSupport(0), m_psrResponder(0),
      m_ndpFeedbackReportSupport(0), m_opsSupport(0),
      m_amsduNotUnderBaInAmpduSupport(0), m_multiTidAggregationTxSupport(0),
      m_heSubchannelSelectiveTxSupport(0), m_ul2x996ToneRuSupport(0),
      m_omControlUlMuDataDisableRxSupport(0), m_heDynamicSmPowerSave(0),
      m_puncturedSoundingSupport(0), m_heVhtTriggerFrameRxSupport(0),
      m_channelWidthSet(0), m_puncturedPreambleRx(0), m_deviceClass(0),
      m_ldpcCodingInPayload(0), m_heSuPpdu1xHeLtf800nsGi(0),
      m_midambleRxMaxNsts(0), m_ndp4xHeLtfAnd32msGi(0), m_stbcTxLeq80MHz(0),
      m_stbcRxLeq80MHz(0), m_dopplerTx(0), m_dopplerRx(0), m_fullBwUlMuMimo(0),
      m_partialBwUlMuMimo(0), m_dcmMaxConstellationTx(0), m_dcmMaxNssTx(0),
      m_dcmMaxConstellationRx(0), m_dcmMaxNssRx(0), m_rxPartialBwSuInHeMu(0),
      m_suBeamformer(0), m_suBeamformee(0), m_muBeamformer(0),
      ...
      m_ngEqual16ForSuFeedbackSupport(0), m_ngEqual16ForMuFeedbackSupport(0),
      m_codebookSize42SuFeedback(0), m_codebookSize75MuFeedback(0),
      m_triggeredSuBfFeedback(0), m_triggeredMuBfFeedback(0),
      m_triggeredCqiFeedback(0), m_erPartialBandwidth(0),
      m_dlMuMimoOnPartialBandwidth(0), m_ppeThresholdPresent(0),
      m_psrBasedSrSupport(0), m_powerBoostFactorAlphaSupport(0),
      m_hePpdu4xHeLtf800nsGi(0), m_maxNc(0), m_stbcTxGt80MHz(0),
      m_stbcRxGt80MHz(0), m_heErSuPpdu4xHeLtf08sGi(0),
      m_hePpdu20MHzIn40MHz24GHz(0), m_hePpdu20MHzIn160MHz(0),
      m_hePpdu80MHzIn160MHz(0), m_heErSuPpdu1xHeLtf08Gi(0),
      m_midamble2xAnd1xHeLtf(0), m_dcmMaxRu(0), m_longerThan16HeSigbOfdm(0),
      m_nonTriggeredCqiFeedback(0), m_tx1024QamLt242Ru(0),
      m_rx1024QamLt242Ru(0), m_rxFullBwSuInHeMuCompressedSigB(0),
      m_rxFullBwSuInHeMuNonCompressedSigB(0), m_nominalPacketPadding(0),
      m_maxHeLtfRxInHeMuMoreThanOneRu(0), m_highestNssSupportedM1(0),
      m_highestMcsSupported(0) {
  m_txBwMap.resize(8, 0);
  m_rxBwMap.resize(8, 0);
}

WifiInformationElementId HeCapabilities::ElementId() const {
  return IE_EXTENSION;
}

WifiInformationElementId HeCapabilities::ElementIdExt() const {
  return IE_EXT_HE_CAPABILITIES;
}

void HeCapabilities::Print(std::ostream &os) const {
  os << "HE Capabilities=" << GetHeMacCapabilitiesInfo1() << "|"
     << +GetHeMacCapabilitiesInfo2() << "|" << GetHePhyCapabilitiesInfo1()
...
void HeCapabilities::SetHeMacCapabilitiesInfo(uint32_t ctrl1, uint16_t ctrl2) {
...
uint32_t HeCapabilities::GetHeMacCapabilitiesInfo1() const {
  uint32_t val = 0;
...
uint16_t HeCapabilities::GetHeMacCapabilitiesInfo2() const {
  uint16_t val = 0;
...
uint64_t HeCapabilities::GetHePhyCapabilitiesInfo1() const {
  ...
  val |=
      (static_cast<uint64_t>(m_beamformeeStsForSmallerOrEqualThan80Mhz) & 0x07)
      << 34;
  val |= (static_cast<uint64_t>(m_beamformeeStsForLargerThan80Mhz) & 0x07)
         << 37;
  val |= (static_cast<uint64_t>(
         ...
         << 40;
  val |=
      (static_cast<uint64_t>(m_numberOfSoundingDimensionsForLargerThan80Mhz) &
       0x07)
      << 43;
...
uint16_t HeCapabilities::GetHePhyCapabilitiesInfo2() const {
  uint16_t val = 0;
...
void HeCapabilities::SetLdpcCodingInPayload(uint8_t ldpcCodingInPayload) {
  m_ldpcCodingInPayload = ldpcCodingInPayload;
}

void HeCapabilities::SetHeSuPpdu1xHeLtf800nsGi(bool heSuPpdu1xHeLtf800nsGi) {
  m_heSuPpdu1xHeLtf800nsGi = (heSuPpdu1xHeLtf800nsGi ? 1 : 0);
}

void HeCapabilities::SetHePpdu4xHeLtf800nsGi(bool hePpdu4xHeLtf800nsGi) {
...
uint8_t HeCapabilities::GetChannelWidthSet() const { return m_channelWidthSet; }

uint8_t HeCapabilities::GetLdpcCodingInPayload() const {
  return m_ldpcCodingInPayload;
}

bool HeCapabilities::GetHeSuPpdu1xHeLtf800nsGi() const {
  return (m_heSuPpdu1xHeLtf800nsGi == 1);
}

bool HeCapabilities::GetHePpdu4xHeLtf800nsGi() const {
  return (m_hePpdu4xHeLtf800nsGi == 1);
}

uint8_t HeCapabilities::GetHighestMcsSupported() const {
  return m_highestMcsSupported + 7;
}

uint8_t HeCapabilities::GetHighestNssSupported() const {
  return m_highestNssSupportedM1 + 1;
}

uint32_t HeCapabilities::GetMaxAmpduLength() const {
  return std::min<uint32_t>((1UL << (20 + m_maxAmpduLengthExponent)) - 1,
                            6500631);
}
