...
TypeId MuSnrTag::GetInstanceTypeId() const { return GetTypeId(); }

MuSnrTag::MuSnrTag() {}

void MuSnrTag::Reset() { m_snrMap.clear(); }

void MuSnrTag::Set(uint16_t staId, double snr) { m_snrMap[staId] = snr; }

bool MuSnrTag::IsPresent(uint16_t staId) const {
  return (m_snrMap.find(staId) != m_snrMap.end());
}

double MuSnrTag::Get(uint16_t staId) const {
  NS_ASSERT(IsPresent(staId));
  return m_snrMap.at(staId);
}

uint32_t MuSnrTag::GetSerializedSize() const {
  return (sizeof(uint16_t) + sizeof(double)) * m_snrMap.size() + 1;
}

void MuSnrTag::Serialize(TagBuffer i) const {
...
void MuSnrTag::Deserialize(TagBuffer i) {
  uint8_t n = i.ReadU8();
...
void MuSnrTag::Print(std::ostream &os) const {
  for (const auto &staIdSnrPair : m_snrMap) {
