...
void HePpdu::SetPhyHeaders(const WifiTxVector &txVector, Time ppduDuration) {
  NS_LOG_FUNCTION(this << txVector << ppduDuration);
  SetLSigHeader(ppduDuration);
  SetHeSigHeader(txVector);
}

void HePpdu::SetLSigHeader(Time ppduDuration) {
...
void HePpdu::SetHeSigHeader(const WifiTxVector &txVector) {
...
WifiTxVector HePpdu::DoGetTxVector() const {
...
void HePpdu::SetTxVectorFromPhyHeaders(WifiTxVector &txVector) const {
  ...
  if (!IsMu()) {
    ...
    txVector.SetChannelWidth(
        GetChannelWidthMhzFromEncoding(heSigHeader->m_bandwidth));
    txVector.SetGuardInterval(
        GetGuardIntervalFromEncoding(heSigHeader->m_giLtfSize));
    ...
    txVector.SetGuardInterval(
        GetGuardIntervalFromEncoding(heSigHeader->m_giLtfSize));
    txVector.SetBssColor(heSigHeader->m_bssColor);
    SetHeMuUserInfos(txVector, heSigHeader->m_ruAllocation,
    ...
    if (heSigHeader->m_center26ToneRuIndication.has_value()) {
      txVector.SetCenter26ToneRuIndication(
          heSigHeader->m_center26ToneRuIndication.value());
    }
...
void HePpdu::SetHeMuUserInfos(WifiTxVector &txVector,
    ...
    for (const auto &userInfo : contentChannel) {
      ...
      if ((ruAllocation.size() == 8) && (ruType == HeRu::RU_996_TONE) &&
          (((txVector.GetChannelWidth() == 160) && sigBcompression) ||
           std::all_of(contentChannel.cbegin(), contentChannel.cend(),
                       [&userInfo](const auto &item) {
                         return userInfo.staId == item.staId;
                       }))) {
      ...
      auto ruIndexOffset = (ruBw < 20)
                               ? (ruSpecs.size() * ruAllocIndex)
...
Time HePpdu::GetTxDuration() const {
...
bool HePpdu::IsMu() const { return (IsDlMu() || IsUlMu()); }

bool HePpdu::IsDlMu() const { return (m_preamble == WIFI_PREAMBLE_HE_MU); }
...
uint16_t HePpdu::GetStaId() const {
  NS_ASSERT(IsUlMu());
  return m_psdus.begin()->first;
}

uint16_t HePpdu::GetTxChannelWidth() const {
  if (const auto &txVector = GetTxVector();
      txVector.IsValid() && txVector.IsUlMu() && GetStaId() != SU_STA_ID) {
    TxPsdFlag flag = GetTxPsdFlag();
    uint16_t ruWidth =
        HeRu::GetBandwidth(txVector.GetRu(GetStaId()).GetRuType());
    uint16_t channelWidth =
        (flag == PSD_NON_HE_PORTION && ruWidth < 20) ? 20 : ruWidth;
    NS_LOG_INFO("Use channelWidth=" << channelWidth << " MHz for HE TB from "
                                    << GetStaId() << " for " << flag);
    return channelWidth;
  } else {
...
HePpdu::TxPsdFlag HePpdu::GetTxPsdFlag() const { return m_txPsdFlag; }

void HePpdu::SetTxPsdFlag(TxPsdFlag flag) const {
  NS_LOG_FUNCTION(this << flag);
  m_txPsdFlag = flag;
}

void HePpdu::UpdateTxVectorForUlMu(
    const std::optional<WifiTxVector> &trigVector) const {
...
HePpdu::GetHeSigBContentChannels(const WifiTxVector &txVector,
                                 uint8_t p20Index) {
  ...
  for (const auto &[ru, staIds] : orderedMap) {
    ...
    for (auto staId : staIds) {
      ...
      contentChannels.at(ccIndex).push_back(
          {staId, userInfo.nss, userInfo.mcs});
    }
  }
  ...
  if (!isSigBCompression) {
    ...
    for (auto &contentChannel : contentChannels) {
      const auto totalUsersInContentChannel =
          (contentChannelIndex == 1) ? numNumRusPerHeSigBContentChannel.first
                                     : numNumRusPerHeSigBContentChannel.second;
      NS_ASSERT(contentChannel.size() <= totalUsersInContentChannel);
      std::size_t unallocatedRus =
          totalUsersInContentChannel - contentChannel.size();
...
uint32_t HePpdu::GetSigBFieldSize(uint16_t channelWidth,
                                  const RuAllocation &ruAllocation,
                                  bool sigBCompression,
  ...
  auto numRusPerContentChannel = GetNumRusPerHeSigBContentChannel(
...
std::string HePpdu::PrintPayload() const {
...
uint8_t HePpdu::GetChannelWidthEncodingFromMhz(uint16_t channelWidth) {
  if (channelWidth == 160) {
    return 3;
...
uint16_t HePpdu::GetChannelWidthMhzFromEncoding(uint8_t bandwidth) {
  if (bandwidth == 3) {
    return 160;
  } else if (bandwidth == 2) {
    return 80;
...
uint8_t HePpdu::GetGuardIntervalAndNltfEncoding(uint16_t gi, uint8_t nltf) {
  if (gi == 800 && nltf == 1) {
    return 0;
...
uint8_t HePpdu::GetNstsEncodingFromNss(uint8_t nss) {
  NS_ASSERT(nss <= 8);
  return nss - 1;
}

uint8_t HePpdu::GetNssFromNstsEncoding(uint8_t nsts) { return nsts + 1; }

uint8_t HePpdu::GetMuMimoUsersEncoding(uint8_t nUsers) {
  NS_ASSERT(nUsers <= 8);
  return (nUsers - 1);
}

uint8_t HePpdu::GetMuMimoUsersFromEncoding(uint8_t encoding) {
  return (encoding + 1);
}
