...
typedef std::unordered_map<uint16_t /* staId */, Ptr<WifiPsdu> /* PSDU */>
    WifiPsduMap;
...
public:
  ...
  struct UlMuInfo {
    ...
    WifiTxParameters
        txParams; //!< the transmission parameters for the Trigger Frame
  };
...
protected:
  ...
  uint16_t
      m_allowedWidth; //!< the allowed width in MHz for the current transmission
...
private:
  ...
  std::map<uint8_t, LastTxInfo>
      m_lastTxInfo;         ///< Information about the last transmission
  EventId m_accessReqTimer; ///< the timer controlling additional channel access
                            ///< requests
  Time m_accessReqInterval; ///< duration of the interval between channel access
                            ///< requests
  AcIndex m_accessReqAc;    ///< AC we request channel access for
  bool m_restartTimerUponAccess; ///< whether the channel access timer has to be
                                 ///< restarted upon channel access
};
