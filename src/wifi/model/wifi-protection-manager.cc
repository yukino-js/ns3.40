...
TypeId WifiProtectionManager::GetTypeId() {
...
WifiProtectionManager::~WifiProtectionManager() { NS_LOG_FUNCTION_NOARGS(); }

void WifiProtectionManager::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_mac = nullptr;
  Object::DoDispose();
}

void WifiProtectionManager::SetWifiMac(Ptr<WifiMac> mac) {
...
void WifiProtectionManager::SetLinkId(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
  m_linkId = linkId;
}

void WifiProtectionManager::AddUserInfoToMuRts(
    CtrlTriggerHeader &muRts, uint16_t txWidth,
  ...
  NS_ABORT_MSG_IF(m_mac->GetTypeOfStation() != AP,
                  "HE APs only can send MU-RTS");
  auto apMac = StaticCast<ApWifiMac>(m_mac);
  ui.SetAid12(apMac->GetAssociationId(receiver, m_linkId));

  const uint16_t ctsTxWidth = std::min(
      txWidth,
      GetWifiRemoteStationManager()->GetChannelWidthSupported(receiver));
