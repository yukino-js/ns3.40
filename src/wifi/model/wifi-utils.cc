...
double DbToRatio(double dB) { return std::pow(10.0, 0.1 * dB); }

double DbmToW(double dBm) { return std::pow(10.0, 0.1 * (dBm - 30.0)); }

double WToDbm(double w) { return 10.0 * std::log10(w) + 30.0; }

double RatioToDb(double ratio) { return 10.0 * std::log10(ratio); }
...
bool TidToLinkMappingValidForNegType1(const WifiTidLinkMapping &dlLinkMapping,
  ...
  for (const auto &linkMapping :
       {std::cref(dlLinkMapping), std::cref(ulLinkMapping)}) {
