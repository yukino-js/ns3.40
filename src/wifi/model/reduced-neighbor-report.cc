...
WifiInformationElementId ReducedNeighborReport::ElementId() const {
  return IE_REDUCED_NEIGHBOR_REPORT;
}

std::size_t ReducedNeighborReport::GetNNbrApInfoFields() const {
  return m_nbrApInfoFields.size();
}

void ReducedNeighborReport::AddNbrApInfoField() {
  m_nbrApInfoFields.emplace_back();
}

void ReducedNeighborReport::SetOperatingChannel(
    std::size_t nbrApInfoId, const WifiPhyOperatingChannel &channel) {
  ...
  case WIFI_PHY_BAND_5GHZ:
    if (channel.GetWidth() == 20) {
      if (channelNumber == 36 || channelNumber == 40 || channelNumber == 44 ||
        ...
        operatingClass = 118;
      } else if (channelNumber == 100 || channelNumber == 104 ||
                 channelNumber == 108 || channelNumber == 112 ||
                 channelNumber == 116 || channelNumber == 120 ||
                 channelNumber == 124 || channelNumber == 128 ||
                 channelNumber == 132 || channelNumber == 136 ||
                 channelNumber == 140 || channelNumber == 144) {
        operatingClass = 121;
      } else if (channelNumber == 149 || channelNumber == 153 ||
                 channelNumber == 157 || channelNumber == 161 ||
                 channelNumber == 165 || channelNumber == 169 ||
                 channelNumber == 173 || channelNumber == 177 ||
                 channelNumber == 181) {
        operatingClass = 125;
      }
    } else if (channel.GetWidth() == 40) {
      if (channelNumber == 38 || channelNumber == 46) {
        ...
        operatingClass = 119;
      } else if (channelNumber == 102 || channelNumber == 110 ||
      ...
      if (channelNumber == 42 || channelNumber == 58 || channelNumber == 106 ||
          channelNumber == 122 || channelNumber == 138 ||
...
ReducedNeighborReport::GetOperatingChannel(std::size_t nbrApInfoId) const {
  ...
  uint8_t primaryChannelNumber =
      m_nbrApInfoFields.at(nbrApInfoId).channelNumber;
  uint16_t primaryChannelCenterFrequency =
      startingFreq + primaryChannelNumber * 5;
    ...
    if (std::get<2>(channel) == width &&
      ...
      if (band != WIFI_PHY_BAND_2_4GHZ) {
        ...
        case 40:
          ...
          break;
        default:
          NS_ABORT_MSG("No channel of width " << width
  ...
  channel.SetPrimary20Index((primaryChannelLowestFreq - channelLowestFreq) /
                            20);

  return channel;
}

std::size_t ReducedNeighborReport::GetNTbttInformationFields(
    std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
  return m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.size();
}

void ReducedNeighborReport::AddTbttInformationField(std::size_t nbrApInfoId) {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
  m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.emplace_back();
}

void ReducedNeighborReport::WriteTbttInformationLength(
    std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
  ...
  if (it->hasBssid && !it->hasShortSsid && !it->hasBssParams &&
      !it->has20MHzPsd && !it->hasMldParams) {
    length = 7;
  } else if (it->hasBssid && it->hasShortSsid && it->hasBssParams &&
             it->has20MHzPsd && it->hasMldParams) {
    length = 16;
  } else {
...
void ReducedNeighborReport::ReadTbttInformationLength(std::size_t nbrApInfoId) {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
...
void ReducedNeighborReport::SetBssid(std::size_t nbrApInfoId, std::size_t index,
                                     Mac48Address bssid) {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
  NS_ASSERT(index <
            m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.size());
...
bool ReducedNeighborReport::HasBssid(std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());

  return m_nbrApInfoFields.at(nbrApInfoId).hasBssid;
}

Mac48Address ReducedNeighborReport::GetBssid(std::size_t nbrApInfoId,
...
void ReducedNeighborReport::SetShortSsid(std::size_t nbrApInfoId,
                                         std::size_t index,
                                         uint32_t shortSsid) {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
  NS_ASSERT(index <
  ...
  m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.at(index).shortSsid =
      shortSsid;

  m_nbrApInfoFields.at(nbrApInfoId).hasShortSsid = true;
}

bool ReducedNeighborReport::HasShortSsid(std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());

  return m_nbrApInfoFields.at(nbrApInfoId).hasShortSsid;
}

uint32_t ReducedNeighborReport::GetShortSsid(std::size_t nbrApInfoId,
                                             std::size_t index) const {
  NS_ASSERT(HasShortSsid(nbrApInfoId));
  NS_ASSERT(index <
            m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.size());

  return m_nbrApInfoFields.at(nbrApInfoId)
      .tbttInformationSet.at(index)
      .shortSsid;
}

void ReducedNeighborReport::SetBssParameters(std::size_t nbrApInfoId,
                                             std::size_t index,
  ...
  m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.at(index).bssParameters =
      bssParameters;

  m_nbrApInfoFields.at(nbrApInfoId).hasBssParams = true;
}

bool ReducedNeighborReport::HasBssParameters(std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());

  return m_nbrApInfoFields.at(nbrApInfoId).hasBssParams;
}

uint8_t ReducedNeighborReport::GetBssParameters(std::size_t nbrApInfoId,
                                                std::size_t index) const {
...
void ReducedNeighborReport::SetPsd20MHz(std::size_t nbrApInfoId,
                                        std::size_t index, uint8_t psd20MHz) {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
  NS_ASSERT(index <
            m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.size());

  m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.at(index).psd20MHz =
      psd20MHz;

  m_nbrApInfoFields.at(nbrApInfoId).has20MHzPsd = true;
}

bool ReducedNeighborReport::HasPsd20MHz(std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());

  return m_nbrApInfoFields.at(nbrApInfoId).has20MHzPsd;
}

uint8_t ReducedNeighborReport::GetPsd20MHz(std::size_t nbrApInfoId,
                                           std::size_t index) const {
...
void ReducedNeighborReport::SetMldParameters(std::size_t nbrApInfoId,
                                             std::size_t index, uint8_t mldId,
                                             uint8_t linkId,
                                             uint8_t changeCount) {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
  NS_ASSERT(index <
...
bool ReducedNeighborReport::HasMldParameters(std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());

  return m_nbrApInfoFields.at(nbrApInfoId).hasMldParams;
}

uint8_t ReducedNeighborReport::GetMldId(std::size_t nbrApInfoId,
                                        std::size_t index) const {
...
uint8_t ReducedNeighborReport::GetLinkId(std::size_t nbrApInfoId,
                                         std::size_t index) const {
  NS_ASSERT(HasMldParameters(nbrApInfoId));
  NS_ASSERT(index <
            m_nbrApInfoFields.at(nbrApInfoId).tbttInformationSet.size());
  ...
  return m_nbrApInfoFields.at(nbrApInfoId)
             .tbttInformationSet.at(index)
             .mldParameters.linkId &
         0x0f;
}

void ReducedNeighborReport::WriteTbttInformationCount(
    std::size_t nbrApInfoId) const {
  NS_ASSERT(nbrApInfoId < m_nbrApInfoFields.size());
...
uint16_t ReducedNeighborReport::GetInformationFieldSize() const {
  uint16_t size = 0;
...
void ReducedNeighborReport::SerializeInformationField(
...
uint16_t
ReducedNeighborReport::DeserializeInformationField(Buffer::Iterator start,
                                                   uint16_t length) {
  ...
  while (count < length) {
    ...
    m_nbrApInfoFields.back().tbttInfoHdr.tbttInfoCount =
        (tbttInfoHdr >> 4) & 0x000f;
    m_nbrApInfoFields.back().tbttInfoHdr.tbttInfoLength =
        (tbttInfoHdr >> 8) & 0x00ff;
    ...
    for (uint8_t j = 0; j < ReadTbttInformationCount(neighborId); j++) {
      ...
      m_nbrApInfoFields.back().tbttInformationSet.back().neighborApTbttOffset =
          i.ReadU8();
      count++;
      ...
      if (m_nbrApInfoFields.back().hasShortSsid) {
        m_nbrApInfoFields.back().tbttInformationSet.back().shortSsid =
            i.ReadLsbtohU32();
        count += 4;
      }
      if (m_nbrApInfoFields.back().hasBssParams) {
        m_nbrApInfoFields.back().tbttInformationSet.back().bssParameters =
            i.ReadU8();
        count += 1;
      }
      if (m_nbrApInfoFields.back().has20MHzPsd) {
        m_nbrApInfoFields.back().tbttInformationSet.back().psd20MHz =
            i.ReadU8();
        count += 1;
      }
      if (m_nbrApInfoFields.back().hasMldParams) {
        ...
        m_nbrApInfoFields.back()
            ...
            .mldParameters.linkId = other & 0x000f;
        m_nbrApInfoFields.back()
            .tbttInformationSet.back()
