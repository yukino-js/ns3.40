...
SupportedRates::SupportedRates() { NS_LOG_FUNCTION(this); }

void SupportedRates::Print(std::ostream &os) const {
  os << "rates=[";
...
bool AllSupportedRates::IsBasicRate(uint64_t bs) const {
  NS_LOG_FUNCTION(this << bs);
  uint8_t rate = static_cast<uint8_t>(bs / 500000) | 0x80;
  return std::find(rates.m_rates.cbegin(), rates.m_rates.cend(), rate) !=
             rates.m_rates.cend() ||
         (extendedRates && std::find(extendedRates->m_rates.cbegin(),
                                     extendedRates->m_rates.cend(),
                                     rate) != extendedRates->m_rates.cend());
}

void AllSupportedRates::AddSupportedRate(uint64_t bs) {
  NS_LOG_FUNCTION(this << bs);
...
void AllSupportedRates::SetBasicRate(uint64_t bs) {
  NS_LOG_FUNCTION(this << bs);
  NS_ASSERT_MSG(IsBssMembershipSelectorRate(bs) == false, "Invalid rate");
...
void AllSupportedRates::AddBssMembershipSelectorRate(uint64_t bs) {
  NS_LOG_FUNCTION(this << bs);
  ...
  NS_LOG_DEBUG("add BSS membership selector rate " << bs << " as rate "
                                                   << +rate);
}

bool AllSupportedRates::IsSupportedRate(uint64_t bs) const {
  NS_LOG_FUNCTION(this << bs);
  auto rate = static_cast<uint8_t>(bs / 500000);
  for (std::size_t i = 0; i < rates.m_rates.size(); i++) {
  ...
  if (extendedRates) {
    for (std::size_t i = 0; i < extendedRates->m_rates.size(); i++) {
      if (rate == extendedRates->m_rates[i] ||
          (rate | 0x80) == extendedRates->m_rates[i]) {
        return true;
      }
    }
  }
...
bool AllSupportedRates::IsBssMembershipSelectorRate(uint64_t bs) const {
  NS_LOG_FUNCTION(this << bs);
...
uint8_t AllSupportedRates::GetNRates() const {
  return rates.m_rates.size() +
         (extendedRates ? extendedRates->m_rates.size() : 0);
}

uint32_t SupportedRates::GetRate(uint8_t i) const {
  return (m_rates[i] & 0x7f) * 500000;
}

WifiInformationElementId SupportedRates::ElementId() const {
  return IE_SUPPORTED_RATES;
}

uint16_t SupportedRates::GetInformationFieldSize() const {
  return m_rates.size();
}

void SupportedRates::SerializeInformationField(Buffer::Iterator start) const {
  for (const uint8_t rate : m_rates) {
    start.WriteU8(rate);
  }
}

uint16_t SupportedRates::DeserializeInformationField(Buffer::Iterator start,
                                                     uint16_t length) {
  NS_ASSERT(length <= 8);
...
WifiInformationElementId ExtendedSupportedRatesIE::ElementId() const {
