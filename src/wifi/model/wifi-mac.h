...
protected:
  ...
  struct LinkEntity {
    ...
    Ptr<ChannelAccessManager>
        channelAccessManager;            //!< channel access manager object
    Ptr<FrameExchangeManager> feManager; //!< Frame Exchange Manager object
    Ptr<WifiRemoteStationManager>
        stationManager; //!< Remote station manager (rate control,
                        //!< RTS/CTS/fragmentation thresholds etc.)
    bool erpSupported{
        false}; //!< set to \c true iff this WifiMac is to model 802.11g
    bool dsssSupported{
        false}; //!< set to \c true iff this WifiMac is to model 802.11b
  };
...
private:
  ...
  std::map<uint8_t, std::unique_ptr<LinkEntity>>
      m_links;                 //!< ID-indexed map of Link objects
  ...
  TracedCallback<const WifiMacHeader &>
