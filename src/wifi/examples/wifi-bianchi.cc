...
std::map<Mac48Address, uint64_t>
    packetsReceived; ///< Map that stores the total packets received
                     ///< per STA (and addressed to that STA)
std::map<Mac48Address, uint64_t>
    bytesReceived; ///< Map that stores the total bytes received per STA (and
                   ///< addressed to that STA)
std::map<Mac48Address, uint64_t>
    packetsTransmitted; ///< Map that stores the total packets transmitted per
                        ///< STA
std::map<Mac48Address, uint64_t>
...
std::map<Mac48Address, uint64_t>
    psduSucceeded; ///< Map that stores the total number of successfully
                   ///< received PSDUs per STA (including PSDUs not addressed to
                   ///< that STA)
std::map<Mac48Address, uint64_t>
    phyHeaderFailed; ///< Map that stores the total number of
                     ///< unsuccessfuly received PHY headers per STA
std::map<Mac48Address, uint64_t>
    rxEventWhileTxing; ///< Map that stores the number of reception events per
                       ///< STA that occurred while PHY was already transmitting
                       ///< a PPDU
std::map<Mac48Address, uint64_t>
    rxEventWhileRxing; ///< Map that stores the number of reception events per
                       ///< STA that occurred while PHY was already receiving a
                       ///< PPDU
std::map<Mac48Address, uint64_t>
...
std::map<Mac48Address, uint64_t>
    rxEventAbortedByTx; ///< Map that stores the number of reception events
                        ///< aborted per STA because the PHY has started to
                        ///< transmit

std::map<Mac48Address, Time>
    timeFirstReceived; ///< Map that stores the time at which the first packet
                       ///< was received per STA (and the packet is addressed to
                       ///< that STA)
std::map<Mac48Address, Time>
    timeLastReceived; ///< Map that stores the time at which the last packet was
                      ///< received per STA (and the packet is addressed to that
                      ///< STA)
std::map<Mac48Address, Time>
    timeFirstTransmitted; ///< Map that stores the time at which the first
                          ///< packet was transmitted per STA
std::map<Mac48Address, Time>
    timeLastTransmitted; ///< Map that stores the time at which the last
                         ///< packet was transmitted per STA

std::set<uint32_t>
    associated; ///< Contains the IDs of the STAs that successfully associated
...
std::map<std::string /* mode */, std::map<unsigned int /* number of nodes */,
                                          double /* calculated throughput */>>
    bianchiResultsEifs = {
        /* 11b */
        {"DsssRate1Mbps",
         {
...
void IncrementCounter(std::map<Mac48Address, uint64_t> &counter,
                      Mac48Address addr, uint64_t increment = 1) {
 ...
 */
void TracePacketReception(std::string context, Ptr<const Packet> p,
  ...
  if (packet->GetSize() >= pktSize) // ignore non-data frames
  {
    ...
    if (itTimeFirstReceived == timeFirstReceived.end()) {
      timeFirstReceived.insert(
          std::make_pair(hdr.GetAddr2(), Simulator::Now()));
    }
 ...
 */
void CwTrace(std::string context, uint32_t cw, uint8_t /* linkId */) {
  NS_LOG_INFO("CW time=" << Simulator::Now() << " node="
                         << ContextToNodeId(context) << " val=" << cw);
  if (tracing) {
    cwTraceFile << Simulator::Now().GetSeconds() << " "
                << ContextToNodeId(context) << " " << cw << std::endl;
  }
}
...
void BackoffTrace(std::string context, uint32_t newVal, uint8_t /* linkId */) {
  NS_LOG_INFO("Backoff time=" << Simulator::Now() << " node="
                              << ContextToNodeId(context) << " val=" << newVal);
  if (tracing) {
    backoffTraceFile << Simulator::Now().GetSeconds() << " "
                     << ContextToNodeId(context) << " " << newVal << std::endl;
  }
}
...
void PhyRxTrace(std::string context, Ptr<const Packet> p,
  ...
  NS_LOG_INFO("PHY-RX-START time=" << Simulator::Now()
 ...
 */
void PhyRxPayloadTrace(std::string context, WifiTxVector txVector,
                       Time psduDuration) {
  NS_LOG_INFO("PHY-RX-PAYLOAD-START time=" << Simulator::Now() << " node="
 ...
 */
void PhyRxDropTrace(std::string context, Ptr<const Packet> p,
                    WifiPhyRxfailureReason reason) {
  NS_LOG_INFO("PHY-RX-DROP time="
              << Simulator::Now() << " node=" << ContextToNodeId(context)
...
void PhyRxDoneTrace(std::string context, Ptr<const Packet> p) {
  NS_LOG_INFO("PHY-RX-END time=" << Simulator::Now()
                                 << " node=" << ContextToNodeId(context)
                                 << " size=" << p->GetSize());
}
...
void PhyTxTrace(std::string context, Ptr<const Packet> p, double txPowerW) {
  NS_LOG_INFO("PHY-TX-START time="
              << Simulator::Now() << " node=" << ContextToNodeId(context)
              << " size=" << p->GetSize() << " " << txPowerW);
  if (tracing) {
    phyTxTraceFile << Simulator::Now().GetSeconds() << " "
...
void PhyTxDoneTrace(std::string context, Ptr<const Packet> p) {
  NS_LOG_INFO("PHY-TX-END time=" << Simulator::Now()
                                 << " node=" << ContextToNodeId(context) << " "
                                 << p->GetSize());
}
...
void MacTxTrace(std::string context, Ptr<const Packet> p) {
  if (tracing) {
    macTxTraceFile << Simulator::Now().GetSeconds() << " "
                   << ContextToNodeId(context) << " " << p->GetSize()
                   << std::endl;
  }
}
...
void MacRxTrace(std::string context, Ptr<const Packet> p) {
  if (tracing) {
    macRxTraceFile << Simulator::Now().GetSeconds() << " "
                   << ContextToNodeId(context) << " " << p->GetSize()
                   << std::endl;
  }
}
 ...
 */
void SocketSendTrace(std::string context, Ptr<const Packet> p,
                     const Address &addr) {
  if (tracing) {
    socketSendTraceFile << Simulator::Now().GetSeconds() << " "
                        << ContextToNodeId(context) << " " << p->GetSize()
                        << " " << addr << std::endl;
  }
}
...
void AssociationLog(std::string context, Mac48Address address) {
  ...
  if (it == associated.end()) {
    NS_LOG_DEBUG("Association: time=" << Simulator::Now()
                                      << " node=" << nodeId);
...
void DisassociationLog(std::string context, Mac48Address address) {
  uint32_t nodeId = ContextToNodeId(context);
  NS_LOG_DEBUG("Disassociation: time=" << Simulator::Now()
                                       << " node=" << nodeId);
...
public:
  ...
  int Run(const WifiHelper &wifi, const YansWifiPhyHelper &wifiPhy,
          ...
          const YansWifiChannelHelper &wifiChannel, uint32_t trialNumber,
          uint32_t networkSize, Time duration, bool pcap, bool infra,
          uint16_t guardIntervalNs, double distanceM, double apTxPowerDbm,
...
int Experiment::Run(const WifiHelper &helper, const YansWifiPhyHelper &wifiPhy,
                    ...
                    uint32_t trialNumber, uint32_t networkSize, Time duration,
                    bool pcap, bool infra, uint16_t guardIntervalNs,
  ...
  if (infra) {
    ...
    uint64_t beaconInterval = std::min<uint64_t>(
        ...
        (65535 * 1024)); // beacon interval needs to be a multiple of time units
                         // (1024 us)
    mac.SetType("ns3::ApWifiMac", "BeaconInterval",
                TimeValue(MicroSeconds(beaconInterval)), "Ssid",
                SsidValue(ssid));
    ...
    mac.SetType("ns3::StaWifiMac", "MaxMissedBeacons",
                UintegerValue(std::numeric_limits<uint32_t>::max()), "Ssid",
                SsidValue(ssid));
  ...
  std::string txop = StaticCast<WifiNetDevice>(wifiNodes.Get(0)->GetDevice(0))
                             ->GetMac()
                             ->GetQosSupported()
                         ? "BE_Txop"
                         : "Txop";
...
int main(int argc, char *argv[]) {
  ...
  uint32_t verbose =
      0; ///< verbosity level that increases the number of debugging traces
  ...
  bool infra = false;    ///< Flag to enable infrastructure model, ring adhoc
                         ///< network if not set
  std::string workDir =
      "./"; ///< the working directory to store generated files
  ...
  double maxRelativeError =
      0.015; ///< Maximum relative error tolerated between ns-3 results and the
             ///< Bianchi model (used for regression, i.e. when the validate
             ///< flag is set)
  double frequency = 5; ///< The operating frequency band in GHz: 2.4, 5 or 6
  uint16_t channelWidth =
      20; ///< The constant channel width in MHz (only for 11n/ac/ax)
  uint16_t guardIntervalNs =
      800; ///< The guard interval in nanoseconds (800 or 400 for 11n/ac,
           ///< 800 or 1600 or 3200 for 11 ax)
  uint16_t pktInterval =
      1000; ///< The socket packet interval in microseconds (a higher value is
            ///< needed to reach saturation conditions as the channel bandwidth
            ///< or the MCS increases)
  double distance =
      0.001; ///< The distance in meters between the AP and the STAs
  double apTxPower =
      16; ///< The transmit power of the AP in dBm (if infrastructure only)
  double staTxPower =
      16; ///< The transmit power of each STA in dBm (or all STAs if adhoc)
  ...
  cmd.AddValue("infra",
               "True to use infrastructure mode, false to use ring adhoc mode",
               infra);
  cmd.AddValue("workDir", "The working directory used to store generated files",
               workDir);
  cmd.AddValue("phyMode",
               "Set the constant PHY mode string used to transmit frames",
               phyMode);
  cmd.AddValue("standard", "Set the standard (11a, 11b, 11g, 11n, 11ac, 11ax)",
               standard);
  cmd.AddValue("nMinStas", "Minimum number of stations to start with",
               nMinStas);
  cmd.AddValue("nMaxStas", "Maximum number of stations to start with",
               nMaxStas);
  cmd.AddValue("nStepSize", "Number of stations to add at each step",
               nStepSize);
  cmd.AddValue(
      "plotBianchiModel",
  ...
  cmd.AddValue("frequency",
               "Set the operating frequency band in GHz: 2.4, 5 or 6",
               frequency);
  ...
  cmd.AddValue("distance",
               "Set the distance in meters between the AP and the STAs",
               distance);
  cmd.AddValue(
      "apTxPower",
  ...
  cmd.AddValue(
      ...
      staTxPower);
  cmd.AddValue("pktInterval", "Set the socket packet interval in microseconds",
               pktInterval);
  cmd.Parse(argc, argv);
  ...
  std::stringstream ss;
  ss << "wifi-" << standard << "-p-" << pktSize
     << (infra ? "-infrastructure" : "-adhoc") << "-r-" << phyModeStr.str()
     << "-min-" << nMinStas << "-max-" << nMaxStas << "-step-" << nStepSize
     << "-throughput.plt";
  ...
  if (standard == "11a") {
    ...
    if (frequency == 2.4) {
      ...
      NS_FATAL_ERROR("Unsupported frequency band "
                     << frequency << " GHz for standard " << standard);
    }
  ...
  std::string channelStr =
      "{0, " + std::to_string(channelWidth) + ", BAND_" +
  ...
  for (uint32_t n = nMinStas; n <= nMaxStas; n += nStepSize) {
    ...
    for (uint32_t runIndex = 0; runIndex < trials; runIndex++) {
      ...
      if (tracing) {
        ...
        socketSendTraceFile << "# Trial " << runIndex + 1 << " of " << trials
                            << "; " << phyModeStr.str() << " for " << n
                            << " nodes" << std::endl;
      }
      experiment.Run(wifi, wifiPhy, wifiMac, wifiChannel, runIndex, n,
                     Seconds(duration), pcap, infra, guardIntervalNs, distance,
                     apTxPower, staTxPower, MicroSeconds(pktInterval));
      uint32_t k = 0;
      if (bytesReceived.size() != n) {
        NS_FATAL_ERROR("Not all stations got traffic!");
      }
      for (auto it = bytesReceived.begin(); it != bytesReceived.end();
           it++, k++) {
        ...
        uint64_t nodeRxEventAbortedByTx =
            GetCount(rxEventAbortedByTx, it->first);
        uint64_t nodeRxEvents =
            nodePhyHeaderFailures + nodePsduFailures + nodePsduSuccess +
        ...
        std::cout << "Node " << it->first << ": TX packets " << nodeTxPackets
                  ...
                  << "; PSDU failures " << nodePsduFailures << "; PSDU success "
                  << nodePsduSuccess << "; RX events while decoding preamble "
                  << nodeRxEventWhileDecodingPreamble
                  ...
                  << "; total RX events " << nodeRxEvents << "; total events "
                  << nodeTxPackets + nodeRxEvents << "; time first RX " << first
                  << "; time last RX " << last << "; dataTransferDuration "
                  << dataTransferDuration << "; throughput " << nodeThroughput
                  << " Mbps" << std::endl;
      }
    ...
    if (itDifs != bianchiResultsDifs.end()) {
      ...
      if (it != itDifs->second.end()) {
        relativeErrorDifs =
            (std::abs(averageThroughput - it->second) / it->second);
