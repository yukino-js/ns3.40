...
void PsrExperiment::Send() {
  Ptr<WifiPsdu> psdu =
      Create<WifiPsdu>(Create<Packet>(m_input.packetSize), WifiMacHeader());
...
void PsrExperiment::Receive(Ptr<const WifiPsdu> psdu, RxSignalInfo rxSignalInfo,
                            WifiTxVector txVector,
...
PsrExperiment::Input::Input()
    : distance(5.0), txMode("OfdmRate6Mbps"), txPowerLevel(0), packetSize(2304),
      nPackets(400) {}
...
void CollisionExperiment::SendA() const {
  Ptr<WifiPsdu> psdu =
      Create<WifiPsdu>(Create<Packet>(m_input.packetSizeA), WifiMacHeader());
...
void CollisionExperiment::SendB() const {
  Ptr<WifiPsdu> psdu =
      Create<WifiPsdu>(Create<Packet>(m_input.packetSizeB), WifiMacHeader());
...
CollisionExperiment::Input::Input()
    : interval(MicroSeconds(0)), xA(-5), xB(5), txModeA("OfdmRate6Mbps"),
      txModeB("OfdmRate6Mbps"), txPowerLevelA(0), txPowerLevelB(0),
      packetSizeA(2304), packetSizeB(2304), nPackets(400) {}
...
CollisionExperiment::Run(CollisionExperiment::Input input) {
  ...
  for (uint32_t i = 0; i < m_input.nPackets; ++i) {
    Simulator::Schedule(Seconds(i) + m_input.interval,
                        &CollisionExperiment::SendB, this);
  }
  m_txA = txA;
  m_txB = txB;
  Simulator::Run();
...
static void PrintSizeVsRange(int argc, char *argv[]) {
  ...
  cmd.AddValue("TxMode", "The mode to use to send each packet", input.txMode);
  cmd.AddValue("NPackets", "The number of packets to send", input.nPackets);
  cmd.AddValue("TargetPsr", "The psr needed to assume that we are within range",
               targetPsr);
  cmd.Parse(argc, argv);
...
static void PrintPsrVsCollisionInterval(int argc, char *argv[]) {
  ...
  input.nPackets = 100;
  CommandLine cmd(__FILE__);
  cmd.AddValue("NPackets", "The number of packets to send for each transmitter",
               input.nPackets);
