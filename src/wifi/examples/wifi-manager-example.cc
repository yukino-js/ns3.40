...
struct StandardInfo {
  ...
  StandardInfo(std::string name, WifiStandard standard, WifiPhyBand band,
      ...
      : m_name(name), m_standard(standard), m_band(band), m_width(width),
...
void ChangeSignalAndReportRate(Ptr<FixedRssLossModel> rssModel, Step step,
  ...
  NS_LOG_INFO("At time " << Simulator::Now().As(Time::S) << "; selected rate "
                         << (g_intervalRate / 1e6) << "; observed rate "
                         << currentRate << "; setting new power to "
                         << rss - step.stepSize);
  g_intervalBytes = 0;
  Simulator::Schedule(Seconds(step.stepTime), &ChangeSignalAndReportRate,
                      rssModel, step, (rss - step.stepSize), rateDataset,
                      actualDataset);
}
...
int main(int argc, char *argv[]) {
  ...
  if (standard == "802.11b") {
    if (serverChannelWidth == 0) {
      serverChannelWidth =
          GetDefaultChannelWidth(WIFI_STANDARD_80211b, WIFI_PHY_BAND_2_4GHZ);
    }
    ...
    if (serverChannelWidth == 0) {
      serverChannelWidth =
          GetDefaultChannelWidth(WIFI_STANDARD_80211g, WIFI_PHY_BAND_2_4GHZ);
    }
    ...
    NS_ABORT_MSG_IF(serverNss == 0 || serverNss > 4,
                    "Invalid nss " << serverNss << " for standard "
                                   << standard);
    if (clientChannelWidth == 0) {
    ...
    NS_ABORT_MSG_IF(clientNss == 0 || clientNss > 4,
                    "Invalid nss " << clientNss << " for standard "
                                   << standard);
  } else if (standard == "802.11ac") {
    if (serverChannelWidth == 0) {
      serverChannelWidth =
          GetDefaultChannelWidth(WIFI_STANDARD_80211ac, WIFI_PHY_BAND_5GHZ);
    }
    NS_ABORT_MSG_IF(serverChannelWidth != 20 && serverChannelWidth != 40 &&
  ...
  serverStandards = {
      StandardInfo("802.11a", WIFI_STANDARD_80211a, WIFI_PHY_BAND_5GHZ, 20, 3,
                   27, 0, 30, 60),
      StandardInfo("802.11b", WIFI_STANDARD_80211b, WIFI_PHY_BAND_2_4GHZ, 22,
                   -5, 11, -6, 15, 15),
      StandardInfo("802.11g", WIFI_STANDARD_80211g, WIFI_PHY_BAND_2_4GHZ, 20,
                   -5, 27, -6, 30, 60),
      StandardInfo("802.11n-5GHz", WIFI_STANDARD_80211n, WIFI_PHY_BAND_5GHZ,
                   serverChannelWidth, 3, 30, 0, 35, 80 * channelRateFactor),
      StandardInfo("802.11n-2.4GHz", WIFI_STANDARD_80211n, WIFI_PHY_BAND_2_4GHZ,
                   serverChannelWidth, 3, 30, 0, 35, 80 * channelRateFactor),
      StandardInfo("802.11ac", WIFI_STANDARD_80211ac, WIFI_PHY_BAND_5GHZ,
                   serverChannelWidth, 5, 50, 0, 55, 120 * channelRateFactor),
      StandardInfo("802.11p-10MHz", WIFI_STANDARD_80211p, WIFI_PHY_BAND_5GHZ,
                   10, 3, 27, 0, 30, 60),
      StandardInfo("802.11p-5MHz", WIFI_STANDARD_80211p, WIFI_PHY_BAND_5GHZ, 5,
                   3, 27, 0, 30, 60),
      StandardInfo("802.11ax-6GHz", WIFI_STANDARD_80211ax, WIFI_PHY_BAND_6GHZ,
                   serverChannelWidth, 5, 55, 0, 60, 120 * channelRateFactor),
      StandardInfo("802.11ax-5GHz", WIFI_STANDARD_80211ax, WIFI_PHY_BAND_5GHZ,
                   serverChannelWidth, 5, 55, 0, 60, 120 * channelRateFactor),
      StandardInfo("802.11ax-2.4GHz", WIFI_STANDARD_80211ax,
  ...
  for (std::vector<StandardInfo>::size_type i = 0; i != serverStandards.size();
       i++) {
    if (standard == serverStandards[i].m_name) {
      serverSelectedStandard = serverStandards[i];
    }
  }
  for (std::vector<StandardInfo>::size_type i = 0; i != clientStandards.size();
       i++) {
  ...
  steps = static_cast<uint32_t>(
      std::abs(static_cast<double>(clientSelectedStandard.m_snrHigh -
  ...
  plotName += standard;
  dataName += standard;
  if (standard == "802.11n-5GHz" || standard == "802.11n-2.4GHz" ||
      standard == "802.11ac" || standard == "802.11ax-6GHz" ||
  ...
  Config::SetDefault("ns3::WifiRemoteStationManager::MaxSlrc",
                     UintegerValue(maxSlrc));
  Config::SetDefault("ns3::WifiRemoteStationManager::MaxSsrc",
                     UintegerValue(maxSsrc));
  Config::SetDefault("ns3::MinstrelWifiManager::PrintStats",
                     BooleanValue(true));
  Config::SetDefault("ns3::MinstrelWifiManager::PrintSamples",
                     BooleanValue(true));
  ...
  NS_LOG_INFO("Setting initial STA position to "
  ...
  Gnuplot2dDataset rateDataset(clientSelectedStandard.m_name +
                               std::string("-rate selected"));
  Gnuplot2dDataset actualDataset(clientSelectedStandard.m_name +
                                 std::string("-observed"));
  Step step;
  ...
  if (serverSelectedStandard.m_name == "802.11n-5GHz" ||
    ...
    wndServer->GetHeConfiguration()->SetGuardInterval(
        NanoSeconds(serverShortGuardInterval));
    wndClient->GetHeConfiguration()->SetGuardInterval(
