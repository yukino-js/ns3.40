...
InterferenceExperiment::Input::Input()
    : interval(MicroSeconds(0)), xA(-5), xB(5), txModeA("OfdmRate54Mbps"),
      txModeB("OfdmRate54Mbps"), txPowerLevelA(16.0206), txPowerLevelB(16.0206),
      packetSizeA(1500), packetSizeB(1500), channelA(36), channelB(36),
      widthA(20), widthB(20), standard(WIFI_STANDARD_80211a),
      band(WIFI_PHY_BAND_5GHZ), preamble(WIFI_PREAMBLE_LONG),
      captureEnabled(false), captureMargin(0) {}
...
void InterferenceExperiment::Run(InterferenceExperiment::Input input) {
  ...
  Ptr<RangePropagationLossModel> loss =
  ...
  m_txA->SetOperatingChannel(
      WifiPhy::ChannelTuple{input.channelA, 0, (int)(input.band), 0});
  m_txB->SetOperatingChannel(
      WifiPhy::ChannelTuple{input.channelB, 0, (int)(input.band), 0});
  rx->SetOperatingChannel(WifiPhy::ChannelTuple{
      std::max(input.channelA, input.channelB), 0, (int)(input.band), 0});

  rx->TraceConnectWithoutContext(
      "PhyRxDrop", MakeCallback(&InterferenceExperiment::PacketDropped, this));

  Simulator::Schedule(Seconds(0), &InterferenceExperiment::SendA, this);
  Simulator::Schedule(Seconds(0) + input.interval,
                      &InterferenceExperiment::SendB, this);
  ...
  if (checkResults && (m_droppedA == expectRxASuccessful ||
                       m_droppedB == expectRxBSuccessful)) {
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("packetSizeA", "Packet size in bytes of transmitter A",
               input.packetSizeA);
  cmd.AddValue("packetSizeB", "Packet size in bytes of transmitter B",
               input.packetSizeB);
  cmd.AddValue("txPowerA", "TX power level of transmitter A",
               input.txPowerLevelA);
  cmd.AddValue("txPowerB", "TX power level of transmitter B",
               input.txPowerLevelB);
  ...
  cmd.AddValue("txModeB", "Wifi mode used for payload transmission of sender B",
               input.txModeB);
  cmd.AddValue("channelA", "The selected channel number of sender A",
               input.channelA);
  cmd.AddValue("channelB", "The selected channel number of sender B",
               input.channelB);
  cmd.AddValue("widthA", "The selected channel width (MHz) of sender A",
               input.widthA);
  cmd.AddValue("widthB", "The selected channel width (MHz) of sender B",
               input.widthB);
  ...
  } else if (str_preamble == "WIFI_PREAMBLE_HT_MF" &&
             input.standard == WIFI_STANDARD_80211n) {
    input.preamble = WIFI_PREAMBLE_HT_MF;
  } else if (str_preamble == "WIFI_PREAMBLE_VHT_SU" &&
             input.standard == WIFI_STANDARD_80211ac) {
    input.preamble = WIFI_PREAMBLE_VHT_SU;
  } else if (str_preamble == "WIFI_PREAMBLE_HE_SU" &&
             input.standard == WIFI_STANDARD_80211ax) {
