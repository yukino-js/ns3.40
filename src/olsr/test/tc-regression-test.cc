...
TcRegressionTest::TcRegressionTest()
    : TestCase("Test OLSR Topology Control message generation"),
      m_time(Seconds(20)), m_countA(0), m_countB(0), m_countC(0) {}

TcRegressionTest::~TcRegressionTest() {}

void TcRegressionTest::DoRun() {
...
void TcRegressionTest::CreateNodes() {
  ...
  m_rxSocketA =
      DynamicCast<Ipv4RawSocketImpl>(rxSocketFactoryA->CreateSocket());
  m_rxSocketA->SetProtocol(UdpL4Protocol::PROT_NUMBER);
  m_rxSocketA->SetRecvCallback(
      MakeCallback(&TcRegressionTest::ReceivePktProbeA, this));

  Ptr<SocketFactory> rxSocketFactoryB =
      c.Get(1)->GetObject<Ipv4RawSocketFactory>();
  m_rxSocketB =
      DynamicCast<Ipv4RawSocketImpl>(rxSocketFactoryB->CreateSocket());
  m_rxSocketB->SetProtocol(UdpL4Protocol::PROT_NUMBER);
  m_rxSocketB->SetRecvCallback(
      MakeCallback(&TcRegressionTest::ReceivePktProbeB, this));

  Ptr<SocketFactory> rxSocketFactoryC =
      c.Get(2)->GetObject<Ipv4RawSocketFactory>();
  m_rxSocketC =
      DynamicCast<Ipv4RawSocketImpl>(rxSocketFactoryC->CreateSocket());
  m_rxSocketC->SetProtocol(UdpL4Protocol::PROT_NUMBER);
  m_rxSocketC->SetRecvCallback(
...
void TcRegressionTest::ReceivePktProbeA(Ptr<Socket> socket) {
  ...
  if (m_countA == 0) {
    ...
    NS_TEST_EXPECT_MSG_EQ(msgHdr.GetOriginatorAddress(),
                          Ipv4Address("10.1.1.2"), "Originator address.");
    NS_TEST_EXPECT_MSG_EQ(hello.linkMessages.size(), 0,
                          "0 - Hello, No messages.");
  } else if (m_countA == 1) {
    MessageHeader msgHdr;
    receivedPacketProbe->RemoveHeader(msgHdr);
    NS_TEST_EXPECT_MSG_EQ(msgHdr.GetOriginatorAddress(),
                          Ipv4Address("10.1.1.2"), "Originator address.");
    const olsr::MessageHeader::Hello &hello = msgHdr.GetHello();
    NS_TEST_EXPECT_MSG_EQ(hello.linkMessages.size(), 2,
                          "1 - Hello, one message.");
    NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[0].linkCode, 1,
                          "1 - Asymmetric Link.");
    NS_TEST_EXPECT_MSG_EQ(
        hello.linkMessages[0].neighborInterfaceAddresses.size(), 1,
        "1 - Neighbor.");
    NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[0].neighborInterfaceAddresses[0],
                          Ipv4Address("10.1.1.3"), "1 - Neighbor.");
    NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[1].linkCode, 1,
                          "1 - Asymmetric Link.");
    NS_TEST_EXPECT_MSG_EQ(
        hello.linkMessages[1].neighborInterfaceAddresses.size(), 1,
        "1 - Neighbor.");
    NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[1].neighborInterfaceAddresses[0],
                          Ipv4Address("10.1.1.1"), "1 - Neighbor.");
  } else {
    if (m_countA == 5 || m_countA == 8) {
      ...
      NS_TEST_EXPECT_MSG_EQ(msgHdr.GetOriginatorAddress(),
                            Ipv4Address("10.1.1.2"), "Originator address.");
      NS_TEST_EXPECT_MSG_EQ(tc.neighborAddresses.size(), 2,
                            int(m_countA) << " - TC, one message.");
      NS_TEST_EXPECT_MSG_EQ(tc.neighborAddresses[0], Ipv4Address("10.1.1.3"),
                            int(m_countA) << " - Neighbor.");
      NS_TEST_EXPECT_MSG_EQ(tc.neighborAddresses[1], Ipv4Address("10.1.1.1"),
                            int(m_countA) << " - Neighbor.");
    }
    if (m_countA != 8) {
      MessageHeader msgHdr;
      ...
      NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[0].linkCode, 6,
      ...
      NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[0].neighborInterfaceAddresses[0],
                            ...
                            int(m_countA) << " - Neighbor.");
      NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[1].linkCode, 6,
                            int(m_countA) << " - Symmetric Link.");
      NS_TEST_EXPECT_MSG_EQ(
          hello.linkMessages[1].neighborInterfaceAddresses.size(), 1,
          int(m_countA) << " - Neighbor.");
...
void TcRegressionTest::ReceivePktProbeB(Ptr<Socket> socket) {
  uint32_t availableData;
  availableData = socket->GetRxAvailable();
  Ptr<Packet> receivedPacketProbe =
      socket->Recv(std::numeric_limits<uint32_t>::max(), 0);
  ...
  if (m_countB == 0 || m_countB == 2 || m_countB == 5 || m_countB == 6 ||
      m_countB == 8 || m_countB == 10 || m_countB == 13 || m_countB == 15 ||
      m_countB == 17 || m_countB == 19) {
    NS_TEST_EXPECT_MSG_EQ(msgHdr.GetOriginatorAddress(),
                          Ipv4Address("10.1.1.3"), "Originator address.");
  } else {
    NS_TEST_EXPECT_MSG_EQ(msgHdr.GetOriginatorAddress(),
                          Ipv4Address("10.1.1.1"), "Originator address.");
  }
  ...
  if (m_countB == 0 || m_countB == 1) {
    ...
    NS_TEST_EXPECT_MSG_EQ(hello.linkMessages.size(), 1,
                          int(m_countC) << " - Hello, links announced.");
    if (m_countB == 2 || m_countB == 3) {
      ...
      NS_TEST_EXPECT_MSG_EQ(hello.linkMessages[0].linkCode, 10,
