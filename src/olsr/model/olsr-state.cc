...
void OlsrState::EraseMprSelectorTuple(const MprSelectorTuple &tuple) {
  for (auto it = m_mprSelectorSet.begin(); it != m_mprSelectorSet.end(); it++) {
    if (*it == tuple) {
...
void OlsrState::EraseMprSelectorTuples(const Ipv4Address &mainAddr) {
...
void OlsrState::InsertMprSelectorTuple(const MprSelectorTuple &tuple) {
...
std::string OlsrState::PrintMprSelectorSet() const {
  ...
  os << "[";
  for (auto iter = m_mprSelectorSet.begin(); iter != m_mprSelectorSet.end();
       iter++) {
    auto next = iter;
    next++;
    os << iter->mainAddr;
...
OlsrState::FindSymNeighborTuple(const Ipv4Address &mainAddr) const {
  for (auto it = m_neighborSet.begin(); it != m_neighborSet.end(); it++) {
    if (it->neighborMainAddr == mainAddr &&
        it->status == NeighborTuple::STATUS_SYM) {
      return &(*it);
    }
  }
  return nullptr;
...
void OlsrState::EraseNeighborTuple(const NeighborTuple &tuple) {
  for (auto it = m_neighborSet.begin(); it != m_neighborSet.end(); it++) {
    if (*it == tuple) {
...
void OlsrState::EraseNeighborTuple(const Ipv4Address &mainAddr) {
  for (auto it = m_neighborSet.begin(); it != m_neighborSet.end(); it++) {
    if (it->neighborMainAddr == mainAddr) {
...
void OlsrState::EraseTwoHopNeighborTuple(const TwoHopNeighborTuple &tuple) {
  for (auto it = m_twoHopNeighborSet.begin(); it != m_twoHopNeighborSet.end();
       it++) {
    if (*it == tuple) {
...
void OlsrState::EraseTwoHopNeighborTuples(
    const Ipv4Address &neighborMainAddr,
    const Ipv4Address &twoHopNeighborAddr) {
  for (auto it = m_twoHopNeighborSet.begin();
...
void OlsrState::EraseTwoHopNeighborTuples(const Ipv4Address &neighborMainAddr) {
  for (auto it = m_twoHopNeighborSet.begin();
       it != m_twoHopNeighborSet.end();) {
    if (it->neighborMainAddr == neighborMainAddr) {
      it = m_twoHopNeighborSet.erase(it);
...
LinkTuple *OlsrState::FindSymLinkTuple(const Ipv4Address &ifaceAddr, Time now) {
  for (auto it = m_linkSet.begin(); it != m_linkSet.end(); it++) {
    if (it->neighborIfaceAddr == ifaceAddr) {
      if (it->symTime > now) {
        return &(*it);
...
void OlsrState::EraseLinkTuple(const LinkTuple &tuple) {
  for (auto it = m_linkSet.begin(); it != m_linkSet.end(); it++) {
    if (*it == tuple) {
      m_linkSet.erase(it);
      break;
...
TopologyTuple *OlsrState::FindNewerTopologyTuple(const Ipv4Address &lastAddr,
                                                 uint16_t ansn) {
  for (auto it = m_topologySet.begin(); it != m_topologySet.end(); it++) {
...
void OlsrState::EraseTopologyTuple(const TopologyTuple &tuple) {
  for (auto it = m_topologySet.begin(); it != m_topologySet.end(); it++) {
    if (*it == tuple) {
...
void OlsrState::EraseOlderTopologyTuples(const Ipv4Address &lastAddr,
                                         uint16_t ansn) {
...
void OlsrState::InsertTopologyTuple(const TopologyTuple &tuple) {
...
void OlsrState::EraseIfaceAssocTuple(const IfaceAssocTuple &tuple) {
  for (auto it = m_ifaceAssocSet.begin(); it != m_ifaceAssocSet.end(); it++) {
    if (*it == tuple) {
...
void OlsrState::InsertIfaceAssocTuple(const IfaceAssocTuple &tuple) {
...
void OlsrState::EraseAssociationTuple(const AssociationTuple &tuple) {
  for (auto it = m_associationSet.begin(); it != m_associationSet.end(); it++) {
    if (*it == tuple) {
...
void OlsrState::InsertAssociationTuple(const AssociationTuple &tuple) {
  m_associationSet.push_back(tuple);
}

void OlsrState::EraseAssociation(const Association &tuple) {
...
void OlsrState::InsertAssociation(const Association &tuple) {
