...
struct RoutingTableEntry {
  ...
  RoutingTableEntry()
      : // default values
        destAddr(), nextAddr(), interface(0), distance(0) {}
};
...
private:
  std::set<uint32_t>
      m_interfaceExclusions; //!< Set of interfaces excluded by OSLR.
  Ptr<Ipv4StaticRouting>
      m_routingTableAssociation; //!< Associations from an Ipv4StaticRouting
                                 //!< instance
...
private:
  std::map<Ipv4Address, RoutingTableEntry>
      m_table; //!< Data structure for the routing table.
...
private:
  ...
  std::map<Ptr<Socket>, Ipv4InterfaceAddress>
      m_sendSockets; //!< Container of sockets and the interfaces they are
                     //!< opened onto.
