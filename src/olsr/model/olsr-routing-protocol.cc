  ...
  static TypeId tid =
      TypeId("ns3::olsr::RoutingProtocol")
          ...
          .AddAttribute("Willingness",
                        ...
                        MakeEnumChecker(Willingness::NEVER, "never",
                                        Willingness::LOW, "low",
                                        Willingness::DEFAULT, "default",
                                        Willingness::HIGH, "high",
                                        Willingness::ALWAYS, "always"))
          .AddTraceSource(
...
RoutingProtocol::RoutingProtocol()
    : m_routingTableAssociation(nullptr), m_ipv4(nullptr),
      m_helloTimer(Timer::CANCEL_ON_DESTROY),
      m_tcTimer(Timer::CANCEL_ON_DESTROY), m_midTimer(Timer::CANCEL_ON_DESTROY),
  ...
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces(); i++) {
    ...
    if (!m_recvSocket) {
      ...
      m_recvSocket->SetRecvCallback(
          MakeCallback(&RoutingProtocol::RecvOlsr, this));
    ...
    socket->SetIpTtl(1);
    InetSocketAddress inetAddr(m_ipv4->GetAddress(i, 0).GetLocal(),
                               OLSR_PORT_NUMBER);
...
void RoutingProtocol::RecvOlsr(Ptr<Socket> socket) {
  ...
  if (m_interfaceExclusions.find(recvInterfaceIndex) !=
  ...
  NS_ASSERT(olsrPacketHeader.GetPacketLength() >=
            olsrPacketHeader.GetSerializedSize());
  uint32_t sizeLeft =
...
void RoutingProtocol::MprComputation() {
  ...
  while (N2.begin() != N2.end()) {
    ...
    for (auto it = rs.begin(); it != rs.end(); it++) {
      ...
      for (auto it2 = reachability[r].begin(); it2 != reachability[r].end();
           it2++) {
...
Ipv4Address RoutingProtocol::GetMainAddress(Ipv4Address iface_addr) const {
...
void RoutingProtocol::RoutingTableComputation() {
  ...
  for (auto it = neighborSet.begin(); it != neighborSet.end(); it++) {
    ...
    if (nb_tuple.status == NeighborTuple::STATUS_SYM) {
      ...
      for (auto it2 = linkSet.begin(); it2 != linkSet.end(); it2++) {
        ...
        if ((GetMainAddress(link_tuple.neighborIfaceAddr) ==
          ...
          NS_LOG_LOGIC("Link tuple: linkMainAddress= "
                       ...
                       << "; expired="
                       << int(link_tuple.time < Simulator::Now())
                       << " => IGNORE");
        }
      }
      ...
      if (!nb_main_addr && lt != nullptr) {
        ...
        AddEntry(nb_tuple.neighborMainAddr, lt->neighborIfaceAddr,
                 lt->localIfaceAddr, 1);
      }
    }
  }
  ...
  for (auto it = twoHopNeighbors.begin(); it != twoHopNeighbors.end(); it++) {
    ...
    if (foundEntry) {
      ...
      AddEntry(nb2hop_tuple.twoHopNeighborAddr, entry.nextAddr, entry.interface,
               2);
    } else {
  ...
  for (auto it = associationSet.begin(); it != associationSet.end(); it++) {
    ...
    if (routeIndex == m_hnaRoutingTable->GetNRoutes()) {
      addRoute = true;
    } else if (gatewayEntryExists && m_hnaRoutingTable->GetMetric(routeIndex) >
    ...
    if (addRoute && gatewayEntryExists) {
      m_hnaRoutingTable->AddNetworkRouteTo(
          tuple.networkAddr, tuple.netmask, gatewayEntry.nextAddr,
          gatewayEntry.interface, gatewayEntry.distance);
    }
  }
...
void RoutingProtocol::ProcessHello(const olsr::MessageHeader &msg,
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    NS_LOG_DEBUG(Simulator::Now().As(Time::S)
                 ...
                 << m_mainAddress);
    for (auto tuple = twoHopNeighbors.begin(); tuple != twoHopNeighbors.end();
         tuple++) {
      NS_LOG_DEBUG(*tuple);
    }
...
void RoutingProtocol::ProcessTc(const olsr::MessageHeader &msg,
                                const Ipv4Address &senderIface) {
...
void RoutingProtocol::ProcessMid(const olsr::MessageHeader &msg,
  ...
  for (auto twoHopNeighbor = twoHopNeighbors.begin();
       twoHopNeighbor != twoHopNeighbors.end(); twoHopNeighbor++) {
    twoHopNeighbor->neighborMainAddr =
        GetMainAddress(twoHopNeighbor->neighborMainAddr);
    twoHopNeighbor->twoHopNeighborAddr =
...
void RoutingProtocol::ProcessHna(const olsr::MessageHeader &msg,
                                 const Ipv4Address &senderIface) {
...
void RoutingProtocol::ForwardDefault(olsr::MessageHeader olsrMessage,
                                     DuplicateTuple *duplicated,
...
void RoutingProtocol::QueueMessage(const olsr::MessageHeader &message,
                                   Time delay) {
...
void RoutingProtocol::SendPacket(Ptr<Packet> packet,
  ...
  for (auto i = m_sendSockets.begin(); i != m_sendSockets.end(); i++) {
    ...
    Ipv4Address bcast =
        i->second.GetLocal().GetSubnetDirectedBroadcast(i->second.GetMask());
...
void RoutingProtocol::SendQueuedMessages() {
  ...
  for (auto message = m_queuedMessages.begin();
...
void RoutingProtocol::SendHello() {
  ...
  std::vector<olsr::MessageHeader::Hello::LinkMessage> &linkMessages =
      hello.linkMessages;

  const LinkSet &links = m_state.GetLinks();
  for (auto link_tuple = links.begin(); link_tuple != links.end();
       link_tuple++) {
    ...
    linkMessage.neighborInterfaceAddresses.insert(
        linkMessage.neighborInterfaceAddresses.end(), interfaces.begin(),
        interfaces.end());

    linkMessages.push_back(linkMessage);
  }
  NS_LOG_DEBUG("OLSR HELLO message size: "
...
void RoutingProtocol::SendHna() {
  ...
  for (auto it = localHnaAssociations.begin(); it != localHnaAssociations.end();
       it++) {
    olsr::MessageHeader::Hna::Association assoc = {it->networkAddr,
                                                   it->netmask};
...
void RoutingProtocol::AddHostNetworkAssociation(Ipv4Address networkAddr,
  ...
  for (auto assocIterator = localHnaAssociations.begin();
       assocIterator != localHnaAssociations.end(); assocIterator++) {
    const Association &localHnaAssoc = *assocIterator;
    if (localHnaAssoc.networkAddr == networkAddr &&
        localHnaAssoc.netmask == netmask) {
      NS_LOG_INFO("HNA association for network "
...
void RoutingProtocol::RemoveHostNetworkAssociation(Ipv4Address networkAddr,
                                                   Ipv4Mask netmask) {
  NS_LOG_INFO("Removing HNA association for network " << networkAddr << "/"
                                                      << netmask << ".");
    ...
    for (auto neighIfaceAddr = linkMessage->neighborInterfaceAddresses.begin();
      ...
      if (*neighIfaceAddr == receiverIface) {
        if (linkType == LinkType::LOST_LINK) {
          ...
          updated = true;
        } else if (linkType == LinkType::SYM_LINK ||
                   linkType == LinkType::ASYM_LINK) {
          NS_LOG_DEBUG(*link_tuple
        ...
        NS_LOG_DEBUG("     \\-> *neighIfaceAddr ("
                     << *neighIfaceAddr << " != receiverIface ("
                     << receiverIface << ") => IGNORING!");
      }
    }
...
void RoutingProtocol::PopulateNeighborSet(
    const olsr::MessageHeader &msg, const olsr::MessageHeader::Hello &hello) {
...
void RoutingProtocol::PopulateTwoHopNeighborSet(
    const olsr::MessageHeader &msg, const olsr::MessageHeader::Hello &hello) {
  Time now = Simulator::Now();
  ...
  for (auto link_tuple = m_state.GetLinks().begin();
...
void RoutingProtocol::PopulateMprSelectorSet(
    const olsr::MessageHeader &msg, const olsr::MessageHeader::Hello &hello) {
  NS_LOG_FUNCTION(this);
  ...
  NS_LOG_DEBUG("Computed MPR selector set for node "
...
void RoutingProtocol::NeighborLoss(const LinkTuple &tuple) {
  NS_LOG_DEBUG(Simulator::Now().As(Time::S)
               << ": OLSR Node " << m_mainAddress << " LinkTuple "
...
void RoutingProtocol::RemoveLinkTuple(const LinkTuple &tuple) {
  NS_LOG_DEBUG(Simulator::Now().As(Time::S)
...
void RoutingProtocol::AddAssociationTuple(const AssociationTuple &tuple) {
  m_state.InsertAssociationTuple(tuple);
}

void RoutingProtocol::RemoveAssociationTuple(const AssociationTuple &tuple) {
  m_state.EraseAssociationTuple(tuple);
}

uint16_t RoutingProtocol::GetPacketSequenceNumber() {
  m_packetSequenceNumber =
      (m_packetSequenceNumber + 1) % (OLSR_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t RoutingProtocol::GetMessageSequenceNumber() {
  m_messageSequenceNumber =
      (m_messageSequenceNumber + 1) % (OLSR_MAX_SEQ_NUM + 1);
...
void RoutingProtocol::DupTupleTimerExpire(Ipv4Address address,
...
void RoutingProtocol::LinkTupleTimerExpire(Ipv4Address neighborIfaceAddr) {
  ...
  if (tuple->time < now) {
    ...
    m_events.Track(Simulator::Schedule(DELAY(tuple->time),
                                       ...
                                       this, neighborIfaceAddr));
  } else {
    m_events.Track(Simulator::Schedule(
        DELAY(std::min(tuple->time, tuple->symTime)),
        &RoutingProtocol::LinkTupleTimerExpire, this, neighborIfaceAddr));
  }
}

void RoutingProtocol::Nb2hopTupleTimerExpire(Ipv4Address neighborMainAddr,
  ...
  if (tuple->expirationTime < Simulator::Now()) {
    ...
    m_events.Track(Simulator::Schedule(
        DELAY(tuple->expirationTime), &RoutingProtocol::Nb2hopTupleTimerExpire,
        this, neighborMainAddr, twoHopNeighborAddr));
  }
}

void RoutingProtocol::MprSelTupleTimerExpire(Ipv4Address mainAddr) {
...
void RoutingProtocol::TopologyTupleTimerExpire(Ipv4Address destAddr,
...
void RoutingProtocol::IfaceAssocTupleTimerExpire(Ipv4Address ifaceAddr) {
  ...
  if (tuple->time < Simulator::Now()) {
    ...
    m_events.Track(Simulator::Schedule(
        DELAY(tuple->time), &RoutingProtocol::IfaceAssocTupleTimerExpire, this,
        ifaceAddr));
  }
}

void RoutingProtocol::AssociationTupleTimerExpire(Ipv4Address gatewayAddr,
...
void RoutingProtocol::RemoveEntry(const Ipv4Address &dest) {
  m_table.erase(dest);
}
...
bool RoutingProtocol::FindSendEntry(const RoutingTableEntry &entry,
                                    RoutingTableEntry &outEntry) const {
  outEntry = entry;
...
void RoutingProtocol::AddEntry(const Ipv4Address &dest, const Ipv4Address &next,
...
void RoutingProtocol::AddEntry(const Ipv4Address &dest, const Ipv4Address &next,
                               ...
                               uint32_t distance) {
  NS_LOG_FUNCTION(this << dest << next << interfaceAddress << distance
                       << m_mainAddress);
...
MprSet RoutingProtocol::GetMprSet() const { return m_state.GetMprSet(); }

const MprSelectorSet &RoutingProtocol::GetMprSelectors() const {
  return m_state.GetMprSelectors();
}

const NeighborSet &RoutingProtocol::GetNeighbors() const {
  return m_state.GetNeighbors();
}

const TwoHopNeighborSet &RoutingProtocol::GetTwoHopNeighbors() const {
  return m_state.GetTwoHopNeighbors();
}

const TopologySet &RoutingProtocol::GetTopologySet() const {
  return m_state.GetTopologySet();
}

const OlsrState &RoutingProtocol::GetOlsrState() const { return m_state; }
...
void RoutingProtocol::Dump() {
  ...
  for (auto iter = m_state.GetNeighbors().begin();
