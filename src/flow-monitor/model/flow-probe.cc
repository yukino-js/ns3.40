...
TypeId FlowProbe::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::FlowProbe").SetParent<Object>().SetGroupName("FlowMonitor")
...
void FlowProbe::DoDispose() {
  m_flowMonitor = nullptr;
  Object::DoDispose();
}

void FlowProbe::AddPacketStats(FlowId flowId, uint32_t packetSize,
...
void FlowProbe::AddPacketDropStats(FlowId flowId, uint32_t packetSize,
                                   uint32_t reasonCode) {
  FlowStats &flow = m_stats[flowId];
...
FlowProbe::Stats FlowProbe::GetStats() const { return m_stats; }

void FlowProbe::SerializeToXmlStream(std::ostream &os, uint16_t indent,
  ...
  for (auto iter = m_stats.begin(); iter != m_stats.end(); iter++) {
    ...
    for (uint32_t reasonCode = 0; reasonCode < iter->second.bytesDropped.size();
         reasonCode++) {
      os << std::string(indent, ' ');
