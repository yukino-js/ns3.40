...
TypeId Ipv6FlowProbeTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t Ipv6FlowProbeTag::GetSerializedSize() const { return 4 + 4 + 4; }

void Ipv6FlowProbeTag::Serialize(TagBuffer buf) const {
  buf.WriteU32(m_flowId);
  buf.WriteU32(m_packetId);
  buf.WriteU32(m_packetSize);
}

void Ipv6FlowProbeTag::Deserialize(TagBuffer buf) {
  m_flowId = buf.ReadU32();
  m_packetId = buf.ReadU32();
  m_packetSize = buf.ReadU32();
}

void Ipv6FlowProbeTag::Print(std::ostream &os) const {
  os << "FlowId=" << m_flowId;
  os << "PacketId=" << m_packetId;
  os << "PacketSize=" << m_packetSize;
}

Ipv6FlowProbeTag::Ipv6FlowProbeTag() : Tag() {}

Ipv6FlowProbeTag::Ipv6FlowProbeTag(uint32_t flowId, uint32_t packetId,
                                   uint32_t packetSize)
    : Tag(), m_flowId(flowId), m_packetId(packetId), m_packetSize(packetSize) {}

void Ipv6FlowProbeTag::SetFlowId(uint32_t id) { m_flowId = id; }

void Ipv6FlowProbeTag::SetPacketId(uint32_t id) { m_packetId = id; }

void Ipv6FlowProbeTag::SetPacketSize(uint32_t size) { m_packetSize = size; }

uint32_t Ipv6FlowProbeTag::GetFlowId() const { return m_flowId; }

uint32_t Ipv6FlowProbeTag::GetPacketId() const { return m_packetId; }
...
Ipv6FlowProbe::Ipv6FlowProbe(Ptr<FlowMonitor> monitor,
  ...
  if (!ipv6->TraceConnectWithoutContext(
          "SendOutgoing", MakeCallback(&Ipv6FlowProbe::SendOutgoingLogger,
                                       Ptr<Ipv6FlowProbe>(this)))) {
    NS_FATAL_ERROR("trace fail");
  }
  if (!ipv6->TraceConnectWithoutContext(
          "UnicastForward", MakeCallback(&Ipv6FlowProbe::ForwardLogger,
                                         Ptr<Ipv6FlowProbe>(this)))) {
    NS_FATAL_ERROR("trace fail");
  }
  if (!ipv6->TraceConnectWithoutContext(
          "LocalDeliver", MakeCallback(&Ipv6FlowProbe::ForwardUpLogger,
                                       Ptr<Ipv6FlowProbe>(this)))) {
    NS_FATAL_ERROR("trace fail");
  }
...
void Ipv6FlowProbe::DoDispose() { FlowProbe::DoDispose(); }

void Ipv6FlowProbe::SendOutgoingLogger(const Ipv6Header &ipHeader,
...
void Ipv6FlowProbe::ForwardLogger(const Ipv6Header &ipHeader,
                                  Ptr<const Packet> ipPayload,
                                  uint32_t interface) {
...
void Ipv6FlowProbe::ForwardUpLogger(const Ipv6Header &ipHeader,
                                    Ptr<const Packet> ipPayload,
                                    uint32_t interface) {
  Ipv6FlowProbeTag fTag;
  bool found = ipPayload->FindFirstMatchingByteTag(fTag);
...
void Ipv6FlowProbe::DropLogger(const Ipv6Header &ipHeader,
                               Ptr<const Packet> ipPayload,
                               Ipv6L3Protocol::DropReason reason,
                               Ptr<Ipv6> ipv6, uint32_t ifIndex) {
#if 0
  switch (reason)
    {
...
void Ipv6FlowProbe::QueueDropLogger(Ptr<const Packet> ipPayload) {
  Ipv6FlowProbeTag fTag;
...
void Ipv6FlowProbe::QueueDiscDropLogger(Ptr<const QueueDiscItem> item) {
  Ipv6FlowProbeTag fTag;
