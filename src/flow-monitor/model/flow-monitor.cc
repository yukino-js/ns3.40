...
TypeId FlowMonitor::GetInstanceTypeId() const { return GetTypeId(); }

FlowMonitor::FlowMonitor() : m_enabled(false) {
...
inline FlowMonitor::FlowStats &FlowMonitor::GetStatsForFlow(FlowId flowId) {
  NS_LOG_FUNCTION(this);
...
void FlowMonitor::ReportFirstTx(Ptr<FlowProbe> probe, uint32_t flowId,
                                uint32_t packetId, uint32_t packetSize) {
  ...
  NS_LOG_DEBUG("ReportFirstTx: adding tracked packet (flowId="
...
void FlowMonitor::ReportForwarding(Ptr<FlowProbe> probe, uint32_t flowId,
                                   uint32_t packetId, uint32_t packetSize) {
  NS_LOG_FUNCTION(this << probe << flowId << packetId << packetSize);
  if (!m_enabled) {
    NS_LOG_DEBUG("FlowMonitor not enabled; returning");
...
void FlowMonitor::ReportLastRx(Ptr<FlowProbe> probe, uint32_t flowId,
                               uint32_t packetId, uint32_t packetSize) {
  NS_LOG_FUNCTION(this << probe << flowId << packetId << packetSize);
  if (!m_enabled) {
    NS_LOG_DEBUG("FlowMonitor not enabled; returning");
...
void FlowMonitor::ReportDrop(Ptr<FlowProbe> probe, uint32_t flowId,
                             uint32_t packetId, uint32_t packetSize,
                             uint32_t reasonCode) {
  NS_LOG_FUNCTION(this << probe << flowId << packetId << packetSize
                       << reasonCode);
  ...
  NS_LOG_DEBUG("++stats.packetsDropped[" << reasonCode << "]; // becomes: "
                                         << stats.packetsDropped[reasonCode]);
...
const FlowMonitor::FlowStatsContainer &FlowMonitor::GetFlowStats() const {
  return m_flowStats;
}
...
void FlowMonitor::PeriodicCheckForLostPackets() {
  CheckForLostPackets();
  Simulator::Schedule(PERIODIC_CHECK_INTERVAL,
                      &FlowMonitor::PeriodicCheckForLostPackets, this);
}
...
const FlowMonitor::FlowProbeContainer &FlowMonitor::GetAllProbes() const {
  return m_flowProbes;
}
...
void FlowMonitor::SerializeToXmlStream(std::ostream &os, uint16_t indent,
...
#define ATTRIB_TIME(name)                                                      \
  << " " #name "=\"" << flowI->second.name.As(Time::NS) << "\""
    os << "<Flow flowId=\"" << flowI->first
       << "\"" ATTRIB_TIME(timeFirstTxPacket) ATTRIB_TIME(timeFirstRxPacket)
              ATTRIB_TIME(timeLastTxPacket) ATTRIB_TIME(timeLastRxPacket)
                  ATTRIB_TIME(delaySum) ATTRIB_TIME(jitterSum)
                      ATTRIB_TIME(lastDelay) ATTRIB(txBytes) ATTRIB(rxBytes)
...
#undef ATTRIB
    ...
    for (uint32_t reasonCode = 0;
         reasonCode < flowI->second.packetsDropped.size(); reasonCode++) {
      os << std::string(indent, ' ');
    ...
    for (uint32_t reasonCode = 0;
         reasonCode < flowI->second.bytesDropped.size(); reasonCode++) {
      os << std::string(indent, ' ');
      os << "<bytesDropped reasonCode=\"" << reasonCode << "\""
         << " bytes=\"" << flowI->second.bytesDropped[reasonCode] << "\" />\n";
    }
    if (enableHistograms) {
      flowI->second.delayHistogram.SerializeToXmlStream(os, indent,
                                                        "delayHistogram");
      flowI->second.jitterHistogram.SerializeToXmlStream(os, indent,
                                                         "jitterHistogram");
      flowI->second.packetSizeHistogram.SerializeToXmlStream(
          os, indent, "packetSizeHistogram");
...
std::string FlowMonitor::SerializeToXmlString(uint16_t indent,
                                              bool enableHistograms,
                                              bool enableProbes) {
  NS_LOG_FUNCTION(this << indent << enableHistograms << enableProbes);
...
void FlowMonitor::SerializeToXmlFile(std::string fileName,
...
void FlowMonitor::ResetAllStats() {
  NS_LOG_FUNCTION(this);
