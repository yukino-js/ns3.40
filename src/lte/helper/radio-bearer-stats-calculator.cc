...
TypeId RadioBearerStatsCalculator::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::RadioBearerStatsCalculator")
          ...
          .AddAttribute(
              ...
              MakeStringAccessor(
                  &RadioBearerStatsCalculator::SetDlPdcpOutputFilename),
              MakeStringChecker())
          .AddAttribute(
              ...
              MakeStringAccessor(
                  &RadioBearerStatsCalculator::SetUlPdcpOutputFilename),
              MakeStringChecker());
  return tid;
}

void RadioBearerStatsCalculator::DoDispose() {
  NS_LOG_FUNCTION(this);
  if (m_pendingOutput) {
    ShowResults();
  }
...
void RadioBearerStatsCalculator::SetStartTime(Time t) {
  m_startTime = t;
  RescheduleEndEpoch();
}

Time RadioBearerStatsCalculator::GetStartTime() const { return m_startTime; }

void RadioBearerStatsCalculator::SetEpoch(Time e) {
  m_epochDuration = e;
  RescheduleEndEpoch();
}

Time RadioBearerStatsCalculator::GetEpoch() const { return m_epochDuration; }

void RadioBearerStatsCalculator::UlTxPdu(uint16_t cellId, uint64_t imsi,
                                         uint16_t rnti, uint8_t lcid,
                                         uint32_t packetSize) {
  NS_LOG_FUNCTION(this << "UlTxPDU" << cellId << imsi << rnti << (uint32_t)lcid
                       << packetSize);
...
void RadioBearerStatsCalculator::DlTxPdu(uint16_t cellId, uint64_t imsi,
                                         uint16_t rnti, uint8_t lcid,
                                         uint32_t packetSize) {
  NS_LOG_FUNCTION(this << "DlTxPDU" << cellId << imsi << rnti << (uint32_t)lcid
                       << packetSize);
  ImsiLcidPair_t p(imsi, lcid);
  if (Simulator::Now() >= m_startTime) {
    m_dlCellId[p] = cellId;
...
void RadioBearerStatsCalculator::UlRxPdu(uint16_t cellId, uint64_t imsi,
                                         uint16_t rnti, uint8_t lcid,
                                         uint32_t packetSize, uint64_t delay) {
  NS_LOG_FUNCTION(this << "UlRxPDU" << cellId << imsi << rnti << (uint32_t)lcid
                       << packetSize << delay);
  ImsiLcidPair_t p(imsi, lcid);
  if (Simulator::Now() >= m_startTime) {
    m_ulCellId[p] = cellId;
    ...
    if (it == m_ulDelay.end()) {
      NS_LOG_DEBUG(this << " Creating UL stats calculators for IMSI "
                        << p.m_imsi << " and LCID " << (uint32_t)p.m_lcId);
...
void RadioBearerStatsCalculator::DlRxPdu(uint16_t cellId, uint64_t imsi,
                                         uint16_t rnti, uint8_t lcid,
                                         uint32_t packetSize, uint64_t delay) {
  NS_LOG_FUNCTION(this << "DlRxPDU" << cellId << imsi << rnti << (uint32_t)lcid
                       << packetSize << delay);
  ImsiLcidPair_t p(imsi, lcid);
  if (Simulator::Now() >= m_startTime) {
    m_dlCellId[p] = cellId;
    ...
    if (it == m_dlDelay.end()) {
      NS_LOG_DEBUG(this << " Creating DL stats calculators for IMSI "
                        << p.m_imsi << " and LCID " << (uint32_t)p.m_lcId);
...
void RadioBearerStatsCalculator::ShowResults() {
...
void RadioBearerStatsCalculator::ResetResults() {
  NS_LOG_FUNCTION(this);
...
void RadioBearerStatsCalculator::EndEpoch() {
  NS_LOG_FUNCTION(this);
  ShowResults();
  ResetResults();
  m_startTime += m_epochDuration;
  m_endEpochEvent = Simulator::Schedule(
      m_epochDuration, &RadioBearerStatsCalculator::EndEpoch, this);
}

uint32_t RadioBearerStatsCalculator::GetUlTxPackets(uint64_t imsi,
                                                    uint8_t lcid) {
...
uint32_t RadioBearerStatsCalculator::GetUlRxPackets(uint64_t imsi,
                                                    uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
...
uint64_t RadioBearerStatsCalculator::GetUlTxData(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  return m_ulTxData[p];
}

uint64_t RadioBearerStatsCalculator::GetUlRxData(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  return m_ulRxData[p];
}

double RadioBearerStatsCalculator::GetUlDelay(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  auto it = m_ulDelay.find(p);
  if (it == m_ulDelay.end()) {
    NS_LOG_ERROR("UL delay for " << imsi << " - " << (uint16_t)lcid
                                 << " not found");
    return 0;
  }
...
std::vector<double> RadioBearerStatsCalculator::GetUlDelayStats(uint64_t imsi,
                                                                uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
...
uint32_t RadioBearerStatsCalculator::GetDlTxPackets(uint64_t imsi,
                                                    uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
...
uint32_t RadioBearerStatsCalculator::GetDlRxPackets(uint64_t imsi,
                                                    uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
...
uint64_t RadioBearerStatsCalculator::GetDlTxData(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  return m_dlTxData[p];
}

uint64_t RadioBearerStatsCalculator::GetDlRxData(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  return m_dlRxData[p];
}

uint32_t RadioBearerStatsCalculator::GetUlCellId(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  return m_ulCellId[p];
}

uint32_t RadioBearerStatsCalculator::GetDlCellId(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  return m_dlCellId[p];
}

double RadioBearerStatsCalculator::GetDlDelay(uint64_t imsi, uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  auto it = m_dlDelay.find(p);
  if (it == m_dlDelay.end()) {
...
std::vector<double> RadioBearerStatsCalculator::GetDlDelayStats(uint64_t imsi,
                                                                uint8_t lcid) {
  NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
  ImsiLcidPair_t p(imsi, lcid);
  std::vector<double> stats;
...
std::string RadioBearerStatsCalculator::GetUlOutputFilename() {
...
std::string RadioBearerStatsCalculator::GetDlOutputFilename() {
  if (m_protocolType == "RLC") {
...
void RadioBearerStatsCalculator::SetUlPdcpOutputFilename(
    std::string outputFilename) {
  m_ulPdcpOutputFilename = outputFilename;
}

std::string RadioBearerStatsCalculator::GetUlPdcpOutputFilename() {
  return m_ulPdcpOutputFilename;
}

void RadioBearerStatsCalculator::SetDlPdcpOutputFilename(
    std::string outputFilename) {
  m_dlPdcpOutputFilename = outputFilename;
}

std::string RadioBearerStatsCalculator::GetDlPdcpOutputFilename() {
