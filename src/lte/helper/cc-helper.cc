...
void CcHelper::DoInitialize() { NS_LOG_FUNCTION(this); }
...
CcHelper::~CcHelper() { NS_LOG_FUNCTION(this); }

void CcHelper::DoDispose() {
  NS_LOG_FUNCTION(this);
  Object::DoDispose();
}

void CcHelper::SetCcAttribute(std::string n, const AttributeValue &v) {
  NS_LOG_FUNCTION(this << n);
  m_ccFactory.Set(n, v);
}

void CcHelper::SetNumberOfComponentCarriers(uint16_t nCc) {
  m_numberOfComponentCarriers = nCc;
}

void CcHelper::SetUlEarfcn(uint32_t ulEarfcn) { m_ulEarfcn = ulEarfcn; }

void CcHelper::SetDlEarfcn(uint32_t dlEarfcn) { m_dlEarfcn = dlEarfcn; }

void CcHelper::SetDlBandwidth(uint16_t dlBandwidth) {
  m_dlBandwidth = dlBandwidth;
}

void CcHelper::SetUlBandwidth(uint16_t ulBandwidth) {
  m_ulBandwidth = ulBandwidth;
}

uint16_t CcHelper::GetNumberOfComponentCarriers() const {
  return m_numberOfComponentCarriers;
}

uint32_t CcHelper::GetUlEarfcn() const { return m_ulEarfcn; }

uint32_t CcHelper::GetDlEarfcn() const { return m_dlEarfcn; }

uint16_t CcHelper::GetDlBandwidth() const { return m_dlBandwidth; }

uint16_t CcHelper::GetUlBandwidth() const { return m_ulBandwidth; }

ComponentCarrier CcHelper::DoCreateSingleCc(uint16_t ulBandwidth,
                                            uint16_t dlBandwidth,
                                            uint32_t ulEarfcn,
                                            uint32_t dlEarfcn, bool isPrimary) {
  return CreateSingleCc(ulBandwidth, dlBandwidth, ulEarfcn, dlEarfcn,
                        isPrimary);
}

std::map<uint8_t, ComponentCarrier> CcHelper::EquallySpacedCcs() {
  ...
  for (uint16_t i = 0; i < m_numberOfComponentCarriers; i++) {
    ...
    ComponentCarrier cc =
        CreateSingleCc(m_ulBandwidth, m_dlBandwidth, ulEarfcn, dlEarfcn, pc);
    ccmap.insert(std::pair<uint8_t, ComponentCarrier>(i, cc));

    NS_LOG_INFO("ulBandwidth: " << m_ulBandwidth << ", dlBandwidth: "
...
ComponentCarrier CcHelper::CreateSingleCc(uint16_t ulBandwidth,
                                          uint16_t dlBandwidth,
