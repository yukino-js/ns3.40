...
void RadioEnvironmentMapHelper::DoDispose() { NS_LOG_FUNCTION(this); }

TypeId RadioEnvironmentMapHelper::GetTypeId() {
      ...
      TypeId("ns3::RadioEnvironmentMapHelper")
          ...
          .AddAttribute(
              ...
              MakeUintegerAccessor(
                  &RadioEnvironmentMapHelper::m_maxPointsPerIteration),
              MakeUintegerChecker<uint32_t>(
...
uint16_t RadioEnvironmentMapHelper::GetBandwidth() const { return m_bandwidth; }

void RadioEnvironmentMapHelper::SetBandwidth(uint16_t bw) {
  switch (bw) {
  case 6:
  case 15:
  case 25:
...
void RadioEnvironmentMapHelper::Install() {
                  ...
                  // attribute
  {
    ...
    NS_ABORT_MSG_IF(!m_channel, "object at "
                                    << m_channelPath
  ...
  Simulator::Schedule(Seconds(startDelay),
                      &RadioEnvironmentMapHelper::DelayedInstall, this);
}

void RadioEnvironmentMapHelper::DelayedInstall() {
  NS_LOG_FUNCTION(this);
    ...
    for (double y = m_yMin; y < m_yMax + 0.5 * m_yStep; y += m_yStep) {
      ...
      if ((numPointsCurrentIteration == m_maxPointsPerIteration) ||
        ...
        Simulator::Schedule(Seconds(remIterationStartTime),
                            &RadioEnvironmentMapHelper::RunOneIteration, this,
                            xMinNext, x, yMinNext, y);
  ...
  Simulator::Schedule(Seconds(remIterationStartTime),
                      &RadioEnvironmentMapHelper::Finalize, this);
}

void RadioEnvironmentMapHelper::RunOneIteration(double xMin, double xMax,
  ...
  for (x = xMin; x < xMax + 0.5 * m_xStep; x += m_xStep) {
    for (y = (x == xMin) ? yMin : m_yMin;
         y < ((x == xMax) ? yMax : m_yMax) + 0.5 * m_yStep; y += m_yStep) {
  ...
  Simulator::Schedule(Seconds(0.0005),
                      &RadioEnvironmentMapHelper::PrintAndReset, this);
}

void RadioEnvironmentMapHelper::PrintAndReset() {
  NS_LOG_FUNCTION(this);
...
void RadioEnvironmentMapHelper::Finalize() {
  NS_LOG_FUNCTION(this);
  m_outFile.close();
  if (m_stopWhenDone) {
    Simulator::Stop();
