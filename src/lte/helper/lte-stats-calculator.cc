...
void LteStatsCalculator::SetUlOutputFilename(std::string outputFilename) {
  m_ulOutputFilename = outputFilename;
}

std::string LteStatsCalculator::GetUlOutputFilename() {
  return m_ulOutputFilename;
}

void LteStatsCalculator::SetDlOutputFilename(std::string outputFilename) {
  m_dlOutputFilename = outputFilename;
}

std::string LteStatsCalculator::GetDlOutputFilename() {
  return m_dlOutputFilename;
}

bool LteStatsCalculator::ExistsImsiPath(std::string path) {
  return m_pathImsiMap.find(path) != m_pathImsiMap.end();
}

void LteStatsCalculator::SetImsiPath(std::string path, uint64_t imsi) {
  NS_LOG_FUNCTION(this << path << imsi);
  m_pathImsiMap[path] = imsi;
}

uint64_t LteStatsCalculator::GetImsiPath(std::string path) {
  return m_pathImsiMap.find(path)->second;
}

bool LteStatsCalculator::ExistsCellIdPath(std::string path) {
  return m_pathCellIdMap.find(path) != m_pathCellIdMap.end();
}

void LteStatsCalculator::SetCellIdPath(std::string path, uint16_t cellId) {
  NS_LOG_FUNCTION(this << path << cellId);
  m_pathCellIdMap[path] = cellId;
}

uint16_t LteStatsCalculator::GetCellIdPath(std::string path) {
...
uint64_t LteStatsCalculator::FindImsiFromEnbRlcPath(std::string path) {
  ...
  if (match.GetN() != 0) {
    ...
    NS_LOG_LOGIC("FindImsiFromEnbRlcPath: "
                 << path << ", " << ueInfo->GetObject<UeManager>()->GetImsi());
...
uint16_t LteStatsCalculator::FindCellIdFromEnbMac(std::string path,
  ...
  NS_LOG_LOGIC("FindCellIdFromEnbMac: " << path << ", " << rnti << ", "
                                        << cellId);
  return cellId;
}

uint64_t LteStatsCalculator::FindImsiForEnb(std::string path, uint16_t rnti) {
  NS_LOG_FUNCTION(path << rnti);
  uint64_t imsi = 0;
  if (path.find("/DlPhyTransmission")) {
    ...
    NS_LOG_LOGIC("FindImsiForEnb[Tx]: " << path << ", " << rnti << ", "
                                        << imsi);
  } else if (path.find("/UlPhyReception")) {
    std::string p = path.substr(0, path.find("/LteUePhy"));
    imsi = FindImsiFromLteNetDevice(p);
    NS_LOG_LOGIC("FindImsiForEnb[Rx]: " << path << ", " << rnti << ", "
                                        << imsi);
  }
  return imsi;
}

uint64_t LteStatsCalculator::FindImsiForUe(std::string path, uint16_t rnti) {
  NS_LOG_FUNCTION(path << rnti);
  uint64_t imsi = 0;
