...
EmuEpcHelper::~EmuEpcHelper() { NS_LOG_FUNCTION(this); }

TypeId EmuEpcHelper::GetTypeId() {
  static TypeId tid =
...
TypeId EmuEpcHelper::GetInstanceTypeId() const { return GetTypeId(); }

void EmuEpcHelper::DoDispose() {
  NS_LOG_FUNCTION(this);
  NoBackhaulEpcHelper::DoDispose();
}

void EmuEpcHelper::AddEnb(Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice,
  ...
  enbMacAddress << m_enbMacAddressBase << ":" << std::hex << std::setfill('0')
                << std::setw(2) << cellIds.at(0);
  ...
  Ipv4InterfaceContainer enbIpIfaces =
      m_epcIpv4AddressHelper.Assign(enbDevices);
  NS_LOG_LOGIC(
...
void EmuEpcHelper::AddX2Interface(Ptr<Node> enb1, Ptr<Node> enb2) {
  ...
  Ptr<LteEnbNetDevice> enb2LteDev =
      enb2->GetDevice(0)->GetObject<LteEnbNetDevice>();
