...
void LteGlobalPathlossDatabase::Print() {
  NS_LOG_FUNCTION(this);
  for (auto cellIdIt = m_pathlossMap.begin(); cellIdIt != m_pathlossMap.end();
       ++cellIdIt) {
    for (auto imsiIt = cellIdIt->second.begin();
         imsiIt != cellIdIt->second.end(); ++imsiIt) {
...
double LteGlobalPathlossDatabase::GetPathloss(uint16_t cellId, uint64_t imsi) {
  NS_LOG_FUNCTION(this);
...
void DownlinkLteGlobalPathlossDatabase::UpdatePathloss(
    std::string context, Ptr<const SpectrumPhy> txPhy,
    Ptr<const SpectrumPhy> rxPhy, double lossDb) {
  NS_LOG_FUNCTION(this << lossDb);
  uint16_t cellId =
      txPhy->GetDevice()->GetObject<LteEnbNetDevice>()->GetCellId();
  uint16_t imsi = rxPhy->GetDevice()->GetObject<LteUeNetDevice>()->GetImsi();
  m_pathlossMap[cellId][imsi] = lossDb;
}

void UplinkLteGlobalPathlossDatabase::UpdatePathloss(
    std::string context, Ptr<const SpectrumPhy> txPhy,
    Ptr<const SpectrumPhy> rxPhy, double lossDb) {
  NS_LOG_FUNCTION(this << lossDb);
  uint16_t imsi = txPhy->GetDevice()->GetObject<LteUeNetDevice>()->GetImsi();
  uint16_t cellId =
      rxPhy->GetDevice()->GetObject<LteEnbNetDevice>()->GetCellId();
  m_pathlossMap[cellId][imsi] = lossDb;
}
