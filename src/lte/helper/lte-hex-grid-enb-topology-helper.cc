  ...
  static TypeId tid =
      TypeId("ns3::LteHexGridEnbTopologyHelper")
          ...
          .AddAttribute(
              ...
              MakeDoubleChecker<double>())
          .AddAttribute(
              "SiteHeight", "The height [m] of each site", DoubleValue(30),
...
void LteHexGridEnbTopologyHelper::DoDispose() {
  NS_LOG_FUNCTION(this);
  Object::DoDispose();
}

void LteHexGridEnbTopologyHelper::SetLteHelper(Ptr<LteHelper> h) {
...
LteHexGridEnbTopologyHelper::SetPositionAndInstallEnbDevice(NodeContainer c) {
  ...
  for (uint32_t n = 0; n < c.GetN(); ++n) {
    ...
    NS_LOG_LOGIC("node " << n << " site " << currentSite << " rowIndex "
                         << rowIndex << " colIndex " << colIndex
                         << " biRowIndex " << biRowIndex << " biRowRemainder "
                         << biRowRemainder);
    ...
    NS_LOG_LOGIC("node " << n << " at " << pos << " antennaOrientation "
                         << antennaOrientation);
    mm->SetPosition(Vector(x, y, m_siteHeight));
    m_lteHelper->SetEnbAntennaModelAttribute("Orientation",
