...
LteHelper::~LteHelper() { NS_LOG_FUNCTION(this); }
...
Ptr<SpectrumChannel> LteHelper::GetUplinkSpectrumChannel() const {
  return m_uplinkChannel;
}

Ptr<SpectrumChannel> LteHelper::GetDownlinkSpectrumChannel() const {
...
void LteHelper::ChannelModelInitialization() {
  ...
  if (dlSplm) {
    ...
    NS_ASSERT_MSG(dlPlm, " " << m_downlinkPathlossModel
  ...
  if (ulSplm) {
    ...
    NS_ASSERT_MSG(ulPlm, " " << m_uplinkPathlossModel
                             << " is neither PropagationLossModel nor "
                                "SpectrumPropagationLossModel");
...
Ptr<NetDevice> LteHelper::InstallSingleEnbDevice(Ptr<Node> n) {
  ...
  for (auto it = ccMap.begin(); it != ccMap.end(); ++it) {
    ...
    rrc->SetLteEnbCphySapProvider(DynamicCast<ComponentCarrierEnb>(it->second)
                                      ->GetPhy()
                                      ->GetLteEnbCphySapProvider(),
                                  it->first);

    rrc->SetLteEnbCmacSapProvider(DynamicCast<ComponentCarrierEnb>(it->second)
                                      ->GetMac()
    ...
    DynamicCast<ComponentCarrierEnb>(it->second)
        ->GetFfMacScheduler()
        ->SetFfMacSchedSapUser(DynamicCast<ComponentCarrierEnb>(it->second)
                                   ->GetMac()
  ...
  dev->SetAttribute(
      "LteFfrAlgorithm",
      PointerValue(
          DynamicCast<ComponentCarrierEnb>(it->second)->GetFfrAlgorithm()));
  ...
  for (it = ccMap.begin(); it != ccMap.end(); ++it) {
    Ptr<LteEnbPhy> ccPhy =
        DynamicCast<ComponentCarrierEnb>(it->second)->GetPhy();
    ccPhy->SetDevice(dev);
    ...
    double dlFreq =
        LteSpectrumValueHelper::GetCarrierFrequency(it->second->GetDlEarfcn());
    ...
    double ulFreq =
        LteSpectrumValueHelper::GetCarrierFrequency(it->second->GetUlEarfcn());
  ...
  for (it = ccMap.begin(); it != ccMap.end(); ++it) {
    m_uplinkChannel->AddRx(DynamicCast<ComponentCarrierEnb>(it->second)
                               ->GetPhy()
                               ->GetUlSpectrumPhy());
  }
...
void LteHelper::Attach(Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice,
  ...
  Ptr<EpcUeNas> ueNas = ueLteDevice->GetNas();
  Ptr<ComponentCarrierEnb> componentCarrier = DynamicCast<ComponentCarrierEnb>(
      enbLteDevice->GetCcMap().at(componentCarrierId));
  ueNas->Connect(componentCarrier->GetCellId(),
...
void LteHelper::DeActivateDedicatedEpsBearer(Ptr<NetDevice> ueDevice,
  ...
  NS_ASSERT_MSG(bearerId != 1, "Default bearer cannot be de-activated until "
...
void LteHelper::DoComponentCarrierConfigure(uint32_t ulEarfcn,
                                            uint32_t dlEarfcn, uint16_t ulbw,
                                            uint16_t dlbw) {
...
void LteHelper::EnableLogComponents() {
...
void LteHelper::EnableRlcTraces() {
  NS_ASSERT_MSG(!m_rlcStats, "please make sure that LteHelper::EnableRlcTraces "
...
int64_t LteHelper::AssignStreams(NetDeviceContainer c, int64_t stream) {
  ...
  if (m_fadingModel && !m_fadingStreamsAssigned) {
    Ptr<TraceFadingLossModel> tflm =
        m_fadingModel->GetObject<TraceFadingLossModel>();
    if (tflm) {
  ...
  for (auto i = c.Begin(); i != c.End(); ++i) {
    ...
    if (lteEnb) {
      std::map<uint8_t, Ptr<ComponentCarrierBaseStation>> tmpMap =
          lteEnb->GetCcMap();
      auto it = tmpMap.begin();
      Ptr<LteSpectrumPhy> dlPhy = DynamicCast<ComponentCarrierEnb>(it->second)
                                      ->GetPhy()
                                      ->GetDownlinkSpectrumPhy();
      Ptr<LteSpectrumPhy> ulPhy = DynamicCast<ComponentCarrierEnb>(it->second)
                                      ->GetPhy()
...
void LteHelper::EnablePhyTraces() {
  EnableDlPhyTraces();
  EnableUlPhyTraces();
  EnableDlTxPhyTraces();
  EnableUlTxPhyTraces();
...
void LteHelper::EnableDlTxPhyTraces() {
  Config::Connect(
...
void LteHelper::EnableUlTxPhyTraces() {
  Config::Connect(
...
void LteHelper::EnableDlRxPhyTraces() {
  Config::Connect(
...
void LteHelper::EnableUlRxPhyTraces() {
  Config::Connect(
...
void LteHelper::EnableMacTraces() {
  EnableDlMacTraces();
  EnableUlMacTraces();
}

void LteHelper::EnableDlMacTraces() {
...
void LteHelper::EnableUlMacTraces() {
  NS_LOG_FUNCTION_NOARGS();
  Config::Connect(
...
void LteHelper::EnableDlPhyTraces() {
  NS_LOG_FUNCTION_NOARGS();
  Config::Connect(
      "/NodeList/*/DeviceList/*/ComponentCarrierMapUe/*/LteUePhy/"
...
void LteHelper::EnableUlPhyTraces() {
  NS_LOG_FUNCTION_NOARGS();
  Config::Connect(
...
Ptr<RadioBearerStatsCalculator> LteHelper::GetRlcStats() { return m_rlcStats; }

void LteHelper::EnablePdcpTraces() {
...
Ptr<RadioBearerStatsCalculator> LteHelper::GetPdcpStats() {
  return m_pdcpStats;
}
