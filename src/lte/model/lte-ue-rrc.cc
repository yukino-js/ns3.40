...
void UeMemberLteUeCmacSapUser::SetTemporaryCellRnti(uint16_t rnti) {
  m_rrc->DoSetTemporaryCellRnti(rnti);
}

void UeMemberLteUeCmacSapUser::NotifyRandomAccessSuccessful() {
  m_rrc->DoNotifyRandomAccessSuccessful();
}

void UeMemberLteUeCmacSapUser::NotifyRandomAccessFailed() {
...
LteUeRrc::LteUeRrc()
    : m_cmacSapProvider(0), m_rrcSapUser(nullptr), m_macSapProvider(nullptr),
      m_asSapUser(nullptr), m_ccmRrcSapProvider(nullptr), m_state(IDLE_START),
      m_imsi(0), m_rnti(0), m_cellId(0), m_useRlcSm(true),
      m_connectionPending(false), m_hasReceivedMib(false),
      m_hasReceivedSib1(false), m_hasReceivedSib2(false), m_csgWhiteList(0),
      m_noOfSyncIndications(0), m_leaveConnectedMode(false),
      m_previousCellId(0), m_connEstFailCountLimit(0), m_connEstFailCount(0),
...
LteUeRrc::~LteUeRrc() { NS_LOG_FUNCTION(this); }

void LteUeRrc::DoDispose() {
  NS_LOG_FUNCTION(this);
      ...
      TypeId("ns3::LteUeRrc")
          ...
          .AddAttribute("T300",
                        ...
                        TimeValue(MilliSeconds(
                            100)), // see 3GPP 36.331 UE-TimersAndConstants &
          ...
          .AddAttribute(
              ...
              TimeValue(
                  MilliSeconds(1000)), // see 3GPP 36.331 UE-TimersAndConstants
                                       // & RLF-TimersAndConstants
          ...
          .AddTraceSource(
              ...
              MakeTraceSourceAccessor(
                  &LteUeRrc::m_initialCellSelectionEndOkTrace),
          ...
          .AddTraceSource(
              ...
              MakeTraceSourceAccessor(
                  &LteUeRrc::m_initialCellSelectionEndErrorTrace),
              "ns3::LteUeRrc::CellSelectionTracedCallback")
          .AddTraceSource(
          ...
          .AddTraceSource("ConnectionReconfiguration",
                          ...
                          MakeTraceSourceAccessor(
                              &LteUeRrc::m_connectionReconfigurationTrace),
                          "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
          .AddTraceSource(
          ...
          .AddTraceSource(
              ...
              "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
          .AddTraceSource("DrbCreated", "trace fired after DRB is created",
...
void LteUeRrc::SetLteUeCphySapProvider(LteUeCphySapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_cphySapProvider.at(0) = s;
}

void LteUeRrc::SetLteUeCphySapProvider(LteUeCphySapProvider *s, uint8_t index) {
  NS_LOG_FUNCTION(this << s);
  m_cphySapProvider.at(index) = s;
}

LteUeCphySapUser *LteUeRrc::GetLteUeCphySapUser() {
  NS_LOG_FUNCTION(this);
  return m_cphySapUser.at(0);
}

LteUeCphySapUser *LteUeRrc::GetLteUeCphySapUser(uint8_t index) {
  NS_LOG_FUNCTION(this);
  return m_cphySapUser.at(index);
}

void LteUeRrc::SetLteUeCmacSapProvider(LteUeCmacSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_cmacSapProvider.at(0) = s;
}

void LteUeRrc::SetLteUeCmacSapProvider(LteUeCmacSapProvider *s, uint8_t index) {
  NS_LOG_FUNCTION(this << s);
  m_cmacSapProvider.at(index) = s;
}

LteUeCmacSapUser *LteUeRrc::GetLteUeCmacSapUser() {
  NS_LOG_FUNCTION(this);
  return m_cmacSapUser.at(0);
}

LteUeCmacSapUser *LteUeRrc::GetLteUeCmacSapUser(uint8_t index) {
  NS_LOG_FUNCTION(this);
  return m_cmacSapUser.at(index);
}

void LteUeRrc::SetLteUeRrcSapUser(LteUeRrcSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_rrcSapUser = s;
}

LteUeRrcSapProvider *LteUeRrc::GetLteUeRrcSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_rrcSapProvider;
}

void LteUeRrc::SetLteMacSapProvider(LteMacSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_macSapProvider = s;
}

void LteUeRrc::SetLteCcmRrcSapProvider(LteUeCcmRrcSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_ccmRrcSapProvider = s;
}

LteUeCcmRrcSapUser *LteUeRrc::GetLteCcmRrcSapUser() {
  NS_LOG_FUNCTION(this);
  return m_ccmRrcSapUser;
}

void LteUeRrc::SetAsSapUser(LteAsSapUser *s) { m_asSapUser = s; }

LteAsSapProvider *LteUeRrc::GetAsSapProvider() { return m_asSapProvider; }

void LteUeRrc::SetImsi(uint64_t imsi) {
...
void LteUeRrc::StorePreviousCellId(uint16_t cellId) {
  NS_LOG_FUNCTION(this << cellId);
  m_previousCellId = cellId;
}

uint64_t LteUeRrc::GetImsi() const { return m_imsi; }

uint16_t LteUeRrc::GetRnti() const {
  NS_LOG_FUNCTION(this);
  return m_rnti;
}

uint16_t LteUeRrc::GetCellId() const {
  NS_LOG_FUNCTION(this);
  return m_cellId;
}

bool LteUeRrc::IsServingCell(uint16_t cellId) const {
  NS_LOG_FUNCTION(this);
...
uint8_t LteUeRrc::GetUlBandwidth() const {
  NS_LOG_FUNCTION(this);
  return m_ulBandwidth;
}

uint8_t LteUeRrc::GetDlBandwidth() const {
  NS_LOG_FUNCTION(this);
  return m_dlBandwidth;
}

uint32_t LteUeRrc::GetDlEarfcn() const { return m_dlEarfcn; }

uint32_t LteUeRrc::GetUlEarfcn() const {
  NS_LOG_FUNCTION(this);
  return m_ulEarfcn;
}

LteUeRrc::State LteUeRrc::GetState() const {
  NS_LOG_FUNCTION(this);
  return m_state;
}

uint16_t LteUeRrc::GetPreviousCellId() const {
  NS_LOG_FUNCTION(this);
  return m_previousCellId;
}

void LteUeRrc::SetUseRlcSm(bool val) {
  NS_LOG_FUNCTION(this);
  m_useRlcSm = val;
}

void LteUeRrc::DoInitialize() {
  NS_LOG_FUNCTION(this);
  ...
  LteMacSapUser *msu = m_ccmRrcSapProvider->ConfigureSignalBearer(
      lcid, lcConfig, rlc->GetLteMacSapUser());
  m_cmacSapProvider.at(0)->AddLc(lcid, lcConfig, msu);
}

void LteUeRrc::InitializeSap() {
  if (m_numberOfComponentCarriers < MIN_NO_CC ||
      m_numberOfComponentCarriers > MAX_NO_CC) {
...
void LteUeRrc::DoSendData(Ptr<Packet> packet, uint8_t bid) {
...
void LteUeRrc::DoNotifyRandomAccessSuccessful() {
  ...
  case IDLE_RANDOM_ACCESS: {
    ...
    m_connectionTimeout =
        Simulator::Schedule(m_t300, &LteUeRrc::ConnectionTimeout, this);
  } break;
...
void LteUeRrc::DoNotifyRandomAccessFailed() {
  NS_LOG_FUNCTION(this << m_imsi << ToString(m_state));
...
void LteUeRrc::DoSetCsgWhiteList(uint32_t csgId) {
  NS_LOG_FUNCTION(this << m_imsi << csgId);
  m_csgWhiteList = csgId;
}

void LteUeRrc::DoStartCellSelection(uint32_t dlEarfcn) {
...
void LteUeRrc::DoForceCampedOnEnb(uint16_t cellId, uint32_t dlEarfcn) {
...
void LteUeRrc::DoRecvMasterInformationBlock(
    uint16_t cellId, LteRrcSap::MasterInformationBlock msg) {
...
void LteUeRrc::DoRecvSystemInformationBlockType1(
    uint16_t cellId, LteRrcSap::SystemInformationBlockType1 msg) {
  NS_LOG_FUNCTION(this);
  switch (m_state) {
  case IDLE_WAIT_SIB1:
    NS_ASSERT_MSG(
        cellId == msg.cellAccessRelatedInfo.cellIdentity,
...
void LteUeRrc::DoReportUeMeasurements(
...
void LteUeRrc::DoCompleteSetup(
    LteUeRrcSapProvider::CompleteSetupParameters params) {
  ...
  if (msg.haveSib2) {
    ...
    case CONNECTED_REESTABLISHING:
      ...
      rc.numberOfRaPreambles =
          msg.sib2.radioResourceConfigCommon.rachConfigCommon.preambleInfo
              .numberOfRaPreambles;
      ...
      rc.raResponseWindowSize =
          msg.sib2.radioResourceConfigCommon.rachConfigCommon.raSupervisionInfo
              .raResponseWindowSize;
      rc.connEstFailCount = msg.sib2.radioResourceConfigCommon.rachConfigCommon
                                .txFailParam.connEstFailCount;
      ...
      m_cphySapProvider.at(0)->ConfigureReferenceSignalPower(
          msg.sib2.radioResourceConfigCommon.pdschConfigCommon
...
void LteUeRrc::DoRecvRrcConnectionSetup(LteRrcSap::RrcConnectionSetup msg) {
  NS_LOG_FUNCTION(this << " RNTI " << m_rnti);
  switch (m_state) {
  case IDLE_CONNECTING: {
...
void LteUeRrc::DoRecvRrcConnectionReconfiguration(
  ...
  case CONNECTED_NORMALLY:
    if (msg.haveMobilityControlInfo) {
      ...
      m_cphySapProvider.at(0)->SynchronizeWithEnb(
          m_cellId, mci.carrierFreq.dlCarrierFreq);
      m_cphySapProvider.at(0)->SetDlBandwidth(mci.carrierBandwidth.dlBandwidth);
      m_cphySapProvider.at(0)->ConfigureUplink(
          mci.carrierFreq.ulCarrierFreq, mci.carrierBandwidth.ulBandwidth);
...
void LteUeRrc::DoRecvRrcConnectionReestablishment(
...
void LteUeRrc::DoRecvRrcConnectionReestablishmentReject(
...
void LteUeRrc::DoRecvRrcConnectionRelease(LteRrcSap::RrcConnectionRelease msg) {
  NS_LOG_FUNCTION(this << " RNTI " << m_rnti);
...
void LteUeRrc::DoRecvRrcConnectionReject(LteRrcSap::RrcConnectionReject msg) {
  NS_LOG_FUNCTION(this);
  m_connectionTimeout.Cancel();
  for (uint16_t i = 0; i < m_numberOfComponentCarriers; i++) {
...
void LteUeRrc::DoSetNumberOfComponentCarriers(uint16_t noOfComponentCarriers) {
  NS_LOG_FUNCTION(this);
  m_numberOfComponentCarriers = noOfComponentCarriers;
}

void LteUeRrc::SynchronizeToStrongestCell() {
  NS_LOG_FUNCTION(this);
  ...
  for (auto it = m_storedMeasValues.begin(); it != m_storedMeasValues.end();
       it++) {
    /*
...
void LteUeRrc::EvaluateCellForSelection() {
  NS_LOG_FUNCTION(this);
  ...
  double qRxLevMin = EutranMeasurementMapping::IeValue2ActualQRxLevMin(
...
void LteUeRrc::ApplyRadioResourceConfigDedicatedSecondaryCarrier(
  ...
  for (auto &scell : nonCec.sCellToAddModList) {
    ...
    uint16_t ulBand = scell.radioResourceConfigCommonSCell.ulConfiguration
                          .ulFreqInfo.ulBandwidth;
    uint32_t ulEarfcn = scell.radioResourceConfigCommonSCell.ulConfiguration
    ...
    m_cphySapProvider.at(ccId)->ConfigureReferenceSignalPower(
        scell.radioResourceConfigCommonSCell.nonUlConfiguration
...
void LteUeRrc::ApplyRadioResourceConfigDedicated(
  ...
  if (pcd.haveAntennaInfoDedicated) {
    m_cphySapProvider.at(0)->SetTransmissionMode(
        pcd.antennaInfo.transmissionMode);
  }
  ...
  if (pcd.havePdschConfigDedicated) {
    ...
    double paDouble =
        LteRrcSap::ConvertPdschConfigDedicated2Double(m_pdschConfigDedicated);
  ...
  if (stamIt != rrcd.srbToAddModList.end()) {
    if (!m_srb1) {
      // SRB1 not setup yet
      NS_ASSERT_MSG((m_state == IDLE_CONNECTING) ||
                        (m_state == CONNECTED_HANDOVER),
      ...
      m_srb1->m_logicalChannelConfig.priority =
      ...
      m_cmacSapProvider.at(0)->AddLc(lcid, lcConfig, msu);
      ++stamIt;
      NS_ASSERT_MSG(stamIt == rrcd.srbToAddModList.end(),
  ...
  for (auto dtamIt = rrcd.drbToAddModList.begin();
       dtamIt != rrcd.drbToAddModList.end(); ++dtamIt) {
    ...
    if (drbMapIt == m_drbMap.end()) {
      ...
      Ptr<LteDataRadioBearerInfo> drbInfo =
          CreateObject<LteDataRadioBearerInfo>();
      drbInfo->m_rlc = rlc;
      ...
      m_drbMap.insert(std::pair<uint8_t, Ptr<LteDataRadioBearerInfo>>(
      ...
      NS_LOG_DEBUG(this << " UE RRC RNTI " << m_rnti
  ...
  for (auto dtdmIt = rrcd.drbToReleaseList.begin();
       dtdmIt != rrcd.drbToReleaseList.end(); ++dtdmIt) {
    uint8_t drbid = *dtdmIt;
    NS_LOG_INFO(this << " IMSI " << m_imsi << " releasing DRB "
                     << (uint32_t)drbid);
    auto it = m_drbMap.find(drbid);
    NS_ASSERT_MSG(it != m_drbMap.end(),
...
void LteUeRrc::ApplyMeasConfig(LteRrcSap::MeasConfig mc) {
  ...
  for (auto it = mc.measObjectToAddModList.begin();
    ...
    if (measObjectIt != m_varMeasConfig.measObjectList.end()) {
      NS_LOG_LOGIC("measObjectId " << (uint32_t)measObjectId
  ...
  for (auto it = mc.reportConfigToRemoveList.begin();
       it != mc.reportConfigToRemoveList.end(); ++it) {
    uint8_t reportConfigId = *it;
    NS_LOG_LOGIC(this << " deleting reportConfigId "
  ...
  // modification
  for (auto it = mc.reportConfigToAddModList.begin();
       it != mc.reportConfigToAddModList.end(); ++it) {
    // simplifying assumptions
    NS_ASSERT_MSG(it->reportConfigEutra.triggerType ==
  ...
  if (mc.haveQuantityConfig) {
    ...
    m_varMeasConfig.aRsrp =
        std::pow(0.5, mc.quantityConfig.filterCoefficientRSRP / 4.0);
    m_varMeasConfig.aRsrq =
        std::pow(0.5, mc.quantityConfig.filterCoefficientRSRQ / 4.0);
    NS_LOG_LOGIC(this << " new filter coefficients: aRsrp="
                      << m_varMeasConfig.aRsrp
  ...
  for (auto it = mc.measIdToRemoveList.begin();
  ...
  for (auto it = mc.measIdToAddModList.begin();
    ...
    m_varMeasConfig.measIdList[it->measId] =
        *it; // side effect: create new entry if not exists
...
void LteUeRrc::SaveUeMeasurements(uint16_t cellId, double rsrp, double rsrq,
                                  bool useLayer3Filtering,
                                  uint8_t componentCarrierId) {
  NS_LOG_FUNCTION(this << cellId << +componentCarrierId << rsrp << rsrq
                       << useLayer3Filtering);
  ...
  NS_LOG_DEBUG(this << " IMSI " << m_imsi << " state " << ToString(m_state)
                    << ", measured cell " << cellId << ", carrier component Id "
                    << componentCarrierId << ", new RSRP " << rsrp << " stored "
                    << storedMeasIt->second.rsrp << ", new RSRQ " << rsrq
                    << " stored " << storedMeasIt->second.rsrq);

} // end of void SaveUeMeasurements

void LteUeRrc::MeasurementReportTriggering(uint8_t measId) {
  ...
  LteRrcSap::ReportConfigEutra &reportConfigEutra =
      reportConfigIt->second.reportConfigEutra;
  ...
  LteRrcSap::MeasObjectEutra &measObjectEutra =
      measObjectIt->second.measObjectEutra;
  ...
  case LteRrcSap::ReportConfigEutra::EVENT_A1: {
    ...
    double hys = EutranMeasurementMapping::IeValue2ActualHysteresis(
    ...
    case LteRrcSap::ReportConfigEutra::RSRP:
      ...
      thresh = EutranMeasurementMapping::RsrpRange2Dbm(
    ...
    case LteRrcSap::ReportConfigEutra::RSRQ:
      ...
      thresh = EutranMeasurementMapping::RsrqRange2Db(
          reportConfigEutra.threshold1.range);
      break;
    default:
    ...
    NS_LOG_LOGIC(this << " event A1: serving cell " << servingCellId
                      << " ms=" << ms << " thresh=" << thresh << " entryCond="
                      << entryCond << " leavingCond=" << leavingCond);
  ...
  case LteRrcSap::ReportConfigEutra::EVENT_A3: {
    ...
    double off = EutranMeasurementMapping::IeValue2ActualA3Offset(
    ...
    for (auto storedMeasIt = m_storedMeasValues.begin();
      ...
      // can trigger event.
      if (m_storedMeasValues.at(cellId).carrierFreq !=
          measObjectEutra.carrierFreq) {
        continue;
      }
  ...
  case LteRrcSap::ReportConfigEutra::EVENT_A5: {
    ...
    case LteRrcSap::ReportConfigEutra::RSRP:
      ...
      NS_ASSERT(reportConfigEutra.threshold2.choice ==
                LteRrcSap::ThresholdEutra::THRESHOLD_RSRP);
      thresh1 = EutranMeasurementMapping::RsrpRange2Dbm(
          reportConfigEutra.threshold1.range);
      thresh2 = EutranMeasurementMapping::RsrpRange2Dbm(
    ...
    case LteRrcSap::ReportConfigEutra::RSRQ:
      ...
      NS_ASSERT(reportConfigEutra.threshold2.choice ==
                LteRrcSap::ThresholdEutra::THRESHOLD_RSRQ);
      thresh1 = EutranMeasurementMapping::RsrqRange2Db(
          reportConfigEutra.threshold1.range);
      thresh2 = EutranMeasurementMapping::RsrqRange2Db(
          reportConfigEutra.threshold2.range);
      break;
    default:
      NS_FATAL_ERROR("unsupported triggerQuantity");
      ...
      if (leavingCond) {
        ...
        for (auto storedMeasIt = m_storedMeasValues.begin();
          ...
          if (measReportIt->second.cellsTriggeredList.find(cellId) !=
            ...
            NS_LOG_LOGIC(this << " event A5: neighbor cell " << cellId << " mn="
                              << mn << " mp=" << mp << " thresh2=" << thresh2
                              << " thresh1=" << thresh1
                              << " leavingCond=" << leavingCond);
  ...
  if (eventEntryCondApplicable) {
    if (reportConfigEutra.timeToTrigger == 0) {
      ...
      t.timer = Simulator::Schedule(
          MilliSeconds(reportConfigEutra.timeToTrigger),
          &LteUeRrc::VarMeasReportListAdd, this, measId, concernedCellsEntry);
  ...
  if (eventLeavingCondApplicable) {
    ...
    if (reportConfigEutra.timeToTrigger == 0) {
      ...
      t.timer =
          Simulator::Schedule(MilliSeconds(reportConfigEutra.timeToTrigger),
...
void LteUeRrc::CancelEnteringTrigger(uint8_t measId) {
  NS_LOG_FUNCTION(this << (uint16_t)measId);
...
void LteUeRrc::CancelEnteringTrigger(uint8_t measId, uint16_t cellId) {
  ...
  while (it2 != it1->second.end()) {
    ...
    for (auto it3 = it2->concernedCells.begin();
...
void LteUeRrc::CancelLeavingTrigger(uint8_t measId) {
  NS_LOG_FUNCTION(this << (uint16_t)measId);
...
void LteUeRrc::CancelLeavingTrigger(uint8_t measId, uint16_t cellId) {
  NS_LOG_FUNCTION(this << (uint16_t)measId << cellId);
...
void LteUeRrc::VarMeasReportListAdd(uint8_t measId,
  ...
  if (!measReportIt->second.periodicReportTimer.IsRunning()) {
    ...
    measReportIt->second.periodicReportTimer =
        Simulator::Schedule(UE_MEASUREMENT_REPORT_DELAY,
                            &LteUeRrc::SendMeasurementReport, this, measId);
  }
...
void LteUeRrc::VarMeasReportListErase(uint8_t measId,
...
void LteUeRrc::VarMeasReportListClear(uint8_t measId) {
  NS_LOG_FUNCTION(this << (uint16_t)measId);
  ...
  auto measReportIt = m_varMeasReportList.find(measId);
  if (measReportIt != m_varMeasReportList.end()) {
    NS_LOG_LOGIC(this << " deleting existing report for measId "
                      << (uint16_t)measId);
    measReportIt->second.periodicReportTimer.Cancel();
    m_varMeasReportList.erase(measReportIt);
  }
...
void LteUeRrc::SendMeasurementReport(uint8_t measId) {
  NS_LOG_FUNCTION(this << (uint16_t)measId);
  ...
  auto measReportIt = m_varMeasReportList.find(measId);
  if (measReportIt == m_varMeasReportList.end()) {
    NS_LOG_ERROR("no entry found in m_varMeasReportList for measId "
                 << (uint32_t)measId);
  } else {
    ...
    for (uint16_t componentCarrierId = 1;
         componentCarrierId < m_numberOfComponentCarriers;
         componentCarrierId++) {
    ...
    if (!(measReportIt->second.cellsTriggeredList.empty())) {
      ...
      for (auto cellsTriggeredIt =
        ...
        if (cellId != m_cellId) {
          ...
          sortedNeighCells.insert(
              std::pair<double, uint16_t>(triggerValue, cellId));
        }
      }
      ...
      for (sortedNeighCellsIt = sortedNeighCells.rbegin(), count = 0;
        ...
        measResultEutra.rsrpResult = EutranMeasurementMapping::Dbm2RsrpRange(
    ...
    measReportIt->second.periodicReportTimer = Simulator::Schedule(
        reportInterval, &LteUeRrc::SendMeasurementReport, this, measId);
...
void LteUeRrc::LeaveConnectedMode() {
  NS_LOG_FUNCTION(this << m_imsi);
  m_leaveConnectedMode = true;
...
void LteUeRrc::ConnectionTimeout() {
  NS_LOG_FUNCTION(this << m_imsi);
  ++m_connEstFailCount;
...
void LteUeRrc::DisposeOldSrb1() {
  NS_LOG_FUNCTION(this);
  m_srb1Old = nullptr;
}

uint8_t LteUeRrc::Bid2Drbid(uint8_t bid) {
...
void LteUeRrc::RadioLinkFailureDetected() {
...
void LteUeRrc::DoNotifyInSync() {
  NS_LOG_FUNCTION(this << m_imsi);
  m_noOfSyncIndications++;
  NS_LOG_INFO("noOfSyncIndications " << (uint16_t)m_noOfSyncIndications);
  m_phySyncDetectionTrace(m_imsi, m_rnti, m_cellId, "Notify in sync",
                          m_noOfSyncIndications);
...
void LteUeRrc::DoNotifyOutOfSync() {
  NS_LOG_FUNCTION(this << m_imsi);
  m_noOfSyncIndications++;
  NS_LOG_INFO(this << " Total Number of Sync indications from PHY "
                   << (uint16_t)m_noOfSyncIndications
                   << "N310 value : " << (uint16_t)m_n310);
  m_phySyncDetectionTrace(m_imsi, m_rnti, m_cellId, "Notify out of sync",
                          m_noOfSyncIndications);
...
void LteUeRrc::DoResetSyncIndicationCounter() {
  NS_LOG_FUNCTION(this << m_imsi);
...
const std::string LteUeRrc::ToString(LteUeRrc::State s) {
