...
struct LogicalChannelConfigListElement_s {
  ...
  uint64_t m_eRabGuaranteedBitrateUl{
      UINT64_MAX}; ///< ERAB guaranteed bit rate UL
  uint64_t m_eRabGuaranteedBitrateDl{
      UINT64_MAX}; ///< ERAB guaranteed bit rate DL
};
...
struct DrxConfig_s {
  ...
  uint16_t m_longDrxCycleStartOffset{
      UINT16_MAX};                         ///< long DRX cycle start offset
...
struct SbMeasResult_s {
  ...
  std::vector<struct HigherLayerSelected_s>
      m_higherLayerSelected; ///< higher layer selected
