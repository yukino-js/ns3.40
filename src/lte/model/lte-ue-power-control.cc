...
void LteUePowerControl::DoInitialize() { NS_LOG_FUNCTION(this); }

void LteUePowerControl::DoDispose() { NS_LOG_FUNCTION(this); }

TypeId LteUePowerControl::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteUePowerControl")
          ...
          .AddAttribute(
              ...
              MakeBooleanChecker())
          .AddAttribute("Alpha", "Value of Alpha parameter", DoubleValue(1.0),
...
void LteUePowerControl::SetPcmax(double value) {
  NS_LOG_FUNCTION(this);
  m_Pcmax = value;
}

double LteUePowerControl::GetPcmax() {
  NS_LOG_FUNCTION(this);
  return m_Pcmax;
}

void LteUePowerControl::SetTxPower(double value) {
  NS_LOG_FUNCTION(this);
  m_txPower = value;
...
void LteUePowerControl::ConfigureReferenceSignalPower(
    int8_t referenceSignalPower) {
  NS_LOG_FUNCTION(this);
  m_referenceSignalPower = referenceSignalPower;
}

void LteUePowerControl::SetCellId(uint16_t cellId) {
  NS_LOG_FUNCTION(this);
  m_cellId = cellId;
}

void LteUePowerControl::SetRnti(uint16_t rnti) {
  NS_LOG_FUNCTION(this);
  m_rnti = rnti;
}

void LteUePowerControl::SetPoNominalPusch(int16_t value) {
  NS_LOG_FUNCTION(this);
...
void LteUePowerControl::SetPoUePusch(int16_t value) {
  NS_LOG_FUNCTION(this);
...
void LteUePowerControl::SetAlpha(double value) {
  NS_LOG_FUNCTION(this);
...
void LteUePowerControl::SetRsrp(double value) {
  NS_LOG_FUNCTION(this);
...
void LteUePowerControl::SetRsrpFilterCoefficient(
    uint8_t rsrpFilterCoefficient) {
  NS_LOG_FUNCTION(this);
  m_pcRsrpFilterCoefficient = rsrpFilterCoefficient;
}

void LteUePowerControl::ReportTpc(uint8_t tpc) {
  NS_LOG_FUNCTION(this);
  ...
  NS_LOG_INFO("ReportTpc: " << (int)tpc << " delta: " << delta
                            << " Fc: " << m_fc);
}

void LteUePowerControl::SetSubChannelMask(std::vector<int> mask) {
  NS_LOG_FUNCTION(this);
  m_M_Pusch = mask.size();
}

void LteUePowerControl::CalculatePuschTxPower() {
  NS_LOG_FUNCTION(this);
  int32_t j = 1;
  ...
  if (m_M_Pusch > 0) {
    m_curPuschTxPower = 10 * log10(1.0 * m_M_Pusch) + PoPusch +
                        m_alpha[j] * m_pathLoss + m_deltaTF + m_fc;
    m_M_Pusch = 0;
  } else {
...
void LteUePowerControl::CalculatePucchTxPower() {
  NS_LOG_FUNCTION(this);
  m_curPucchTxPower = m_curPuschTxPower;
  NS_LOG_INFO("PucchTxPower: " << m_curPucchTxPower);
}

void LteUePowerControl::CalculateSrsTxPower() {
  NS_LOG_FUNCTION(this);
  int32_t j = 1;
  int32_t PoPusch = m_PoNominalPusch[j] + m_PoUePusch[j];
  ...
  m_curSrsTxPower = pSrsOffsetValue + 10 * log10(m_srsBandwidth) + PoPusch +
                    m_alpha[j] * m_pathLoss + m_fc;
...
double LteUePowerControl::GetPuschTxPower(std::vector<int> dlRb) {
  NS_LOG_FUNCTION(this);
...
double LteUePowerControl::GetPucchTxPower(std::vector<int> dlRb) {
  NS_LOG_FUNCTION(this);
...
double LteUePowerControl::GetSrsTxPower(std::vector<int> dlRb) {
  NS_LOG_FUNCTION(this);
