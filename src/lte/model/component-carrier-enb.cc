...
TypeId ComponentCarrierEnb::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ComponentCarrierEnb")
          ...
          .AddAttribute("LteEnbPhy", "The PHY associated to this EnbNetDevice",
                        PointerValue(),
                        MakePointerAccessor(&ComponentCarrierEnb::m_phy),
                        MakePointerChecker<LteEnbPhy>())
          .AddAttribute("LteEnbMac", "The MAC associated to this EnbNetDevice",
                        PointerValue(),
...
ComponentCarrierEnb::ComponentCarrierEnb() { NS_LOG_FUNCTION(this); }

ComponentCarrierEnb::~ComponentCarrierEnb() { NS_LOG_FUNCTION(this); }

void ComponentCarrierEnb::DoDispose() {
...
void ComponentCarrierEnb::DoInitialize() {
  NS_LOG_FUNCTION(this);
  m_phy->Initialize();
  m_mac->Initialize();
...
Ptr<LteEnbPhy> ComponentCarrierEnb::GetPhy() {
  NS_LOG_FUNCTION(this);
  return m_phy;
}

void ComponentCarrierEnb::SetPhy(Ptr<LteEnbPhy> s) {
  NS_LOG_FUNCTION(this);
  m_phy = s;
}

Ptr<LteEnbMac> ComponentCarrierEnb::GetMac() {
  NS_LOG_FUNCTION(this);
  return m_mac;
}

void ComponentCarrierEnb::SetMac(Ptr<LteEnbMac> s) {
  NS_LOG_FUNCTION(this);
  m_mac = s;
}

Ptr<LteFfrAlgorithm> ComponentCarrierEnb::GetFfrAlgorithm() {
  NS_LOG_FUNCTION(this);
  return m_ffrAlgorithm;
}

void ComponentCarrierEnb::SetFfrAlgorithm(Ptr<LteFfrAlgorithm> s) {
  NS_LOG_FUNCTION(this);
  m_ffrAlgorithm = s;
}

Ptr<FfMacScheduler> ComponentCarrierEnb::GetFfMacScheduler() {
  NS_LOG_FUNCTION(this);
  return m_scheduler;
}

void ComponentCarrierEnb::SetFfMacScheduler(Ptr<FfMacScheduler> s) {
  NS_LOG_FUNCTION(this);
  m_scheduler = s;
}
