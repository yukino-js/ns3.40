...
LteRlcTm::~LteRlcTm() { NS_LOG_FUNCTION(this); }

TypeId LteRlcTm::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteRlcTm")
          .SetParent<LteRlc>()
          .SetGroupName("Lte")
...
void LteRlcTm::DoNotifyTxOpportunity(
  ...
  if (txOpParams.bytes < packet->GetSize()) {
    NS_LOG_WARN("TX opportunity too small = "
                << txOpParams.bytes << " (PDU size: " << packet->GetSize()
                << ")");
    return;
  }
  ...
  if (!m_txBuffer.empty()) {
    m_rbsTimer.Cancel();
    m_rbsTimer =
        Simulator::Schedule(MilliSeconds(10), &LteRlcTm::ExpireRbsTimer, this);
  }
}

void LteRlcTm::DoNotifyHarqDeliveryFailure() { NS_LOG_FUNCTION(this); }

void LteRlcTm::DoReceivePdu(LteMacSapUser::ReceivePduParameters rxPduParams) {
