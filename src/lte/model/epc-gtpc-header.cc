...
GtpcHeader::GtpcHeader()
    : m_teidFlag(false), m_messageType(0), m_messageLength(4), m_teid(0),
      m_sequenceNumber(0) {}

GtpcHeader::~GtpcHeader() {}

TypeId GtpcHeader::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t GtpcHeader::GetSerializedSize() const { return m_teidFlag ? 12 : 8; }

void GtpcHeader::Serialize(Buffer::Iterator start) const {
  NS_FATAL_ERROR("Serialize GTP-C header is forbidden");
}

void GtpcHeader::PreSerialize(Buffer::Iterator &i) const {
...
uint32_t GtpcHeader::Deserialize(Buffer::Iterator start) {
  return PreDeserialize(start);
}

uint32_t GtpcHeader::PreDeserialize(Buffer::Iterator &i) {
...
void GtpcHeader::Print(std::ostream &os) const {
  os << " messageType " << (uint32_t)m_messageType << " messageLength "
     << m_messageLength;
  os << " TEID " << m_teid << " sequenceNumber " << m_sequenceNumber;
}

uint32_t GtpcHeader::GetMessageSize() const { return 0; }

uint8_t GtpcHeader::GetMessageType() const { return m_messageType; }

uint16_t GtpcHeader::GetMessageLength() const { return m_messageLength; }

uint32_t GtpcHeader::GetTeid() const { return m_teid; }

uint32_t GtpcHeader::GetSequenceNumber() const { return m_sequenceNumber; }

void GtpcHeader::SetMessageType(uint8_t messageType) {
  m_messageType = messageType;
}

void GtpcHeader::SetMessageLength(uint16_t messageLength) {
  m_messageLength = messageLength;
}

void GtpcHeader::SetTeid(uint32_t teid) {
  m_teidFlag = true;
  m_teid = teid;
  m_messageLength = m_teidFlag ? 8 : 4;
}

void GtpcHeader::SetSequenceNumber(uint32_t sequenceNumber) {
  m_sequenceNumber = sequenceNumber;
}

void GtpcHeader::SetIesLength(uint16_t iesLength) {
...
uint32_t GtpcIes::DeserializeImsi(Buffer::Iterator &i, uint64_t &imsi) const {
...
uint32_t GtpcIes::DeserializeEbi(Buffer::Iterator &i,
...
void GtpcIes::WriteHtonU40(Buffer::Iterator &i, uint64_t data) const {
...
uint64_t GtpcIes::ReadNtohU40(Buffer::Iterator &i) {
  uint64_t retval = 0;
  retval |= i.ReadU8();
  retval <<= 8;
  retval |= i.ReadU8();
...
uint32_t GtpcIes::DeserializeBearerQos(Buffer::Iterator &i,
                                       EpsBearer &bearerQos) {
  uint8_t type = i.ReadU8();
...
uint32_t GtpcIes::DeserializeBearerTft(Buffer::Iterator &i,
...
uint32_t GtpcIes::GetSerializedSizeBearerTft(
...
uint32_t GtpcIes::DeserializeUliEcgi(Buffer::Iterator &i,
...
TypeId GtpcCreateSessionRequestMessage::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t GtpcCreateSessionRequestMessage::GetMessageSize() const {
  uint32_t serializedSize =
      serializedSizeImsi + serializedSizeUliEcgi + serializedSizeFteid;
...
uint32_t GtpcCreateSessionRequestMessage::GetSerializedSize() const {
  return GtpcHeader::GetSerializedSize() + GetMessageSize();
}

void GtpcCreateSessionRequestMessage::Serialize(Buffer::Iterator start) const {
  ...
  for (auto &bc : m_bearerContextsToBeCreated) {
    ...
    SerializeBearerContextHeader(
        i, serializedSizeEbi + GetSerializedSizeBearerTft(packetFilters) +
               serializedSizeFteid + serializedSizeBearerQos);
...
uint32_t GtpcCreateSessionRequestMessage::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
void GtpcCreateSessionRequestMessage::Print(std::ostream &os) const {
  os << " imsi " << m_imsi << " uliEcgi " << m_uliEcgi;
}

uint64_t GtpcCreateSessionRequestMessage::GetImsi() const { return m_imsi; }

void GtpcCreateSessionRequestMessage::SetImsi(uint64_t imsi) { m_imsi = imsi; }

uint32_t GtpcCreateSessionRequestMessage::GetUliEcgi() const {
  return m_uliEcgi;
}

void GtpcCreateSessionRequestMessage::SetUliEcgi(uint32_t uliEcgi) {
  m_uliEcgi = uliEcgi;
}

GtpcHeader::Fteid_t GtpcCreateSessionRequestMessage::GetSenderCpFteid() const {
  return m_senderCpFteid;
}

void GtpcCreateSessionRequestMessage::SetSenderCpFteid(
...
void GtpcCreateSessionRequestMessage::SetBearerContextsToBeCreated(
    std::list<GtpcCreateSessionRequestMessage::BearerContextToBeCreated>
        bearerContexts) {
...
TypeId GtpcCreateSessionResponseMessage::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t GtpcCreateSessionResponseMessage::GetMessageSize() const {
...
uint32_t GtpcCreateSessionResponseMessage::GetSerializedSize() const {
  return GtpcHeader::GetSerializedSize() + GetMessageSize();
}

void GtpcCreateSessionResponseMessage::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t GtpcCreateSessionResponseMessage::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  GtpcHeader::PreDeserialize(i);
...
void GtpcCreateSessionResponseMessage::Print(std::ostream &os) const {
  os << " cause " << m_cause << " FTEID " << m_senderCpFteid.addr << ","
     << m_senderCpFteid.teid;
}
...
void GtpcCreateSessionResponseMessage::SetCause(
    GtpcCreateSessionResponseMessage::Cause_t cause) {
  m_cause = cause;
}

GtpcHeader::Fteid_t GtpcCreateSessionResponseMessage::GetSenderCpFteid() const {
  return m_senderCpFteid;
}

void GtpcCreateSessionResponseMessage::SetSenderCpFteid(
    GtpcHeader::Fteid_t fteid) {
  m_senderCpFteid = fteid;
}
...
void GtpcCreateSessionResponseMessage::SetBearerContextsCreated(
    std::list<GtpcCreateSessionResponseMessage::BearerContextCreated>
        bearerContexts) {
...
TypeId GtpcModifyBearerRequestMessage::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t GtpcModifyBearerRequestMessage::GetMessageSize() const {
...
uint32_t GtpcModifyBearerRequestMessage::GetSerializedSize() const {
  return GtpcHeader::GetSerializedSize() + GetMessageSize();
}

void GtpcModifyBearerRequestMessage::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t GtpcModifyBearerRequestMessage::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  GtpcHeader::PreDeserialize(i);
...
void GtpcModifyBearerRequestMessage::Print(std::ostream &os) const {
  os << " imsi " << m_imsi << " uliEcgi " << m_uliEcgi;
}

uint64_t GtpcModifyBearerRequestMessage::GetImsi() const { return m_imsi; }

void GtpcModifyBearerRequestMessage::SetImsi(uint64_t imsi) { m_imsi = imsi; }

uint32_t GtpcModifyBearerRequestMessage::GetUliEcgi() const {
  return m_uliEcgi;
}

void GtpcModifyBearerRequestMessage::SetUliEcgi(uint32_t uliEcgi) {
  m_uliEcgi = uliEcgi;
}
...
void GtpcModifyBearerRequestMessage::SetBearerContextsToBeModified(
    std::list<GtpcModifyBearerRequestMessage::BearerContextToBeModified>
        bearerContexts) {
...
TypeId GtpcModifyBearerResponseMessage::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t GtpcModifyBearerResponseMessage::GetMessageSize() const {
  return serializedSizeCause;
}

uint32_t GtpcModifyBearerResponseMessage::GetSerializedSize() const {
  return GtpcHeader::GetSerializedSize() + GetMessageSize();
}

void GtpcModifyBearerResponseMessage::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t GtpcModifyBearerResponseMessage::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  GtpcHeader::PreDeserialize(i);
...
void GtpcModifyBearerResponseMessage::Print(std::ostream &os) const {
...
void GtpcModifyBearerResponseMessage::SetCause(
...
TypeId GtpcDeleteBearerCommandMessage::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t GtpcDeleteBearerCommandMessage::GetMessageSize() const {
  uint32_t serializedSize =
      m_bearerContexts.size() *
      (serializedSizeBearerContextHeader + serializedSizeEbi);
  return serializedSize;
}

uint32_t GtpcDeleteBearerCommandMessage::GetSerializedSize() const {
  return GtpcHeader::GetSerializedSize() + GetMessageSize();
}

void GtpcDeleteBearerCommandMessage::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t GtpcDeleteBearerCommandMessage::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  GtpcHeader::PreDeserialize(i);
...
void GtpcDeleteBearerCommandMessage::Print(std::ostream &os) const {
  os << " bearerContexts [";
  for (auto &bearerContext : m_bearerContexts) {
...
void GtpcDeleteBearerCommandMessage::SetBearerContexts(
...
TypeId GtpcDeleteBearerRequestMessage::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t GtpcDeleteBearerRequestMessage::GetMessageSize() const {
  uint32_t serializedSize = m_epsBearerIds.size() * serializedSizeEbi;
  return serializedSize;
}

uint32_t GtpcDeleteBearerRequestMessage::GetSerializedSize() const {
  return GtpcHeader::GetSerializedSize() + GetMessageSize();
}

void GtpcDeleteBearerRequestMessage::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t GtpcDeleteBearerRequestMessage::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  GtpcHeader::PreDeserialize(i);
...
void GtpcDeleteBearerRequestMessage::Print(std::ostream &os) const {
  os << " epsBearerIds [";
  for (auto &epsBearerId : m_epsBearerIds) {
...
std::list<uint8_t> GtpcDeleteBearerRequestMessage::GetEpsBearerIds() const {
  return m_epsBearerIds;
}

void GtpcDeleteBearerRequestMessage::SetEpsBearerIds(
...
TypeId GtpcDeleteBearerResponseMessage::GetInstanceTypeId() const {
  return GetTypeId();
}

uint32_t GtpcDeleteBearerResponseMessage::GetMessageSize() const {
  uint32_t serializedSize =
      serializedSizeCause + m_epsBearerIds.size() * serializedSizeEbi;
  return serializedSize;
}

uint32_t GtpcDeleteBearerResponseMessage::GetSerializedSize() const {
  return GtpcHeader::GetSerializedSize() + GetMessageSize();
}

void GtpcDeleteBearerResponseMessage::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
...
uint32_t GtpcDeleteBearerResponseMessage::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  GtpcHeader::PreDeserialize(i);
...
void GtpcDeleteBearerResponseMessage::Print(std::ostream &os) const {
...
void GtpcDeleteBearerResponseMessage::SetCause(
    GtpcDeleteBearerResponseMessage::Cause_t cause) {
  m_cause = cause;
}

std::list<uint8_t> GtpcDeleteBearerResponseMessage::GetEpsBearerIds() const {
  return m_epsBearerIds;
}

void GtpcDeleteBearerResponseMessage::SetEpsBearerIds(
    std::list<uint8_t> epsBearerId) {
  m_epsBearerIds = epsBearerId;
}
