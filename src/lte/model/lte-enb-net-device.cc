...
TypeId LteEnbNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteEnbNetDevice")
          ...
          .AddAttribute("LteEnbRrc", "The RRC associated to this EnbNetDevice",
                        PointerValue(),
          ...
          .AddAttribute(
              ...
              "The RRC associated to this EnbNetDevice", PointerValue(),
...
LteEnbNetDevice::LteEnbNetDevice()
    : m_isConstructed(false), m_isConfigured(false), m_anr(nullptr),
      m_componentCarrierManager(nullptr) {
  NS_LOG_FUNCTION(this);
}

LteEnbNetDevice::~LteEnbNetDevice() { NS_LOG_FUNCTION(this); }

void LteEnbNetDevice::DoDispose() {
  NS_LOG_FUNCTION(this);
...
Ptr<LteEnbMac> LteEnbNetDevice::GetMac() const { return GetMac(0); }

Ptr<LteEnbPhy> LteEnbNetDevice::GetPhy() const { return GetPhy(0); }

Ptr<LteEnbMac> LteEnbNetDevice::GetMac(uint8_t index) const {
  return DynamicCast<ComponentCarrierEnb>(m_ccMap.at(index))->GetMac();
}

Ptr<LteEnbPhy> LteEnbNetDevice::GetPhy(uint8_t index) const {
  return DynamicCast<ComponentCarrierEnb>(m_ccMap.at(index))->GetPhy();
}

Ptr<LteEnbRrc> LteEnbNetDevice::GetRrc() const { return m_rrc; }
...
uint16_t LteEnbNetDevice::GetCellId() const { return m_cellId; }

std::vector<uint16_t> LteEnbNetDevice::GetCellIds() const {
...
bool LteEnbNetDevice::HasCellId(uint16_t cellId) const {
  return m_rrc->HasCellId(cellId);
}

uint16_t LteEnbNetDevice::GetUlBandwidth() const { return m_ulBandwidth; }

void LteEnbNetDevice::SetUlBandwidth(uint16_t bw) {
...
uint16_t LteEnbNetDevice::GetDlBandwidth() const { return m_dlBandwidth; }

void LteEnbNetDevice::SetDlBandwidth(uint16_t bw) {
...
uint32_t LteEnbNetDevice::GetDlEarfcn() const { return m_dlEarfcn; }

void LteEnbNetDevice::SetDlEarfcn(uint32_t earfcn) {
  NS_LOG_FUNCTION(this << earfcn);
  m_dlEarfcn = earfcn;
}

uint32_t LteEnbNetDevice::GetUlEarfcn() const { return m_ulEarfcn; }

void LteEnbNetDevice::SetUlEarfcn(uint32_t earfcn) {
  NS_LOG_FUNCTION(this << earfcn);
  m_ulEarfcn = earfcn;
}

uint32_t LteEnbNetDevice::GetCsgId() const { return m_csgId; }
...
void LteEnbNetDevice::SetCcMap(
    std::map<uint8_t, Ptr<ComponentCarrierBaseStation>> ccm) {
  NS_ASSERT_MSG(!m_isConfigured, "attempt to set CC map after configuration");
  m_ccMap = ccm;
}

void LteEnbNetDevice::DoInitialize() {
  NS_LOG_FUNCTION(this);
...
bool LteEnbNetDevice::Send(Ptr<Packet> packet, const Address &dest,
...
void LteEnbNetDevice::UpdateConfig() {
  NS_LOG_FUNCTION(this);
  ...
  if (m_isConstructed) {
    ...
    NS_LOG_LOGIC(this << " Updating SIB1 of cell " << m_cellId
                      << " with CSG ID " << m_csgId << " and CSG indication "
                      << m_csgIndication);
