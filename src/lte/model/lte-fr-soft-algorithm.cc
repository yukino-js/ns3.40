...
LteFrSoftAlgorithm::LteFrSoftAlgorithm()
  ...
  m_ffrRrcSapProvider =
      new MemberLteFfrRrcSapProvider<LteFrSoftAlgorithm>(this);
}

LteFrSoftAlgorithm::~LteFrSoftAlgorithm() { NS_LOG_FUNCTION(this); }

void LteFrSoftAlgorithm::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_ffrSapProvider;
  delete m_ffrRrcSapProvider;
}

TypeId LteFrSoftAlgorithm::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteFrSoftAlgorithm")
          ...
          .AddAttribute("AllowCenterUeUseEdgeSubBand",
                        ...
                        BooleanValue(true),
                        MakeBooleanAccessor(
                            &LteFrSoftAlgorithm::m_isEdgeSubBandForCenterUe),
                        MakeBooleanChecker())
          .AddAttribute(
              "RsrqThreshold",
...
void LteFrSoftAlgorithm::SetLteFfrSapUser(LteFfrSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_ffrSapUser = s;
}

LteFfrSapProvider *LteFrSoftAlgorithm::GetLteFfrSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_ffrSapProvider;
}

void LteFrSoftAlgorithm::SetLteFfrRrcSapUser(LteFfrRrcSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_ffrRrcSapUser = s;
}

LteFfrRrcSapProvider *LteFrSoftAlgorithm::GetLteFfrRrcSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_ffrRrcSapProvider;
}

void LteFrSoftAlgorithm::DoInitialize() {
  NS_LOG_FUNCTION(this);
...
void LteFrSoftAlgorithm::Reconfigure() {
  NS_LOG_FUNCTION(this);
  if (m_frCellTypeId != 0) {
    SetDownlinkConfiguration(m_frCellTypeId, m_dlBandwidth);
    SetUplinkConfiguration(m_frCellTypeId, m_ulBandwidth);
...
void LteFrSoftAlgorithm::SetDownlinkConfiguration(uint16_t cellId,
                                                  uint8_t bandwidth) {
  NS_LOG_FUNCTION(this);
  for (uint16_t i = 0; i < NUM_DOWNLINK_CONFS; ++i) {
    if ((g_frSoftDownlinkDefaultConfiguration[i].cellId == cellId) &&
      ...
      m_dlEdgeSubBandOffset =
          g_frSoftDownlinkDefaultConfiguration[i].dlEdgeSubBandOffset;
      m_dlEdgeSubBandwidth =
          g_frSoftDownlinkDefaultConfiguration[i].dlEdgeSubBandwidth;
    }
  }
}

void LteFrSoftAlgorithm::SetUplinkConfiguration(uint16_t cellId,
                                                uint8_t bandwidth) {
  NS_LOG_FUNCTION(this);
  for (uint16_t i = 0; i < NUM_UPLINK_CONFS; ++i) {
    if ((g_frSoftUplinkDefaultConfiguration[i].cellId == cellId) &&
      ...
      m_ulEdgeSubBandOffset =
          g_frSoftUplinkDefaultConfiguration[i].ulEdgeSubBandOffset;
      m_ulEdgeSubBandwidth =
          g_frSoftUplinkDefaultConfiguration[i].ulEdgeSubBandwidth;
    }
  }
}

void LteFrSoftAlgorithm::InitializeDownlinkRbgMaps() {
  m_dlRbgMap.clear();
  ...
  NS_ASSERT_MSG(
      (m_dlEdgeSubBandOffset + m_dlEdgeSubBandwidth) <= m_dlBandwidth,
...
void LteFrSoftAlgorithm::InitializeUplinkRbgMaps() {
  m_ulRbgMap.clear();
  ...
  NS_ASSERT_MSG(
      (m_ulEdgeSubBandOffset + m_ulEdgeSubBandwidth) <= m_dlBandwidth,
      "(UlEdgeSubBandOffset+UlEdgeSubBandwidth) higher than DlBandwidth");

  for (uint8_t i = m_ulEdgeSubBandOffset;
       i < (m_ulEdgeSubBandOffset + m_ulEdgeSubBandwidth); i++) {
    m_ulEdgeRbgMap[i] = true;
  }
}

std::vector<bool> LteFrSoftAlgorithm::DoGetAvailableDlRbg() {
  NS_LOG_FUNCTION(this);
...
bool LteFrSoftAlgorithm::DoIsDlRbgAvailableForUe(int rbgId, uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
std::vector<bool> LteFrSoftAlgorithm::DoGetAvailableUlRbg() {
  NS_LOG_FUNCTION(this);
...
bool LteFrSoftAlgorithm::DoIsUlRbgAvailableForUe(int rbgId, uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
void LteFrSoftAlgorithm::DoReportDlCqiInfo(
...
void LteFrSoftAlgorithm::DoReportUlCqiInfo(
...
uint8_t LteFrSoftAlgorithm::DoGetTpc(uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
uint16_t LteFrSoftAlgorithm::DoGetMinContinuousUlBandwidth() {
  NS_LOG_FUNCTION(this);
  ...
  uint8_t rightBandwidth =
      m_ulBandwidth - m_ulEdgeSubBandwidth - m_ulEdgeSubBandOffset;

  minContinuousUlBandwidth =
      ((leftBandwidth > 0) && (leftBandwidth < minContinuousUlBandwidth))
          ? leftBandwidth
          : minContinuousUlBandwidth;
  ...
  minContinuousUlBandwidth =
      ((rightBandwidth > 0) && (rightBandwidth < minContinuousUlBandwidth))
          ? rightBandwidth
          : minContinuousUlBandwidth;
...
void LteFrSoftAlgorithm::DoReportUeMeas(uint16_t rnti,
...
void LteFrSoftAlgorithm::DoRecvLoadInformation(
