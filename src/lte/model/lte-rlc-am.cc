...
LteRlcAm::~LteRlcAm() { NS_LOG_FUNCTION(this); }

TypeId LteRlcAm::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteRlcAm")
          .SetParent<LteRlc>()
          .SetGroupName("Lte")
          ...
          .AddAttribute("TxOpportunityForRetxAlwaysBigEnough",
                        ...
                        BooleanValue(false),
                        MakeBooleanAccessor(
                            &LteRlcAm::m_txOpportunityForRetxAlwaysBigEnough),
                        MakeBooleanChecker())
          .AddAttribute(
              "MaxTxBufferSize",
...
void LteRlcAm::DoNotifyTxOpportunity(
  ...
  if (m_statusPduRequested && !m_statusProhibitTimer.IsRunning()) {
    ...
    NS_ASSERT_MSG(sn <= m_vrMs, "first SN not reported as missing = "
                                    << sn << ", VR(MS) = " << m_vrMs);
    ...
    m_statusPduRequested = false;
    m_statusPduBufferSize = 0;
    m_statusProhibitTimer = Simulator::Schedule(
        m_statusProhibitTimerValue, &LteRlcAm::ExpireStatusProhibitTimer, this);
    return;
        ...
        if ((packet->GetSize() <= txOpParams.bytes) ||
          ...
          if (((m_txonBuffer.empty()) &&
            ...
            if (!m_pollRetransmitTimer.IsRunning()) {
              ...
              m_pollRetransmitTimer = Simulator::Schedule(
  ...
  while (firstSegment && (firstSegment->GetSize() > 0) &&
    ...
    if ((firstSegment->GetSize() > nextSegmentSize) ||
      ...
      if (firstSegment->GetSize() > 0) {
        ...
        m_txonBuffer.insert(m_txonBuffer.begin(),
        ...
        NS_LOG_LOGIC("    Front buffer size = "
  ...
  m_txedBuffer.at(rlcAmHeader.GetSequenceNumber().GetValue()).m_waitingSince =
      Simulator::Now();
...
void LteRlcAm::DoNotifyHarqDeliveryFailure() { NS_LOG_FUNCTION(this); }

void LteRlcAm::DoReceivePdu(LteMacSapUser::ReceivePduParameters rxPduParams) {
  ...
  if (rlcAmHeader.IsDataPdu()) {
    ...
    if (!IsInsideReceivingWindow(seqNumber)) {
      ...
      if (it != m_rxonBuffer.end()) {
        ...
        NS_LOG_LOGIC("Place PDU in the reception buffer ( SN = " << seqNumber
                                                                 << " )");
        m_rxonBuffer[seqNumber.GetValue()].m_byteSegments.push_back(
            rxPduParams.p);
    ...
    if (it != m_rxonBuffer.end() && it->second.m_pduComplete) {
      ...
      while (it != m_rxonBuffer.end() && it->second.m_pduComplete) {
        ...
        NS_ASSERT_MSG(firstVrMs != m_vrMs.GetValue(),
    ...
    if (seqNumber == m_vrR) {
      ...
      if (it != m_rxonBuffer.end() && it->second.m_pduComplete) {
        ...
        while (it != m_rxonBuffer.end() && it->second.m_pduComplete) {
          ...
          NS_ASSERT_MSG(
              it->second.m_byteSegments.size() == 1,
    ...
    if (!m_reorderingTimer.IsRunning()) {
      ...
      if (m_vrH > m_vrR) {
        ...
        m_reorderingTimer = Simulator::Schedule(
            m_reorderingTimerValue, &LteRlcAm::ExpireReorderingTimer, this);
        m_vrX = m_vrH;
...
bool LteRlcAm::IsInsideReceivingWindow(SequenceNumber10 seqNumber) {
  NS_LOG_FUNCTION(this << seqNumber);
  NS_LOG_LOGIC("Receiving Window: " << m_vrR << " <= " << seqNumber
                                    << " <= " << m_vrMr);
...
void LteRlcAm::ReassembleAndDeliver(Ptr<Packet> packet) {
  LteRlcAmHeader rlcAmHeader;
  RlcTag rlcTag;
...
void LteRlcAm::DoReportBufferStatus() {
  NS_LOG_FUNCTION(this);
  ...
  if (r.txQueueSize != 0 || r.retxQueueSize != 0 || r.statusPduSize != 0) {
    NS_LOG_INFO("Send ReportBufferStatus: "
...
void LteRlcAm::ExpireReorderingTimer() {
  NS_LOG_FUNCTION(this);
  ...
  if ((m_txonBufferSize == 0 && m_retxBufferSize == 0) || (m_vtS == m_vtMs)) {
    ...
    for (SequenceNumber10 sn = m_vtA; sn < m_vtS; sn++) {
      ...
      if (pduAvailable) {
        ...
        m_retxBuffer.at(snValue).m_retxCount =
            m_txedBuffer.at(snValue).m_retxCount;
        m_retxBuffer.at(snValue).m_waitingSince =
            m_txedBuffer.at(snValue).m_waitingSince;
...
void LteRlcAm::ExpireStatusProhibitTimer() { NS_LOG_FUNCTION(this); }

void LteRlcAm::ExpireRbsTimer() {
