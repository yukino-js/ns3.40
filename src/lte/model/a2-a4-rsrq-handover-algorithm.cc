...
A2A4RsrqHandoverAlgorithm::A2A4RsrqHandoverAlgorithm()
  ...
  m_handoverManagementSapProvider =
      new MemberLteHandoverManagementSapProvider<A2A4RsrqHandoverAlgorithm>(
          this);
}
      ...
      TypeId("ns3::A2A4RsrqHandoverAlgorithm")
          ...
          .AddAttribute(
              ...
              MakeUintegerAccessor(
                  &A2A4RsrqHandoverAlgorithm::m_servingCellThreshold),
          ...
          .AddAttribute(
              ...
              UintegerValue(1),
              MakeUintegerAccessor(
                  &A2A4RsrqHandoverAlgorithm::m_neighbourCellOffset),
              MakeUintegerChecker<uint8_t>());
  return tid;
}

void A2A4RsrqHandoverAlgorithm::SetLteHandoverManagementSapUser(
...
void A2A4RsrqHandoverAlgorithm::DoInitialize() {
  NS_LOG_FUNCTION(this);
  ...
  m_a2MeasIds = m_handoverManagementSapUser->AddUeMeasReportConfigForHandover(
      reportConfigA2);
...
void A2A4RsrqHandoverAlgorithm::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_handoverManagementSapProvider;
}

void A2A4RsrqHandoverAlgorithm::DoReportUeMeas(
  ...
  if (std::find(begin(m_a2MeasIds), end(m_a2MeasIds), measResults.measId) !=
      std::end(m_a2MeasIds)) {
    NS_ASSERT_MSG(measResults.measResultPCell.rsrqResult <=
                      m_servingCellThreshold,
                  "Invalid UE measurement report");
    EvaluateHandover(rnti, measResults.measResultPCell.rsrqResult);
  } else if (std::find(begin(m_a4MeasIds), end(m_a4MeasIds),
                       measResults.measId) != std::end(m_a4MeasIds)) {
...
void A2A4RsrqHandoverAlgorithm::UpdateNeighbourMeasurements(uint16_t rnti,
                                                            uint16_t cellId,
                                                            uint8_t rsrq) {
