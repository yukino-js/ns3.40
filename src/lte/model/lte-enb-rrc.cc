...
EnbRrcMemberLteEnbCmacSapUser::EnbRrcMemberLteEnbCmacSapUser(
    LteEnbRrc *rrc, uint8_t componentCarrierId)
    : m_rrc(rrc), m_componentCarrierId{componentCarrierId} {}

uint16_t EnbRrcMemberLteEnbCmacSapUser::AllocateTemporaryCellRnti() {
  return m_rrc->DoAllocateTemporaryCellRnti(m_componentCarrierId);
}

void EnbRrcMemberLteEnbCmacSapUser::NotifyLcConfigResult(uint16_t rnti,
                                                         uint8_t lcid,
                                                         bool success) {
  m_rrc->DoNotifyLcConfigResult(rnti, lcid, success);
}

void EnbRrcMemberLteEnbCmacSapUser::RrcConfigurationUpdateInd(UeConfig params) {
  m_rrc->DoRrcConfigurationUpdateInd(params);
}

bool EnbRrcMemberLteEnbCmacSapUser::IsRandomAccessCompleted(uint16_t rnti) {
...
static const std::string g_ueManagerStateName[UeManager::NUM_STATES] = {
    ...
    "CONNECTION_REESTABLISHMENT", "HANDOVER_PREPARATION",
...
UeManager::UeManager(Ptr<LteEnbRrc> rrc, uint16_t rnti, State s,
    ...
    : m_lastAllocatedDrbid(0), m_rnti(rnti), m_imsi(0),
      m_componentCarrierId(componentCarrierId),
      m_lastRrcTransactionIdentifier(0), m_rrc(rrc), m_state(s),
      m_pendingRrcConnectionReconfiguration(false), m_sourceX2apId(0),
      m_sourceCellId(0), m_needPhyMacConfiguration(false),
      m_caSupportConfigured(false), m_pendingStartDataRadioBearers(false) {
...
void UeManager::DoInitialize() {
  ...
  m_physicalConfigDedicated.antennaInfo.transmissionMode =
      m_rrc->m_defaultTransmissionMode;
  ...
  m_physicalConfigDedicated.pdschConfigDedicated.pa =
  ...
  case INITIAL_RANDOM_ACCESS:
    m_connectionRequestTimeout = Simulator::Schedule(
        m_rrc->m_connectionRequestTimeoutDuration,
  ...
  static TypeId tid =
      TypeId("ns3::UeManager")
          ...
          .AddAttribute("Srb0", "SignalingRadioBearerInfo for SRB0",
                        PointerValue(), MakePointerAccessor(&UeManager::m_srb0),
...
void UeManager::SetSource(uint16_t sourceCellId, uint16_t sourceX2apId) {
  m_sourceX2apId = sourceX2apId;
  m_sourceCellId = sourceCellId;
}

void UeManager::SetImsi(uint64_t imsi) { m_imsi = imsi; }

void UeManager::InitialContextSetupRequest() {
...
void UeManager::SetupDataRadioBearer(EpsBearer bearer, uint8_t bearerId,
                                     uint32_t gtpTeid,
  ...
  drbInfo->m_logicalChannelIdentity = lcid;
  drbInfo->m_logicalChannelConfig.priority =
...
void UeManager::RecordDataRadioBearersToBeStarted() {
  NS_LOG_FUNCTION(this << (uint32_t)m_rnti);
  for (auto it = m_drbMap.begin(); it != m_drbMap.end(); ++it) {
...
void UeManager::StartDataRadioBearers() {
  NS_LOG_FUNCTION(this << (uint32_t)m_rnti);
  for (auto drbIdIt = m_drbsToBeStarted.begin();
...
void UeManager::ReleaseDataRadioBearer(uint8_t drbid) {
...
void LteEnbRrc::DoSendReleaseDataRadioBearer(uint64_t imsi, uint16_t rnti,
                                             uint8_t bearerId) {
...
void UeManager::ScheduleRrcConnectionReconfiguration() {
  NS_LOG_FUNCTION(this);
  switch (m_state) {
  case INITIAL_RANDOM_ACCESS:
...
void UeManager::PrepareHandover(uint16_t cellId) {
  ...
  case CONNECTED_NORMALLY: {
    ...
    if (m_rrc->HasCellId(cellId)) {
      ...
      uint16_t rnti =
          m_rrc->AddUe(UeManager::HANDOVER_JOINING, componentCarrierId);
      LteEnbCmacSapProvider::AllocateNcRaPreambleReturnValue anrcrv =
          m_rrc->m_cmacSapProvider.at(componentCarrierId)
      ...
      for (auto &it : m_drbMap) {
        ueManager->SetupDataRadioBearer(
            ...
            it.second->m_gtpTeid, it.second->m_transportLayerAddress);
      }
      ...
      handoverCommand.mobilityControlInfo.radioResourceConfigCommon
          .rachConfigCommon.preambleInfo.numberOfRaPreambles =
          rc.numberOfRaPreambles;
      handoverCommand.mobilityControlInfo.radioResourceConfigCommon
          .rachConfigCommon.raSupervisionInfo.preambleTransMax =
          rc.preambleTransMax;
      handoverCommand.mobilityControlInfo.radioResourceConfigCommon
          .rachConfigCommon.raSupervisionInfo.raResponseWindowSize =
          rc.raResponseWindowSize;
      ...
      hpi.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
          .plmnIdentityInfo.plmnIdentity =
          m_rrc->m_sib1.at(m_componentCarrierId)
              .cellAccessRelatedInfo.plmnIdentityInfo.plmnIdentity;
      hpi.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
          .cellIdentity = m_rrc->ComponentCarrierToCellId(m_componentCarrierId);
      hpi.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
          .csgIndication = m_rrc->m_sib1.at(m_componentCarrierId)
                               .cellAccessRelatedInfo.csgIndication;
      hpi.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
          .csgIdentity = m_rrc->m_sib1.at(m_componentCarrierId)
...
void UeManager::RecvHandoverRequestAck(
  ...
  m_handoverLeavingTimeout =
      Simulator::Schedule(m_rrc->m_handoverLeavingTimeoutDuration,
                          &LteEnbRrc::HandoverLeavingTimeout, m_rrc, m_rnti);
...
UeManager::GetRrcConnectionReconfigurationForHandover(
  ...
  LteRrcSap::RrcConnectionReconfiguration result =
      BuildRrcConnectionReconfiguration();

  auto targetComponentCarrier = DynamicCast<ComponentCarrierEnb>(
      m_rrc->m_componentCarrierPhyConf.at(componentCarrierId));
  result.haveMobilityControlInfo = true;
  result.mobilityControlInfo.targetPhysCellId =
      targetComponentCarrier->GetCellId();
  result.mobilityControlInfo.haveCarrierFreq = true;
  result.mobilityControlInfo.carrierFreq.dlCarrierFreq =
      targetComponentCarrier->GetDlEarfcn();
  result.mobilityControlInfo.carrierFreq.ulCarrierFreq =
      targetComponentCarrier->GetUlEarfcn();
...
void UeManager::SendPacket(uint8_t bid, Ptr<Packet> p) {
  ...
  if (it != m_drbMap.end()) {
    ...
    if (bearerInfo) {
      ...
      LtePdcpSapProvider *pdcpSapProvider =
          bearerInfo->m_pdcp->GetLtePdcpSapProvider();
...
std::vector<EpcX2Sap::ErabToBeSetupItem> UeManager::GetErabList() {
  NS_LOG_FUNCTION(this);
...
void UeManager::SendUeContextRelease() {
  NS_LOG_FUNCTION(this);
  switch (m_state) {
  case HANDOVER_PATH_SWITCH:
...
void UeManager::RecvHandoverPreparationFailure(uint16_t cellId) {
  NS_LOG_FUNCTION(this << cellId);
  switch (m_state) {
  case HANDOVER_PREPARATION:
...
void UeManager::RecvSnStatusTransfer(
...
void UeManager::RecvUeContextRelease(
    EpcX2SapUser::UeContextReleaseParams params) {
  NS_LOG_FUNCTION(this);
  NS_ASSERT_MSG(m_state == HANDOVER_LEAVING,
                "method unexpected in state " << ToString(m_state));
  m_handoverLeavingTimeout.Cancel();
}

void UeManager::RecvHandoverCancel(EpcX2SapUser::HandoverCancelParams params) {
  NS_LOG_FUNCTION(this);
  NS_ASSERT_MSG(m_state == HANDOVER_JOINING,
                "method unexpected in state " << ToString(m_state));
...
void UeManager::RecvRrcConnectionRequest(LteRrcSap::RrcConnectionRequest msg) {
  NS_LOG_FUNCTION(this);
  switch (m_state) {
  case INITIAL_RANDOM_ACCESS: {
...
void UeManager::RecvRrcConnectionSetupCompleted(
...
void UeManager::RecvRrcConnectionReconfigurationCompleted(
...
void UeManager::RecvRrcConnectionReestablishmentRequest(
...
void UeManager::RecvRrcConnectionReestablishmentComplete(
...
void UeManager::RecvMeasurementReport(LteRrcSap::MeasurementReport msg) {
  ...
  NS_LOG_LOGIC("measId " << (uint16_t)measId << " haveMeasResultNeighCells "
                         << msg.measResults.haveMeasResultNeighCells
                         << " measResultListEutra "
                         << msg.measResults.measResultListEutra.size()
                         << " haveMeasResultServFreqList "
                         << msg.measResults.haveMeasResultServFreqList
                         << " measResultServFreqList "
                         << msg.measResults.measResultServFreqList.size());
  NS_LOG_LOGIC("serving cellId "
               << m_rrc->ComponentCarrierToCellId(m_componentCarrierId)
               << " RSRP "
               << (uint16_t)msg.measResults.measResultPCell.rsrpResult
               << " RSRQ "
...
uint16_t UeManager::GetRnti() const { return m_rnti; }

uint64_t UeManager::GetImsi() const { return m_imsi; }

uint8_t UeManager::GetComponentCarrierId() const {
  return m_componentCarrierId;
}

uint16_t UeManager::GetSrsConfigurationIndex() const {
  return m_physicalConfigDedicated.soundingRsUlConfigDedicated.srsConfigIndex;
}

void UeManager::SetSrsConfigurationIndex(uint16_t srsConfIndex) {
  NS_LOG_FUNCTION(this);
  m_physicalConfigDedicated.soundingRsUlConfigDedicated.srsConfigIndex =
      srsConfIndex;
  for (uint16_t i = 0; i < m_rrc->m_numberOfComponentCarriers; i++) {
    m_rrc->m_cphySapProvider.at(i)->SetSrsConfigurationIndex(m_rnti,
                                                             srsConfIndex);
  }
  switch (m_state) {
  case INITIAL_RANDOM_ACCESS:
...
UeManager::State UeManager::GetState() const { return m_state; }

void UeManager::SetPdschConfigDedicated(
...
void UeManager::CancelPendingEvents() {
  NS_LOG_FUNCTION(this);
  m_connectionRequestTimeout.Cancel();
...
EpcX2Sap::HandoverPreparationFailureParams UeManager::BuildHoPrepFailMsg() {
  NS_LOG_FUNCTION(this);
...
uint8_t UeManager::AddDataRadioBearerInfo(Ptr<LteDataRadioBearerInfo> drbInfo) {
  NS_LOG_FUNCTION(this);
...
Ptr<LteDataRadioBearerInfo> UeManager::GetDataRadioBearerInfo(uint8_t drbid) {
...
void UeManager::RemoveDataRadioBearerInfo(uint8_t drbid) {
  NS_LOG_FUNCTION(this << (uint32_t)drbid);
  auto it = m_drbMap.find(drbid);
  NS_ASSERT_MSG(it != m_drbMap.end(),
                "request to remove radio bearer with unknown drbid " << drbid);
...
UeManager::BuildRrcConnectionReconfiguration() {
  ...
  if (!m_caSupportConfigured && m_rrc->m_numberOfComponentCarriers > 1) {
    ...
    NS_LOG_FUNCTION(this << " haveNonCriticalExtension "
...
uint8_t UeManager::GetNewRrcTransactionIdentifier() {
  NS_LOG_FUNCTION(this);
...
uint8_t UeManager::Lcid2Drbid(uint8_t lcid) {
  NS_ASSERT(lcid > 2);
  return lcid - 2;
}

uint8_t UeManager::Drbid2Lcid(uint8_t drbid) { return drbid + 2; }

uint8_t UeManager::Lcid2Bid(uint8_t lcid) {
  NS_ASSERT(lcid > 2);
  return lcid - 2;
}

uint8_t UeManager::Bid2Lcid(uint8_t bid) { return bid + 2; }

uint8_t UeManager::Drbid2Bid(uint8_t drbid) { return drbid; }

uint8_t UeManager::Bid2Drbid(uint8_t bid) { return bid; }
...
UeManager::BuildNonCriticalExtensionConfigurationCa() {
  ...
  for (auto &it : m_rrc->m_componentCarrierPhyConf) {
    ...
    component.radioResourceConfigCommonSCell.nonUlConfiguration
        .antennaInfoCommon.antennaPortsCount = 0;
    component.radioResourceConfigCommonSCell.nonUlConfiguration
        .pdschConfigCommon.referenceSignalPower =
        m_rrc->m_cphySapProvider.at(0)->GetReferenceSignalPower();
    component.radioResourceConfigCommonSCell.nonUlConfiguration
        .pdschConfigCommon.pb = 0;
    component.radioResourceConfigCommonSCell.haveUlConfiguration = true;
    component.radioResourceConfigCommonSCell.ulConfiguration.ulFreqInfo
        .ulCarrierFreq = eNbCcm->GetUlEarfcn();
    component.radioResourceConfigCommonSCell.ulConfiguration.ulFreqInfo
    ...
    component.radioResourceConfigCommonSCell.ulConfiguration
        .soundingRsUlConfigCommon.srsSubframeConfig = 0;
    component.radioResourceConfigCommonSCell.ulConfiguration.prachConfigSCell
        .index = 0;
    ...
    component.radioResourceConfigDedicatedSCell.physicalConfigDedicatedSCell
        .antennaInfo.transmissionMode = m_rrc->m_defaultTransmissionMode;
    component.radioResourceConfigDedicatedSCell.physicalConfigDedicatedSCell
...
LteEnbRrc::LteEnbRrc()
    : m_x2SapProvider(nullptr), m_cmacSapProvider(0),
      m_handoverManagementSapProvider(nullptr), m_ccmRrcSapProvider(nullptr),
      m_anrSapProvider(nullptr), m_ffrRrcSapProvider(0), m_rrcSapUser(nullptr),
      m_macSapProvider(nullptr), m_s1SapProvider(nullptr), m_cphySapProvider(0),
      m_configured(false), m_lastAllocatedRnti(0), m_srsCurrentPeriodicityId(0),
      m_lastAllocatedConfigurationIndex(0), m_reconfigureUes(false),
      m_numberOfComponentCarriers(0), m_carriersConfigured(false) {
  NS_LOG_FUNCTION(this);
  m_cmacSapUser.push_back(new EnbRrcMemberLteEnbCmacSapUser(this, 0));
  m_handoverManagementSapUser =
      new MemberLteHandoverManagementSapUser<LteEnbRrc>(this);
...
void LteEnbRrc::ConfigureCarriers(
    std::map<uint8_t, Ptr<ComponentCarrierBaseStation>> ccPhyConf) {
  NS_ASSERT_MSG(!m_carriersConfigured,
                "Secondary carriers can be configured only once.");
  m_componentCarrierPhyConf = ccPhyConf;
  NS_ABORT_MSG_IF(m_numberOfComponentCarriers !=
                      m_componentCarrierPhyConf.size(),
...
LteEnbRrc::~LteEnbRrc() { NS_LOG_FUNCTION(this); }

void LteEnbRrc::DoDispose() {
  NS_LOG_FUNCTION(this);
      ...
      TypeId("ns3::LteEnbRrc")
          ...
          .AddAttribute("EpsBearerToRlcMapping",
                        ...
                        MakeEnumChecker(RLC_SM_ALWAYS, "RlcSmAlways",
                                        ...
                                        RLC_AM_ALWAYS, "RlcAmAlways", PER_BASED,
          ...
          .AddTraceSource("ConnectionReconfiguration",
                          ...
                          MakeTraceSourceAccessor(
                              &LteEnbRrc::m_connectionReconfigurationTrace),
                          "ns3::LteEnbRrc::ConnectionHandoverTracedCallback")
          .AddTraceSource(
          ...
          .AddTraceSource(
              ...
              MakeTraceSourceAccessor(
                  &LteEnbRrc::m_handoverFailureNoPreambleTrace),
          ...
          .AddTraceSource("HandoverFailureMaxRach",
                          ...
                          MakeTraceSourceAccessor(
                              &LteEnbRrc::m_handoverFailureMaxRachTrace),
                          "ns3::LteEnbRrc::HandoverFailureTracedCallback")
          .AddTraceSource("HandoverFailureLeaving",
                          "trace fired upon handover failure due to handover "
                          "leaving timeout at source eNB",
                          MakeTraceSourceAccessor(
                              &LteEnbRrc::m_handoverFailureLeavingTrace),
                          "ns3::LteEnbRrc::HandoverFailureTracedCallback")
          .AddTraceSource("HandoverFailureJoining",
                          "trace fired upon handover failure due to handover "
                          "joining timeout at target eNB",
                          MakeTraceSourceAccessor(
                              &LteEnbRrc::m_handoverFailureJoiningTrace),
                          "ns3::LteEnbRrc::HandoverFailureTracedCallback");
  return tid;
}

void LteEnbRrc::SetEpcX2SapProvider(EpcX2SapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_x2SapProvider = s;
}

EpcX2SapUser *LteEnbRrc::GetEpcX2SapUser() {
  NS_LOG_FUNCTION(this);
  return m_x2SapUser;
}

void LteEnbRrc::SetLteEnbCmacSapProvider(LteEnbCmacSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
...
LteEnbCmacSapUser *LteEnbRrc::GetLteEnbCmacSapUser() {
  NS_LOG_FUNCTION(this);
  return m_cmacSapUser.at(0);
}

LteEnbCmacSapUser *LteEnbRrc::GetLteEnbCmacSapUser(uint8_t pos) {
  NS_LOG_FUNCTION(this);
  return m_cmacSapUser.at(pos);
}

void LteEnbRrc::SetLteHandoverManagementSapProvider(
    LteHandoverManagementSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_handoverManagementSapProvider = s;
}

LteHandoverManagementSapUser *LteEnbRrc::GetLteHandoverManagementSapUser() {
  NS_LOG_FUNCTION(this);
  return m_handoverManagementSapUser;
}

void LteEnbRrc::SetLteCcmRrcSapProvider(LteCcmRrcSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_ccmRrcSapProvider = s;
}

LteCcmRrcSapUser *LteEnbRrc::GetLteCcmRrcSapUser() {
  NS_LOG_FUNCTION(this);
  return m_ccmRrcSapUser;
}

void LteEnbRrc::SetLteAnrSapProvider(LteAnrSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_anrSapProvider = s;
}

LteAnrSapUser *LteEnbRrc::GetLteAnrSapUser() {
  NS_LOG_FUNCTION(this);
  return m_anrSapUser;
}

void LteEnbRrc::SetLteFfrRrcSapProvider(LteFfrRrcSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
...
LteFfrRrcSapUser *LteEnbRrc::GetLteFfrRrcSapUser() {
  NS_LOG_FUNCTION(this);
  return m_ffrRrcSapUser.at(0);
}

LteFfrRrcSapUser *LteEnbRrc::GetLteFfrRrcSapUser(uint8_t index) {
  NS_LOG_FUNCTION(this);
...
void LteEnbRrc::SetLteEnbRrcSapUser(LteEnbRrcSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_rrcSapUser = s;
}

LteEnbRrcSapProvider *LteEnbRrc::GetLteEnbRrcSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_rrcSapProvider;
}

void LteEnbRrc::SetLteMacSapProvider(LteMacSapProvider *s) {
  NS_LOG_FUNCTION(this);
  m_macSapProvider = s;
}

void LteEnbRrc::SetS1SapProvider(EpcEnbS1SapProvider *s) {
  m_s1SapProvider = s;
}

EpcEnbS1SapUser *LteEnbRrc::GetS1SapUser() { return m_s1SapUser; }

void LteEnbRrc::SetLteEnbCphySapProvider(LteEnbCphySapProvider *s) {
  NS_LOG_FUNCTION(this << s);
...
LteEnbCphySapUser *LteEnbRrc::GetLteEnbCphySapUser() {
  NS_LOG_FUNCTION(this);
...
LteEnbCphySapUser *LteEnbRrc::GetLteEnbCphySapUser(uint8_t pos) {
  NS_LOG_FUNCTION(this);
  return m_cphySapUser.at(pos);
}

bool LteEnbRrc::HasUeManager(uint16_t rnti) const {
  NS_LOG_FUNCTION(this << (uint32_t)rnti);
  auto it = m_ueMap.find(rnti);
  return (it != m_ueMap.end());
}

Ptr<UeManager> LteEnbRrc::GetUeManager(uint16_t rnti) {
  NS_LOG_FUNCTION(this << (uint32_t)rnti);
  NS_ASSERT(0 != rnti);
  auto it = m_ueMap.find(rnti);
  NS_ASSERT_MSG(it != m_ueMap.end(),
                "UE manager for RNTI " << rnti << " not found");
  return it->second;
}
...
void LteEnbRrc::ConfigureCell(
  ...
  for (const auto &it : ccPhyConf) {
    ...
    if (m_ffrRrcSapProvider.size() > it.first) {
      ...
      m_ffrRrcSapProvider.at(it.first)->SetBandwidth(
          it.second->GetUlBandwidth(), it.second->GetDlBandwidth());
    }
  }
  ...
  for (const auto &it : ccPhyConf) {
    ...
    measObject.measObjectEutra.allowedMeasBandwidth =
        it.second->GetDlBandwidth();
...
uint8_t LteEnbRrc::CellToComponentCarrierId(uint16_t cellId) {
  NS_LOG_FUNCTION(this << cellId);
...
uint16_t LteEnbRrc::ComponentCarrierToCellId(uint8_t componentCarrierId) {
  NS_LOG_FUNCTION(this << +componentCarrierId);
  return m_componentCarrierPhyConf.at(componentCarrierId)->GetCellId();
}

bool LteEnbRrc::HasCellId(uint16_t cellId) const {
  for (auto &it : m_componentCarrierPhyConf) {
    if (it.second->GetCellId() == cellId) {
      return true;
    }
...
bool LteEnbRrc::SendData(Ptr<Packet> packet) {
  ...
  NS_LOG_INFO("Sending a packet of " << packet->GetSize() << " bytes to IMSI "
                                     << ueManager->GetImsi() << ", RNTI "
                                     << ueManager->GetRnti() << ", BID "
                                     << (uint16_t)tag.GetBid());
...
void LteEnbRrc::SetForwardUpCallback(Callback<void, Ptr<Packet>> cb) {
  m_forwardUpCallback = cb;
}

void LteEnbRrc::ConnectionRequestTimeout(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
  NS_ASSERT_MSG(GetUeManager(rnti)->GetState() ==
                    UeManager::INITIAL_RANDOM_ACCESS,
...
void LteEnbRrc::ConnectionSetupTimeout(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void LteEnbRrc::ConnectionRejectedTimeout(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
  NS_ASSERT_MSG(GetUeManager(rnti)->GetState() ==
                    UeManager::CONNECTION_REJECTED,
...
void LteEnbRrc::HandoverJoiningTimeout(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void LteEnbRrc::HandoverLeavingTimeout(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void LteEnbRrc::SendHandoverRequest(uint16_t rnti, uint16_t cellId) {
...
void LteEnbRrc::DoCompleteSetupUe(
    uint16_t rnti, LteEnbRrcSapProvider::CompleteSetupUeParameters params) {
  NS_LOG_FUNCTION(this << rnti);
  GetUeManager(rnti)->CompleteSetupUe(params);
}

void LteEnbRrc::DoRecvRrcConnectionRequest(
    uint16_t rnti, LteRrcSap::RrcConnectionRequest msg) {
  NS_LOG_FUNCTION(this << rnti);
  GetUeManager(rnti)->RecvRrcConnectionRequest(msg);
}

void LteEnbRrc::DoRecvRrcConnectionSetupCompleted(
    uint16_t rnti, LteRrcSap::RrcConnectionSetupCompleted msg) {
  NS_LOG_FUNCTION(this << rnti);
  GetUeManager(rnti)->RecvRrcConnectionSetupCompleted(msg);
}

void LteEnbRrc::DoRecvRrcConnectionReconfigurationCompleted(
...
void LteEnbRrc::DoRecvRrcConnectionReestablishmentRequest(
...
void LteEnbRrc::DoRecvRrcConnectionReestablishmentComplete(
...
void LteEnbRrc::DoRecvMeasurementReport(uint16_t rnti,
                                        LteRrcSap::MeasurementReport msg) {
  NS_LOG_FUNCTION(this << rnti);
  GetUeManager(rnti)->RecvMeasurementReport(msg);
}

void LteEnbRrc::DoInitialContextSetupRequest(
    EpcEnbS1SapUser::InitialContextSetupRequestParameters msg) {
  NS_LOG_FUNCTION(this);
...
void LteEnbRrc::DoDataRadioBearerSetupRequest(
    EpcEnbS1SapUser::DataRadioBearerSetupRequestParameters request) {
  NS_LOG_FUNCTION(this);
  Ptr<UeManager> ueManager = GetUeManager(request.rnti);
  ueManager->SetupDataRadioBearer(request.bearer, request.bearerId,
                                  request.gtpTeid,
                                  request.transportLayerAddress);
}

void LteEnbRrc::DoPathSwitchRequestAcknowledge(
...
void LteEnbRrc::DoRecvHandoverRequest(EpcX2SapUser::HandoverRequestParams req) {
  NS_LOG_FUNCTION(this);
  ...
  for (auto it = req.bearers.begin(); it != req.bearers.end(); ++it) {
    ueManager->SetupDataRadioBearer(it->erabLevelQosParameters, it->erabId,
                                    it->gtpTeid, it->transportLayerAddress);
  ...
  Ptr<Packet> encodedHandoverCommand =
      m_rrcSapUser->EncodeHandoverCommand(handoverCommand);
...
void LteEnbRrc::DoRecvHandoverPreparationFailure(
...
void LteEnbRrc::DoRecvSnStatusTransfer(
    EpcX2SapUser::SnStatusTransferParams params) {
  NS_LOG_FUNCTION(this);
...
void LteEnbRrc::DoRecvLoadInformation(
...
void LteEnbRrc::DoRecvUeData(EpcX2SapUser::UeDataParams params) {
  NS_LOG_FUNCTION(this);
...
void LteEnbRrc::DoRecvHandoverCancel(
...
void LteEnbRrc::DoRrcConfigurationUpdateInd(
    LteEnbCmacSapUser::UeConfig cmacParams) {
  Ptr<UeManager> ueManager = GetUeManager(cmacParams.m_rnti);
  ueManager->CmacUeConfigUpdateInd(cmacParams);
}

void LteEnbRrc::DoNotifyLcConfigResult(uint16_t rnti, uint8_t lcid,
                                       bool success) {
  NS_LOG_FUNCTION(this << (uint32_t)rnti);
  NS_FATAL_ERROR("not implemented");
}

std::vector<uint8_t> LteEnbRrc::DoAddUeMeasReportConfigForHandover(
...
uint8_t LteEnbRrc::DoAddUeMeasReportConfigForComponentCarrier(
    LteRrcSap::ReportConfigEutra reportConfig) {
  NS_LOG_FUNCTION(this);
...
void LteEnbRrc::DoSetNumberOfComponentCarriers(
    uint16_t numberOfComponentCarriers) {
  m_numberOfComponentCarriers = numberOfComponentCarriers;
}

void LteEnbRrc::DoTriggerHandover(uint16_t rnti, uint16_t targetCellId) {
...
uint8_t LteEnbRrc::DoAddUeMeasReportConfigForAnr(
    LteRrcSap::ReportConfigEutra reportConfig) {
  NS_LOG_FUNCTION(this);
  uint8_t measId = AddUeMeasReportConfig(reportConfig).at(0);
...
uint8_t LteEnbRrc::DoAddUeMeasReportConfigForFfr(
    LteRrcSap::ReportConfigEutra reportConfig) {
  NS_LOG_FUNCTION(this);
  uint8_t measId = AddUeMeasReportConfig(reportConfig).at(0);
...
void LteEnbRrc::DoSetPdschConfigDedicated(
...
void LteEnbRrc::DoSendLoadInformation(EpcX2Sap::LoadInformationParams params) {
  NS_LOG_FUNCTION(this);

  m_x2SapProvider->SendLoadInformation(params);
}

uint16_t LteEnbRrc::AddUe(UeManager::State state, uint8_t componentCarrierId) {
  NS_LOG_FUNCTION(this);
  bool found = false;
  uint16_t rnti;
  for (rnti = m_lastAllocatedRnti + 1;
       (rnti != m_lastAllocatedRnti - 1) && (!found); ++rnti) {
  ...
  Ptr<UeManager> ueManager =
      CreateObject<UeManager>(this, rnti, state, componentCarrierId);
  ...
  NS_LOG_DEBUG(this << " New UE RNTI " << rnti << " cellId " << cellId
                    << " srs CI " << ueManager->GetSrsConfigurationIndex());
  m_newUeContextTrace(cellId, rnti);
  return rnti;
}

void LteEnbRrc::RemoveUe(uint16_t rnti) {
  NS_LOG_FUNCTION(this << (uint32_t)rnti);
  auto it = m_ueMap.find(rnti);
  NS_ASSERT_MSG(it != m_ueMap.end(),
...
void LteEnbRrc::AddX2Neighbour(uint16_t cellId) {
  NS_LOG_FUNCTION(this << cellId);
...
void LteEnbRrc::SetCsgId(uint32_t csgId, bool csgIndication) {
  ...
  for (std::size_t componentCarrierId = 0; componentCarrierId < m_sib1.size();
    ...
    m_sib1.at(componentCarrierId).cellAccessRelatedInfo.csgIndication =
        csgIndication;
...
void LteEnbRrc::SetSrsPeriodicity(uint32_t p) {
  ...
  NS_FATAL_ERROR("illecit SRS periodicity value "
                 << p << ". Allowed values: " << allowedValues.str());
}

uint32_t LteEnbRrc::GetSrsPeriodicity() const {
  NS_LOG_FUNCTION(this);
...
uint16_t LteEnbRrc::GetNewSrsConfigurationIndex() {
  ...
  if (m_ueSrsConfigurationIndexSet.size() >=
...
void LteEnbRrc::RemoveSrsConfigurationIndex(uint16_t srcCi) {
...
bool LteEnbRrc::IsMaxSrsReached() {
  NS_ASSERT(m_srsCurrentPeriodicityId > 0);
  NS_ASSERT(m_srsCurrentPeriodicityId < SRS_ENTRIES);
  NS_LOG_DEBUG(this << " SRS p " << g_srsPeriodicity[m_srsCurrentPeriodicityId]
                    << " set " << m_ueSrsConfigurationIndexSet.size());
  return m_ueSrsConfigurationIndexSet.size() >=
         g_srsPeriodicity[m_srsCurrentPeriodicityId];
}
...
uint8_t LteEnbRrc::GetLogicalChannelPriority(EpsBearer bearer) {
  return bearer.qci;
}
...
void LteEnbRrc::SendSystemInformation() {
  ...
  for (auto &it : m_componentCarrierPhyConf) {
    ...
    LteEnbCmacSapProvider::RachConfig rc =
        m_cmacSapProvider.at(ccId)->GetRachConfig();
...
bool LteEnbRrc::IsRandomAccessCompleted(uint16_t rnti) {
  NS_LOG_FUNCTION(this << (uint32_t)rnti);
  Ptr<UeManager> ueManager = GetUeManager(rnti);
