...
private:
  ...
  std::vector<LteUeCphySapProvider *>
      m_cphySapProvider; ///< UE CPhy SAP provider

  std::vector<LteUeCmacSapUser *> m_cmacSapUser; ///< UE CMac SAP user
  std::vector<LteUeCmacSapProvider *>
      m_cmacSapProvider; ///< UE CMac SAP provider
  ...
  LteRrcSap::PdschConfigDedicated
      m_pdschConfigDedicated; ///< the PDSCH config dedicated
  ...
  std::list<LteRrcSap::SCellToAddMod>
      m_sCellToAddModList; /**< Secondary carriers. */
  ...
  struct VarMeasConfig {
    std::map<uint8_t, LteRrcSap::MeasIdToAddMod>
        measIdList; ///< measure ID list
    std::map<uint8_t, LteRrcSap::MeasObjectToAddMod>
        measObjectList; ///< measure object list
    std::map<uint8_t, LteRrcSap::ReportConfigToAddMod>
        reportConfigList;                     ///< report config list
  ...
  struct PendingTrigger_t {
    ...
    ConcernedCells_t
        concernedCells; ///< The list of cells responsible for this trigger.
    EventId timer; ///< The pending reporting event, scheduled at the end of the
                   ///< time-to-trigger.
  };
  ...
  uint16_t
      m_previousCellId; ///< the cell id of the previous cell UE was attached to
