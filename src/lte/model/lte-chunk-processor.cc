...
LteChunkProcessor::LteChunkProcessor() { NS_LOG_FUNCTION(this); }

LteChunkProcessor::~LteChunkProcessor() { NS_LOG_FUNCTION(this); }

void LteChunkProcessor::AddCallback(LteChunkProcessorCallback c) {
  NS_LOG_FUNCTION(this);
  m_lteChunkProcessorCallbacks.push_back(c);
}

void LteChunkProcessor::Start() {
  NS_LOG_FUNCTION(this);
  m_sumValues = nullptr;
  m_totDuration = MicroSeconds(0);
}

void LteChunkProcessor::EvaluateChunk(const SpectrumValue &sinr,
                                      Time duration) {
...
void LteSpectrumValueCatcher::ReportValue(const SpectrumValue &value) {
  m_value = value.Copy();
}

Ptr<SpectrumValue> LteSpectrumValueCatcher::GetValue() { return m_value; }
