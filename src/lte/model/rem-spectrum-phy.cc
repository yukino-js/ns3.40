...
RemSpectrumPhy::RemSpectrumPhy()
    : m_mobility(nullptr), m_referenceSignalPower(0), m_sumPower(0),
      m_active(true), m_useDataChannel(false), m_rbId(-1) {
  NS_LOG_FUNCTION(this);
}

RemSpectrumPhy::~RemSpectrumPhy() { NS_LOG_FUNCTION(this); }

void RemSpectrumPhy::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_mobility = nullptr;
  SpectrumPhy::DoDispose();
}

TypeId RemSpectrumPhy::GetTypeId() {
...
Ptr<NetDevice> RemSpectrumPhy::GetDevice() const { return nullptr; }

Ptr<const SpectrumModel> RemSpectrumPhy::GetRxSpectrumModel() const {
  return m_rxSpectrumModel;
}

Ptr<Object> RemSpectrumPhy::GetAntenna() const { return nullptr; }

void RemSpectrumPhy::StartRx(Ptr<SpectrumSignalParameters> params) {
...
void RemSpectrumPhy::SetRxSpectrumModel(Ptr<const SpectrumModel> m) {
  NS_LOG_FUNCTION(this << m);
  m_rxSpectrumModel = m;
}

double RemSpectrumPhy::GetSinr(double noisePower) const {
  return m_referenceSignalPower /
         (m_sumPower - m_referenceSignalPower + noisePower);
}

void RemSpectrumPhy::Deactivate() { m_active = false; }

bool RemSpectrumPhy::IsActive() const { return m_active; }

void RemSpectrumPhy::Reset() {
  m_referenceSignalPower = 0;
  m_sumPower = 0;
}

void RemSpectrumPhy::SetUseDataChannel(bool value) { m_useDataChannel = value; }

void RemSpectrumPhy::SetRbId(int32_t rbId) { m_rbId = rbId; }
