...
LtePhy::LtePhy(Ptr<LteSpectrumPhy> dlPhy, Ptr<LteSpectrumPhy> ulPhy)
    : m_downlinkSpectrumPhy(dlPhy), m_uplinkSpectrumPhy(ulPhy), m_tti(0.001),
      m_ulBandwidth(0), m_dlBandwidth(0), m_rbgSize(0), m_dlEarfcn(0),
      m_ulEarfcn(0), m_macChTtiDelay(0), m_cellId(0), m_componentCarrierId(0) {
  NS_LOG_FUNCTION(this);
}

TypeId LtePhy::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LtePhy").SetParent<Object>().SetGroupName("Lte");
  return tid;
}

LtePhy::~LtePhy() { NS_LOG_FUNCTION(this); }

void LtePhy::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void LtePhy::SetDevice(Ptr<LteNetDevice> d) {
  NS_LOG_FUNCTION(this << d);
  m_netDevice = d;
}

Ptr<LteNetDevice> LtePhy::GetDevice() const {
  NS_LOG_FUNCTION(this);
  return m_netDevice;
}

Ptr<LteSpectrumPhy> LtePhy::GetDownlinkSpectrumPhy() {
  return m_downlinkSpectrumPhy;
}

Ptr<LteSpectrumPhy> LtePhy::GetUplinkSpectrumPhy() {
  return m_uplinkSpectrumPhy;
}

void LtePhy::SetDownlinkChannel(Ptr<SpectrumChannel> c) {
  NS_LOG_FUNCTION(this << c);
  m_downlinkSpectrumPhy->SetChannel(c);
}

void LtePhy::SetUplinkChannel(Ptr<SpectrumChannel> c) {
  NS_LOG_FUNCTION(this << c);
  m_uplinkSpectrumPhy->SetChannel(c);
}

void LtePhy::SetTti(double tti) {
  NS_LOG_FUNCTION(this << tti);
  m_tti = tti;
}

double LtePhy::GetTti() const {
...
uint8_t LtePhy::GetRbgSize() const { return m_rbgSize; }

void LtePhy::SetMacPdu(Ptr<Packet> p) {
  m_packetBurstQueue.at(m_packetBurstQueue.size() - 1)->AddPacket(p);
}

Ptr<PacketBurst> LtePhy::GetPacketBurst() {
...
std::list<Ptr<LteControlMessage>> LtePhy::GetControlMessages() {
  NS_LOG_FUNCTION(this);
...
void LtePhy::DoSetCellId(uint16_t cellId) {
  m_cellId = cellId;
  m_downlinkSpectrumPhy->SetCellId(cellId);
  m_uplinkSpectrumPhy->SetCellId(cellId);
}

void LtePhy::SetComponentCarrierId(uint8_t index) {
...
uint8_t LtePhy::GetComponentCarrierId() const { return m_componentCarrierId; }
