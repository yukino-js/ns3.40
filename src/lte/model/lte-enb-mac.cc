...
void EnbMacMemberLteEnbCmacSapProvider::ConfigureMac(uint16_t ulBandwidth,
                                                     uint16_t dlBandwidth) {
  m_mac->DoConfigureMac(ulBandwidth, dlBandwidth);
}

void EnbMacMemberLteEnbCmacSapProvider::AddUe(uint16_t rnti) {
  m_mac->DoAddUe(rnti);
}

void EnbMacMemberLteEnbCmacSapProvider::RemoveUe(uint16_t rnti) {
  m_mac->DoRemoveUe(rnti);
}

void EnbMacMemberLteEnbCmacSapProvider::AddLc(LcInfo lcinfo,
                                              LteMacSapUser *msu) {
  m_mac->DoAddLc(lcinfo, msu);
}

void EnbMacMemberLteEnbCmacSapProvider::ReconfigureLc(LcInfo lcinfo) {
...
void EnbMacMemberLteEnbCmacSapProvider::UeUpdateConfigurationReq(
    UeConfig params) {
...
void EnbMacMemberFfMacSchedSapUser::SchedDlConfigInd(
    const SchedDlConfigIndParameters &params) {
  m_mac->DoSchedDlConfigInd(params);
}

void EnbMacMemberFfMacSchedSapUser::SchedUlConfigInd(
    const SchedUlConfigIndParameters &params) {
...
public:
  ...
  void
  CschedCellConfigCnf(const CschedCellConfigCnfParameters &params) override;
  ...
  void CschedUeConfigUpdateInd(
      const CschedUeConfigUpdateIndParameters &params) override;
...
void EnbMacMemberFfMacCschedSapUser::CschedCellConfigCnf(
    const CschedCellConfigCnfParameters &params) {
  m_mac->DoCschedCellConfigCnf(params);
}

void EnbMacMemberFfMacCschedSapUser::CschedUeConfigCnf(
    const CschedUeConfigCnfParameters &params) {
  m_mac->DoCschedUeConfigCnf(params);
}

void EnbMacMemberFfMacCschedSapUser::CschedLcConfigCnf(
    const CschedLcConfigCnfParameters &params) {
  m_mac->DoCschedLcConfigCnf(params);
}

void EnbMacMemberFfMacCschedSapUser::CschedLcReleaseCnf(
    const CschedLcReleaseCnfParameters &params) {
  m_mac->DoCschedLcReleaseCnf(params);
}

void EnbMacMemberFfMacCschedSapUser::CschedUeReleaseCnf(
    const CschedUeReleaseCnfParameters &params) {
...
public:
  ...
  void UlCqiReport(
      FfMacSchedSapProvider::SchedUlCqiInfoReqParameters ulcqi) override;
...
void EnbMacMemberLteEnbPhySapUser::SubframeIndication(uint32_t frameNo,
                                                      uint32_t subframeNo) {
  m_mac->DoSubframeIndication(frameNo, subframeNo);
}

void EnbMacMemberLteEnbPhySapUser::ReceiveLteControlMessage(
...
void EnbMacMemberLteEnbPhySapUser::UlInfoListElementHarqFeedback(
    UlInfoListElement_s params) {
  m_mac->DoUlInfoListElementHarqFeedback(params);
}

void EnbMacMemberLteEnbPhySapUser::DlInfoListElementHarqFeedback(
...
LteEnbMac::LteEnbMac() : m_ccmMacSapUser(nullptr) {
...
void LteEnbMac::SetComponentCarrierId(uint8_t index) {
  m_componentCarrierId = index;
}

void LteEnbMac::SetFfMacSchedSapProvider(FfMacSchedSapProvider *s) {
  m_schedSapProvider = s;
}

FfMacSchedSapUser *LteEnbMac::GetFfMacSchedSapUser() { return m_schedSapUser; }

void LteEnbMac::SetFfMacCschedSapProvider(FfMacCschedSapProvider *s) {
  m_cschedSapProvider = s;
}

FfMacCschedSapUser *LteEnbMac::GetFfMacCschedSapUser() {
  return m_cschedSapUser;
}

void LteEnbMac::SetLteMacSapUser(LteMacSapUser *s) { m_macSapUser = s; }

LteMacSapProvider *LteEnbMac::GetLteMacSapProvider() {
  return m_macSapProvider;
}

void LteEnbMac::SetLteEnbCmacSapUser(LteEnbCmacSapUser *s) {
  m_cmacSapUser = s;
}

LteEnbCmacSapProvider *LteEnbMac::GetLteEnbCmacSapProvider() {
  return m_cmacSapProvider;
}

void LteEnbMac::SetLteEnbPhySapProvider(LteEnbPhySapProvider *s) {
  m_enbPhySapProvider = s;
}

LteEnbPhySapUser *LteEnbMac::GetLteEnbPhySapUser() { return m_enbPhySapUser; }

void LteEnbMac::SetLteCcmMacSapUser(LteCcmMacSapUser *s) {
  m_ccmMacSapUser = s;
}

LteCcmMacSapProvider *LteEnbMac::GetLteCcmMacSapProvider() {
...
void LteEnbMac::DoSubframeIndication(uint32_t frameNo, uint32_t subframeNo) {
    ...
    for (auto it = m_receivedRachPreambleCount.begin();
      ...
      if (it->second > 1) {
        ...
        if (jt != m_allocatedNcRaPreambleMap.end()) {
          ...
          NS_LOG_INFO("preambleId " << (uint32_t)it->first
  ...
  for (std::size_t i = 0; i < m_ulCqiReceived.size(); i++) {
    if (subframeNo > 1) {
      m_ulCqiReceived.at(i).m_sfnSf =
          ((0x3FF & frameNo) << 4) | (0xF & (subframeNo - 1));
    } else {
      m_ulCqiReceived.at(i).m_sfnSf =
          ((0x3FF & (frameNo - 1)) << 4) | (0xF & 10);
    }
  ...
  if (!m_ulCeReceived.empty()) {
    ...
    ulMacReq.m_macCeList.insert(ulMacReq.m_macCeList.begin(),
                                m_ulCeReceived.begin(), m_ulCeReceived.end());
  ...
  if (ulSchedSubframeNo + (m_macChTtiDelay + UL_PUSCH_TTIS_DELAY) > 10) {
    ulSchedFrameNo++;
    ulSchedSubframeNo =
        (ulSchedSubframeNo + (m_macChTtiDelay + UL_PUSCH_TTIS_DELAY)) % 10;
  } else {
    ulSchedSubframeNo =
...
void LteEnbMac::DoReceiveLteControlMessage(Ptr<LteControlMessage> msg) {
  ...
  if (msg->GetMessageType() == LteControlMessage::DL_CQI) {
    Ptr<DlCqiLteControlMessage> dlcqi =
        DynamicCast<DlCqiLteControlMessage>(msg);
...
void LteEnbMac::DoAddUe(uint16_t rnti) {
  ...
  m_miDlHarqProcessesPackets.insert(
      std::pair<uint16_t, DlHarqProcessesBuffer_t>(rnti, buf));
}
...
void LteEnbMac::DoAddLc(LteEnbCmacSapProvider::LcInfo lcinfo,
                        LteMacSapUser *msu) {
  ...
  if (lcidIt == rntiIt->second.end()) {
    rntiIt->second.insert(
        std::pair<uint8_t, LteMacSapUser *>(lcinfo.lcId, msu));
  } else {
  ...
  if (lcinfo.lcId != 0) {
    ...
    lccle.m_qosBearerType =
        static_cast<LogicalChannelConfigListElement_s::QosBearerType_e>(
            lcinfo.resourceType);
...
void LteEnbMac::DoReconfigureLc(LteEnbCmacSapProvider::LcInfo lcinfo) {
...
void LteEnbMac::DoUeUpdateConfigurationReq(
...
LteEnbCmacSapProvider::RachConfig LteEnbMac::DoGetRachConfig() const {
  ...
  for (preambleId = m_numberOfRaPreambles; preambleId < 64; ++preambleId) {
    ...
    if ((it == m_allocatedNcRaPreambleMap.end()) ||
      ...
      preambleInfo.expiryTime =
          Simulator::Now() + MilliSeconds(expiryIntervalMs);
...
void LteEnbMac::DoReportBufferStatus(
...
void LteEnbMac::DoSchedDlConfigInd(
    ...
    for (std::size_t layer = 0;
      ...
      if (ind.m_buildDataList.at(i).m_dci.m_ndi.at(layer) == 1) {
        ...
        for (std::size_t lcId = 0; lcId < (*it).second.size(); lcId++) {
          Ptr<PacketBurst> pb = CreateObject<PacketBurst>();
          (*it).second.at(lcId).at(
              ind.m_buildDataList.at(i).m_dci.m_harqProcess) = pb;
        }
      }
    }
    for (std::size_t j = 0; j < ind.m_buildDataList.at(i).m_rlcPduList.size();
         j++) {
      for (std::size_t k = 0;
        ...
        if (ind.m_buildDataList.at(i).m_dci.m_ndi.at(k) == 1) {
          ...
          if (ind.m_buildDataList.at(i).m_dci.m_tbsSize.at(k) > 0) {
            ...
            NS_ASSERT(it != m_miDlHarqProcessesPackets.end());
            Ptr<PacketBurst> pb = (*it).second.at(k).at(
                ind.m_buildDataList.at(i).m_dci.m_harqProcess);
  ...
  for (uint32_t i = 0; i < ind.m_buildDataList.size(); i++) {
    ...
    if (ind.m_buildDataList.at(i).m_dci.m_tbsSize.size() == 1) {
      ...
      dlSchedulingCallbackInfo.mcsTb1 =
          ind.m_buildDataList.at(i).m_dci.m_mcs.at(0);
      dlSchedulingCallbackInfo.sizeTb1 =
    ...
    // Two TBs used
      ...
      dlSchedulingCallbackInfo.sizeTb1 =
          ind.m_buildDataList.at(i).m_dci.m_tbsSize.at(0);
      dlSchedulingCallbackInfo.mcsTb2 =
          ind.m_buildDataList.at(i).m_dci.m_mcs.at(1);
      dlSchedulingCallbackInfo.sizeTb2 =
  ...
  for (unsigned int i = 0; i < ind.m_buildRarList.size(); i++) {
    ...
    if (itRapId == m_rapIdRntiMap.end()) {
      NS_FATAL_ERROR("Unable to find rapId of RNTI "
                     << ind.m_buildRarList.at(i).m_rnti);
    }
    RarLteControlMessage::Rar rar;
    rar.rapId = itRapId->second;
    rar.rarPayload = ind.m_buildRarList.at(i);
    rarMsg->AddRar(rar);
    NS_LOG_INFO(this << " Send RAR message to RNTI "
                     << ind.m_buildRarList.at(i).m_rnti << " rapId "
                     << itRapId->second);
  }
...
void LteEnbMac::DoSchedUlConfigInd(
    FfMacSchedSapUser::SchedUlConfigIndParameters ind) {
  NS_LOG_FUNCTION(this);
...
void LteEnbMac::DoCschedUeConfigCnf(
...
void LteEnbMac::DoCschedUeReleaseCnf(
...
void LteEnbMac::DoCschedCellConfigUpdateInd(
    FfMacCschedSapUser::CschedCellConfigUpdateIndParameters params) {
  NS_LOG_FUNCTION(this);
}

void LteEnbMac::DoUlInfoListElementHarqFeedback(UlInfoListElement_s params) {
  NS_LOG_FUNCTION(this);
