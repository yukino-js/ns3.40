...
void LteRlcSpecificLteMacSapUser::NotifyTxOpportunity(
    TxOpportunityParameters params) {
  m_rlc->DoNotifyTxOpportunity(params);
}

void LteRlcSpecificLteMacSapUser::NotifyHarqDeliveryFailure() {
  m_rlc->DoNotifyHarqDeliveryFailure();
}

void LteRlcSpecificLteMacSapUser::ReceivePdu(
...
TypeId LteRlc::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteRlc")
          ...
          .AddTraceSource("RxPDU", "PDU received.",
...
void LteRlc::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete (m_rlcSapProvider);
  delete (m_macSapUser);
}

void LteRlc::SetRnti(uint16_t rnti) {
  NS_LOG_FUNCTION(this << (uint32_t)rnti);
  m_rnti = rnti;
}

void LteRlc::SetLcId(uint8_t lcId) {
  NS_LOG_FUNCTION(this << (uint32_t)lcId);
  m_lcid = lcId;
}

void LteRlc::SetPacketDelayBudgetMs(uint16_t packetDelayBudget) {
  NS_LOG_FUNCTION(this << +packetDelayBudget);
  m_packetDelayBudgetMs = packetDelayBudget;
}

void LteRlc::SetLteRlcSapUser(LteRlcSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_rlcSapUser = s;
}

LteRlcSapProvider *LteRlc::GetLteRlcSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_rlcSapProvider;
}

void LteRlc::SetLteMacSapProvider(LteMacSapProvider *s) {
  NS_LOG_FUNCTION(this << s);
  m_macSapProvider = s;
}

LteMacSapUser *LteRlc::GetLteMacSapUser() {
  NS_LOG_FUNCTION(this);
  return m_macSapUser;
}
...
void LteRlcSm::DoInitialize() {
  NS_LOG_FUNCTION(this);
  ReportBufferStatus();
}

void LteRlcSm::DoDispose() {
  NS_LOG_FUNCTION(this);
  LteRlc::DoDispose();
}

void LteRlcSm::DoTransmitPdcpPdu(Ptr<Packet> p) { NS_LOG_FUNCTION(this << p); }
...
void LteRlcSm::DoNotifyTxOpportunity(
...
void LteRlcSm::DoNotifyHarqDeliveryFailure() { NS_LOG_FUNCTION(this); }

void LteRlcSm::ReportBufferStatus() {
  NS_LOG_FUNCTION(this);
