...
struct CqasFlowPerf_t {
  ...
  unsigned int lastTtiBytesTransmitted; ///< Total bytes send by eNB in last tti
                                        ///< for this UE
...
private:
   ...
   */
  std::map<LteFlowId_t, LogicalChannelConfigListElement_s>
  ...
  std::map<uint16_t, DlHarqProcessesTimer_t>
      m_dlHarqProcessesTimer; ///< DL HARQ process timers
  ...
  std::vector<DlInfoListElement_s>
      m_dlInfoListBuffered; ///< DL HARQ retx buffered
