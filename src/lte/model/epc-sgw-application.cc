...
EpcSgwApplication::EpcSgwApplication(const Ptr<Socket> s1uSocket,
  ...
  m_s1uSocket->SetRecvCallback(
      MakeCallback(&EpcSgwApplication::RecvFromS1uSocket, this));
  m_s5uSocket->SetRecvCallback(
      MakeCallback(&EpcSgwApplication::RecvFromS5uSocket, this));
  m_s5cSocket->SetRecvCallback(
      MakeCallback(&EpcSgwApplication::RecvFromS5cSocket, this));
}

EpcSgwApplication::~EpcSgwApplication() { NS_LOG_FUNCTION(this); }

void EpcSgwApplication::DoDispose() {
...
TypeId EpcSgwApplication::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::EpcSgwApplication").SetParent<Object>().SetGroupName("Lte");
  return tid;
}

void EpcSgwApplication::AddMme(Ipv4Address mmeS11Addr, Ptr<Socket> s11Socket) {
  NS_LOG_FUNCTION(this << mmeS11Addr << s11Socket);
  m_mmeS11Addr = mmeS11Addr;
  m_s11Socket = s11Socket;
  m_s11Socket->SetRecvCallback(
      MakeCallback(&EpcSgwApplication::RecvFromS11Socket, this));
}

void EpcSgwApplication::AddPgw(Ipv4Address pgwAddr) {
  NS_LOG_FUNCTION(this << pgwAddr);
  m_pgwAddr = pgwAddr;
}

void EpcSgwApplication::AddEnb(uint16_t cellId, Ipv4Address enbAddr,
                               Ipv4Address sgwAddr) {
...
void EpcSgwApplication::RecvFromS11Socket(Ptr<Socket> socket) {
...
void EpcSgwApplication::RecvFromS5uSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
...
void EpcSgwApplication::RecvFromS5cSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
...
void EpcSgwApplication::RecvFromS1uSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
...
void EpcSgwApplication::SendToS1uSocket(Ptr<Packet> packet, Ipv4Address enbAddr,
                                        uint32_t teid) {
...
void EpcSgwApplication::SendToS5uSocket(Ptr<Packet> packet, Ipv4Address pgwAddr,
                                        uint32_t teid) {
...
void EpcSgwApplication::DoRecvModifyBearerRequest(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
  ...
  std::list<GtpcModifyBearerRequestMessage::BearerContextToBeModified>
      bearerContextsOut;
  std::list<GtpcModifyBearerRequestMessage::BearerContextToBeModified>
      bearerContexts = msg.GetBearerContextsToBeModified();
  ...
  for (auto &bearerContext : bearerContexts) {
    ...
    NS_LOG_DEBUG("bearerId " << (uint16_t)bearerContext.epsBearerId << " TEID "
                             << teid);
...
void EpcSgwApplication::DoRecvDeleteBearerCommand(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
...
void EpcSgwApplication::DoRecvDeleteBearerResponse(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
...
void EpcSgwApplication::DoRecvCreateSessionResponse(Ptr<Packet> packet) {
  ...
  std::list<GtpcCreateSessionResponseMessage::BearerContextCreated>
      bearerContexts = msg.GetBearerContextsCreated();
  ...
  m_s11Socket->SendTo(packetOut, 0,
                      InetSocketAddress(mmeS11Fteid.addr, m_gtpcUdpPort));
}

void EpcSgwApplication::DoRecvModifyBearerResponse(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
  ...
  m_s11Socket->SendTo(packetOut, 0,
                      InetSocketAddress(m_mmeS11Addr, m_gtpcUdpPort));
}

void EpcSgwApplication::DoRecvDeleteBearerRequest(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
