...
GbrQosInformation::GbrQosInformation()
    : gbrDl(0), gbrUl(0), mbrDl(0), mbrUl(0) {}

AllocationRetentionPriority::AllocationRetentionPriority()
    : priorityLevel(0), preemptionCapability(false),
      preemptionVulnerability(false) {}

TypeId EpsBearer::GetTypeId() {
  static TypeId tid =
...
TypeId EpsBearer::GetInstanceTypeId() const { return EpsBearer::GetTypeId(); }

EpsBearer::EpsBearer() : ObjectBase(), qci(NGBR_VIDEO_TCP_DEFAULT) {
  ObjectBase::ConstructSelf(AttributeConstructionList());
}

EpsBearer::EpsBearer(Qci x) : ObjectBase(), qci(x) {
  ObjectBase::ConstructSelf(AttributeConstructionList());
}

EpsBearer::EpsBearer(Qci x, GbrQosInformation y)
    : ObjectBase(), qci(x), gbrQosInfo(y) {
  ObjectBase::ConstructSelf(AttributeConstructionList());
}
...
void EpsBearer::SetRelease(uint8_t release) {
  switch (release) {
  case 8:
  case 9:
  case 10:
...
uint8_t EpsBearer::GetResourceType() const {
  return GetResourceType(m_requirements, qci);
}

uint8_t EpsBearer::GetPriority() const {
  return GetPriority(m_requirements, qci);
}

uint16_t EpsBearer::GetPacketDelayBudgetMs() const {
  return GetPacketDelayBudgetMs(m_requirements, qci);
}

double EpsBearer::GetPacketErrorLossRate() const {
