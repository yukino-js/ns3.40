...
PdcpTag::PdcpTag() : m_senderTimestamp(Seconds(0)) {
  // Nothing to do here
}
...
TypeId PdcpTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t PdcpTag::GetSerializedSize() const { return sizeof(Time); }

void PdcpTag::Serialize(TagBuffer i) const {
  int64_t senderTimestamp = m_senderTimestamp.GetNanoSeconds();
  i.Write((const uint8_t *)&senderTimestamp, sizeof(int64_t));
}

void PdcpTag::Deserialize(TagBuffer i) {
  int64_t senderTimestamp;
  i.Read((uint8_t *)&senderTimestamp, 8);
  m_senderTimestamp = NanoSeconds(senderTimestamp);
}

void PdcpTag::Print(std::ostream &os) const { os << m_senderTimestamp; }

Time PdcpTag::GetSenderTimestamp() const { return m_senderTimestamp; }

void PdcpTag::SetSenderTimestamp(Time senderTimestamp) {
