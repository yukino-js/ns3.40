...
LteRlcAmHeader::LteRlcAmHeader()
    : m_headerLength(0), m_dataControlBit(0xff), m_resegmentationFlag(0xff),
      m_pollingBit(0xff), m_framingInfo(0xff), m_sequenceNumber(0xfffa),
      m_segmentOffset(0xffff), m_lastOffset(0xffff), m_controlPduType(0xff),
      m_ackSn(0xffff) {}
...
void LteRlcAmHeader::SetDataPdu() {
  m_headerLength = 4;
  m_dataControlBit = DATA_PDU;
}

void LteRlcAmHeader::SetControlPdu(uint8_t controlPduType) {
  m_headerLength = 2;
  m_dataControlBit = CONTROL_PDU;
  m_controlPduType = controlPduType;
}

bool LteRlcAmHeader::IsDataPdu() const { return m_dataControlBit == DATA_PDU; }

bool LteRlcAmHeader::IsControlPdu() const {
  return m_dataControlBit == CONTROL_PDU;
}

void LteRlcAmHeader::SetFramingInfo(uint8_t framingInfo) {
  m_framingInfo = framingInfo & 0x03;
}

void LteRlcAmHeader::SetSequenceNumber(SequenceNumber10 sequenceNumber) {
  m_sequenceNumber = sequenceNumber;
}

uint8_t LteRlcAmHeader::GetFramingInfo() const { return m_framingInfo; }

SequenceNumber10 LteRlcAmHeader::GetSequenceNumber() const {
  return m_sequenceNumber;
}

void LteRlcAmHeader::PushExtensionBit(uint8_t extensionBit) {
...
void LteRlcAmHeader::PushLengthIndicator(uint16_t lengthIndicator) {
  m_lengthIndicators.push_back(lengthIndicator);
}

uint8_t LteRlcAmHeader::PopExtensionBit() {
...
uint16_t LteRlcAmHeader::PopLengthIndicator() {
...
void LteRlcAmHeader::SetResegmentationFlag(uint8_t resegFlag) {
  m_resegmentationFlag = resegFlag & 0x01;
}

uint8_t LteRlcAmHeader::GetResegmentationFlag() const {
  return m_resegmentationFlag;
}

void LteRlcAmHeader::SetPollingBit(uint8_t pollingBit) {
  m_pollingBit = pollingBit & 0x01;
}

uint8_t LteRlcAmHeader::GetPollingBit() const { return m_pollingBit; }

void LteRlcAmHeader::SetLastSegmentFlag(uint8_t lsf) {
  m_lastSegmentFlag = lsf & 0x01;
}

uint8_t LteRlcAmHeader::GetLastSegmentFlag() const { return m_lastSegmentFlag; }

void LteRlcAmHeader::SetSegmentOffset(uint16_t segmentOffset) {
  m_segmentOffset = segmentOffset & 0x7FFF;
}

uint16_t LteRlcAmHeader::GetSegmentOffset() const { return m_segmentOffset; }

uint16_t LteRlcAmHeader::GetLastOffset() const { return m_lastOffset; }

void LteRlcAmHeader::SetAckSn(SequenceNumber10 ackSn) { m_ackSn = ackSn; }

bool LteRlcAmHeader::OneMoreNackWouldFitIn(uint16_t bytes) {
  NS_LOG_FUNCTION(this << bytes);
  NS_ASSERT_MSG(m_dataControlBit == CONTROL_PDU &&
                    m_controlPduType == LteRlcAmHeader::STATUS_PDU,
...
void LteRlcAmHeader::PushNack(int nack) {
...
bool LteRlcAmHeader::IsNackPresent(SequenceNumber10 nack) {
  NS_LOG_FUNCTION(this);
  NS_ASSERT_MSG(m_dataControlBit == CONTROL_PDU &&
                    m_controlPduType == LteRlcAmHeader::STATUS_PDU,
                "method allowed only for STATUS PDUs");
  for (auto nackIt = m_nackSnList.begin(); nackIt != m_nackSnList.end();
       ++nackIt) {
...
SequenceNumber10 LteRlcAmHeader::GetAckSn() const { return m_ackSn; }

TypeId LteRlcAmHeader::GetTypeId() {
...
TypeId LteRlcAmHeader::GetInstanceTypeId() const { return GetTypeId(); }

void LteRlcAmHeader::Print(std::ostream &os) const {
...
uint32_t LteRlcAmHeader::GetSerializedSize() const { return m_headerLength; }
