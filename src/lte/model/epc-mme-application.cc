...
EpcMmeApplication::~EpcMmeApplication() { NS_LOG_FUNCTION(this); }

void EpcMmeApplication::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_s1apSapMme;
}

TypeId EpcMmeApplication::GetTypeId() {
...
EpcS1apSapMme *EpcMmeApplication::GetS1apSapMme() { return m_s1apSapMme; }

void EpcMmeApplication::AddSgw(Ipv4Address sgwS11Addr, Ipv4Address mmeS11Addr,
  ...
  m_s11Socket->SetRecvCallback(
      MakeCallback(&EpcMmeApplication::RecvFromS11Socket, this));
}

void EpcMmeApplication::AddEnb(uint16_t gci, Ipv4Address enbS1uAddr,
...
void EpcMmeApplication::AddUe(uint64_t imsi) {
...
uint8_t EpcMmeApplication::AddBearer(uint64_t imsi, Ptr<EpcTft> tft,
                                     EpsBearer bearer) {
  NS_LOG_FUNCTION(this << imsi);
  auto it = m_ueInfoMap.find(imsi);
  NS_ASSERT_MSG(it != m_ueInfoMap.end(),
                "could not find any UE with IMSI " << imsi);
...
void EpcMmeApplication::DoInitialUeMessage(uint64_t mmeUeS1Id,
  ...
  m_s11Socket->SendTo(packet, 0,
                      InetSocketAddress(m_sgwS11Addr, m_gtpcUdpPort));
}

void EpcMmeApplication::DoInitialContextSetupResponse(
...
void EpcMmeApplication::DoPathSwitchRequest(
  ...
  NS_ASSERT_MSG(it != m_ueInfoMap.end(),
                "could not find any UE with IMSI " << imsi);
  NS_LOG_INFO("IMSI " << imsi << " old eNB: " << it->second->cellId
                      << ", new eNB: " << gci);
  it->second->cellId = gci;
  ...
  for (auto &erab : erabToBeSwitchedInDownlinkList) {
    NS_LOG_DEBUG("erabId " << erab.erabId << " eNB "
                           << erab.enbTransportLayerAddress << " TEID "
                           << erab.enbTeid);
...
void EpcMmeApplication::DoErabReleaseIndication(
    uint64_t mmeUeS1Id, uint16_t enbUeS1Id,
...
void EpcMmeApplication::RecvFromS11Socket(Ptr<Socket> socket) {
...
void EpcMmeApplication::DoRecvCreateSessionResponse(GtpcHeader &header,
                                                    Ptr<Packet> packet) {
  ...
  NS_LOG_DEBUG("Send InitialContextSetupRequest to eNB "
               << jt->second->s1apSapEnb);
  jt->second->s1apSapEnb->InitialContextSetupRequest(mmeUeS1Id, enbUeS1Id,
                                                     erabToBeSetupList);
}

void EpcMmeApplication::DoRecvModifyBearerResponse(GtpcHeader &header,
                                                   Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this << header);
  GtpcModifyBearerResponseMessage msg;
  packet->RemoveHeader(msg);
  NS_ASSERT(msg.GetCause() ==
            GtpcModifyBearerResponseMessage::REQUEST_ACCEPTED);
  ...
  NS_LOG_DEBUG("Send PathSwitchRequestAcknowledge to eNB "
               << jt->second->s1apSapEnb);
  jt->second->s1apSapEnb->PathSwitchRequestAcknowledge(
      enbUeS1Id, mmeUeS1Id, cellId, erabToBeSwitchedInUplinkList);
}

void EpcMmeApplication::DoRecvDeleteBearerRequest(GtpcHeader &header,
                                                  Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this << header);
  uint64_t imsi = header.GetTeid();
  NS_LOG_DEBUG("TEID/IMSI " << imsi);
