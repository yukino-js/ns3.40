...
bool operator==(const EpcEnbApplication::EpsFlowId_t &a,
...
bool operator<(const EpcEnbApplication::EpsFlowId_t &a,
  ...
  return ((a.m_rnti < b.m_rnti) ||
          ((a.m_rnti == b.m_rnti) && (a.m_bid < b.m_bid)));
}

TypeId EpcEnbApplication::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::EpcEnbApplication")
          ...
          .AddTraceSource("RxFromEnb",
                          ...
                          MakeTraceSourceAccessor(
                              &EpcEnbApplication::m_rxLteSocketPktTrace),
...
void EpcEnbApplication::DoDispose() {
...
EpcEnbApplication::EpcEnbApplication(Ptr<Socket> lteSocket,
  ...
  m_lteSocket->SetRecvCallback(
      MakeCallback(&EpcEnbApplication::RecvFromLteSocket, this));
  m_lteSocket6->SetRecvCallback(
      MakeCallback(&EpcEnbApplication::RecvFromLteSocket, this));
...
void EpcEnbApplication::AddS1Interface(Ptr<Socket> s1uSocket,
  ...
  m_s1uSocket->SetRecvCallback(
      MakeCallback(&EpcEnbApplication::RecvFromS1uSocket, this));
  m_enbS1uAddress = enbAddress;
  m_sgwS1uAddress = sgwAddress;
}

EpcEnbApplication::~EpcEnbApplication() { NS_LOG_FUNCTION(this); }

void EpcEnbApplication::SetS1SapUser(EpcEnbS1SapUser *s) { m_s1SapUser = s; }

EpcEnbS1SapProvider *EpcEnbApplication::GetS1SapProvider() {
  return m_s1SapProvider;
}

void EpcEnbApplication::SetS1apSapMme(EpcS1apSapMme *s) { m_s1apSapMme = s; }

EpcS1apSapEnb *EpcEnbApplication::GetS1apSapEnb() { return m_s1apSapEnb; }
...
void EpcEnbApplication::DoPathSwitchRequest(
  ...
  for (auto bit = params.bearersToBeSwitched.begin();
  ...
  m_s1apSapMme->PathSwitchRequest(enbUeS1Id, mmeUeS1Id, gci,
                                  erabToBeSwitchedInDownlinkList);
}

void EpcEnbApplication::DoUeContextRelease(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
  auto rntiIt = m_rbidTeidMap.find(rnti);
  if (rntiIt != m_rbidTeidMap.end()) {
    for (auto bidIt = rntiIt->second.begin(); bidIt != rntiIt->second.end();
         ++bidIt) {
...
void EpcEnbApplication::DoInitialContextSetupRequest(
...
void EpcEnbApplication::DoPathSwitchRequestAcknowledge(
...
void EpcEnbApplication::RecvFromLteSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this);
  if (m_lteSocket6) {
...
void EpcEnbApplication::RecvFromS1uSocket(Ptr<Socket> socket) {
  ...
  if (it == m_teidRbidMap.end()) {
    NS_LOG_WARN("UE context at cell id " << m_cellId
...
void EpcEnbApplication::SendToLteSocket(Ptr<Packet> packet, uint16_t rnti,
                                        uint8_t bid) {
  NS_LOG_FUNCTION(this << packet << rnti << bid << packet->GetSize());
  EpsBearerTag tag(rnti, bid);
  packet->AddPacketTag(tag);
  NS_LOG_INFO("Add EpsBearerTag with RNTI " << rnti << " and bearer ID "
                                            << +bid);
  uint8_t ipType;
...
void EpcEnbApplication::SendToS1uSocket(Ptr<Packet> packet, uint32_t teid) {
  ...
  m_s1uSocket->SendTo(packet, flags,
                      InetSocketAddress(m_sgwS1uAddress, m_gtpuUdpPort));
}

void EpcEnbApplication::DoReleaseIndication(uint64_t imsi, uint16_t rnti,
                                            uint8_t bearerId) {
