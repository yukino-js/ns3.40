...
void LteHarqPhy::SubframeIndication(uint32_t frameNo, uint32_t subframeNo) {
  NS_LOG_FUNCTION(this);

  // left shift UL HARQ buffers
  for (auto it = m_miUlHarqProcessesInfoMap.begin();
       it != m_miUlHarqProcessesInfoMap.end(); it++) {
...
double LteHarqPhy::GetAccumulatedMiDl(uint8_t harqProcId, uint8_t layer) {
  NS_LOG_FUNCTION(this << (uint32_t)harqProcId << (uint16_t)layer);
  HarqProcessInfoList_t list =
      m_miDlHarqProcessesInfoMap.at(layer).at(harqProcId);
  double mi = 0.0;
...
HarqProcessInfoList_t LteHarqPhy::GetHarqProcessInfoDl(uint8_t harqProcId,
                                                       uint8_t layer) {
  NS_LOG_FUNCTION(this << (uint32_t)harqProcId << (uint16_t)layer);
  return (m_miDlHarqProcessesInfoMap.at(layer).at(harqProcId));
}

double LteHarqPhy::GetAccumulatedMiUl(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);

  auto it = m_miUlHarqProcessesInfoMap.find(rnti);
  NS_ASSERT_MSG(it != m_miUlHarqProcessesInfoMap.end(),
...
HarqProcessInfoList_t LteHarqPhy::GetHarqProcessInfoUl(uint16_t rnti,
                                                       uint8_t harqProcId) {
  ...
  if (it == m_miUlHarqProcessesInfoMap.end()) {
    ...
    m_miUlHarqProcessesInfoMap.insert(
        std::pair<uint16_t, std::vector<HarqProcessInfoList_t>>(rnti,
                                                                harqList));
...
void LteHarqPhy::UpdateDlHarqProcessStatus(uint8_t id, uint8_t layer, double mi,
                                           uint16_t infoBytes,
                                           uint16_t codeBytes) {
...
void LteHarqPhy::ResetDlHarqProcessStatus(uint8_t id) {
...
void LteHarqPhy::UpdateUlHarqProcessStatus(uint16_t rnti, double mi,
                                           uint16_t infoBytes,
                                           uint16_t codeBytes) {
...
void LteHarqPhy::ResetUlHarqProcessStatus(uint16_t rnti, uint8_t id) {
...
void LteHarqPhy::ClearDlHarqBuffer(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
