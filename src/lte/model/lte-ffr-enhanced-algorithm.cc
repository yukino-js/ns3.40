...
static const FfrEnhancedDownlinkDefaultConfiguration
    g_ffrEnhancedDownlinkDefaultConfiguration[]{
        ...
        {1, 50, 0, 9, 6},    {2, 50, 15, 9, 6},    {3, 50, 30, 9, 6},
        {1, 75, 0, 8, 16},   {2, 75, 24, 8, 16},   {3, 75, 48, 8, 16},
...
LteFfrEnhancedAlgorithm::LteFfrEnhancedAlgorithm()
    : m_ffrSapUser(nullptr), m_ffrRrcSapUser(nullptr), m_measId(0) {
  NS_LOG_FUNCTION(this);
  m_ffrSapProvider = new MemberLteFfrSapProvider<LteFfrEnhancedAlgorithm>(this);
  m_ffrRrcSapProvider =
      new MemberLteFfrRrcSapProvider<LteFfrEnhancedAlgorithm>(this);
}

LteFfrEnhancedAlgorithm::~LteFfrEnhancedAlgorithm() { NS_LOG_FUNCTION(this); }

void LteFfrEnhancedAlgorithm::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_ffrSapProvider;
  delete m_ffrRrcSapProvider;
}

TypeId LteFfrEnhancedAlgorithm::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteFfrEnhancedAlgorithm")
          ...
          .AddAttribute("UlReuse3SubBandwidth",
                        ...
                        MakeUintegerAccessor(
                            &LteFfrEnhancedAlgorithm::m_ulReuse3SubBandwidth),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute("UlReuse1SubBandwidth",
                        ...
                        UintegerValue(4),
                        MakeUintegerAccessor(
                            &LteFfrEnhancedAlgorithm::m_ulReuse1SubBandwidth),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute(
              "DlSubBandOffset",
          ...
          .AddAttribute("DlReuse3SubBandwidth",
                        ...
                        UintegerValue(4),
                        MakeUintegerAccessor(
                            &LteFfrEnhancedAlgorithm::m_dlReuse3SubBandwidth),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute("DlReuse1SubBandwidth",
                        ...
                        UintegerValue(4),
                        MakeUintegerAccessor(
                            &LteFfrEnhancedAlgorithm::m_dlReuse1SubBandwidth),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute(
              "RsrqThreshold",
          ...
          .AddAttribute("CenterAreaPowerOffset",
                        ...
                        UintegerValue(5),
                        MakeUintegerAccessor(
                            &LteFfrEnhancedAlgorithm::m_centerAreaPowerOffset),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute("EdgeAreaPowerOffset",
                        ...
                        UintegerValue(5),
                        MakeUintegerAccessor(
                            &LteFfrEnhancedAlgorithm::m_edgeAreaPowerOffset),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute(
              "DlCqiThreshold",
...
void LteFfrEnhancedAlgorithm::SetLteFfrSapUser(LteFfrSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_ffrSapUser = s;
}

LteFfrSapProvider *LteFfrEnhancedAlgorithm::GetLteFfrSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_ffrSapProvider;
}

void LteFfrEnhancedAlgorithm::SetLteFfrRrcSapUser(LteFfrRrcSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_ffrRrcSapUser = s;
}

LteFfrRrcSapProvider *LteFfrEnhancedAlgorithm::GetLteFfrRrcSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_ffrRrcSapProvider;
}

void LteFfrEnhancedAlgorithm::DoInitialize() {
  NS_LOG_FUNCTION(this);
  LteFfrAlgorithm::DoInitialize();

  NS_ASSERT_MSG(m_dlBandwidth > 24,
                "DlBandwidth must be at least 25 to use EFFR algorithm");
  NS_ASSERT_MSG(m_ulBandwidth > 24,
                "UlBandwidth must be at least 25 to use EFFR algorithm");
...
void LteFfrEnhancedAlgorithm::Reconfigure() {
  NS_LOG_FUNCTION(this);
  if (m_frCellTypeId != 0) {
    SetDownlinkConfiguration(m_frCellTypeId, m_dlBandwidth);
    SetUplinkConfiguration(m_frCellTypeId, m_ulBandwidth);
...
void LteFfrEnhancedAlgorithm::SetDownlinkConfiguration(uint16_t cellId,
                                                       uint8_t bandwidth) {
  NS_LOG_FUNCTION(this);
  for (uint16_t i = 0; i < NUM_DOWNLINK_CONFS; ++i) {
    if ((g_ffrEnhancedDownlinkDefaultConfiguration[i].cellId == cellId) &&
        g_ffrEnhancedDownlinkDefaultConfiguration[i].dlBandwidth ==
            m_dlBandwidth) {
      m_dlSubBandOffset =
          g_ffrEnhancedDownlinkDefaultConfiguration[i].dlSubBandOffset;
...
void LteFfrEnhancedAlgorithm::SetUplinkConfiguration(uint16_t cellId,
                                                     uint8_t bandwidth) {
  NS_LOG_FUNCTION(this);
  for (uint16_t i = 0; i < NUM_UPLINK_CONFS; ++i) {
    if ((g_ffrEnhancedUplinkDefaultConfiguration[i].cellId == cellId) &&
        g_ffrEnhancedUplinkDefaultConfiguration[i].ulBandwidth ==
            m_ulBandwidth) {
      m_ulSubBandOffset =
          g_ffrEnhancedUplinkDefaultConfiguration[i].ulSubBandOffset;
...
int LteFfrEnhancedAlgorithm::GetCqiFromSpectralEfficiency(double s) {
  NS_LOG_FUNCTION(s);
...
void LteFfrEnhancedAlgorithm::InitializeDownlinkRbgMaps() {
  m_dlRbgMap.clear();
  ...
  NS_ASSERT_MSG(m_dlSubBandOffset <= m_dlBandwidth,
                "DlSubBandOffset higher than DlBandwidth");
  NS_ASSERT_MSG(m_dlSubBandOffset + m_dlReuse3SubBandwidth +
                        m_dlReuse1SubBandwidth <=
                    m_dlBandwidth,
  ...
  for (int i = 0; i < m_dlReuse3SubBandwidth / rbgSize; i++) {
    uint8_t offset =
        (m_dlReuse3SubBandwidth + m_dlReuse1SubBandwidth) / rbgSize;
...
void LteFfrEnhancedAlgorithm::InitializeUplinkRbgMaps() {
  m_ulRbgMap.clear();
  ...
  NS_ASSERT_MSG(m_ulSubBandOffset <= m_ulBandwidth,
                "UlSubBandOffset higher than UlBandwidth");
  NS_ASSERT_MSG(m_ulSubBandOffset + m_ulReuse3SubBandwidth +
                        m_ulReuse1SubBandwidth <=
                    m_ulBandwidth,
...
std::vector<bool> LteFfrEnhancedAlgorithm::DoGetAvailableDlRbg() {
  NS_LOG_FUNCTION(this);
  ...
  for (auto it = m_dlRbgAvailableforUe.begin();
...
bool LteFfrEnhancedAlgorithm::DoIsDlRbgAvailableForUe(int rbgId,
                                                      uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
std::vector<bool> LteFfrEnhancedAlgorithm::DoGetAvailableUlRbg() {
  NS_LOG_FUNCTION(this);
  ...
  for (auto it = m_ulRbAvailableforUe.begin(); it != m_ulRbAvailableforUe.end();
       it++) {
    NS_LOG_INFO("RNTI : " << it->first);
...
bool LteFfrEnhancedAlgorithm::DoIsUlRbgAvailableForUe(int rbgId,
                                                      uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
void LteFfrEnhancedAlgorithm::DoReportDlCqiInfo(
  ...
  for (auto it = m_dlCqi.begin(); it != m_dlCqi.end(); it++) {
    ...
    for (uint32_t i = 0; i < (*it).second.m_higherLayerSelected.size(); i++) {
      ...
      NS_LOG_INFO(this << " RNTI " << rnti << " RBG  " << i
                       << " DL-CQI: " << (int)rbgCqi);
    ...
    m_dlRbgAvailableforUe.insert(
        std::pair<uint16_t, std::vector<bool>>(rnti, rbgAvailableMap));
  }
  ...
  for (auto it = m_dlRbgAvailableforUe.begin();
    ...
    m_ulRbAvailableforUe.insert(
        std::pair<uint16_t, std::vector<bool>>(rnti, ulRbAvailableMap));
  }
}

void LteFfrEnhancedAlgorithm::DoReportUlCqiInfo(
...
double LteFfrEnhancedAlgorithm::EstimateUlSinr(
...
uint8_t LteFfrEnhancedAlgorithm::DoGetTpc(uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
uint16_t LteFfrEnhancedAlgorithm::DoGetMinContinuousUlBandwidth() {
  NS_LOG_FUNCTION(this);
  ...
  minContinuousUlBandwidth =
      ((m_ulReuse3SubBandwidth > 0) &&
       (m_ulReuse3SubBandwidth < minContinuousUlBandwidth))
          ? m_ulReuse3SubBandwidth
          : minContinuousUlBandwidth;

  minContinuousUlBandwidth =
      ((m_ulReuse1SubBandwidth > 0) &&
       (m_ulReuse1SubBandwidth < minContinuousUlBandwidth))
          ? m_ulReuse1SubBandwidth
          : minContinuousUlBandwidth;
...
void LteFfrEnhancedAlgorithm::DoReportUeMeas(
...
void LteFfrEnhancedAlgorithm::DoRecvLoadInformation(
