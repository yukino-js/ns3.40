...
LteRlcHeader::LteRlcHeader()
    : m_headerLength(0), m_framingInfo(0xff), m_sequenceNumber(0xfffa) {}
...
void LteRlcHeader::SetFramingInfo(uint8_t framingInfo) {
  m_framingInfo = framingInfo & 0x03;
}

void LteRlcHeader::SetSequenceNumber(SequenceNumber10 sequenceNumber) {
  m_sequenceNumber = sequenceNumber;
}

uint8_t LteRlcHeader::GetFramingInfo() const { return m_framingInfo; }

SequenceNumber10 LteRlcHeader::GetSequenceNumber() const {
...
void LteRlcHeader::PushLengthIndicator(uint16_t lengthIndicator) {
  m_lengthIndicators.push_back(lengthIndicator);
}

uint8_t LteRlcHeader::PopExtensionBit() {
...
uint16_t LteRlcHeader::PopLengthIndicator() {
...
TypeId LteRlcHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
uint32_t LteRlcHeader::GetSerializedSize() const { return m_headerLength; }
