...
TypeId LteUeNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteUeNetDevice")
          ...
          .AddAttribute("LteUeRrc", "The RRC associated to this UeNetDevice",
                        PointerValue(),
          ...
          .AddAttribute("ComponentCarrierMapUe",
                        "List of all component Carrier.", ObjectMapValue(),
                        MakeObjectMapAccessor(&LteUeNetDevice::m_ccMap),
                        MakeObjectMapChecker<ComponentCarrierUe>())
          .AddAttribute(
              "Imsi",
              "International Mobile Subscriber Identity assigned to this UE",
              UintegerValue(0), MakeUintegerAccessor(&LteUeNetDevice::m_imsi),
...
LteUeNetDevice::LteUeNetDevice() : m_isConstructed(false) {
  NS_LOG_FUNCTION(this);
}

LteUeNetDevice::~LteUeNetDevice() { NS_LOG_FUNCTION(this); }

void LteUeNetDevice::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_targetEnb = nullptr;
...
void LteUeNetDevice::UpdateConfig() {
  NS_LOG_FUNCTION(this);
...
Ptr<LteUeMac> LteUeNetDevice::GetMac() const {
  NS_LOG_FUNCTION(this);
  return m_ccMap.at(0)->GetMac();
}

Ptr<LteUeRrc> LteUeNetDevice::GetRrc() const {
  NS_LOG_FUNCTION(this);
  return m_rrc;
}

Ptr<LteUePhy> LteUeNetDevice::GetPhy() const {
  NS_LOG_FUNCTION(this);
...
Ptr<EpcUeNas> LteUeNetDevice::GetNas() const {
  NS_LOG_FUNCTION(this);
  return m_nas;
}

uint64_t LteUeNetDevice::GetImsi() const {
  NS_LOG_FUNCTION(this);
  return m_imsi;
}

uint32_t LteUeNetDevice::GetDlEarfcn() const {
  NS_LOG_FUNCTION(this);
  return m_dlEarfcn;
}

void LteUeNetDevice::SetDlEarfcn(uint32_t earfcn) {
  NS_LOG_FUNCTION(this << earfcn);
  m_dlEarfcn = earfcn;
}

uint32_t LteUeNetDevice::GetCsgId() const {
  NS_LOG_FUNCTION(this);
  return m_csgId;
}
...
Ptr<LteEnbNetDevice> LteUeNetDevice::GetTargetEnb() {
  NS_LOG_FUNCTION(this);
  return m_targetEnb;
}

std::map<uint8_t, Ptr<ComponentCarrierUe>> LteUeNetDevice::GetCcMap() {
  return m_ccMap;
}

void LteUeNetDevice::SetCcMap(std::map<uint8_t, Ptr<ComponentCarrierUe>> ccm) {
  m_ccMap = ccm;
}

void LteUeNetDevice::DoInitialize() {
  NS_LOG_FUNCTION(this);
...
bool LteUeNetDevice::Send(Ptr<Packet> packet, const Address &dest,
