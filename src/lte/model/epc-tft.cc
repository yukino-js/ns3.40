...
std::ostream &operator<<(std::ostream &os, const EpcTft::PacketFilter &f) {
  os << " direction: " << f.direction << " remoteAddress: " << f.remoteAddress
     ...
     << " localAddress: " << f.localAddress << " localMask: " << f.localMask
     ...
     << (uint16_t)f.typeOfService << std::dec << " typeOfServiceMask: 0x"
     << std::hex << (uint16_t)f.typeOfServiceMask << std::dec;
  return os;
}

EpcTft::PacketFilter::PacketFilter()
    : precedence(255), direction(BIDIRECTIONAL), remoteMask("0.0.0.0"),
      localMask("0.0.0.0"), remotePortStart(0), remotePortEnd(65535),
      localPortStart(0), localPortEnd(65535), typeOfService(0),
      typeOfServiceMask(0) {
  NS_LOG_FUNCTION(this);
}

bool EpcTft::PacketFilter::Matches(Direction d, Ipv4Address ra, Ipv4Address la,
                                   uint16_t rp, uint16_t lp, uint8_t tos) {
  ...
  if (d & direction) {
      ...
      if (localMask.IsMatch(localAddress, la)) {
        ...
        if (remotePortStart <= rp && rp <= remotePortEnd) {
          ...
          if (localPortStart <= lp && lp <= localPortEnd) {
            ...
            if ((tos & typeOfServiceMask) ==
                (typeOfService & typeOfServiceMask)) {
            ...
            NS_LOG_LOGIC("lp doesn't match: lp=" << lp
                                                 << " f.lps=" << localPortStart
                                                 << " f.lpe=" << localPortEnd);
          }
        } else {
          NS_LOG_LOGIC("rp doesn't match: rp=" << rp
    ...
    NS_LOG_LOGIC("d doesn't match: d=0x" << std::hex << d << " f.d=0x"
                                         << std::hex << direction << std::dec);
  }
  return false;
}

bool EpcTft::PacketFilter::Matches(Direction d, Ipv6Address ra, Ipv6Address la,
                                   uint16_t rp, uint16_t lp, uint8_t tos) {
  NS_LOG_FUNCTION(this << d << ra << la << rp << lp << (uint16_t)tos);
  if (d & direction) {
    NS_LOG_LOGIC("d matches");
    if (remoteIpv6Prefix.IsMatch(remoteIpv6Address, ra)) {
      ...
      if (localIpv6Prefix.IsMatch(localIpv6Address, la)) {
        ...
        NS_LOG_LOGIC("la doesn't match: la="
                     << la << " f.la=" << localIpv6Address
                     << " f.lprefix=" << localIpv6Prefix);
      }
    } else {
      NS_LOG_LOGIC("ra doesn't match: ra="
                   << ra << " f.ra=" << remoteIpv6Address
...
EpcTft::EpcTft() : m_numFilters(0) { NS_LOG_FUNCTION(this); }

uint8_t EpcTft::Add(PacketFilter f) {
  NS_LOG_FUNCTION(this << f);
  NS_ABORT_IF(m_numFilters >= 16);

  std::list<PacketFilter>::iterator it;
  for (it = m_filters.begin();
       (it != m_filters.end()) && (it->precedence <= f.precedence); ++it) {
  }
...
bool EpcTft::Matches(Direction direction, Ipv4Address remoteAddress,
                     Ipv4Address localAddress, uint16_t remotePort,
                     uint16_t localPort, uint8_t typeOfService) {
  NS_LOG_FUNCTION(this << direction << remoteAddress << localAddress << std::dec
                       << remotePort << localPort << (uint16_t)typeOfService);
  for (auto it = m_filters.begin(); it != m_filters.end(); ++it) {
    if (it->Matches(direction, remoteAddress, localAddress, remotePort,
...
bool EpcTft::Matches(Direction direction, Ipv6Address remoteAddress,
                     Ipv6Address localAddress, uint16_t remotePort,
                     uint16_t localPort, uint8_t typeOfService) {
  NS_LOG_FUNCTION(this << direction << remoteAddress << localAddress << std::dec
                       << remotePort << localPort << (uint16_t)typeOfService);
  for (auto it = m_filters.begin(); it != m_filters.end(); ++it) {
...
std::list<EpcTft::PacketFilter> EpcTft::GetPacketFilters() const {
  NS_LOG_FUNCTION(this);
  return m_filters;
};
