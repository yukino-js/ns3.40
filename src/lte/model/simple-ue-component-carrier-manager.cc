...
SimpleUeCcmMacSapProvider::SimpleUeCcmMacSapProvider(
    SimpleUeComponentCarrierManager *mac)
    : m_mac(mac) {}

void SimpleUeCcmMacSapProvider::TransmitPdu(TransmitPduParameters params) {
  m_mac->DoTransmitPdu(params);
}

void SimpleUeCcmMacSapProvider::ReportBufferStatus(
...
void SimpleUeCcmMacSapUser::NotifyTxOpportunity(
...
void SimpleUeCcmMacSapUser::ReceivePdu(
...
SimpleUeComponentCarrierManager::SimpleUeComponentCarrierManager() {
  ...
  m_ccmRrcSapProvider =
      new MemberLteUeCcmRrcSapProvider<SimpleUeComponentCarrierManager>(this);
...
void SimpleUeComponentCarrierManager::DoDispose() {
  NS_LOG_FUNCTION(this);
...
TypeId SimpleUeComponentCarrierManager::GetTypeId() {
...
LteMacSapProvider *SimpleUeComponentCarrierManager::GetLteMacSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_ccmMacSapProvider;
}

void SimpleUeComponentCarrierManager::DoInitialize() {
  NS_LOG_FUNCTION(this);
  LteUeComponentCarrierManager::DoInitialize();
}

void SimpleUeComponentCarrierManager::DoReportUeMeas(
    uint16_t rnti, LteRrcSap::MeasResults measResults) {
  NS_LOG_FUNCTION(this << rnti << (uint16_t)measResults.measId);
}

void SimpleUeComponentCarrierManager::DoTransmitPdu(
...
void SimpleUeComponentCarrierManager::DoReportBufferStatus(
  ...
  NS_LOG_DEBUG("Size of component carrier LC map "
               << m_componentCarrierLcMap.size());
...
void SimpleUeComponentCarrierManager::DoNotifyHarqDeliveryFailure() {
  NS_LOG_FUNCTION(this);
}

void SimpleUeComponentCarrierManager::DoNotifyTxOpportunity(
    LteMacSapUser::TxOpportunityParameters txOpParams) {
  NS_LOG_FUNCTION(this);
  ...
  NS_LOG_DEBUG(this << " MAC is asking component carrier id = "
                    ...
                    << " with lcid = " << (uint32_t)txOpParams.lcid
                    << " to transmit " << txOpParams.bytes << " bytes");
  (*lcidIt).second->NotifyTxOpportunity(txOpParams);
}

void SimpleUeComponentCarrierManager::DoReceivePdu(
    LteMacSapUser::ReceivePduParameters rxPduParams) {
  NS_LOG_FUNCTION(this);
...
SimpleUeComponentCarrierManager::DoAddLc(
  ...
  NS_ABORT_MSG_IF(it != m_lcAttached.end(),
                  "Warning, LCID " << lcId << " already exist");
  ...
  for (uint8_t ncc = 0; ncc < m_noOfComponentCarriers; ncc++) {
    ...
    if (ccLcMapIt != m_componentCarrierLcMap.end()) {
      ccLcMapIt->second.insert(std::pair<uint8_t, LteMacSapProvider *>(
          lcId, m_macSapProvidersMap.at(ncc)));
    } else {
      std::map<uint8_t, LteMacSapProvider *> empty;
      auto ret = m_componentCarrierLcMap.insert(
          std::pair<uint8_t, std::map<uint8_t, LteMacSapProvider *>>(ncc,
                                                                     empty));
      NS_ABORT_MSG_IF(
          !ret.second,
...
LteMacSapUser *SimpleUeComponentCarrierManager::DoConfigureSignalBearer(
    uint8_t lcid, LteUeCmacSapProvider::LogicalChannelConfig lcConfig,
    LteMacSapUser *msu) {
  NS_LOG_FUNCTION(this);
  auto it = m_lcAttached.find(lcid);
