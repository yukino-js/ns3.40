...
LteInterference::~LteInterference() { NS_LOG_FUNCTION(this); }

void LteInterference::DoDispose() {
  NS_LOG_FUNCTION(this);
...
TypeId LteInterference::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteInterference").SetParent<Object>().SetGroupName("Lte");
  return tid;
}

void LteInterference::StartRx(Ptr<const SpectrumValue> rxPsd) {
  ...
  if (!m_receiving) {
    ...
    for (auto it = m_rsPowerChunkProcessorList.begin();
         it != m_rsPowerChunkProcessorList.end(); ++it) {
      (*it)->Start();
    }
    for (auto it = m_interfChunkProcessorList.begin();
         it != m_interfChunkProcessorList.end(); ++it) {
      (*it)->Start();
    }
    for (auto it = m_sinrChunkProcessorList.begin();
...
void LteInterference::AddSignal(Ptr<const SpectrumValue> spd,
                                const Time duration) {
...
void LteInterference::DoAddSignal(Ptr<const SpectrumValue> spd) {
  NS_LOG_FUNCTION(this << *spd);
  ConditionallyEvaluateChunk();
  (*m_allSignals) += (*spd);
}

void LteInterference::DoSubtractSignal(Ptr<const SpectrumValue> spd,
                                       uint32_t signalId) {
  NS_LOG_FUNCTION(this << *spd);
  ConditionallyEvaluateChunk();
...
void LteInterference::ConditionallyEvaluateChunk() {
  NS_LOG_FUNCTION(this);
  if (m_receiving) {
  ...
  if (m_receiving && (Now() > m_lastChangeTime)) {
    NS_LOG_LOGIC(this << " signal = " << *m_rxSignal << " allSignals = "
                      << *m_allSignals << " noise = " << *m_noise);
...
void LteInterference::SetNoisePowerSpectralDensity(
...
void LteInterference::AddRsPowerChunkProcessor(Ptr<LteChunkProcessor> p) {
  NS_LOG_FUNCTION(this << p);
  m_rsPowerChunkProcessorList.push_back(p);
}

void LteInterference::AddSinrChunkProcessor(Ptr<LteChunkProcessor> p) {
  NS_LOG_FUNCTION(this << p);
  m_sinrChunkProcessorList.push_back(p);
}

void LteInterference::AddInterferenceChunkProcessor(Ptr<LteChunkProcessor> p) {
  NS_LOG_FUNCTION(this << p);
