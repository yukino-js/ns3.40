...
Ipv4Address EpcPgwApplication::UeInfo::GetSgwAddr() { return m_sgwAddr; }

void EpcPgwApplication::UeInfo::SetSgwAddr(Ipv4Address sgwAddr) {
  m_sgwAddr = sgwAddr;
}

Ipv4Address EpcPgwApplication::UeInfo::GetUeAddr() { return m_ueAddr; }

void EpcPgwApplication::UeInfo::SetUeAddr(Ipv4Address ueAddr) {
  m_ueAddr = ueAddr;
}

Ipv6Address EpcPgwApplication::UeInfo::GetUeAddr6() { return m_ueAddr6; }
...
TypeId EpcPgwApplication::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::EpcPgwApplication")
          ...
          .AddTraceSource(
              "RxFromS1u", "Receive data packets from S5 Socket",
              MakeTraceSourceAccessor(&EpcPgwApplication::m_rxS5PktTrace),
              "ns3::EpcPgwApplication::RxTracedCallback");
  return tid;
}

void EpcPgwApplication::DoDispose() {
      ...
      m_gtpcUdpPort(2123)                          // fixed by the standard
{
  ...
  m_s5cSocket->SetRecvCallback(
      MakeCallback(&EpcPgwApplication::RecvFromS5cSocket, this));
}

EpcPgwApplication::~EpcPgwApplication() { NS_LOG_FUNCTION(this); }

bool EpcPgwApplication::RecvFromTunDevice(Ptr<Packet> packet,
                                          const Address &source,
                                          const Address &dest,
                                          uint16_t protocolNumber) {
  NS_LOG_FUNCTION(this << source << dest << protocolNumber << packet
                       << packet->GetSize());
...
void EpcPgwApplication::RecvFromS5uSocket(Ptr<Socket> socket) {
...
void EpcPgwApplication::RecvFromS5cSocket(Ptr<Socket> socket) {
  NS_LOG_FUNCTION(this << socket);
...
void EpcPgwApplication::DoRecvCreateSessionRequest(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
  ...
  for (auto &bearerContext : bearerContexts) {
    ...
    NS_LOG_DEBUG("bearerId " << (uint16_t)bearerContext.epsBearerId << " SGW "
                             << bearerContext.sgwS5uFteid.addr << " TEID "
                             << teid);
  ...
  m_s5cSocket->SendTo(packetOut, 0,
                      InetSocketAddress(sgwS5cFteid.addr, m_gtpcUdpPort));
}

void EpcPgwApplication::DoRecvModifyBearerRequest(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
  ...
  m_s5cSocket->SendTo(packetOut, 0,
                      InetSocketAddress(m_sgwS5Addr, m_gtpcUdpPort));
}

void EpcPgwApplication::DoRecvDeleteBearerCommand(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
...
void EpcPgwApplication::DoRecvDeleteBearerResponse(Ptr<Packet> packet) {
  NS_LOG_FUNCTION(this);
...
void EpcPgwApplication::SendToTunDevice(Ptr<Packet> packet, uint32_t teid) {
  ...
  m_tunDevice->Receive(packet, protocol, m_tunDevice->GetAddress(),
                       m_tunDevice->GetAddress(), NetDevice::PACKET_HOST);
}

void EpcPgwApplication::SendToS5uSocket(Ptr<Packet> packet, Ipv4Address sgwAddr,
                                        uint32_t teid) {
...
void EpcPgwApplication::AddSgw(Ipv4Address sgwS5Addr) {
  NS_LOG_FUNCTION(this << sgwS5Addr);
  m_sgwS5Addr = sgwS5Addr;
}

void EpcPgwApplication::AddUe(uint64_t imsi) {
  NS_LOG_FUNCTION(this << imsi);
  Ptr<UeInfo> ueInfo = Create<UeInfo>();
  m_ueInfoByImsiMap[imsi] = ueInfo;
}

void EpcPgwApplication::SetUeAddress(uint64_t imsi, Ipv4Address ueAddr) {
...
void EpcPgwApplication::SetUeAddress6(uint64_t imsi, Ipv6Address ueAddr) {
  NS_LOG_FUNCTION(this << imsi << ueAddr);
  auto ueit = m_ueInfoByImsiMap.find(imsi);
  NS_ASSERT_MSG(ueit != m_ueInfoByImsiMap.end(), "unknown IMSI " << imsi);
