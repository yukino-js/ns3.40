...
A3RsrpHandoverAlgorithm::~A3RsrpHandoverAlgorithm() { NS_LOG_FUNCTION(this); }

TypeId A3RsrpHandoverAlgorithm::GetTypeId() {
  static TypeId tid =
...
void A3RsrpHandoverAlgorithm::SetLteHandoverManagementSapUser(
...
void A3RsrpHandoverAlgorithm::DoInitialize() {
  NS_LOG_FUNCTION(this);

  uint8_t hysteresisIeValue =
      EutranMeasurementMapping::ActualHysteresis2IeValue(m_hysteresisDb);
  ...
  m_measIds = m_handoverManagementSapUser->AddUeMeasReportConfigForHandover(
      reportConfig);

  LteHandoverAlgorithm::DoInitialize();
}

void A3RsrpHandoverAlgorithm::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_handoverManagementSapProvider;
}

void A3RsrpHandoverAlgorithm::DoReportUeMeas(
    uint16_t rnti, LteRrcSap::MeasResults measResults) {
  NS_LOG_FUNCTION(this << rnti << (uint16_t)measResults.measId);

  if (std::find(begin(m_measIds), end(m_measIds), measResults.measId) ==
      std::end(m_measIds)) {
  ...
  if (measResults.haveMeasResultNeighCells &&
    ...
    for (auto it = measResults.measResultListEutra.begin();
      ...
      if (it->haveRsrpResult) {
        ...
        NS_LOG_WARN("RSRP measurement is missing from cell ID "
                    << it->physCellId);
      }
    }
