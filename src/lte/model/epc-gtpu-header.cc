...
TypeId GtpuHeader::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t GtpuHeader::GetSerializedSize() const { return 12; }

void GtpuHeader::Serialize(Buffer::Iterator start) const {
  Buffer::Iterator i = start;
  uint8_t firstByte = m_version << 5 | m_protocolType << 4 | 0x1 << 3;
  firstByte |=
      m_extensionHeaderFlag << 2 | m_sequenceNumberFlag << 1 | m_nPduNumberFlag;
  i.WriteU8(firstByte);
...
uint32_t GtpuHeader::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
void GtpuHeader::Print(std::ostream &os) const {
...
bool GtpuHeader::GetExtensionHeaderFlag() const {
  return m_extensionHeaderFlag;
}

uint16_t GtpuHeader::GetLength() const { return m_length; }

uint8_t GtpuHeader::GetMessageType() const { return m_messageType; }

uint8_t GtpuHeader::GetNPduNumber() const { return m_nPduNumber; }

bool GtpuHeader::GetNPduNumberFlag() const { return m_nPduNumberFlag; }

uint8_t GtpuHeader::GetNextExtensionType() const { return m_nextExtensionType; }

bool GtpuHeader::GetProtocolType() const { return m_protocolType; }

uint16_t GtpuHeader::GetSequenceNumber() const { return m_sequenceNumber; }

bool GtpuHeader::GetSequenceNumberFlag() const { return m_sequenceNumberFlag; }

uint32_t GtpuHeader::GetTeid() const { return m_teid; }

uint8_t GtpuHeader::GetVersion() const { return m_version; }

void GtpuHeader::SetExtensionHeaderFlag(bool m_extensionHeaderFlag) {
  this->m_extensionHeaderFlag = m_extensionHeaderFlag;
}

void GtpuHeader::SetLength(uint16_t m_length) { this->m_length = m_length; }

void GtpuHeader::SetMessageType(uint8_t m_messageType) {
  this->m_messageType = m_messageType;
}

void GtpuHeader::SetNPduNumber(uint8_t m_nPduNumber) {
  this->m_nPduNumber = m_nPduNumber;
}

void GtpuHeader::SetNPduNumberFlag(bool m_nPduNumberFlag) {
  this->m_nPduNumberFlag = m_nPduNumberFlag;
}

void GtpuHeader::SetNextExtensionType(uint8_t m_nextExtensionType) {
  this->m_nextExtensionType = m_nextExtensionType;
}

void GtpuHeader::SetProtocolType(bool m_protocolType) {
  this->m_protocolType = m_protocolType;
}

void GtpuHeader::SetSequenceNumber(uint16_t m_sequenceNumber) {
  this->m_sequenceNumber = m_sequenceNumber;
}

void GtpuHeader::SetSequenceNumberFlag(bool m_sequenceNumberFlag) {
...
bool GtpuHeader::operator==(const GtpuHeader &b) const {
  return m_version == b.m_version && m_protocolType == b.m_protocolType &&
         ...
         m_messageType == b.m_messageType && m_length == b.m_length &&
         m_teid == b.m_teid && m_sequenceNumber == b.m_sequenceNumber &&
