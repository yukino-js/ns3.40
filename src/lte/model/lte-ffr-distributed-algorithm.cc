...
LteFfrDistributedAlgorithm::LteFfrDistributedAlgorithm()
  ...
  m_ffrSapProvider =
      new MemberLteFfrSapProvider<LteFfrDistributedAlgorithm>(this);
  m_ffrRrcSapProvider =
      new MemberLteFfrRrcSapProvider<LteFfrDistributedAlgorithm>(this);
}
...
void LteFfrDistributedAlgorithm::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_ffrSapProvider;
  delete m_ffrRrcSapProvider;
}

TypeId LteFfrDistributedAlgorithm::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteFfrDistributedAlgorithm")
          ...
          .AddAttribute("CalculationInterval",
                        ...
                        TimeValue(Seconds(1)),
                        MakeTimeAccessor(
                            &LteFfrDistributedAlgorithm::m_calculationInterval),
                        MakeTimeChecker())
          .AddAttribute(
              "RsrqThreshold",
              ...
              MakeUintegerAccessor(
                  &LteFfrDistributedAlgorithm::m_edgeSubBandRsrqThreshold),
          ...
          .AddAttribute(
              ...
              UintegerValue(20),
              MakeUintegerAccessor(
                  &LteFfrDistributedAlgorithm::m_rsrpDifferenceThreshold),
              MakeUintegerChecker<uint8_t>())
          .AddAttribute("CenterPowerOffset",
                        ...
                        UintegerValue(5),
                        MakeUintegerAccessor(
                            &LteFfrDistributedAlgorithm::m_centerPowerOffset),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute("EdgePowerOffset",
                        ...
                        UintegerValue(5),
                        MakeUintegerAccessor(
                            &LteFfrDistributedAlgorithm::m_edgePowerOffset),
                        MakeUintegerChecker<uint8_t>())
          .AddAttribute(
          ...
          .AddAttribute(
              ...
              UintegerValue(1),
              MakeUintegerAccessor(
                  &LteFfrDistributedAlgorithm::m_centerAreaTpc),
              MakeUintegerChecker<uint8_t>())
          .AddAttribute(
              "EdgeAreaTpc",
...
void LteFfrDistributedAlgorithm::SetLteFfrSapUser(LteFfrSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_ffrSapUser = s;
}

LteFfrSapProvider *LteFfrDistributedAlgorithm::GetLteFfrSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_ffrSapProvider;
}

void LteFfrDistributedAlgorithm::SetLteFfrRrcSapUser(LteFfrRrcSapUser *s) {
  NS_LOG_FUNCTION(this << s);
  m_ffrRrcSapUser = s;
}

LteFfrRrcSapProvider *LteFfrDistributedAlgorithm::GetLteFfrRrcSapProvider() {
  NS_LOG_FUNCTION(this);
  return m_ffrRrcSapProvider;
}

void LteFfrDistributedAlgorithm::DoInitialize() {
  NS_LOG_FUNCTION(this);
  ...
  m_calculationEvent =
      Simulator::ScheduleNow(&LteFfrDistributedAlgorithm::Calculate, this);
}

void LteFfrDistributedAlgorithm::Reconfigure() {
  NS_LOG_FUNCTION(this);
  if (m_frCellTypeId != 0) {
    SetDownlinkConfiguration(m_frCellTypeId, m_dlBandwidth);
    SetUplinkConfiguration(m_frCellTypeId, m_ulBandwidth);
...
void LteFfrDistributedAlgorithm::SetDownlinkConfiguration(uint16_t cellId,
                                                          uint8_t bandwidth) {
  NS_LOG_FUNCTION(this);
}

void LteFfrDistributedAlgorithm::SetUplinkConfiguration(uint16_t cellId,
                                                        uint8_t bandwidth) {
  NS_LOG_FUNCTION(this);
}

void LteFfrDistributedAlgorithm::InitializeDownlinkRbgMaps() {
  NS_LOG_FUNCTION(this);
  m_dlRbgMap.clear();
  int rbgSize = GetRbgSize(m_dlBandwidth);
...
void LteFfrDistributedAlgorithm::InitializeUplinkRbgMaps() {
  NS_LOG_FUNCTION(this);
  m_ulRbgMap.clear();
  m_ulRbgMap.resize(m_ulBandwidth, false);
}

std::vector<bool> LteFfrDistributedAlgorithm::DoGetAvailableDlRbg() {
  NS_LOG_FUNCTION(this);
...
bool LteFfrDistributedAlgorithm::DoIsDlRbgAvailableForUe(int rbgId,
                                                         uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
std::vector<bool> LteFfrDistributedAlgorithm::DoGetAvailableUlRbg() {
  NS_LOG_FUNCTION(this);
...
bool LteFfrDistributedAlgorithm::DoIsUlRbgAvailableForUe(int rbId,
                                                         uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
void LteFfrDistributedAlgorithm::DoReportDlCqiInfo(
...
void LteFfrDistributedAlgorithm::DoReportUlCqiInfo(
...
uint8_t LteFfrDistributedAlgorithm::DoGetTpc(uint16_t rnti) {
  NS_LOG_FUNCTION(this);
...
uint16_t LteFfrDistributedAlgorithm::DoGetMinContinuousUlBandwidth() {
  NS_LOG_FUNCTION(this);
  ...
  minContinuousUlBandwidth =
      ((m_edgeRbNum > 0) && (m_edgeRbNum < minContinuousUlBandwidth))
          ? m_edgeRbNum
          : minContinuousUlBandwidth;
...
void LteFfrDistributedAlgorithm::DoReportUeMeas(
    uint16_t rnti, LteRrcSap::MeasResults measResults) {
  NS_LOG_FUNCTION(this << rnti << (uint16_t)measResults.measId);
  NS_LOG_INFO("CellId: " << m_cellId << " RNTI :" << rnti << " MeasId: "
                         << (uint16_t)measResults.measId << " RSRP: "
  ...
  if (measResults.measId == m_rsrqMeasId) {
    ...
    UpdateNeighbourMeasurements(rnti, m_cellId,
    ...
    if (measResults.haveMeasResultNeighCells &&
      ...
      for (auto it = measResults.measResultListEutra.begin();
        ...
        NS_ASSERT_MSG(it->haveRsrpResult == true,
                      "RSRP measurement is missing from cellId "
                          << it->physCellId);
        NS_ASSERT_MSG(it->haveRsrqResult == true,
                      "RSRQ measurement is missing from cellId "
                          << it->physCellId);
        UpdateNeighbourMeasurements(rnti, it->physCellId, it->rsrpResult,
                                    it->rsrqResult);

        bool found = false;
        for (auto ncIt = m_neighborCell.begin(); ncIt != m_neighborCell.end();
             ncIt++) {
...
void LteFfrDistributedAlgorithm::Calculate() {
  NS_LOG_FUNCTION(this);
  m_calculationEvent = Simulator::Schedule(
      m_calculationInterval, &LteFfrDistributedAlgorithm::Calculate, this);
  ...
  if (edgeUeNum != 0) {
    ...
    for (auto cellIt = m_cellWeightMap.begin(); cellIt != m_cellWeightMap.end();
         cellIt++) {
    ...
    for (auto it = metricA.begin(); it != metricA.end(); ++it) {
      sortedMetricA.insert(
          std::pair<uint64_t, uint16_t>(it->second, it->first));
    }
...
void LteFfrDistributedAlgorithm::SendLoadInformation(uint16_t targetCellId) {
  NS_LOG_FUNCTION(this);
  ...
  cii.ulInterferenceOverloadIndicationList =
      m_currentUlInterferenceOverloadIndicationList;
  cii.ulHighInterferenceInformationList =
      m_currentUlHighInterferenceInformationList;
...
void LteFfrDistributedAlgorithm::DoRecvLoadInformation(
    EpcX2Sap::LoadInformationParams params) {
  NS_LOG_FUNCTION(this);
  NS_LOG_INFO("CellId: " << m_cellId
                         << " Recv X2 message: LOAD INFORMATION from CellId:"
...
void LteFfrDistributedAlgorithm::UpdateNeighbourMeasurements(uint16_t rnti,
                                                             uint16_t cellId,
                                                             uint8_t rsrp,
                                                             uint8_t rsrq) {
