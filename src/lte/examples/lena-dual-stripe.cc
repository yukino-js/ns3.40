...
FemtocellBlockAllocator::FemtocellBlockAllocator(Box area,
                                                 ...
                                                 uint32_t nFloors)
    : m_area(area), m_nApartmentsX(nApartmentsX), m_nFloors(nFloors),
      m_xSize(nApartmentsX * 10 + 20), m_ySize(70) {
...
void FemtocellBlockAllocator::Create() {
  ...
  uint32_t attempt = 0;
  do {
    NS_ASSERT_MSG(attempt < 100, "Too many failed attempts to position "
  ...
  for (auto it = NodeList::Begin(); it != NodeList::End(); ++it) {
    ...
    for (int j = 0; j < nDevs; j++) {
      Ptr<LteEnbNetDevice> enbdev =
          node->GetDevice(j)->GetObject<LteEnbNetDevice>();
      if (enbdev) {
        Vector pos = node->GetObject<MobilityModel>()->GetPosition();
...
int main(int argc, char *argv[]) {
  ...
  if (nMacroEnbSites > 0) {
    ...
    uint32_t biRowIndex =
        (currentSite / (nMacroEnbSitesX + nMacroEnbSitesX + 1));
    uint32_t biRowRemainder =
        currentSite % (nMacroEnbSitesX + nMacroEnbSitesX + 1);
  ...
  uint32_t nHomeEnbs = round(4 * nApartmentsX * nBlocks * nFloors *
                             homeEnbDeploymentRatio * homeEnbActivationRatio);
  ...
  lteHexGridEnbTopologyHelper->SetAttribute("MinX",
  ...
  if (generateRem) {
    ...
    remHelper->SetAttribute(
        "Channel", PointerValue(lteHelper->GetDownlinkSpectrumChannel()));
