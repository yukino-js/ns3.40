...
void LteSecondaryCellSelectionTestCase::DoRun() {
  NS_LOG_FUNCTION(this << GetName());
  ...
  for (auto it : ueCcMap) {
    NS_LOG_DEBUG("Assign DL EARFCN "
                 << it.second->GetDlEarfcn() << " to UE "
                 << ueDevs.Get(it.first)->GetNode()->GetId());
    DynamicCast<LteUeNetDevice>(ueDevs.Get(it.first))
...
void LteSecondaryCellSelectionTestCase::ConnectionEstablishedCallback(
    std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti) {
