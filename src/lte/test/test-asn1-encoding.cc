...
RrcHeaderTestCase::CreateRadioResourceConfigDedicated() {
  ...
  physicalConfigDedicated.pdschConfigDedicated.pa =
...
void RrcHeaderTestCase::AssertEqualRadioResourceConfigDedicated(
    LteRrcSap::RadioResourceConfigDedicated rrcd1,
    LteRrcSap::RadioResourceConfigDedicated rrcd2) {
  ...
  for (; it3 != srcDrbToAddModList.end(); it3++, it4++) {
    NS_TEST_ASSERT_MSG_EQ(it3->epsBearerIdentity, it4->epsBearerIdentity,
                          "epsBearerIdentity");
    NS_TEST_ASSERT_MSG_EQ(it3->drbIdentity, it4->drbIdentity, "drbIdentity");
    NS_TEST_ASSERT_MSG_EQ(it3->rlcConfig.choice, it4->rlcConfig.choice,
                          "rlcConfig.choice");
  ...
  if (rrcd1.havePhysicalConfigDedicated) {
    NS_TEST_ASSERT_MSG_EQ(
        rrcd1.physicalConfigDedicated.haveSoundingRsUlConfigDedicated,
        rrcd2.physicalConfigDedicated.haveSoundingRsUlConfigDedicated,
        "haveSoundingRsUlConfigDedicated");

    NS_TEST_ASSERT_MSG_EQ(
        rrcd1.physicalConfigDedicated.soundingRsUlConfigDedicated.type,
    ...
    NS_TEST_ASSERT_MSG_EQ(rrcd1.physicalConfigDedicated
                              .soundingRsUlConfigDedicated.srsConfigIndex,
                          rrcd2.physicalConfigDedicated
                              .soundingRsUlConfigDedicated.srsConfigIndex,
                          "soundingRsUlConfigDedicated.srsConfigIndex");
    ...
    if (rrcd1.physicalConfigDedicated.haveAntennaInfoDedicated) {
      NS_TEST_ASSERT_MSG_EQ(
          rrcd1.physicalConfigDedicated.antennaInfo.transmissionMode,
    ...
    if (rrcd1.physicalConfigDedicated.havePdschConfigDedicated) {
      NS_TEST_ASSERT_MSG_EQ(
          rrcd1.physicalConfigDedicated.pdschConfigDedicated.pa,
...
void RrcConnectionRequestTestCase::DoRun() {
...
void RrcConnectionSetupTestCase::DoRun() {
  packet = Create<Packet>();
  ...
  AssertEqualRadioResourceConfigDedicated(
      source.GetRadioResourceConfigDedicated(),
...
void RrcConnectionSetupCompleteTestCase::DoRun() {
  packet = Create<Packet>();
...
RrcConnectionReconfigurationCompleteTestCase::
    RrcConnectionReconfigurationCompleteTestCase()
    : RrcHeaderTestCase(
          "Testing RrcConnectionReconfigurationCompleteTestCase") {}

void RrcConnectionReconfigurationCompleteTestCase::DoRun() {
  packet = Create<Packet>();
  NS_LOG_DEBUG(
      "============= RrcConnectionReconfigurationCompleteTestCase ===========");
...
void RrcConnectionReconfigurationTestCase::DoRun() {
  packet = Create<Packet>();
  NS_LOG_DEBUG(
      "============= RrcConnectionReconfigurationTestCase ===========");
  ...
  measObjectToAddMod.measObjectEutra.blackCellsToAddModList.push_back(
      blackCellsToAddMod);
  ...
  reportConfigToAddMod.reportConfigEutra.eventId =
  ...
  reportConfigToAddMod.reportConfigEutra.reportInterval =
  ...
  msg.mobilityControlInfo.radioResourceConfigCommon.rachConfigCommon
  ...
  NS_TEST_ASSERT_MSG_EQ(source.GetHaveMeasConfig(),
                        destination.GetHaveMeasConfig(), "GetHaveMeasConfig");
  ...
  if (source.GetHaveMobilityControlInfo()) {
    ...
    if (source.GetMobilityControlInfo().haveCarrierFreq) {
      NS_TEST_ASSERT_MSG_EQ(
          source.GetMobilityControlInfo().carrierFreq.dlCarrierFreq,
          destination.GetMobilityControlInfo().carrierFreq.dlCarrierFreq,
          "GetMobilityControlInfo().carrierFreq.dlCarrierFreq");
      NS_TEST_ASSERT_MSG_EQ(
          source.GetMobilityControlInfo().carrierFreq.ulCarrierFreq,
    ...
    if (source.GetMobilityControlInfo().haveCarrierBandwidth) {
      NS_TEST_ASSERT_MSG_EQ(
          source.GetMobilityControlInfo().carrierBandwidth.dlBandwidth,
          destination.GetMobilityControlInfo().carrierBandwidth.dlBandwidth,
          "GetMobilityControlInfo().carrierBandwidth.dlBandwidth");
      NS_TEST_ASSERT_MSG_EQ(
          source.GetMobilityControlInfo().carrierBandwidth.ulBandwidth,
    ...
    if (source.GetMobilityControlInfo().haveRachConfigDedicated) {
      NS_TEST_ASSERT_MSG_EQ(
          ...
          destination.GetMobilityControlInfo()
              .rachConfigDedicated.raPreambleIndex,
      ...
      NS_TEST_ASSERT_MSG_EQ(
          ...
          destination.GetMobilityControlInfo()
              .rachConfigDedicated.raPrachMaskIndex,
...
void HandoverPreparationInfoTestCase::DoRun() {
  packet = Create<Packet>();
  ...
  msg.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
      .csgIndication = true;
  msg.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
      .cellIdentity = 5;
  msg.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
      .csgIdentity = 4;
  msg.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo
      .plmnIdentityInfo.plmnIdentity = 123;
  ...
  msg.asConfig.sourceSystemInformationBlockType2.radioResourceConfigCommon
  ...
  NS_TEST_ASSERT_MSG_EQ(source.GetAsConfig().sourceUeIdentity,
                        ...
                        "sourceUeIdentity");
  NS_TEST_ASSERT_MSG_EQ(
      source.GetAsConfig().sourceMasterInformationBlock.dlBandwidth,
      destination.GetAsConfig().sourceMasterInformationBlock.dlBandwidth,
      "dlBandwidth");
  NS_TEST_ASSERT_MSG_EQ(
      source.GetAsConfig().sourceMasterInformationBlock.systemFrameNumber,
      destination.GetAsConfig().sourceMasterInformationBlock.systemFrameNumber,
      "systemFrameNumber");
  NS_TEST_ASSERT_MSG_EQ(
      source.GetAsConfig()
          .sourceSystemInformationBlockType1.cellAccessRelatedInfo
          .plmnIdentityInfo.plmnIdentity,
      destination.GetAsConfig()
          .sourceSystemInformationBlockType1.cellAccessRelatedInfo
          .plmnIdentityInfo.plmnIdentity,
      "plmnIdentity");
  NS_TEST_ASSERT_MSG_EQ(source.GetAsConfig()
...
void RrcConnectionReestablishmentRequestTestCase::DoRun() {
  packet = Create<Packet>();
  NS_LOG_DEBUG(
      "============= RrcConnectionReestablishmentRequestTestCase ===========");
...
void RrcConnectionReestablishmentTestCase::DoRun() {
  packet = Create<Packet>();
  NS_LOG_DEBUG(
      "============= RrcConnectionReestablishmentTestCase ===========");
...
void RrcConnectionReestablishmentCompleteTestCase::DoRun() {
  packet = Create<Packet>();
  NS_LOG_DEBUG(
      "============= RrcConnectionReestablishmentCompleteTestCase ===========");
...
void RrcConnectionRejectTestCase::DoRun() {
  packet = Create<Packet>();
...
void MeasurementReportTestCase::DoRun() {
  packet = Create<Packet>();
  ...
  if (srcMeas.haveMeasResultNeighCells) {
    ...
    for (; itsrc != srcMeas.measResultListEutra.end(); itsrc++, itdst++) {
      NS_TEST_ASSERT_MSG_EQ(itsrc->physCellId, itdst->physCellId,
                            "Different physCellId!");

      NS_TEST_ASSERT_MSG_EQ(itsrc->haveCgiInfo, itdst->haveCgiInfo,
      ...
      if (itsrc->haveCgiInfo) {
        ...
        if (!itsrc->cgiInfo.plmnIdentityList.empty()) {
          ...
          for (; itsrc2 != itsrc->cgiInfo.plmnIdentityList.begin();
               itsrc2++, itdst2++) {
            NS_TEST_ASSERT_MSG_EQ(*itsrc2, *itdst2,
      ...
      if (itsrc->haveRsrpResult) {
        NS_TEST_ASSERT_MSG_EQ(itsrc->rsrpResult, itdst->rsrpResult,
                              "Different rsrpResult!");
      }
      ...
      if (itsrc->haveRsrqResult) {
        NS_TEST_ASSERT_MSG_EQ(itsrc->rsrqResult, itdst->rsrqResult,
                              "Different rsrqResult!");
      }
    }
  }
