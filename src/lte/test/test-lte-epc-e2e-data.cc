...
void LteEpcE2eDataTestCase::DoRun() {
  ...
  enbMobility.SetPositionAllocator(
      ...
      DoubleValue(10000.0), "GridWidth", UintegerValue(3), "LayoutType",
  ...
  for (auto enbit = m_enbTestData.begin(); enbit < m_enbTestData.end();
       ++enbit, ++enbLteDevIt) {
    ...
    Vector enbPosition =
        (*enbLteDevIt)->GetNode()->GetObject<MobilityModel>()->GetPosition();
    MobilityHelper ueMobility;
    ueMobility.SetPositionAllocator(
        "ns3::UniformDiscPositionAllocator", "X", DoubleValue(enbPosition.x),
        "Y", DoubleValue(enbPosition.y), "rho", DoubleValue(100.0));
  ...
  lteHelper->GetPdcpStats()->SetAttribute("StartTime",
                                          TimeValue(Seconds(statsStartTime)));
  lteHelper->GetPdcpStats()->SetAttribute("EpochDuration",
    ...
    for (auto ueit = enbit->ues.begin(); ueit < enbit->ues.end(); ++ueit) {
      ...
      for (uint32_t b = 0; b < ueit->bearers.size(); ++b) {
        ...
        uint32_t txPktsPdcpDl =
            lteHelper->GetPdcpStats()->GetDlTxPackets(imsi, lcid);
        uint32_t rxPktsPdcpDl =
            lteHelper->GetPdcpStats()->GetDlRxPackets(imsi, lcid);
        uint32_t txPktsPdcpUl =
            lteHelper->GetPdcpStats()->GetUlTxPackets(imsi, lcid);
        uint32_t rxPktsPdcpUl =
            lteHelper->GetPdcpStats()->GetUlRxPackets(imsi, lcid);
...
LteEpcE2eDataTestSuite::LteEpcE2eDataTestSuite()
  ...
  v5.push_back(e5);
  AddTestCase(
      new LteEpcE2eDataTestCase("1 eNB, 1UE with 1000 byte packets", v5),
      TestCase::EXTENSIVE);
  ...
  v6.push_back(e6);
  AddTestCase(
      new LteEpcE2eDataTestCase("1 eNB, 1UE with 1400 byte packets", v6),
      TestCase::EXTENSIVE);
