...
void LteTestDlSchedulingCallback2(
    LteCqiGenerationDlPowerControlTestCase *testcase, std::string path,
    DlSchedulingCallbackInfo dlInfo) {
  testcase->DlScheduling(dlInfo);
}

void LteTestUlSchedulingCallback2(
    LteCqiGenerationDlPowerControlTestCase *testcase, std::string path,
...
LteCqiGenerationTestSuite::LteCqiGenerationTestSuite()
  ...
  AddTestCase(
      new LteCqiGenerationTestCase("UsePdcchForCqiGeneration", false, 4, 2),
      TestCase::QUICK);
  AddTestCase(
      new LteCqiGenerationTestCase("UsePdschForCqiGeneration", true, 28, 2),
      TestCase::QUICK);

  AddTestCase(new LteCqiGenerationDlPowerControlTestCase(
                  "CqiGenerationWithDlPowerControl",
...
LteCqiGenerationTestCase::LteCqiGenerationTestCase(
    std::string name, bool usePdcchForCqiGeneration, uint16_t dlMcs,
    uint16_t ulMcs)
    : TestCase("Downlink Power Control: " + name), m_dlMcs(dlMcs),
      m_ulMcs(ulMcs) {
...
void LteCqiGenerationTestCase::DoRun() {
...
LteCqiGenerationDlPowerControlTestCase::LteCqiGenerationDlPowerControlTestCase(
    std::string name, uint8_t cell0Pa, uint8_t cell1Pa, uint16_t dlMcs,
    uint16_t ulMcs)
    : TestCase("Downlink Power Control: " + name), m_cell0Pa(cell0Pa),
      m_cell1Pa(cell1Pa), m_dlMcs(dlMcs), m_ulMcs(ulMcs) {
  NS_LOG_INFO("Creating LteCqiGenerationTestCase");
}
...
void LteCqiGenerationDlPowerControlTestCase::UlScheduling(uint32_t frameNo,
                                                          uint32_t subframeNo,
                                                          uint16_t rnti,
                                                          uint8_t mcs,
                                                          uint16_t sizeTb) {
...
void LteCqiGenerationDlPowerControlTestCase::DoRun() {
  NS_LOG_DEBUG("LteCqiGenerationTestCase");
  ...
  Ptr<LteFfrSimple> simpleFfrAlgorithmEnb0 =
  ...
  Ptr<LteFfrSimple> simpleFfrAlgorithmEnb1 =
      DynamicCast<LteFfrSimple>(tmp.GetObject());
