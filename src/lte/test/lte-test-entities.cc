...
void LteTestRrc::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_pdcpSapUser;
}

void LteTestRrc::SetDevice(Ptr<NetDevice> device) { m_device = device; }

void LteTestRrc::SetLtePdcpSapProvider(LtePdcpSapProvider *s) {
  m_pdcpSapProvider = s;
}

LtePdcpSapUser *LteTestRrc::GetLtePdcpSapUser() { return m_pdcpSapUser; }

std::string LteTestRrc::GetDataReceived() {
  NS_LOG_FUNCTION(this);
...
uint32_t LteTestRrc::GetRxPdus() {
  NS_LOG_FUNCTION(this << m_rxPdus);
  return m_rxPdus;
}

uint32_t LteTestRrc::GetRxBytes() {
  NS_LOG_FUNCTION(this << m_rxBytes);
  return m_rxBytes;
}

Time LteTestRrc::GetTxLastTime() {
  NS_LOG_FUNCTION(this << m_txLastTime);
  return m_txLastTime;
}

Time LteTestRrc::GetRxLastTime() {
  NS_LOG_FUNCTION(this << m_rxLastTime);
  return m_rxLastTime;
}

void LteTestRrc::SetArrivalTime(Time arrivalTime) {
  NS_LOG_FUNCTION(this << arrivalTime);
  m_arrivalTime = arrivalTime;
}

void LteTestRrc::SetPduSize(uint32_t pduSize) {
...
void LteTestRrc::Start() {
  ...
  if (haveContext) {
    ...
    Simulator::Schedule(Seconds(0), &LtePdcpSapProvider::TransmitPdcpSdu,
                        m_pdcpSapProvider, p);
  }
...
LteTestPdcp::~LteTestPdcp() { NS_LOG_FUNCTION(this); }

void LteTestPdcp::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_rlcSapUser;
}

void LteTestPdcp::SetLteRlcSapProvider(LteRlcSapProvider *s) {
  m_rlcSapProvider = s;
}

LteRlcSapUser *LteTestPdcp::GetLteRlcSapUser() { return m_rlcSapUser; }

std::string LteTestPdcp::GetDataReceived() {
  NS_LOG_FUNCTION(this);
...
void LteTestPdcp::SendData(Time time, std::string dataToSend) {
...
void LteTestMac::SetDevice(Ptr<NetDevice> device) { m_device = device; }

void LteTestMac::SetLteMacSapUser(LteMacSapUser *s) { m_macSapUser = s; }

LteMacSapProvider *LteTestMac::GetLteMacSapProvider() {
  return m_macSapProvider;
}

void LteTestMac::SetLteMacLoopback(Ptr<LteTestMac> s) { m_macLoopback = s; }

std::string LteTestMac::GetDataReceived() {
  NS_LOG_FUNCTION(this);
  return m_receivedData;
}
...
uint32_t LteTestMac::GetTxBytes() {
  NS_LOG_FUNCTION(this << m_txBytes);
  return m_txBytes;
}

uint32_t LteTestMac::GetRxPdus() {
  NS_LOG_FUNCTION(this << m_rxPdus);
  return m_rxPdus;
}

uint32_t LteTestMac::GetRxBytes() {
  NS_LOG_FUNCTION(this << m_rxBytes);
  return m_rxBytes;
}

void LteTestMac::SendTxOpportunity(Time time, uint32_t bytes) {
  ...
  if (haveContext) {
    Simulator::ScheduleWithContext(node->GetId(), time,
  ...
  if (m_txOpportunityMode == RANDOM_MODE) {
    if (m_txOppTime != Seconds(0)) {
      Simulator::Schedule(m_txOppTime, &LteTestMac::SendTxOpportunity, this,
                          m_txOppTime, m_txOppSize);
    }
  }
}

void LteTestMac::SetPdcpHeaderPresent(bool present) {
  NS_LOG_FUNCTION(this << present);
  m_pdcpHeaderPresent = present;
}

void LteTestMac::SetRlcHeaderType(uint8_t rlcHeaderType) {
  NS_LOG_FUNCTION(this << rlcHeaderType);
  m_rlcHeaderType = rlcHeaderType;
}

void LteTestMac::SetTxOpportunityMode(uint8_t mode) {
...
void LteTestMac::SetTxOppTime(Time txOppTime) {
  NS_LOG_FUNCTION(this << txOppTime);
  m_txOppTime = txOppTime;
}

void LteTestMac::SetTxOppSize(uint32_t txOppSize) {
...
void LteTestMac::DoReportBufferStatus(
    LteMacSapProvider::ReportBufferStatusParameters params) {
  NS_LOG_FUNCTION(this << params.txQueueSize << params.retxQueueSize
  ...
  if (m_txOpportunityMode == AUTOMATIC_MODE) {
    ...
    int32_t size =
        params.statusPduSize + params.txQueueSize + params.retxQueueSize;
    ...
    while (size > 0) {
      EventId e = Simulator::Schedule(time, &LteMacSapUser::NotifyTxOpportunity,
                                      m_macSapUser, txOpParmas);
...
bool LteTestMac::Receive(Ptr<NetDevice> nd, Ptr<const Packet> p,
...
EpcTestRrc::EpcTestRrc() : m_s1SapProvider(nullptr) {
  NS_LOG_FUNCTION(this);
  m_s1SapUser = new MemberEpcEnbS1SapUser<EpcTestRrc>(this);
}

EpcTestRrc::~EpcTestRrc() { NS_LOG_FUNCTION(this); }

void EpcTestRrc::DoDispose() {
  NS_LOG_FUNCTION(this);
  delete m_s1SapUser;
}

TypeId EpcTestRrc::GetTypeId() {
...
void EpcTestRrc::SetS1SapProvider(EpcEnbS1SapProvider *s) {
  m_s1SapProvider = s;
}

EpcEnbS1SapUser *EpcTestRrc::GetS1SapUser() { return m_s1SapUser; }

void EpcTestRrc::DoInitialContextSetupRequest(
    EpcEnbS1SapUser::InitialContextSetupRequestParameters request) {}

void EpcTestRrc::DoDataRadioBearerSetupRequest(
    EpcEnbS1SapUser::DataRadioBearerSetupRequestParameters request) {}

void EpcTestRrc::DoPathSwitchRequestAcknowledge(
