...
LteUplinkDataSinrTestCase::LteUplinkDataSinrTestCase(Ptr<SpectrumValue> sv1,
                                                     ...
                                                     std::string name)
    : TestCase("SINR calculation in uplink data frame: " + name), m_sv1(sv1),
      m_sv2(sv2), m_sm(sv1->GetSpectrumModel()), m_expectedSinr(sinr) {
...
void LteUplinkDataSinrTestCase::DoRun() {
  ...
  Ptr<LteSpectrumSignalParametersDataFrame> sp2 =
      Create<LteSpectrumSignalParametersDataFrame>();
  sp2->psd = m_sv2;
  sp2->txPhy = nullptr;
  sp2->duration = ds;
  ...
  NS_TEST_EXPECT_MSG_NE(actualSinrCatcher.GetValue(), nullptr,
...
LteUplinkSrsSinrTestCase::LteUplinkSrsSinrTestCase(Ptr<SpectrumValue> sv1,
                                                   ...
                                                   Ptr<SpectrumValue> sinr,
                                                   std::string name)
    : TestCase("SINR calculation in uplink srs frame: " + name), m_sv1(sv1),
      m_sv2(sv2), m_sm(sv1->GetSpectrumModel()), m_expectedSinr(sinr) {
...
void LteUplinkSrsSinrTestCase::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_SPECTRUM_VALUE_EQ_TOL(
      *m_actualSinr, *m_expectedSinr, 0.0000001, "Data Frame - Wrong SINR !");
  ulPhy->Dispose();
  Simulator::Destroy();
}
