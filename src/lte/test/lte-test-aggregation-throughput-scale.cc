...
LteAggregationThroughputScaleTestSuite::LteAggregationThroughputScaleTestSuite()
  ...
  AddTestCase(new LteAggregationThroughputScaleTestCase(
                  "Carrier aggregation throughput scale"),
              TestCase::QUICK);
}
...
double LteAggregationThroughputScaleTestCase::GetThroughput(
  ...
  sink->SetAttribute(
      "Local", AddressValue(InetSocketAddress(ueIpIface.GetAddress(0), port)));
...
void LteAggregationThroughputScaleTestCase::DoRun() {
  ...
  double average = std::accumulate(begin(throughputs), end(throughputs), 0.0) /
                   throughputs.size();
  for (double throughput : throughputs) {
    NS_TEST_ASSERT_MSG_EQ_TOL(
        throughput, average, average * 0.01,
