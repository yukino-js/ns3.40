...
void GlobalPathlossDatabase::Print() {
  for (auto txit = m_pathlossMap.begin(); txit != m_pathlossMap.end(); ++txit) {
    for (auto rxit = txit->second.begin(); rxit != txit->second.end(); ++rxit) {
      std::cout << txit->first << " --> " << rxit->first << " : "
                << rxit->second << " dB" << std::endl;
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  cmd.AddValue("simDuration", "duration of the simulation in seconds",
               simDuration);
  cmd.AddValue("channelType", "which SpectrumChannel implementation to be used",
               channelType);
  cmd.Parse(argc, argv);
  ...
  if (g_verbose) {
    ...
    std::cout << "throughput:       " << std::setw(20) << std::fixed
              << throughputBps << " bps" << std::endl;
    std::cout << "phy rate  :       " << std::setw(20) << std::fixed
              << phyRate * 1.0 << " bps" << std::endl;
    double rxPowerW = txPowerW / (std::pow(10.0, lossDb / 10.0));
    double capacity = 20e6 * log2(1.0 + (rxPowerW / 20.0e6) / noisePsdValue);
    std::cout << "shannon capacity: " << std::setw(20) << std::fixed << capacity
              << " bps" << std::endl;
  }
