...
SpectrumInterferenceTestCase::SpectrumInterferenceTestCase(Ptr<SpectrumValue> s,
                                                           ...
                                                           bool rxCorrect,
                                                           std::string name)
    : TestCase(name), m_s(s), m_txBytes(txBytes),
...
void SpectrumInterferenceTestCase::DoRun() {
...
void SpectrumInterferenceTestCase::RetrieveTestResult(
    SpectrumInterference *si) {
...
SpectrumInterferenceTestSuite::SpectrumInterferenceTestSuite()
  ...
  b = 10067205.5632012;
  AddTestCase(new SpectrumInterferenceTestCase(
                  s1, 0, true, "sdBm  = [-46 -48]  tx bytes: 1"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s1, static_cast<uint32_t>(b * 0.5 + 0.5), true,
                  "sdBm  = [-46 -48]  tx bytes: b*0.5"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s1, static_cast<uint32_t>(b * (1 - e) + 0.5), true,
                  "sdBm  = [-46 -48]  tx bytes: b*(1-e)"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s1, static_cast<uint32_t>(b * (1 + e) + 0.5), false,
                  "sdBm  = [-46 -48]  tx bytes: b*(1+e)"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s1, static_cast<uint32_t>(b * 1.5 + 0.5), false,
  ...
  b = 882401.591840728;
  AddTestCase(new SpectrumInterferenceTestCase(
                  s2, 1, true, "sdBm  = [-63 -61]  tx bytes: 1"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s2, static_cast<uint32_t>(b * 0.5 + 0.5), true,
                  "sdBm  = [-63 -61]  tx bytes: b*0.5"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s2, static_cast<uint32_t>(b * (1 - e) + 0.5), true,
                  "sdBm  = [-63 -61]  tx bytes: b*(1-e)"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s2, static_cast<uint32_t>(b * (1 + e) + 0.5), false,
                  "sdBm  = [-63 -61]  tx bytes: b*(1+e)"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s2, static_cast<uint32_t>(b * 1.5 + 0.5), false,
                  "sdBm  = [-63 -61]  tx bytes: b*1.5"),
              TestCase::QUICK);
  AddTestCase(new SpectrumInterferenceTestCase(
                  s2, 0xffffffff, false, "sdBm  = [-63 -61]  tx bytes: 2^32-1"),
              TestCase::QUICK);
}
