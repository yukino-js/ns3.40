...
SpectrumIdealPhyTestCase::SpectrumIdealPhyTestCase(double snrLinear,
    ...
    : TestCase(Name(channelType, snrLinear, phyRate)), m_snrLinear(snrLinear),
      m_phyRate(phyRate), m_rateIsAchievable(rateIsAchievable),
...
void SpectrumIdealPhyTestCase::DoRun() {
  ...
  propLoss->SetLoss(c.Get(0)->GetObject<MobilityModel>(),
                    c.Get(1)->GetObject<MobilityModel>(), lossDb, true);
  ...
  client->SetAttribute("Interval", TimeValue(Seconds(double(pktSize * 8) /
                                                     (1.2 * double(phyRate)))));
...
SpectrumIdealPhyTestSuite::SpectrumIdealPhyTestSuite()
  ...
  for (double snr = 0.01; snr <= 10; snr *= 2) {
    ...
    AddTestCase(new SpectrumIdealPhyTestCase(
                    snr, static_cast<uint64_t>(achievableRate * 0.1), true,
                    "ns3::SingleModelSpectrumChannel"),
                TestCase::QUICK);
    AddTestCase(new SpectrumIdealPhyTestCase(
                    snr, static_cast<uint64_t>(achievableRate * 0.5), true,
                    "ns3::SingleModelSpectrumChannel"),
                TestCase::QUICK);
    AddTestCase(new SpectrumIdealPhyTestCase(
                    snr, static_cast<uint64_t>(achievableRate * 0.95), true,
                    "ns3::SingleModelSpectrumChannel"),
                TestCase::QUICK);
    AddTestCase(new SpectrumIdealPhyTestCase(
                    snr, static_cast<uint64_t>(achievableRate * 1.05), false,
                    "ns3::SingleModelSpectrumChannel"),
                TestCase::QUICK);
    AddTestCase(new SpectrumIdealPhyTestCase(
                    snr, static_cast<uint64_t>(achievableRate * 2), false,
                    "ns3::SingleModelSpectrumChannel"),
                TestCase::QUICK);
    AddTestCase(new SpectrumIdealPhyTestCase(
                    snr, static_cast<uint64_t>(achievableRate * 4), false,
                    "ns3::SingleModelSpectrumChannel"),
                TestCase::QUICK);
  }
