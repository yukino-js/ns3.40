...
SpectrumChannelHelper SpectrumChannelHelper::Default() {
...
void SpectrumChannelHelper::AddPropagationLoss(Ptr<PropagationLossModel> m) {
  m->SetNext(m_propagationLossModel);
  m_propagationLossModel = m;
}

void SpectrumChannelHelper::AddSpectrumPropagationLoss(
    Ptr<SpectrumPropagationLossModel> m) {
  m->SetNext(m_spectrumPropagationLossModel);
  m_spectrumPropagationLossModel = m;
}

Ptr<SpectrumChannel> SpectrumChannelHelper::Create() const {
  Ptr<SpectrumChannel> channel =
      (m_channel.Create())->GetObject<SpectrumChannel>();
...
void SpectrumPhyHelper::SetChannel(Ptr<SpectrumChannel> channel) {
  m_channel = channel;
}

void SpectrumPhyHelper::SetChannel(std::string channelName) {
  Ptr<SpectrumChannel> channel = Names::Find<SpectrumChannel>(channelName);
  m_channel = channel;
}

void SpectrumPhyHelper::SetPhyAttribute(std::string name,
                                        const AttributeValue &v) {
  m_phy.Set(name, v);
}

Ptr<SpectrumPhy> SpectrumPhyHelper::Create(Ptr<Node> node,
