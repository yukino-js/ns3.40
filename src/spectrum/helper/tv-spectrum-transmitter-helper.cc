...
void TvSpectrumTransmitterHelper::SetChannel(Ptr<SpectrumChannel> c) {
  NS_LOG_FUNCTION(this << c);
  m_channel = c;
}

void TvSpectrumTransmitterHelper::SetAttribute(std::string name,
...
TvSpectrumTransmitterHelper::Install(NodeContainer nodeCont) {
  ...
  for (auto i = nodeCont.Begin(); i != nodeCont.End(); ++i) {
    ...
    Ptr<TvSpectrumTransmitter> phy =
        m_factory.Create()->GetObject<TvSpectrumTransmitter>();
    phy->CreateTvPsd();
...
NetDeviceContainer TvSpectrumTransmitterHelper::InstallAdjacent(
    NodeContainer nodeCont, Region region, uint16_t channelNumber) {
  NS_LOG_FUNCTION(this);
  NetDeviceContainer devCont;
  double startFrequency;
  double channelBandwidth;
...
int64_t TvSpectrumTransmitterHelper::AssignStreams(int64_t streamNum) {
  m_uniRand->SetStream(streamNum);
  return 1;
}

void TvSpectrumTransmitterHelper::CreateRegionalTvTransmitters(
    Region region, Density density, double originLatitude,
  ...
  if (region == REGION_NORTH_AMERICA) {
    transmitterIndicesToCreate = GenerateRegionalTransmitterIndices(
        northAmericaStartFrequencies, northAmericaArrayLength, density);
  } else if (region == REGION_EUROPE) {
    transmitterIndicesToCreate = GenerateRegionalTransmitterIndices(
        europeStartFrequencies, europeArrayLength, density);
  } else if (region == REGION_JAPAN) {
    transmitterIndicesToCreate = GenerateRegionalTransmitterIndices(
        japanStartFrequencies, japanArrayLength, density);
  }
  std::list<Vector> tvTransmitterLocations =
      GeographicPositions::RandCartesianPointsAroundGeographicPoint(
          originLatitude, originLongitude, maxAltitude,
          transmitterIndicesToCreate.size(), maxRadius, m_uniRand);
  InstallRandomRegionalTransmitters(region, transmitterIndicesToCreate,
...
std::list<int> TvSpectrumTransmitterHelper::GenerateRegionalTransmitterIndices(
  ...
  for (int i = 0; i < (int)transmitterStartFreqsToCreate.size(); i++) {
    for (int channelNumberIndex = 0;
      ...
      if (startFrequencies[channelNumberIndex] ==
          transmitterStartFreqsToCreate[i]) {
...
int TvSpectrumTransmitterHelper::GetRandomNumTransmitters(
    Density density, uint32_t numChannels) {
  int numTransmitters;
  if (density == DENSITY_LOW) {
    numTransmitters = m_uniRand->GetInteger(1, ceil(0.33 * numChannels));
  } else if (density == DENSITY_MEDIUM) {
    numTransmitters = m_uniRand->GetInteger(ceil(0.33 * numChannels) + 1,
                                            ceil(0.66 * numChannels));
  } else {
    numTransmitters =
...
void TvSpectrumTransmitterHelper::InstallRandomRegionalTransmitters(
