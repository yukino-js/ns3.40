 ...
 */
static void WriteAveragePowerSpectralDensityReport(
  ...
  if (ostream->good()) {
    ...
    while (fi != avgPowerSpectralDensity->ConstBandsEnd()) {
      ...
      *ostream << Now().GetSeconds() << " " << fi->fc << " " << *vi
               << std::endl;
      ++fi;
      ++vi;
    }
...
SpectrumAnalyzerHelper::~SpectrumAnalyzerHelper() { NS_LOG_FUNCTION(this); }

void SpectrumAnalyzerHelper::SetChannel(Ptr<SpectrumChannel> channel) {
  NS_LOG_FUNCTION(this);
  m_channel = channel;
}

void SpectrumAnalyzerHelper::SetChannel(std::string channelName) {
  NS_LOG_FUNCTION(this);
  Ptr<SpectrumChannel> channel = Names::Find<SpectrumChannel>(channelName);
  m_channel = channel;
}

void SpectrumAnalyzerHelper::SetPhyAttribute(std::string name,
                                             const AttributeValue &v) {
  NS_LOG_FUNCTION(this);
  m_phy.Set(name, v);
}

void SpectrumAnalyzerHelper::SetDeviceAttribute(std::string name,
                                                const AttributeValue &v) {
  NS_LOG_FUNCTION(this);
  m_device.Set(name, v);
}

void SpectrumAnalyzerHelper::SetRxSpectrumModel(Ptr<SpectrumModel> m) {
  NS_LOG_FUNCTION(this);
  m_rxSpectrumModel = m;
}

void SpectrumAnalyzerHelper::EnableAsciiAll(std::string prefix) {
  NS_LOG_FUNCTION(this);
  m_prefix = prefix;
}

NetDeviceContainer SpectrumAnalyzerHelper::Install(NodeContainer c) const {
  NS_LOG_FUNCTION(this);
...
NetDeviceContainer SpectrumAnalyzerHelper::Install(Ptr<Node> node) const {
  NS_LOG_FUNCTION(this);
  return Install(NodeContainer(node));
}

NetDeviceContainer SpectrumAnalyzerHelper::Install(std::string nodeName) const {
  NS_LOG_FUNCTION(this);
