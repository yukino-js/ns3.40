...
HalfDuplexIdealPhy::HalfDuplexIdealPhy()
    : m_mobility(nullptr), m_netDevice(nullptr), m_channel(nullptr),
      m_txPsd(nullptr), m_state(IDLE) {
  m_interference.SetErrorModel(CreateObject<ShannonSpectrumErrorModel>());
}

HalfDuplexIdealPhy::~HalfDuplexIdealPhy() {}

void HalfDuplexIdealPhy::DoDispose() {
...
TypeId HalfDuplexIdealPhy::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::HalfDuplexIdealPhy")
          .SetParent<SpectrumPhy>()
          .SetGroupName("Spectrum")
          .AddConstructor<HalfDuplexIdealPhy>()
          .AddAttribute("Rate", "The PHY rate used by this device",
...
Ptr<NetDevice> HalfDuplexIdealPhy::GetDevice() const {
  NS_LOG_FUNCTION(this);
  return m_netDevice;
}

Ptr<MobilityModel> HalfDuplexIdealPhy::GetMobility() const {
  NS_LOG_FUNCTION(this);
  return m_mobility;
}

void HalfDuplexIdealPhy::SetDevice(Ptr<NetDevice> d) {
  NS_LOG_FUNCTION(this << d);
  m_netDevice = d;
}

void HalfDuplexIdealPhy::SetMobility(Ptr<MobilityModel> m) {
  NS_LOG_FUNCTION(this << m);
  m_mobility = m;
}

void HalfDuplexIdealPhy::SetChannel(Ptr<SpectrumChannel> c) {
  NS_LOG_FUNCTION(this << c);
  m_channel = c;
}

Ptr<const SpectrumModel> HalfDuplexIdealPhy::GetRxSpectrumModel() const {
  if (m_txPsd) {
...
void HalfDuplexIdealPhy::SetTxPowerSpectralDensity(Ptr<SpectrumValue> txPsd) {
...
void HalfDuplexIdealPhy::SetNoisePowerSpectralDensity(
...
void HalfDuplexIdealPhy::SetRate(DataRate rate) {
  NS_LOG_FUNCTION(this << rate);
  m_rate = rate;
}

DataRate HalfDuplexIdealPhy::GetRate() const {
  NS_LOG_FUNCTION(this);
  return m_rate;
}

void HalfDuplexIdealPhy::SetGenericPhyTxEndCallback(GenericPhyTxEndCallback c) {
  NS_LOG_FUNCTION(this);
  m_phyMacTxEndCallback = c;
}

void HalfDuplexIdealPhy::SetGenericPhyRxStartCallback(
    GenericPhyRxStartCallback c) {
  NS_LOG_FUNCTION(this);
  m_phyMacRxStartCallback = c;
}

void HalfDuplexIdealPhy::SetGenericPhyRxEndErrorCallback(
    GenericPhyRxEndErrorCallback c) {
  NS_LOG_FUNCTION(this);
  m_phyMacRxEndErrorCallback = c;
}

void HalfDuplexIdealPhy::SetGenericPhyRxEndOkCallback(
    GenericPhyRxEndOkCallback c) {
  NS_LOG_FUNCTION(this);
  m_phyMacRxEndOkCallback = c;
}

Ptr<Object> HalfDuplexIdealPhy::GetAntenna() const {
  NS_LOG_FUNCTION(this);
  return m_antenna;
}

void HalfDuplexIdealPhy::SetAntenna(Ptr<AntennaModel> a) {
  NS_LOG_FUNCTION(this << a);
  m_antenna = a;
}

void HalfDuplexIdealPhy::ChangeState(State newState) {
  NS_LOG_LOGIC(this << " state: " << m_state << " -> " << newState);
  m_state = newState;
}

bool HalfDuplexIdealPhy::StartTx(Ptr<Packet> p) {
  ...
  case IDLE: {
    ...
    NS_LOG_LOGIC(this << " tx power: "
                      << 10 * std::log10(Integral(*(txParams->psd))) + 30
                      << " dBm");
...
void HalfDuplexIdealPhy::StartRx(Ptr<SpectrumSignalParameters> spectrumParams) {
  NS_LOG_FUNCTION(this << spectrumParams);
  NS_LOG_LOGIC(this << " state: " << m_state);
  NS_LOG_LOGIC(this << " rx power: "
  ...
  if (rxParams) {
    ...
    case IDLE:
      ...
      NS_LOG_LOGIC(this << " scheduling EndRx with delay "
                        << rxParams->duration);
      m_endRxEventId = Simulator::Schedule(rxParams->duration,
                                           &HalfDuplexIdealPhy::EndRx, this);
...
void HalfDuplexIdealPhy::AbortRx() {
  NS_LOG_FUNCTION(this);
  NS_LOG_LOGIC(this << "state: " << m_state);
