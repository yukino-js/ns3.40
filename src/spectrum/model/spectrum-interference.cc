...
SpectrumInterference::SpectrumInterference()
    : m_receiving(false), m_rxSignal(nullptr), m_allSignals(nullptr),
      m_noise(nullptr), m_errorModel(nullptr) {
  NS_LOG_FUNCTION(this);
}

SpectrumInterference::~SpectrumInterference() { NS_LOG_FUNCTION(this); }

TypeId SpectrumInterference::GetTypeId() {
...
void SpectrumInterference::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_rxSignal = nullptr;
...
void SpectrumInterference::StartRx(Ptr<const Packet> p,
...
void SpectrumInterference::AbortRx() { m_receiving = false; }

bool SpectrumInterference::EndRx() {
  NS_LOG_FUNCTION(this);
...
void SpectrumInterference::AddSignal(Ptr<const SpectrumValue> spd,
                                     const Time duration) {
  NS_LOG_FUNCTION(this << *spd << duration);
  DoAddSignal(spd);
  Simulator::Schedule(duration, &SpectrumInterference::DoSubtractSignal, this,
                      spd);
}

void SpectrumInterference::DoAddSignal(Ptr<const SpectrumValue> spd) {
...
void SpectrumInterference::DoSubtractSignal(Ptr<const SpectrumValue> spd) {
  NS_LOG_FUNCTION(this << *spd);
  ConditionallyEvaluateChunk();
  (*m_allSignals) -= (*spd);
  m_lastChangeTime = Now();
...
void SpectrumInterference::ConditionallyEvaluateChunk() {
  NS_LOG_FUNCTION(this);
  ...
  if (condition) {
    SpectrumValue sinr =
        (*m_rxSignal) / ((*m_allSignals) - (*m_rxSignal) + (*m_noise));
...
void SpectrumInterference::SetNoisePowerSpectralDensity(
...
void SpectrumInterference::SetErrorModel(Ptr<SpectrumErrorModel> e) {
