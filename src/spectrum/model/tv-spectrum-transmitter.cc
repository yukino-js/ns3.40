...
TvSpectrumTransmitter::TvSpectrumTransmitter()
    : m_mobility(nullptr), m_antenna(CreateObject<IsotropicAntennaModel>()),
      m_netDevice(nullptr), m_channel(nullptr), m_tvType(TVTYPE_8VSB),
      m_startFrequency(500e6), m_channelBandwidth(6e6), m_basePsd(20),
      m_txPsd(nullptr), m_startingTime(Seconds(0)),
      m_transmitDuration(Seconds(0.2)), m_active(false) {
...
TypeId TvSpectrumTransmitter::GetTypeId() {
  static TypeId tid =
...
void TvSpectrumTransmitter::SetChannel(Ptr<SpectrumChannel> c) {
  NS_LOG_FUNCTION(this << c);
  m_channel = c;
}

void TvSpectrumTransmitter::SetMobility(Ptr<MobilityModel> m) {
  NS_LOG_FUNCTION(this << m);
  m_mobility = m;
}

void TvSpectrumTransmitter::SetDevice(Ptr<NetDevice> d) {
  NS_LOG_FUNCTION(this << d);
  m_netDevice = d;
}

Ptr<MobilityModel> TvSpectrumTransmitter::GetMobility() const {
  NS_LOG_FUNCTION(this);
  return m_mobility;
}

Ptr<NetDevice> TvSpectrumTransmitter::GetDevice() const {
  NS_LOG_FUNCTION(this);
  return m_netDevice;
}

Ptr<const SpectrumModel> TvSpectrumTransmitter::GetRxSpectrumModel() const {
  NS_LOG_FUNCTION(this);
  return nullptr;
}

Ptr<Object> TvSpectrumTransmitter::GetAntenna() const {
  NS_LOG_FUNCTION(this);
  return m_antenna;
}

void TvSpectrumTransmitter::StartRx(Ptr<SpectrumSignalParameters> params) {
  NS_LOG_FUNCTION(this << params);
}

Ptr<SpectrumChannel> TvSpectrumTransmitter::GetChannel() const {
  NS_LOG_FUNCTION(this);
  return m_channel;
}
...
Ptr<SpectrumValue> TvSpectrumTransmitter::GetTxPsd() const {
  NS_LOG_FUNCTION(this);
  return m_txPsd;
}

void TvSpectrumTransmitter::SetupTx() {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(m_txPsd);
...
void TvSpectrumTransmitter::Start() {
  NS_LOG_FUNCTION(this);
  if (!m_active) {
...
void TvSpectrumTransmitter::Stop() {
  NS_LOG_FUNCTION(this);
  m_active = false;
}
