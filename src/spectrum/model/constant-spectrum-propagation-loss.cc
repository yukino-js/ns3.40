...
TypeId ConstantSpectrumPropagationLossModel::GetTypeId() {
  static TypeId tid =
...
void ConstantSpectrumPropagationLossModel::SetLossDb(double lossDb) {
  NS_LOG_FUNCTION(this);
  m_lossDb = lossDb;
  m_lossLinear = std::pow(10, m_lossDb / 10);
}

double ConstantSpectrumPropagationLossModel::GetLossDb() const {
  NS_LOG_FUNCTION(this);
  return m_lossDb;
}
