...
WaveformGenerator::WaveformGenerator()
    : m_mobility(nullptr), m_netDevice(nullptr), m_channel(nullptr),
      m_txPowerSpectralDensity(nullptr), m_startTime(Seconds(0)) {}

WaveformGenerator::~WaveformGenerator() {}

void WaveformGenerator::DoDispose() {
...
TypeId WaveformGenerator::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WaveformGenerator")
          .SetParent<SpectrumPhy>()
          .SetGroupName("Spectrum")
          .AddConstructor<WaveformGenerator>()
          .AddAttribute("Period", "the period (=1/frequency)",
...
Ptr<NetDevice> WaveformGenerator::GetDevice() const { return m_netDevice; }
...
void WaveformGenerator::SetDevice(Ptr<NetDevice> d) { m_netDevice = d; }

void WaveformGenerator::SetMobility(Ptr<MobilityModel> m) { m_mobility = m; }

void WaveformGenerator::SetChannel(Ptr<SpectrumChannel> c) {
  NS_LOG_FUNCTION_NOARGS();
  m_channel = c;
}

void WaveformGenerator::StartRx(Ptr<SpectrumSignalParameters> params) {
  NS_LOG_FUNCTION(this << params);
}

void WaveformGenerator::SetTxPowerSpectralDensity(Ptr<SpectrumValue> txPsd) {
  NS_LOG_FUNCTION(this << *txPsd);
  m_txPowerSpectralDensity = txPsd;
}

Ptr<Object> WaveformGenerator::GetAntenna() const { return m_antenna; }

void WaveformGenerator::SetAntenna(Ptr<AntennaModel> a) {
  NS_LOG_FUNCTION(this << a);
  m_antenna = a;
}

void WaveformGenerator::SetPeriod(Time period) { m_period = period; }

Time WaveformGenerator::GetPeriod() const { return m_period; }

void WaveformGenerator::SetDutyCycle(double dutyCycle) {
  m_dutyCycle = dutyCycle;
}

double WaveformGenerator::GetDutyCycle() const { return m_dutyCycle; }
