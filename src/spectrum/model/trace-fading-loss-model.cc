...
TypeId TraceFadingLossModel::GetTypeId() {
  static TypeId tid =
...
void TraceFadingLossModel::SetTraceFileName(std::string fileName) {
  NS_LOG_FUNCTION(this << "Set Fading Trace " << fileName);

  m_traceFile = fileName;
}

void TraceFadingLossModel::SetTraceLength(Time t) { m_traceLength = t; }

void TraceFadingLossModel::DoInitialize() { LoadTrace(); }

void TraceFadingLossModel::LoadTrace() {
...
Ptr<SpectrumValue> TraceFadingLossModel::DoCalcRxPowerSpectralDensity(
  ...
  if (itOff != m_windowOffsetsMap.end()) {
    ...
    startV->SetAttribute("Max", DoubleValue((m_traceLength.GetSeconds() -
    ...
    m_startVariableMap.insert(
        std::pair<ChannelRealizationId_t, Ptr<UniformRandomVariable>>(
            mobilityPair, startV));
    itOff = m_windowOffsetsMap
                .insert(std::pair<ChannelRealizationId_t, int>(
                    mobilityPair, startV->GetValue()))
                .first;
  }
  ...
  int now_ms =
      static_cast<int>(Simulator::Now().GetMilliSeconds() * m_timeGranularity);
  int lastUpdate_ms = static_cast<int>(m_lastWindowUpdate.GetMilliSeconds() *
                                       m_timeGranularity);
...
int64_t TraceFadingLossModel::AssignStreams(int64_t stream) {
