...
SpectrumModel::SpectrumModel(Bands &&bands) : m_bands(std::move(bands)) {
  m_uid = ++m_uidCount;
  NS_LOG_INFO("creating new SpectrumModel, m_uid=" << m_uid);
}

Bands::const_iterator SpectrumModel::Begin() const { return m_bands.begin(); }

Bands::const_iterator SpectrumModel::End() const { return m_bands.end(); }

size_t SpectrumModel::GetNumBands() const { return m_bands.size(); }

SpectrumModelUid_t SpectrumModel::GetUid() const { return m_uid; }

bool SpectrumModel::IsOrthogonal(const SpectrumModel &other) const {
