...
SpectrumAnalyzer::SpectrumAnalyzer()
    : m_mobility(nullptr), m_netDevice(nullptr), m_channel(nullptr),
      m_spectrumModel(nullptr), m_sumPowerSpectralDensity(nullptr),
      m_resolution(MilliSeconds(50)), m_active(false) {
  NS_LOG_FUNCTION(this);
}

SpectrumAnalyzer::~SpectrumAnalyzer() { NS_LOG_FUNCTION(this); }

void SpectrumAnalyzer::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_mobility = nullptr;
  m_netDevice = nullptr;
  m_channel = nullptr;
...
TypeId SpectrumAnalyzer::GetTypeId() {
  static TypeId tid =
...
Ptr<NetDevice> SpectrumAnalyzer::GetDevice() const { return m_netDevice; }

Ptr<MobilityModel> SpectrumAnalyzer::GetMobility() const { return m_mobility; }

Ptr<const SpectrumModel> SpectrumAnalyzer::GetRxSpectrumModel() const {
  return m_spectrumModel;
}

void SpectrumAnalyzer::SetDevice(Ptr<NetDevice> d) {
  NS_LOG_FUNCTION(this << d);
  m_netDevice = d;
}

void SpectrumAnalyzer::SetMobility(Ptr<MobilityModel> m) {
  NS_LOG_FUNCTION(this << m);
  m_mobility = m;
}

void SpectrumAnalyzer::SetChannel(Ptr<SpectrumChannel> c) {
  NS_LOG_FUNCTION(this << c);
  m_channel = c;
}

Ptr<Object> SpectrumAnalyzer::GetAntenna() const { return m_antenna; }

void SpectrumAnalyzer::SetAntenna(Ptr<AntennaModel> a) {
  NS_LOG_FUNCTION(this << a);
  m_antenna = a;
}

void SpectrumAnalyzer::StartRx(Ptr<SpectrumSignalParameters> params) {
  NS_LOG_FUNCTION(this << params);
  AddSignal(params->psd);
  Simulator::Schedule(params->duration, &SpectrumAnalyzer::SubtractSignal, this,
                      params->psd);
}

void SpectrumAnalyzer::AddSignal(Ptr<const SpectrumValue> psd) {
  NS_LOG_FUNCTION(this << *psd);
  UpdateEnergyReceivedSoFar();
  (*m_sumPowerSpectralDensity) += (*psd);
}

void SpectrumAnalyzer::SubtractSignal(Ptr<const SpectrumValue> psd) {
  NS_LOG_FUNCTION(this << *psd);
  UpdateEnergyReceivedSoFar();
  (*m_sumPowerSpectralDensity) -= (*psd);
}

void SpectrumAnalyzer::UpdateEnergyReceivedSoFar() {
  NS_LOG_FUNCTION(this);
...
void SpectrumAnalyzer::GenerateReport() {
  NS_LOG_FUNCTION(this);
  ...
  (*avgPowerSpectralDensity) =
      (*m_energySpectralDensity) / m_resolution.GetSeconds();
...
void SpectrumAnalyzer::SetRxSpectrumModel(Ptr<SpectrumModel> f) {
...
void SpectrumAnalyzer::Stop() { m_active = false; }
