...
void SingleModelSpectrumChannel::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_phyList.clear();
...
TypeId SingleModelSpectrumChannel::GetTypeId() {
...
void SingleModelSpectrumChannel::RemoveRx(Ptr<SpectrumPhy> phy) {
...
void SingleModelSpectrumChannel::AddRx(Ptr<SpectrumPhy> phy) {
  NS_LOG_FUNCTION(this << phy);
  if (std::find(m_phyList.cbegin(), m_phyList.cend(), phy) ==
      m_phyList.cend()) {
    m_phyList.push_back(phy);
  }
}

void SingleModelSpectrumChannel::StartTx(
    Ptr<SpectrumSignalParameters> txParams) {
  NS_LOG_FUNCTION(this << txParams->psd << txParams->duration
                       << txParams->txPhy);
  ...
  for (auto rxPhyIterator = m_phyList.begin(); rxPhyIterator != m_phyList.end();
       ++rxPhyIterator) {
    ...
    if ((*rxPhyIterator) != txParams->txPhy) {
      ...
      if (senderMobility && receiverMobility) {
        ...
        if (m_propagationLoss) {
          propagationGainDb = m_propagationLoss->CalcRxPower(0, senderMobility,
                                                             receiverMobility);
        ...
        *(rxParams->psd) *= pathGainLinear;
        ...
        if (m_propagationDelay) {
          delay =
              m_propagationDelay->GetDelay(senderMobility, receiverMobility);
        }
      }
...
void SingleModelSpectrumChannel::StartRx(Ptr<SpectrumSignalParameters> params,
...
std::size_t SingleModelSpectrumChannel::GetNDevices() const {
  NS_LOG_FUNCTION(this);
  return m_phyList.size();
}

Ptr<NetDevice> SingleModelSpectrumChannel::GetDevice(std::size_t i) const {
