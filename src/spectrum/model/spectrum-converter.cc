...
SpectrumConverter::SpectrumConverter(Ptr<const SpectrumModel> fromSpectrumModel,
  ...
  size_t rowPtr = 0;
  for (auto toit = toSpectrumModel->Begin(); toit != toSpectrumModel->End();
       ++toit) {
    size_t colInd = 0;
    for (auto fromit = fromSpectrumModel->Begin();
         fromit != fromSpectrumModel->End(); ++fromit) {
...
double SpectrumConverter::GetCoefficient(const BandInfo &from,
                                         const BandInfo &to) const {
  NS_LOG_FUNCTION(this);
