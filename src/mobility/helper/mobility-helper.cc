...
void MobilityHelper::SetPositionAllocator(Ptr<PositionAllocator> allocator) {
  m_position = allocator;
}

void MobilityHelper::PushReferenceMobilityModel(Ptr<Object> reference) {
  Ptr<MobilityModel> mobility = reference->GetObject<MobilityModel>();
  m_mobilityStack.push_back(mobility);
}

void MobilityHelper::PushReferenceMobilityModel(std::string referenceName) {
  Ptr<MobilityModel> mobility = Names::Find<MobilityModel>(referenceName);
  m_mobilityStack.push_back(mobility);
}

void MobilityHelper::PopReferenceMobilityModel() { m_mobilityStack.pop_back(); }

std::string MobilityHelper::GetMobilityModelType() const {
  return m_mobility.GetTypeId().GetName();
}

void MobilityHelper::Install(Ptr<Node> node) const {
...
void MobilityHelper::Install(std::string nodeName) const {
  Ptr<Node> node = Names::Find<Node>(nodeName);
  Install(node);
}

void MobilityHelper::Install(NodeContainer c) const {
  for (auto i = c.Begin(); i != c.End(); ++i) {
    Install(*i);
  }
}

void MobilityHelper::InstallAll() const { Install(NodeContainer::GetGlobal()); }
...
void MobilityHelper::CourseChanged(Ptr<OutputStreamWrapper> stream,
  ...
  *os << " pos=" << pos.x << ":" << pos.y << ":" << pos.z << " vel=" << vel.x
      << ":" << vel.y << ":" << vel.z << std::endl;
  os->flags(saved_flags);
  os->precision(saved_precision);
}

void MobilityHelper::EnableAscii(Ptr<OutputStreamWrapper> stream,
                                 uint32_t nodeid) {
...
void MobilityHelper::EnableAsciiAll(Ptr<OutputStreamWrapper> stream) {
  EnableAscii(stream, NodeContainer::GetGlobal());
}

int64_t MobilityHelper::AssignStreams(NodeContainer c, int64_t stream) {
...
double MobilityHelper::GetDistanceSquaredBetween(Ptr<Node> n1, Ptr<Node> n2) {
