...
void GroupMobilityHelper::SetReferencePositionAllocator(
    Ptr<PositionAllocator> allocator) {
  m_referencePosition = allocator;
}

void GroupMobilityHelper::SetMemberPositionAllocator(
    Ptr<PositionAllocator> allocator) {
  m_memberPosition = allocator;
}

void GroupMobilityHelper::SetReferenceMobilityModel(
    Ptr<MobilityModel> mobility) {
  m_referenceMobility = mobility;
}

void GroupMobilityHelper::Install(Ptr<Node> node) {
  NS_ABORT_MSG_IF(node->GetObject<MobilityModel>(),
                  "Mobility model already installed");
  NS_ABORT_MSG_IF(!m_referenceMobility, "Reference mobility model is empty");
  NS_ABORT_MSG_UNLESS(m_memberMobilityFactory.IsTypeIdSet(),
                      "Member mobility factory is unset");
  ...
  Ptr<HierarchicalMobilityModel> hierarchical =
      CreateObject<HierarchicalMobilityModel>();
  hierarchical->SetParent(m_referenceMobility);
  Ptr<MobilityModel> child =
      m_memberMobilityFactory.Create()->GetObject<MobilityModel>();
  NS_ABORT_MSG_IF(!child,
...
void GroupMobilityHelper::Install(std::string nodeName) {
  Ptr<Node> node = Names::Find<Node>(nodeName);
  Install(node);
}

void GroupMobilityHelper::Install(NodeContainer c) {
  for (auto i = c.Begin(); i != c.End(); ++i) {
    Install(*i);
  }
}

int64_t GroupMobilityHelper::AssignStreams(NodeContainer c, int64_t stream) {
