...
void SteadyStateRandomWaypointTest::DoTeardown() { mobilityStack.clear(); }
...
void SteadyStateRandomWaypointTest::DoRun() {
  ...
  Simulator::Schedule(Seconds(0.001),
                      &SteadyStateRandomWaypointTest::DistribCompare, this);
  Simulator::Schedule(Seconds(totalTime),
                      &SteadyStateRandomWaypointTest::DistribCompare, this);
...
void SteadyStateRandomWaypointTest::DistribCompare() {
  double velocity;
  double sum_x = 0;
  double sum_y = 0;
  double sum_v = 0;
  for (auto i = mobilityStack.begin(); i != mobilityStack.end(); ++i) {
    auto model = (*i);
    velocity = std::sqrt(std::pow(model->GetVelocity().x, 2) +
                         std::pow(model->GetVelocity().y, 2));
  ...
  NS_TEST_EXPECT_MSG_EQ_TOL(mean_x, 500, 25.0,
                            "Got unexpected x-position mean value");
  NS_TEST_EXPECT_MSG_EQ_TOL(mean_y, 300, 15.0,
                            "Got unexpected y-position mean value");
  NS_TEST_EXPECT_MSG_EQ_TOL(mean_v, 2.6, 0.13,
  ...
  NS_TEST_EXPECT_MSG_EQ_TOL(dev_x, 230, 10.0,
                            "Got unexpected x-position standard deviation");
  NS_TEST_EXPECT_MSG_EQ_TOL(dev_y, 140, 7.0,
                            "Got unexpected y-position standard deviation");
  NS_TEST_EXPECT_MSG_EQ_TOL(dev_v, 4.4, 0.22,
