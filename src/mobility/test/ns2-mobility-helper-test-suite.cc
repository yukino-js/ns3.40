...
private:
  ...
  bool WriteTrace() {
    ...
    NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL(of.is_open(), true,
  ...
  bool CheckInitialPositions() {
    ...
    while (m_nextRefPoint < m_reference.size() &&
      ...
      NS_TEST_ASSERT_MSG_NE_RETURNS_BOOL(node, nullptr,
                                         "Can't find node with id " << rp.node);
      Ptr<MobilityModel> mob = node->GetObject<MobilityModel>();
      NS_TEST_ASSERT_MSG_NE_RETURNS_BOOL(
          mob, nullptr, "Can't find mobility for node " << rp.node);
  ...
  void CourseChange(std::string context, Ptr<const MobilityModel> mobility) {
    ...
    NS_TEST_EXPECT_MSG_LT(m_nextRefPoint, m_reference.size(),
    ...
    NS_TEST_EXPECT_MSG_EQ(
        id, ref.node, "Node ID mismatch at time " << time.GetSeconds() << " s");

    double tol = 0.001;
    NS_TEST_EXPECT_MSG_EQ(AreVectorsEqual(pos, ref.pos, tol), true,
                          "Position mismatch at time " << time.GetSeconds()
                                                       << " s for node " << id);
    NS_TEST_EXPECT_MSG_EQ(AreVectorsEqual(vel, ref.vel, tol), true,
                          "Velocity mismatch at time " << time.GetSeconds()
                                                       << " s for node " << id);
  }

  void DoSetup() override { CreateNodes(); }
