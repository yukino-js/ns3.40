...
TypeId MobilityModel::GetTypeId() {
  static TypeId tid =
...
Vector MobilityModel::GetPosition() const { return DoGetPosition(); }
...
Vector MobilityModel::GetVelocity() const { return DoGetVelocity(); }

void MobilityModel::SetPosition(const Vector &position) {
  DoSetPosition(position);
}

double MobilityModel::GetDistanceFrom(Ptr<const MobilityModel> other) const {
  Vector oPosition = other->DoGetPosition();
  Vector position = DoGetPosition();
  return CalculateDistance(position, oPosition);
}

double MobilityModel::GetRelativeSpeed(Ptr<const MobilityModel> other) const {
  return (GetVelocity() - other->GetVelocity()).GetLength();
}

void MobilityModel::NotifyCourseChange() const { m_courseChangeTrace(this); }

int64_t MobilityModel::AssignStreams(int64_t start) {
