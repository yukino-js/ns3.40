...
TypeId RandomWaypointMobilityModel::GetTypeId() {
  static TypeId tid =
...
void RandomWaypointMobilityModel::BeginWalk() {
  m_helper.Update();
  Vector m_current = m_helper.GetCurrentPosition();
  NS_ASSERT_MSG(m_position,
  ...
  m_event = Simulator::Schedule(
      travelDelay, &RandomWaypointMobilityModel::DoInitializePrivate, this);
  NotifyCourseChange();
}

void RandomWaypointMobilityModel::DoInitialize() {
  DoInitializePrivate();
  MobilityModel::DoInitialize();
}

void RandomWaypointMobilityModel::DoInitializePrivate() {
  m_helper.Update();
  m_helper.Pause();
  Time pause = Seconds(m_pause->GetValue());
  m_event =
      Simulator::Schedule(pause, &RandomWaypointMobilityModel::BeginWalk, this);
  NotifyCourseChange();
}

Vector RandomWaypointMobilityModel::DoGetPosition() const {
  m_helper.Update();
  return m_helper.GetCurrentPosition();
}

void RandomWaypointMobilityModel::DoSetPosition(const Vector &position) {
...
Vector RandomWaypointMobilityModel::DoGetVelocity() const {
  return m_helper.GetVelocity();
}

int64_t RandomWaypointMobilityModel::DoAssignStreams(int64_t stream) {
