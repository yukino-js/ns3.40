...
TypeId WaypointMobilityModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WaypointMobilityModel")
          ...
          .AddAttribute(
              ...
              BooleanValue(false),
              MakeBooleanAccessor(
                  &WaypointMobilityModel::m_initialPositionIsWaypoint),
              MakeBooleanChecker());
  return tid;
}
...
void WaypointMobilityModel::DoDispose() { MobilityModel::DoDispose(); }

void WaypointMobilityModel::AddWaypoint(const Waypoint &waypoint) {
  if (m_first) {
    m_first = false;
    m_current = m_next = waypoint;
  } else {
    NS_ABORT_MSG_IF(!m_waypoints.empty() &&
                        (m_waypoints.back().time >= waypoint.time),
                    "Waypoints must be added in ascending time order");
    m_waypoints.push_back(waypoint);
  }

  if (!m_lazyNotify) {
    Simulator::Schedule(waypoint.time - Simulator::Now(),
                        &WaypointMobilityModel::Update, this);
  }
}

Waypoint WaypointMobilityModel::GetNextWaypoint() const {
  Update();
  return m_next;
}

uint32_t WaypointMobilityModel::WaypointsLeft() const {
  Update();
  return m_waypoints.size();
}

void WaypointMobilityModel::Update() const {
...
Vector WaypointMobilityModel::DoGetPosition() const {
  Update();
  return m_current.position;
}

void WaypointMobilityModel::DoSetPosition(const Vector &position) {
  const Time now = Simulator::Now();
...
void WaypointMobilityModel::EndMobility() {
  m_waypoints.clear();
...
Vector WaypointMobilityModel::DoGetVelocity() const { return m_velocity; }
