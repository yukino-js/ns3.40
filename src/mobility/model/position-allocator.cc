...
TypeId PositionAllocator::GetTypeId() {
...
TypeId ListPositionAllocator::GetTypeId() {
...
void ListPositionAllocator::Add(Vector v) {
  m_positions.push_back(v);
  m_current = m_positions.begin();
}

void ListPositionAllocator::Add(const std::string filePath,
                                double defaultZ /* = 0 */,
                                char delimiter /* = ',' */) {
...
Vector ListPositionAllocator::GetNext() const {
  Vector v = *m_current;
  m_current++;
...
int64_t ListPositionAllocator::AssignStreams(int64_t stream) { return 0; }

uint32_t ListPositionAllocator::GetSize() const { return m_positions.size(); }

NS_OBJECT_ENSURE_REGISTERED(GridPositionAllocator);

TypeId GridPositionAllocator::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::GridPositionAllocator")
          ...
          .AddAttribute("MinY", "The y coordinate where the grid starts.",
                        ...
                        MakeDoubleChecker<double>())
          .AddAttribute("Z", "The z coordinate of all the positions allocated.",
                        DoubleValue(0.0),
          ...
          .AddAttribute("LayoutType", "The type of layout.",
                        ...
                        MakeEnumChecker(ROW_FIRST, "RowFirst", COLUMN_FIRST,
                                        "ColumnFirst"));
  return tid;
}
...
void GridPositionAllocator::SetMinX(double xMin) { m_xMin = xMin; }

void GridPositionAllocator::SetMinY(double yMin) { m_yMin = yMin; }

void GridPositionAllocator::SetZ(double z) { m_z = z; }

void GridPositionAllocator::SetDeltaX(double deltaX) { m_deltaX = deltaX; }

void GridPositionAllocator::SetDeltaY(double deltaY) { m_deltaY = deltaY; }

void GridPositionAllocator::SetN(uint32_t n) { m_n = n; }

void GridPositionAllocator::SetLayoutType(LayoutType layoutType) {
  m_layoutType = layoutType;
}

double GridPositionAllocator::GetMinX() const { return m_xMin; }

double GridPositionAllocator::GetMinY() const { return m_yMin; }

double GridPositionAllocator::GetDeltaX() const { return m_deltaX; }

double GridPositionAllocator::GetDeltaY() const { return m_deltaY; }

uint32_t GridPositionAllocator::GetN() const { return m_n; }

GridPositionAllocator::LayoutType GridPositionAllocator::GetLayoutType() const {
  return m_layoutType;
}

Vector GridPositionAllocator::GetNext() const {
  double x = 0.0;
  double y = 0.0;
  switch (m_layoutType) {
  case ROW_FIRST:
...
int64_t GridPositionAllocator::AssignStreams(int64_t stream) { return 0; }

NS_OBJECT_ENSURE_REGISTERED(RandomRectanglePositionAllocator);

TypeId RandomRectanglePositionAllocator::GetTypeId() {
  static TypeId tid =
...
void RandomRectanglePositionAllocator::SetX(Ptr<RandomVariableStream> x) {
  m_x = x;
}

void RandomRectanglePositionAllocator::SetY(Ptr<RandomVariableStream> y) {
  m_y = y;
}

void RandomRectanglePositionAllocator::SetZ(double z) { m_z = z; }

Vector RandomRectanglePositionAllocator::GetNext() const {
  double x = m_x->GetValue();
  double y = m_y->GetValue();
  return Vector(x, y, m_z);
}

int64_t RandomRectanglePositionAllocator::AssignStreams(int64_t stream) {
...
void RandomBoxPositionAllocator::SetX(Ptr<RandomVariableStream> x) { m_x = x; }

void RandomBoxPositionAllocator::SetY(Ptr<RandomVariableStream> y) { m_y = y; }

void RandomBoxPositionAllocator::SetZ(Ptr<RandomVariableStream> z) { m_z = z; }

Vector RandomBoxPositionAllocator::GetNext() const {
  double x = m_x->GetValue();
  double y = m_y->GetValue();
...
int64_t RandomBoxPositionAllocator::AssignStreams(int64_t stream) {
  m_x->SetStream(stream);
  m_y->SetStream(stream + 1);
...
TypeId RandomDiscPositionAllocator::GetTypeId() {
  static TypeId tid =
...
void RandomDiscPositionAllocator::SetTheta(Ptr<RandomVariableStream> theta) {
  m_theta = theta;
}

void RandomDiscPositionAllocator::SetRho(Ptr<RandomVariableStream> rho) {
  m_rho = rho;
}

void RandomDiscPositionAllocator::SetX(double x) { m_x = x; }

void RandomDiscPositionAllocator::SetY(double y) { m_y = y; }

void RandomDiscPositionAllocator::SetZ(double z) { m_z = z; }

Vector RandomDiscPositionAllocator::GetNext() const {
...
int64_t RandomDiscPositionAllocator::AssignStreams(int64_t stream) {
...
TypeId UniformDiscPositionAllocator::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::UniformDiscPositionAllocator")
          ...
          .AddAttribute(
              "rho", "The radius of the disc", DoubleValue(0.0),
              MakeDoubleAccessor(&UniformDiscPositionAllocator::m_rho),
              MakeDoubleChecker<double>())
          .AddAttribute("X", "The x coordinate of the center of the  disc.",
                        DoubleValue(0.0),
                        MakeDoubleAccessor(&UniformDiscPositionAllocator::m_x),
                        MakeDoubleChecker<double>())
          .AddAttribute("Y", "The y coordinate of the center of the  disc.",
                        DoubleValue(0.0),
...
void UniformDiscPositionAllocator::SetRho(double rho) { m_rho = rho; }

void UniformDiscPositionAllocator::SetX(double x) { m_x = x; }

void UniformDiscPositionAllocator::SetY(double y) { m_y = y; }

void UniformDiscPositionAllocator::SetZ(double z) { m_z = z; }

Vector UniformDiscPositionAllocator::GetNext() const {
  double x;
  double y;
  do {
...
int64_t UniformDiscPositionAllocator::AssignStreams(int64_t stream) {
