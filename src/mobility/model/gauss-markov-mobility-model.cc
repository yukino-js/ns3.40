...
TypeId GaussMarkovMobilityModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::GaussMarkovMobilityModel")
          ...
          .AddAttribute(
              ...
              MakePointerAccessor(
                  &GaussMarkovMobilityModel::m_rndMeanDirection),
              MakePointerChecker<RandomVariableStream>())
          .AddAttribute(
              "MeanPitch",
...
void GaussMarkovMobilityModel::Start() {
  ...
  m_Velocity =
      m_alpha * m_Velocity + one_minus_alpha * m_meanVelocity + sqrt_alpha * rv;
  m_Direction = m_alpha * m_Direction + one_minus_alpha * m_meanDirection +
                sqrt_alpha * rd;
...
void GaussMarkovMobilityModel::DoWalk(Time delayLeft) {
...
void GaussMarkovMobilityModel::DoSetPosition(const Vector &position) {
  m_helper.SetPosition(position);
  m_event.Cancel();
  m_event = Simulator::ScheduleNow(&GaussMarkovMobilityModel::Start, this);
}

Vector GaussMarkovMobilityModel::DoGetVelocity() const {
  return m_helper.GetVelocity();
}

int64_t GaussMarkovMobilityModel::DoAssignStreams(int64_t stream) {
