...
TypeId RandomDirection2dMobilityModel::GetTypeId() {
  static TypeId tid =
...
void RandomDirection2dMobilityModel::DoDispose() {
  // chain up.
  MobilityModel::DoDispose();
}

void RandomDirection2dMobilityModel::DoInitialize() {
  DoInitializePrivate();
  MobilityModel::DoInitialize();
}

void RandomDirection2dMobilityModel::DoInitializePrivate() {
  double direction = m_direction->GetValue(0, 2 * M_PI);
  SetDirectionAndSpeed(direction);
}

void RandomDirection2dMobilityModel::BeginPause() {
  m_helper.Update();
  m_helper.Pause();
  Time pause = Seconds(m_pause->GetValue());
  m_event.Cancel();
  m_event = Simulator::Schedule(
      pause, &RandomDirection2dMobilityModel::ResetDirectionAndSpeed, this);
  NotifyCourseChange();
}

void RandomDirection2dMobilityModel::SetDirectionAndSpeed(double direction) {
  NS_LOG_FUNCTION_NOARGS();
  m_helper.UpdateWithBounds(m_bounds);
  Vector position = m_helper.GetCurrentPosition();
  double speed = m_speed->GetValue();
  const Vector vector(std::cos(direction) * speed, std::sin(direction) * speed,
                      0.0);
  ...
  m_event.Cancel();
  m_event = Simulator::Schedule(
      delay, &RandomDirection2dMobilityModel::BeginPause, this);
  NotifyCourseChange();
}
...
Vector RandomDirection2dMobilityModel::DoGetPosition() const {
  m_helper.UpdateWithBounds(m_bounds);
  return m_helper.GetCurrentPosition();
}

void RandomDirection2dMobilityModel::DoSetPosition(const Vector &position) {
  m_helper.SetPosition(position);
  m_event.Cancel();
  m_event = Simulator::ScheduleNow(
      &RandomDirection2dMobilityModel::DoInitializePrivate, this);
}

Vector RandomDirection2dMobilityModel::DoGetVelocity() const {
  return m_helper.GetVelocity();
}

int64_t RandomDirection2dMobilityModel::DoAssignStreams(int64_t stream) {
