...
TypeId HierarchicalMobilityModel::GetTypeId() {
  static TypeId tid =
...
HierarchicalMobilityModel::HierarchicalMobilityModel()
    : m_child(nullptr), m_parent(nullptr) {
  NS_LOG_FUNCTION(this);
}

void HierarchicalMobilityModel::SetChild(Ptr<MobilityModel> model) {
...
void HierarchicalMobilityModel::SetParent(Ptr<MobilityModel> model) {
  NS_LOG_FUNCTION(this << model);
  Vector pos;
  if (m_child) {
    pos = GetPosition();
...
Ptr<MobilityModel> HierarchicalMobilityModel::GetChild() const {
  return m_child;
}

Ptr<MobilityModel> HierarchicalMobilityModel::GetParent() const {
  return m_parent;
}

Vector HierarchicalMobilityModel::DoGetPosition() const {
  if (!m_parent) {
...
void HierarchicalMobilityModel::DoSetPosition(const Vector &position) {
...
Vector HierarchicalMobilityModel::DoGetVelocity() const {
  if (m_parent) {
...
void HierarchicalMobilityModel::ParentChanged(Ptr<const MobilityModel> model) {
  MobilityModel::NotifyCourseChange();
}

void HierarchicalMobilityModel::ChildChanged(Ptr<const MobilityModel> model) {
  MobilityModel::NotifyCourseChange();
}

void HierarchicalMobilityModel::DoInitialize() {
  NS_LOG_FUNCTION(this);
...
int64_t HierarchicalMobilityModel::DoAssignStreams(int64_t stream) {
