...
TypeId RandomWalk2dMobilityModel::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::RandomWalk2dMobilityModel")
          ...
          .AddAttribute(
              ...
              MakeEnumChecker(RandomWalk2dMobilityModel::MODE_DISTANCE,
                              "Distance", RandomWalk2dMobilityModel::MODE_TIME,
                              "Time"))
          .AddAttribute(
              "Direction",
...
void RandomWalk2dMobilityModel::DoInitialize() {
  DoInitializePrivate();
  MobilityModel::DoInitialize();
}

void RandomWalk2dMobilityModel::DoInitializePrivate() {
  m_helper.Update();
  ...
  if (!m_bounds.IsOnTheBorder(position)) {
    ...
    Ptr<UniformRandomVariable> directionOverride =
...
void RandomWalk2dMobilityModel::DoWalk(Time delayLeft) {
  ...
  if (m_bounds.IsInside(nextPosition)) {
    m_event = Simulator::Schedule(
        delayLeft, &RandomWalk2dMobilityModel::DoInitializePrivate, this);
  } else {
    ...
    m_event = Simulator::Schedule(delay, &RandomWalk2dMobilityModel::Rebound,
                                  this, delayLeft - delay);
  }
  NotifyCourseChange();
}

void RandomWalk2dMobilityModel::Rebound(Time delayLeft) {
...
void RandomWalk2dMobilityModel::DoDispose() {
  // chain up
  MobilityModel::DoDispose();
}

Vector RandomWalk2dMobilityModel::DoGetPosition() const {
  m_helper.UpdateWithBounds(m_bounds);
  return m_helper.GetCurrentPosition();
}

void RandomWalk2dMobilityModel::DoSetPosition(const Vector &position) {
  NS_ASSERT(m_bounds.IsInside(position));
  m_helper.SetPosition(position);
  m_event.Cancel();
  m_event = Simulator::ScheduleNow(
      &RandomWalk2dMobilityModel::DoInitializePrivate, this);
}

Vector RandomWalk2dMobilityModel::DoGetVelocity() const {
  return m_helper.GetVelocity();
}

int64_t RandomWalk2dMobilityModel::DoAssignStreams(int64_t stream) {
