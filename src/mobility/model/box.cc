...
Box::Box(double _xMin, double _xMax, double _yMin, double _yMax, double _zMin,
         double _zMax)
...
bool Box::IsInside(const Vector &position) const {
...
Box::Side Box::GetClosestSide(const Vector &position) const {
...
Vector Box::CalculateIntersection(const Vector &current,
...
bool Box::IsIntersect(const Vector &l1, const Vector &l2) const {
  ...
  if (std::abs(lMid.x * l.z - lMid.z * l.x) >
      (boxSize.x * lExt.z + boxSize.z * lExt.x)) {
    return false;
  }
  if (std::abs(lMid.x * l.y - lMid.y * l.x) >
      (boxSize.x * lExt.y + boxSize.y * lExt.x)) {
    return false;
  }
...
std::istream &operator>>(std::istream &is, Box &box) {
  ...
  char c4;
  char c5;
  is >> box.xMin >> c1 >> box.xMax >> c2 >> box.yMin >> c3 >> box.yMax >> c4 >>
      box.zMin >> c5 >> box.zMax;
  if (c1 != '|' || c2 != '|' || c3 != '|' || c4 != '|' || c5 != '|') {
