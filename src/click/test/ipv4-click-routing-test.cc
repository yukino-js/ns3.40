...
static void AddClickInternetStack(Ptr<Node> node) {
  ...
  internet.SetClickFile(
      node, "src/click/test/nsclick-test-lan-single-interface.click");
...
void ClickIfidFromNameTest::DoRun() {
  Ptr<Node> node = CreateObject<Node>();
  AddClickInternetStack(node);
  AddNetworkDevice(node, Mac48Address("00:00:00:00:00:01"),
                   Ipv4Address("10.1.1.1"), Ipv4Mask("255.255.255.0"));
  Ptr<Ipv4> ipv4 = node->GetObject<Ipv4>();
  Ptr<Ipv4ClickRouting> click =
      DynamicCast<Ipv4ClickRouting>(ipv4->GetRoutingProtocol());
  click->DoInitialize();
...
void ClickIpMacAddressFromNameTest::DoRun() {
  Ptr<Node> node = CreateObject<Node>();
  AddClickInternetStack(node);
  AddNetworkDevice(node, Mac48Address("00:00:00:00:00:01"),
                   Ipv4Address("10.1.1.1"), Ipv4Mask("255.255.255.0"));
  AddNetworkDevice(node, Mac48Address("00:00:00:00:00:02"),
  ...
  simclick_sim_command(click->m_simNode, SIMCLICK_IPADDR_FROM_NAME, "eth0", buf,
                       255);
  NS_TEST_EXPECT_MSG_EQ(std::string(buf), "10.1.1.1", "eth0 has IP 10.1.1.1");

  simclick_sim_command(click->m_simNode, SIMCLICK_MACADDR_FROM_NAME, "eth0",
                       buf, 255);
  NS_TEST_EXPECT_MSG_EQ(std::string(buf), "00:00:00:00:00:01",
                        "eth0 has Mac Address 00:00:00:00:00:01");

  simclick_sim_command(click->m_simNode, SIMCLICK_IPADDR_FROM_NAME, "eth1", buf,
                       255);
  NS_TEST_EXPECT_MSG_EQ(std::string(buf), "10.1.1.2", "eth1 has IP 10.1.1.2");

  simclick_sim_command(click->m_simNode, SIMCLICK_MACADDR_FROM_NAME, "eth1",
                       buf, 255);
  ...
  simclick_sim_command(click->m_simNode, SIMCLICK_MACADDR_FROM_NAME, "tap0",
                       buf, 255);
  NS_TEST_EXPECT_MSG_EQ(std::string(buf), "00:00:00:00:00:00",
...
void ClickTrivialTest::DoRun() {
  ...
  ret =
      simclick_sim_command(click->m_simNode, SIMCLICK_GET_NODE_NAME, buf, 255);
