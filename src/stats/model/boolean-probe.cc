...
TypeId BooleanProbe::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::BooleanProbe")
          .SetParent<Probe>()
          .SetGroupName("Stats")
...
BooleanProbe::~BooleanProbe() { NS_LOG_FUNCTION(this); }

bool BooleanProbe::GetValue() const {
  NS_LOG_FUNCTION(this);
  return m_output;
}

void BooleanProbe::SetValue(bool newVal) {
  NS_LOG_FUNCTION(this << newVal);
  m_output = newVal;
}

void BooleanProbe::SetValueByPath(std::string path, bool newVal) {
...
bool BooleanProbe::ConnectByObject(std::string traceSource, Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << traceSource << obj);
  NS_LOG_DEBUG(
      "Name of probe (if any) in names database: " << Names::FindPath(obj));
  bool connected = obj->TraceConnectWithoutContext(
      traceSource, MakeCallback(&ns3::BooleanProbe::TraceSink, this));
  return connected;
}

void BooleanProbe::ConnectByPath(std::string path) {
  NS_LOG_FUNCTION(this << path);
  NS_LOG_DEBUG("Name of probe to search for in config database: " << path);
  Config::ConnectWithoutContext(
      path, MakeCallback(&ns3::BooleanProbe::TraceSink, this));
}

void BooleanProbe::TraceSink(bool oldData, bool newData) {
