...
/* static */
TypeId TimeMinMaxAvgTotalCalculator::GetTypeId() {
...
void TimeMinMaxAvgTotalCalculator::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void TimeMinMaxAvgTotalCalculator::Output(DataOutputCallback &callback) const {
  NS_LOG_FUNCTION(this << &callback);

  callback.OutputSingleton(m_context, m_key + "-count", m_count);
  if (m_count > 0) {
    callback.OutputSingleton(m_context, m_key + "-total", m_total);
    callback.OutputSingleton(m_context, m_key + "-average",
                             Time(m_total / m_count));
