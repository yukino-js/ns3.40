...
TypeId Probe::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Probe")
          .SetParent<DataCollectionObject>()
          .SetGroupName("Stats")
          .AddAttribute("Start", "Time data collection starts",
                        TimeValue(Seconds(0)),
                        MakeTimeAccessor(&Probe::m_start), MakeTimeChecker())
          .AddAttribute("Stop",
                        "Time when data collection stops.  The special time "
                        "value of 0 disables this attribute",
                        TimeValue(Seconds(0)), MakeTimeAccessor(&Probe::m_stop),
                        MakeTimeChecker());
  return tid;
}

Probe::Probe() { NS_LOG_FUNCTION(this); }
