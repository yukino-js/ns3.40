...
TypeId TimeProbe::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::TimeProbe")
          .SetParent<Probe>()
          .SetGroupName("Stats")
...
TimeProbe::~TimeProbe() { NS_LOG_FUNCTION(this); }

double TimeProbe::GetValue() const {
  NS_LOG_FUNCTION(this);
  return m_output;
}

void TimeProbe::SetValue(Time newVal) {
  NS_LOG_FUNCTION(this << newVal.As(Time::S));
  m_output = newVal.GetSeconds();
}

void TimeProbe::SetValueByPath(std::string path, Time newVal) {
...
bool TimeProbe::ConnectByObject(std::string traceSource, Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << traceSource << obj);
  NS_LOG_DEBUG("Name of trace source (if any) in names database: "
               << Names::FindPath(obj));
  bool connected = obj->TraceConnectWithoutContext(
      traceSource, MakeCallback(&ns3::TimeProbe::TraceSink, this));
  return connected;
}

void TimeProbe::ConnectByPath(std::string path) {
...
void TimeProbe::TraceSink(Time oldData, Time newData) {
