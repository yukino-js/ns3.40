...
TypeId Uinteger16Probe::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Uinteger16Probe")
          .SetParent<Probe>()
          .SetGroupName("Stats")
...
Uinteger16Probe::~Uinteger16Probe() { NS_LOG_FUNCTION(this); }

uint16_t Uinteger16Probe::GetValue() const {
  NS_LOG_FUNCTION(this);
  return m_output;
}

void Uinteger16Probe::SetValue(uint16_t newVal) {
  NS_LOG_FUNCTION(this << newVal);
  m_output = newVal;
}

void Uinteger16Probe::SetValueByPath(std::string path, uint16_t newVal) {
...
bool Uinteger16Probe::ConnectByObject(std::string traceSource,
                                      Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << traceSource << obj);
  NS_LOG_DEBUG(
      "Name of probe (if any) in names database: " << Names::FindPath(obj));
  bool connected = obj->TraceConnectWithoutContext(
      traceSource, MakeCallback(&ns3::Uinteger16Probe::TraceSink, this));
  return connected;
}

void Uinteger16Probe::ConnectByPath(std::string path) {
  NS_LOG_FUNCTION(this << path);
  NS_LOG_DEBUG("Name of probe to search for in config database: " << path);
  Config::ConnectWithoutContext(
      path, MakeCallback(&ns3::Uinteger16Probe::TraceSink, this));
}

void Uinteger16Probe::TraceSink(uint16_t oldData, uint16_t newData) {
