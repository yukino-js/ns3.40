...
double Histogram::GetBinStart(uint32_t index) const {
  return index * m_binWidth;
}

double Histogram::GetBinEnd(uint32_t index) const {
...
uint32_t Histogram::GetBinCount(uint32_t index) const {
...
void Histogram::Clear() { m_histogram.clear(); }

Histogram::Histogram(double binWidth) { m_binWidth = binWidth; }

Histogram::Histogram() { m_binWidth = DEFAULT_BIN_WIDTH; }
