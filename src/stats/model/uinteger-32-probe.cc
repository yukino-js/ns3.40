...
TypeId Uinteger32Probe::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Uinteger32Probe")
          .SetParent<Probe>()
          .SetGroupName("Stats")
...
Uinteger32Probe::~Uinteger32Probe() { NS_LOG_FUNCTION(this); }

uint32_t Uinteger32Probe::GetValue() const {
  NS_LOG_FUNCTION(this);
  return m_output;
}

void Uinteger32Probe::SetValue(uint32_t newVal) {
  NS_LOG_FUNCTION(this << newVal);
  m_output = newVal;
}

void Uinteger32Probe::SetValueByPath(std::string path, uint32_t newVal) {
...
bool Uinteger32Probe::ConnectByObject(std::string traceSource,
                                      Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << traceSource << obj);
  NS_LOG_DEBUG(
      "Name of probe (if any) in names database: " << Names::FindPath(obj));
  bool connected = obj->TraceConnectWithoutContext(
      traceSource, MakeCallback(&ns3::Uinteger32Probe::TraceSink, this));
  return connected;
}

void Uinteger32Probe::ConnectByPath(std::string path) {
  NS_LOG_FUNCTION(this << path);
  NS_LOG_DEBUG("Name of probe to search for in config database: " << path);
  Config::ConnectWithoutContext(
      path, MakeCallback(&ns3::Uinteger32Probe::TraceSink, this));
}

void Uinteger32Probe::TraceSink(uint32_t oldData, uint32_t newData) {
