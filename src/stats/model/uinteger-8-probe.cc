...
TypeId Uinteger8Probe::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Uinteger8Probe")
          .SetParent<Probe>()
          .SetGroupName("Stats")
...
Uinteger8Probe::~Uinteger8Probe() { NS_LOG_FUNCTION(this); }

uint8_t Uinteger8Probe::GetValue() const {
  NS_LOG_FUNCTION(this);
  return m_output;
}

void Uinteger8Probe::SetValue(uint8_t newVal) {
  NS_LOG_FUNCTION(this << newVal);
  m_output = newVal;
}

void Uinteger8Probe::SetValueByPath(std::string path, uint8_t newVal) {
...
bool Uinteger8Probe::ConnectByObject(std::string traceSource, Ptr<Object> obj) {
  NS_LOG_FUNCTION(this << traceSource << obj);
  NS_LOG_DEBUG(
      "Name of probe (if any) in names database: " << Names::FindPath(obj));
  bool connected = obj->TraceConnectWithoutContext(
      traceSource, MakeCallback(&ns3::Uinteger8Probe::TraceSink, this));
  return connected;
}

void Uinteger8Probe::ConnectByPath(std::string path) {
  NS_LOG_FUNCTION(this << path);
  NS_LOG_DEBUG("Name of probe to search for in config database: " << path);
  Config::ConnectWithoutContext(
      path, MakeCallback(&ns3::Uinteger8Probe::TraceSink, this));
}

void Uinteger8Probe::TraceSink(uint8_t oldData, uint8_t newData) {
