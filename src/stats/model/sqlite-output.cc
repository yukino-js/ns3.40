...
void SQLiteOutput::SetJournalInMemory() {
  NS_LOG_FUNCTION(this);
  SpinExec("PRAGMA journal_mode = MEMORY");
}

bool SQLiteOutput::SpinExec(const std::string &cmd) const {
  return (SpinExec(m_db, cmd) == SQLITE_OK);
}

bool SQLiteOutput::SpinExec(sqlite3_stmt *stmt) const {
  int rc = SpinExec(m_db, stmt);
  return !CheckError(m_db, rc, "", false);
}

bool SQLiteOutput::WaitExec(const std::string &cmd) const {
  int rc = WaitExec(m_db, cmd);
  return !CheckError(m_db, rc, cmd, false);
}

bool SQLiteOutput::WaitExec(sqlite3_stmt *stmt) const {
  return (WaitExec(m_db, stmt) == SQLITE_OK);
}

bool SQLiteOutput::WaitPrepare(sqlite3_stmt **stmt,
                               const std::string &cmd) const {
  return (WaitPrepare(m_db, stmt, cmd) == SQLITE_OK);
}

bool SQLiteOutput::SpinPrepare(sqlite3_stmt **stmt,
                               const std::string &cmd) const {
...
int SQLiteOutput::WaitPrepare(sqlite3 *db, sqlite3_stmt **stmt,
                              const std::string &cmd) const {
  int rc;
  bool ret;
  ...
  rc = sqlite3_prepare_v2(db, cmd.c_str(), static_cast<int>(cmd.size()), stmt,
                          nullptr);
...
int SQLiteOutput::SpinPrepare(sqlite3 *db, sqlite3_stmt **stmt,
...
int SQLiteOutput::SpinStep(sqlite3_stmt *stmt) {
  int rc;
  do {
...
int SQLiteOutput::SpinFinalize(sqlite3_stmt *stmt) {
  int rc;
  do {
...
void SQLiteOutput::Error(sqlite3 *db, const std::string &cmd) {
...
int SQLiteOutput::SpinExec(sqlite3 *db, const std::string &cmd) {
  sqlite3_stmt *stmt;
  bool ret;
...
int SQLiteOutput::SpinExec(sqlite3 *db, sqlite3_stmt *stmt) {
  bool ret;
...
int SQLiteOutput::WaitExec(sqlite3 *db, sqlite3_stmt *stmt) const {
  bool ret;
  int rc = SQLITE_ERROR;
...
int SQLiteOutput::WaitExec(sqlite3 *db, const std::string &cmd) const {
  sqlite3_stmt *stmt;
  bool ret;
  int rc = SQLITE_ERROR;
