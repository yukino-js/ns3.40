...
void DhcpHelper::SetClientAttribute(std::string name,
                                    const AttributeValue &value) {
  m_clientFactory.Set(name, value);
}

void DhcpHelper::SetServerAttribute(std::string name,
                                    const AttributeValue &value) {
...
DhcpHelper::InstallDhcpClientPriv(Ptr<NetDevice> netDevice) const {
  ...
  NS_ASSERT_MSG(node, "DhcpClientHelper: NetDevice is not not associated with "
                      "any node -> fail");
  ...
  Ptr<DhcpClient> app =
      DynamicCast<DhcpClient>(m_clientFactory.Create<DhcpClient>());
...
Ipv4InterfaceContainer DhcpHelper::InstallFixedAddress(Ptr<NetDevice> netDevice,
                                                       Ipv4Address addr,
                                                       Ipv4Mask mask) {
