...
RadvdHelper::RadvdHelper() { m_factory.SetTypeId(Radvd::GetTypeId()); }

void RadvdHelper::AddAnnouncedPrefix(uint32_t interface, Ipv6Address prefix,
  ...
  if (m_radvdInterfaces.find(interface) == m_radvdInterfaces.end()) {
    ...
    RadvdInterface::RadvdPrefixList prefixList =
        m_radvdInterfaces[interface]->GetPrefixes();
    RadvdInterface::RadvdPrefixListCI iter;
    for (iter = prefixList.begin(); iter != prefixList.end(); iter++) {
      if ((*iter)->GetNetwork() == prefix) {
        NS_LOG_LOGIC("Not adding the same prefix twice, skipping "
...
void RadvdHelper::EnableDefaultRouterForInterface(uint32_t interface) {
  ...
  m_radvdInterfaces[interface]->SetDefaultLifeTime(3 * maxRtrAdvInterval /
                                                   1000);
}
...
void RadvdHelper::ClearPrefixes() { m_radvdInterfaces.clear(); }
...
ApplicationContainer RadvdHelper::Install(Ptr<Node> node) {
  ...
  for (auto iter = m_radvdInterfaces.begin(); iter != m_radvdInterfaces.end();
       iter++) {
