...
void RadvdTestCase::CheckAddresses(Ptr<NetDevice> n0Dev, Ptr<NetDevice> r0Dev,
                                   Ptr<NetDevice> r1Dev, Ptr<NetDevice> n1Dev) {
  Ptr<Ipv6L3Protocol> ipv6;

  ipv6 = n0Dev->GetNode()->GetObject<Ipv6L3Protocol>();
  m_addresses.push_back(
      ipv6->GetAddress(ipv6->GetInterfaceForDevice(n0Dev), 1).GetAddress());

  ipv6 = r0Dev->GetNode()->GetObject<Ipv6L3Protocol>();
  m_addresses.push_back(
      ipv6->GetAddress(ipv6->GetInterfaceForDevice(r0Dev), 1).GetAddress());

  ipv6 = r1Dev->GetNode()->GetObject<Ipv6L3Protocol>();
  m_addresses.push_back(
      ipv6->GetAddress(ipv6->GetInterfaceForDevice(r1Dev), 1).GetAddress());

  ipv6 = n1Dev->GetNode()->GetObject<Ipv6L3Protocol>();
  m_addresses.push_back(
      ipv6->GetAddress(ipv6->GetInterfaceForDevice(n1Dev), 1).GetAddress());
}

void RadvdTestCase::CheckRouting(Ptr<NetDevice> n0Dev, Ptr<NetDevice> r0Dev,
                                 Ptr<NetDevice> r1Dev, Ptr<NetDevice> n1Dev) {
  Ptr<Ipv6L3Protocol> ipv6;
...
void RadvdTestCase::DoRun() {
  ...
  Simulator::Schedule(Seconds(2), &RadvdTestCase::CheckAddresses, this,
                      d1.Get(0), d1.Get(1), d2.Get(0), d2.Get(1));
  Simulator::Schedule(Seconds(3), &RadvdTestCase::CheckRouting, this, d1.Get(0),
                      d1.Get(1), d2.Get(0), d2.Get(1));
  ...
  NS_TEST_ASSERT_MSG_EQ(m_routingResults[1], Socket::ERROR_NOROUTETOHOST,
                        (int)m_routingResults[1]
                            << " instead of Socket::ERROR_NOROUTETOHOST");

  NS_TEST_ASSERT_MSG_EQ(m_routingResults[2], Socket::ERROR_NOTERROR,
                        (int)m_routingResults[2]
                            << " instead of Socket::ERROR_NOTERROR");
...
RadvdTestSuite::RadvdTestSuite() : TestSuite("radvd", UNIT) {
