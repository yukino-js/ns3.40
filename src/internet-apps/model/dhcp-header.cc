...
uint8_t DhcpHeader::GetType() const { return m_op; }
...
void DhcpHeader::SetTran(uint32_t tran) { m_xid = tran; }

uint32_t DhcpHeader::GetTran() const { return m_xid; }

void DhcpHeader::SetTime() { m_secs = (uint16_t)Simulator::Now().GetSeconds(); }
...
void DhcpHeader::SetYiaddr(Ipv4Address addr) { m_yiAddr = addr; }

Ipv4Address DhcpHeader::GetYiaddr() const { return m_yiAddr; }
...
Ipv4Address DhcpHeader::GetDhcps() const { return m_dhcps; }
...
Ipv4Address DhcpHeader::GetReq() const { return m_req; }
...
uint32_t DhcpHeader::GetMask() const { return m_mask; }
...
Ipv4Address DhcpHeader::GetRouter() const { return m_route; }
...
uint32_t DhcpHeader::GetLease() const { return m_lease; }
...
uint32_t DhcpHeader::GetRenew() const { return m_renew; }
...
uint32_t DhcpHeader::GetRebind() const { return m_rebind; }
...
uint32_t DhcpHeader::GetSerializedSize() const { return m_len; }
...
TypeId DhcpHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
uint32_t DhcpHeader::Deserialize(Buffer::Iterator start) {
  ...
  if (m_magic_cookie[0] != 99 || m_magic_cookie[1] != 130 ||
      m_magic_cookie[2] != 83 || m_magic_cookie[3] != 99) {
    NS_LOG_WARN("Malformed Packet");
    return 0;
  }
  len = 240;
