...
DhcpClient::~DhcpClient() { NS_LOG_FUNCTION(this); }

Ptr<NetDevice> DhcpClient::GetDhcpClientNetDevice() { return m_device; }

void DhcpClient::SetDhcpClientNetDevice(Ptr<NetDevice> netDevice) {
  m_device = netDevice;
}

Ipv4Address DhcpClient::GetDhcpServer() { return m_server; }

void DhcpClient::DoDispose() {
  NS_LOG_FUNCTION(this);
...
int64_t DhcpClient::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_ran->SetStream(stream);
  return 1;
}

void DhcpClient::StartApplication() {
  NS_LOG_FUNCTION(this);
  ...
  if (!found) {
    ipv4->AddAddress(
        ifIndex, Ipv4InterfaceAddress(Ipv4Address("0.0.0.0"), Ipv4Mask("/0")));
  }
  if (!m_socket) {
  ...
  if (m_firstBoot) {
    m_device->AddLinkChangeCallback(
...
void DhcpClient::Request() {
  ...
  if (m_state != REFRESH_LEASE) {
    ...
    m_state = WAIT_ACK;
    m_nextOfferEvent =
        Simulator::Schedule(m_nextoffer, &DhcpClient::Select, this);
  } else {
    ...
    packet->AddHeader(header);
    if ((m_socket->SendTo(
            packet, 0, InetSocketAddress(m_remoteAddress, DHCP_PEER_PORT))) >=
        0) {
...
void DhcpClient::AcceptAck(DhcpHeader header, Address from) {
  ...
  if (m_myAddress != m_offeredAddress) {
    for (uint32_t i = 0; i < ipv4->GetNAddresses(ifIndex); i++) {
      if (ipv4->GetAddress(ifIndex, i).GetLocal() == m_myAddress) {
        NS_LOG_LOGIC("Got a new address ("
                     << m_offeredAddress
  ...
  InetSocketAddress remote = InetSocketAddress(
      InetSocketAddress::ConvertFrom(from).GetIpv4(), DHCP_PEER_PORT);
