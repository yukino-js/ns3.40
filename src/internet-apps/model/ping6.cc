...
TypeId Ping6::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ping6")
          ...
          .AddAttribute("LocalIpv6", "Local Ipv6Address of the sender",
                        Ipv6AddressValue(),
...
void Ping6::StartApplication() {
  ...
  if (!m_socket) {
    ...
    if (!m_localAddress.IsAny()) {
      m_ipInterfaceIndex =
          m_ipv6Protocol->GetInterfaceForAddress(m_localAddress);
    }
  }
...
void Ping6::SetIfIndex(uint32_t ifIndex) { m_ifIndex = ifIndex; }
...
void Ping6::Send() {
  ...
  if (!m_routers.empty()) {
    ...
    m_socket->SetAttribute("Protocol",
                           UintegerValue(Ipv6Header::IPV6_EXT_ROUTING));
  }
    ...
    if (Inet6SocketAddress::IsMatchingType(from)) {
      ...
      case Icmpv6Header::ICMPV6_ECHO_REPLY:
        ...
        NS_LOG_INFO("Received Echo Reply size  = "
                    << std::dec << packet->GetSize() << " bytes from "
                    << address.GetIpv6() << " id =  " << (uint16_t)reply.GetId()
        ...
        if (m_ifIndex) {
          ...
          m_ipv6Protocol->ReachabilityHint(m_ipInterfaceIndex,
                                           address.GetIpv6());
        }
      ...
      case Icmpv6Header::ICMPV6_ERROR_DESTINATION_UNREACHABLE:
        ...
        NS_LOG_INFO("Received Destination Unreachable from "
                    << address.GetIpv6());
        break;
