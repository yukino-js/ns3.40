...
Radvd::Radvd() { NS_LOG_FUNCTION(this); }
...
void Radvd::StartApplication() {
  ...
  if (!m_recvSocket) {
    ...
    m_recvSocket->Bind(
        Inet6SocketAddress(Ipv6Address::GetAllRoutersMulticast(), 0));
    m_recvSocket->SetAttribute("Protocol",
  ...
  for (auto it = m_configurations.begin(); it != m_configurations.end(); it++) {
    if ((*it)->IsSendAdvert()) {
      m_unsolicitedEventIds[(*it)->GetInterface()] =
          Simulator::Schedule(Seconds(0.), &Radvd::Send, this, (*it),
                              Ipv6Address::GetAllNodesMulticast(), true);
    }
...
void Radvd::StopApplication() {
  ...
  for (auto it = m_unsolicitedEventIds.begin();
       it != m_unsolicitedEventIds.end(); ++it) {
    Simulator::Cancel((*it).second);
  }
  m_unsolicitedEventIds.clear();

  for (auto it = m_solicitedEventIds.begin(); it != m_solicitedEventIds.end();
       ++it) {
    Simulator::Cancel((*it).second);
  }
...
int64_t Radvd::AssignStreams(int64_t stream) {
...
void Radvd::Send(Ptr<RadvdInterface> config, Ipv6Address dst, bool reschedule) {
  ...
  if (reschedule) {
    auto delay = static_cast<uint64_t>(
        m_jitter->GetValue(config->GetMinRtrAdvInterval(),
                           config->GetMaxRtrAdvInterval()) +
        0.5);
    ...
    m_unsolicitedEventIds[config->GetInterface()] =
        Simulator::Schedule(t, &Radvd::Send, this, config,
                            Ipv6Address::GetAllNodesMulticast(), true);
  }
}
        ...
        for (auto it = m_configurations.begin(); it != m_configurations.end();
          ...
          if (ipInterfaceIndex == (*it)->GetInterface()) {
            ...
            if (scheduleSingle) {
              ...
              m_solicitedEventIds[(*it)->GetInterface()] = Simulator::Schedule(
                  MilliSeconds(delay), &Radvd::Send, this, (*it),
