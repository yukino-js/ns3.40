...
V4TraceRoute::V4TraceRoute()
    : m_interval(Seconds(0)), m_size(56), m_socket(nullptr), m_seq(0),
      m_verbose(true), m_probeCount(0), m_maxProbes(3), m_ttl(1), m_maxTtl(30),
...
void V4TraceRoute::Print(Ptr<OutputStreamWrapper> stream) {
  m_printStream = stream;
}

void V4TraceRoute::StartApplication() {
  NS_LOG_FUNCTION(this);
  NS_LOG_LOGIC("Application started");
  m_started = Simulator::Now();

  if (m_verbose) {
    NS_LOG_UNCOND("Traceroute to " << m_remote << ", " << m_maxTtl
                                   << " hops Max, " << m_size
                                   << " bytes of data.");
  }
  ...
  if (m_printStream) {
    *m_printStream->GetStream()
        << "Traceroute to " << m_remote << ", " << m_maxTtl << " hops Max, "
  ...
  m_socket->SetAttribute("Protocol",
                         UintegerValue(Icmpv4L4Protocol::PROT_NUMBER));
...
void V4TraceRoute::StopApplication() {
  NS_LOG_FUNCTION(this);
...
uint32_t V4TraceRoute::GetApplicationId() const {
  NS_LOG_FUNCTION(this);
...
void V4TraceRoute::Receive(Ptr<Socket> socket) {
  ...
  while (m_socket->GetRxAvailable() > 0) {
    ...
    if (icmp.GetType() == Icmpv4Header::ICMPV4_TIME_EXCEEDED) {
      ...
      if (i != m_sent.end()) {
        ...
        if (m_ttl < m_maxTtl + 1) {
          m_next = Simulator::Schedule(
              m_interval, &V4TraceRoute::StartWaitReplyTimer, this);
        }
      }
      ...
      if (m_probeCount == m_maxProbes) {
        ...
        m_next = Simulator::Schedule(m_interval,
                                     &V4TraceRoute::StartWaitReplyTimer, this);
      }
    }
  }
}

void V4TraceRoute::Send() {
...
void V4TraceRoute::StartWaitReplyTimer() {
  NS_LOG_FUNCTION(this);
  if (!m_waitIcmpReplyTimer.IsRunning()) {
    NS_LOG_LOGIC("Starting WaitIcmpReplyTimer at "
                 << Simulator::Now() << " for " << m_waitIcmpReplyTimeout);

    m_waitIcmpReplyTimer = Simulator::Schedule(
        m_waitIcmpReplyTimeout, &V4TraceRoute::HandleWaitReplyTimeout, this);
    Send();
  }
}

void V4TraceRoute::HandleWaitReplyTimeout() {
  if (m_ttl < m_maxTtl + 1) {
    m_next = Simulator::Schedule(m_interval, &V4TraceRoute::StartWaitReplyTimer,
                                 this);
  }
