  ...
  static TypeId tid =
      TypeId("ns3::Ping")
          ...
          .AddAttribute(
              ...
              AddressValue(), MakeAddressAccessor(&Ping::m_destination),
              MakeAddressChecker())
          .AddAttribute("VerboseMode",
                        ...
                        MakeEnumChecker(VerboseMode::VERBOSE, "Verbose",
                                        VerboseMode::QUIET, "Quiet",
                                        VerboseMode::SILENT, "Silent"))
          .AddAttribute("Interval", "Time interval between sending each packet",
                        TimeValue(Seconds(1)),
                        MakeTimeAccessor(&Ping::m_interval), MakeTimeChecker())
          .AddAttribute("Size",
                        "The number of data bytes to be sent, before ICMP and "
                        "IP headers are added",
                        UintegerValue(56), MakeUintegerAccessor(&Ping::m_size),
                        MakeUintegerChecker<uint32_t>(16))
          .AddAttribute("Count",
                        "The maximum number of packets the application will "
                        "send (zero means no limits)",
                        UintegerValue(0), MakeUintegerAccessor(&Ping::m_count),
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute("InterfaceAddress", "Local address of the sender",
                        AddressValue(),
          ...
          .AddAttribute(
              ...
              TimeValue(Seconds(1)), MakeTimeAccessor(&Ping::m_timeout),
              MakeTimeChecker())
          .AddTraceSource(
              "Tx", "The sequence number and ICMP echo response packet.",
              MakeTraceSourceAccessor(&Ping::m_txTrace), "ns3::Ping::TxTrace")
          .AddTraceSource("Rtt", "The sequence number and RTT sample.",
          ...
          .AddTraceSource(
              ...
              "ns3::Ping::DropTrace")
          .AddTraceSource("Report", "Summary report at close of application.",
                          MakeTraceSourceAccessor(&Ping::m_reportTrace),
                          "ns3::Ping::ReportTrace");
  return tid;
}

Ping::Ping() { NS_LOG_FUNCTION(this); }
...
uint64_t Ping::GetApplicationSignature() const {
  NS_LOG_FUNCTION(this);
    ...
    if (InetSocketAddress::IsMatchingType(from)) {
      ...
      case Icmpv4Header::ICMPV4_ECHO_REPLY: {
        ...
        NS_LOG_INFO("Received Echo Reply size  = "
                    << std::dec << recvSize << " bytes from "
                    << realFrom.GetIpv4() << " id =  " << echo.GetIdentifier()
      ...
      case Icmpv6Header::ICMPV6_ECHO_REPLY: {
        ...
        NS_LOG_INFO("Received Echo Reply size  = "
                    << std::dec << recvSize << " bytes from "
                    << realFrom.GetIpv6() << " id =  " << echo.GetId()
                    << " seq = " << echo.GetSeq() << " Hop Count = "
                    << static_cast<uint16_t>(ipv6Hdr.GetHopLimit()));
...
void Ping::Write64(uint8_t *buffer, const uint64_t data) {
...
uint64_t Ping::Read64(const uint8_t *buffer) {
...
void Ping::Send() {
  ...
  if (!m_useIpv6) {
    ...
    p->AddHeader(header);
    returnValue = m_socket->SendTo(
        p, 0, InetSocketAddress(Ipv4Address::ConvertFrom(m_destination), 0));
  } else {
    // Using IPv6
    ...
    returnValue = m_socket->SendTo(
        p, 0, Inet6SocketAddress(Ipv6Address::ConvertFrom(m_destination), 0));
  ...
  if (m_count > 0 && m_seq == m_count) {
    Time lingerTime =
        m_avgRtt.Count() > 0 ? MilliSeconds(2 * m_avgRtt.Max()) : m_timeout;
...
void Ping::StartApplication() {
  ...
  if (m_verbose == VerboseMode::VERBOSE || m_verbose == VerboseMode::QUIET) {
    if (Ipv4Address::IsMatchingType(m_destination)) {
      ...
      std::cout << "PING " << realFrom.GetIpv4() << " - " << m_size
      ...
      std::cout << "PING " << realFrom.GetIpv6() << " - " << m_size
                << " bytes of data; " << m_size + 48
  ...
  if (Ipv4Address::IsMatchingType(m_destination)) {
    ...
    m_socket = Socket::CreateSocket(
        GetNode(), TypeId::LookupByName("ns3::Ipv6RawSocketFactory"));
    NS_ASSERT_MSG(m_socket, "Ping::StartApplication: can not create socket.");
    m_socket->SetAttribute("Protocol", UintegerValue(Ipv6Header::IPV6_ICMPV6));
    m_socket->SetRecvCallback(MakeCallback(&Ping::Receive, this));
  ...
  if (!m_interfaceAddress.IsInvalid()) {
    if (Ipv4Address::IsMatchingType(m_interfaceAddress)) {
      InetSocketAddress senderInet(
          Ipv4Address::ConvertFrom(m_interfaceAddress));
...
void Ping::SetRouters(const std::vector<Ipv6Address> &routers) {
  m_routers = routers;
}
