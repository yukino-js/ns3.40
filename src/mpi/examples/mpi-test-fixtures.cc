...
void SinkTracer::SinkTrace(const ns3::Ptr<const ns3::Packet> packet,
                           const ns3::Address &srcAddress,
                           const ns3::Address &destAddress) {
  m_sinkCount++;
}

void SinkTracer::Verify(unsigned long expectedCount) {
  unsigned long globalCount;

#ifdef NS3_MPI
  MPI_Reduce(&m_sinkCount, &globalCount, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0,
             MPI_COMM_WORLD);
#else
  ...
  if (expectedCount == globalCount) {
    ...
    RANK0COUT("FAILED  Observed sink traces ("
              << globalCount << ") not equal to expected (" << expectedCount
              << ")\n");
  }
}

std::string SinkTracer::FormatAddress(const ns3::Address address) {
  std::stringstream ss;
