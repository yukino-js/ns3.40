...
RemoteChannelBundle::RemoteChannelBundle()
    : m_remoteSystemId(UINT32_MAX), m_guaranteeTime(0), m_delay(Time::Max()) {}

RemoteChannelBundle::RemoteChannelBundle(const uint32_t remoteSystemId)
    : m_remoteSystemId(remoteSystemId), m_guaranteeTime(0),
      m_delay(Time::Max()) {}

void RemoteChannelBundle::AddChannel(Ptr<Channel> channel, Time delay) {
  m_channels[channel->GetId()] = channel;
  m_delay = ns3::Min(m_delay, delay);
}

uint32_t RemoteChannelBundle::GetSystemId() const { return m_remoteSystemId; }

Time RemoteChannelBundle::GetGuaranteeTime() const { return m_guaranteeTime; }

void RemoteChannelBundle::SetGuaranteeTime(Time time) {
  NS_ASSERT(time >= Simulator::Now());

  m_guaranteeTime = time;
}

Time RemoteChannelBundle::GetDelay() const { return m_delay; }

void RemoteChannelBundle::SetEventId(EventId id) { m_nullEventId = id; }

EventId RemoteChannelBundle::GetEventId() const { return m_nullEventId; }

std::size_t RemoteChannelBundle::GetSize() const { return m_channels.size(); }

void RemoteChannelBundle::Send(Time time) {
