...
ParallelCommunicationInterface *MpiInterface::g_parallelCommunicationInterface =
    nullptr;

void MpiInterface::Destroy() {
  NS_ASSERT(g_parallelCommunicationInterface);
  g_parallelCommunicationInterface->Destroy();
}

uint32_t MpiInterface::GetSystemId() {
...
void MpiInterface::SetParallelSimulatorImpl() {
  StringValue simulationTypeValue;
  bool useDefault = true;

  if (GlobalValue::GetValueByNameFailSafe("SimulatorImplementationType",
                                          simulationTypeValue)) {
