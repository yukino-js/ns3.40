...
TypeId NullMessageSimulatorImpl::GetTypeId() {
  static TypeId tid =
...
NullMessageSimulatorImpl::~NullMessageSimulatorImpl() { NS_LOG_FUNCTION(this); }

void NullMessageSimulatorImpl::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void NullMessageSimulatorImpl::Destroy() {
  NS_LOG_FUNCTION(this);
...
void NullMessageSimulatorImpl::SetScheduler(ObjectFactory schedulerFactory) {
...
void NullMessageSimulatorImpl::ProcessOneEvent() {
  NS_LOG_FUNCTION(this);
...
bool NullMessageSimulatorImpl::IsFinished() const {
  return m_events->IsEmpty() || m_stop;
}

Time NullMessageSimulatorImpl::Next() const {
  NS_LOG_FUNCTION(this);
...
void NullMessageSimulatorImpl::ScheduleNullMessageEvent(
  ...
  bundle->SetEventId(Simulator::Schedule(
      delay, &NullMessageSimulatorImpl::NullMessageEventHandler, this,
      PeekPointer(bundle)));
}

void NullMessageSimulatorImpl::RescheduleNullMessageEvent(
    Ptr<RemoteChannelBundle> bundle) {
  NS_LOG_FUNCTION(this << bundle);
...
void NullMessageSimulatorImpl::RescheduleNullMessageEvent(uint32_t nodeSysId) {
...
void NullMessageSimulatorImpl::HandleArrivingMessagesNonBlocking() {
  NS_LOG_FUNCTION(this);
...
void NullMessageSimulatorImpl::HandleArrivingMessagesBlocking() {
  NS_LOG_FUNCTION(this);
...
Time NullMessageSimulatorImpl::GetSafeTime() { return m_safeTime; }

uint32_t NullMessageSimulatorImpl::GetSystemId() const { return m_myId; }

void NullMessageSimulatorImpl::Stop() {
  NS_LOG_FUNCTION(this);

  m_stop = true;
}

void NullMessageSimulatorImpl::Stop(const Time &delay) {
...
void NullMessageSimulatorImpl::ScheduleWithContext(uint32_t context,
                                                   const Time &delay,
                                                   EventImpl *event) {
...
EventId NullMessageSimulatorImpl::ScheduleNow(EventImpl *event) {
  NS_LOG_FUNCTION(this << event);
  return Schedule(Time(0), event);
}

EventId NullMessageSimulatorImpl::ScheduleDestroy(EventImpl *event) {
  NS_LOG_FUNCTION(this << event);
...
Time NullMessageSimulatorImpl::Now() const { return TimeStep(m_currentTs); }

Time NullMessageSimulatorImpl::GetDelayLeft(const EventId &id) const {
  if (IsExpired(id)) {
    return TimeStep(0);
  } else {
...
void NullMessageSimulatorImpl::Cancel(const EventId &id) {
  if (!IsExpired(id)) {
    id.PeekEventImpl()->Cancel();
  }
}

bool NullMessageSimulatorImpl::IsExpired(const EventId &id) const {
  if (id.GetUid() == EventId::UID::DESTROY) {
...
uint32_t NullMessageSimulatorImpl::GetContext() const {
  return m_currentContext;
}

uint64_t NullMessageSimulatorImpl::GetEventCount() const {
  return m_eventCount;
}
...
void NullMessageSimulatorImpl::NullMessageEventHandler(
    RemoteChannelBundle *bundle) {
  NS_LOG_FUNCTION(this << bundle);
...
NullMessageSimulatorImpl *NullMessageSimulatorImpl::GetInstance() {
  NS_ASSERT(g_instance != nullptr);
  return g_instance;
}
} // namespace ns3
