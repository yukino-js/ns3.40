...
Ptr<RemoteChannelBundle> RemoteChannelBundleManager::Find(uint32_t systemId) {
...
Ptr<RemoteChannelBundle> RemoteChannelBundleManager::Add(uint32_t systemId) {
  NS_ASSERT(!g_initialized);
  NS_ASSERT(g_remoteChannelBundles.find(systemId) ==
            g_remoteChannelBundles.end());

  Ptr<RemoteChannelBundle> remoteChannelBundle =
      Create<RemoteChannelBundle>(systemId);
...
std::size_t RemoteChannelBundleManager::Size() {
  return g_remoteChannelBundles.size();
}

void RemoteChannelBundleManager::InitializeNullMessageEvents() {
  NS_ASSERT(!g_initialized);

  for (auto iter = g_remoteChannelBundles.begin();
       iter != g_remoteChannelBundles.end(); ++iter) {
...
Time RemoteChannelBundleManager::GetSafeTime() {
  NS_ASSERT(g_initialized);

  Time safeTime = Simulator::GetMaximumSimulationTime();

  for (auto kv = g_remoteChannelBundles.begin();
       kv != g_remoteChannelBundles.end(); ++kv) {
...
void RemoteChannelBundleManager::Destroy() {
  NS_ASSERT(g_initialized);
