...
template <int N> bool BenchHeader<N>::IsOk() const { return m_ok; }

template <int N> std::string BenchHeader<N>::GetTypeName() {
  std::ostringstream oss;
  oss << "ns3::BenchHeader<" << N << ">";
  return oss.str();
}

template <int N> TypeId BenchHeader<N>::GetTypeId() {
...
template <int N> TypeId BenchHeader<N>::GetInstanceTypeId() const {
  return GetTypeId();
}

template <int N> void BenchHeader<N>::Print(std::ostream &os) const {
  NS_ASSERT(false);
}

template <int N> uint32_t BenchHeader<N>::GetSerializedSize() const {
  return N;
}

template <int N> void BenchHeader<N>::Serialize(Buffer::Iterator start) const {
  start.WriteU8(N, N);
}

template <int N> uint32_t BenchHeader<N>::Deserialize(Buffer::Iterator start) {
  m_ok = true;
  for (int i = 0; i < N; i++) {
    if (start.ReadU8() != N) {
      m_ok = false;
...
public:
  ...
  void Print(std::ostream &os) const override { os << "N=" << N; }
...
static void runBench(void (*bench)(uint32_t), uint32_t n,
                     uint32_t minIterations, const char *name) {
