...
def should_analyze_directory(dirpath: str) -> bool:
    ...
    return not (
        ...
        or (directory.startswith(".") and directory != ".")
    )
...
def check_style_clang_format(
    ...
    (
        ...
        files_to_check_whitespace,
        files_to_check_tabs,
    ) = find_files_to_check_style(path)
...
def check_trailing_whitespace_file(
    filename: str,
    fix: bool,
    verbose: bool,
) -> Tuple[str, bool, List[str]]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path", action="store", type=str, help="Path to the files to check"
    )
    ...
    parser.add_argument(
        "--no-tabs", action="store_true", help="Do not check / fix tabs"
    )
