...
static void CwndTracer(std::string context, uint32_t oldval, uint32_t newval) {
  ...
  *cWndStream[nodeId]->GetStream()
  ...
  if (!firstSshThr[nodeId]) {
    *ssThreshStream[nodeId]->GetStream() << Simulator::Now().GetSeconds() << " "
                                         << ssThreshValue[nodeId] << std::endl;
  }
}
...
static void SsThreshTracer(std::string context, uint32_t oldval,
  ...
  *ssThreshStream[nodeId]->GetStream()
  ...
  if (!firstCwnd[nodeId]) {
    *cWndStream[nodeId]->GetStream() << Simulator::Now().GetSeconds() << " "
                                     << cWndValue[nodeId] << std::endl;
  }
}
...
static void RttTracer(std::string context, Time oldval, Time newval) {
  ...
  if (firstRtt[nodeId]) {
    *rttStream[nodeId]->GetStream()
        << "0.0 " << oldval.GetSeconds() << std::endl;
    firstRtt[nodeId] = false;
  }
  *rttStream[nodeId]->GetStream() << Simulator::Now().GetSeconds() << " "
...
static void RtoTracer(std::string context, Time oldval, Time newval) {
  ...
  *rtoStream[nodeId]->GetStream() << Simulator::Now().GetSeconds() << " "
                                  << newval.GetSeconds() << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  for (uint32_t i = 0; i < sources.GetN(); i++) {
    AddressValue remoteAddress(
        InetSocketAddress(sink_interfaces.GetAddress(i, 0), port));
  ...
  if (tracing) {
    ...
    ascii_wrap =
        new OutputStreamWrapper(prefix_file_name + "-ascii", std::ios::out);
