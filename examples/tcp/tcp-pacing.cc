...
int main(int argc, char *argv[]) {
  ...
  DataRate bottleneckBandwidth(
      "10Mbps"); // value of x as shown in the above network topology
  ...
  cmd.AddValue("tracing", "Flag to enable/disable Ascii and Pcap tracing",
               tracing);
  ...
  Config::SetDefault("ns3::TcpSocketBase::UseEcn",
                     (useEcn ? EnumValue(TcpSocketState::On)
                             : EnumValue(TcpSocketState::Off)));
  Config::SetDefault("ns3::TcpSocketState::MaxPacingRate",
                     DataRateValue(maxPacingRate));
  ...
  bottleNeckLink.SetDeviceAttribute("DataRate",
                                    DataRateValue(bottleneckBandwidth));
  ...
  ApplicationContainer sinkApps4 =
      packetSinkHelper.Install(c.Get(4)); // n4 as sink
  ApplicationContainer sinkApps5 =
      packetSinkHelper.Install(c.Get(5)); // n5 as sink
  ...
  for (auto i = stats.begin(); i != stats.end(); ++i) {
    ...
    std::cout << "  TxOffered:  "
              << i->second.txBytes * 8.0 / simulationEndTime.GetSeconds() /
                     1000 / 1000
              << " Mbps\n";
    std::cout << "  Rx Packets: " << i->second.rxPackets << "\n";
    std::cout << "  Rx Bytes:   " << i->second.rxBytes << "\n";
    std::cout << "  Throughput: "
              << i->second.rxBytes * 8.0 / simulationEndTime.GetSeconds() /
                     1000 / 1000
              << " Mbps\n";
  }
