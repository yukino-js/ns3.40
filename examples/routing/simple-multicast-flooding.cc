...
int main(int argc, char *argv[]) {
  ...
  for (auto diter = devices.Begin(); diter != devices.End(); ++diter) {
    ...
    if (Names::FindName(node) == "A") {
      ...
      NS_ASSERT_MSG((bool)ipv4, "Node " << Names::FindName(node)
      ...
      staticRouting.AddMulticastRoute(node, Ipv4Address::GetAny(),
  ...
  OnOffHelper onoffHelper("ns3::UdpSocketFactory",
                          InetSocketAddress(targetAddr.c_str(), 9));
  ...
  for (auto end = sinks.End(), iter = sinks.Begin(); iter != end; ++iter) {
    ...
    std::cout << "Node " << Names::FindName(node) << " received "
              << sink->GetTotalRx() << " bytes" << std::endl;
  }
