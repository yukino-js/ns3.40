...
set yrange [0:1]\n";
  ...
  const std::vector<std::string> colors{
      "green", "blue", "red",     "black", "orange",    "purple", "yellow",
      "pink",  "grey", "magenta", "brown", "turquoise", "olive",  "beige",
  };

  NS_ASSERT_MSG(colors.size() == modes.size(),
                "Colors and modes vectors have different sizes");

  for (std::size_t i = 0; i < modes.size(); i++) {
    plotExtra << "set style line " << (i + 1) << " linewidth 5 linecolor rgb \""
              << colors[i] << "\" \n";
  }
