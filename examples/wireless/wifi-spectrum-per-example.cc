...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("index", "restrict index to single value between 0 and 31",
               index);
  ...
  std::cout << "wifiType: " << wifiType << " distance: " << distance
            << "m; time: " << simulationTime << "; TxPower: 1 dBm (1.3 mW)"
            << std::endl;
  std::cout << std::setw(5) << "index" << std::setw(6) << "MCS" << std::setw(13)
            << "Rate (Mb/s)" << std::setw(12) << "Tput (Mb/s)" << std::setw(10)
            << "Received " << std::setw(12) << "Signal (dBm)" << std::setw(12)
            << "Noise (dBm)" << std::setw(9) << "SNR (dB)" << std::endl;
  for (uint16_t i = startIndex; i <= stopIndex; i++) {
    ...
    if (wifiType == "ns3::YansWifiPhy") {
      ...
      phy.Set("ChannelSettings",
              StringValue(std::string("{0, ") + (i <= 15 ? "20" : "40") +
                          ", BAND_5GHZ, 0}"));
    ...
    if (totalPacketsThrough > 0) {
      std::cout << std::setw(12) << g_signalDbmAvg << std::setw(12)
                << g_noiseDbmAvg << std::setw(12)
