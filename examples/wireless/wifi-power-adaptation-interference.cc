...
void NodeStatistics::SetupPhy(Ptr<WifiPhy> phy) {
  for (const auto &mode : phy->GetModeList()) {
    ...
    Time time =
        phy->CalculateTxDuration(packetSize, txVector, phy->GetPhyBand());
    NS_LOG_DEBUG(mode.GetUniqueName()
                 << " " << time.GetSeconds() << " " << dataRate);
...
void NodeStatistics::PhyCallback(std::string path, Ptr<const Packet> packet,
                                 double powerW) {
  WifiMacHeader head;
...
void NodeStatistics::PowerCallback(std::string path, double oldPower,
                                   double newPower, Mac48Address dest) {
  m_currentPower[dest] = newPower;
}

void NodeStatistics::RateCallback(std::string path, DataRate oldRate,
                                  DataRate newRate, Mac48Address dest) {
  m_currentRate[dest] = newRate;
}

void NodeStatistics::StateCallback(std::string path, Time init, Time duration,
                                   WifiPhyState state) {
...
void NodeStatistics::CheckStatistics(double time) {
  ...
  Simulator::Schedule(Seconds(time), &NodeStatistics::CheckStatistics, this,
                      time);
}
...
Gnuplot2dDataset NodeStatistics::GetIdleDatafile() { return m_output_idle; }

Gnuplot2dDataset NodeStatistics::GetBusyDatafile() { return m_output_busy; }

Gnuplot2dDataset NodeStatistics::GetRxDatafile() { return m_output_rx; }

Gnuplot2dDataset NodeStatistics::GetTxDatafile() { return m_output_tx; }
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("maxPower", "Maximum available transmission level (dbm).",
               maxPower);
  cmd.AddValue("minPower", "Minimum available transmission level (dbm).",
               minPower);
  ...
  PacketSinkHelper sink1("ns3::UdpSocketFactory",
                         InetSocketAddress(sinkAddress1, port));
    ...
    if ((t.sourceAddress == "10.1.1.3" && t.destinationAddress == "10.1.1.1")) {
      ...
      NS_LOG_UNCOND("  Throughput to 10.1.1.1: "
                    << i->second.rxBytes * 8.0 /
                           ...
                           1024 / 1024
                    << " Mbps\n");
      NS_LOG_INFO("  Mean delay:   "
                  << i->second.delaySum.GetSeconds() / i->second.rxPackets
                  << "\n");
      NS_LOG_INFO("  Mean jitter:   " << i->second.jitterSum.GetSeconds() /
  ...
  if (manager == "ns3::ParfWifiManager" || manager == "ns3::AparfWifiManager" ||
    ...
    gnuplot =
        Gnuplot("power-" + outputFileName + "-0.eps", "Average Transmit Power");
    gnuplot.SetTerminal("post eps color enhanced");
  ...
  if (manager == "ns3::ParfWifiManager" || manager == "ns3::AparfWifiManager" ||
    ...
    gnuplot =
        Gnuplot("power-" + outputFileName + "-1.eps", "Average Transmit Power");
    gnuplot.SetTerminal("post eps color enhanced");
    gnuplot.SetLegend("Time (seconds)", "Power (mW)");
