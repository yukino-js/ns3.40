...
void Experiment::GenerateTraffic(Ptr<Socket> socket, uint32_t pktSize,
  ...
  if (pktCount > 0) {
    ...
    Simulator::Schedule(pktInterval, &Experiment::GenerateTraffic, this, socket,
...
uint32_t Experiment::Run(const WifiHelper &wifi,
  ...
  Simulator::Schedule(Seconds(1.0), &Experiment::GenerateTraffic, this, source,
                      packetSize, maxPacketCount, interPacketInterval);
  Simulator::Run();
