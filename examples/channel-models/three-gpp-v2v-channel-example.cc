...
static Ptr<ChannelConditionModel>
    m_condModel; //!< the ChannelConditionModel object
...
static void ComputeSnr(const ComputeSnrParams &params) {
  ...
  *(params.txParams->psd) *= propagationGainLinear;
  ...
  f << Simulator::Now().GetSeconds() << " " // time [s]
    ...
    << params.rxMob->GetPosition().y << " " << cond->GetLosCondition()
    << " "                                             // channel state
...
int main(int argc, char *argv[]) {
  ...
  std::string scenario =
      "V2V-Urban";   // 3GPP propagation scenario, V2V-Urban or V2V-Highway
